// ----------------------------------------------------------------------------
// free_funcs.hpp :  implementation of the free functions of streams_boost::format
// ----------------------------------------------------------------------------
...
#ifndef STREAMS_BOOST_FORMAT_FUNCS_HPP
#define STREAMS_BOOST_FORMAT_FUNCS_HPP
...
namespace streams_boost {
    ...
    namespace io {
         using ::streams_boost::str; // keep compatibility with when it was defined in this N.S.
    }   // - namespace io
    ...
    {
        typedef streams_boost::basic_format<Ch, Tr, Alloc>   format_t;
        if(f.items_.size()==0) 
            os << f.prefix_;
        else {
            if(f.cur_arg_ < f.num_args_)
                if( f.exceptions() & io::too_few_args_bit )
                    ...
                    streams_boost::throw_exception(io::too_few_args(f.cur_arg_, f.num_args_)); 
...
#endif // STREAMS_BOOST_FORMAT_FUNCS_HPP
