...
#ifndef STREAMS_BOOST_PTR_CONTAINER_DETAIL_ASSOCIATIVE_PTR_CONTAINER_HPP
#define STREAMS_BOOST_PTR_CONTAINER_DETAIL_ASSOCIATIVE_PTR_CONTAINER_HPP
...
namespace ptr_container_detail
{
        ...
        public reversible_ptr_container<Config,CloneAllocator>
    {
        ...
        typedef STREAMS_BOOST_DEDUCED_TYPENAME base_type::scoped_deleter
                                scoped_deleter;

        typedef STREAMS_BOOST_DEDUCED_TYPENAME Config::container_type
                                container_type;
    public: // typedefs
        typedef STREAMS_BOOST_DEDUCED_TYPENAME Config::key_type
                                key_type;
        typedef STREAMS_BOOST_DEDUCED_TYPENAME Config::key_compare
                                key_compare;
        typedef STREAMS_BOOST_DEDUCED_TYPENAME Config::value_compare
                                value_compare;
        typedef STREAMS_BOOST_DEDUCED_TYPENAME Config::hasher
                                hasher;
        typedef STREAMS_BOOST_DEDUCED_TYPENAME Config::key_equal
                                key_equal;
        typedef STREAMS_BOOST_DEDUCED_TYPENAME Config::iterator
                                iterator;
        typedef STREAMS_BOOST_DEDUCED_TYPENAME Config::const_iterator
                                const_iterator;
        typedef STREAMS_BOOST_DEDUCED_TYPENAME Config::local_iterator
                                local_iterator;
        typedef STREAMS_BOOST_DEDUCED_TYPENAME Config::const_local_iterator
    ...
    public: // associative container interface
        ...
        iterator erase( iterator before ) // nothrow
        {
            ...
            STREAMS_BOOST_ASSERT( before != this->end() );
        ...
        STREAMS_BOOST_DEDUCED_TYPENAME streams_boost::disable_if< streams_boost::is_convertible<Range&,key_type&>, 
                                                  iterator >::type
        erase( const Range& r )
        {
    ...
    protected:
        ...
        void multi_transfer( STREAMS_BOOST_DEDUCED_TYPENAME AssociatePtrCont::iterator object,
                             AssociatePtrCont& from ) // strong
        {
            STREAMS_BOOST_ASSERT( (void*)&from != (void*)this );
            STREAMS_BOOST_ASSERT( !from.empty() && "Cannot transfer from empty container" );
        ...
        template< class AssociatePtrCont >
        size_type multi_transfer( STREAMS_BOOST_DEDUCED_TYPENAME AssociatePtrCont::iterator first,
                                  STREAMS_BOOST_DEDUCED_TYPENAME AssociatePtrCont::iterator last,
                                  AssociatePtrCont& from ) // basic
        {
            ...
            for( ; first != last; )
            {
                STREAMS_BOOST_ASSERT( first != from.end() );
                this->base().insert( *first.base() );     // strong
                STREAMS_BOOST_DEDUCED_TYPENAME AssociatePtrCont::iterator 
                    to_delete( first );
                ++first;
        ...
        template< class AssociatePtrCont >
        bool single_transfer( STREAMS_BOOST_DEDUCED_TYPENAME AssociatePtrCont::iterator object,
                              AssociatePtrCont& from ) // strong
        {
            STREAMS_BOOST_ASSERT( (void*)&from != (void*)this );
            STREAMS_BOOST_ASSERT( !from.empty() && "Cannot transfer from empty container" );

            std::pair<STREAMS_BOOST_DEDUCED_TYPENAME base_type::ptr_iterator,bool> p =
                this->base().insert( *object.base() );     // strong
            if( p.second )
                from.base().erase( object.base() );        // nothrow
        ...
        template< class AssociatePtrCont >
        size_type single_transfer( STREAMS_BOOST_DEDUCED_TYPENAME AssociatePtrCont::iterator first,
                                   STREAMS_BOOST_DEDUCED_TYPENAME AssociatePtrCont::iterator last,
                                   AssociatePtrCont& from ) // basic
        {
            STREAMS_BOOST_ASSERT( (void*)&from != (void*)this );
        ...
        {
            STREAMS_BOOST_ASSERT( !this->empty() );
            STREAMS_BOOST_ASSERT( *this->begin().base() != 0 );
            return *this->begin(); 
        }
        ...
        {
            STREAMS_BOOST_ASSERT( !this->empty() );
            STREAMS_BOOST_ASSERT( *(--this->end()).base() != 0 );
            return *--this->end(); 
        }
    ...
    public:
#if STREAMS_BOOST_WORKAROUND(__DECCXX_VER, STREAMS_BOOST_TESTED_AT(70190006))
        iterator begin()
        {
