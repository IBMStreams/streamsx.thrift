...
#ifndef STREAMS_BOOST_PTR_CONTAINER_SERIALIZE_PTR_ARRAY_HPP
#define STREAMS_BOOST_PTR_CONTAINER_SERIALIZE_PTR_ARRAY_HPP

#include <streams_boost/ptr_container/detail/serialize_reversible_cont.hpp>
#include <streams_boost/ptr_container/ptr_array.hpp>
...
void load(Archive& ar, ptr_array<T, N, CloneAllocator>& c, unsigned int /*version*/)
{
    ...
    {
        T* p;
        ar >> streams_boost::serialization::make_nvp( ptr_container_detail::item(), p );
        c.replace(i, p);
    }
}
