...
#ifndef STREAMS_BOOST_PTR_CONTAINER_PTR_SEQUENCE_ADAPTER_HPP
#define STREAMS_BOOST_PTR_CONTAINER_PTR_SEQUENCE_ADAPTER_HPP
...
#include <streams_boost/ptr_container/indirect_fun.hpp>
#include <streams_boost/ptr_container/detail/void_ptr_iterator.hpp>
...
{    
    ...
    {
        typedef STREAMS_BOOST_DEDUCED_TYPENAME remove_nullable<T>::type
                    U;
        typedef VoidPtrSeq
                    void_container_type;

        typedef STREAMS_BOOST_DEDUCED_TYPENAME VoidPtrSeq::allocator_type
                    allocator_type;
        
        typedef U   value_type;

        typedef void_ptr_iterator<
                        STREAMS_BOOST_DEDUCED_TYPENAME VoidPtrSeq::iterator, U > 
                    iterator;
       
        typedef void_ptr_iterator<
                        STREAMS_BOOST_DEDUCED_TYPENAME VoidPtrSeq::const_iterator, const U >
                    const_iterator;

#if defined(STREAMS_BOOST_NO_SFINAE) || defined(STREAMS_BOOST_NO_FUNCTION_TEMPLATE_ORDERING)
...
#if defined(STREAMS_BOOST_NO_SFINAE) && !STREAMS_BOOST_WORKAROUND(__MWERKS__, <= 0x3003)
        ...
        STREAMS_BOOST_STATIC_CONSTANT(bool, allow_null = streams_boost::is_nullable<T>::value );
    };
    ...
    protected:
        typedef STREAMS_BOOST_DEDUCED_TYPENAME base_type::scoped_deleter scoped_deleter;
         
    public:
        typedef STREAMS_BOOST_DEDUCED_TYPENAME base_type::value_type  value_type; 
        typedef STREAMS_BOOST_DEDUCED_TYPENAME base_type::reference   reference; 
        typedef STREAMS_BOOST_DEDUCED_TYPENAME base_type::const_reference 
                                                              const_reference;
        typedef STREAMS_BOOST_DEDUCED_TYPENAME base_type::auto_type   auto_type;
        typedef STREAMS_BOOST_DEDUCED_TYPENAME base_type::clone_allocator_type
                                                              clone_allocator_type;
        typedef STREAMS_BOOST_DEDUCED_TYPENAME base_type::iterator    iterator;          
        typedef STREAMS_BOOST_DEDUCED_TYPENAME base_type::size_type   size_type;  
        typedef STREAMS_BOOST_DEDUCED_TYPENAME base_type::allocator_type  
                                                              allocator_type;
        ...
        auto_type pop_back()
        {
            STREAMS_BOOST_ASSERT( !this->empty() && 
        ...
        { 
            ...
            STREAMS_BOOST_ASSERT( !::streams_boost::is_null( this->begin() ) );
            return *this->begin(); 
        }
        ...
        reference back()
        {
            ...
            STREAMS_BOOST_ASSERT( !::streams_boost::is_null( --this->end() ) );
            return *--this->end(); 
        }
    ...
    public: // deque/vector inerface
        ...
        {
            STREAMS_BOOST_ASSERT( n < this->size() );
            STREAMS_BOOST_ASSERT( !this->is_null( n ) );
        ...
        {
            STREAMS_BOOST_PTR_CONTAINER_THROW_EXCEPTION( n >= this->size(), bad_index, 
                                                 "'at()' out of bounds" );
            STREAMS_BOOST_ASSERT( !this->is_null( n ) );
            return (*this)[n];
        }
    ...
    public: // assign, insert, transfer
        ...
        {
            assign( streams_boost::begin(r), streams_boost::end(r ) );
        }
    ...
    public:
        ...
        {
            insert_impl( before, first, last, STREAMS_BOOST_DEDUCED_TYPENAME
        ...
        streams_boost::disable_if< ptr_container_detail::is_pointer_or_integral<Range> >::type
        insert( iterator before, const Range& r )
        {
            insert( before, streams_boost::begin(r), streams_boost::end(r) );
        }
        ...
        void transfer( iterator before, 
                       STREAMS_BOOST_DEDUCED_TYPENAME PtrSeqAdapter::iterator first, 
                       STREAMS_BOOST_DEDUCED_TYPENAME PtrSeqAdapter::iterator last, 
                       PtrSeqAdapter& from ) // strong
        {
            STREAMS_BOOST_ASSERT( (void*)&from != (void*)this );
            if( from.empty() )
                return;
            this->base().
        ...
        template< class PtrSeqAdapter >
        void transfer( iterator before, 
                       STREAMS_BOOST_DEDUCED_TYPENAME PtrSeqAdapter::iterator object, 
                       PtrSeqAdapter& from ) // strong
        {
            STREAMS_BOOST_ASSERT( (void*)&from != (void*)this );
            if( from.empty() )
        ...
        STREAMS_BOOST_DEDUCED_TYPENAME streams_boost::disable_if< streams_boost::is_same< Range,
                      STREAMS_BOOST_DEDUCED_TYPENAME PtrSeqAdapter::iterator > >::type
        transfer( iterator before, const Range& r, PtrSeqAdapter& from ) // strong
        {
            transfer( before, streams_boost::begin(r), streams_boost::end(r), from );
        }
    ...
    public: // C-array support
        ...
        {
            STREAMS_BOOST_ASSERT( from != 0 );
            if( delete_from )
            {
                STREAMS_BOOST_DEDUCED_TYPENAME base_type::scoped_deleter 
    ...
    public: // null functions
        ...
        {
            STREAMS_BOOST_ASSERT( idx < this->size() );
    ...
    public: // resize
        ...
        void resize( size_type size ) // basic
        {
            ...
            if( old_size > size )
            {
                this->erase( streams_boost::next( this->begin(), size ), this->end() );  
            }
            else if( size > old_size )
            {
                for( ; old_size != size; ++old_size )
                    this->push_back( new STREAMS_BOOST_DEDUCED_TYPENAME 
                                     streams_boost::remove_pointer<value_type>::type() ); 
            }

            STREAMS_BOOST_ASSERT( this->size() == size );
        }
        ...
        void rresize( size_type size ) // basic
        {
            ...
            if( old_size > size )
            {
                this->erase( this->begin(), 
                             streams_boost::next( this->begin(), old_size - size ) );  
            }
            else if( size > old_size )
            {
                for( ; old_size != size; ++old_size )
                    this->push_front( new STREAMS_BOOST_DEDUCED_TYPENAME 
                                      streams_boost::remove_pointer<value_type>::type() ); 
            }
    ...
    public: // algorithms
        ...
        {
            STREAMS_BOOST_ASSERT( first <= last && "out of range sort()" );
            STREAMS_BOOST_ASSERT( this->begin() <= first && "out of range sort()" );
            STREAMS_BOOST_ASSERT( last <= this->end() && "out of range sort()" ); 
    ...
    private:
        ...
        {
            STREAMS_BOOST_ASSERT( first <= last && "out of range unique()/erase_if()" );
            STREAMS_BOOST_ASSERT( this->begin() <= first && "out of range unique()/erase_if()" );
            STREAMS_BOOST_ASSERT( last <= this->end() && "out of range unique()/erase_if)(" );             
        }
        
        void range_check( iterator first, iterator last )
        {
            range_check_impl( first, last, 
                              STREAMS_BOOST_DEDUCED_TYPENAME iterator_category<iterator>::type() );
        }
    ...
    public:
        ...
        void unique( iterator first, iterator last, Compare comp )
        {
            ...
            for( ; next != last; ++next )
            {
                STREAMS_BOOST_ASSERT( !::streams_boost::is_null(prev) );
                STREAMS_BOOST_ASSERT( !::streams_boost::is_null(next) );
                if( comp( *prev, *next ) )
                {
