...
//      great as streams_boost::intmax_t can cause some differences to be
...
// Revision History
// 11 Feb 2001 - Use STREAMS_BOOST_STATIC_CONSTANT (David Abrahams)
...
// 06 Feb 2001 - Factored if_true out into streams_boost/detail/select_type.hpp
//               (David Abrahams)
...
#ifndef STREAMS_BOOST_NUMERIC_TRAITS_HPP_DWA20001901
# define STREAMS_BOOST_NUMERIC_TRAITS_HPP_DWA20001901
...
namespace streams_boost { namespace detail {
  ...
  struct is_signed
  {
    ...
    STREAMS_BOOST_STATIC_CONSTANT(bool, value = (Number(-1) < Number(0)));
#else
    STREAMS_BOOST_STATIC_CONSTANT(bool, value = std::numeric_limits<Number>::is_signed);
#endif
  };
...
# ifndef STREAMS_BOOST_NO_LIMITS_COMPILE_TIME_CONSTANTS
  ...
  template <> struct digit_traits_select<true>
  {
      ...
      {
          STREAMS_BOOST_STATIC_CONSTANT(int, digits = std::numeric_limits<T>::digits);
      };
  };
  ...
  template <> struct digit_traits_select<false>
  {
      template <class T> struct traits
      {
          STREAMS_BOOST_STATIC_CONSTANT(int, digits = (
  ...
  template <class T> struct digit_traits
  {
      ...
      STREAMS_BOOST_STATIC_CONSTANT(int, digits = traits::digits);
  };
#endif

  // Template class integer_traits<Integer> -- traits of various integer types
  // This should probably be rolled into streams_boost::integer_traits one day, but I
...
#   if defined(STREAMS_BOOST_MSVC) && STREAMS_BOOST_MSVC <= 1300
      ...
      // local definitions
      STREAMS_BOOST_STATIC_CONSTANT(bool, is_integer = x::is_integer);
      STREAMS_BOOST_STATIC_CONSTANT(bool, is_specialized = x::is_specialized);
      
      STREAMS_BOOST_STATIC_ASSERT(is_integer);
      STREAMS_BOOST_STATIC_ASSERT(is_specialized);
#   endif
   public:
      typedef typename
      if_true<(int(x::is_signed)
              && (!int(x::is_bounded)
                 // digits is the number of no-sign bits
                  || (int(x::digits) + 1 >= digit_traits<streams_boost::intmax_t>::digits)))>::template then<
        Integer,
...
#else
      STREAMS_BOOST_STATIC_ASSERT(streams_boost::is_integral<Integer>::value);
...
#endif // STREAMS_BOOST_NUMERIC_TRAITS_HPP_DWA20001901
