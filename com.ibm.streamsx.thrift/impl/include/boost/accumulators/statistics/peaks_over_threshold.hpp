...
#ifndef STREAMS_BOOST_ACCUMULATORS_STATISTICS_PEAKS_OVER_THRESHOLD_HPP_DE_01_01_2006
#define STREAMS_BOOST_ACCUMULATORS_STATISTICS_PEAKS_OVER_THRESHOLD_HPP_DE_01_01_2006

#include <vector>
#include <limits>
#include <numeric>
#include <functional>
#include <streams_boost/config/no_tr1/cmath.hpp> // pow
#include <sstream> // stringstream
#include <stdexcept> // runtime_error
#include <streams_boost/throw_exception.hpp>
#include <streams_boost/range.hpp>
...
#include <streams_boost/accumulators/statistics/parameters/quantile_probability.hpp>
#include <streams_boost/accumulators/statistics/count.hpp>
#include <streams_boost/accumulators/statistics/tail.hpp>
...
//
STREAMS_BOOST_PARAMETER_NESTED_KEYWORD(tag, pot_threshold_value, threshold_value)
STREAMS_BOOST_PARAMETER_NESTED_KEYWORD(tag, pot_threshold_probability, threshold_probability)
...
namespace impl
{
    ...
    */
      ...
      : accumulator_base
    {
        ...
        typedef streams_boost::tuple<float_type, float_type, float_type> result_type;
        ...
        peaks_over_threshold_impl(Args const &args)
          ...
          , fit_parameters_(streams_boost::make_tuple(0., 0., 0.))
          , is_dirty_(true)
        {
        }
        ...
        result_type result(Args const &args) const
        {
            if (this->is_dirty_)
            {
                ...
                this->fit_parameters_ = streams_boost::make_tuple(u_bar, beta_bar, xi_hat);
            }
    ...
    private:
        ...
        mutable result_type fit_parameters_; // streams_boost::tuple that stores fit parameters
        ...
        result_type result(Args const &args) const
        {
            if (this->is_dirty_)
            {
                ...
                if ( n >= static_cast<std::size_t>(tail(args).size()))
                {
                    ...
                    {
                        return streams_boost::make_tuple(
                    ...
                    else
                    {
                        ...
                        streams_boost::throw_exception(std::runtime_error(msg.str()));
                        return streams_boost::make_tuple(Sample(0), Sample(0), Sample(0));
                    }
                }
                else
                {
...
namespace extract
{
    ...
    STREAMS_BOOST_ACCUMULATORS_IGNORE_GLOBAL(peaks_over_threshold)
}
