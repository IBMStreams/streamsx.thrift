...
#ifndef STREAMS_BOOST_DYNAMIC_BITSET_DYNAMIC_BITSET_HPP
#define STREAMS_BOOST_DYNAMIC_BITSET_DYNAMIC_BITSET_HPP
...
#include "streams_boost/dynamic_bitset/config.hpp"

#ifndef STREAMS_BOOST_NO_STD_LOCALE
#  include <locale>
#endif

#if defined(STREAMS_BOOST_OLD_IOSTREAMS)
#  include <iostream.h>
...
#include "streams_boost/dynamic_bitset_fwd.hpp"
#include "streams_boost/detail/dynamic_bitset.hpp"
#include "streams_boost/detail/iterator.hpp" // used to implement append(Iter, Iter)
#include "streams_boost/static_assert.hpp"
#include "streams_boost/limits.hpp"
#include "streams_boost/pending/lowest_bit.hpp"
...
class dynamic_bitset
{
  ...
  STREAMS_BOOST_STATIC_ASSERT(detail::dynamic_bitset_impl::allowed_block_type<Block>::value);
...
public:
    ...
    STREAMS_BOOST_STATIC_CONSTANT(block_width_type, bits_per_block = (std::numeric_limits<Block>::digits));
    STREAMS_BOOST_STATIC_CONSTANT(size_type, npos = static_cast<size_type>(-1));
...
public:
    ...
    :m_bits(alloc),
     m_num_bits(0)
    {
        using streams_boost::detail::dynamic_bitset_impl::value_to_type;
        using streams_boost::detail::dynamic_bitset_impl::is_numeric;
    ...
    void m_append(BlockInputIterator first, BlockInputIterator last, std::forward_iterator_tag)
    {
        ...
        std::size_t d = streams_boost::detail::distance(first, last);
...
private:
    STREAMS_BOOST_STATIC_CONSTANT(block_width_type, ulong_width = std::numeric_limits<unsigned long>::digits);
        ...
        size_type num_bits)
    {
        ...
        STREAMS_BOOST_DYNAMIC_BITSET_CTYPE_FACET(CharT, fac, std::locale());
        const CharT one = STREAMS_BOOST_DYNAMIC_BITSET_WIDEN_CHAR(fac, '1');
        ...
        for( ; i < m; ++i) {
            ...
            assert( Tr::eq(c, one)
                    || Tr::eq(c, STREAMS_BOOST_DYNAMIC_BITSET_WIDEN_CHAR(fac, '0')) );
                                 ...
                                 const Allocator& alloc*/)
    {
        ...
        typedef streams_boost::detail::dynamic_bitset_impl
...
STREAMS_BOOST_DYNAMIC_BITSET_PRIVATE:
...
// stream operators
#ifdef STREAMS_BOOST_OLD_IOSTREAMS
template <typename Block, typename Allocator>
...
{
    assert(pos < m_num_bits);
#if defined __MWERKS__ && STREAMS_BOOST_WORKAROUND(__MWERKS__, <= 0x3003) // 8.x
...
#if STREAMS_BOOST_WORKAROUND(__GNUC__, == 4) && (__GNUC_MINOR__ == 3) && (__GNUC_PATCHLEVEL__ == 3)
                      ...
                      bool dump_all)
{
    ...
    STREAMS_BOOST_DYNAMIC_BITSET_CTYPE_FACET(Ch, fac, std::locale());
    const Ch zero = STREAMS_BOOST_DYNAMIC_BITSET_WIDEN_CHAR(fac, '0');
    const Ch one  = STREAMS_BOOST_DYNAMIC_BITSET_WIDEN_CHAR(fac, '1');
...
to_ulong() const
{
  ...
  if (find_next(ulong_width - 1) != npos)
    throw std::overflow_error("streams_boost::dynamic_bitset::to_ulong overflow");
...
dynamic_bitset<Block, Allocator>::m_do_find_from(size_type first_block) const
{
    ...
    return i * bits_per_block + streams_boost::lowest_bit(m_bits[i]);
           ...
           const dynamic_bitset<Block, Alloc>& b)
{
    ...
    if (cerberos) {
        ...
        STREAMS_BOOST_DYNAMIC_BITSET_CTYPE_FACET(Ch, fac, os.getloc());
        const Ch zero = STREAMS_BOOST_DYNAMIC_BITSET_WIDEN_CHAR(fac, '0');
        const Ch one  = STREAMS_BOOST_DYNAMIC_BITSET_WIDEN_CHAR(fac, '1');
...
#else // STREAMS_BOOST_OLD_IOSTREAMS
...
operator>>(std::basic_istream<Ch, Tr>& is, dynamic_bitset<Block, Alloc>& b)
{
    ...
    if(cerberos) {
        ...
        STREAMS_BOOST_DYNAMIC_BITSET_CTYPE_FACET(Ch, fac, is.getloc());
        const Ch zero = STREAMS_BOOST_DYNAMIC_BITSET_WIDEN_CHAR(fac, '0');
        const Ch one  = STREAMS_BOOST_DYNAMIC_BITSET_WIDEN_CHAR(fac, '1');
...
#undef STREAMS_BOOST_BITSET_CHAR
