...
#ifndef STREAMS_UUID_618474C2DE1511DEB74A388C56D89593
#define STREAMS_UUID_618474C2DE1511DEB74A388C56D89593
...
#include <streams_boost/exception/diagnostic_information.hpp>
#include <streams_boost/exception/detail/type_info.hpp>
...
streams_boost
    ...
    exception_ptr
        ...
        exception_ptr( shared_ptr<exception_detail::clone_base const> const & c ):
            ...
            bad_alloc_(false)
            {
            STREAMS_BOOST_ASSERT(c);
            }

        void
        rethrow() const
            {
            STREAMS_BOOST_ASSERT(*this);
            if( bad_alloc_ )
    ...
    unknown_exception:
        ...
        explicit
        unknown_exception( streams_boost::exception const & e ):
            streams_boost::exception(e)
            {
            add_original_type(e);
            }
    ...
    exception_detail
        ...
        current_exception_std_exception_wrapper:
            public T,
            public streams_boost::exception,
            public clone_base
            {
            public:
            ...
            current_exception_std_exception_wrapper( T const & e1, streams_boost::exception const & e2 ):
                T(e1),
                streams_boost::exception(e2)
                {
                add_original_type(e1);
                }
...
#ifdef STREAMS_BOOST_NO_RTTI
        template <class T>
        exception const *
        get_streams_boost_exception( T const * )
            {
            try
                {
                throw;
        ...
        template <class T>
        exception const *
        get_streams_boost_exception( T const * x )
            {
        ...
        current_exception_std_exception( T const & e1 )
            {
            if( streams_boost::exception const * e2 = get_streams_boost_exception(&e1) )
        ...
        inline
        shared_ptr<clone_base const>
        current_exception_unknown_streams_boost_exception( streams_boost::exception const & e )
            {
        ...
        current_exception_unknown_std_exception( std::exception const & e )
            {
            if( streams_boost::exception const * be = get_streams_boost_exception(&e) )
                return current_exception_unknown_streams_boost_exception(*be);
            else
                return shared_ptr<unknown_exception const>(new unknown_exception(e));
            }
        ...
        current_exception_impl()
            ...
            streams_boost::exception & e )
                {
                return exception_detail::current_exception_unknown_streams_boost_exception(e);
                }
            catch(
            ... )
                {
