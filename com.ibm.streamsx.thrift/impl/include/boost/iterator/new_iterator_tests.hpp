#ifndef STREAMS_BOOST_NEW_ITERATOR_TESTS_HPP
# define STREAMS_BOOST_NEW_ITERATOR_TESTS_HPP
...
# include <streams_boost/concept_archetype.hpp> // for detail::dummy_constructor
# include <streams_boost/detail/iterator.hpp>
# include <streams_boost/pending/iterator_tests.hpp>
# include <streams_boost/iterator/is_readable_iterator.hpp>
# include <streams_boost/iterator/is_lvalue_iterator.hpp>

# include <streams_boost/iterator/detail/config_def.hpp>
# include <streams_boost/detail/is_incrementable.hpp>
# include <streams_boost/detail/lightweight_test.hpp>
...
void readable_iterator_test(const Iterator i1, T v)
{
  ...
  T v1 = r1;
  T v2 = r2;
  STREAMS_BOOST_TEST(v1 == v);
  STREAMS_BOOST_TEST(v2 == v);
...
# if !STREAMS_BOOST_WORKAROUND(__MWERKS__, <= 0x2407)
  ...
  // the above code.
  STREAMS_BOOST_STATIC_ASSERT(is_readable_iterator<Iterator>::value);
# endif 
}
...
void swappable_iterator_test(Iterator i, Iterator j)
{
  ...
  STREAMS_BOOST_TEST(bi == aj && bj == ai);
}
...
void constant_lvalue_iterator_test(Iterator i, T v1)
{
  ...
  STREAMS_BOOST_STATIC_ASSERT((is_same<const value_type&, reference>::value));
  const T& v2 = *i2;
  STREAMS_BOOST_TEST(v1 == v2);
# ifndef STREAMS_BOOST_NO_LVALUE_RETURN_DETECTION
  STREAMS_BOOST_STATIC_ASSERT(is_lvalue_iterator<Iterator>::value);
  STREAMS_BOOST_STATIC_ASSERT(!is_non_const_lvalue_iterator<Iterator>::value);
# endif 
}
...
void non_const_lvalue_iterator_test(Iterator i, T v1, T v2)
{
  ...
  typedef typename detail::iterator_traits<Iterator>::value_type value_type;
  typedef typename detail::iterator_traits<Iterator>::reference reference;
  STREAMS_BOOST_STATIC_ASSERT((is_same<value_type&, reference>::value));
  T& v3 = *i2;
  STREAMS_BOOST_TEST(v1 == v3);
...
void forward_readable_iterator_test(Iterator i, Iterator j, T val1, T val2)
{
  ...
  Iterator i3(i);
  i2 = i;
  STREAMS_BOOST_TEST(i2 == i3);
  STREAMS_BOOST_TEST(i != j);
  STREAMS_BOOST_TEST(i2 != j);
  readable_iterator_test(i, val1);
  readable_iterator_test(i2, val1);
  readable_iterator_test(i3, val1);

  STREAMS_BOOST_TEST(i == i2++);
  STREAMS_BOOST_TEST(i != ++i3);
...
void random_access_readable_iterator_test(Iterator i, int N, TrueVals vals)
{
  ...
  for (c = 0; c < N-1; ++c)
  {
    STREAMS_BOOST_TEST(i == j + c);
    STREAMS_BOOST_TEST(*i == vals[c]);
    typename detail::iterator_traits<Iterator>::value_type x = j[c];
    STREAMS_BOOST_TEST(*i == x);
    STREAMS_BOOST_TEST(*i == *(j + c));
    STREAMS_BOOST_TEST(*i == *(c + j));
    ++i;
    STREAMS_BOOST_TEST(i > j);
    STREAMS_BOOST_TEST(i >= j);
    STREAMS_BOOST_TEST(j <= i);
    STREAMS_BOOST_TEST(j < i);
  }
  ...
  for (c = 0; c < N-1; ++c)
  {
    STREAMS_BOOST_TEST(i == k - c);
    STREAMS_BOOST_TEST(*i == vals[N - 1 - c]);
    typename detail::iterator_traits<Iterator>::value_type x = j[N - 1 - c];
    STREAMS_BOOST_TEST(*i == x);
    Iterator q = k - c; 
    STREAMS_BOOST_TEST(*i == *q);
    STREAMS_BOOST_TEST(i > j);
    STREAMS_BOOST_TEST(i >= j);
    STREAMS_BOOST_TEST(j <= i);
    STREAMS_BOOST_TEST(j < i);
...
#endif // STREAMS_BOOST_NEW_ITERATOR_TESTS_HPP
