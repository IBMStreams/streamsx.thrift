...
#include <streams_boost/wave/cpp_throw.hpp>
...
namespace util {
    ...
    get_severity(int level) 
    {
        ...
        STREAMS_BOOST_ASSERT(severity_remark <= level && 
...
public:
    ...
    {
        return "streams_boost::wave::preprocess_exception";
    }
    ...
    // the error enum above
        ...
        STREAMS_BOOST_ASSERT(no_error <= code && code <= last_error_number);
...
public:
    ...
    virtual char const *what() const throw()
    {
        return "streams_boost::wave::macro_handling_exception";
    }
