...
#include <streams_boost/wave/util/time_conversion_helper.hpp> // time_conversion_helper 
...
namespace util {
    ...
    {
        typedef STREAMS_BOOST_WAVE_STRINGTYPE string_type;
        
    public:
    // list of static predefined macros
        struct static_macros {
            char const *name;
            streams_boost::wave::token_id token_id;
            char const *value;
        };
    ...
    private:
        streams_boost::wave::util::time_conversion_helper compilation_time_;
    ...
    protected:
        ...
        long seconds = long(difftime(compilation_time_.get_time(), mktime(&first_day)));
            ...
            sprintf(buffer, "0x%02d%1d%1d%04ld", STREAMS_BOOST_WAVE_VERSION_MAJOR,
                 STREAMS_BOOST_WAVE_VERSION_MINOR, STREAMS_BOOST_WAVE_VERSION_SUBMINOR, 
                 seconds/(3600*24));
            version_ = buffer;
        }
        ...
        {
        char buffer[sizeof("\"00.00.00.0000 \"")+sizeof(STREAMS_BOOST_PLATFORM)+sizeof(STREAMS_BOOST_COMPILER)+4];
        ...
        long seconds = long(difftime(compilation_time_.get_time(), mktime(&first_day)));
            ...
            sprintf(buffer, "\"%d.%d.%d.%ld [%s/%s]\"", STREAMS_BOOST_WAVE_VERSION_MAJOR,
                 STREAMS_BOOST_WAVE_VERSION_MINOR, STREAMS_BOOST_WAVE_VERSION_SUBMINOR, 
                 seconds/(3600*24), STREAMS_BOOST_PLATFORM, STREAMS_BOOST_COMPILER);
            versionstr_ = buffer;
        }
    ...
    // __SPIRIT_PP__/__WAVE__
        string_type get_version() const
        {
            ...
            sprintf(buffer, "0x%02d%1d%1d", STREAMS_BOOST_WAVE_VERSION_MAJOR, 
                STREAMS_BOOST_WAVE_VERSION_MINOR, STREAMS_BOOST_WAVE_VERSION_SUBMINOR);
            return buffer;
        }
    ...
    // __WAVE_CONFIG__
        string_type get_config() const
        {
            ...
            using namespace std;     // for some systems sprintf is in namespace std
            sprintf(buffer, "0x%08x", STREAMS_BOOST_WAVE_CONFIG);
            return buffer;
        }
    ...
    // C++ mode 
        ...
        static static_macros data[] = {
            ...
            STREAMS_BOOST_ASSERT(i < sizeof(data)/sizeof(data[0]));
            return data[i];
        }
