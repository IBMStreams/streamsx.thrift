...
#include <streams_boost/spirit/include/classic_multi_pass.hpp>
#include <streams_boost/spirit/include/classic_parse_tree_utils.hpp>

#include <streams_boost/wave/wave_config.hpp>
#include <streams_boost/pool/pool_alloc.hpp>

#include <streams_boost/wave/util/insert_whitespace_detection.hpp>
#include <streams_boost/wave/util/macro_helpers.hpp>
#include <streams_boost/wave/util/cpp_macromap_utils.hpp>
#include <streams_boost/wave/util/interpret_pragma.hpp>
#include <streams_boost/wave/util/transform_iterator.hpp>
#include <streams_boost/wave/util/functor_input.hpp>
#include <streams_boost/wave/util/filesystem_compatibility.hpp>

#include <streams_boost/wave/grammars/cpp_grammar_gen.hpp>
#include <streams_boost/wave/grammars/cpp_expression_grammar_gen.hpp>
#if STREAMS_BOOST_WAVE_ENABLE_COMMANDLINE_MACROS != 0
#include <streams_boost/wave/grammars/cpp_predef_macros_gen.hpp>
#endif

#include <streams_boost/wave/whitespace_handling.hpp>
#include <streams_boost/wave/cpp_iteration_context.hpp>
#include <streams_boost/wave/cpp_exceptions.hpp>
#include <streams_boost/wave/language_support.hpp>
...
retrieve_macroname(ContextT& ctx, ParseNodeT const &node, 
    streams_boost::spirit::classic::parser_id id, TokenT &macroname, PositionT& act_pos,
    bool update_position)
{
ParseNodeT const *name_node = 0;

    using streams_boost::spirit::classic::find_node;
    if (!find_node(node, id, &name_node)) 
    {
        // ill formed define statement (unexpected, should not happen)
        STREAMS_BOOST_WAVE_THROW_CTX(ctx, preprocess_exception, bad_define_statement, 
...
retrieve_macrodefinition(
    ParseNodeT const &node, streams_boost::spirit::classic::parser_id id, 
...
std::pair<const_tree_iterator, const_tree_iterator> nodes;
    ...
    using streams_boost::spirit::classic::get_node_range;
...
bool add_macro_definition(ContextT &ctx, std::string macrostring,
    bool is_predefined, streams_boost::wave::language_support language)
{
    typedef typename ContextT::token_type token_type;
    typedef typename ContextT::lexer_type lexer_type;
    typedef typename token_type::position_type position_type;
    typedef streams_boost::wave::grammars::predefined_macros_grammar_gen<lexer_type> 
        predef_macros_type;
...
streams_boost::spirit::classic::tree_parse_info<lexer_type> hit = 
    ...
    if (!hit.match || (!hit.full && T_EOF != token_id(*hit.stop))) {
        STREAMS_BOOST_WAVE_THROW_CTX(ctx, preprocess_exception, bad_macro_definition, 
...
bool has_parameters = false;
    ...
    if (!streams_boost::wave::util::retrieve_macroname(ctx, *hit.trees.begin(), 
            STREAMS_BOOST_WAVE_PLAIN_DEFINE_ID, macroname, act_pos, true))
        return false;
    has_parameters = streams_boost::wave::util::retrieve_macrodefinition(*hit.trees.begin(), 
        STREAMS_BOOST_WAVE_MACRO_PARAMETERS_ID, macroparameters, act_pos, true);
    streams_boost::wave::util::retrieve_macrodefinition(*hit.trees.begin(), 
        STREAMS_BOOST_WAVE_MACRO_DEFINITION_ID, macrodefinition, act_pos, true);
...
#endif // STREAMS_BOOST_WAVE_ENABLE_COMMANDLINE_MACROS != 0
...
public:
// interface to the streams_boost::spirit::classic::iterator_policies::functor_input policy
...
// type of a token sequence
    ...
    typedef streams_boost::wave::grammars::cpp_grammar_gen<lexer_type, token_sequence_type> 
        cpp_grammar_type;
...
// parse tree related types
    ...
    typedef streams_boost::spirit::classic::tree_parse_info<lexer_type, node_factory_type> 
        tree_parse_info_type;
    typedef streams_boost::spirit::classic::tree_match<lexer_type, node_factory_type> 
    ...
    :   ctx(ctx_), 
        iter_ctx(new base_iteration_context_type(ctx,
                lexer_type(first_, last_, pos_, 
                    streams_boost::wave::enable_prefer_pp_numbers(ctx.get_language())), 
                lexer_type(), 
                pos_.get_file().c_str()
            )), 
        seen_newline(true), must_emit_line_directive(false),
        act_pos(ctx_.get_main_pos()),
        whitespace(streams_boost::wave::need_insert_whitespace(ctx.get_language()))
    {
...
//         typename parse_tree_type::const_iterator const &end);
    ...
    void on_error(typename parse_tree_type::const_iterator const &begin,
        typename parse_tree_type::const_iterator const &end);
#if STREAMS_BOOST_WAVE_SUPPORT_WARNING_DIRECTIVE != 0
...
private:
    ...
    streams_boost::shared_ptr<base_iteration_context_type> iter_ctx;
    ...
    // re-tokenized
    streams_boost::wave::util::insert_whitespace_detection whitespace; 
};
...
pp_iterator_functor<ContextT>::returned_from_include()
{
...
#if STREAMS_BOOST_WAVE_USE_DEPRECIATED_PREPROCESSING_HOOKS != 0
    ...
    // of the included file
        STREAMS_BOOST_WAVE_STRINGTYPE oldfile = iter_ctx->real_filename;
    ...
    // report unbalanced #if/#endif now to make it possible to recover properly
        if (iter_ctx->if_block_depth != ctx.get_if_block_depth()) {
            using streams_boost::wave::util::impl::escape_lit;
            STREAMS_BOOST_WAVE_THROW_CTX(ctx, preprocess_exception, unbalanced_if_endif, 
...
pp_iterator_functor<ContextT>::operator()()
{
    ...
    catch (streams_boost::wave::cpplexer::lexing_exception const& e) {
        ...
        token_id id = token_id(act_token);
            ...
            // now make sure this line has a newline
                ...
                // warn, if this file does not end with a newline
                    STREAMS_BOOST_WAVE_THROW_CTX(ctx, preprocess_exception, 
...
//             whitespace.shift_tokens(T_NEWLINE);
        ...
        std::string file("\"");
        streams_boost::filesystem::path filename(
...
namespace impl {
    ...
    bool next_token_is_pp_directive(ContextT &ctx, IteratorT &it, IteratorT const &end)
    {
        ...
        STREAMS_BOOST_ASSERT(it == end || id != T_UNKNOWN);
...
tree_parse_info_type hit = cpp_grammar_type::parse_cpp_grammar(
    ...
    // recognized invalid directive
        ...
        string_type str(streams_boost::wave::util::impl::as_string<string_type>(
            iter_ctx->first, it));
        iter_ctx->first = it;
...
{
    STREAMS_BOOST_ASSERT(ctx.get_if_block_status());
...
typename string_type::size_type pos_end = s.find_last_of(is_system ? '>' : '\"');
    ...
    if (string_type::npos == pos_end) {
        STREAMS_BOOST_WAVE_THROW_CTX(ctx, preprocess_exception, bad_include_statement, 
            s.c_str(), act_pos);
        return;
    }
...
// call the 'found_include_directive' hook function
    ...
    if (!ctx.find_include_file (file_path, dir_path, is_system, current_name)) {
        STREAMS_BOOST_WAVE_THROW_CTX(ctx, preprocess_exception, bad_include_file, 
...
// test, if this file is known through a #pragma once directive
    ...
    streams_boost::shared_ptr<base_iteration_context_type> new_iter_ctx (
        new iteration_context_type(ctx, native_path_str.c_str(), 
            act_pos, streams_boost::wave::enable_prefer_pp_numbers(ctx.get_language())));
...
string_type s (impl::trim_whitespace(streams_boost::wave::util::impl::as_string(expanded)));
...
position_type pos(act_token.get_position());
    ...
    if (!streams_boost::wave::util::retrieve_macroname(ctx, node, 
            STREAMS_BOOST_WAVE_PLAIN_DEFINE_ID, macroname, pos, false))
        return;
    has_parameters = streams_boost::wave::util::retrieve_macrodefinition(node, 
        STREAMS_BOOST_WAVE_MACRO_PARAMETERS_ID, macroparameters, pos, false);
    streams_boost::wave::util::retrieve_macrodefinition(node, 
        STREAMS_BOOST_WAVE_MACRO_DEFINITION_ID, macrodefinition, pos, false);

    if (has_parameters) {
#if STREAMS_BOOST_WAVE_SUPPORT_VARIADICS_PLACEMARKERS != 0
        if (streams_boost::wave::need_variadics(ctx.get_language())) {
...
#endif // STREAMS_BOOST_WAVE_SUPPORT_VARIADICS_PLACEMARKERS != 0
        {
...
pp_iterator_functor<ContextT>::on_else()
{
    ...
    // #else without matching #if
        STREAMS_BOOST_WAVE_THROW_CTX(ctx, preprocess_exception, missing_matching_if, 
            "#else", act_pos);
    }
}
...
token_sequence_type expanded;
    ...
    // replace all remaining (== undefined) identifiers with an integer literal '0'
        ...
        {
            string_type outstr(streams_boost::wave::util::impl::as_string(toexpand));
            outstr += "(" + streams_boost::wave::util::impl::as_string(expanded) + ")";
            STREAMS_BOOST_WAVE_DUMP_CONDITIONAL_EXPRESSIONS_OUT << "#if " << outstr 
                << std::endl;
        }
#endif
        try {
        ...
        catch (streams_boost::wave::preprocess_exception const& e) {
    ...
    // division or other error by zero occurred
        ...
        if (grammars::error_division_by_zero & status) {
            STREAMS_BOOST_WAVE_THROW_CTX(ctx, preprocess_exception, division_by_zero, 
                expression.c_str(), act_pos);
        }
        else if (grammars::error_integer_overflow & status) {
            STREAMS_BOOST_WAVE_THROW_CTX(ctx, preprocess_exception, integer_overflow, 
                expression.c_str(), act_pos);
        }
...
token_sequence_type expanded;
    ...
    // replace all remaining (== undefined) identifiers with an integer literal '0'
...
#if STREAMS_BOOST_WAVE_DUMP_CONDITIONAL_EXPRESSIONS != 0
        {
            string_type outstr(streams_boost::wave::util::impl::as_string(toexpand));
            outstr += "(" + streams_boost::wave::util::impl::as_string(expanded) + ")";
            STREAMS_BOOST_WAVE_DUMP_CONDITIONAL_EXPRESSIONS_OUT << "#elif " << outstr << std::endl;
        }
#endif
    ...
    typename result_type::string_type s)
{
    ...
    // throw the exception
    STREAMS_BOOST_WAVE_THROW_CTX(ctx, preprocess_exception, ill_formed_directive, 
        s.c_str(), act_pos);
}
...
namespace impl {
    ...
    bool retrieve_line_info (IteratorT first, IteratorT const &last,
        ...
        streams_boost::wave::preprocess_exception::error_code& error)
    {
        using namespace streams_boost::wave;
...
token_sequence_type toexpand;
    ...
    // preprocess the body of this #line message
        ...
        {
            STREAMS_BOOST_WAVE_THROW_VAR_CTX(ctx, preprocess_exception, error, 
                streams_boost::wave::util::impl::as_string(expanded).c_str(), act_pos)
            return;
        }
...
// the queues should be empty at this point
    STREAMS_BOOST_ASSERT(unput_queue.empty());
    STREAMS_BOOST_ASSERT(pending_queue.empty());
...
// set new line number/filename only if ok
    if (!file_name.empty()) {    // reuse current file name 
        using streams_boost::wave::util::impl::unescape_lit;
...
#if STREAMS_BOOST_WAVE_PREPROCESS_ERROR_MESSAGE_BODY != 0
...
// message
    ...
    // report the corresponding error
        STREAMS_BOOST_WAVE_THROW_CTX(ctx, preprocess_exception, error_directive, 
            streams_boost::wave::util::impl::as_string(expanded).c_str(), act_pos);
    }
}
...
// message
    ...
    {
    // report the corresponding error
        STREAMS_BOOST_WAVE_THROW_CTX(ctx, preprocess_exception, warning_directive, 
            streams_boost::wave::util::impl::as_string(expanded).c_str(), act_pos);
    }
}
#endif // STREAMS_BOOST_WAVE_SUPPORT_WARNING_DIRECTIVE != 0
...
const_tree_iterator_t last = make_ref_transform_iterator(end, get_value);
    ...
    if (first != last) {
        if (T_IDENTIFIER == token_id(*first) && 
            streams_boost::wave::need_c99(ctx.get_language()) && 
...
#if STREAMS_BOOST_WAVE_EMIT_PRAGMA_DIRECTIVES != 0
// Move the resulting token sequence into the pending_queue, so it will be 
// returned to the caller.
    if (streams_boost::wave::need_emit_pragma_directives(ctx.get_language())) {
        pending_queue.splice(pending_queue.begin(), expanded);
    ...
    token_sequence_type const &pragma_body, token_sequence_type &result)
{
    ...
    return streams_boost::wave::util::interpret_pragma(
...
//  pp_iterator
//
//      The streams_boost::wave::pp_iterator template is the iterator, through which
...
template <typename ContextT>
class pp_iterator 
:   public streams_boost::spirit::classic::multi_pass<
        streams_boost::wave::impl::pp_iterator_functor<ContextT>,
        streams_boost::wave::util::functor_input
    >
{
public:
    typedef streams_boost::wave::impl::pp_iterator_functor<ContextT> input_policy_type;

private:
    typedef 
        streams_boost::spirit::classic::multi_pass<input_policy_type, streams_boost::wave::util::functor_input>
        base_type;
    typedef pp_iterator<ContextT> self_type;
    typedef streams_boost::wave::util::functor_input functor_input_type;
