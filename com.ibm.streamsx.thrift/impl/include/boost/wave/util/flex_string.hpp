...
//
// #HK030306: 
//      - Moved into the namespace streams_boost::wave::util 
//      - Added a bunch of missing typename(s) 
//      - Integrated with streams_boost config
...
#define STREAMS_BOOST_WAVE_FLEX_STRING_SERIALIZATION_HACK 1
#endif
...
public:
    ...
    pointer allocate(size_type n, const_pointer = 0) 
    {
        ...
        if (!p) streams_boost::throw_exception(std::bad_alloc());
...
private:
    ...
    {
        STREAMS_BOOST_ASSERT(size <= capacity);
        if (capacity == 0) 
        {
        ...
        else
        {
            ...
            if (!pData_) streams_boost::throw_exception(std::bad_alloc());
...
//             STREAMS_BOOST_ASSERT(!(le_type()(begin(), &*b) && le_type()(&*b, end())));
            reserve(neededCapacity);
        }
        std::copy(b, e, end());
        pData_->pEnd_ += sz;
...
public: // protected:
    ...
    void reserve(size_type res_arg)
    { 
        STREAMS_BOOST_ASSERT(res_arg < max_size());
...
private:
    ...
    Storage& GetStorage()
    {
        STREAMS_BOOST_ASSERT(buf_[maxSmallString] == magic);
...
public:
  SmallStringOpt(const SmallStringOpt& s)
    ...
    size_type size() const
    {
        STREAMS_BOOST_ASSERT(!Small() || maxSmallString >= buf_[maxSmallString]);
        return Small() 
    ...
    void reserve(size_type res_arg)
    {
        ...
        STREAMS_BOOST_ASSERT(capacity() >= res_arg);
    }
    ...
    void resize(size_type n, value_type c)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                // 11-17=2001: bug fix in the STREAMS_BOOST_ASSERTion below
                STREAMS_BOOST_ASSERT(capacity() > n);
...
template <
    typename Storage, 
    typename Align = STREAMS_BOOST_DEDUCED_TYPENAME Storage::value_type*
>
class CowString
{
...
private:
    ...
    RefCountType GetRefs() const
    {
        ...
        STREAMS_BOOST_ASSERT(d.size() > 0);
        STREAMS_BOOST_ASSERT(static_cast<RefCountType>(*d.begin()) != 0);
        return *d.begin();
    }
    ...
    {
        STREAMS_BOOST_ASSERT(GetRefs() >= 1);
        if (GetRefs() == 1) return;
...
public:
    CowString(const CowString& s)
    {
        ...
        STREAMS_BOOST_ASSERT(Data().size() > 0);
    }
...
//        CowString(rhs).swap(*this);
    ...
    size_type max_size() const
    { 
        STREAMS_BOOST_ASSERT(Data().max_size() > 0);
        return Data().max_size() - 1;
    }

    size_type capacity() const
    { 
        STREAMS_BOOST_ASSERT(Data().capacity() > 0);
...
{
#if defined(STREAMS_BOOST_WAVE_FLEXSTRING_THROW_ON_ENFORCE)
    template <typename Exception>
    static void Enforce(bool condition, Exception*, const char* msg)
    { if (!condition) streams_boost::throw_exception(Exception(msg)); }
#else
    template <typename Exception>
    static inline void Enforce(bool condition, Exception*, const char* msg)
    { STREAMS_BOOST_ASSERT(condition && msg); }
#endif // defined(STREAMS_BOOST_WAVE_FLEXSTRING_THROW_ON_ENFORCE)
...
#if STREAMS_BOOST_WAVE_FLEX_STRING_SERIALIZATION_HACK != 0
...
#if !defined(STREAMS_BOOST_WAVE_FLEX_STRING_SERIALIZATION_HACK)

// FIXME: This doesn't work because of the missing flex_string::operator>>()
template <typename E, class T, class A, class S>
struct implementation_level<streams_boost::wave::util::flex_string<E, T, A, S> >
{
    typedef mpl::integral_c_tag tag;
    typedef mpl::int_<streams_boost::serialization::primitive_type> type;
...
inline void save(Archive & ar, 
    streams_boost::wave::util::flex_string<E, T, A, S> const &t,
...
template<class Archive, typename E, class T, class A, class S>
inline void load(Archive & ar, streams_boost::wave::util::flex_string<E, T, A, S> &t,
    const unsigned int file_version)
{
    streams_boost::serialization::stl::load_collection<
        Archive, streams_boost::wave::util::flex_string<E, T, A, S>,
        streams_boost::serialization::stl::archive_input_seq<
            Archive, streams_boost::wave::util::flex_string<E, T, A, S> >,
        streams_boost::serialization::stl::reserve_imp<
            streams_boost::wave::util::flex_string<E, T, A, S> >
    >(ar, t);
}
...
template<class Archive, typename E, class T, class A, class S>
inline void serialize(Archive & ar, streams_boost::wave::util::flex_string<E, T, A, S> &t,
    const unsigned int file_version)
{
