...
#include <streams_boost/wave/util/time_conversion_helper.hpp>
#include <streams_boost/wave/util/unput_queue_iterator.hpp>
#include <streams_boost/wave/util/macro_helpers.hpp>
#include <streams_boost/wave/util/macro_definition.hpp>
#include <streams_boost/wave/util/symbol_table.hpp>
#include <streams_boost/wave/util/cpp_macromap_utils.hpp>
#include <streams_boost/wave/util/cpp_macromap_predef.hpp>
#include <streams_boost/wave/util/filesystem_compatibility.hpp>
#include <streams_boost/wave/grammars/cpp_defined_grammar_gen.hpp>
...
private:
    ...
    streams_boost::shared_ptr<defined_macros_type> defined_macros; // global symbol table
    ...
    bool is_predefined, defined_macros_type *scope)
{
    ...
    // exclude special macro names
        STREAMS_BOOST_WAVE_THROW_NAME_CTX(ctx, macro_handling_exception, 
...
token_id id = token_id(*begin);
    ...
    {
        STREAMS_BOOST_WAVE_THROW_CTX(ctx, preprocess_exception, invalid_macroname, 
    ...
    position_type const& pos, bool even_predefined)
{
    ...
    token_type tok(T_IDENTIFIER, name, pos);
        ...
        STREAMS_BOOST_WAVE_THROW_CTX(ctx, preprocess_exception, bad_undefine_statement, 
            name.c_str(), pos);
    }
...
//  T_IDENTIFIER token, try to replace this as a macro etc.
    ...
    // ignore placeholder tokens
        ...
        // try to replace this identifier as a macro
            ...
            else if (streams_boost::wave::need_variadics(ctx.get_language()) && 
...
// element
    if (0 == parameter_count && 0 == count_arguments) {
        STREAMS_BOOST_ASSERT(1 == arguments.size());
        arguments.clear();
    }
...
// should have returned all expanded tokens
    STREAMS_BOOST_ASSERT(pending_queue.empty()/* && unput_queue.empty()*/);
}
...
bool adjacent_stringize = false;
    ...
    token_id base_id = BASE_TOKEN(token_id(*cit));
        ...
        bool is_ellipsis = false;
            ...
            if (IS_EXTCATEGORY((*cit), ExtParameterTokenType)) {
                STREAMS_BOOST_ASSERT(streams_boost::wave::need_variadics(ctx.get_language()));
            ...
            STREAMS_BOOST_ASSERT(i < arguments.size());
            if (use_replaced_arg) {
    ...
    // error, '#' should not be the last token
        STREAMS_BOOST_WAVE_THROW_CTX(ctx, preprocess_exception, ill_formed_operator,
            "stringize ('#')", main_pos);
        return;
    }
    ...
    defined_macros_type *scope, ContainerT *queue_symbol) 
{
    ...
    STREAMS_BOOST_ASSERT(T_IDENTIFIER == token_id(curr_token) ||
...
{
    using namespace streams_boost::wave;
    using namespace streams_boost::wave::grammars;

ContainerT result;
IteratorT start = first;
streams_boost::spirit::classic::parse_info<IteratorT> hit = 
    ...
    if (!hit.hit) {
        ...
        STREAMS_BOOST_WAVE_THROW_CTX(ctx, preprocess_exception, ill_formed_expression, 
            msg.c_str(), main_pos);
...
// verify the parameter count
    ...
    // too few macro arguments
        STREAMS_BOOST_WAVE_THROW_CTX(ctx, preprocess_exception, too_few_macroarguments, 
            pragma_token.get_value().c_str(), pragma_token.get_position());
        return false;
    }
    if (count_args > 1 || arguments.size() > 1) {
    // too many macro arguments
        STREAMS_BOOST_WAVE_THROW_CTX(ctx, preprocess_exception, too_many_macroarguments, 
            pragma_token.get_value().c_str(), pragma_token.get_position());
        return false;
    }
...
// re-tokenize the newly generated string
    ...
    streams_boost::wave::language_support lang = 
        streams_boost::wave::enable_prefer_pp_numbers(ctx.get_language());
    lang = streams_boost::wave::enable_single_line(lang);
...
//    STREAMS_BOOST_ASSERT(T_EOF == token_id(rescanned.back()));
    ...
    for (iterator_type it = expanded.begin(); it != end; /**/) 
    {
        ...
        // analyze the validity of the concatenation result
            ...
            string_type error_string("\"");
                ...
                error_string += "\"";
                STREAMS_BOOST_WAVE_THROW_CTX(ctx, preprocess_exception, invalid_concat,
...
#if STREAMS_BOOST_WAVE_SUPPORT_VARIADICS_PLACEMARKERS != 0
    if (streams_boost::wave::need_c99(ctx.get_language())) {
...
template<typename ContextT>
struct version<streams_boost::wave::util::macromap<ContextT> >
{
    typedef streams_boost::wave::util::macromap<ContextT> target_type;
