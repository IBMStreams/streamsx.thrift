    ...
    IteratorT it, IteratorT const &end, ContainerT &pending)
{
    ...
    // check for pragma wave ...
        if ((*it).get_value() == STREAMS_BOOST_WAVE_PRAGMA_KEYWORD) 
        {
        ...
        //                STREAMS_BOOST_WAVE_PRAGMA_KEYWORD (defaults to "wave") and
        ...
        // remove the falsely matched surrounding parenthesis's
            if (values.size() >= 2) {
                STREAMS_BOOST_ASSERT(T_LEFTPAREN == values.front() && T_RIGHTPAREN == values.back());
...
#endif 
#if STREAMS_BOOST_WAVE_SUPPORT_PRAGMA_MESSAGE != 0
        ...
        // remove the falsely matched closing parenthesis/newline
            if (values.size() > 0) {
                STREAMS_BOOST_ASSERT(T_RIGHTPAREN == values.back() || T_NEWLINE == values.back());
                typename ContainerT::reverse_iterator rit = values.rbegin();
                values.erase((++rit).base());
            }
