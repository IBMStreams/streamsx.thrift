...
// #include <streams_boost/spirit/include/iterator/classic_multi_pass.hpp> // make_multi_pass 
...
//      streams_boost::wave::iteration_context which allows to control, how a given 
    ...
    struct load_file_to_string 
    {
        ...
        public:
                ...
                PositionT const &act_pos, language_support language)
            {
                ...
                if (!instream.is_open()) {
                    STREAMS_BOOST_WAVE_THROW_CTX(iter_ctx.ctx, preprocess_exception, 
...
public:
    base_iteration_context(ContextT& ctx_,
            STREAMS_BOOST_WAVE_STRINGTYPE const &fname, std::size_t if_block_depth = 0)   
...
// the actual input stream
    ...
    STREAMS_BOOST_WAVE_STRINGTYPE real_filename;  // real name of the current file
    STREAMS_BOOST_WAVE_STRINGTYPE filename;       // actual processed file
        ...
        inner<iteration_context<ContextT, IteratorT, InputPolicyT> >
{
    ...
    iteration_context(ContextT& ctx, STREAMS_BOOST_WAVE_STRINGTYPE const &fname, 
