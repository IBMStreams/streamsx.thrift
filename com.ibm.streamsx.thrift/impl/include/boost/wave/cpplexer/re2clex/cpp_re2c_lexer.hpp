...
#include <streams_boost/wave/util/file_position.hpp>
#include <streams_boost/wave/cpplexer/validate_universal_char.hpp>
#include <streams_boost/wave/cpplexer/cpplexer_exceptions.hpp>
#include <streams_boost/wave/cpplexer/token_cache.hpp>
#include <streams_boost/wave/cpplexer/convert_trigraphs.hpp>

#include <streams_boost/wave/cpplexer/cpp_lex_interface.hpp>
#include <streams_boost/wave/cpplexer/re2clex/scanner.hpp>
#include <streams_boost/wave/cpplexer/re2clex/cpp_re.hpp>
#if STREAMS_BOOST_WAVE_SUPPORT_PRAGMA_ONCE != 0
#include <streams_boost/wave/cpplexer/detect_include_guards.hpp>
#endif

#include <streams_boost/wave/cpplexer/cpp_lex_interface_generator.hpp>
...
template <typename IteratorT, 
    typename PositionT = streams_boost::wave::util::file_position_type,
...
public:
    ...
    lexer(IteratorT const &first, IteratorT const &last, 
        PositionT const &pos, streams_boost::wave::language_support language_);
    ~lexer();
...
private:
    ...
    string_type value;
    bool at_eof;
    streams_boost::wave::language_support language;
#if STREAMS_BOOST_WAVE_SUPPORT_PRAGMA_ONCE != 0
...
lexer<IteratorT, PositionT, TokenT>::lexer(IteratorT const &first, 
        ...
        streams_boost::wave::language_support language_) 
...
#endif
{
    ...
    scanner.act_in_c99_mode = streams_boost::wave::need_c99(language_);
#endif

#if STREAMS_BOOST_WAVE_SUPPORT_IMPORT_KEYWORD != 0
    scanner.enable_import_keyword = !streams_boost::wave::need_c99(language_);
#else
    scanner.enable_import_keyword = false;
#endif

    scanner.detect_pp_numbers = streams_boost::wave::need_prefer_pp_numbers(language_);
    scanner.single_line_only = streams_boost::wave::need_single_line(language_);
}
...
lexer<IteratorT, PositionT, TokenT>::get(TokenT& result)
{
    ...
    // test identifier characters for validity (throws if invalid chars found)
        ...
        if (!streams_boost::wave::need_no_character_validation(language))
    ...
    // test literal characters for validity (throws if invalid chars found)
        value = string_type((char const *)scanner.tok, 
            scanner.cur-scanner.tok);
        if (streams_boost::wave::need_convert_trigraphs(language))
    ...
    case T_LONGINTLIT:  // supported in C99 and long_long mode
        value = string_type((char const *)scanner.tok, 
            scanner.cur-scanner.tok);
        if (!streams_boost::wave::need_long_long(language)) {
        // syntax error: not allowed in C++ mode
            STREAMS_BOOST_WAVE_LEXER_THROW(lexing_exception, invalid_long_long_literal, 
...
//     std::cerr << streams_boost::wave::get_token_name(id) << ": " << value << std::endl;
...
lexer<IteratorT, PositionT, TokenT>::report_error(Scanner const *s, int errcode, 
    char const *msg, ...)
{
    STREAMS_BOOST_ASSERT(0 != s);
    STREAMS_BOOST_ASSERT(0 != msg);
    ...
    STREAMS_BOOST_WAVE_LEXER_THROW_VAR(lexing_exception, errcode, buffer, s->line, 
        s->column, s->file_name);
//    STREAMS_BOOST_UNREACHABLE_RETURN(0);
    return 0;
}
...
public:
    ...
    lex_functor(IteratorT const &first, IteratorT const &last, 
            PositionT const &pos, streams_boost::wave::language_support language)
...
#if STREAMS_BOOST_WAVE_SEPARATE_LEXER_INSTANTIATION != 0
#define STREAMS_BOOST_WAVE_RE2C_NEW_LEXER_INLINE
#else
#define STREAMS_BOOST_WAVE_RE2C_NEW_LEXER_INLINE inline
#endif 
...
template <typename IteratorT, typename PositionT, typename TokenT>
STREAMS_BOOST_WAVE_RE2C_NEW_LEXER_INLINE
...
#undef STREAMS_BOOST_WAVE_RE2C_NEW_LEXER_INLINE
