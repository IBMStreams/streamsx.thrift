...
#if !defined(STREAMS_BOOST_WAVE_LEXER_THROW)
#ifdef STREAMS_BOOST_NO_STRINGSTREAM
#include <strstream>
#define STREAMS_BOOST_WAVE_LEXER_THROW(cls, code, msg, line, column, name)            \
    {                                                                         \
    ...
    streams_boost::throw_exception(cls(throwmsg.c_str(), cls::code, line, column,     \
        name));                                                               \
    }                                                                         \
    /**/
#else
...
#define STREAMS_BOOST_WAVE_LEXER_THROW(cls, code, msg, line, column, name)            \
    ...
    if ((msg)[0] != 0) stream << ": " << (msg);                               \
    stream << std::ends;                                                      \
    streams_boost::throw_exception(cls(stream.str().c_str(), cls::code, line, column, \
        name));                                                               \
    }                                                                         \
    /**/
#endif // STREAMS_BOOST_NO_STRINGSTREAM
#endif // STREAMS_BOOST_WAVE_LEXER_THROW

#if !defined(STREAMS_BOOST_WAVE_LEXER_THROW_VAR)
#ifdef STREAMS_BOOST_NO_STRINGSTREAM
#include <strstream>
#define STREAMS_BOOST_WAVE_LEXER_THROW_VAR(cls, codearg, msg, line, column, name)     \
    {                                                                         \
    using namespace streams_boost::wave;                                              \
    ...
    stream << std::ends;                                                      \
    std::string throwmsg = stream.str(); stream.freeze(false);                \
    streams_boost::throw_exception(cls(throwmsg.c_str(), code, line, column,          \
        name));                                                               \
    }                                                                         \
    /**/
#else
...
#define STREAMS_BOOST_WAVE_LEXER_THROW_VAR(cls, codearg, msg, line, column, name)     \
    ...
    if ((msg)[0] != 0) stream << ": " << (msg);                               \
    stream << std::ends;                                                      \
    streams_boost::throw_exception(cls(stream.str().c_str(), code, line, column,      \
        name));                                                               \
    }                                                                         \
    /**/
#endif // STREAMS_BOOST_NO_STRINGSTREAM
...
namespace util {
    ...
    get_severity(severity level) 
    {
        ...
        STREAMS_BOOST_ASSERT(severity_remark <= level && level <= severity_fatal);
...
public:
    ...
    {
        return "streams_boost::wave::lexing_exception";
    }
