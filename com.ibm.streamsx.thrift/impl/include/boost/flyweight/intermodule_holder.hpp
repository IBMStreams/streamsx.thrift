...
#ifndef STREAMS_BOOST_FLYWEIGHT_INTERMODULE_HOLDER_HPP
#define STREAMS_BOOST_FLYWEIGHT_INTERMODULE_HOLDER_HPP
...
#include <streams_boost/flyweight/intermodule_holder_fwd.hpp>
#include <streams_boost/flyweight/detail/process_id.hpp>
...
#include <streams_boost/interprocess/managed_shared_memory.hpp>
...
private:
  ...
  private:
    ...
    const char* compute_mutex_name()
    {
      std::sprintf(
        mutex_name,
        "streams_boost_flyweight_intermodule_holder_mutex_"
        "%ld_%u_%u",
    ...
    {      
      std::sprintf(
        segment_name,
        "streams_boost_flyweight_intermodule_holder_segment_"
        "%ld_%u_%u",
        (long)detail::process_id(),
        (unsigned)compute_hash(typeid(C).name(),0),
        (unsigned)compute_hash(typeid(C).name(),1));
...
public:
  ...
  STREAMS_BOOST_MPL_AUX_LAMBDA_SUPPORT(1,intermodule_holder_class,(C))
};
