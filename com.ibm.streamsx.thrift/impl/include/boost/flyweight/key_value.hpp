...
#ifndef STREAMS_BOOST_FLYWEIGHT_KEY_VALUE_HPP
#define STREAMS_BOOST_FLYWEIGHT_KEY_VALUE_HPP

#if defined(_MSC_VER)&&(_MSC_VER>=1200)
#pragma once
#endif

#include <streams_boost/flyweight/detail/value_tag.hpp>
#include <streams_boost/flyweight/key_value_fwd.hpp>
...
#define STREAMS_BOOST_FLYWEIGHT_PERFECT_FWD_NAME explicit rep_type
#define STREAMS_BOOST_FLYWEIGHT_PERFECT_FWD_BODY(n)          \
  :value_ptr(0)                                      \
{                                                    \
  new(spc_ptr())key_type(STREAMS_BOOST_PP_ENUM_PARAMS(n,t)); \
}
#include <streams_boost/flyweight/detail/perfect_fwd.hpp>
  ...
  private:
    ...
    mutable typename streams_boost::aligned_storage<
      (sizeof(key_type)>sizeof(value_type))?
        sizeof(key_type):sizeof(value_type),
      (streams_boost::alignment_of<key_type>::value >
       streams_boost::alignment_of<value_type>::value)?
        streams_boost::alignment_of<key_type>::value:
        streams_boost::alignment_of<value_type>::value
    >::type                                    spc;
...
#define STREAMS_BOOST_FLYWEIGHT_PERFECT_FWD_NAME explicit rep_type
#define STREAMS_BOOST_FLYWEIGHT_PERFECT_FWD_BODY(n) \
  :key(STREAMS_BOOST_PP_ENUM_PARAMS(n,t)),value_ptr(0){}
#include <streams_boost/flyweight/detail/perfect_fwd.hpp>
