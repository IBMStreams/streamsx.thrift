...
#include <streams_boost/parameter/aux_/parameter_requirements.hpp>
...
struct empty_arg_list
{
    ...
    // Constructor taking STREAMS_BOOST_PARAMETER_MAX_ARITY empty_arg_list
    // arguments; this makes initialization
    empty_arg_list(
        STREAMS_BOOST_PP_ENUM_PARAMS(
            STREAMS_BOOST_PARAMETER_MAX_ARITY, void_ STREAMS_BOOST_PP_INTERCEPT
        ))
    {}
    ...
    typename result_of0<F>::type
    operator[](
        STREAMS_BOOST_PARAMETER_lazy_default_fallback<K,F> x) const
    {
        return x.compute_default();
    }
...
struct arg_list : Next
{
    ...
    arg_list( // A0& a0, A1& a1, ...
        STREAMS_BOOST_PP_ENUM_BINARY_PARAMS(STREAMS_BOOST_PARAMETER_MAX_ARITY, A, & a)
    )
      : Next( // a1, a2, ...
            STREAMS_BOOST_PP_ENUM_SHIFTED_PARAMS(STREAMS_BOOST_PARAMETER_MAX_ARITY, a)
          , void_reference()
        )
      , arg(a0)
    {}
    ...
    struct binding
    {
        ...
        {
          typedef typename mpl::eval_if<
                streams_boost::is_same<KW, key_type>
...
#if !STREAMS_BOOST_WORKAROUND(__BORLANDC__, STREAMS_BOOST_TESTED_AT(0x564)) && !STREAMS_BOOST_WORKAROUND(__GNUC__, == 2)
# if STREAMS_BOOST_WORKAROUND(STREAMS_BOOST_MSVC, <= 1300)
    friend yes_tag operator*(arg_list, key_type*);
#  define STREAMS_BOOST_PARAMETER_CALL_HAS_KEY(next, key) (*(next*)0 * (key*)0)
# else
...
#  define STREAMS_BOOST_PARAMETER_CALL_HAS_KEY(next, key) next::has_key((key*)0)  
# endif

    STREAMS_BOOST_MPL_ASSERT_MSG(
        sizeof(STREAMS_BOOST_PARAMETER_CALL_HAS_KEY(Next,key_type)) == sizeof(no_tag)
      , duplicate_keyword, (key_type)
    );

# undef STREAMS_BOOST_PARAMETER_CALL_HAS_KEY
#endif
    //
    ...
    {
        STREAMS_BOOST_MPL_ASSERT_NOT((holds_maybe));
        return arg.value;
    }
