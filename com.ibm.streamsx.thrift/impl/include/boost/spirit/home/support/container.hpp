...
=============================================================================*/
#if !defined(STREAMS_BOOST_SPIRIT_CONTAINER_FEBRUARY_06_2007_1001AM)
#define STREAMS_BOOST_SPIRIT_CONTAINER_FEBRUARY_06_2007_1001AM
...
namespace streams_boost { namespace spirit { namespace traits
{
    ...
    namespace detail
    {
        ...
        STREAMS_BOOST_MPL_HAS_XXX_TRAIT_DEF(iterator)
        STREAMS_BOOST_MPL_HAS_XXX_TRAIT_DEF(size_type)
...
#define STREAMS_BOOST_SPIRIT_IS_CONTAINER(z, N, data)                                 \
        is_container<STREAMS_BOOST_PP_CAT(T, N)>::value ||                            \
    /***/

    template <STREAMS_BOOST_VARIANT_ENUM_PARAMS(typename T)>
    struct is_container<variant<STREAMS_BOOST_VARIANT_ENUM_PARAMS(T)> > 
       : mpl::bool_<STREAMS_BOOST_PP_REPEAT(STREAMS_BOOST_VARIANT_LIMIT_TYPES
            , STREAMS_BOOST_SPIRIT_IS_CONTAINER, _) false> 
    {};

#undef STREAMS_BOOST_SPIRIT_IS_CONTAINER
    ...
    template <STREAMS_BOOST_VARIANT_ENUM_PARAMS(typename T)>
    struct container_value<variant<STREAMS_BOOST_VARIANT_ENUM_PARAMS(T)> >
    {
        typedef typename 
            variant<STREAMS_BOOST_VARIANT_ENUM_PARAMS(T)>::types 
        types;
        typedef typename 
    ...
    struct optional_attribute<optional<T> >
    {
        ...
        {
            return streams_boost::get<T>(val);
        }
    ...
    struct push_back_container<optional<Container>, T>
    {
        static bool call(optional<Container>& c, T const& val)
        {
            if (!c)
                c = Container();
            return push_back(streams_boost::get<Container>(c), val);
        }
    };
    ...
    template <STREAMS_BOOST_VARIANT_ENUM_PARAMS(typename T_), typename T>
    struct push_back_container<variant<STREAMS_BOOST_VARIANT_ENUM_PARAMS(T_)>, T>
    {
        static bool call(variant<STREAMS_BOOST_VARIANT_ENUM_PARAMS(T_)>& c, T const& val)
        {
