...
#if !defined(STREAMS_BOOST_SPIRIT_ITERATOR_FIXED_SIZE_QUEUE_MAR_16_2007_1137AM)
#define STREAMS_BOOST_SPIRIT_ITERATOR_FIXED_SIZE_QUEUE_MAR_16_2007_1137AM
...
#define STREAMS_BOOST_SPIRIT_ASSERT_FSQ_SIZE                                          \
    STREAMS_BOOST_ASSERT(((m_tail + N + 1) - m_head) % (N+1) == m_size % (N+1))       \
    /**/
...
namespace streams_boost { namespace spirit { namespace detail
{
    ...
    private:
            ...
            const &x) const
        {
            ...
            //  containers
            STREAMS_BOOST_ASSERT(p.self == p2.self);
    ...
    fixed_size_queue<T, N>::fixed_size_queue()
        ...
        , m_size(0)
    {
        STREAMS_BOOST_ASSERT(m_size <= N+1);
        STREAMS_BOOST_SPIRIT_ASSERT_FSQ_SIZE;
        STREAMS_BOOST_ASSERT(m_head <= N+1);
        STREAMS_BOOST_ASSERT(m_tail <= N+1);
    }
