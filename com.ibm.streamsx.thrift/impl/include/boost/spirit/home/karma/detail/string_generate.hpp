...
#if !defined(STREAMS_BOOST_SPIRIT_KARMA_STRING_GENERATE_FEB_23_2007_1232PM)
#define STREAMS_BOOST_SPIRIT_KARMA_STRING_GENERATE_FEB_23_2007_1232PM
...
namespace streams_boost { namespace spirit { namespace karma { namespace detail
{
    ...
    {
        typename range_const_iterator<Container>::type end = streams_boost::end(c);
        for (typename range_const_iterator<Container>::type it = streams_boost::begin(c); 
             it != end; ++it)
        {
            *sink = filter(*it);
            ++sink;
