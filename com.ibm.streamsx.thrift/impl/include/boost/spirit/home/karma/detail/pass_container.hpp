...
#define STREAMS_BOOST_SPIRIT_IS_CONVERTIBLE(z, N, data)                               \
        has_same_elements<RHS, STREAMS_BOOST_PP_CAT(T, N)>::value ||                  \
    /***/

    // Note: variants are treated as containers if one of the held types is a
    //       container (see support/container.hpp).
    template <typename RHS, STREAMS_BOOST_VARIANT_ENUM_PARAMS(typename T)>
    struct has_same_elements<
            RHS, streams_boost::variant<STREAMS_BOOST_VARIANT_ENUM_PARAMS(T)>, true>
      : mpl::bool_<STREAMS_BOOST_PP_REPEAT(STREAMS_BOOST_VARIANT_LIMIT_TYPES
          , STREAMS_BOOST_SPIRIT_IS_CONVERTIBLE, _) false> {};

#undef STREAMS_BOOST_SPIRIT_IS_CONVERTIBLE
