...
==============================================================================*/
#if !defined(STREAMS_BOOST_SPIRIT_SYMBOLS_MARCH_11_2007_1055AM)
#define STREAMS_BOOST_SPIRIT_SYMBOLS_MARCH_11_2007_1055AM
...
#include <streams_boost/spirit/home/qi/string/tst.hpp>
...
namespace streams_boost { namespace spirit { namespace qi
{
      ...
      , primitive_parser<symbols<Char, T, Lookup, Filter> >
    {
        ...
        symbols(Symbols const& syms)
          ...
          , lookup(new Lookup())
        {
            typename range_const_iterator<Symbols>::type si = streams_boost::begin(syms);
            while (si != streams_boost::end(syms))
                add(*si++);
        }
        ...
        {
            typename range_const_iterator<Symbols>::type si = streams_boost::begin(syms);
            typename range_const_iterator<Data>::type di = streams_boost::begin(data);
            while (si != streams_boost::end(syms))
                add(*si++, *di++);
        }
