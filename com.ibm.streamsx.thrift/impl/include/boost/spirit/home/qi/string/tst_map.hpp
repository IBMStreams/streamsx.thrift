...
==============================================================================*/
#if !defined(STREAMS_BOOST_SPIRIT_TST_MAP_JUNE_03_2007_1143AM)
#define STREAMS_BOOST_SPIRIT_TST_MAP_JUNE_03_2007_1143AM

#if defined(_MSC_VER)
#pragma once
#endif

#include <streams_boost/spirit/home/qi/string/detail/tst.hpp>
#include <streams_boost/unordered_map.hpp>
#include <streams_boost/pool/object_pool.hpp>
...
namespace streams_boost { namespace spirit { namespace qi
{
    ...
    struct tst_map
    {
        ...
        bool add(
            Iterator first
          , Iterator last
          , typename streams_boost::call_traits<T>::param_type val)
        {
            if (first != last)
            {
                map_data x = {0, 0};
        ...
        void clear()
        {
            STREAMS_BOOST_FOREACH(typename map_type::value_type& x, map)
            {
        ...
        {
            STREAMS_BOOST_FOREACH(typename map_type::value_type const& x, map)
            {
    ...
    private:
        ...
        {
            STREAMS_BOOST_FOREACH(typename map_type::value_type const& x, rhs.map)
            {
        ...
        T* new_data(typename streams_boost::call_traits<T>::param_type val)
        {
