...
#define STREAMS_BOOST_SPIRIT_IS_CONVERTIBLE(z, N, data)                               \
        has_same_elements<LHS, STREAMS_BOOST_PP_CAT(T, N)>::value ||                  \
    /***/

    // Note: variants are treated as containers if one of the held types is a
    //       container (see support/container.hpp).
    template <typename LHS, STREAMS_BOOST_VARIANT_ENUM_PARAMS(typename T)>
    struct has_same_elements<
            LHS, streams_boost::variant<STREAMS_BOOST_VARIANT_ENUM_PARAMS(T)>, true>
