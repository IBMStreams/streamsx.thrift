...
#if !defined(STREAMS_BOOST_SPIRIT_LEX_LEXER_MAR_17_2007_0139PM)
#define STREAMS_BOOST_SPIRIT_LEX_LEXER_MAR_17_2007_0139PM
...
#include <streams_boost/spirit/home/support/detail/lexer/generator.hpp>
...
#include <streams_boost/spirit/home/lex/lexer/lexertl/token.hpp>
#include <streams_boost/spirit/home/lex/lexer/lexertl/functor.hpp>
#include <streams_boost/spirit/home/lex/lexer/lexertl/functor_data.hpp>
#include <streams_boost/spirit/home/lex/lexer/lexertl/iterator.hpp>
#if defined(STREAMS_BOOST_SPIRIT_LEXERTL_DEBUG)
...
{ 
    ...
    namespace detail
    {
        ...
        //  
        ///////////////////////////////////////////////////////////////////////
        inline streams_boost::lexer::regex_flags map_flags(unsigned int flags)
        {
            unsigned int retval = streams_boost::lexer::none;
            if (flags & match_flags::match_not_dot_newline)
                retval |= streams_boost::lexer::dot_not_newline;
            if (flags & match_flags::match_icase)
                retval |= streams_boost::lexer::icase;

            return streams_boost::lexer::regex_flags(retval);
        }
    }
    ...
    public:
        ...
        typedef typename streams_boost::detail::iterator_traits<Iterator>::value_type 
            char_type;
    ...
    private:
        ...
        struct iterator_data_type 
        {
            ...
            iterator_data_type(
                    streams_boost::lexer::basic_state_machine<char_type> const& sm
                  , streams_boost::lexer::basic_rules<char_type> const& rules
                  , semantic_actions_type const& actions)
              : state_machine_(sm), rules_(rules), actions_(actions)
            {}

            streams_boost::lexer::basic_state_machine<char_type> const& state_machine_;
            streams_boost::lexer::basic_rules<char_type> const& rules_;
    ...
    public:
        ...
        streams_boost::lexer::rules const& get_rules() const { return rules_; }

        void clear(char_type const* state)
        {
            std::size_t s = rules_.state(state);
            if (streams_boost::lexer::npos != s)
                rules_.clear(state);
            initialized_dfa_ = false;
        }
        std::size_t add_state(char_type const* state)
        {
            ...
            if (streams_boost::lexer::npos == stateid) {
        ...
        bool init_dfa() const
        {
            if (!initialized_dfa_) {
                ...
                typedef streams_boost::lexer::basic_generator<char_type> generator;
                generator::build (rules_, state_machine_);
                generator::minimise (state_machine_);

#if defined(STREAMS_BOOST_SPIRIT_LEXERTL_DEBUG)
                streams_boost::lexer::debug::dump(state_machine_, std::cerr);
#endif
    ...
    private:
        ...
        mutable streams_boost::lexer::basic_state_machine<char_type> state_machine_;
        streams_boost::lexer::regex_flags flags_;
        streams_boost::lexer::basic_rules<char_type> rules_;
