...
#if !defined(STREAMS_BOOST_SPIRIT_LEX_TOKEN_FEB_10_2008_0751PM)
#define STREAMS_BOOST_SPIRIT_LEX_TOKEN_FEB_10_2008_0751PM
...
#include <streams_boost/fusion/include/value_at.hpp>
...
{ 
    ...
    struct token<Iterator, lex::omit, mpl::false_>
    {
        ...
        token() : id_(streams_boost::lexer::npos) {}
        ...
        bool is_valid() const 
        { 
            return 0 != id_ && id_type(streams_boost::lexer::npos) != id_; 
        }
...
// works only starting MSVC V8
    ...
    public:
        ...
        //  default constructed tokens correspond to EOI tokens
        token() : state_(streams_boost::lexer::npos) {}

        //  construct an invalid token
        explicit token(int) : base_type(0), state_(streams_boost::lexer::npos) {}

        token(id_type id, std::size_t state)
          : base_type(id, streams_boost::lexer::npos), state_(state) {}

        token(id_type id, std::size_t state, token_value_type)
          : base_type(id, streams_boost::lexer::npos, unused)
          , state_(state) {}

        token(id_type id, std::size_t state
              , Iterator const& first, Iterator const& last)
          : base_type(id, streams_boost::lexer::npos, first, last)
          , state_(state) {}
...
#if !STREAMS_BOOST_WORKAROUND(STREAMS_BOOST_MSVC, <= 1300)
        STREAMS_BOOST_STATIC_ASSERT((mpl::is_sequence<AttributeTypes>::value || 
    ...
    public:
...
#if STREAMS_BOOST_WORKAROUND(STREAMS_BOOST_MSVC, == 1600)
        ...
        token& operator= (token const& rhs)
        {
            if (this != &rhs) 
            {
                ...
                if (this->id_ != streams_boost::lexer::npos && this->id_ != 0) 
                    value_ = rhs.value_;
            }
            return *this;
        }
