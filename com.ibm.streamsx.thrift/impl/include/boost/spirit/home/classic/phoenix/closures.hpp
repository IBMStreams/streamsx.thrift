...
namespace impl
{
    ...
    private:
        streams_boost::thread_specific_ptr<frame_ptr> tsp_frame;
...
private:
...
#ifdef PHOENIX_THREADSAFE
    static streams_boost::thread_specific_ptr<holder_t*> &
    tsp_frame_instance()
    {
        static streams_boost::thread_specific_ptr<holder_t*> the_instance;
        return the_instance;
    }
    ...
    closure_frame_holder_ref(holder_t* holder_ = 0)
    {
        ...
        streams_boost::call_once(been_here, tsp_frame_instance_init);
        streams_boost::thread_specific_ptr<holder_t*> &tsp_frame = tsp_frame_instance();
        if (!tsp_frame.get())
