...
#define STREAMS_BOOST_SPIRIT_ASSERT_FSQ_SIZE \
    STREAMS_BOOST_SPIRIT_ASSERT(((m_tail + N + 1) - m_head) % (N+1) == m_size % (N+1)) \
    /**/
...
private:
        ...
        const &x) const
    {
        ...
        //  containers
        STREAMS_BOOST_SPIRIT_ASSERT(p.self == p2.self);
...
private:
    ...
    STREAMS_BOOST_STATIC_CONSTANT(std::size_t, MAX_SIZE = N);
...
fixed_size_queue<T, N>::fixed_size_queue()
    ...
    , m_size(0)
{
    STREAMS_BOOST_SPIRIT_ASSERT(m_size <= N+1);
    STREAMS_BOOST_SPIRIT_ASSERT_FSQ_SIZE;
    STREAMS_BOOST_SPIRIT_ASSERT(m_head <= N+1);
    STREAMS_BOOST_SPIRIT_ASSERT(m_tail <= N+1);
}
...
fixed_size_queue<T, N>::push_back(const T& e)
{
    ...
    STREAMS_BOOST_SPIRIT_ASSERT(!full());
...
#undef STREAMS_BOOST_SPIRIT_ASSERT_FSQ_SIZE
