...
#include <strstream>
#define STREAMS_BOOST_SPIRIT_OSSTREAM std::ostrstream
inline
std::string STREAMS_BOOST_SPIRIT_GETSTRING(std::ostrstream& ss)
{
    ss << std::ends;
...
#else
#include <sstream>
#define STREAMS_BOOST_SPIRIT_GETSTRING(ss) ss.str()
#define STREAMS_BOOST_SPIRIT_OSSTREAM std::basic_ostringstream<CharT>
#endif
...
namespace impl {
    ...
    struct string_lit<wchar_t>
    {
        ...
        static std::basic_string<wchar_t> get(char const* source = "")
        {
            ...
            STREAMS_BOOST_ASSERT(std::has_facet<std::ctype<wchar_t> >(std::locale()));
...
namespace xml {
    ...
    public:
        ...
        ~document()
        {
            STREAMS_BOOST_SPIRIT_ASSERT(-1 == this->get_indent());
        }
    };
...
namespace impl {
    ...
    get_rulename (AssocContainerT const &id_to_name_map,
        STREAMS_BOOST_SPIRIT_CLASSIC_NS::parser_id const &id)
    {
        ...
        std::basic_string<CharT> const &input_line)
{
    return basic_tree_to_xml<CharT>(ostrm, tree, input_line,
        std::map<STREAMS_BOOST_SPIRIT_CLASSIC_NS::parser_id, std::basic_string<CharT> >());
}
...
#undef STREAMS_BOOST_SPIRIT_OSSTREAM
#undef STREAMS_BOOST_SPIRIT_GETSTRING
