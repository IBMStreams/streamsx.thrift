...
=============================================================================*/
#ifndef STREAMS_BOOST_SPIRIT_TREE_COMMON_HPP
#define STREAMS_BOOST_SPIRIT_TREE_COMMON_HPP

#if !defined(STREAMS_BOOST_SPIRIT_USE_LIST_FOR_TREES)
#include <vector>
#else
#include <list>
#endif

#if defined(STREAMS_BOOST_SPIRIT_USE_STREAMS_BOOST_ALLOCATOR_FOR_TREES)
...
#include <streams_boost/spirit/home/classic/core.hpp>
#include <streams_boost/detail/iterator.hpp> // for streams_boost::detail::iterator_traits

#if defined(STREAMS_BOOST_SPIRIT_DEBUG) && \
    (STREAMS_BOOST_SPIRIT_DEBUG_FLAGS & STREAMS_BOOST_SPIRIT_DEBUG_FLAGS_NODES)
#include <iostream>
...
#elif !defined(STREAMS_BOOST_SPIRIT_USE_LIST_FOR_TREES)
    typedef streams_boost::pool_allocator<tree_node<T> > allocator_type;
#else
    typedef streams_boost::fast_pool_allocator<tree_node<T> > allocator_type;
#endif
...
#endif  // STREAMS_BOOST_SPIRIT_USE_LIST_FOR_TREES
...
{
    typedef
        typename streams_boost::detail::iterator_traits<IteratorT>::value_type
        value_type;

#if !defined(STREAMS_BOOST_SPIRIT_USE_STREAMS_BOOST_ALLOCATOR_FOR_TREES)
    typedef std::allocator<value_type> allocator_type;
#elif !defined(STREAMS_BOOST_SPIRIT_USE_LIST_FOR_TREES)
    typedef streams_boost::pool_allocator<value_type> allocator_type;
#else
    typedef streams_boost::fast_pool_allocator<value_type> allocator_type;
#endif
...
namespace impl {
    ...
    {
        using std::swap;
        using STREAMS_BOOST_SPIRIT_CLASSIC_NS::swap;
...
public:
    ...
    typedef T attr_t;
    typedef typename streams_boost::call_traits<T>::param_type      param_type;
...
struct common_tree_match_policy : public match_policy
{
    ...
    (STREAMS_BOOST_SPIRIT_DEBUG_FLAGS & STREAMS_BOOST_SPIRIT_DEBUG_FLAGS_NODES)
        ...
        for (Iterator1T it = first; it != last; ++it)
            impl::token_printer(STREAMS_BOOST_SPIRIT_DEBUG_OUT, *it);
        STREAMS_BOOST_SPIRIT_DEBUG_OUT << "\"\n";
        STREAMS_BOOST_SPIRIT_DEBUG_OUT << ">>> create_node(end) <<<\n\n"; 
#endif
    ...
    (STREAMS_BOOST_SPIRIT_DEBUG_FLAGS & STREAMS_BOOST_SPIRIT_DEBUG_FLAGS_NODES)
        ...
        STREAMS_BOOST_SPIRIT_DEBUG_OUT << "\n>>> concat_match(begin) <<<\n";
        STREAMS_BOOST_SPIRIT_DEBUG_OUT << "tree a:\n" << a << "\n";
        STREAMS_BOOST_SPIRIT_DEBUG_OUT << "tree b:\n" << b << "\n";
        STREAMS_BOOST_SPIRIT_DEBUG_OUT << ">>> concat_match(end) <<<\n\n";
#endif
        STREAMS_BOOST_SPIRIT_ASSERT(a && b);
        if (a.length() == 0)
        {
            a = b;
            return;
...
#ifdef STREAMS_BOOST_SPIRIT_NO_TREE_NODE_COLLAPSING
...
#if defined(STREAMS_BOOST_SPIRIT_DEBUG) && \
    (STREAMS_BOOST_SPIRIT_DEBUG_FLAGS & STREAMS_BOOST_SPIRIT_DEBUG_FLAGS_TREES)
        ...
        for (IteratorT2 it = first; it != last; ++it)
            impl::token_printer(STREAMS_BOOST_SPIRIT_DEBUG_OUT, *it);
        STREAMS_BOOST_SPIRIT_DEBUG_OUT << "\"\n";
        STREAMS_BOOST_SPIRIT_DEBUG_OUT << "new child tree (before grouping):\n" << m << "\n";

        tree_policy_t::group_match(m, id, first, last);

        STREAMS_BOOST_SPIRIT_DEBUG_OUT << "new child tree (after grouping):\n" << m << "\n";
        STREAMS_BOOST_SPIRIT_DEBUG_OUT << ">>> group_match(end) <<<\n\n";
#else
        tree_policy_t::group_match(m, id, first, last);
#endif
    }
...
struct infix_node_op
{
    ...
    void operator()(MatchT& m) const
    {
        ...
        STREAMS_BOOST_SPIRIT_ASSERT(tree_size >= 1);
...
struct inner_node_op
{
    ...
    void operator()(MatchT& m) const
    {
        ...
        STREAMS_BOOST_SPIRIT_ASSERT(tree_size >= 2);
