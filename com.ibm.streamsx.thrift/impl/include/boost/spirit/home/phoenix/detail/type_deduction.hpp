...
/*=============================================================================
    ...
    through a streams_boost::mpl vector which holds each of the corresponding
    result types.
...
#include <streams_boost/spirit/home/phoenix/detail/local_reference.hpp>
...
namespace streams_boost { namespace type_deduction_detail
{
...
}} // namespace streams_boost::type_deduction_detail

#define STREAMS_BOOST_RESULT_OF_COMMON(expr, name, Y, SYMMETRY)                         \
    struct name                                                                 \
    {                                                                           \
        ...
        STREAMS_BOOST_STATIC_CONSTANT(int,                                              \
            size = sizeof(                                                      \
                ...
                >(expr)                                                         \
                                                                                ...
                                                                                \
        STREAMS_BOOST_STATIC_CONSTANT(int, index = (size / sizeof(char)) - 1);          \
                                                                                \
...
#define STREAMS_BOOST_UNARY_RESULT_OF(expr, name)                                       \
    template <typename X>                                                       \
    STREAMS_BOOST_RESULT_OF_COMMON(expr, name,                                          \
        type_deduction_detail::asymmetric, type_deduction_detail::asymmetric)

#define STREAMS_BOOST_BINARY_RESULT_OF(expr, name)                                      \
    template <typename X, typename Y>                                           \
    STREAMS_BOOST_RESULT_OF_COMMON(expr, name, Y, typename base_type::y_type)

#define STREAMS_BOOST_ASYMMETRIC_BINARY_RESULT_OF(expr, name)                           \
    template <typename X, typename Y>                                           \
    STREAMS_BOOST_RESULT_OF_COMMON(expr, name, Y, type_deduction_detail::asymmetric)
