...
#ifndef STREAMS_BOOST_LOGIC_TRIBOOL_IO_HPP
#define STREAMS_BOOST_LOGIC_TRIBOOL_IO_HPP

#include <streams_boost/logic/tribool.hpp>
#include <streams_boost/detail/workaround.hpp>
#include <streams_boost/noncopyable.hpp>

#if STREAMS_BOOST_WORKAROUND(_MSC_VER, >= 1200)
#  pragma once
#endif
...
namespace streams_boost { namespace logic {
...
#  ifndef STREAMS_BOOST_NO_WCHAR_T
/**
...
class indeterminate_name : public std::locale::facet, private streams_boost::noncopyable
{
public:
...
#ifndef STREAMS_BOOST_NO_STD_LOCALE
        if (STREAMS_BOOST_HAS_FACET(indeterminate_name<CharT>, out.getloc())) {
          const indeterminate_name<CharT>& facet =
            STREAMS_BOOST_USE_FACET(indeterminate_name<CharT>, out.getloc());
          out << facet.name();
        } else {
  ...
  if (in.flags() & std::ios_base::boolalpha) {
    ...
    if (cerberus) {
      ...
      const std::numpunct<CharT>& numpunct_facet =
        STREAMS_BOOST_USE_FACET(std::numpunct<CharT>, in.getloc());

      string_type falsename = numpunct_facet.falsename();
      string_type truename = numpunct_facet.truename();

      string_type othername;
      if (STREAMS_BOOST_HAS_FACET(indeterminate_name<CharT>, in.getloc())) {
        othername =
          STREAMS_BOOST_USE_FACET(indeterminate_name<CharT>, in.getloc()).name();
      } else {
...
} } // end namespace streams_boost::logic

#endif // STREAMS_BOOST_LOGIC_TRIBOOL_IO_HPP
