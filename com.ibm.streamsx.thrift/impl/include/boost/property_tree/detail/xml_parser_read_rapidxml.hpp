...
// ----------------------------------------------------------------------------
#ifndef STREAMS_BOOST_PROPERTY_TREE_DETAIL_XML_PARSER_READ_RAPIDXML_HPP_INCLUDED
#define STREAMS_BOOST_PROPERTY_TREE_DETAIL_XML_PARSER_READ_RAPIDXML_HPP_INCLUDED

#include <streams_boost/property_tree/ptree.hpp>
#include <streams_boost/property_tree/detail/xml_parser_error.hpp>
#include <streams_boost/property_tree/detail/xml_parser_flags.hpp>
#include <streams_boost/property_tree/detail/xml_parser_utils.hpp>
#include <streams_boost/property_tree/detail/rapidxml.hpp>
#include <vector>
...
namespace streams_boost { namespace property_tree { namespace xml_parser
{
                           ...
                           const std::string &filename)
    {
        ...
        if (!stream.good())
            STREAMS_BOOST_PROPERTY_TREE_THROW(
        ...
        try {
            ...
            if (flags & no_comments) {
                if (flags & trim_whitespace)
                    doc.STREAMS_BOOST_NESTED_TEMPLATE parse<f_tws>(&v.front());
                else
                    doc.STREAMS_BOOST_NESTED_TEMPLATE parse<0>(&v.front());
            } else {
                if (flags & trim_whitespace)
                    doc.STREAMS_BOOST_NESTED_TEMPLATE parse<f_tws | f_c>(&v.front());
                else
                    doc.STREAMS_BOOST_NESTED_TEMPLATE parse<f_c>(&v.front());
            }
