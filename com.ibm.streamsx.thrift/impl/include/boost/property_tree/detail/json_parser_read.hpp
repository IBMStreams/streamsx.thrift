...
// ----------------------------------------------------------------------------
#ifndef STREAMS_BOOST_PROPERTY_TREE_DETAIL_JSON_PARSER_READ_HPP_INCLUDED
#define STREAMS_BOOST_PROPERTY_TREE_DETAIL_JSON_PARSER_READ_HPP_INCLUDED

//#define STREAMS_BOOST_SPIRIT_DEBUG

#include <streams_boost/property_tree/ptree.hpp>
#include <streams_boost/property_tree/detail/ptree_utils.hpp>
#include <streams_boost/property_tree/detail/json_parser_error.hpp>
#include <streams_boost/spirit/include/classic.hpp>
...
namespace streams_boost { namespace property_tree { namespace json_parser
{
    ...
    struct context
    {
        ...
        struct a_object_e
        {
            ...
            void operator()(Ch) const
            {
                STREAMS_BOOST_ASSERT(c.stack.size() >= 1);
                c.stack.pop_back();
            }
        };
        ...
        struct a_escape
        {
            ...
            void operator()(Ch ch) const
            {
                switch (ch)
                {
                    ...
                    default: STREAMS_BOOST_ASSERT(0);
                }
            }
        };
    ...
    template<class Ptree>
    struct json_grammar :
        public streams_boost::spirit::classic::grammar<json_grammar<Ptree> >
    {
        ...
        struct definition
        {
            ...
            streams_boost::spirit::classic::rule<Scanner>
                root, object, member, array, item, value, string, number;
            streams_boost::spirit::classic::rule<
                typename streams_boost::spirit::classic::lexeme_scanner<Scanner>::type>
                character, escape;
            ...
            definition(const json_grammar &self)
            {
                ...
                // Debug
                STREAMS_BOOST_SPIRIT_DEBUG_RULE(root);
                STREAMS_BOOST_SPIRIT_DEBUG_RULE(object);
                STREAMS_BOOST_SPIRIT_DEBUG_RULE(member);
                STREAMS_BOOST_SPIRIT_DEBUG_RULE(array);
                STREAMS_BOOST_SPIRIT_DEBUG_RULE(item);
                STREAMS_BOOST_SPIRIT_DEBUG_RULE(value);
                STREAMS_BOOST_SPIRIT_DEBUG_RULE(string);
                STREAMS_BOOST_SPIRIT_DEBUG_RULE(number);
                STREAMS_BOOST_SPIRIT_DEBUG_RULE(escape);
                STREAMS_BOOST_SPIRIT_DEBUG_RULE(character);

            }

            const streams_boost::spirit::classic::rule<Scanner> &start() const
            {
                return root;
            }
                            ...
                            const std::string &filename)
    {
        ...
        if (!stream.good())
            STREAMS_BOOST_PROPERTY_TREE_THROW(json_parser_error("read error", filename, 0));
        ...
        try
        {
            ...
            if (!pi.hit || !pi.full)
                STREAMS_BOOST_PROPERTY_TREE_THROW((parser_error<std::string, It>(v.begin(), "syntax error")));
        }
        catch (parser_error<std::string, It> &e)
        {
            STREAMS_BOOST_PROPERTY_TREE_THROW(json_parser_error(e.descriptor, filename, count_lines<It, Ch>(v.begin(), e.where)));
        }
