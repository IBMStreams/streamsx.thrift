...
// ----------------------------------------------------------------------------
#ifndef STREAMS_BOOST_PROPERTY_TREE_DETAIL_JSON_PARSER_WRITE_HPP_INCLUDED
#define STREAMS_BOOST_PROPERTY_TREE_DETAIL_JSON_PARSER_WRITE_HPP_INCLUDED
...
namespace streams_boost { namespace property_tree { namespace json_parser
{
                           ...
                           int indent)
    {
        ...
        else if (indent > 0 && pt.count(Str()) == pt.size())
        {
            ...
            for (; it != pt.end(); ++it)
            {
                ...
                if (streams_boost::next(it) != pt.end())
                    stream << Ch(',');
                stream << Ch('\n');
            }
    ...
    {
        if (!verify_json(pt, 0))
            STREAMS_BOOST_PROPERTY_TREE_THROW(json_parser_error("ptree contains data that cannot be represented in JSON format", filename, 0));
        write_json_helper(stream, pt, 0);
        stream << std::endl;
        if (!stream.good())
            STREAMS_BOOST_PROPERTY_TREE_THROW(json_parser_error("write error", filename, 0));
    }
