...
// ----------------------------------------------------------------------------
#ifndef STREAMS_BOOST_PROPERTY_TREE_DETAIL_INFO_PARSER_READ_HPP_INCLUDED
#define STREAMS_BOOST_PROPERTY_TREE_DETAIL_INFO_PARSER_READ_HPP_INCLUDED

#include "streams_boost/property_tree/ptree.hpp"
#include "streams_boost/property_tree/detail/info_parser_error.hpp"
#include "streams_boost/property_tree/detail/info_parser_utils.hpp"
#include <iterator>
#include <string>
#include <stack>
#include <fstream>
...
namespace streams_boost { namespace property_tree { namespace info_parser
{
        ...
        expand_escapes(It b, It e)
    {
        ...
        while (b != e)
        {
            if (*b == Ch('\\'))
            {
                ...
                if (b == e)
                {
                    STREAMS_BOOST_PROPERTY_TREE_THROW(info_parser_error(
    ...
    std::basic_string<Ch> read_string(const Ch *&text, bool *need_more_lines)
    {
        ...
        if (*text == Ch('\"'))
        {
            ...
            STREAMS_BOOST_PROPERTY_TREE_THROW(info_parser_error("expected \"", "", 0));
    }
                            ...
                            int include_depth)
    {
                ...
                while (1) {
                    ...
                    switch (state)
                    {
                        ...
                        case s_key:
                        {
                            ...
                            {
                                if (!last)
                                    STREAMS_BOOST_PROPERTY_TREE_THROW(info_parser_error("unexpected {", "", 0));
                                stack.push(last);
                                last = NULL;
                                ++text;
                            }
                            ...
                            {
                                if (stack.size() <= 1)
                                    STREAMS_BOOST_PROPERTY_TREE_THROW(info_parser_error("unmatched }", "", 0));
                                stack.pop();
                                last = NULL;
                                ++text;
                            }
                        ...
                        case s_data_cont:
                        {
                            ...
                            if (*text == Ch('\"'))  // Continuation must start with "
                            {
                                ...
                                STREAMS_BOOST_PROPERTY_TREE_THROW(info_parser_error("expected \" after \\ in previous line", "", 0));
        ...
        catch (info_parser_error &e)
        {
            ...
            if (e.line() == 0)
            {
                STREAMS_BOOST_PROPERTY_TREE_THROW(info_parser_error(e.message(), filename, line_no));
            }
            else
                STREAMS_BOOST_PROPERTY_TREE_THROW(e);
