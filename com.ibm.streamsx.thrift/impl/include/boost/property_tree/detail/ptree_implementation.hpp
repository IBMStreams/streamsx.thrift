...
// ----------------------------------------------------------------------------
#ifndef STREAMS_BOOST_PROPERTY_TREE_DETAIL_PTREE_IMPLEMENTATION_HPP_INCLUDED
#define STREAMS_BOOST_PROPERTY_TREE_DETAIL_PTREE_IMPLEMENTATION_HPP_INCLUDED

#include <streams_boost/iterator/iterator_adaptor.hpp>
#include <streams_boost/iterator/reverse_iterator.hpp>
#include <memory>

namespace streams_boost { namespace property_tree
{
    ...
    struct basic_ptree<K, D, C>::subs
    {
        ...
        static by_name_index& assoc(self_type *s) {
            return ch(s).STREAMS_BOOST_NESTED_TEMPLATE get<by_name>();
        }
    ...
    template <class K, class D, class C>
    class basic_ptree<K, D, C>::iterator : public streams_boost::iterator_adaptor<
    ...
    template <class K, class D, class C>
    class basic_ptree<K, D, C>::const_iterator : public streams_boost::iterator_adaptor<
    ...
    class basic_ptree<K, D, C>::reverse_iterator
        : public streams_boost::reverse_iterator<iterator>
    {
    public:
        reverse_iterator() {}
        explicit reverse_iterator(iterator b)
            : streams_boost::reverse_iterator<iterator>(b)
        {}
    };
    template <class K, class D, class C>
    class basic_ptree<K, D, C>::const_reverse_iterator
        : public streams_boost::reverse_iterator<const_iterator>
    {
    public:
        const_reverse_iterator() {}
        explicit const_reverse_iterator(const_iterator b)
            : streams_boost::reverse_iterator<const_iterator>(b)
        {}
        const_reverse_iterator(
    ...
    class basic_ptree<K, D, C>::assoc_iterator
        : public streams_boost::iterator_adaptor<assoc_iterator,
                                         typename subs::by_name_index::iterator,
                                         value_type>
    {
        friend class streams_boost::iterator_core_access;
        typedef streams_boost::iterator_adaptor<assoc_iterator,
                                         typename subs::by_name_index::iterator,
                                         value_type>
            baset;
    public:
    ...
    class basic_ptree<K, D, C>::const_assoc_iterator
        : public streams_boost::iterator_adaptor<const_assoc_iterator,
        ...
        basic_ptree<K, D, C>::get_child(const path_type &path)
    {
        ...
        if (!n) {
            STREAMS_BOOST_PROPERTY_TREE_THROW(ptree_bad_path("No such node", path));
        }
        return *n;
    }
    ...
    typename streams_boost::enable_if<detail::is_translator<Translator>, Type>::type
    basic_ptree<K, D, C>::get_value(Translator tr) const
    {
        if(streams_boost::optional<Type> o = get_value_optional<Type>(tr)) {
            return *o;
        }
        STREAMS_BOOST_PROPERTY_TREE_THROW(ptree_bad_data(
    ...
    template<class K, class D, class C>
    template<class Type> inline
    typename streams_boost::disable_if<detail::is_translator<Type>, Type>::type
    ...
    basic_ptree<K, D, C>::get(const path_type &path,
                              Translator tr) const
    {
        return get_child(path).STREAMS_BOOST_NESTED_TEMPLATE get_value<Type>(tr);
    }
    ...
    {
        return get_child(path).STREAMS_BOOST_NESTED_TEMPLATE get_value<Type>();
    }
    ...
    {
        if (optional<const self_type&> child = get_child_optional(path))
            return child.get().STREAMS_BOOST_NESTED_TEMPLATE get_value_optional<Type>();
        else
            return optional<Type>();
    }
    ...
    void basic_ptree<K, D, C>::put_value(const Type &value, Translator tr)
    {
        if(optional<data_type> o = tr.put_value(value)) {
            ...
            STREAMS_BOOST_PROPERTY_TREE_THROW(ptree_bad_data(
                ...
                "\" to data failed", streams_boost::any()));
        }
    }
