...
// ----------------------------------------------------------------------------
#ifndef STREAMS_BOOST_PROPERTY_TREE_DETAIL_XML_PARSER_UTILS_HPP_INCLUDED
#define STREAMS_BOOST_PROPERTY_TREE_DETAIL_XML_PARSER_UTILS_HPP_INCLUDED
    ...
    std::basic_string<Ch> decode_char_entities(const std::basic_string<Ch> &s)
    {
        ...
        for (typename Str::const_iterator it = s.begin(); it != end; ++it)
        {
            if (*it == Ch('&'))
            {
                ...
                if (semicolon == end)
                    STREAMS_BOOST_PROPERTY_TREE_THROW(xml_parser_error("invalid character entity", "", 0));
