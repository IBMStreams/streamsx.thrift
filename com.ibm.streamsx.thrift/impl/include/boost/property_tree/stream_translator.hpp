...
#ifndef STREAMS_BOOST_PROPERTY_TREE_STREAM_TRANSLATOR_HPP_INCLUDED
#define STREAMS_BOOST_PROPERTY_TREE_STREAM_TRANSLATOR_HPP_INCLUDED

#include <streams_boost/property_tree/ptree_fwd.hpp>

#include <streams_boost/optional.hpp>
#include <streams_boost/optional/optional_io.hpp>
...
namespace streams_boost { namespace property_tree
{
    ...
    namespace detail
    {
        ...
        struct is_inexact_impl
        {
            ...
            struct test
            {
                typedef streams_boost::false_type type;
            };
        };
        template <>
        struct is_inexact_impl<true>
        {
            ...
            struct test
            {
              typedef streams_boost::integral_constant<bool,
        ...
        struct is_inexact
        {
            typedef typename streams_boost::decay<F>::type decayed;
            typedef typename is_inexact_impl<
                std::numeric_limits<decayed>::is_specialized
            >::STREAMS_BOOST_NESTED_TEMPLATE test<decayed>::type type;
    ...
    struct customize_stream<Ch, Traits, F,
        typename streams_boost::enable_if< detail::is_inexact<F> >::type
    >
    {
    ...
    public:
        ...
        streams_boost::optional<E> get_value(const internal_type &v) {
            std::basic_istringstream<Ch, Traits, Alloc> iss(v);
            iss.imbue(m_loc);
            E e;
            customized::extract(iss, e);
            if(iss.fail() || iss.bad() || iss.get() != Traits::eof()) {
                return streams_boost::optional<E>();
            }
            return e;
        }
        streams_boost::optional<internal_type> put_value(const E &v) {
            ...
            return streams_boost::optional<internal_type>();
        }
