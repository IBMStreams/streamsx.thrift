...
#ifndef STREAMS_BOOST_ASIO_BUFFERS_ITERATOR_HPP
#define STREAMS_BOOST_ASIO_BUFFERS_ITERATOR_HPP
...
namespace detail
{
  ...
  struct buffers_iterator_types_helper<false>
  {
    ...
    struct byte_type
    {
      typedef typename streams_boost::add_const<ByteType>::type type;
    };
  };
  ...
  struct buffers_iterator_types
  {
    enum
    {
      is_mutable = streams_boost::is_convertible<
...
public:
...
#if STREAMS_BOOST_WORKAROUND(__GNUC__, == 4) && STREAMS_BOOST_WORKAROUND(__GNUC_MINOR__, == 3)
...
#endif
  {
    ...
    while (new_iter.current_ != new_iter.end_)
    {
      ...
      if (streams_boost::asio::buffer_size(new_iter.current_buffer_) > 0)
        break;
      ++new_iter.current_;
    }
    return new_iter;
...
#endif
  {
    ...
    while (new_iter.current_ != new_iter.end_)
    {
      ...
      new_iter.position_ += streams_boost::asio::buffer_size(buffer);
      ++new_iter.current_;
    }
    return new_iter;
  }
...
private:
  ...
  void increment()
  {
    STREAMS_BOOST_ASSERT(current_ != end_ && "iterator out of bounds");
    ++position_;
  ...
  void decrement()
  {
    STREAMS_BOOST_ASSERT(position_ > 0 && "iterator out of bounds");
    --position_;
  ...
  void advance(std::ptrdiff_t n)
  {
    if (n > 0)
    {
      ...
      for (;;)
      {
        std::ptrdiff_t current_buffer_balance
          = streams_boost::asio::buffer_size(current_buffer_)
          - current_buffer_position_;
        ...
        if (++current_ == end_)
        {
          STREAMS_BOOST_ASSERT(n == 0 && "iterator out of bounds");
    ...
    {
      std::size_t abs_n = -n;
      STREAMS_BOOST_ASSERT(position_ >= abs_n && "iterator out of bounds");
      for (;;)
      {
        // Check if the advance can be satisfied by the current buffer.
        ...
        if (current_ == begin_)
        {
          STREAMS_BOOST_ASSERT(abs_n == 0 && "iterator out of bounds");
          current_buffer_position_ = 0;
          return;
        }
...
#endif // STREAMS_BOOST_ASIO_BUFFERS_ITERATOR_HPP
