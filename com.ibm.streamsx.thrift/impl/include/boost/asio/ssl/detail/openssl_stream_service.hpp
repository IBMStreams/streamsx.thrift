...
#ifndef STREAMS_BOOST_ASIO_SSL_DETAIL_OPENSSL_STREAM_SERVICE_HPP
#define STREAMS_BOOST_ASIO_SSL_DETAIL_OPENSSL_STREAM_SERVICE_HPP
...
#include <streams_boost/asio/ssl/detail/openssl_operation.hpp>
...
class openssl_stream_service
  : public streams_boost::asio::detail::service_base<openssl_stream_service>
{
private:
  ...
  {
  public:
    typedef streams_boost::function<
      void (const streams_boost::system::error_code&, size_t)> func_t;

    base_handler(streams_boost::asio::io_service& io_service)
      : op_(NULL)
      , io_service_(io_service)
      , work_(io_service)
    {}
    
    void do_func(const streams_boost::system::error_code& error, size_t size)
    {
      func_(error, size);
    }
  ...
  {
  public:
    io_handler(Handler handler, streams_boost::asio::io_service& io_service)
      : base_handler<Stream>(io_service)
      , handler_(handler)
    {
      set_func(streams_boost::bind(
        &io_handler<Stream, Handler>::handler_impl, 
        this, streams_boost::arg<1>(), streams_boost::arg<2>() ));
    }

  private:
    Handler handler_;
    void handler_impl(const streams_boost::system::error_code& error, size_t size)
    {
  ...
  {
  public:
    handshake_handler(Handler handler, streams_boost::asio::io_service& io_service)
      : base_handler<Stream>(io_service)
      , handler_(handler)
    {
      set_func(streams_boost::bind(
  ...
  private:
    Handler handler_;
    void handler_impl(const streams_boost::system::error_code& error, size_t)
    {
  ...
  {
  public:
    shutdown_handler(Handler handler, streams_boost::asio::io_service& io_service)
      : base_handler<Stream>(io_service),
        handler_(handler)
    { 
      set_func(streams_boost::bind(
...
public:
  ...
  explicit openssl_stream_service(streams_boost::asio::io_service& io_service)
    : streams_boost::asio::detail::service_base<openssl_stream_service>(io_service),
      strand_(io_service)
  {
  }
      ...
      stream_base::handshake_type type, streams_boost::system::error_code& ec)
  {
    ...
    catch (streams_boost::system::system_error& e)
    {
      ec = e.code();
      return ec;
    }
      ...
      stream_base::handshake_type type, Handler handler)
  {
    ...
    strand_.post(streams_boost::bind(&openssl_operation<Stream>::start, op));
  }
  ...
  std::size_t write_some(impl_type& impl, Stream& next_layer,
      const Const_Buffers& buffers, streams_boost::system::error_code& ec)
  {
    size_t bytes_transferred = 0;
    try
    {
      std::size_t buffer_size = streams_boost::asio::buffer_size(*buffers.begin());
      if (buffer_size > max_buffer_size)
        buffer_size = max_buffer_size;

      streams_boost::function<int (SSL*)> send_func =
        streams_boost::bind(streams_boost::type<int>(), &::SSL_write, streams_boost::arg<1>(),  
            streams_boost::asio::buffer_cast<const void*>(*buffers.begin()),
  ...
  std::size_t read_some(impl_type& impl, Stream& next_layer,
      const Mutable_Buffers& buffers, streams_boost::system::error_code& ec)
  {
    size_t bytes_transferred = 0;
    try
    {
      ...
      streams_boost::function<int (SSL*)> recv_func =
        streams_boost::bind(streams_boost::type<int>(), &::SSL_read, streams_boost::arg<1>(),
            streams_boost::asio::buffer_cast<void*>(*buffers.begin()),
            static_cast<int>(buffer_size));
...
private:  
  ...
  typedef streams_boost::asio::detail::mutex mutex_type;
...
#endif // STREAMS_BOOST_ASIO_SSL_DETAIL_OPENSSL_STREAM_SERVICE_HPP
