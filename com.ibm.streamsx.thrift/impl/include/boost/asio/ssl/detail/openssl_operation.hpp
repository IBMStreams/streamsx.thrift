...
#ifndef STREAMS_BOOST_ASIO_SSL_DETAIL_OPENSSL_OPERATION_HPP
#define STREAMS_BOOST_ASIO_SSL_DETAIL_OPENSSL_OPERATION_HPP
...
typedef streams_boost::function<int (::SSL*)> ssl_primitive_func; 
typedef streams_boost::function<void (const streams_boost::system::error_code&, int)>
  user_handler_func;
...
public:
  ...
  int start()
  {
    ...
    if (error_code == SSL_ERROR_SSL)
      return handler_(streams_boost::system::error_code(
            error_code, streams_boost::asio::error::get_ssl_category()), rc);
      ...
      && !is_shut_down_sent)
    {
      ...
      {
        return handler_(streams_boost::system::error_code(
              sys_error_code, streams_boost::asio::error::system_category), rc); 
      }
      else
      {
        return handler_(streams_boost::system::error_code(
      ...
      if (recv_buf_.get_data_len() > 0)
      {
        ...
        else if (written < 0)
        {
          if (!BIO_should_retry(ssl_bio_))
          {
            ...
            return handler_(streams_boost::asio::error::no_recovery, 0);
          }
        }
...
private:
  typedef streams_boost::function<int (const streams_boost::system::error_code&, int)>
    int_handler_func;
  typedef streams_boost::function<int (bool, int)> write_func;
  typedef streams_boost::function<int ()> read_func;
  ...
  //
  int sync_user_handler(const streams_boost::system::error_code& error, int rc)
  {
    if (!error)
      return rc;

    throw streams_boost::system::system_error(error);
  }
    
  int async_user_handler(streams_boost::system::error_code error, int rc)
  {
    if (rc < 0)
    {
      if (!error)
        error = streams_boost::asio::error::no_recovery;
      rc = 0;
    }
  ...
  int  do_async_write(bool is_operation_done, int rc) 
  {
    ...
    { 
      ...
      if (len > 0)
      {
        ...
        STREAMS_BOOST_ASSERT(strand_); 
        streams_boost::asio::async_write
        ( 
          socket_, 
          streams_boost::asio::buffer(data_start, len),
          strand_->wrap
          (
            streams_boost::bind
            (
      ...
      else if (!BIO_should_retry(ssl_bio_))
      {
        ...
        handler_(streams_boost::asio::error::no_recovery, 0);
        return 0;
      }
    }
    ...
    if (is_operation_done)
    {
      ...
      handler_(streams_boost::system::error_code(), rc);
      return 0;
    }
  ...
  void async_write_handler(bool is_operation_done, int rc, 
    const streams_boost::system::error_code& error, size_t bytes_sent)
  {
    if (!error)
    {
  ...
  int do_async_read()
  {
    ...
    ( 
      streams_boost::asio::buffer(recv_buf_.get_unused_start(),
  ...
  void async_read_handler(const streams_boost::system::error_code& error,
      size_t bytes_recvd)
  {
    if (!error)
    {
  ...
  int do_sync_write(bool is_operation_done, int rc)
  {
    ...
    { 
      ...
      if (len > 0)
      {
        size_t sent_len = streams_boost::asio::write( 
                  socket_, 
                  streams_boost::asio::buffer(send_buf_.get_unused_start(), len)
                  );
      ...
      else if (!BIO_should_retry(ssl_bio_))
      {
        // Seems like fatal error
        // reading from SSL BIO has failed...
        throw streams_boost::system::system_error(streams_boost::asio::error::no_recovery);
      }
    }
...
#endif // STREAMS_BOOST_ASIO_SSL_DETAIL_OPENSSL_OPERATION_HPP
