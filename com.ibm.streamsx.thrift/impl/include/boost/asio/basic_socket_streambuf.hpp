...
#ifndef STREAMS_BOOST_ASIO_BASIC_SOCKET_STREAMBUF_HPP
#define STREAMS_BOOST_ASIO_BASIC_SOCKET_STREAMBUF_HPP
...
#if !defined(STREAMS_BOOST_ASIO_SOCKET_STREAMBUF_MAX_ARITY)
#define STREAMS_BOOST_ASIO_SOCKET_STREAMBUF_MAX_ARITY 5
#endif // !defined(STREAMS_BOOST_ASIO_SOCKET_STREAMBUF_MAX_ARITY)
...
#define STREAMS_BOOST_ASIO_PRIVATE_CONNECT_DEF( z, n, data ) \
  template <STREAMS_BOOST_PP_ENUM_PARAMS(n, typename T)> \
  basic_socket_streambuf<Protocol, StreamSocketService>* connect( \
      STREAMS_BOOST_PP_ENUM_BINARY_PARAMS(n, T, x)) \
  { \
    init_buffers(); \
    streams_boost::system::error_code ec; \
    this->basic_socket<Protocol, StreamSocketService>::close(ec); \
    typedef typename Protocol::resolver_query resolver_query; \
    resolver_query query(STREAMS_BOOST_PP_ENUM_PARAMS(n, x)); \
    resolve_and_connect(query, ec); \
    return !ec ? this : 0; \
  } \
  /**/
...
class basic_socket_streambuf
  : public std::streambuf,
    private streams_boost::base_from_member<io_service>,
...
public:
  ...
  basic_socket_streambuf()
    : basic_socket<Protocol, StreamSocketService>(
        streams_boost::base_from_member<streams_boost::asio::io_service>::member),
      unbuffered_(false)
  {
    init_buffers();
  }
  ...
  STREAMS_BOOST_PP_REPEAT_FROM_TO(
      1, STREAMS_BOOST_PP_INC(STREAMS_BOOST_ASIO_SOCKET_STREAMBUF_MAX_ARITY),
      STREAMS_BOOST_ASIO_PRIVATE_CONNECT_DEF, _ )
#endif
  ...
  int_type underflow()
  {
    if (gptr() == egptr())
    {
      ...
      std::size_t bytes_transferred = this->service.receive(
          this->implementation,
          streams_boost::asio::buffer(streams_boost::asio::buffer(get_buffer_) + putback_max),
          0, ec);
      if (ec)
  ...
  int_type overflow(int_type c)
  {
    if (unbuffered_)
    {
      ...
      else
      {
        ...
        this->service.send(this->implementation,
            streams_boost::asio::buffer(&ch, sizeof(char_type)), 0, ec);
        if (ec)
          return traits_type::eof();
        return c;
      }
    ...
    else
    {
      ...
      streams_boost::asio::const_buffer buffer =
        streams_boost::asio::buffer(pbase(), pptr() - pbase());
      while (streams_boost::asio::buffer_size(buffer) > 0)
      {
        streams_boost::system::error_code ec;
        std::size_t bytes_transferred = this->service.send(
            this->implementation, streams_boost::asio::buffer(buffer),
            0, ec);
        if (ec)
          return traits_type::eof();
...
private:
      ...
      streams_boost::system::error_code& ec)
  {
    ...
    {
      iterator_type end;
      ec = streams_boost::asio::error::host_not_found;
      while (ec && i != end)
      {
  ...
  streams_boost::array<char, buffer_size> get_buffer_;
  streams_boost::array<char, buffer_size> put_buffer_;
  bool unbuffered_;
};
...
#undef STREAMS_BOOST_ASIO_PRIVATE_CONNECT_DEF

#endif // !defined(STREAMS_BOOST_NO_IOSTREAM)

#include <streams_boost/asio/detail/pop_options.hpp>

#endif // STREAMS_BOOST_ASIO_BASIC_SOCKET_STREAMBUF_HPP
