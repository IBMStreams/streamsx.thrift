...
#ifndef STREAMS_BOOST_ASIO_BUFFERED_READ_STREAM_HPP
#define STREAMS_BOOST_ASIO_BUFFERED_READ_STREAM_HPP
...
#include <streams_boost/asio/detail/buffer_resize_guard.hpp>
#include <streams_boost/asio/detail/buffered_stream_storage.hpp>
...
public:
  ...
  typedef typename streams_boost::remove_reference<Stream>::type next_layer_type;
...
#else
  STREAMS_BOOST_STATIC_CONSTANT(std::size_t, default_buffer_size = 1024);
#endif
  ...
  std::size_t fill(streams_boost::system::error_code& ec)
  {
    detail::buffer_resize_guard<detail::buffered_stream_storage>
      resize_guard(storage_);
    std::size_t previous_size = storage_.size();
  ...
  {
  public:
    fill_handler(streams_boost::asio::io_service& io_service,
  ...
  {
  public:
    read_some_handler(streams_boost::asio::io_service& io_service,
        detail::buffered_stream_storage& storage,
    ...
    void operator()(const streams_boost::system::error_code& ec, std::size_t)
    {
      ...
      ReadHandler handler)
  {
    ...
    if (total_buffer_size == 0)
    {
      get_io_service().post(detail::bind_handler(
            handler, streams_boost::system::error_code(), 0));
    }
    ...
    else
    {
      ...
      get_io_service().post(detail::bind_handler(
            handler, streams_boost::system::error_code(), length));
    }
  }
  ...
  /// Determine the amount of data that may be read without blocking.
  std::size_t in_avail(streams_boost::system::error_code& ec)
  {
    ec = streams_boost::system::error_code();
    return storage_.size();
  }
...
#endif // STREAMS_BOOST_ASIO_BUFFERED_READ_STREAM_HPP
