...
#ifndef STREAMS_BOOST_ASIO_DETAIL_REACTIVE_SOCKET_SERVICE_HPP
#define STREAMS_BOOST_ASIO_DETAIL_REACTIVE_SOCKET_SERVICE_HPP
...
#include <streams_boost/asio/detail/handler_base_from_member.hpp>
...
public:
  ...
  // Constructor.
  reactive_socket_service(streams_boost::asio::io_service& io_service)
    : streams_boost::asio::detail::service_base<
        reactive_socket_service<Protocol, Reactor> >(io_service),
      reactor_(streams_boost::asio::use_service<Reactor>(io_service))
  {
    reactor_.init_task();
  }
      ...
      const protocol_type& protocol, streams_boost::system::error_code& ec)
  {
    ...
    {
      ec = streams_boost::system::error_code(err,
...
# if defined(ENOTTY)
    if (ec.value() == ENOTTY)
      ec = streams_boost::asio::error::not_socket;
...
|| defined(__NetBSD__) || defined(__FreeBSD__) || defined(__OpenBSD__)
      ...
      socket_base::message_flags flags, streams_boost::system::error_code& ec)
  {
    ...
    for (; iter != end && i < max_buffers; ++iter, ++i)
    {
      ...
      socket_ops::init_buf(bufs[i],
          streams_boost::asio::buffer_cast<const void*>(buffer),
          streams_boost::asio::buffer_size(buffer));
    ...
    for (;;)
    {
      ...
      if ((impl.flags_ & implementation_type::user_set_non_blocking)
          || (ec != streams_boost::asio::error::would_block
            && ec != streams_boost::asio::error::try_again))
        return 0;
  ...
  {
  public:
    send_operation(socket_type socket, streams_boost::asio::io_service& io_service,
        ...
        std::size_t& bytes_transferred)
    {
      ...
      for (;;)
      {
        ...
        if (ec == streams_boost::asio::error::interrupted)
          continue;

        // Check if we need to run the operation again.
        if (ec == streams_boost::asio::error::would_block
            || ec == streams_boost::asio::error::try_again)
          return false;
      ...
      socket_base::message_flags flags, Handler handler)
  {
    ...
    else
    {
      if (impl.protocol_.type() == SOCK_STREAM)
      {
        ...
        {
          this->get_io_service().post(bind_handler(handler,
                streams_boost::system::error_code(), 0));
          return;
        }
      }
  ...
  {
  public:
    send_to_operation(socket_type socket, streams_boost::asio::io_service& io_service,
      ...
      socket_base::message_flags flags, streams_boost::system::error_code& ec)
  {
    ...
    for (; iter != end && i < max_buffers; ++iter, ++i)
    {
      ...
      socket_ops::init_buf(bufs[i],
          streams_boost::asio::buffer_cast<void*>(buffer),
          streams_boost::asio::buffer_size(buffer));
      total_buffer_size += streams_boost::asio::buffer_size(buffer);
    }
  ...
  streams_boost::system::error_code accept(implementation_type& impl,
      Socket& peer, endpoint_type* peer_endpoint, streams_boost::system::error_code& ec)
  {
    if (!is_open(impl))
    {
      ec = streams_boost::asio::error::bad_descriptor;
    ...
    for (;;)
    {
      ...
      else if (ec == streams_boost::asio::error::connection_aborted)
      {
        if (impl.flags_ & implementation_type::enable_connection_aborted)
          return ec;
        // Fall through to retry operation.
  ...
  {
  public:
    accept_operation(socket_type socket, streams_boost::asio::io_service& io_service,
    ...
    bool perform(streams_boost::system::error_code& ec, std::size_t&)
    {
      // Check whether the operation was successful.
      if (ec)
        return true;
      ...
      const endpoint_type& peer_endpoint, streams_boost::system::error_code& ec)
  {
    ...
    if (ec != streams_boost::asio::error::in_progress
        && ec != streams_boost::asio::error::would_block)
    {
      ...
      const endpoint_type& peer_endpoint, Handler handler)
  {
          ...
          peer_endpoint.size(), ec) == 0)
    {
      ...
      // handler immediately.
      this->get_io_service().post(bind_handler(handler,
            streams_boost::system::error_code()));
    }
...
#endif // STREAMS_BOOST_ASIO_DETAIL_REACTIVE_SOCKET_SERVICE_HPP
