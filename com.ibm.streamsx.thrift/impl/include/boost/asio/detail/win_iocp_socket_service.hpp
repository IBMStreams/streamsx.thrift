...
#ifndef STREAMS_BOOST_ASIO_DETAIL_WIN_IOCP_SOCKET_SERVICE_HPP
#define STREAMS_BOOST_ASIO_DETAIL_WIN_IOCP_SOCKET_SERVICE_HPP
...
#include <streams_boost/asio/detail/win_iocp_io_service_fwd.hpp>
...
#include <streams_boost/asio/detail/socket_holder.hpp>
...
class win_iocp_socket_service
  : public streams_boost::asio::detail::service_base<win_iocp_socket_service<Protocol> >
{
public:
  // The protocol type.
  ...
  typedef streams_boost::shared_ptr<void> shared_cancel_token_type;
  typedef streams_boost::weak_ptr<void> weak_cancel_token_type;
...
#if defined(STREAMS_BOOST_ASIO_ENABLE_CANCELIO)
    // The ID of the thread from which it is safe to cancel asynchronous
    // operations. 0 means no asynchronous operations have been started yet.
    // ~0 means asynchronous operations have been started from more than one
    // thread, and cancellation is not supported for the socket.
    DWORD safe_cancellation_thread_id_;
#endif // defined(STREAMS_BOOST_ASIO_ENABLE_CANCELIO)
  ...
  // Constructor.
  win_iocp_socket_service(streams_boost::asio::io_service& io_service)
    : streams_boost::asio::detail::service_base<
        win_iocp_socket_service<Protocol> >(io_service),
      iocp_service_(streams_boost::asio::use_service<win_iocp_io_service>(io_service)),
      reactor_(0),
      mutex_(),
      impl_list_(0)
  {
  ...
  void shutdown_service()
  {
    ...
    while (impl)
    {
      streams_boost::system::error_code ignored_ec;
      ...
      const protocol_type& protocol, streams_boost::system::error_code& ec)
  {
    if (is_open(impl))
    {
      ec = streams_boost::asio::error::already_open;
      return ec;
    }
      ...
      streams_boost::system::error_code& ec)
  {
    if (!is_open(impl))
    {
      ec = streams_boost::asio::error::bad_descriptor;
      return ec;
    }
          ...
          ::GetModuleHandleA("KERNEL32"), "CancelIoEx"))
    {
      ...
      if (!cancel_io_ex(sock_as_handle, 0))
      {
        ...
        {
          ec = streams_boost::system::error_code(last_error,
...
#else // defined(STREAMS_BOOST_ASIO_ENABLE_CANCELIO)
    else
    {
      ...
      streams_boost::system::error_code& ec) const
  {
    ...
    streams_boost::asio::detail::ioctl_arg_type value = 0;
  ...
  streams_boost::system::error_code listen(implementation_type& impl, int backlog,
      streams_boost::system::error_code& ec)
  {
    if (!is_open(impl))
    {
  ...
  streams_boost::system::error_code set_option(implementation_type& impl,
      const Option& option, streams_boost::system::error_code& ec)
  {
    if (!is_open(impl))
    {
      ec = streams_boost::asio::error::bad_descriptor;
  ...
  streams_boost::system::error_code get_option(const implementation_type& impl,
      Option& option, streams_boost::system::error_code& ec) const
  {
    if (!is_open(impl))
    {
      ec = streams_boost::asio::error::bad_descriptor;
  ...
  streams_boost::system::error_code io_control(implementation_type& impl,
      IO_Control_Command& command, streams_boost::system::error_code& ec)
  {
    if (!is_open(impl))
    {
      ec = streams_boost::asio::error::bad_descriptor;
      ...
      streams_boost::system::error_code& ec) const
  {
    ...
    if (impl.socket_.have_remote_endpoint())
    {
      ...
      {
        ec = streams_boost::asio::error::not_connected;
        return endpoint_type();
      }
      ...
      socket_base::message_flags flags, streams_boost::system::error_code& ec)
  {
    ...
    for (; iter != end && i < max_buffers; ++iter, ++i)
    {
      ...
      bufs[i].len = static_cast<u_long>(streams_boost::asio::buffer_size(buffer));
      bufs[i].buf = const_cast<char*>(
          streams_boost::asio::buffer_cast<const char*>(buffer));
      total_buffer_size += streams_boost::asio::buffer_size(buffer);
    }
  ...
  size_t send(implementation_type& impl, const null_buffers&,
      socket_base::message_flags, streams_boost::system::error_code& ec)
  {
    if (!is_open(impl))
    {
      ec = streams_boost::asio::error::bad_descriptor;
...
#endif // defined(STREAMS_BOOST_ASIO_ENABLE_BUFFER_DEBUGGING)

      // Map non-portable errors to their portable counterparts.
      streams_boost::system::error_code ec(last_error,
          streams_boost::asio::error::get_system_category());
      if (ec.value() == ERROR_NETNAME_DELETED)
      {
        if (handler_op->cancel_token_.expired())
          ec = streams_boost::asio::error::operation_aborted;
        else
          ec = streams_boost::asio::error::connection_reset;
      }
      else if (ec.value() == ERROR_PORT_UNREACHABLE)
      {
        ec = streams_boost::asio::error::connection_refused;
      }
  ...
  {
  public:
    null_buffers_operation(streams_boost::asio::io_service& io_service, Handler handler)
      : work_(io_service),
        handler_(handler)
    {
    }

    bool perform(streams_boost::system::error_code&,
      ...
      socket_base::message_flags, Handler handler)
  {
    if (!is_open(impl))
    {
      this->get_io_service().post(bind_handler(handler,
            streams_boost::asio::error::bad_descriptor, 0));
    }
    else
    {
      ...
      if (!reactor)
      {
        reactor = &(streams_boost::asio::use_service<reactor_type>(
              this->get_io_service()));
      ...
      socket_base::message_flags flags, streams_boost::system::error_code& ec)
  {
    ...
    for (; iter != end && i < max_buffers; ++iter, ++i)
    {
      streams_boost::asio::mutable_buffer buffer(*iter);
      bufs[i].len = static_cast<u_long>(streams_boost::asio::buffer_size(buffer));
      bufs[i].buf = streams_boost::asio::buffer_cast<char*>(buffer);
      total_buffer_size += streams_boost::asio::buffer_size(buffer);
    }
    ...
    {
      ec = streams_boost::asio::error::eof;
      return 0;
    }
  ...
  private:
        ...
        DWORD last_error, size_t bytes_transferred)
    {
      ...
      {
        streams_boost::asio::mutable_buffer buffer(*iter);
        streams_boost::asio::buffer_cast<char*>(buffer);
        ++iter;
      }
#endif // defined(STREAMS_BOOST_ASIO_ENABLE_BUFFER_DEBUGGING)
      ...
      // Check for connection closed.
          ...
          && !streams_boost::is_same<MutableBufferSequence, null_buffers>::value)
      {
        ec = streams_boost::asio::error::eof;
      }
  ...
  streams_boost::system::error_code accept(implementation_type& impl, Socket& peer,
      endpoint_type* peer_endpoint, streams_boost::system::error_code& ec)
  {
    if (!is_open(impl))
    {
      ec = streams_boost::asio::error::bad_descriptor;
    ...
    for (;;)
    {
      ...
      if (ec)
      {
        if (ec == streams_boost::asio::error::connection_aborted
            && !(impl.flags_ & implementation_type::enable_connection_aborted))
        {
      ...
      endpoint_type* peer_endpoint, Handler handler)
  {
    ...
    {
      this->get_io_service().post(bind_handler(handler,
            streams_boost::asio::error::bad_descriptor));
      return;
    }
    ...
    {
      this->get_io_service().post(bind_handler(handler,
            streams_boost::asio::error::already_open));
      return;
    }
    ...
    if (!result && last_error != WSA_IO_PENDING)
    {
      ...
      {
        streams_boost::asio::io_service::work work(this->get_io_service());
        ptr.reset();
        streams_boost::system::error_code ec(last_error,
            streams_boost::asio::error::get_system_category());
  ...
  public:
    ...
    bool perform(streams_boost::system::error_code& ec,
        std::size_t& bytes_transferred)
    {
      bytes_transferred = 0;
      ...
      if (connect_error)
      {
        ec = streams_boost::system::error_code(connect_error,
            streams_boost::asio::error::get_system_category());
        return true;
      }
    ...
    void complete(const streams_boost::system::error_code& ec, std::size_t)
    {
      ...
      const endpoint_type& peer_endpoint, Handler handler)
  {
    ...
    else if (ec == streams_boost::asio::error::in_progress
        || ec == streams_boost::asio::error::would_block)
    {
      // The connection is happening in the background, and we need to wait
      // until the socket becomes writeable.
      streams_boost::shared_ptr<bool> completed(new bool(false));
...
#endif // STREAMS_BOOST_ASIO_DETAIL_WIN_IOCP_SOCKET_SERVICE_HPP
