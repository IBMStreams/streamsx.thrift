...
#ifndef STREAMS_BOOST_ASIO_DETAIL_TIMER_QUEUE_HPP
#define STREAMS_BOOST_ASIO_DETAIL_TIMER_QUEUE_HPP
...
#include <streams_boost/asio/detail/timer_queue_base.hpp>
...
public:
  ...
  virtual streams_boost::posix_time::time_duration wait_duration() const
  {
    if (heap_.empty())
      return streams_boost::posix_time::pos_infin;
  ...
  virtual void dispatch_timers()
  {
    ...
    while (!heap_.empty() && !Time_Traits::less_than(now, heap_[0]->time_))
    {
      ...
      remove_timer(t);
      t->result_ = streams_boost::system::error_code();
      t->prev_ = 0;
  ...
  virtual void dispatch_cancellations()
  {
    while (cancelled_timers_)
    {
      ...
      this_timer->result_ = streams_boost::asio::error::operation_aborted;
...
private:
  ...
  protected:
    typedef void (*complete_func_type)(timer_base*,
        const streams_boost::system::error_code&);
    ...
    timer_base(complete_func_type complete_func, destroy_func_type destroy_func,
      ...
      : complete_func_(complete_func),
        ...
        prev_(0),
        heap_index_(
            std::numeric_limits<size_t>::max STREAMS_BOOST_PREVENT_MACRO_SUBSTITUTION())
    {
    }
...
#endif // STREAMS_BOOST_ASIO_DETAIL_TIMER_QUEUE_HPP
