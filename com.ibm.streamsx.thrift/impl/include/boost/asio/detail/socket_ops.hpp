...
#ifndef STREAMS_BOOST_ASIO_DETAIL_SOCKET_OPS_HPP
#define STREAMS_BOOST_ASIO_DETAIL_SOCKET_OPS_HPP
...
{
#if defined(STREAMS_BOOST_WINDOWS) || defined(__CYGWIN__)
  ec = streams_boost::system::error_code(WSAGetLastError(),
...
inline socket_type accept(socket_type s, socket_addr_type* addr,
    std::size_t* addrlen, streams_boost::system::error_code& ec)
{
  clear_error(ec);
...
inline int close(socket_type s, streams_boost::system::error_code& ec)
{
  clear_error(ec);
#if defined(STREAMS_BOOST_WINDOWS) || defined(__CYGWIN__)
...
inline int shutdown(socket_type s, int what, streams_boost::system::error_code& ec)
{
  clear_error(ec);
...
inline int socketpair(int af, int type, int protocol,
    socket_type sv[2], streams_boost::system::error_code& ec)
{
#if defined(STREAMS_BOOST_WINDOWS) || defined(__CYGWIN__)
  (void)(af);
  (void)(type);
  (void)(protocol);
  (void)(sv);
  ec = streams_boost::asio::error::operation_not_supported;
  return -1;
#else
  clear_error(ec);
...
inline int listen(socket_type s, int backlog, streams_boost::system::error_code& ec)
{
  clear_error(ec);
...
inline int setsockopt(socket_type s, int level, int optname,
    const void* optval, std::size_t optlen, streams_boost::system::error_code& ec)
{
  ...
  ec = streams_boost::asio::error::fault;
  return -1;
...
inline int getsockopt(socket_type s, int level, int optname, void* optval,
    size_t* optlen, streams_boost::system::error_code& ec)
{
  if (level == custom_socket_option_level && optname == always_fail_option)
  {
    ec = streams_boost::asio::error::invalid_argument;
...
inline int select(int nfds, fd_set* readfds, fd_set* writefds,
    fd_set* exceptfds, timeval* timeout, streams_boost::system::error_code& ec)
{
  clear_error(ec);
#if defined(STREAMS_BOOST_WINDOWS) || defined(__CYGWIN__)
  ...
  // system clock (as wrapped by streams_boost::posix_time::microsec_clock) typically
...
inline int poll_read(socket_type s, streams_boost::system::error_code& ec)
{
#if defined(STREAMS_BOOST_WINDOWS) || defined(__CYGWIN__)
  FD_SET fds;
  FD_ZERO(&fds);
...
inline int poll_write(socket_type s, streams_boost::system::error_code& ec)
{
#if defined(STREAMS_BOOST_WINDOWS) || defined(__CYGWIN__)
  FD_SET fds;
  FD_ZERO(&fds);
...
inline int poll_connect(socket_type s, streams_boost::system::error_code& ec)
{
#if defined(STREAMS_BOOST_WINDOWS) || defined(__CYGWIN__)
  FD_SET write_fds;
  FD_ZERO(&write_fds);
...
inline const char* inet_ntop(int af, const void* src, char* dest, size_t length,
    unsigned long scope_id, streams_boost::system::error_code& ec)
{
  clear_error(ec);
#if defined(STREAMS_BOOST_WINDOWS) || defined(__CYGWIN__)
  ...
  {
    ec = streams_boost::asio::error::address_family_not_supported;
    return 0;
  }
...
#if defined(STREAMS_BOOST_NO_ANSI_APIS)
...
inline int gethostname(char* name, int namelen, streams_boost::system::error_code& ec)
{
  clear_error(ec);
...
inline streams_boost::system::error_code translate_netdb_error(int error)
{
  switch (error)
  {
  case 0:
  ...
  case HOST_NOT_FOUND:
    return streams_boost::asio::error::host_not_found;
  case TRY_AGAIN:
    return streams_boost::asio::error::host_not_found_try_again;
  case NO_RECOVERY:
    return streams_boost::asio::error::no_recovery;
  case NO_DATA:
    return streams_boost::asio::error::no_data;
  default:
    STREAMS_BOOST_ASSERT(false);
    return streams_boost::asio::error::invalid_argument;
  }
}

inline hostent* gethostbyaddr(const char* addr, int length, int af,
    hostent* result, char* buffer, int buflength, streams_boost::system::error_code& ec)
{
  clear_error(ec);
#if defined(STREAMS_BOOST_WINDOWS) || defined(__CYGWIN__)
  (void)(buffer);
...
inline hostent* gethostbyname(const char* name, int af, struct hostent* result,
    char* buffer, int buflength, int ai_flags, streams_boost::system::error_code& ec)
{
  clear_error(ec);
#if defined(STREAMS_BOOST_WINDOWS) || defined(__CYGWIN__)
  (void)(buffer);
...
{
  using namespace std;
#if STREAMS_BOOST_WORKAROUND(STREAMS_BOOST_MSVC, >= 1400) && !defined(UNDER_CE)
    ...
    const addrinfo_type* hintsp, addrinfo_type** result)
{
  ...
  for (gai_search* sptr = search; sptr < search + search_count; ++sptr)
  {
    ...
    if (hptr == 0)
    {
      ...
      freeaddrinfo_emulation(aihead);
      gai_free(canon);
      if (ec == streams_boost::asio::error::host_not_found)
        return EAI_NONAME;
      if (ec == streams_boost::asio::error::host_not_found_try_again)
        return EAI_AGAIN;
      if (ec == streams_boost::asio::error::no_recovery)
        return EAI_FAIL;
      if (ec == streams_boost::asio::error::no_data)
        return EAI_NONAME;
      return EAI_NONAME;
    }
...
inline streams_boost::system::error_code getnameinfo_emulation(
    ...
    streams_boost::system::error_code& ec)
{
  ...
  case AF_INET:
    ...
    {
      return ec = streams_boost::asio::error::invalid_argument;
    }
  ...
  case AF_INET6:
    ...
    break;
  default:
    return ec = streams_boost::asio::error::address_family_not_supported;
  }
  ...
  if (host && hostlen > 0)
  {
    ...
    else
    {
      ...
      else
      {
        ...
        {
          return ec = streams_boost::asio::error::host_not_found;
        }
  ...
  if (serv && servlen > 0)
  {
    if (flags & NI_NUMERICSERV)
    {
      if (servlen < 6)
      {
        return ec = streams_boost::asio::error::no_buffer_space;
      }
#if STREAMS_BOOST_WORKAROUND(STREAMS_BOOST_MSVC, >= 1400) && !defined(UNDER_CE)
...
#endif // defined(STREAMS_BOOST_HAS_THREADS) && defined(STREAMS_BOOST_HAS_PTHREADS)
...
inline streams_boost::system::error_code translate_addrinfo_error(int error)
{
  switch (error)
  {
  case 0:
  ...
  case EAI_FAIL:
    return streams_boost::asio::error::no_recovery;
  case EAI_FAMILY:
    return streams_boost::asio::error::address_family_not_supported;
  case EAI_MEMORY:
    return streams_boost::asio::error::no_memory;
  case EAI_NONAME:
  ...
  case EAI_NODATA:
    ...
    return streams_boost::asio::error::host_not_found;
  case EAI_SERVICE:
    return streams_boost::asio::error::service_not_found;
  case EAI_SOCKTYPE:
    return streams_boost::asio::error::socket_type_not_supported;
  default: // Possibly the non-portable EAI_SYSTEM.
#if defined(STREAMS_BOOST_WINDOWS) || defined(__CYGWIN__)
    return streams_boost::system::error_code(
        WSAGetLastError(), streams_boost::asio::error::get_system_category());
#else
    return streams_boost::system::error_code(
        errno, streams_boost::asio::error::get_system_category());
#endif
  }
}

inline streams_boost::system::error_code getaddrinfo(const char* host,
...
inline streams_boost::system::error_code getnameinfo(const socket_addr_type* addr,
    std::size_t addrlen, char* host, std::size_t hostlen,
    char* serv, std::size_t servlen, int flags, streams_boost::system::error_code& ec)
{
#if defined(STREAMS_BOOST_WINDOWS) || defined(__CYGWIN__)
# if defined(_WIN32_WINNT) && (_WIN32_WINNT >= 0x0501) || defined(UNDER_CE)
  // Building for Windows XP, Windows Server 2003, or later.
...
#endif // STREAMS_BOOST_ASIO_DETAIL_SOCKET_OPS_HPP
