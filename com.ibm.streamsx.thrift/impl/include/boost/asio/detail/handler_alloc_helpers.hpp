...
#ifndef STREAMS_BOOST_ASIO_DETAIL_HANDLER_ALLOC_HELPERS_HPP
#define STREAMS_BOOST_ASIO_DETAIL_HANDLER_ALLOC_HELPERS_HPP
...
// streams_boost_asio_handler_alloc_helpers namespace is defined here for that purpose.
namespace streams_boost_asio_handler_alloc_helpers {
...
inline void* allocate(std::size_t s, Handler& h)
{
  ...
  return asio_handler_allocate(s, streams_boost::addressof(h));
#endif
}
...
#else
  using namespace streams_boost::asio;
  asio_handler_deallocate(p, s, streams_boost::addressof(h));
#endif
}
...
struct handler_alloc_traits
{
  ...
  STREAMS_BOOST_STATIC_CONSTANT(std::size_t, value_size = sizeof(Object));
};
...
public:
  ...
  STREAMS_BOOST_STATIC_CONSTANT(std::size_t, value_size = Alloc_Traits::value_size);

  // Constructor allocates the memory.
  raw_handler_ptr(handler_type& handler)
    : handler_(handler),
      pointer_(static_cast<pointer_type>(
            streams_boost_asio_handler_alloc_helpers::allocate(value_size, handler_)))
  {
  }
...
#endif // STREAMS_BOOST_ASIO_DETAIL_HANDLER_ALLOC_HELPERS_HPP
