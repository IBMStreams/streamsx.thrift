...
#ifndef STREAMS_BOOST_ASIO_DETAIL_KQUEUE_REACTOR_HPP
#define STREAMS_BOOST_ASIO_DETAIL_KQUEUE_REACTOR_HPP
...
#if defined(STREAMS_BOOST_ASIO_HAS_KQUEUE)
...
class kqueue_reactor
  : public streams_boost::asio::detail::service_base<kqueue_reactor<Own_Thread> >
{
public:
  // Per-descriptor data.
  ...
  // Constructor.
  kqueue_reactor(streams_boost::asio::io_service& io_service)
      ...
      else if (events[i].filter == EVFILT_READ)
      {
        ...
        if (events[i].flags & EV_ERROR)
        {
          streams_boost::system::error_code error(
              events[i].data, streams_boost::asio::error::get_system_category());
        ...
        if (::kevent(kqueue_fd_, &event, 1, 0, 0, 0) == -1)
        {
          streams_boost::system::error_code error(errno,
              streams_boost::asio::error::get_system_category());
          except_op_queue_.perform_all_operations(descriptor, error);
          read_op_queue_.perform_all_operations(descriptor, error);
        }
...
#endif // defined(STREAMS_BOOST_ASIO_HAS_KQUEUE)

#include <streams_boost/asio/detail/pop_options.hpp>

#endif // STREAMS_BOOST_ASIO_DETAIL_KQUEUE_REACTOR_HPP
