...
#ifndef STREAMS_BOOST_ASIO_DETAIL_WIN_IOCP_HANDLE_SERVICE_HPP
#define STREAMS_BOOST_ASIO_DETAIL_WIN_IOCP_HANDLE_SERVICE_HPP
...
class win_iocp_handle_service
  : public streams_boost::asio::detail::service_base<win_iocp_handle_service>
{
public:
  ...
  win_iocp_handle_service(streams_boost::asio::io_service& io_service)
    : streams_boost::asio::detail::service_base<win_iocp_handle_service>(io_service),
  ...
  {
  public:
    explicit overlapped_wrapper(streams_boost::system::error_code& ec)
    {
      Internal = 0;
      InternalHigh = 0;
      Offset = 0;
  ...
  // written.
  template <typename ConstBufferSequence>
  size_t write_some_at(implementation_type& impl, streams_boost::uint64_t offset,
      const ConstBufferSequence& buffers, streams_boost::system::error_code& ec)
  {
    if (!is_open(impl))
    {
    ...
    streams_boost::asio::const_buffer buffer;
    typename ConstBufferSequence::const_iterator iter = buffers.begin();
    typename ConstBufferSequence::const_iterator end = buffers.end();
    for (DWORD i = 0; iter != end; ++iter, ++i)
    {
      buffer = streams_boost::asio::const_buffer(*iter);
      if (streams_boost::asio::buffer_size(buffer) != 0)
        break;
    }
    ...
    BOOL ok = ::WriteFile(impl.handle_,
        streams_boost::asio::buffer_cast<LPCVOID>(buffer),
        static_cast<DWORD>(streams_boost::asio::buffer_size(buffer)), 0, &overlapped);
    if (!ok) 
    {
      DWORD last_error = ::GetLastError();
  ...
  template <typename ConstBufferSequence, typename Handler>
  void async_write_some_at(implementation_type& impl, streams_boost::uint64_t offset,
      const ConstBufferSequence& buffers, Handler handler)
  {
    ...
    BOOL ok = ::WriteFile(impl.handle_,
        streams_boost::asio::buffer_cast<LPCVOID>(buffer),
        static_cast<DWORD>(streams_boost::asio::buffer_size(buffer)),
  ...
  template <typename MutableBufferSequence>
  size_t read_some_at(implementation_type& impl, streams_boost::uint64_t offset,
      const MutableBufferSequence& buffers, streams_boost::system::error_code& ec)
  {
    if (!is_open(impl))
    {
    ...
    // Find first buffer of non-zero length.
    streams_boost::asio::mutable_buffer buffer;
    typename MutableBufferSequence::const_iterator iter = buffers.begin();
    typename MutableBufferSequence::const_iterator end = buffers.end();
    for (DWORD i = 0; iter != end; ++iter, ++i)
    {
      buffer = streams_boost::asio::mutable_buffer(*iter);
      if (streams_boost::asio::buffer_size(buffer) != 0)
        break;
    }
    ...
    BOOL ok = ::ReadFile(impl.handle_,
        streams_boost::asio::buffer_cast<LPVOID>(buffer),
        static_cast<DWORD>(streams_boost::asio::buffer_size(buffer)), 0, &overlapped);
    if (!ok) 
    {
      DWORD last_error = ::GetLastError();
  ...
  private:
    ...
    static void destroy_impl(operation* op)
    {
      ...
      typedef read_operation<MutableBufferSequence, Handler> op_type;
      op_type* handler_op(static_cast<op_type*>(op));
      typedef streams_boost::asio::detail::handler_alloc_traits<
        Handler, op_type> alloc_traits;
      streams_boost::asio::detail::handler_ptr<alloc_traits> ptr(
  ...
  // operation.
  template <typename MutableBufferSequence, typename Handler>
  void async_read_some_at(implementation_type& impl, streams_boost::uint64_t offset,
      const MutableBufferSequence& buffers, Handler handler)
  {
    // Update the ID of the thread from which cancellation is safe.
    if (impl.safe_cancellation_thread_id_ == 0)
...
private:
  ...
  size_t write_some(implementation_type& impl,
      const null_buffers& buffers, streams_boost::system::error_code& ec);
  size_t write_some_at(implementation_type& impl, streams_boost::uint64_t offset,
      const null_buffers& buffers, streams_boost::system::error_code& ec);
...
#endif // STREAMS_BOOST_ASIO_DETAIL_WIN_IOCP_HANDLE_SERVICE_HPP
