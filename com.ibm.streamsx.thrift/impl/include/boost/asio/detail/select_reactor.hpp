...
#ifndef STREAMS_BOOST_ASIO_DETAIL_SELECT_REACTOR_HPP
#define STREAMS_BOOST_ASIO_DETAIL_SELECT_REACTOR_HPP
...
#include <streams_boost/asio/detail/fd_set_adapter.hpp>
#include <streams_boost/asio/detail/mutex.hpp>
#include <streams_boost/asio/detail/noncopyable.hpp>
#include <streams_boost/asio/detail/reactor_op_queue.hpp>
#include <streams_boost/asio/detail/select_interrupter.hpp>
#include <streams_boost/asio/detail/select_reactor_fwd.hpp>
#include <streams_boost/asio/detail/service_base.hpp>
#include <streams_boost/asio/detail/signal_blocker.hpp>
...
#include <streams_boost/asio/detail/timer_queue.hpp>
...
class select_reactor
  : public streams_boost::asio::detail::service_base<select_reactor<Own_Thread> >
{
public:
  // Per-descriptor data.
  ...
  // Constructor.
  select_reactor(streams_boost::asio::io_service& io_service)
      ...
      shutdown_(false)
  {
    if (Own_Thread)
    {
      streams_boost::asio::detail::signal_blocker sb;
      thread_ = new streams_boost::asio::detail::thread(
  ...
  public:
    connect_handler_wrapper(socket_type descriptor,
        streams_boost::shared_ptr<bool> completed,
  ...
  private:
    ...
    streams_boost::shared_ptr<bool> completed_;
...
private:
  ...
  void run(bool block)
  {
    ...
    if (retval > 0)
    {
      ...
      except_op_queue_.perform_operations_for_descriptors(
          except_fds, streams_boost::system::error_code());
      read_op_queue_.perform_operations_for_descriptors(
          read_fds, streams_boost::system::error_code());
      write_op_queue_.perform_operations_for_descriptors(
          write_fds, streams_boost::system::error_code());
      except_op_queue_.perform_cancellations();
      read_op_queue_.perform_cancellations();
      write_op_queue_.perform_cancellations();
    }
  ...
  timeval* get_timeout(timeval& tv)
  {
    ...
    streams_boost::posix_time::time_duration minimum_wait_duration
      = streams_boost::posix_time::minutes(5);

    for (std::size_t i = 0; i < timer_queues_.size(); ++i)
    {
      streams_boost::posix_time::time_duration wait_duration
        = timer_queues_[i]->wait_duration();
      if (wait_duration < minimum_wait_duration)
        minimum_wait_duration = wait_duration;
    }

    if (minimum_wait_duration > streams_boost::posix_time::time_duration())
    {
  ...
  void complete_operations_and_timers(
      streams_boost::asio::detail::mutex::scoped_lock& lock)
  {
  ...
  streams_boost::asio::detail::thread* thread_;
...
#endif // STREAMS_BOOST_ASIO_DETAIL_SELECT_REACTOR_HPP
