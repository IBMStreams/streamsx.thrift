...
#ifndef STREAMS_BOOST_ASIO_READ_UNTIL_IPP
#define STREAMS_BOOST_ASIO_READ_UNTIL_IPP
    ...
    streams_boost::system::error_code& ec)
{
  ...
  for (;;)
  {
    ...
    typedef typename streams_boost::asio::basic_streambuf<
      Allocator>::const_buffers_type const_buffers_type;
    typedef streams_boost::asio::buffers_iterator<const_buffers_type> iterator;
    ...
    streams_boost::system::error_code& ec)
{
  ...
  for (;;)
  {
    ...
    // Look for a match.
    std::pair<iterator, bool> result = streams_boost::asio::detail::partial_search(
    ...
    streams_boost::system::error_code& ec)
{
  ...
  for (;;)
  {
    ...
    // Look for a match.
    streams_boost::match_results<iterator> match_results;
    if (streams_boost::regex_search(start, end, match_results, expr,
          streams_boost::match_default | streams_boost::match_partial))
    {
...
std::size_t read_until(SyncReadStream& s,
    ...
    typename streams_boost::enable_if<is_match_condition<MatchCondition> >::type*)
{
  std::size_t next_search_start = 0;
  for (;;)
  {
...
namespace detail
{
  ...
  public:
    read_until_delim_handler(AsyncReadStream& stream,
        streams_boost::asio::basic_streambuf<Allocator>& streambuf, char delim,
        ...
        std::size_t bytes_transferred)
    {
      ...
      {
        std::size_t bytes = 0;
        streams_boost::system::error_code ec(error::not_found);
        handler_(ec, bytes);
        return;
      }
...
void async_read_until(AsyncReadStream& s,
    streams_boost::asio::basic_streambuf<Allocator>& b, char delim, ReadHandler handler)
{
  // Determine the range of the data to be searched.
  typedef typename streams_boost::asio::basic_streambuf<
    Allocator>::const_buffers_type const_buffers_type;
...
namespace detail
{
  ...
  public:
        ...
        std::size_t bytes_transferred)
    {
      ...
      {
        std::size_t bytes = 0;
        streams_boost::system::error_code ec2(error::not_found);
        handler_(ec2, bytes);
        return;
      }
...
namespace detail
{
  ...
  public:
    read_until_expr_handler(AsyncReadStream& stream,
        streams_boost::asio::basic_streambuf<Allocator>& streambuf,
        const streams_boost::regex& expr, std::size_t next_search_start,
        ReadHandler handler)
      : stream_(stream),
        streambuf_(streambuf),
        expr_(expr),
        ...
        std::size_t bytes_transferred)
    {
      ...
      // Look for a match.
      streams_boost::match_results<iterator> match_results;
      if (streams_boost::regex_search(start, end, match_results, expr_,
            streams_boost::match_default | streams_boost::match_partial))
      {
        if (match_results[0].matched)
        {
    ...
    ReadHandler handler)
{
  ...
  std::size_t next_search_start;
  streams_boost::match_results<iterator> match_results;
  if (streams_boost::regex_search(begin, end, match_results, expr,
        streams_boost::match_default | streams_boost::match_partial))
  {
    if (match_results[0].matched)
    {
...
#endif // STREAMS_BOOST_ASIO_READ_UNTIL_IPP
