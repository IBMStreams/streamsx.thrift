...
#ifndef STREAMS_BOOST_XPRESSIVE_ALGORITHMS_HPP_EAN_10_04_2005
#define STREAMS_BOOST_XPRESSIVE_ALGORITHMS_HPP_EAN_10_04_2005
...
#include <streams_boost/xpressive/detail/utility/save_restore.hpp>

/// INTERNAL ONLY
///
#define STREAMS_BOOST_XPR_NONDEDUCED_TYPE_(x) typename mpl::identity<x>::type
...
namespace detail
{
    ...
    (
        STREAMS_BOOST_XPR_NONDEDUCED_TYPE_(BidiIter) begin
      , STREAMS_BOOST_XPR_NONDEDUCED_TYPE_(BidiIter) end
    ...
    )
    {
        ...
        STREAMS_BOOST_ASSERT(0 != re.regex_id());
...
inline bool regex_match
(
    STREAMS_BOOST_XPR_NONDEDUCED_TYPE_(Char) *begin
...
)
{
    ...
    // to BidiIter here.
    BidiIter begin = streams_boost::begin(rng), end = streams_boost::end(rng);
    return detail::regex_match_impl(begin, end, what, re, flags);
}
        ...
        else if(impl.finder_ && (!partial_ok || impl.finder_->ok_for_partial_matches()))
        {
            ...
            if(find(state))
            {
                ...
                do
                {
                    ...
                    STREAMS_BOOST_ASSERT(state.cur_ == sub0begin);
                    not_null.restore();
                }
...
)
{
    ...
    // Note that the result iterator of the range must be convertible
    // to BidiIter here.
    BidiIter begin = streams_boost::begin(str), end = streams_boost::end(str);
...
(
    STREAMS_BOOST_XPR_NONDEDUCED_TYPE_(Char) *str
  , basic_regex<Char *> const &re
  , Formatter const &format
  , regex_constants::match_flag_type flags = regex_constants::match_default
  , typename disable_if<detail::is_char_ptr<Formatter> >::type * = 0
