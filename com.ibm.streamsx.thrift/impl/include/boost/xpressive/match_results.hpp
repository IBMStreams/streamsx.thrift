...
#ifndef STREAMS_BOOST_XPRESSIVE_MATCH_RESULTS_HPP_EAN_10_04_2005
#define STREAMS_BOOST_XPRESSIVE_MATCH_RESULTS_HPP_EAN_10_04_2005
...
#include <streams_boost/numeric/conversion/converter.hpp>
...
#if STREAMS_BOOST_ITERATOR_ADAPTORS_VERSION >= 0x0200
# include <streams_boost/iterator/filter_iterator.hpp>
#endif
...
#include <streams_boost/xpressive/detail/core/sub_match_vector.hpp>
#include <streams_boost/xpressive/detail/utility/sequence_stack.hpp>
#include <streams_boost/xpressive/detail/core/results_cache.hpp>
#include <streams_boost/xpressive/detail/utility/literals.hpp>
#include <streams_boost/xpressive/detail/utility/algorithm.hpp>
#include <streams_boost/xpressive/detail/utility/counted_base.hpp>
// Doxygen can't handle proto :-(
#ifndef STREAMS_BOOST_XPRESSIVE_DOXYGEN_INVOKED
  ...
  : std::iterator<std::output_iterator_tag, Char, void, void, case_converting_iterator<OutputIterator, Char> >
{
    ...
    {
        STREAMS_BOOST_ASSERT(scope == Next || scope == Rest);
        if(scope == Next)
            iter.next_ = trans;
        else
            iter.rest_ = trans;
...
#if STREAMS_BOOST_WORKAROUND(__GNUC__, == 4) && (__GNUC_MINOR__ == 0)
...
public:
    ...
    {
        return this->format_(out, streams_boost::as_literal(fmt), flags, mpl::size_t<0>());
    }
    ...
    match_results<BidiIter> &let(Arg const &arg)
    {
        ...
        STREAMS_BOOST_MPL_ASSERT((proto::matches<Arg, detail::ActionArgBinding>));
        STREAMS_BOOST_MPL_ASSERT((is_same<typename arg_left_type::type, arg_right_type>));
...
private:
    ...
    {
        return std::copy(streams_boost::begin(result), streams_boost::end(result), out);
    }
    ...
    ) const
    {
        ...
        iterator cur = streams_boost::begin(format), end = streams_boost::end(format);
    ...
    OutputIterator format_perl_(ForwardIterator cur, ForwardIterator end, OutputIterator out) const
    {
        ...
        while(cur != end)
        {
            ...
            case STREAMS_BOOST_XPR_CHAR_(char_type, '\\'):
                if(++cur != end && STREAMS_BOOST_XPR_CHAR_(char_type, 'g') == *cur)
                {
    ...
    OutputIterator format_all_(ForwardIterator cur, ForwardIterator end, OutputIterator out) const
    {
        ...
        STREAMS_BOOST_XPR_ENSURE_(cur == end
    ...
    OutputIterator format_all_impl_(ForwardIterator &cur, ForwardIterator end, OutputIterator out, bool metacolon = false) const
    {
        ...
        while(cur != end)
        {
            ...
            case STREAMS_BOOST_XPR_CHAR_(char_type, '('):
                ...
                STREAMS_BOOST_XPR_ENSURE_(STREAMS_BOOST_XPR_CHAR_(char_type, ')') == *(cur-1)
                  , regex_constants::error_paren, "unbalanced parentheses in format string");
                break;
            ...
            case STREAMS_BOOST_XPR_CHAR_(char_type, '?'):
                ...
                STREAMS_BOOST_XPR_ENSURE_(0 != sub, regex_constants::error_subreg, "invalid back-reference");
                if(this->sub_matches_[ sub ].matched)
                {
                    out = this->format_all_impl_(cur, end, out, true);
                    if(STREAMS_BOOST_XPR_CHAR_(char_type, ':') == *(cur-1))
    ...
    ) const
    {
        if(cur == end)
        {
            *out++ = STREAMS_BOOST_XPR_CHAR_(char_type, '$');
        }
        else if(STREAMS_BOOST_XPR_CHAR_(char_type, '$') == *cur)
        {
            *out++ = *cur++;
        }
        else if(STREAMS_BOOST_XPR_CHAR_(char_type, '&') == *cur) // whole match
        {
            ++cur;
            out = std::copy(this->sub_matches_[ 0 ].first, this->sub_matches_[ 0 ].second, out);
        }
        else if(STREAMS_BOOST_XPR_CHAR_(char_type, '`') == *cur) // prefix
        {
            ++cur;
            out = std::copy(this->prefix().first, this->prefix().second, out);
        }
        else if(STREAMS_BOOST_XPR_CHAR_(char_type, '\'') == *cur) // suffix
        {
            ++cur;
    ...
    ) const
    {
        ...
        typedef typename streams_boost::uint_t<CHAR_BIT * sizeof(char_type)>::least uchar_t;
        STREAMS_BOOST_MPL_ASSERT_RELATION(sizeof(uchar_t), ==, sizeof(char_type));
        ...
        switch(ch)
        {
        case STREAMS_BOOST_XPR_CHAR_(char_type, 'a'):
            *out++ = STREAMS_BOOST_XPR_CHAR_(char_type, '\a');
            break;

        case STREAMS_BOOST_XPR_CHAR_(char_type, 'e'):
            *out++ = converter(27);
            break;

        case STREAMS_BOOST_XPR_CHAR_(char_type, 'f'):
            *out++ = STREAMS_BOOST_XPR_CHAR_(char_type, '\f');
            break;

        case STREAMS_BOOST_XPR_CHAR_(char_type, 'n'):
            *out++ = STREAMS_BOOST_XPR_CHAR_(char_type, '\n');
            break;

        case STREAMS_BOOST_XPR_CHAR_(char_type, 'r'):
            *out++ = STREAMS_BOOST_XPR_CHAR_(char_type, '\r');
            break;

        case STREAMS_BOOST_XPR_CHAR_(char_type, 't'):
            *out++ = STREAMS_BOOST_XPR_CHAR_(char_type, '\t');
            break;

        case STREAMS_BOOST_XPR_CHAR_(char_type, 'v'):
            *out++ = STREAMS_BOOST_XPR_CHAR_(char_type, '\v');
            break;

        case STREAMS_BOOST_XPR_CHAR_(char_type, 'x'):
            STREAMS_BOOST_XPR_ENSURE_(cur != end, error_escape, "unexpected end of format found");
            if(STREAMS_BOOST_XPR_CHAR_(char_type, '{') == *cur)
            {
                STREAMS_BOOST_XPR_ENSURE_(++cur != end, error_escape, "unexpected end of format found");
                tmp = cur;
                *out++ = converter(detail::toi(cur, end, *this->traits_, 16, 0xffff));
                STREAMS_BOOST_XPR_ENSURE_(4 == std::distance(tmp, cur) && cur != end && STREAMS_BOOST_XPR_CHAR_(char_type, '}') == *cur++
            ...
            else
            {
                ...
                *out++ = converter(detail::toi(cur, end, *this->traits_, 16, 0xff));
                STREAMS_BOOST_XPR_ENSURE_(2 == std::distance(tmp, cur), error_escape
                  , "invalid hex escape : must be \\x HexDigit HexDigit");
            }
            break;

        case STREAMS_BOOST_XPR_CHAR_(char_type, 'c'):
            STREAMS_BOOST_XPR_ENSURE_(cur != end, error_escape, "unexpected end of format found");
            STREAMS_BOOST_XPR_ENSURE_
            (
                this->traits_->in_range(STREAMS_BOOST_XPR_CHAR_(char_type, 'a'), STREAMS_BOOST_XPR_CHAR_(char_type, 'z'), *cur)
             || this->traits_->in_range(STREAMS_BOOST_XPR_CHAR_(char_type, 'A'), STREAMS_BOOST_XPR_CHAR_(char_type, 'Z'), *cur)
              , error_escape
        ...
        case STREAMS_BOOST_XPR_CHAR_(char_type, 'l'):
            if(!set_transform(out, detail::Lower, detail::Next))
            {
                *out++ = STREAMS_BOOST_XPR_CHAR_(char_type, 'l');
            }
            break;
        ...
        case STREAMS_BOOST_XPR_CHAR_(char_type, 'u'):
            if(!set_transform(out, detail::Upper, detail::Next))
            {
                *out++ = STREAMS_BOOST_XPR_CHAR_(char_type, 'u');
            }
            break;
        ...
        case STREAMS_BOOST_XPR_CHAR_(char_type, 'E'):
            ...
            {
                *out++ = STREAMS_BOOST_XPR_CHAR_(char_type, 'E');
            }
            break;
    ...
    {
        using namespace regex_constants;
        STREAMS_BOOST_XPR_ENSURE_(cur != end && STREAMS_BOOST_XPR_CHAR_(char_type, '<') == *cur++
            , error_badmark, "invalid named back-reference");
        ForwardIterator begin = cur;
        for(; cur != end && STREAMS_BOOST_XPR_CHAR_(char_type, '>') != *cur; ++cur)
        {}
        STREAMS_BOOST_XPR_ENSURE_(cur != begin && cur != end && STREAMS_BOOST_XPR_CHAR_(char_type, '>') == *cur
            , error_badmark, "invalid named back-reference");
        ...
        STREAMS_BOOST_THROW_EXCEPTION(regex_error(error_badmark, "invalid named back-reference"));
    ...
    streams_boost::optional<BidiIter> base_;
    streams_boost::optional<sub_match<BidiIter> > prefix_;
    streams_boost::optional<sub_match<BidiIter> > suffix_;
...
#ifdef STREAMS_BOOST_HAS_CONCEPTS
// Better living through concepts. :-P
namespace std
{
    template<typename Iter_, typename Char_>
    concept_map OutputIterator<
        streams_boost::xpressive::detail::case_converting_iterator<Iter_, Char_>
      , Char_
    >
    {};
    ...
    concept_map OutputIterator<
        streams_boost::xpressive::detail::noop_output_iterator<Char_>
      , Char_
    >
    {};
}
