...
#ifndef STREAMS_BOOST_XPRESSIVE_REGEX_COMPILER_HPP_EAN_10_04_2005
#define STREAMS_BOOST_XPRESSIVE_REGEX_COMPILER_HPP_EAN_10_04_2005
...
#include <streams_boost/xpressive/detail/dynamic/parser.hpp>
#include <streams_boost/xpressive/detail/dynamic/parse_charset.hpp>
#include <streams_boost/xpressive/detail/dynamic/parser_enum.hpp>
#include <streams_boost/xpressive/detail/dynamic/parser_traits.hpp>
#include <streams_boost/xpressive/detail/core/linker.hpp>
#include <streams_boost/xpressive/detail/core/optimize.hpp>
...
struct regex_compiler
{
    ...
    {
        return this->compile(streams_boost::begin(pat), streams_boost::end(pat), flags);
    }
    ...
    {
        STREAMS_BOOST_ASSERT(!name.empty());
...
private:
    ...
    {
        STREAMS_BOOST_MPL_ASSERT((is_same<char_type, typename iterator_value<FwdIter>::type>));
        ...
        if(token_group_begin == this->traits_.get_token(tmp, end) &&
           STREAMS_BOOST_XPR_ENSURE_(tmp != end, error_paren, "mismatched parenthesis") &&
        ...
        STREAMS_BOOST_XPR_ENSURE_(begin == end, error_paren, "mismatched parenthesis");
    ...
    basic_regex<BidiIter> compile_(InputIter begin, InputIter end, flag_type flags, std::input_iterator_tag)
    {
        ...
        return this->compile_(streams_boost::begin(pat), streams_boost::end(pat), flags, std::forward_iterator_tag());
    }
    ...
    detail::sequence<BidiIter> parse_group(FwdIter &begin, FwdIter end)
    {
        ...
        case token_comment:
            while(STREAMS_BOOST_XPR_ENSURE_(begin != end, error_paren, "mismatched parenthesis"))
            {
                switch(this->traits_.get_token(begin, end))
                {
                case token_group_end: return this->parse_atom(begin, end);
                case token_escape: STREAMS_BOOST_XPR_ENSURE_(begin != end, error_escape, "incomplete escape sequence");
        ...
        case token_named_mark:
            ...
            {
                STREAMS_BOOST_XPR_ENSURE_(this->self_->named_marks_[i].name_ != name, error_badmark, "named mark already exists");
            }
    ...
    detail::sequence<BidiIter> parse_atom(FwdIter &begin, FwdIter end)
    {
        ...
        case token_invalid_quantifier:
            STREAMS_BOOST_THROW_EXCEPTION(regex_error(error_badrepeat, "quantifier not expected"));
            break;
    ...
    detail::sequence<BidiIter> parse_quant(FwdIter &begin, FwdIter end)
    {
        ...
        if(!seq.empty() && begin != end && detail::quant_none != seq.quant())
        {
            ...
            {
                STREAMS_BOOST_ASSERT(spec.min_ <= spec.max_);
    ...
    string_type parse_literal(FwdIter &begin, FwdIter end)
    {
        using namespace regex_constants;
        STREAMS_BOOST_ASSERT(begin != end);
        STREAMS_BOOST_ASSERT(token_literal == this->traits_.get_token(begin, end));
        escape_value esc = { 0, 0, 0, detail::escape_char };
        ...
        for(FwdIter prev = begin, tmp = ++begin; begin != end; prev = begin, begin = tmp)
        {
            ...
            if(this->traits_.get_quant_spec(tmp, end, spec))
            {
                ...
                {
                    begin = prev;
                    literal.erase(streams_boost::prior(literal.end()));
                }
                return literal;
            }
    ...
    {
        STREAMS_BOOST_XPR_ENSURE_(begin != end, regex_constants::error_escape, "incomplete escape sequence");
