...
#ifndef STREAMS_BOOST_XPRESSIVE_DETAIL_CORE_MATCHER_SIMPLE_REPEAT_MATCHER_HPP_EAN_10_04_2005
#define STREAMS_BOOST_XPRESSIVE_DETAIL_CORE_MATCHER_SIMPLE_REPEAT_MATCHER_HPP_EAN_10_04_2005
...
#include <streams_boost/xpressive/detail/static/type_traits.hpp>
...
namespace streams_boost { namespace xpressive { namespace detail
{
      ...
      : quant_style_variable_width
    {
          ...
          , leading_(false)
        {
            ...
            STREAMS_BOOST_ASSERT(min <= max);
            STREAMS_BOOST_ASSERT(0 != max);
            STREAMS_BOOST_ASSERT(0 != width && unknown_width() != width);
            STREAMS_BOOST_ASSERT(Xpr::width == unknown_width() || Xpr::width == width);
        }
        ...
        bool match_(match_state<BidiIter> &state, Next const &next, greedy_slow_tag) const
        {
            ...
            if(this->leading_)
            {
                state.next_search_ = (matches && matches < this->max_)
                                   ? state.cur_
                                   : (tmp == state.end_) ? tmp : streams_boost::next(tmp);
            }
        ...
        {
            STREAMS_BOOST_ASSERT(!this->leading_);
            BidiIter const tmp = state.cur_;
            unsigned int matches = 0;
        ...
        bool match_(match_state<BidiIter> &state, Next const &next, greedy_fast_tag) const
        {
            ...
            if(this->min_ > diff_to_end)
            {
                if(this->leading_)
                {
                    state.next_search_ = (tmp == state.end_) ? tmp : streams_boost::next(tmp);
                }
                return false;
            }
