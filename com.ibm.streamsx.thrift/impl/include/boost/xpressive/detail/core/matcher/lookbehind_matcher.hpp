...
#ifndef STREAMS_BOOST_XPRESSIVE_DETAIL_CORE_MATCHER_LOOKBEHIND_MATCHER_HPP_EAN_10_04_2005
#define STREAMS_BOOST_XPRESSIVE_DETAIL_CORE_MATCHER_LOOKBEHIND_MATCHER_HPP_EAN_10_04_2005
...
namespace streams_boost { namespace xpressive { namespace detail
{
      ...
      : quant_style<quant_none, 0, Xpr::pure>
    {
        lookbehind_matcher(Xpr const &xpr, std::size_t wid, bool no, bool pure = Xpr::pure)
          ...
          , width_(wid)
        {
            STREAMS_BOOST_XPR_ENSURE_(!is_unknown(this->width_), regex_constants::error_badlookbehind,
        ...
        bool match_(match_state<BidiIter> &state, Next const &next, mpl::true_) const
        {
            ...
            if(this->not_)
            {
                ...
                {
                    STREAMS_BOOST_ASSERT(state.cur_ == tmp);
                    return false;
                }
                state.cur_ = tmp;
                if(next.match(state))
