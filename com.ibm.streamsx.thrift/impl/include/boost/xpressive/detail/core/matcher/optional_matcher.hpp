...
#ifndef STREAMS_BOOST_XPRESSIVE_DETAIL_CORE_MATCHER_OPTIONAL_MATCHER_HPP_EAN_10_04_2005
#define STREAMS_BOOST_XPRESSIVE_DETAIL_CORE_MATCHER_OPTIONAL_MATCHER_HPP_EAN_10_04_2005
...
namespace streams_boost { namespace xpressive { namespace detail
{
    ...
    private:
        ...
        {
            return this->xpr_.STREAMS_BOOST_NESTED_TEMPLATE push_match<Next>(state)
                || next.match(state);
        }
        ...
        bool match_(match_state<BidiIter> &state, Next const &next, mpl::false_) const // Non-greedy
        {
            return next.match(state)
                || this->xpr_.STREAMS_BOOST_NESTED_TEMPLATE push_match<Next>(state);
        }
