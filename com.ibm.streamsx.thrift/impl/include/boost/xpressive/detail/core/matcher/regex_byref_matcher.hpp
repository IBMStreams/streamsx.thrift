...
#ifndef STREAMS_BOOST_XPRESSIVE_DETAIL_CORE_MATCHER_REGEX_BYREF_MATCHER_HPP_EAN_10_04_2005
#define STREAMS_BOOST_XPRESSIVE_DETAIL_CORE_MATCHER_REGEX_BYREF_MATCHER_HPP_EAN_10_04_2005
...
#include <streams_boost/xpressive/detail/core/adaptor.hpp>
...
namespace streams_boost { namespace xpressive { namespace detail
{
      ...
      : quant_style<quant_variable_width, unknown_width::value, false>
    {
        ...
        regex_byref_matcher(shared_ptr<regex_impl<BidiIter> > const &impl)
          ...
          , pimpl_(impl.get())
        {
            STREAMS_BOOST_ASSERT(this->pimpl_);
        }

        template<typename Next>
        bool match(match_state<BidiIter> &state, Next const &next) const
        {
            STREAMS_BOOST_ASSERT(this->pimpl_ == this->wimpl_.lock().get());
            STREAMS_BOOST_XPR_ENSURE_(this->pimpl_->xpr_, regex_constants::error_badref, "bad regex reference");
    ...
    private:
        ...
        static xpression_adaptor<reference_wrapper<Next const>, matchable<BidiIter> > wrap_(Next const &next, mpl::true_)
        {
            ...
            return xpression_adaptor<reference_wrapper<Next const>, matchable<BidiIter> >(streams_boost::cref(next));
        }
