...
#ifndef STREAMS_BOOST_XPRESSIVE_DETAIL_CORE_MATCHER_STRING_MATCHER_HPP_EAN_10_04_2005
#define STREAMS_BOOST_XPRESSIVE_DETAIL_CORE_MATCHER_STRING_MATCHER_HPP_EAN_10_04_2005
...
namespace streams_boost { namespace xpressive { namespace detail
{
      ...
      : quant_style_fixed_unknown_width
    {
        ...
        string_matcher(string_type const &str, Traits const &tr)
          ...
          , end_()
        {
            typename range_iterator<string_type>::type cur = streams_boost::begin(this->str_);
            typename range_iterator<string_type>::type end = streams_boost::end(this->str_);
            for(; cur != end; ++cur)
            {
        ...
        {
            return streams_boost::size(this->str_);
        }
    };
