...
#ifndef STREAMS_BOOST_XPRESSIVE_DETAIL_STATIC_STATIC_HPP_EAN_10_04_2005
#define STREAMS_BOOST_XPRESSIVE_DETAIL_STATIC_STATIC_HPP_EAN_10_04_2005
...
#include <streams_boost/xpressive/detail/core/peeker.hpp>
#include <streams_boost/xpressive/detail/static/placeholders.hpp>
  ...
  : Next
{
    ...
    bool match(match_state<BidiIter> &state) const
    {
        return static_cast<Next const *>(this)->
            STREAMS_BOOST_NESTED_TEMPLATE push_match<Top>(state);
    }
...
//protected:
    ...
    {
        return that.STREAMS_BOOST_NESTED_TEMPLATE push_match<Top>(state);
    }
};
...
inline stacked_xpression<Top, Next> const &stacked_xpression_cast(Next const &next)
{
    ...
    STREAMS_BOOST_MPL_ASSERT_RELATION(sizeof(stacked_xpression<Top, Next>), ==, sizeof(Next));
  ...
  : Matcher
{
    ...
    STREAMS_BOOST_STATIC_CONSTANT(bool, pure = Matcher::pure && Next::pure);
...
struct no_next
{
    STREAMS_BOOST_STATIC_CONSTANT(std::size_t, width = 0);
    STREAMS_BOOST_STATIC_CONSTANT(bool, pure = true);
