...
#ifndef STREAMS_BOOST_XPRESSIVE_DETAIL_SYMBOLS_HPP_DRJ_06_11_2007
#define STREAMS_BOOST_XPRESSIVE_DETAIL_SYMBOLS_HPP_DRJ_06_11_2007
...
namespace streams_boost { namespace xpressive { namespace detail
{
    ...
    struct symbols
    {
        ...
        {
            iterator begin = streams_boost::begin(map);
            iterator end = streams_boost::end(map);
            node* root_p = this->root.get();
            for(; begin != end; ++begin)
            {
                key_iterator kbegin = streams_boost::begin(begin->first);
                key_iterator kend = streams_boost::end(begin->first);
    ...
    private:
            ...
            : streams_boost::noncopyable
        {
            node(char_type c)
              ...
              , eq(0)
              , hi(0)
              #ifdef STREAMS_BOOST_DISABLE_THREADS
              , tau(0)
              #endif
            {}
        ...
        streams_boost::shared_ptr<node> root;
    };
