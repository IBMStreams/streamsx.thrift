...
#ifndef STREAMS_BOOST_XPRESSIVE_ACTIONS_HPP_EAN_03_22_2007
#define STREAMS_BOOST_XPRESSIVE_ACTIONS_HPP_EAN_03_22_2007
...
#include <streams_boost/xpressive/detail/core/matcher/attr_end_matcher.hpp>
#include <streams_boost/xpressive/detail/core/matcher/attr_begin_matcher.hpp>
...
#include <streams_boost/typeof/std/map.hpp>
...
namespace streams_boost { namespace xpressive
{
    ...
    namespace detail
    {
        ...
        struct let_domain
          : streams_boost::proto::domain<streams_boost::proto::pod_generator<let_> >
        {};

        template<typename Expr>
        struct let_
        {
            STREAMS_BOOST_PROTO_BASIC_EXTENDS(Expr, let_<Expr>, let_domain)
        ...
        typedef streams_boost::proto::functional::make_expr<proto::tag::function, proto::default_domain> make_function;
    }
    ...
    namespace op
    {
        ...
        struct insert
        {
            ...
            struct result<This(Cont, Value), void>
            {
                ...
                STREAMS_BOOST_STATIC_CONSTANT(bool, is_iterator = (sizeof(yes_type) == sizeof(check_insert_return(scont_.insert(svalue_)))));
                typedef
                    typename mpl::if_c<
                        is_iterator
        ...
        struct throw_
        {
            ...
            void operator()() const
            {
                STREAMS_BOOST_THROW_EXCEPTION(Except());
            }

            template<typename A0>
            void operator()(A0 const &a0) const
            {
                STREAMS_BOOST_THROW_EXCEPTION(Except(a0));
            }

            template<typename A0, typename A1>
            void operator()(A0 const &a0, A1 const &a1) const
            {
                STREAMS_BOOST_THROW_EXCEPTION(Except(a0, a1));
            }

            template<typename A0, typename A1, typename A2>
            void operator()(A0 const &a0, A1 const &a1, A2 const &a2) const
            {
                STREAMS_BOOST_THROW_EXCEPTION(Except(a0, a1, a2));
            }
        };
    }
      ...
      : proto::extends<typename proto::terminal<reference_wrapper<T> >::type, reference<T> >
    {
        ...
        explicit reference(T &t)
          : base_type(base_type::proto_base_expr::make(streams_boost::ref(t)))
        {}
      ...
      , proto::terminal<reference_wrapper<T> >::type
    {
        ...
        local()
          : detail::value_wrapper<T>()
          , base_type(base_type::make(streams_boost::ref(detail::value_wrapper<T>::value)))
        {}
    ...
    struct placeholder
    {
        ...
        STREAMS_BOOST_PROTO_EXTENDS(action_arg_type, this_type, proto::default_domain)
    };
    ...
    \
    template<typename X2_0 STREAMS_BOOST_PP_COMMA_IF(N) typename_A(N)>\
    typename detail::make_function::impl<op::construct<X2_0> const STREAMS_BOOST_PP_COMMA_IF(N) A_const_ref(N)>::result_type const\
    construct(A_const_ref_a(N))\
    {\
        return detail::make_function::impl<op::construct<X2_0> const STREAMS_BOOST_PP_COMMA_IF(N) A_const_ref(N)>()((op::construct<X2_0>()) STREAMS_BOOST_PP_COMMA_IF(N) a(N));\
    }\
    \
    template<typename X2_0 STREAMS_BOOST_PP_COMMA_IF(N) typename_A(N)>\
    typename detail::make_function::impl<op::throw_<X2_0> const STREAMS_BOOST_PP_COMMA_IF(N) A_const_ref(N)>::result_type const\
    throw_(A_const_ref_a(N))\
    {\
        return detail::make_function::impl<op::throw_<X2_0> const STREAMS_BOOST_PP_COMMA_IF(N) A_const_ref(N)>()((op::throw_<X2_0>()) STREAMS_BOOST_PP_COMMA_IF(N) a(N));\
    }\
    /**/

    #define STREAMS_BOOST_PROTO_LOCAL_a       STREAMS_BOOST_PROTO_a
    #define STREAMS_BOOST_PROTO_LOCAL_LIMITS  (0, STREAMS_BOOST_PP_DEC(STREAMS_BOOST_PROTO_MAX_ARITY))
...
#endif // STREAMS_BOOST_XPRESSIVE_ACTIONS_HPP_EAN_03_22_2007
