...
#ifndef STREAMS_BOOST_XPRESSIVE_TRAITS_CPP_REGEX_TRAITS_HPP_EAN_10_04_2005
#define STREAMS_BOOST_XPRESSIVE_TRAITS_CPP_REGEX_TRAITS_HPP_EAN_10_04_2005
...
#if STREAMS_BOOST_WORKAROUND(__GLIBCPP__, != 0)
# define STREAMS_BOOST_XPRESSIVE_BUGGY_CTYPE_FACET
#endif
...
namespace detail
{
    ...
    typedef streams_boost::uint_t<sizeof(std::ctype_base::mask) * CHAR_BIT>::least umask_t;
    STREAMS_BOOST_MPL_ASSERT_RELATION(sizeof(std::ctype_base::mask), ==, sizeof(umask_t));
    ...
    typedef streams_boost::uint_t<umaskex_bits>::fast umaskex_t;
    STREAMS_BOOST_MPL_ASSERT_RELATION(sizeof(umask_t), <=, sizeof(umaskex_t));

    // cast a ctype mask to a umaskex_t
    template<std::ctype_base::mask Mask>
    struct mask_cast
    {
        STREAMS_BOOST_STATIC_CONSTANT(umaskex_t, value = static_cast<umask_t>(Mask));
    };
    ...
    {
        STREAMS_BOOST_MPL_ASSERT_RELATION('\227', ==, std::ctype_base::print);
        STREAMS_BOOST_STATIC_CONSTANT(umaskex_t, value = 0227);
    };
    #endif
    ...
    #ifndef STREAMS_BOOST_XPRESSIVE_BUGGY_CTYPE_FACET
    ...
    struct unused_mask
    {
        STREAMS_BOOST_MPL_ASSERT_RELATION(1, !=, Out);
        STREAMS_BOOST_STATIC_CONSTANT(umaskex_t, value = (unused_mask<In, (Out >> 1)>::value));
    };

    template<umaskex_t In, umaskex_t Out>
    struct unused_mask<In, Out, true>
    {
        STREAMS_BOOST_STATIC_CONSTANT(umaskex_t, value = Out);
    };
    ...
    #elif defined(_CPPLIB_VER) && defined(STREAMS_BOOST_WINDOWS)
    ...
    private:
        ...
        {
            STREAMS_BOOST_MPL_ASSERT_RELATION('\t', ==, L'\t');
            STREAMS_BOOST_MPL_ASSERT_RELATION(' ', ==, L' ');
            return L' ' == ch || L'\t' == ch;
        }
        ...
        {
            STREAMS_BOOST_MPL_ASSERT_RELATION('_', ==, L'_');
            return L'_' == ch;
        }

        static bool is_newline(Char ch)
        {
            STREAMS_BOOST_MPL_ASSERT_RELATION('\r', ==, L'\r');
            STREAMS_BOOST_MPL_ASSERT_RELATION('\n', ==, L'\n');
            STREAMS_BOOST_MPL_ASSERT_RELATION('\f', ==, L'\f');
            return L'\r' == ch || L'\n' == ch || L'\f' == ch
    ...
    protected:
        void imbue(std::locale const &loc)
        {
            ...
            std::ctype<Char> const &ct = STREAMS_BOOST_USE_FACET(std::ctype<Char>, loc);
            std::ctype_base::mask tmp[UCHAR_MAX + 1];
            ct.is(allchars, allchars + UCHAR_MAX + 1, tmp);
            for(i = 0; i <= static_cast<int>(UCHAR_MAX); ++i)
            {
                this->masks_[i] = static_cast<umask_t>(tmp[i]);
                STREAMS_BOOST_ASSERT(0 == (this->masks_[i] & non_std_ctype_masks));
            }
  ...
  : detail::cpp_regex_traits_base<Char>
{
    ...
    {
        STREAMS_BOOST_MPL_ASSERT((is_same<char_type, char>));
        char_type ntcs[] = {
    ...
    {
        STREAMS_BOOST_ASSERT(false); // TODO implement me
        return string_type();
    }
    ...
    {
        STREAMS_BOOST_ASSERT(8 == radix || 10 == radix || 16 == radix);
        int val = -1;
    ...
    locale_type imbue(locale_type loc)
    {
        ...
        this->loc_ = loc;
        this->ctype_ = &STREAMS_BOOST_USE_FACET(std::ctype<char_type>, this->loc_);
        //this->collate_ = &STREAMS_BOOST_USE_FACET(std::collate<char_type>, this->loc_);
...
private:
    ...
    static char_class_pair const &char_class(std::size_t j)
    {
        ...
        {
            { STREAMS_BOOST_XPR_CSTR_(char_type, "alnum"),  detail::std_ctype_alnum }
          , { STREAMS_BOOST_XPR_CSTR_(char_type, "alpha"),  detail::std_ctype_alpha }
          , { STREAMS_BOOST_XPR_CSTR_(char_type, "blank"),  detail::non_std_ctype_blank }
          , { STREAMS_BOOST_XPR_CSTR_(char_type, "cntrl"),  detail::std_ctype_cntrl }
          , { STREAMS_BOOST_XPR_CSTR_(char_type, "d"),      detail::std_ctype_digit }
          , { STREAMS_BOOST_XPR_CSTR_(char_type, "digit"),  detail::std_ctype_digit }
          , { STREAMS_BOOST_XPR_CSTR_(char_type, "graph"),  detail::std_ctype_graph }
          , { STREAMS_BOOST_XPR_CSTR_(char_type, "lower"),  detail::std_ctype_lower }
          , { STREAMS_BOOST_XPR_CSTR_(char_type, "newline"),detail::non_std_ctype_newline }
          , { STREAMS_BOOST_XPR_CSTR_(char_type, "print"),  detail::std_ctype_print }
          , { STREAMS_BOOST_XPR_CSTR_(char_type, "punct"),  detail::std_ctype_punct }
          , { STREAMS_BOOST_XPR_CSTR_(char_type, "s"),      detail::std_ctype_space }
          , { STREAMS_BOOST_XPR_CSTR_(char_type, "space"),  detail::std_ctype_space }
          , { STREAMS_BOOST_XPR_CSTR_(char_type, "upper"),  detail::std_ctype_upper }
          , { STREAMS_BOOST_XPR_CSTR_(char_type, "w"),      detail::std_ctype_alnum | detail::non_std_ctype_underscore }
          , { STREAMS_BOOST_XPR_CSTR_(char_type, "xdigit"), detail::std_ctype_xdigit }
          , { 0, 0 }
        };
...
#ifndef STREAMS_BOOST_XPRESSIVE_NO_WREGEX
template<>
