...
#ifndef OBJECT_CORE_DWA2002615_STREAMS_HPP
# define OBJECT_CORE_DWA2002615_STREAMS_HPP

# include <streams_boost/python/detail/prefix.hpp>

# include <streams_boost/type.hpp>

# include <streams_boost/python/call.hpp>
...
# include <streams_boost/python/detail/string_literal.hpp>
# include <streams_boost/python/detail/def_helper_fwd.hpp>
...
// attribute/item access or addition.
  ...
  //
  STREAMS_BOOST_PYTHON_IS_XXX_DEF(proxy, streams_boost::python::api::proxy, 1)
...
# define STREAMS_BOOST_PP_ITERATION_PARAMS_1 (3, (1, STREAMS_BOOST_PYTHON_MAX_ARITY, <streams_boost/python/object_call.hpp>))
...
# ifdef STREAMS_BOOST_NO_TEMPLATE_PARTIAL_SPECIALIZATION
  ...
  struct is_derived_impl
  {
      ...
      STREAMS_BOOST_STATIC_CONSTANT(bool, value = sizeof(test(to_pointer(x))) == 1);
  };
...
# endif
  ...
  PyObject* object_base_initializer(T const& x)
  {
      typedef typename is_derived<
          STREAMS_BOOST_DEDUCED_TYPENAME objects::unforward_cref<T>::type
        , object
      >::type is_obj;

      return object_initializer<
          STREAMS_BOOST_DEDUCED_TYPENAME unwrap_reference<T>::type
      >::get(
            x
          , is_obj()
      );
...
# endif 
      ...
      STREAMS_BOOST_PYTHON_DECL explicit object(handle<> const&);
   private:
...
# define STREAMS_BOOST_PYTHON_FORWARD_OBJECT_CONSTRUCTORS_(derived, base)       \
...
# if !defined(STREAMS_BOOST_MSVC) || STREAMS_BOOST_MSVC >= 1300
#  define STREAMS_BOOST_PYTHON_FORWARD_OBJECT_CONSTRUCTORS STREAMS_BOOST_PYTHON_FORWARD_OBJECT_CONSTRUCTORS_
# else
...
#  define STREAMS_BOOST_PYTHON_FORWARD_OBJECT_CONSTRUCTORS(derived, base)       \
    STREAMS_BOOST_PYTHON_FORWARD_OBJECT_CONSTRUCTORS_(derived, base)            \
    template <class T>                                                  \
...
# endif
  ...
  struct object_initializer_impl<false, true>
  {
      ...
      get(T const& x, U)
      {
          return python::incref(get_managed_object(x, streams_boost::python::tag));
      }
  };
...
{ 
  ...
  return streams_boost::python::detail::args_proxy(x); 
} 
...
{ 
  ...
  PyObject *result = PyObject_Call(get_managed_object(self, streams_boost::python::tag), 
                                   args.operator object().ptr(), 
                                   0); 
  return object(streams_boost::python::detail::new_reference(result)); 
...
# include <streams_boost/python/slice_nil.hpp>

#endif // OBJECT_CORE_DWA2002615_STREAMS_HPP
