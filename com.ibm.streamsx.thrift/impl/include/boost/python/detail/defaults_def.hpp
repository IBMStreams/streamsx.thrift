...
#include <streams_boost/python/detail/defaults_gen.hpp>
...
#include <streams_boost/python/scope.hpp>
#include <streams_boost/preprocessor/debug/line.hpp>
#include <streams_boost/python/detail/scope.hpp>
...
namespace detail
{
  ...
  //      }
  //
  //  where N runs from 0 to STREAMS_BOOST_PYTHON_MAX_ARITY.
  //
...
#define STREAMS_BOOST_PP_ITERATION_PARAMS_1                                             \
    (3, (0, STREAMS_BOOST_PYTHON_MAX_ARITY, <streams_boost/python/detail/defaults_def.hpp>))
...
#include STREAMS_BOOST_PP_ITERATE()
      ...
      SigT const&)
  {
      ...
      typedef typename mpl::if_c<
          streams_boost::is_same<void, return_type>::value
          , void_return_type
...
template <>
struct define_stub_function<STREAMS_BOOST_PP_ITERATION()> {
    template <class StubsT, class CallPolicies, class NameSpaceT>
    static void define(
        char const* name
        , StubsT const&
        ...
        , char const* doc)
    {
        detail::name_space_def(
            name_space
            , name
            , &StubsT::STREAMS_BOOST_PP_CAT(func_, STREAMS_BOOST_PP_ITERATION())
            , kw
            , policies
            , doc
            , &name_space);
