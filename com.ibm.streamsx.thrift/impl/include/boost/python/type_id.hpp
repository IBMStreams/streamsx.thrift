...
#ifndef TYPE_ID_DWA2002517_STREAMS_HPP
# define TYPE_ID_DWA2002517_STREAMS_HPP

# include <streams_boost/python/detail/prefix.hpp>

# include <streams_boost/python/detail/msvc_typeinfo.hpp>
...
#  ifndef STREAMS_BOOST_PYTHON_HAVE_GCC_CP_DEMANGLE
#   if defined(__GNUC__)                                                \
    && ((__GNUC__ > 3) || ((__GNUC__ == 3) && (__GNUC_MINOR__ >= 1)))   \
    && !defined(__EDG_VERSION__)
#    define STREAMS_BOOST_PYTHON_HAVE_GCC_CP_DEMANGLE
#   endif
#  endif
...
#  define STREAMS_BOOST_PYTHON_TYPE_ID_NAME
# endif 

#ifdef STREAMS_BOOST_PYTHON_HAVE_GCC_CP_DEMANGLE
// Runtime detection of broken cxxabi::__cxa_demangle versions,
// to avoid #ifdef clutter.
bool cxxabi_cxa_demangle_is_broken();
#define STREAMS_BOOST_PYTHON_HAVE_CXXABI_CXA_DEMANGLE_IS_BROKEN
#endif
...
struct type_info : private totally_ordered<type_info>
{
    ...
    friend STREAMS_BOOST_PYTHON_DECL std::ostream& operator<<(
        std::ostream&, type_info const&);
    
 private: // data members
#  ifdef STREAMS_BOOST_PYTHON_TYPE_ID_NAME
...
#   define STREAMS_BOOST_PYTHON_EXPLICIT_TT_DEF(T) ::streams_boost::type<T>*
#  else
#   define STREAMS_BOOST_PYTHON_EXPLICIT_TT_DEF(T)
#  endif

template <class T>
inline type_info type_id(STREAMS_BOOST_EXPLICIT_TEMPLATE_TYPE(T))
{
    return type_info(
#  if !defined(_MSC_VER)                                       \
      || (!STREAMS_BOOST_WORKAROUND(STREAMS_BOOST_MSVC, <= 1300)                \
          && !STREAMS_BOOST_WORKAROUND(STREAMS_BOOST_INTEL_CXX_VERSION, <= 700))
        typeid(T)
#  else // strip the decoration which msvc and Intel mistakenly leave in
        python::detail::msvc_typeid((streams_boost::type<T>*)0)
#  endif 
        );
}
...
#   define STREAMS_BOOST_PYTHON_SIGNED_INTEGRAL_TYPE_ID(T)              \
template <>                                                     \
inline type_info type_id<T>(STREAMS_BOOST_PYTHON_EXPLICIT_TT_DEF(T))    \
{                                                               \
    return type_info(typeid(T));                                \
}

STREAMS_BOOST_PYTHON_SIGNED_INTEGRAL_TYPE_ID(short)
STREAMS_BOOST_PYTHON_SIGNED_INTEGRAL_TYPE_ID(int)
STREAMS_BOOST_PYTHON_SIGNED_INTEGRAL_TYPE_ID(long)
// using Python's macro instead of Boost's - we don't seem to get the
// config right all the time.
#   ifdef HAVE_LONG_LONG
STREAMS_BOOST_PYTHON_SIGNED_INTEGRAL_TYPE_ID(long long)
#   endif
#   undef STREAMS_BOOST_PYTHON_SIGNED_INTEGRAL_TYPE_ID
#  endif
...
namespace detail
{
  STREAMS_BOOST_PYTHON_DECL char const* gcc_demangle(char const*);
}
#  endif
...
inline char const* type_info::name() const
{
    char const* raw_name
        = m_base_type
#  ifndef STREAMS_BOOST_PYTHON_TYPE_ID_NAME
          ->name()
#  endif
        ;
...
STREAMS_BOOST_PYTHON_DECL std::ostream& operator<<(std::ostream&, type_info const&);

#  if !STREAMS_BOOST_WORKAROUND(STREAMS_BOOST_MSVC, == 1200)
template<>
inline type_info type_id<void>(STREAMS_BOOST_PYTHON_EXPLICIT_TT_DEF(void))
{
    return type_info (typeid (void *));
}
#   ifndef STREAMS_BOOST_NO_CV_VOID_SPECIALIZATIONS
template<>
inline type_info type_id<const volatile void>(STREAMS_BOOST_PYTHON_EXPLICIT_TT_DEF(const volatile void))
{
    return type_info (typeid (void *));
}
#  endif
...
#endif // TYPE_ID_DWA2002517_STREAMS_HPP
