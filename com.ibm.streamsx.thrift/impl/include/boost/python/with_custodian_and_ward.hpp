...
#ifndef WITH_CUSTODIAN_AND_WARD_DWA2002131_STREAMS_HPP
# define WITH_CUSTODIAN_AND_WARD_DWA2002131_STREAMS_HPP

# include <streams_boost/python/detail/prefix.hpp>

# include <streams_boost/python/default_call_policies.hpp>
# include <streams_boost/python/object/life_support.hpp>
# include <algorithm>
...
{
    STREAMS_BOOST_STATIC_ASSERT(custodian != ward);
    STREAMS_BOOST_STATIC_ASSERT(custodian > 0);
    STREAMS_BOOST_STATIC_ASSERT(ward > 0);
    ...
    static bool precall(ArgumentPackage const& args_)
    {
        ...
        if (custodian > arity_ || ward > arity_)
        {
            PyErr_SetString(
                PyExc_IndexError
              , "streams_boost::python::with_custodian_and_ward: argument index out of range"
            );
            return false;
        }
...
struct with_custodian_and_ward_postcall : BasePolicy_
{
    ...
    static PyObject* postcall(ArgumentPackage const& args_, PyObject* result)
    {
...
#endif
        {
            PyErr_SetString(
                PyExc_IndexError
              , "streams_boost::python::with_custodian_and_ward_postcall: argument index out of range"
            );
            return 0;
        }
...
#endif // WITH_CUSTODIAN_AND_WARD_DWA2002131_STREAMS_HPP
