...
// STREAMS_BOOST_PYTHON_OPAQUE_SPECIALIZED_TYPE_ID(Pointee)
//
...
PyTypeObject opaque<Pointee>::type_object =
{
    ...
    0,
    sizeof( STREAMS_BOOST_DEDUCED_TYPENAME opaque<Pointee>::python_instance ),
    0,
    ::streams_boost::python::detail::dealloc,
    0,          /* tp_print */
    0,          /* tp_getattr */
    0,          /* tp_setattr */
    0,          /* tp_compare */
...
#  define STREAMS_BOOST_PYTHON_OPAQUE_SPECIALIZED_TYPE_ID(Pointee)
...
#   define STREAMS_BOOST_PYTHON_OPAQUE_SPECIALIZED_TYPE_ID(Pointee)                     \
    namespace streams_boost { namespace python {                                        \
    template<>                                                                  \
    inline type_info type_id<Pointee>(STREAMS_BOOST_PYTHON_EXPLICIT_TT_DEF(Pointee))    \
    {                                                                           \
        return type_info (typeid (Pointee *));                                  \
    }                                                                           \
    template<>                                                                  \
    inline type_info type_id<const volatile Pointee&>(                          \
        STREAMS_BOOST_PYTHON_EXPLICIT_TT_DEF(const volatile Pointee&))                  \
    {                                                                           \
        return type_info (typeid (Pointee *));                                  \
    }                                                                           \
    }}
