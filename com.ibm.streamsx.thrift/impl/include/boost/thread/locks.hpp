...
#ifndef STREAMS_BOOST_THREAD_LOCKS_HPP
#define STREAMS_BOOST_THREAD_LOCKS_HPP
...
#define STREAMS_BOOST_THREAD_NO_AUTO_DETECT_MUTEX_TYPES
#endif

#ifndef STREAMS_BOOST_THREAD_NO_AUTO_DETECT_MUTEX_TYPES
    namespace detail
    {
        template<typename T>
        struct has_member_lock
        {
            ...
            STREAMS_BOOST_STATIC_CONSTANT(bool, value=sizeof(has_member_lock<T>::has_member((T*)NULL))==sizeof(true_type));
        };
        ...
        struct has_member_unlock
        {
            ...
            STREAMS_BOOST_STATIC_CONSTANT(bool, value=sizeof(has_member_unlock<T>::has_member((T*)NULL))==sizeof(true_type));
        };
        ...
        struct has_member_try_lock
        {
            ...
            STREAMS_BOOST_STATIC_CONSTANT(bool, value=sizeof(has_member_try_lock<T>::has_member((T*)NULL))==sizeof(true_type));
        };
    ...
    {
        STREAMS_BOOST_STATIC_CONSTANT(bool, value = detail::has_member_lock<T>::value &&
...
#ifdef STREAMS_BOOST_THREAD_NO_AUTO_DETECT_MUTEX_TYPES
    template<typename T>
    ...
    public:
        ...
        void lock()
        {
            if(owns_lock())
            {
                throw streams_boost::lock_error();
            }
            m->lock();
            is_locked=true;
        }
        ...
        bool timed_lock(::streams_boost::system_time const& absolute_time)
        {
    ...
    public:
        ...
        void swap(streams_boost::detail::thread_move_t<shared_lock<Mutex> > other)
        {
            std::swap(m,other->m);
            std::swap(is_locked,other->is_locked);
        }
        ...
        bool timed_lock(streams_boost::system_time const& target_time)
        {
            if(owns_lock())
            {
                throw streams_boost::lock_error();
    ...
    namespace detail
    {
        ...
        {
            streams_boost::unique_lock<MutexType1> l1(m1,streams_boost::try_to_lock);
            if(!l1)
            {
                return 1;
            }
        ...
        {
            streams_boost::unique_lock<MutexType1> l1(m1);
            if(!m2.try_lock())
            {
                return 1;
            }
