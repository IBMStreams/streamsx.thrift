#ifndef STREAMS_BOOST_THREAD_WIN32_ONCE_HPP
#define STREAMS_BOOST_THREAD_WIN32_ONCE_HPP
...
#include <streams_boost/thread/win32/interlocked_read.hpp>
...
#define STREAMS_BOOST_ONCE_INIT 0
    ...
    namespace detail
    {
        struct win32_mutex_scoped_lock
        {
            ...
            {
                STREAMS_BOOST_VERIFY(!win32::WaitForSingleObject(mutex_handle,win32::infinite));
            }
            ~win32_mutex_scoped_lock()
            {
                STREAMS_BOOST_VERIFY(win32::ReleaseMutex(mutex_handle)!=0);
            }
        private:
            void operator=(win32_mutex_scoped_lock&);
        };

#ifdef STREAMS_BOOST_NO_ANSI_APIS
        template <class I>
        ...
        inline void* create_once_mutex(void* flag_address)
        {
            ...
            STREAMS_BOOST_STATIC_ASSERT(sizeof(void*) == sizeof(std::ptrdiff_t));
    ...
    void call_once(once_flag& flag,Function f)
    {
        ...
        if(::streams_boost::detail::interlocked_read_acquire(&flag)!=function_complete_flag_value)
        {
            void* const mutex_handle(::streams_boost::detail::create_once_mutex(&flag));
            STREAMS_BOOST_ASSERT(mutex_handle);
            ...
            {
                f();
                STREAMS_BOOST_INTERLOCKED_EXCHANGE(&flag,function_complete_flag_value);
            }
        }
    }
}
