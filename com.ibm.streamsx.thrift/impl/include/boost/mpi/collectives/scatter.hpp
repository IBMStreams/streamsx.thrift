...
#ifndef STREAMS_BOOST_MPI_SCATTER_HPP
#define STREAMS_BOOST_MPI_SCATTER_HPP
...
namespace detail {
               ...
               int n, int root, mpl::true_)
  {
    ...
    STREAMS_BOOST_MPI_CHECK_RESULT(MPI_Scatter,
...
{
  if (comm.rank() == root)
    ::streams_boost::mpi::scatter<T>(comm, &in_values[0], out_value, root);
  else
    ::streams_boost::mpi::scatter<T>(comm, static_cast<const T*>(0), out_value, 
                             root);
}
...
{
  if (comm.rank() == root)
    ::streams_boost::mpi::scatter(comm, &in_values[0], out_values, n, root);
  else
    ::streams_boost::mpi::scatter(comm, static_cast<const T*>(0), out_values, 
                                    n, root);
}
...
#endif // STREAMS_BOOST_MPI_SCATTER_HPP
