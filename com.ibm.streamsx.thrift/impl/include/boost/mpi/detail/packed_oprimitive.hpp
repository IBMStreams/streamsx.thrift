...
#ifndef STREAMS_BOOST_MPI_PACKED_OPRIMITIVE_HPP
#define STREAMS_BOOST_MPI_PACKED_OPRIMITIVE_HPP
...
class STREAMS_BOOST_MPI_DECL packed_oprimitive
{
public:
...
private:
    ...
    void save_impl(void const * p, MPI_Datatype t, int l)
      ...
      int memory_needed;
      STREAMS_BOOST_MPI_CHECK_RESULT(MPI_Pack_size,(l,t,comm,&memory_needed));

         int position = buffer_.size();
          buffer_.resize(position + memory_needed);

          // pack the data into the buffer
          STREAMS_BOOST_MPI_CHECK_RESULT(MPI_Pack,
        (const_cast<void*>(p), l, t, streams_boost::serialization::detail::get_data(buffer_), buffer_.size(), &position, comm));

          // reduce the buffer size if needed
          STREAMS_BOOST_ASSERT(std::size_t(position) <= buffer_.size());
...
#endif // STREAMS_BOOST_MPI_PACKED_OPRIMITIVE_HPP
