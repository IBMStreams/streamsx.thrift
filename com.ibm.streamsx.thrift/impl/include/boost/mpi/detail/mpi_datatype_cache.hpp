...
#ifndef STREAMS_BOOST_MPI_DETAIL_TYPE_MPI_DATATYPE_CACHE_HPP
#define STREAMS_BOOST_MPI_DETAIL_TYPE_MPI_DATATYPE_CACHE_HPP

#include <streams_boost/mpi/datatype_fwd.hpp>
#include <streams_boost/mpi/detail/mpi_datatype_oarchive.hpp>
...
///
///
class STREAMS_BOOST_MPI_DECL mpi_datatype_map
 : public streams_boost::noncopyable
{
...
public:
  ...
  template <class T>
  MPI_Datatype datatype(const T& x = T(), typename streams_boost::enable_if<is_mpi_builtin_datatype<T> >::type* =0)
  {
    return get_mpi_datatype<T>(x);
  }

  template <class T>
  MPI_Datatype datatype(const T& x =T(), typename streams_boost::disable_if<is_mpi_builtin_datatype<T> >::type* =0 )
  {
...
STREAMS_BOOST_MPI_DECL mpi_datatype_map& mpi_datatype_cache();
...
#endif // STREAMS_BOOST_MPI_DETAIL_TYPE_MPI_DATATYPE_CACHE_HPP
