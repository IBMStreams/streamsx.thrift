 ...
 */
#ifndef STREAMS_BOOST_MPI_COMMUNICATOR_HPP
#define STREAMS_BOOST_MPI_COMMUNICATOR_HPP
...
#include <streams_boost/mpi/skeleton_and_content_fwd.hpp>
...
#include <streams_boost/mpi/status.hpp>
#include <streams_boost/mpi/request.hpp>
 ...
 * in the communicator.
 */
class STREAMS_BOOST_MPI_DECL communicator
{
 public:
  /**
   ...
   */
  communicator(const communicator& comm, const streams_boost::mpi::group& subgroup);
   ...
   */
  streams_boost::mpi::group group() const;
 ...
 protected:
  ...
  struct comm_free
  {
    void operator()(MPI_Comm* comm) const
    {
      ...
      if (!finalized)
        STREAMS_BOOST_MPI_CHECK_RESULT(MPI_Comm_free, (comm));
      delete comm;
    }
  };
 ...
 */
STREAMS_BOOST_MPI_DECL bool operator==(const communicator& comm1, const communicator& comm2);
...
optional<int> status::count_impl(mpl::true_) const
{
  ...
  int return_value;
  STREAMS_BOOST_MPI_CHECK_RESULT(MPI_Get_count,
...
{
  STREAMS_BOOST_MPI_CHECK_RESULT(MPI_Send,
...
{
  packed_oarchive oa(*this);
  oa << n << streams_boost::serialization::make_array(values, n);
  send(dest, tag, oa);
}
...
status communicator::recv_impl(int source, int tag, T& value, mpl::true_) const
{
  ...
  STREAMS_BOOST_MPI_CHECK_RESULT(MPI_Recv,
                              ...
                              mpl::false_) const
{
  ...
  // Deserialize the data in the message
  streams_boost::serialization::array<T> arr(values, count > n? n : count);
  ia >> arr;
...
{
  request req;
  STREAMS_BOOST_MPI_CHECK_RESULT(MPI_Isend,
                         (const_cast<T*>(&value), 1, 
                          get_mpi_datatype<T>(value),
...
{
  shared_ptr<packed_oarchive> archive(new packed_oarchive(*this));
  *archive << n << streams_boost::serialization::make_array(values, n);
  request result = isend(dest, tag, *archive);
  result.m_data = archive;
  return result;
}
...
request::handle_serialized_irecv(request* self, request_action action)
{
  ...
  if (action == ra_wait) {
    ...
    if (self->m_requests[1] == MPI_REQUEST_NULL) {
      ...
      STREAMS_BOOST_MPI_CHECK_RESULT(MPI_Test,
 ...
 */
template<>
STREAMS_BOOST_MPI_DECL request
...
#ifdef STREAMS_BOOST_MPI_SKELETON_AND_CONTENT_HPP
...
#endif // STREAMS_BOOST_MPI_COMMUNICATOR_HPP
