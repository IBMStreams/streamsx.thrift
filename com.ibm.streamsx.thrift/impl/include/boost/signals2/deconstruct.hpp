#ifndef STREAMS_BOOST_SIGNALS2_DECONSTRUCT_HPP
#define STREAMS_BOOST_SIGNALS2_DECONSTRUCT_HPP
...
//
//  See http://www.streams_boost.org
//  for more information

#include <streams_boost/config.hpp>
#include <streams_boost/shared_ptr.hpp>
#include <streams_boost/signals2/deconstruct_ptr.hpp>
...
#if !defined(STREAMS_BOOST_NO_RVALUE_REFERENCES)
...
public:
    ...
    const shared_ptr<T>& postconstruct() const
    {
        if(!_postconstructed)
        {
            adl_postconstruct(_sp, const_cast<typename streams_boost::remove_const<T>::type *>(_sp.get()));
            _postconstructed = true;
        }
        return _sp;
    }
#if !defined(STREAMS_BOOST_NO_VARIADIC_TEMPLATES) && !defined(STREAMS_BOOST_NO_RVALUE_REFERENCES)
    template<class... Args>
...
#else // !defined(STREAMS_BOOST_NO_VARIADIC_TEMPLATES) && !defined(STREAMS_BOOST_NO_RVALUE_REFERENCES)
...
#endif // !defined(STREAMS_BOOST_NO_VARIADIC_TEMPLATES) && !defined(STREAMS_BOOST_NO_RVALUE_REFERENCES)
private:
    friend class streams_boost::signals2::deconstruct_access;
...
private:
    ...
    void destroy()
    {
        if( initialized_ )
        {
            ...
            using streams_boost::signals2::detail::adl_predestruct;
            adl_predestruct(const_cast<typename streams_boost::remove_const<T>::type *>(p));
            p->~T();
            initialized_ = false;
        }
    }
...
public:
    ...
    {
        streams_boost::shared_ptr< T > pt( static_cast< T* >( 0 ), detail::deconstruct_deleter< T >() );

        detail::deconstruct_deleter< T > * pd = streams_boost::get_deleter< detail::deconstruct_deleter< T > >( pt );
        ...
        streams_boost::shared_ptr< T > retval( pt, static_cast< T* >( pv ) );
        streams_boost::detail::sp_enable_shared_from_this(&retval, retval.get(), retval.get());
        return retval;
...
#endif // #ifndef STREAMS_BOOST_SIGNALS2_DECONSTRUCT_HPP
