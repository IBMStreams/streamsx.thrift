...
#ifndef STREAMS_BOOST_SIGNALS2_DETAIL_AUTO_BUFFER_HPP_25_02_2009
#define STREAMS_BOOST_SIGNALS2_DETAIL_AUTO_BUFFER_HPP_25_02_2009
...
#include <streams_boost/swap.hpp>
...
namespace detail
{
    ...
    {
        STREAMS_BOOST_STATIC_CONSTANT( unsigned, value = N );
    };
    ...
    namespace auto_buffer_detail
    {
        ...
        {
            STREAMS_BOOST_STATIC_CONSTANT( unsigned, value = Policy::value * sizeof(T) );
        };
        ...
        {
            STREAMS_BOOST_STATIC_CONSTANT( unsigned, value = Policy::value );
        };

        template< unsigned N, class T >
        struct compute_buffer_objects< store_n_bytes<N>, T >
        {
            STREAMS_BOOST_STATIC_CONSTANT( unsigned, value = N / sizeof(T) );
        };
    }
    ...
    private:
        ...
        STREAMS_BOOST_STATIC_CONSTANT( bool, is_stack_buffer_empty = N == 0u );
    ...
    public:
        ...
        typedef streams_boost::reverse_iterator<iterator>        reverse_iterator;
        typedef streams_boost::reverse_iterator<const_iterator>  const_reverse_iterator;
        typedef typename streams_boost::mpl::if_c< streams_boost::has_trivial_assign<T>::value
    ...
    private:
        ...
        {
            copy_rai( begin, end, where, streams_boost::has_trivial_assign<T>() );
        }

        static void copy_rai( const T* begin, const T* end,
                              pointer where, const streams_boost::true_type& )
        {
        ...
        static void copy_rai( I begin, I end,
                              pointer where, const streams_boost::integral_constant<bool, b>& )
        {
        ...
        {
            assign_impl( begin, end, where, streams_boost::has_trivial_assign<T>() );
        }

        template< class I, class I2 >
        static void assign_impl( I begin, I end, I2 where, const streams_boost::true_type& )
        {
            std::memcpy( where, begin, sizeof(T) * std::distance(begin,end) );
        }

        template< class I, class I2 >
        static void assign_impl( I begin, I end, I2 where, const streams_boost::false_type& )
        {
            for( ; begin != end; ++begin, ++where )
                *where = *begin;
        }

        void unchecked_push_back_n( size_type n, const streams_boost::true_type& )
        {
            std::uninitialized_fill( end(), end() + n, T() );
            size_ += n;
        }

        void unchecked_push_back_n( size_type n, const streams_boost::false_type& )
        {
            for( size_type i = 0u; i < n; ++i )
                unchecked_push_back();
        }

        void auto_buffer_destroy( pointer where, const streams_boost::false_type& )
        {
            (*where).~T();
        }

        void auto_buffer_destroy( pointer, const streams_boost::true_type& )
        { }

        void auto_buffer_destroy( pointer where )
        {
            auto_buffer_destroy( where, streams_boost::has_trivial_destructor<T>() );
        }

        void destroy_back_n( size_type n, const streams_boost::false_type& )
        {
            STREAMS_BOOST_ASSERT( n > 0 );
        ...
        void destroy_back_n( size_type n, const streams_boost::true_type& )
        { }

        void destroy_back_n( size_type n )
        {
            destroy_back_n( n, streams_boost::has_trivial_destructor<T>() );
        }

        void auto_buffer_destroy( const streams_boost::false_type& x )
        {
            if( size_ )
        ...
        void auto_buffer_destroy( const streams_boost::true_type& )
        {
            deallocate( buffer_, members_.capacity_ );
        }

        pointer move_to_new_buffer( size_type new_capacity, const streams_boost::false_type& )
        {
            pointer new_buffer = allocate( new_capacity ); // strong
            streams_boost::multi_index::detail::scope_guard guard =
                streams_boost::multi_index::detail::make_obj_guard( *this,
        ...
        pointer move_to_new_buffer( size_type new_capacity, const streams_boost::true_type& )
        {
            pointer new_buffer = allocate( new_capacity ); // strong
        ...
        void reserve_impl( size_type new_capacity )
        {
            pointer new_buffer = move_to_new_buffer( new_capacity,
                                                 streams_boost::has_nothrow_copy<T>() );
            (*this).~auto_buffer();
            buffer_   = new_buffer;
            members_.capacity_ = new_capacity;
            STREAMS_BOOST_ASSERT( size_ <= members_.capacity_ );
        }

        size_type new_capacity_impl( size_type n )
        {
            STREAMS_BOOST_ASSERT( n > members_.capacity_ );
        ...
        static void swap_helper( auto_buffer& l, auto_buffer& r,
                                 const streams_boost::true_type& )
        {
            STREAMS_BOOST_ASSERT( l.is_on_stack() && r.is_on_stack() );

            auto_buffer temp( l.begin(), l.end() );
            assign_impl( r.begin(), r.end(), l.begin() );
            assign_impl( temp.begin(), temp.end(), r.begin() );
            streams_boost::swap( l.size_, r.size_ );
            streams_boost::swap( l.members_.capacity_, r.members_.capacity_ );
        }

        static void swap_helper( auto_buffer& l, auto_buffer& r,
                                 const streams_boost::false_type& )
        {
            STREAMS_BOOST_ASSERT( l.is_on_stack() && r.is_on_stack() );
            ...
            size_type i = 0u;
            for(  ; i < min_size; ++i )
                streams_boost::swap( (*smallest)[i], (*largest)[i] );
        ...
        void grow_back( size_type n, const streams_boost::true_type& )
        {
            STREAMS_BOOST_ASSERT( size_ + n <= members_.capacity_ );
            size_ += n;
        }

        void grow_back( size_type n, const streams_boost::false_type& )
        {
        ...
        {
            grow_back( n, streams_boost::has_trivial_constructor<T>() );
        }

        void grow_back_one( const streams_boost::true_type& )
        {
            STREAMS_BOOST_ASSERT( size_ + 1 <= members_.capacity_ );
            size_ += 1;
        }

        void grow_back_one( const streams_boost::false_type& )
        {
            unchecked_push_back();
        }
        ...
        void grow_back_one()
        {
            grow_back_one( streams_boost::has_trivial_constructor<T>() );
        }
    ...
    public:
        ...
        auto_buffer& operator=( const auto_buffer& r ) // basic
        {
            ...
            STREAMS_BOOST_ASSERT( size() == r.size() );
            STREAMS_BOOST_ASSERT( is_valid() );
            return *this;
        }
        ...
        ~auto_buffer()
        {
            ...
            if( buffer_ ) // do we need this check? Yes, but only
                ...
                auto_buffer_destroy( streams_boost::has_trivial_destructor<T>() );
        }
    ...
    public:
        ...
        {
            STREAMS_BOOST_ASSERT( size_ + n <= members_.capacity_ );
            unchecked_push_back_n( n, streams_boost::has_trivial_assign<T>() );
        }
        ...
        {
            STREAMS_BOOST_ASSERT( size_ + std::distance(begin,end) <= members_.capacity_ );
        ...
        {
            STREAMS_BOOST_ASSERT( members_.capacity_  >= N );
        ...
        void pop_back()
        {
            ...
            auto_buffer_destroy( buffer_ + size_ - 1, streams_boost::has_trivial_destructor<T>() );
            --size_;
        }
        ...
        {
            STREAMS_BOOST_ASSERT( !empty() );
            STREAMS_BOOST_ASSERT( cbegin() <= where );
            STREAMS_BOOST_ASSERT( cend() > where );
            ...
            STREAMS_BOOST_ASSERT( result <= end() );
            return result;
        }

        iterator erase( const_iterator from, const_iterator to )
        {
            STREAMS_BOOST_ASSERT( !(std::distance(from,to)>0) ||
                          !empty() );
            STREAMS_BOOST_ASSERT( cbegin() <= from );
            STREAMS_BOOST_ASSERT( cend() >= to );

            unsigned elements = std::distance(to,cend());

            if( elements > 0u )
            {
                STREAMS_BOOST_ASSERT( elements > 0u );
        ...
        void shrink_to_fit()
        {
            ...
            STREAMS_BOOST_ASSERT( is_on_stack() || size_ == members_.capacity_ );
            STREAMS_BOOST_ASSERT( !is_on_stack() || size_ <= members_.capacity_ );
        }
        ...
        void swap( auto_buffer& r )
        {
            ...
            if( both_on_heap )
            {
                streams_boost::swap( get_allocator(), r.get_allocator() );
                streams_boost::swap( members_.capacity_, r.members_.capacity_ );
                streams_boost::swap( buffer_, r.buffer_ );
                streams_boost::swap( size_, r.size_ );
                STREAMS_BOOST_ASSERT( is_valid() );
                STREAMS_BOOST_ASSERT( r.is_valid() );
                return;
            }

            STREAMS_BOOST_ASSERT( on_stack || r_on_stack );
            ...
            if( exactly_one_on_stack )
            {
                ...
                streams_boost::swap( get_allocator(), r.get_allocator() );  // assume nothrow
                streams_boost::swap( members_.capacity_, r.members_.capacity_ );
                streams_boost::swap( size_, r.size_ );
                one_on_stack->buffer_ = other->buffer_;
                other->buffer_        = new_buffer;
                STREAMS_BOOST_ASSERT( other->is_on_stack() );
                STREAMS_BOOST_ASSERT( !one_on_stack->is_on_stack() );
                STREAMS_BOOST_ASSERT( is_valid() );
                STREAMS_BOOST_ASSERT( r.is_valid() );
                return;
            }
            ...
            STREAMS_BOOST_ASSERT( on_stack && r_on_stack );
            swap_helper( *this, r, streams_boost::has_trivial_assign<T>() );
            STREAMS_BOOST_ASSERT( is_valid() );
            STREAMS_BOOST_ASSERT( r.is_valid() );
        }

    private:
        typedef streams_boost::aligned_storage< N * sizeof(T),
