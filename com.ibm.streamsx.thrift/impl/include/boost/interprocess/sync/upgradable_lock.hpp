...
#ifndef STREAMS_BOOST_INTERPROCESS_UPGRADABLE_LOCK_HPP
#define STREAMS_BOOST_INTERPROCESS_UPGRADABLE_LOCK_HPP
...
class upgradable_lock
{
   ...
   STREAMS_BOOST_INTERPROCESS_MOVABLE_BUT_NOT_COPYABLE(upgradable_lock)
   /// @endcond
   public:
   ...
   upgradable_lock(mutex_type& m, const streams_boost::posix_time::ptime& abs_time)
      : mp_mutex(&m), m_locked(false)
   ...
   //!   expression: "streams_boost::interprocess::move(lock);". This constructor does not alter the
   //!   state of the mutex, only potentially who owns it.
   upgradable_lock(STREAMS_BOOST_INTERPROCESS_RV_REF(upgradable_lock<mutex_type>) upgr)
   ...
   //!   expression: "streams_boost::interprocess::move(lock);".
   template<class T>
   upgradable_lock(STREAMS_BOOST_INTERPROCESS_RV_REF(scoped_lock<T>) scop
   ...
   template<class T>
   upgradable_lock( STREAMS_BOOST_INTERPROCESS_RV_REF(sharable_lock<T>) shar, try_to_lock_type
                  , typename detail::enable_if< detail::is_same<T, UpgradableMutex> >::type * = 0)
      : mp_mutex(0), m_locked(false)
   {
   ...
   upgradable_lock &operator=(STREAMS_BOOST_INTERPROCESS_RV_REF(upgradable_lock) upgr)
   {
      if(this->owns())
         this->unlock();
      m_locked = upgr.owns();
...
#endif // STREAMS_BOOST_INTERPROCESS_UPGRADABLE_LOCK_HPP
