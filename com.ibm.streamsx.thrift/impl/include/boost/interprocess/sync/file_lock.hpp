...
#ifndef STREAMS_BOOST_INTERPROCESS_FILE_LOCK_HPP
#define STREAMS_BOOST_INTERPROCESS_FILE_LOCK_HPP
...
class file_lock
{
   /// @cond
   //Non-copyable
   STREAMS_BOOST_INTERPROCESS_MOVABLE_BUT_NOT_COPYABLE(file_lock)
   /// @endcond
   ...
   //!Does not throw
   file_lock(STREAMS_BOOST_INTERPROCESS_RV_REF(file_lock) moved)
      :  m_file_hnd(file_handle_t(detail::invalid_file()))
   {  this->swap(moved);   }
   ...
   //!Does not throw
   file_lock &operator=(STREAMS_BOOST_INTERPROCESS_RV_REF(file_lock) moved)
   {  
      file_lock tmp(streams_boost::interprocess::move(moved));
      this->swap(tmp);
      return *this;  
   }
   ...
   bool timed_acquire_file_lock
      (file_handle_t hnd, bool &acquired, const streams_boost::posix_time::ptime &abs_time)
   {
      //Obtain current count and target time
      streams_boost::posix_time::ptime now = microsec_clock::universal_time();
      using namespace streams_boost::detail;
...
inline bool file_lock::timed_lock(const streams_boost::posix_time::ptime &abs_time)
{
...
inline bool file_lock::timed_lock_sharable(const streams_boost::posix_time::ptime &abs_time)
{
   if(abs_time == streams_boost::posix_time::pos_infin){
      this->lock_sharable();
      return true;
...
#endif   //STREAMS_BOOST_INTERPROCESS_FILE_LOCK_HPP
