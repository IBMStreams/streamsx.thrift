...
#ifndef STREAMS_BOOST_INTERPROCESS_SHARABLE_LOCK_HPP
#define STREAMS_BOOST_INTERPROCESS_SHARABLE_LOCK_HPP
...
class sharable_lock
{
   ...
   STREAMS_BOOST_INTERPROCESS_MOVABLE_BUT_NOT_COPYABLE(sharable_lock)
   /// @endcond
   public:
   ...
   sharable_lock(mutex_type& m, const streams_boost::posix_time::ptime& abs_time)
      : mp_mutex(&m), m_locked(false)
   ...
   //!   "streams_boost::interprocess::move(lock);". This constructor does not alter the state of the mutex,
   //!   only potentially who owns it.
   sharable_lock(STREAMS_BOOST_INTERPROCESS_RV_REF(sharable_lock<mutex_type>) upgr)
   ...
   //!   "streams_boost::interprocess::move(lock);".*/
   template<class T>
   sharable_lock(STREAMS_BOOST_INTERPROCESS_RV_REF(upgradable_lock<T>) upgr
   ...
   //!   "streams_boost::interprocess::move(lock);".
   template<class T>
   sharable_lock(STREAMS_BOOST_INTERPROCESS_RV_REF(scoped_lock<T>) scop
               , typename detail::enable_if< detail::is_same<T, SharableMutex> >::type * = 0)
      : mp_mutex(0), m_locked(false)
   {
   ...
   sharable_lock &operator=(STREAMS_BOOST_INTERPROCESS_RV_REF(sharable_lock<mutex_type>) upgr)
   {  
      if(this->owns())
         this->unlock();
      m_locked = upgr.owns();
...
#endif // STREAMS_BOOST_INTERPROCESS_SHARABLE_LOCK_HPP
