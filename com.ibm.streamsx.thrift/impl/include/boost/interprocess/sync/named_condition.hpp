...
#ifndef STREAMS_BOOST_INTERPROCESS_NAMED_CONDITION_HPP
#define STREAMS_BOOST_INTERPROCESS_NAMED_CONDITION_HPP
...
#include <streams_boost/interprocess/shared_memory_object.hpp>
#include <streams_boost/interprocess/sync/interprocess_condition.hpp>
#include <streams_boost/interprocess/detail/managed_open_or_create_impl.hpp>
#include <streams_boost/interprocess/detail/posix_time_types_wrk.hpp>
#include <streams_boost/interprocess/sync/emulation/named_creation_functor.hpp>
#include <streams_boost/interprocess/sync/named_mutex.hpp>
#if defined STREAMS_BOOST_INTERPROCESS_NAMED_MUTEX_USES_POSIX_SEMAPHORES
...
class named_condition
{
   ...
   void do_wait(Lock& lock)
   {
      ...
      STREAMS_BOOST_STATIC_ASSERT((detail::is_convertible<typename Lock::mutex_type&, named_mutex&>::value == true));
   ...
   template <class Lock>
   bool do_timed_wait(Lock& lock, const streams_boost::posix_time::ptime &abs_time)
   {
      //named_condition only works with named_mutex
      STREAMS_BOOST_STATIC_ASSERT((detail::is_convertible<typename Lock::mutex_type&, named_mutex&>::value == true));
      //lock internal before unlocking external to avoid race with a notifier  
   ...
   template <class T, class Arg> friend class streams_boost::interprocess::detail::named_creation_functor;
...
inline bool named_condition::timed_wait
   (L& lock, const streams_boost::posix_time::ptime &abs_time)
{
...
template <typename L, typename Pr>
inline bool named_condition::timed_wait
   (L& lock, const streams_boost::posix_time::ptime &abs_time, Pr pred)
{
   if(abs_time == streams_boost::posix_time::pos_infin){
      this->wait(lock, pred);
      return true;
...
#endif // STREAMS_BOOST_INTERPROCESS_NAMED_CONDITION_HPP
