...
{  detail::atomic_write32(&this->m_count, streams_boost::uint32_t(initialCount));  }
...
{
   while(!detail::atomic_add_unless32(&m_count, streams_boost::uint32_t(-1), streams_boost::uint32_t(0))){
...
{
   return detail::atomic_add_unless32(&m_count, streams_boost::uint32_t(-1), streams_boost::uint32_t(0));
}
