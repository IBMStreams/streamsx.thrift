...
inline void interprocess_condition::notify(streams_boost::uint32_t command)
{
   ...
   while(SLEEP != detail::atomic_cas32(const_cast<streams_boost::uint32_t*>(&m_command), command, SLEEP)){
      detail::thread_yield();
   }
/*
   //Wait until the threads are woken
   while(SLEEP != detail::atomic_cas32(const_cast<streams_boost::uint32_t*>(&m_command), 0)){
      detail::thread_yield();
   }
*/
...
{
   this->do_timed_wait(false, streams_boost::posix_time::ptime(), mut);
}

inline bool interprocess_condition::do_timed_wait
   (const streams_boost::posix_time::ptime &abs_time, interprocess_mutex &mut)
{
   return this->do_timed_wait(true, abs_time, mut);
}

inline bool interprocess_condition::do_timed_wait(bool tout_enabled,
                                     const streams_boost::posix_time::ptime &abs_time, 
                                     interprocess_mutex &mut)
{
   ...
   typedef streams_boost::interprocess::scoped_lock<interprocess_mutex> InternalLock;
   ...
   //no other thread can execute the do_timed_wait method. 
   {
      ...
      if(tout_enabled){
         ...
         lock = streams_boost::interprocess::move(dummy);
      }
      else{
      ...
      detail::atomic_inc32(const_cast<streams_boost::uint32_t*>(&m_num_waiters));
   ...
   while(1){
      ...
      if(tout_enabled && timed_out){
         ...
         detail::atomic_dec32(const_cast<streams_boost::uint32_t*>(&m_num_waiters));
         ...
         */
         ...
         streams_boost::uint32_t result = detail::atomic_cas32
                        (const_cast<streams_boost::uint32_t*>(&m_command), SLEEP, NOTIFY_ONE);
         if(result == SLEEP){
            ...
            unlock_enter_mut = 1 == detail::atomic_dec32(const_cast<streams_boost::uint32_t*>(&m_num_waiters));
            //Check if this is the last thread of notify_all waiters
            //Only the last thread will release the interprocess_mutex
            if(unlock_enter_mut){
               detail::atomic_cas32(const_cast<streams_boost::uint32_t*>(&m_command), SLEEP, NOTIFY_ALL);
            }
            break;
         }
      }
