...
#ifndef STREAMS_BOOST_CONTAINERS_STRING_HPP
#define STREAMS_BOOST_CONTAINERS_STRING_HPP
...
{
   basic_string_base();
   STREAMS_BOOST_INTERPROCESS_MOVABLE_BUT_NOT_COPYABLE(basic_string_base)
 ...
 public:
   ...
   basic_string_base(STREAMS_BOOST_INTERPROCESS_RV_REF(basic_string_base) b)
      :  members_(b.members_)
   {  
      init();
      this->swap(b); 
...
//      long_alignment_type;
   ...
   STREAMS_BOOST_STATIC_ASSERT((containers_detail::alignment_of<long_alignment_type>::value % 
   ...
   :  private containers_detail::basic_string_base<A> 
{
   /// @cond
   private:
   STREAMS_BOOST_COPYABLE_AND_MOVABLE(basic_string)
   ...
   basic_string(STREAMS_BOOST_INTERPROCESS_RV_REF(basic_string) s) 
      : base_t(streams_boost::interprocess::move((base_t&)s))
   {}
   ...
   basic_string& operator=(STREAMS_BOOST_INTERPROCESS_COPY_ASSIGN_REF(basic_string) s)
   {
      if (&s != this) 
   ...
   //! 
   //! <b>Complexity</b>: Constant.
   basic_string& operator=(STREAMS_BOOST_INTERPROCESS_RV_REF(basic_string) ms)
   {
      basic_string &s = ms;
      if (&s != this){
         this->swap(s);
   ...
   basic_string& assign(STREAMS_BOOST_INTERPROCESS_RV_REF(basic_string) ms) 
...
STREAMS_BOOST_INTERPROCESS_RV_REF_3_TEMPL_ARGS(basic_string, CharT, Traits, A)
   operator+(
   STREAMS_BOOST_INTERPROCESS_RV_REF_3_TEMPL_ARGS(basic_string, CharT, Traits, A) mx
   , const basic_string<CharT,Traits,A>& y)
{
   mx += y;
   return streams_boost::interprocess::move(mx);
}

template <class CharT, class Traits, class A> inline
STREAMS_BOOST_INTERPROCESS_RV_REF_3_TEMPL_ARGS(basic_string, CharT, Traits, A)
   operator+(const basic_string<CharT,Traits,A>& x,
         STREAMS_BOOST_INTERPROCESS_RV_REF_3_TEMPL_ARGS(basic_string, CharT, Traits, A) my)
{
...
{
   typedef typename basic_string<CharT,Traits,A>::size_type size_type;
   return streams_boost::interprocess::move(my.get().replace(size_type(0), size_type(0), s));
}
...
template <class CharT, class Traits, class A>
STREAMS_BOOST_INTERPROCESS_RV_REF_3_TEMPL_ARGS(basic_string, CharT, Traits, A)
operator+(STREAMS_BOOST_INTERPROCESS_RV_REF_3_TEMPL_ARGS(basic_string, CharT, Traits, A) mx
         , const CharT* s)
{
   mx += s;
   return streams_boost::interprocess::move(mx);
...
struct has_trivial_destructor_after_move<streams_boost::container::basic_string<C, T, A> >
{
...
#endif // STREAMS_BOOST_CONTAINERS_STRING_HPP
