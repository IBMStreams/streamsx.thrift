...
#ifndef STREAMS_BOOST_CONTAINERS_CONTAINERS_VECTOR_HPP
#define STREAMS_BOOST_CONTAINERS_CONTAINERS_VECTOR_HPP
...
#include <streams_boost/interprocess/containers/container/container_fwd.hpp>
...
#include <streams_boost/interprocess/containers/container/detail/version_type.hpp>
#include <streams_boost/interprocess/containers/container/detail/allocation_type.hpp>
#include <streams_boost/interprocess/containers/container/detail/utilities.hpp>
#include <streams_boost/interprocess/containers/container/detail/iterators.hpp>
#include <streams_boost/interprocess/containers/container/detail/algorithms.hpp>
#include <streams_boost/interprocess/containers/container/detail/destroyers.hpp>
#include <streams_boost/interprocess/containers/container/container_fwd.hpp>
...
#include <streams_boost/interprocess/containers/container/detail/advanced_insert_int.hpp>
...
class vector_const_iterator
   : public std::iterator<std::random_access_iterator_tag
                          ...
                          ,typename streams_boost::pointer_to_other
                              <Pointer
                              ,const typename std::iterator_traits<Pointer>::value_type
                              >::type
                          ,const typename std::iterator_traits<Pointer>::value_type &>
{
   ...
   typedef typename streams_boost::pointer_to_other<Pointer, value_type>::type      pointer;
...
struct vector_value_traits
{
   ...
   static const bool trivial_dctr = streams_boost::has_trivial_destructor<value_type>::value;
   static const bool trivial_dctr_after_move = false;
      //::streams_boost::has_trivial_destructor_after_move<value_type>::value || trivial_dctr;
...
struct vector_alloc_holder 
{
   ...
   typedef containers_detail::integral_constant<unsigned,
      streams_boost::container::containers_detail::version<A>::value> alloc_version;
...
//! streams_boost::container::vector is similar to std::vector but it's compatible
...
class vector : private containers_detail::vector_alloc_holder<A>
{
   ...
   /// @cond
   private:
   STREAMS_BOOST_COPYABLE_AND_MOVABLE(vector)
   ...
   typedef streams_boost::interprocess::move_iterator<repeat_it>         repeat_move_it;
   /// @endcond
   ...
   //! 
   //! <b>Complexity</b>: Constant.
   vector(STREAMS_BOOST_INTERPROCESS_RV_REF(vector) mx) 
   ...
   void reserve(size_type new_cap)
   {
      if (this->capacity() < new_cap){
         ...
         if(same_buffer_start){
            #ifdef STREAMS_BOOST_CONTAINERS_VECTOR_ALLOC_STATS
            ++this->num_expand_fwd;
            #endif
         ...
         //If there is no forward expansion, move objects
            ...
            containers_detail::advanced_insert_aux_proxy<T, streams_boost::interprocess::move_iterator<T*>, T*>
               proxy(::streams_boost::interprocess::make_move_iterator(dummy_it), ::streams_boost::interprocess::make_move_iterator(dummy_it));
   ...
   vector& operator=(STREAMS_BOOST_INTERPROCESS_COPY_ASSIGN_REF(vector) x)
   {
      if (&x != this){
   ...
   vector& operator=(STREAMS_BOOST_INTERPROCESS_RV_REF(vector) x)
   {
      if (&x != this){
         this->swap(x);
         x.clear();
   ...
   #if !defined(STREAMS_BOOST_HAS_RVALUE_REFS) && !defined(STREAMS_BOOST_MOVE_DOXYGEN_INVOKED)
   void push_back(T &x) { push_back(const_cast<const T &>(x)); }

   template<class U>
   void push_back(const U &u, typename containers_detail::enable_if_c<containers_detail::is_same<T, U>::value && !::streams_boost::interprocess::is_movable<U>::value >::type* =0)
   ...
   //!
   //! <b>Complexity</b>: Amortized constant time.
   void push_back(STREAMS_BOOST_INTERPROCESS_RV_REF(T) x) 
   {
      if (this->members_.m_size < this->members_.m_capacity){
         //There is more memory, just construct a new object at the end
         new((void*)containers_detail::get_pointer(this->members_.m_start + this->members_.m_size))value_type(streams_boost::interprocess::move(x));
         ++this->members_.m_size;
      }
      else{
         this->insert(this->cend(), streams_boost::interprocess::move(x));
      }
   }

   #if defined(STREAMS_BOOST_CONTAINERS_PERFECT_FORWARDING) || defined(STREAMS_BOOST_CONTAINER_DOXYGEN_INVOKED)
   ...
   void emplace_back(Args &&...args)
   {
      ...
      if (this->members_.m_size < this->members_.m_capacity){
         //There is more memory, just construct a new object at the end
         new((void*)(back_pos))value_type(streams_boost::interprocess::forward<Args>(args)...);
         ++this->members_.m_size;
      }
      else{
         typedef containers_detail::advanced_insert_aux_emplace<T, T*, Args...> type;
         type &&proxy = type(streams_boost::interprocess::forward<Args>(args)...);
   ...
   template<STREAMS_BOOST_PP_ENUM_PARAMS(n, class P)>                                                   \
   void emplace_back(STREAMS_BOOST_PP_ENUM(n, STREAMS_BOOST_CONTAINERS_PP_PARAM_LIST, _))                     \
   {                                                                                            \
      T* back_pos = containers_detail::get_pointer(this->members_.m_start) + this->members_.m_size;        \
      if (this->members_.m_size < this->members_.m_capacity){                                   \
         new((void*)(back_pos))value_type                                                       \
            (STREAMS_BOOST_PP_ENUM(n, STREAMS_BOOST_CONTAINERS_PP_PARAM_FORWARD, _));                         \
         ++this->members_.m_size;                                                               \
      }                                                                                         \
      else{                                                                                     \
         containers_detail::STREAMS_BOOST_PP_CAT(STREAMS_BOOST_PP_CAT(advanced_insert_aux_emplace, n), arg)                \
            <value_type, T*, STREAMS_BOOST_PP_ENUM_PARAMS(n, P)>                                        \
               proxy(STREAMS_BOOST_PP_ENUM(n, STREAMS_BOOST_CONTAINERS_PP_PARAM_FORWARD, _));                 \
         priv_range_insert(back_pos, 1, proxy);                                                 \
      }                                                                                         \
   }                                                                                            \
                                                                                                \
   template<STREAMS_BOOST_PP_ENUM_PARAMS(n, class P)>                                                   \
   iterator emplace(const_iterator pos, STREAMS_BOOST_PP_ENUM(n, STREAMS_BOOST_CONTAINERS_PP_PARAM_LIST, _))  \
   {                                                                                            \
   ...
   #endif   //#ifdef STREAMS_BOOST_CONTAINERS_PERFECT_FORWARDING
   ...
   template<class U>
   iterator insert(const_iterator position, const U &u, typename containers_detail::enable_if_c<containers_detail::is_same<T, U>::value && !::streams_boost::interprocess::is_movable<U>::value >::type* =0)
   ...
   //! <b>Complexity</b>: If position is end(), amortized constant time
   //!   Linear time otherwise.
   iterator insert(const_iterator position, STREAMS_BOOST_INTERPROCESS_RV_REF(T) x) 
   {
      //Just call more general insert(pos, size, value) and return iterator
      size_type pos_n = position - cbegin();
   ...
   iterator erase(const_iterator position) 
   {
      ...
      streams_boost::interprocess::move(pos + 1, beg + this->members_.m_size, pos);
      --this->members_.m_size;
      //Destroy last element
   ...
   iterator erase(const_iterator first, const_iterator last) 
   {
      if (first != last){   // worth doing, copy down over hole
         ...
         T* ptr = containers_detail::get_pointer(streams_boost::interprocess::move
   ...
   void priv_range_insert_expand_forward(T* pos, size_type n, advanced_insert_aux_int_t &interf)
   {
      ...
      if (elems_after > n){
         ...
         ::streams_boost::interprocess::uninitialized_move(old_finish - n, old_finish, old_finish);
         this->members_.m_size += n;
         //Copy previous to last objects to the initialized end
         streams_boost::interprocess::move_backward(pos, old_finish - n, old_finish);
         ...
         ::streams_boost::interprocess::uninitialized_move
      ...
      (T* new_start, size_type new_cap, T* pos, size_type n, advanced_insert_aux_int_t &interf)
   {
      ...
      if(old_buffer){
         new_finish = ::streams_boost::interprocess::uninitialized_move
          ...
          T* pos, const size_type n, advanced_insert_aux_int_t &interf)
   {
      ...
      if(difference_type(s_before) >= difference_type(elemsbefore + n)){
         ...
         ::streams_boost::interprocess::uninitialized_move(old_start, pos, new_start);
         ...
         //s_before is so big that divides old_end
            ...
            ::streams_boost::interprocess::uninitialized_move(pos, pos + raw_gap, new_start + elemsbefore + n);
            ...
            T *to_destroy = streams_boost::interprocess::move(pos + raw_gap, old_finish, old_start);
         ...
         if (s_before <= elemsbefore) {
            ...
            ::streams_boost::interprocess::uninitialized_move(old_start, start_n, new_start);
            //The buffer is all constructed until old_end,
            //release destroyer and update size
            old_values_destroyer.release();
            this->members_.m_size = old_size + s_before;
            //Now copy the second part of old_begin overwriting himself
            T* next = streams_boost::interprocess::move(start_n, pos, old_start);
            if(do_after){
               ...
               T* move_end   = streams_boost::interprocess::move(pos, old_finish, move_start);
         ...
         if(do_after){
            ...
            if (elemsafter > difference_type(n_after)){
               ...
               ::streams_boost::interprocess::uninitialized_move(finish_n, old_finish, old_finish);
               this->members_.m_size += n_after;
               //Displace the rest of old_end to the new position
               streams_boost::interprocess::move_backward(pos, finish_n, old_finish);
               ...
               ::streams_boost::interprocess::uninitialized_move(pos, old_finish, old_finish + mid_last_dist);
   ...
   void priv_range_insert(const_iterator pos, InIt first, InIt last, std::input_iterator_tag)
   {
      for(;first != last; ++first){
         this->insert(pos, streams_boost::interprocess::move(value_type(*first)));
      }
   }
   ...
   void priv_assign_aux(FwdIt first, FwdIt last, std::forward_iterator_tag)
   {
      ...
      if(same_buffer_start){
         ...
         if (this->size() >= n){
            ...
            ::streams_boost::interprocess::uninitialized_copy_or_move(mid, last, end);
            this->members_.m_size = n;
         }
      }
      else if(!ret.second){
         ...
         ::streams_boost::interprocess::uninitialized_copy_or_move(first, last, containers_detail::get_pointer(ret.first));
         scoped_alloc.release();
         ...
         ::streams_boost::interprocess::uninitialized_copy_or_move(first, mid, containers_detail::get_pointer(ret.first));
...
template <class T, class A>
struct has_trivial_destructor_after_move<streams_boost::container::vector<T, A> >
{
...
#endif //   #ifndef  STREAMS_BOOST_CONTAINERS_CONTAINERS_VECTOR_HPP
