...
#ifndef STREAMS_BOOST_CONTAINERS_ADVANCED_INSERT_INT_HPP
#define STREAMS_BOOST_CONTAINERS_ADVANCED_INSERT_INT_HPP
...
namespace streams_boost { namespace container { namespace containers_detail {
   ...
   :  public advanced_insert_aux_int<T, Iterator>
{
   ...
   {  ::streams_boost::interprocess::uninitialized_copy_or_move(first_, last_, p);  }
   ...
   virtual void uninitialized_copy_some_and_update(Iterator pos, difference_type division_count, bool first_n)
   {
      ...
      if(first_n){
         ::streams_boost::interprocess::uninitialized_copy_or_move(first_, mid, pos);
         first_ = mid;
      }
      else{
         ::streams_boost::interprocess::uninitialized_copy_or_move(mid, last_, pos);
         last_ = mid;
      }
   }
...
#include <streams_boost/interprocess/containers/container/detail/variadic_templates_tools.hpp>
#include <streams_boost/interprocess/detail/move.hpp>
#include <typeinfo>
   ...
   :  public advanced_insert_aux_int<T, Iterator>
{
   ...
   {
      if(!used_){
         *p = streams_boost::interprocess::move(T (streams_boost::interprocess::forward<Args>(get<IdxPack>(args_))...));
         used_ = true;
      }
   }
   ...
   {
      if(!used_){
         new(containers_detail::get_pointer(&*p))T(streams_boost::interprocess::forward<Args>(get<IdxPack>(args_))...);
         used_ = true;
      }
   }
   ...
   :  public advanced_insert_aux_int<T, Iterator>
{
   ...
   virtual void copy_all_to(Iterator p)
   {
      if(!used_){
         value_init<T>v;
         *p = streams_boost::interprocess::move(v.m_t);
         used_ = true;
      }
   }
   ...
   template<class T, class Iterator, STREAMS_BOOST_PP_ENUM_PARAMS(n, class P) >                \
   struct STREAMS_BOOST_PP_CAT(STREAMS_BOOST_PP_CAT(advanced_insert_aux_emplace, n), arg)                \
      :  public advanced_insert_aux_int<T, Iterator>                                     \
   {                                                                                   \
      typedef typename advanced_insert_aux_int<T, Iterator>::difference_type difference_type;  \
                                                                                       \
      STREAMS_BOOST_PP_CAT(STREAMS_BOOST_PP_CAT(advanced_insert_aux_emplace, n), arg)                    \
         ( STREAMS_BOOST_PP_ENUM(n, STREAMS_BOOST_CONTAINERS_PP_PARAM_LIST, _) )          \
         : used_(false), STREAMS_BOOST_PP_ENUM(n, STREAMS_BOOST_CONTAINERS_AUX_PARAM_INIT, _) {}     \
                                                                                       \
      virtual void copy_all_to(Iterator p)                                             \
      {                                                                                \
         if(!used_){                                                                   \
            T v(STREAMS_BOOST_PP_ENUM(n, STREAMS_BOOST_CONTAINERS_PP_MEMBER_FORWARD, _));            \
            *p = streams_boost::interprocess::move(v);                                                 \
            used_ = true;                                                              \
         }                                                                             \
      }                                                                                \
                                                                                       \
      ...
      {                                                                                \
         if(!used_){                                                                   \
            new(containers_detail::get_pointer(&*p))T                                             \
               (STREAMS_BOOST_PP_ENUM(n, STREAMS_BOOST_CONTAINERS_PP_MEMBER_FORWARD, _));            \
            used_ = true;                                                              \
         }                                                                             \
      }                                                                                \
                                                                                       \
            ...
            if(!used_){                                                                \
               ...
               *p = streams_boost::interprocess::move(v);                                              \
               used_ = true;                                                           \
                                                                                       ...
                                                                                       \
      bool used_;                                                                      \
      STREAMS_BOOST_PP_REPEAT(n, STREAMS_BOOST_CONTAINERS_AUX_PARAM_DEFINE, _)                       \
   };                                                                                  \
//!
...
#endif //#ifndef STREAMS_BOOST_CONTAINERS_ADVANCED_INSERT_INT_HPP
