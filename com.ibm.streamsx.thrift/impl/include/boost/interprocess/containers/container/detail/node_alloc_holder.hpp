...
#ifndef STREAMS_BOOST_CONTAINERS_DETAIL_NODE_ALLOC_HPP_
#define STREAMS_BOOST_CONTAINERS_DETAIL_NODE_ALLOC_HPP_
...
struct scoped_deallocator
{
   ...
   typedef containers_detail::integral_constant<unsigned,
      streams_boost::container::containers_detail::
   ...
   STREAMS_BOOST_INTERPROCESS_MOVABLE_BUT_NOT_COPYABLE(scoped_deallocator)
   ...
   scoped_deallocator(STREAMS_BOOST_INTERPROCESS_RV_REF(scoped_deallocator) o)
...
class allocator_multialloc_chain_node_deallocator
{
   ...
   {
      if(!c_.empty())
         a_.deallocate_individual(streams_boost::interprocess::move(c_));
   }
};
...
struct node_alloc_holder
{
   ...
   private:
   STREAMS_BOOST_COPYABLE_AND_MOVABLE(node_alloc_holder)
   ...
   node_alloc_holder(STREAMS_BOOST_INTERPROCESS_RV_REF(node_alloc_holder) other)
      : members_(streams_boost::interprocess::move(other.node_alloc()))
   {  this->swap(other);  }

   node_alloc_holder & operator=(STREAMS_BOOST_INTERPROCESS_COPY_ASSIGN_REF(node_alloc_holder) other)
   {  members_.assign(other.node_alloc());   }

   node_alloc_holder & operator=(STREAMS_BOOST_INTERPROCESS_RV_REF(node_alloc_holder) other)
   {  members_.assign(other.node_alloc());   }
   ...
   template<class Pred>
   node_alloc_holder(STREAMS_BOOST_INTERPROCESS_RV_REF(ValAlloc) a, const Pred &c) 
      : members_(a, typename ICont::value_compare(c))
   {}
   ...
   static void construct(const NodePtr &ptr,
      STREAMS_BOOST_INTERPROCESS_RV_REF_2_TEMPL_ARGS(std::pair, Convertible1, Convertible2) value)
   {  
      ...
      new((void*)&valueptr->first) first_type(streams_boost::interprocess::move(value.first));
      STREAMS_BOOST_TRY{
         new((void*)&valueptr->second) second_type(streams_boost::interprocess::move(value.second));
      }
      STREAMS_BOOST_CATCH(...){
   ...
   {  new((void*)containers_detail::get_pointer(ptr)) Node(streams_boost::interprocess::forward<Args>(args)...);  }
   ...
   NodePtr create_node(Args &&...args)
   {
      ...
      self_t::construct(p, streams_boost::interprocess::forward<Args>(args)...);
   ...
   void construct(const NodePtr &ptr, STREAMS_BOOST_PP_ENUM(n, STREAMS_BOOST_CONTAINERS_PP_PARAM_LIST, _)) \
   {                                                                                         \
   ...
   #define STREAMS_BOOST_PP_LOCAL_MACRO(n)                                                        \
   template<STREAMS_BOOST_PP_ENUM_PARAMS(n, class P)>                                             \
   NodePtr create_node(STREAMS_BOOST_PP_ENUM(n, STREAMS_BOOST_CONTAINERS_PP_PARAM_LIST, _))             \
   {                                                                                      \
      NodePtr p = this->allocate_one();                                                   \
      Deallocator node_deallocator(p, this->node_alloc());                                \
      self_t::construct(p, STREAMS_BOOST_PP_ENUM(n, STREAMS_BOOST_CONTAINERS_PP_PARAM_FORWARD, _));     \
      node_deallocator.release();                                                         \
      return (p);                                                                         \
   }                                                                                      \
   //!
   ...
   NodePtr create_node_from_it(It it)
   {
      NodePtr p = this->allocate_one();
      Deallocator node_deallocator(p, this->node_alloc());
      ::streams_boost::container::construct_in_place(containers_detail::get_pointer(p), it);
      node_deallocator.release();
      return (p);
   }
      ...
      (FwdIterator beg, difference_type n, Inserter inserter)
   {
      if(n){
         ...
         STREAMS_BOOST_TRY{
               ...
               //This can throw
               constructed = 0;
               streams_boost::container::construct_in_place(p, beg);
               ++constructed;
         ...
         STREAMS_BOOST_CATCH(...){
            ...
            this->node_alloc().deallocate_individual(streams_boost::interprocess::move(mem));
            STREAMS_BOOST_RETHROW
         }
         STREAMS_BOOST_CATCH_END
      }
   ...
   void clear(allocator_v2)
   {
      ...
      STREAMS_BOOST_STATIC_ASSERT((::streams_boost::interprocess::is_movable<typename NodeAlloc::multiallocation_chain>::value == true));
      if(!chain.empty())
         this->node_alloc().deallocate_individual(streams_boost::interprocess::move(chain));
   }
...
#endif // STREAMS_BOOST_CONTAINERS_DETAIL_NODE_ALLOC_HPP_
