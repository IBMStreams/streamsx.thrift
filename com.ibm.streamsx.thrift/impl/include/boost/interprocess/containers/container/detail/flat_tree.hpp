...
#ifndef STREAMS_BOOST_CONTAINERS_FLAT_TREE_HPP
#define STREAMS_BOOST_CONTAINERS_FLAT_TREE_HPP
...
class flat_tree
{
   typedef streams_boost::container::vector<Value, Alloc>  vector_t;
 ...
 private:
   ...
   {
      private:
      STREAMS_BOOST_COPYABLE_AND_MOVABLE(Data)
      public:
      Data(const Data &d)
      ...
      Data& operator=(STREAMS_BOOST_INTERPROCESS_COPY_ASSIGN_REF(Data) d)
      {
      ...
      Data& operator=(STREAMS_BOOST_INTERPROCESS_RV_REF(Data) d)
      {
         value_compare::operator=(streams_boost::interprocess::move(static_cast<value_compare &>(d)));
         m_vect = streams_boost::interprocess::move(d.m_vect);
         return *this;
      }
   ...
   Data m_data;
   STREAMS_BOOST_COPYABLE_AND_MOVABLE(flat_tree)
   ...
   flat_tree(STREAMS_BOOST_INTERPROCESS_RV_REF(flat_tree) x)
      :  m_data(streams_boost::interprocess::move(x.m_data))
   { }
   ...
   flat_tree&  operator=(STREAMS_BOOST_INTERPROCESS_COPY_ASSIGN_REF(flat_tree) x)
   {  m_data = x.m_data;   return *this;  }

   flat_tree&  operator=(STREAMS_BOOST_INTERPROCESS_RV_REF(flat_tree) mx)
   {  m_data = streams_boost::interprocess::move(mx.m_data); return *this;  }
   ...
   std::pair<iterator,bool> insert_unique(STREAMS_BOOST_INTERPROCESS_RV_REF(value_type) val)
   {
      insert_commit_data data;
      std::pair<iterator,bool> ret = priv_insert_unique_prepare(val, data);
      if(ret.second){
         ret.first = priv_insert_commit(data, streams_boost::interprocess::move(val));
      }
      return ret;
   }
   ...
   iterator insert_equal(STREAMS_BOOST_INTERPROCESS_RV_REF(value_type) mval)
   {
      iterator i = this->upper_bound(KeyOfValue()(mval));
      i = this->m_data.m_vect.insert(i, streams_boost::interprocess::move(mval));
      return i;
   }
   ...
   iterator insert_unique(const_iterator pos, STREAMS_BOOST_INTERPROCESS_RV_REF(value_type) mval)
   {
      insert_commit_data data;
      std::pair<iterator,bool> ret = priv_insert_unique_prepare(pos, mval, data);
      if(ret.second){
         ret.first = priv_insert_commit(data, streams_boost::interprocess::move(mval));
      }
      return ret.first;
   }
   ...
   iterator insert_equal(const_iterator pos, STREAMS_BOOST_INTERPROCESS_RV_REF(value_type) mval)
   {
      insert_commit_data data;
      priv_insert_equal_prepare(pos, mval, data);
      return priv_insert_commit(data, streams_boost::interprocess::move(mval));
   }
   ...
   {
      value_type && val = value_type(streams_boost::interprocess::forward<Args>(args)...);
      insert_commit_data data;
      std::pair<iterator,bool> ret =
         priv_insert_unique_prepare(val, data);
      if(ret.second){
         ret.first = priv_insert_commit(data, streams_boost::interprocess::move<value_type>(val));
      }
      return ret.first;
   }
   ...
   iterator emplace_equal(Args&&... args)
   {
      value_type &&val = value_type(streams_boost::interprocess::forward<Args>(args)...);
      iterator i = this->upper_bound(KeyOfValue()(val));
      i = this->m_data.m_vect.insert(i, streams_boost::interprocess::move<value_type>(val));
      return i;
   }
   ...
   iterator emplace_hint_equal(const_iterator hint, Args&&... args)
   {
      ...
      return priv_insert_commit(data, streams_boost::interprocess::move<value_type>(val));
   }
   ...
   {                                                                                      \
      value_type val(STREAMS_BOOST_PP_ENUM(n, STREAMS_BOOST_CONTAINERS_PP_PARAM_FORWARD, _));           \
      insert_commit_data data;                                                            \
      std::pair<iterator,bool> ret = priv_insert_unique_prepare(val, data);               \
      if(ret.second){                                                                     \
         ret.first = priv_insert_commit(data, streams_boost::interprocess::move<value_type>(val));        \
   ...
   iterator priv_insert_commit
      (insert_commit_data &commit_data, STREAMS_BOOST_INTERPROCESS_FWD_REF(Convertible) convertible)
   {
      return this->m_data.m_vect.insert
         ( commit_data.position
         , streams_boost::interprocess::forward<Convertible>(convertible));
   }
...
class C, class A>
struct has_trivial_destructor_after_move<streams_boost::container::containers_detail::flat_tree<K, V, KOV, C, A> >
{
...
#endif // STREAMS_BOOST_CONTAINERS_FLAT_TREE_HPP
