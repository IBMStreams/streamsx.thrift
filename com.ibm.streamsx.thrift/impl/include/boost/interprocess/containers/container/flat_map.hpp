...
#ifndef STREAMS_BOOST_CONTAINERS_FLAT_MAP_HPP
#define STREAMS_BOOST_CONTAINERS_FLAT_MAP_HPP
...
class flat_map 
{
   /// @cond
   private:
   STREAMS_BOOST_COPYABLE_AND_MOVABLE(flat_map)
   ...
   flat_map(STREAMS_BOOST_INTERPROCESS_RV_REF(flat_map) x) 
   ...
   //! 
   //! <b>Complexity</b>: Linear in x.size().
   flat_map<Key,T,Pred,Alloc>& operator=(STREAMS_BOOST_INTERPROCESS_COPY_ASSIGN_REF(flat_map) x)
   ...
   //! 
   //! <b>Postcondition</b>: x is emptied.
   flat_map<Key,T,Pred,Alloc>& operator=(STREAMS_BOOST_INTERPROCESS_RV_REF(flat_map) mx)
   ...
   T &operator[](STREAMS_BOOST_INTERPROCESS_RV_REF(key_type) mk) 
   {
      ...
      // i->first is greater than or equivalent to k.
      if (i == end() || key_comp()(k, (*i).first))
         i = insert(i, value_type(streams_boost::interprocess::move(k), streams_boost::interprocess::move(T())));
      return (*i).second;
   }
   ...
   {  return force<std::pair<iterator,bool> >(
      m_flat_tree.insert_unique(streams_boost::interprocess::move(force<impl_value_type>(x)))); }
   ...
   //!
   //! <b>Note</b>: If an element it's inserted it might invalidate elements.
   std::pair<iterator,bool> insert(STREAMS_BOOST_INTERPROCESS_RV_REF(impl_value_type) x) 
   {
      return force<std::pair<iterator,bool> >
      (m_flat_tree.insert_unique(streams_boost::interprocess::move(x)));
   }
   ...
   iterator insert(const_iterator position, STREAMS_BOOST_INTERPROCESS_RV_REF(value_type) x)
      { return force_copy<iterator>(
         m_flat_tree.insert_unique(force<impl_const_iterator>(position), streams_boost::interprocess::move(force<impl_value_type>(x)))); }
   ...
   //!
   //! <b>Note</b>: If an element it's inserted it might invalidate elements.
   iterator insert(const_iterator position, STREAMS_BOOST_INTERPROCESS_RV_REF(impl_value_type) x)
   {
      return force_copy<iterator>(
         m_flat_tree.insert_unique(force<impl_const_iterator>(position), streams_boost::interprocess::move(x)));
   }
   ...
   {  return force_copy<iterator>(m_flat_tree.emplace_unique(streams_boost::interprocess::forward<Args>(args)...)); }
   ...
   {  return force_copy<iterator>(m_flat_tree.emplace_hint_unique(force<impl_const_iterator>(hint), streams_boost::interprocess::forward<Args>(args)...)); }
...
struct has_trivial_destructor_after_move<streams_boost::container::flat_map<K, T, C, A> >
{
...
class flat_multimap 
{
   /// @cond
   private:
   STREAMS_BOOST_COPYABLE_AND_MOVABLE(flat_multimap)
   typedef containers_detail::flat_tree<Key, 
                           std::pair<Key, T>, 
                           containers_detail::select1st< std::pair<Key, T> >, 
                           Pred, 
   ...
   //! 
   //! <b>Postcondition</b>: x is emptied.
   flat_multimap(STREAMS_BOOST_INTERPROCESS_RV_REF(flat_multimap) x) 
      : m_flat_tree(streams_boost::interprocess::move(x.m_flat_tree))
   { }
   ...
   //! 
   //! <b>Complexity</b>: Linear in x.size().
   flat_multimap<Key,T,Pred,Alloc>& operator=(STREAMS_BOOST_INTERPROCESS_COPY_ASSIGN_REF(flat_multimap) x) 
      {  m_flat_tree = x.m_flat_tree;   return *this;  }

   //! <b>Effects</b>: this->swap(x.get()).
   //! 
   //! <b>Complexity</b>: Constant.
   flat_multimap<Key,T,Pred,Alloc>& operator=(STREAMS_BOOST_INTERPROCESS_RV_REF(flat_multimap) mx) 
      {  m_flat_tree = streams_boost::interprocess::move(mx.m_flat_tree);   return *this;  }
   ...
   { return force_copy<iterator>(m_flat_tree.insert_equal(streams_boost::interprocess::move(x))); }
   ...
   //!
   //! <b>Note</b>: If an element it's inserted it might invalidate elements.
   iterator insert(STREAMS_BOOST_INTERPROCESS_RV_REF(impl_value_type) x) 
      { return force_copy<iterator>(m_flat_tree.insert_equal(streams_boost::interprocess::move(x))); }
   ...
   iterator insert(const_iterator position, STREAMS_BOOST_INTERPROCESS_RV_REF(value_type) x) 
   {
      return force_copy<iterator>
         (m_flat_tree.insert_equal(force<impl_const_iterator>(position)
                                  , streams_boost::interprocess::move(x)));
   }
   ...
   {
      return force_copy<iterator>(
         m_flat_tree.insert_equal(force<impl_const_iterator>(position), streams_boost::interprocess::move(x)));
   }
   ...
   template <class... Args>
   iterator emplace(Args&&... args)
   {  return force_copy<iterator>(m_flat_tree.emplace_equal(streams_boost::interprocess::forward<Args>(args)...)); }
   ...
   iterator emplace_hint(const_iterator hint, Args&&... args)
   {
      return force_copy<iterator>(m_flat_tree.emplace_hint_equal
         (force<impl_const_iterator>(hint), streams_boost::interprocess::forward<Args>(args)...));
   }
...
//!specialization for optimizations
template <class K, class T, class C, class A>
struct has_trivial_destructor_after_move< streams_boost::container::flat_multimap<K, T, C, A> >
{
   static const bool value = has_trivial_destructor<A>::value && has_trivial_destructor<C>::value;
};
*/
...
#endif /* STREAMS_BOOST_CONTAINERS_FLAT_MAP_HPP */
