...
#ifndef STREAMS_BOOST_CONTAINERS_DEQUE_HPP
#define STREAMS_BOOST_CONTAINERS_DEQUE_HPP
...
#ifdef STREAMS_BOOST_CONTAINER_DOXYGEN_INVOKED
...
class deque : protected deque_base<T, Alloc>
{
  ...
  typedef deque_base<T, Alloc> Base;
   ...
   STREAMS_BOOST_COPYABLE_AND_MOVABLE(deque)
   typedef ptr_alloc_ptr index_pointer;
   ...
   typedef streams_boost::interprocess::move_iterator<r_iterator>    move_it;
   ...
   void priv_range_check(size_type n) const 
      {  if (n >= this->size())  STREAMS_BOOST_RETHROW std::out_of_range("deque");   }
   ...
   deque(STREAMS_BOOST_INTERPROCESS_RV_REF(deque) mx) 
      :  Base(mx.alloc())
   {  this->swap(mx);   }
   ...
   deque& operator= (STREAMS_BOOST_INTERPROCESS_COPY_ASSIGN_REF(deque) x) 
   {
   ...
   deque& operator= (STREAMS_BOOST_INTERPROCESS_RV_REF(deque) x)
   {
      this->clear();
      this->swap(x);
      return *this;
   ...
   void push_back(STREAMS_BOOST_INTERPROCESS_RV_REF(value_type) t) 
   {
      if(this->priv_push_back_simple_available()){
         new(this->priv_push_back_simple_pos())value_type(streams_boost::interprocess::move(t));
   ...
   template<class U>
   void push_front(const U &u, typename containers_detail::enable_if_c<containers_detail::is_same<T, U>::value && !::streams_boost::interprocess::is_movable<U>::value >::type* =0)
   ...
   void push_front(STREAMS_BOOST_INTERPROCESS_RV_REF(value_type) t)
   {
      if(this->priv_push_front_simple_available()){
         new(this->priv_push_front_simple_pos())value_type(streams_boost::interprocess::move(t));
   ...
   iterator insert(const_iterator position, STREAMS_BOOST_INTERPROCESS_RV_REF(value_type) mx) 
   {
      if (position == cbegin()) {
         this->push_front(streams_boost::interprocess::move(mx));
         return begin();
      }
      else if (position == cend()) {
         this->push_back(streams_boost::interprocess::move(mx));
         return(end()-1);
      }
      else {
   ...
   {
      if(this->priv_push_back_simple_available()){
         new(this->priv_push_back_simple_pos())value_type(streams_boost::interprocess::forward<Args>(args)...);
         this->priv_push_back_simple_commit();
      }
      else{
   ...
   {
      if(this->priv_push_front_simple_available()){
         new(this->priv_push_front_simple_pos())value_type(streams_boost::interprocess::forward<Args>(args)...);
         this->priv_push_front_simple_commit();
      }
      else{
         typedef containers_detail::advanced_insert_aux_emplace<T, iterator, Args...> type;
   ...
   {
      if(p == this->cbegin()){
         this->emplace_front(streams_boost::interprocess::forward<Args>(args)...);
         return this->begin();
      }
      else if(p == this->cend()){
         this->emplace_back(streams_boost::interprocess::forward<Args>(args)...);
         return (this->end()-1);
      }
      else{
   ...
   #else //#ifdef STREAMS_BOOST_CONTAINERS_PERFECT_FORWARDING
   ...
   {                                                                                         \
      if(priv_push_back_simple_available()){                                                 \
         ...
         containers_detail::STREAMS_BOOST_PP_CAT(STREAMS_BOOST_PP_CAT(advanced_insert_aux_emplace, n), arg)             \
            <value_type, iterator, STREAMS_BOOST_PP_ENUM_PARAMS(n, P)>                               \
         ...
         priv_insert_aux_impl(cend(), 1, proxy);                                             \
                                                                                             ...
                                                                                             \
   template<STREAMS_BOOST_PP_ENUM_PARAMS(n, class P)>                                                \
   void emplace_front(STREAMS_BOOST_PP_ENUM(n, STREAMS_BOOST_CONTAINERS_PP_PARAM_LIST, _))                 \
   {                                                                                         \
      if(priv_push_front_simple_available()){                                                \
         new(priv_push_front_simple_pos())value_type                                         \
            (STREAMS_BOOST_PP_ENUM(n, STREAMS_BOOST_CONTAINERS_PP_PARAM_FORWARD, _));                      \
         ...
         priv_insert_aux_impl(cbegin(), 1, proxy);                                           \
                                                                                             ...
                                                                                             \
   template<STREAMS_BOOST_PP_ENUM_PARAMS(n, class P)>                                                \
   iterator emplace(const_iterator p, STREAMS_BOOST_PP_ENUM(n, STREAMS_BOOST_CONTAINERS_PP_PARAM_LIST, _)) \
   {                                                                                         \
      if(p == this->cbegin()){                                                               \
         this->emplace_front(STREAMS_BOOST_PP_ENUM(n, STREAMS_BOOST_CONTAINERS_PP_PARAM_FORWARD, _));      \
         return this->begin();                                                               \
      }                                                                                      \
      else if(p == cend()){                                                                  \
         this->emplace_back(STREAMS_BOOST_PP_ENUM(n, STREAMS_BOOST_CONTAINERS_PP_PARAM_FORWARD, _));       \
         return (this->end()-1);                                                             \
      }                                                                                      \
      else{                                                                                  \
   ...
   iterator erase(const_iterator pos) 
   {
      ...
      if (size_type(index) < (this->size() >> 1)) {
         streams_boost::interprocess::move_backward(begin(), iterator(pos), iterator(next));
         pop_front();
      }
      else {
         streams_boost::interprocess::move(iterator(next), end(), iterator(pos));
         pop_back();
      }
   ...
   iterator erase(const_iterator first, const_iterator last)
   {
      if (first == this->members_.m_start && last == this->members_.m_finish) {
         ...
         if (elems_before < static_cast<difference_type>(this->size() - n) - elems_before) {
            streams_boost::interprocess::move_backward(begin(), iterator(first), iterator(last));
            ...
            streams_boost::interprocess::move(iterator(last), end(), iterator(first));
   ...
   void priv_insert_aux_impl(const_iterator p, size_type n, advanced_insert_aux_int_t &interf)
   {
      ...
      if (elemsbefore < static_cast<difference_type>(length / 2)) {
         ...
         if (elemsbefore >= difference_type(n)) {
            ...
            ::streams_boost::interprocess::uninitialized_move(this->members_.m_start, start_n, new_start);
            this->members_.m_start = new_start;
            streams_boost::interprocess::move(start_n, pos, old_start);
         ...
         if (elemsafter >= difference_type(n)) {
            ...
            ::streams_boost::interprocess::uninitialized_move(finish_n, this->members_.m_finish, this->members_.m_finish);
            this->members_.m_finish = new_finish;
            ...
            ::streams_boost::interprocess::uninitialized_move(pos, old_finish, this->members_.m_finish);
            this->members_.m_finish = new_finish;
            interf.copy_all_to(pos);
         }
      }
   ...
   void priv_range_initialize(FwdIt first, FwdIt last, std::forward_iterator_tag)
   {
      ...
      STREAMS_BOOST_TRY {
         for (cur_node = this->members_.m_start.m_node; 
            ...
            ::streams_boost::interprocess::uninitialized_copy_or_move(first, mid, *cur_node);
            first = mid;
         }
         ::streams_boost::interprocess::uninitialized_copy_or_move(first, last, this->members_.m_finish.m_first);
      }
      STREAMS_BOOST_CATCH(...){
   ...
   void priv_reallocate_map(size_type nodes_to_add, bool add_at_front)
   {
      ...
      if (this->members_.m_map_size > 2 * new_num_nodes) {
         ...
         if (new_nstart < this->members_.m_start.m_node)
            streams_boost::interprocess::move(this->members_.m_start.m_node, this->members_.m_finish.m_node + 1, new_nstart);
         else
            streams_boost::interprocess::move_backward
...
template <class T, class A>
struct has_trivial_destructor_after_move<streams_boost::container::deque<T, A> >
{
...
#endif //   #ifndef  STREAMS_BOOST_CONTAINERS_DEQUE_HPP
