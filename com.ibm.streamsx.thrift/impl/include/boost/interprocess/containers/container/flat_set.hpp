...
#ifndef STREAMS_BOOST_CONTAINERS_FLAT_SET_HPP
#define STREAMS_BOOST_CONTAINERS_FLAT_SET_HPP
...
#include <functional>
#include <memory>
#include <streams_boost/interprocess/containers/container/detail/flat_tree.hpp>
...
class flat_set 
{
   /// @cond
   private:
   STREAMS_BOOST_COPYABLE_AND_MOVABLE(flat_set)
   ...
   flat_set(STREAMS_BOOST_INTERPROCESS_RV_REF(flat_set) mx) 
      : m_flat_tree(streams_boost::interprocess::move(mx.m_flat_tree))
   {}

   //! <b>Effects</b>: Makes *this a copy of x.
   //! 
   //! <b>Complexity</b>: Linear in x.size().
   flat_set<T,Pred,Alloc>& operator=(STREAMS_BOOST_INTERPROCESS_COPY_ASSIGN_REF(flat_set) x)
      {  m_flat_tree = x.m_flat_tree;   return *this;  }

   //! <b>Effects</b>: Makes *this a copy of x.
   //! 
   //! <b>Complexity</b>: Linear in x.size().
   flat_set<T,Pred,Alloc>& operator=(STREAMS_BOOST_INTERPROCESS_RV_REF(flat_set) mx)
   {  m_flat_tree = streams_boost::interprocess::move(mx.m_flat_tree);   return *this;  }
   ...
   template<class U>
   std::pair<iterator, bool> insert(const U &u, typename containers_detail::enable_if_c<containers_detail::is_same<T, U>::value && !::streams_boost::interprocess::is_movable<U>::value >::type* =0)
   ...
   {  return m_flat_tree.insert_unique(streams_boost::interprocess::move(x));  }
   ...
   //!
   //! <b>Note</b>: If an element it's inserted it might invalidate elements.
   iterator insert(const_iterator position, STREAMS_BOOST_INTERPROCESS_RV_REF(value_type) x) 
   {  return m_flat_tree.insert_unique(position, streams_boost::interprocess::move(x)); }
   ...
   {  return m_flat_tree.emplace_unique(streams_boost::interprocess::forward<Args>(args)...); }
   ...
   {  return m_flat_tree.emplace_hint_unique(hint, streams_boost::interprocess::forward<Args>(args)...); }
   ...
   {  return m_flat_tree.emplace_unique(STREAMS_BOOST_PP_ENUM(n, STREAMS_BOOST_CONTAINERS_PP_PARAM_FORWARD, _)); }           \
                                                                                                               \
   template<STREAMS_BOOST_PP_ENUM_PARAMS(n, class P)>                                                                  \
   iterator emplace_hint(const_iterator hint, STREAMS_BOOST_PP_ENUM(n, STREAMS_BOOST_CONTAINERS_PP_PARAM_LIST, _))           \
   {  return m_flat_tree.emplace_hint_unique(hint, STREAMS_BOOST_PP_ENUM(n, STREAMS_BOOST_CONTAINERS_PP_PARAM_FORWARD, _)); }\
   //!
...
struct has_trivial_destructor_after_move<streams_boost::container::flat_set<T, C, A> >
{
...
class flat_multiset 
{
   /// @cond
   private:
   STREAMS_BOOST_COPYABLE_AND_MOVABLE(flat_multiset)
   typedef containers_detail::flat_tree<T, T, containers_detail::identity<T>, Pred, Alloc> tree_t;
   ...
   flat_multiset(STREAMS_BOOST_INTERPROCESS_RV_REF(flat_multiset) x) 
      : m_flat_tree(streams_boost::interprocess::move(x.m_flat_tree))
   {}

   flat_multiset<T,Pred,Alloc>& operator=(STREAMS_BOOST_INTERPROCESS_COPY_ASSIGN_REF(flat_multiset) x) 
      {  m_flat_tree = x.m_flat_tree;   return *this;  }

   flat_multiset<T,Pred,Alloc>& operator=(STREAMS_BOOST_INTERPROCESS_RV_REF(flat_multiset) mx) 
   {  m_flat_tree = streams_boost::interprocess::move(mx.m_flat_tree);   return *this;  }
   ...
   template<class U>
   iterator insert(const U &u, typename containers_detail::enable_if_c<containers_detail::is_same<T, U>::value && !::streams_boost::interprocess::is_movable<U>::value >::type* =0)
   {  return priv_insert(u); }
   #endif
   ...
   //!
   //! <b>Note</b>: If an element it's inserted it might invalidate elements.
   iterator insert(STREAMS_BOOST_INTERPROCESS_RV_REF(value_type) x) 
   {  return m_flat_tree.insert_equal(streams_boost::interprocess::move(x));   }
   ...
   //! <b>Note</b>: If an element it's inserted it might invalidate elements.
   iterator insert(const_iterator position, STREAMS_BOOST_INTERPROCESS_RV_REF(value_type) x) 
   {  return m_flat_tree.insert_equal(position, streams_boost::interprocess::move(x));  }
   ...
   template <class... Args>
   iterator emplace(Args&&... args)
   {  return m_flat_tree.emplace_equal(streams_boost::interprocess::forward<Args>(args)...); }
   ...
   template <class... Args>
   iterator emplace_hint(const_iterator hint, Args&&... args)
   {  return m_flat_tree.emplace_hint_equal(hint, streams_boost::interprocess::forward<Args>(args)...); }
   ...
   template<STREAMS_BOOST_PP_ENUM_PARAMS(n, class P)>                                                                  \
   iterator emplace(STREAMS_BOOST_PP_ENUM(n, STREAMS_BOOST_CONTAINERS_PP_PARAM_LIST, _))                                     \
   {  return m_flat_tree.emplace_equal(STREAMS_BOOST_PP_ENUM(n, STREAMS_BOOST_CONTAINERS_PP_PARAM_FORWARD, _)); }            \
                                                                                                               \
   template<STREAMS_BOOST_PP_ENUM_PARAMS(n, class P)>                                                                  \
   iterator emplace_hint(const_iterator hint, STREAMS_BOOST_PP_ENUM(n, STREAMS_BOOST_CONTAINERS_PP_PARAM_LIST, _))           \
   {  return m_flat_tree.emplace_hint_equal(hint, STREAMS_BOOST_PP_ENUM(n, STREAMS_BOOST_CONTAINERS_PP_PARAM_FORWARD, _)); } \
   //!
   #define STREAMS_BOOST_PP_LOCAL_LIMITS (1, STREAMS_BOOST_CONTAINERS_MAX_CONSTRUCTOR_PARAMETERS)
   #include STREAMS_BOOST_PP_LOCAL_ITERATE()
...
//!specialization for optimizations
template <class T, class C, class A>
struct has_trivial_destructor_after_move<streams_boost::container::flat_multiset<T, C, A> >
{
   static const bool value = has_trivial_destructor<A>::value && has_trivial_destructor<C>::value;
};
*/
...
#endif /* STREAMS_BOOST_CONTAINERS_FLAT_SET_HPP */
