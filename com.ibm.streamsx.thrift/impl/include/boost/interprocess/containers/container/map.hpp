...
#ifndef STREAMS_BOOST_CONTAINERS_MAP_HPP
#define STREAMS_BOOST_CONTAINERS_MAP_HPP
...
#include <memory>
#include <stdexcept>
#include <streams_boost/interprocess/containers/container/detail/tree.hpp>
#include <streams_boost/interprocess/containers/container/detail/value_init.hpp>
...
#include <streams_boost/interprocess/containers/container/detail/pair.hpp>
...
class map 
{
   /// @cond
   private:
   STREAMS_BOOST_COPYABLE_AND_MOVABLE(map)
   ...
   map(STREAMS_BOOST_INTERPROCESS_RV_REF(map) x) 
      : m_tree(streams_boost::interprocess::move(x.m_tree))
   {}

   //! <b>Effects</b>: Makes *this a copy of x.
   //! 
   //! <b>Complexity</b>: Linear in x.size().
   map& operator=(STREAMS_BOOST_INTERPROCESS_COPY_ASSIGN_REF(map) x)
   {  m_tree = x.m_tree;   return *this;  }

   //! <b>Effects</b>: this->swap(x.get()).
   //! 
   //! <b>Complexity</b>: Constant.
   map& operator=(STREAMS_BOOST_INTERPROCESS_RV_REF(map) x)
   {  m_tree = streams_boost::interprocess::move(x.m_tree);   return *this;  }
   ...
   T& operator[](const key_type& k) 
   {
      ...
      if (i == end() || key_comp()(k, (*i).first)){
         ...
         value_type val(k, streams_boost::interprocess::move(v.m_t));
         i = insert(i, streams_boost::interprocess::move(val));
      }
      return (*i).second;
   }

   //! Effects: If there is no key equivalent to x in the map, inserts 
   //! value_type(streams_boost::interprocess::move(x), T()) into the map (the key is move-constructed)
   //! 
   //! Returns: A reference to the mapped_type corresponding to x in *this.
   //! 
   //! Complexity: Logarithmic.
   T& operator[](STREAMS_BOOST_INTERPROCESS_RV_REF(key_type) mk) 
   {
      key_type &k = mk;
      //we can optimize this
      iterator i = lower_bound(k);
      // i->first is greater than or equivalent to k.
      if (i == end() || key_comp()(k, (*i).first)){
         value_type val(streams_boost::interprocess::move(k), streams_boost::interprocess::move(T()));
         i = insert(i, streams_boost::interprocess::move(val));
      }
      return (*i).second;
   }
   ...
   //!
   //! <b>Complexity</b>: Logarithmic.
   std::pair<iterator,bool> insert(STREAMS_BOOST_INTERPROCESS_RV_REF(nonconst_value_type) x) 
   { return m_tree.insert_unique(streams_boost::interprocess::move(x)); }
   ...
   //!
   //! <b>Complexity</b>: Logarithmic.
   std::pair<iterator,bool> insert(STREAMS_BOOST_INTERPROCESS_RV_REF(nonconst_impl_value_type) x) 
   { return m_tree.insert_unique(streams_boost::interprocess::move(x)); }
   ...
   //!
   //! <b>Complexity</b>: Logarithmic.
   std::pair<iterator,bool> insert(STREAMS_BOOST_INTERPROCESS_RV_REF(value_type) x) 
   { return m_tree.insert_unique(streams_boost::interprocess::move(x)); }
   ...
   //! <b>Complexity</b>: Logarithmic in general, but amortized constant if t
   //!   is inserted right before p.
   iterator insert(iterator position, STREAMS_BOOST_INTERPROCESS_RV_REF(nonconst_value_type) x)
   { return m_tree.insert_unique(position, streams_boost::interprocess::move(x)); }
   ...
   //! <b>Complexity</b>: Logarithmic in general, but amortized constant if t
   //!   is inserted right before p.
   iterator insert(iterator position, STREAMS_BOOST_INTERPROCESS_RV_REF(nonconst_impl_value_type) x)
   { return m_tree.insert_unique(position, streams_boost::interprocess::move(x)); }
   ...
   //!
   //! <b>Complexity</b>: Logarithmic.
   iterator insert(iterator position, STREAMS_BOOST_INTERPROCESS_RV_REF(value_type) x)
   { return m_tree.insert_unique(position, streams_boost::interprocess::move(x)); }
   ...
   {  return m_tree.emplace_unique(streams_boost::interprocess::forward<Args>(args)...); }
   ...
   {  return m_tree.emplace_hint_unique(hint, streams_boost::interprocess::forward<Args>(args)...); }
   ...
   iterator emplace(STREAMS_BOOST_PP_ENUM(n, STREAMS_BOOST_CONTAINERS_PP_PARAM_LIST, _))                               \
   {  return m_tree.emplace_unique(STREAMS_BOOST_PP_ENUM(n, STREAMS_BOOST_CONTAINERS_PP_PARAM_FORWARD, _)); }          \
                                                                                                         \
   template<STREAMS_BOOST_PP_ENUM_PARAMS(n, class P)>                                                            \
   iterator emplace_hint(const_iterator hint, STREAMS_BOOST_PP_ENUM(n, STREAMS_BOOST_CONTAINERS_PP_PARAM_LIST, _))     \
   {  return m_tree.emplace_hint_unique(hint, STREAMS_BOOST_PP_ENUM(n, STREAMS_BOOST_CONTAINERS_PP_PARAM_FORWARD, _));}\
   //!
...
struct has_trivial_destructor_after_move<streams_boost::container::map<K, T, C, A> >
{
...
class multimap 
{
   /// @cond
   private:
   STREAMS_BOOST_COPYABLE_AND_MOVABLE(multimap)
   typedef containers_detail::rbtree<Key, 
                           std::pair<const Key, T>, 
                           containers_detail::select1st< std::pair<const Key, T> >, 
                           Pred, 
   ...
   //! 
   //! <b>Postcondition</b>: x is emptied.
   multimap(STREAMS_BOOST_INTERPROCESS_RV_REF(multimap) x) 
      : m_tree(streams_boost::interprocess::move(x.m_tree))
   {}
   ...
   //! 
   //! <b>Complexity</b>: Linear in x.size().
   multimap& operator=(STREAMS_BOOST_INTERPROCESS_COPY_ASSIGN_REF(multimap) x) 
   {  m_tree = x.m_tree;   return *this;  }

   //! <b>Effects</b>: this->swap(x.get()).
   //! 
   //! <b>Complexity</b>: Constant.
   multimap& operator=(STREAMS_BOOST_INTERPROCESS_RV_REF(multimap) x) 
   {  m_tree = streams_boost::interprocess::move(x.m_tree);   return *this;  }
   ...
   //!
   //! <b>Complexity</b>: Logarithmic.
   iterator insert(STREAMS_BOOST_INTERPROCESS_RV_REF(nonconst_value_type) x) 
   { return m_tree.insert_equal(streams_boost::interprocess::move(x)); }

   //! <b>Effects</b>: Inserts a new value move-constructed from x and returns 
   //!   the iterator pointing to the newly inserted element. 
   //!
   //! <b>Complexity</b>: Logarithmic.
   iterator insert(STREAMS_BOOST_INTERPROCESS_RV_REF(nonconst_impl_value_type) x) 
   { return m_tree.insert_equal(streams_boost::interprocess::move(x)); }
   ...
   //!   is inserted right before p.
   iterator insert(iterator position, STREAMS_BOOST_INTERPROCESS_RV_REF(nonconst_value_type) x)
   { return m_tree.insert_equal(position, streams_boost::interprocess::move(x)); }
   ...
   template <class... Args>
   iterator emplace(Args&&... args)
   {  return m_tree.emplace_equal(streams_boost::interprocess::forward<Args>(args)...); }
   ...
   template <class... Args>
   iterator emplace_hint(const_iterator hint, Args&&... args)
   {  return m_tree.emplace_hint_equal(hint, streams_boost::interprocess::forward<Args>(args)...); }
   ...
   template<STREAMS_BOOST_PP_ENUM_PARAMS(n, class P)>                                                            \
   iterator emplace(STREAMS_BOOST_PP_ENUM(n, STREAMS_BOOST_CONTAINERS_PP_PARAM_LIST, _))                               \
   {  return m_tree.emplace_equal(STREAMS_BOOST_PP_ENUM(n, STREAMS_BOOST_CONTAINERS_PP_PARAM_FORWARD, _)); }           \
                                                                                                         \
   template<STREAMS_BOOST_PP_ENUM_PARAMS(n, class P)>                                                            \
   iterator emplace_hint(const_iterator hint, STREAMS_BOOST_PP_ENUM(n, STREAMS_BOOST_CONTAINERS_PP_PARAM_LIST, _))     \
   {  return m_tree.emplace_hint_equal(hint, STREAMS_BOOST_PP_ENUM(n, STREAMS_BOOST_CONTAINERS_PP_PARAM_FORWARD, _)); }\
   //!
   #define STREAMS_BOOST_PP_LOCAL_LIMITS (1, STREAMS_BOOST_CONTAINERS_MAX_CONSTRUCTOR_PARAMETERS)
   #include STREAMS_BOOST_PP_LOCAL_ITERATE()
...
//!specialization for optimizations
template <class K, class T, class C, class A>
struct has_trivial_destructor_after_move<streams_boost::container::multimap<K, T, C, A> >
{
   static const bool value = has_trivial_destructor<A>::value && has_trivial_destructor<C>::value;
};
*/
...
#endif /* STREAMS_BOOST_CONTAINERS_MAP_HPP */
