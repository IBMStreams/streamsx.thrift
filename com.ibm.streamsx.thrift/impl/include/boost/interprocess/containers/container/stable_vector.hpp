...
#ifndef STREAMS_BOOST_CONTAINER_STABLE_VECTOR_HPP
#define STREAMS_BOOST_CONTAINER_STABLE_VECTOR_HPP
...
#include <streams_boost/interprocess/containers/container/detail/multiallocation_chain.hpp>
...
#include <streams_boost/interprocess/containers/container/vector.hpp>
#else
#include <vector>
   ...
   : public node_type_base<VoidPointer>
{
   ...
   template<class ...Args>
   node_type(Args &&...args)
      : value(streams_boost::interprocess::forward<Args>(args)...)
   {}

   #else //STREAMS_BOOST_CONTAINERS_PERFECT_FORWARDING
   ...
   node_type(STREAMS_BOOST_PP_ENUM(n, STREAMS_BOOST_CONTAINERS_PP_PARAM_LIST, _))       \
      : value(STREAMS_BOOST_PP_ENUM(n, STREAMS_BOOST_CONTAINERS_PP_PARAM_FORWARD, _))   \
   {}                                                                   \
   //!
   ...
   #endif//STREAMS_BOOST_CONTAINERS_PERFECT_FORWARDING
                         ...
                         , Reference>
{
   ...
   friend class iterator<T, const T, typename streams_boost::pointer_to_other<Pointer, T>::type>;
   ...
   iterator(const iterator<T, T&, typename streams_boost::pointer_to_other<Pointer, T>::type >& x)
      : pn(x.pn)
   {}
...
struct select_multiallocation_chain<Allocator, 1>
{
   ...
   typedef streams_boost::container::containers_detail::transform_multiallocation_chain
...
invariant_checker STREAMS_BOOST_JOIN(check_invariant_,__LINE__)(*this); \
...
class stable_vector
{
   ...
   typedef ::streams_boost::container::containers_detail::
      integral_constant<unsigned, 1>                  allocator_v1;
   typedef ::streams_boost::container::containers_detail::
      integral_constant<unsigned, 2>                  allocator_v2;
   typedef ::streams_boost::container::containers_detail::integral_constant 
      <unsigned, streams_boost::container::containers_detail::
   ...
   ///@cond
   private:
   STREAMS_BOOST_COPYABLE_AND_MOVABLE(stable_vector)
   ...
   stable_vector(STREAMS_BOOST_INTERPROCESS_RV_REF(stable_vector) x) 
      : internal_data(x.get_al()),impl(x.get_al())
   {  this->swap(x);   }
   ...
   stable_vector& operator=(STREAMS_BOOST_INTERPROCESS_COPY_ASSIGN_REF(stable_vector) x)
   {
      STABLE_VECTOR_CHECK_INVARIANT;
      if (this != &x) {
   ...
   stable_vector& operator=(STREAMS_BOOST_INTERPROCESS_RV_REF(stable_vector) x)
   {
      if (&x != this){
         this->swap(x);
         x.clear();
   ...
   {
      assign_dispatch(first, last, streams_boost::is_integral<InputIterator>());
   }

   void assign(size_type n,const T& t)
   {
      typedef constant_iterator<value_type, difference_type> cvalue_iterator;
      return assign_dispatch(cvalue_iterator(t, n), cvalue_iterator(), streams_boost::mpl::false_());
   }
   ...
   void push_back(STREAMS_BOOST_INTERPROCESS_RV_REF(T) t) 
   {  this->insert(end(), streams_boost::interprocess::move(t));  }
   ...
   void insert(const_iterator position,InputIterator first, InputIterator last)
   {
      ...
      this->insert_iter(position,first,last,
                        streams_boost::mpl::not_<streams_boost::is_integral<InputIterator> >());
   }
   ...
   void emplace_back(Args &&...args)
   {
      ...
      EmplaceFunctor &&ef = EmplaceFunctor(streams_boost::interprocess::forward<Args>(args)...);
   ...
   {                                                                                            \
      typedef STREAMS_BOOST_PP_CAT(STREAMS_BOOST_PP_CAT(emplace_functor, n), arg)                               \
         <node_type_t, STREAMS_BOOST_PP_ENUM_PARAMS(n, P)>           EmplaceFunctor;                    \
      typedef emplace_iterator<node_type_t, EmplaceFunctor>  EmplaceIterator;                   \
      EmplaceFunctor ef(STREAMS_BOOST_PP_ENUM(n, STREAMS_BOOST_CONTAINERS_PP_PARAM_FORWARD, _));                \
      this->insert(this->cend(), EmplaceIterator(ef), EmplaceIterator());                       \
   }                                                                                            \
                                                                                                \
   template<STREAMS_BOOST_PP_ENUM_PARAMS(n, class P)>                                                   \
   ...
   void clear_pool(allocator_v2)
   {
      ...
      if(!impl.empty() && impl.back()){
         ...
         get_al().deallocate_individual(streams_boost::interprocess::move(holder));
         p1 = p2 = 0;
         this->internal_data.pool_size = 0;
      }
   }
   ...
   void add_to_pool(size_type n, allocator_v2)
   {
      ...
      multiallocation_chain holder;
      holder.incorporate_after(holder.before_begin(), p1, p2, internal_data.pool_size);
      STREAMS_BOOST_STATIC_ASSERT((::streams_boost::interprocess::is_movable<multiallocation_chain>::value == true));
   ...
   template<typename InputIterator>
   void assign_dispatch(InputIterator first, InputIterator last, streams_boost::mpl::false_)
   {
      STABLE_VECTOR_CHECK_INVARIANT;
   ...
   void assign_dispatch(Integer n, Integer t, streams_boost::mpl::true_)
   {
      typedef constant_iterator<value_type, difference_type> cvalue_iterator;
      this->assign_dispatch(cvalue_iterator(t, n), cvalue_iterator(), streams_boost::mpl::false_());
   }
   ...
   iterator priv_erase(const_iterator first, const_iterator last, AllocatorVersion,
      typename streams_boost::container::containers_detail::enable_if_c
         <streams_boost::container::containers_detail::is_same<AllocatorVersion, allocator_v2>
            ::value>::type * = 0)
   {
      STABLE_VECTOR_CHECK_INVARIANT;
   ...
   void_ptr new_node(void_ptr up, Iter it)
   {
      ...
      try{
         streams_boost::container::construct_in_place(&*p, it);
         p->set_pointer(up);
      }
      catch(...){
   ...
   template <class InputIterator>
   void insert_iter(const_iterator position,InputIterator first,InputIterator last, streams_boost::mpl::true_)
   {
   ...
   void insert_iter_fwd_alloc(const impl_iterator it, FwdIterator first, FwdIterator last, difference_type n, allocator_v2)
   {
      ...
      try{
         while(first != last){
            ...
            mem.pop_front();
            //This can throw
            streams_boost::container::construct_in_place(&*p, first);
         ...
         get_al().deallocate_one(p);
         get_al().deallocate_many(streams_boost::interprocess::move(mem));
         impl.erase(it+i, it+n);
         this->align_nodes(it+i,get_last_align());
         throw;
      }
   ...
   template <class InputIterator>
   void insert_iter(const_iterator position, InputIterator first, InputIterator last, streams_boost::mpl::false_)
   {
   ...
   class invariant_checker:private streams_boost::noncopyable
   {
      const stable_vector* p;
      public:
      invariant_checker(const stable_vector& v):p(&v){}
      ~invariant_checker(){STREAMS_BOOST_ASSERT(p->invariant());}
      void touch(){}
   };
   #endif
...
#endif   //STREAMS_BOOST_CONTAINER_STABLE_VECTOR_HPP
