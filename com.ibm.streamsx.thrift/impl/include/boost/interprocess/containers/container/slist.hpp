...
#ifndef STREAMS_BOOST_CONTAINERS_SLIST_HPP
#define STREAMS_BOOST_CONTAINERS_SLIST_HPP
   ...
   :  public slist_hook<VoidPointer>::type
{
   ...
   slist_node(STREAMS_BOOST_PP_ENUM(n, STREAMS_BOOST_CONTAINERS_PP_PARAM_LIST, _))                         \
      ...
      <A, typename containers_detail::intrusive_slist_type<A>::type>
{
   ...
   /// @cond
   private:
   STREAMS_BOOST_COPYABLE_AND_MOVABLE(slist)
   ...
   //! 
   //! <b>Complexity</b>: Constant.
   slist(STREAMS_BOOST_INTERPROCESS_RV_REF(slist) x)
      : AllocHolder(streams_boost::interprocess::move((AllocHolder&)x))
   {}
   ...
   slist& operator= (STREAMS_BOOST_INTERPROCESS_COPY_ASSIGN_REF(slist) x)
   {
      if (&x != this){
   ...
   //!
   //! <b>Complexity</b>: Linear to the number of elements in x.
   slist& operator= (STREAMS_BOOST_INTERPROCESS_RV_REF(slist) mx)
   {
      if (&mx != this){
         this->clear();
         this->swap(mx);
   ...
   {  this->icont().push_front(*this->create_node(streams_boost::interprocess::move(x)));  }
   ...
   template<class U>
   iterator insert_after(const_iterator position, const U &u, typename containers_detail::enable_if_c<containers_detail::is_same<T, U>::value && !::streams_boost::interprocess::is_movable<U>::value >::type* =0)
   ...
   //! <b>Note</b>: Does not affect the validity of iterators and references of
   //!   previous values.
   iterator insert_after(const_iterator prev_pos, STREAMS_BOOST_INTERPROCESS_RV_REF(value_type) x) 
   {  return iterator(this->icont().insert_after(prev_pos.get(), *this->create_node(streams_boost::interprocess::move(x)))); }
   ...
   {  return this->insert_after(previous(p), streams_boost::interprocess::move(x)); }
   ...
   {  this->emplace_after(this->cbefore_begin(), streams_boost::interprocess::forward<Args>(args)...); }
   ...
   {  return this->emplace_after(this->previous(p), streams_boost::interprocess::forward<Args>(args)...);  }
   ...
   {                                                                                         \
      this->emplace                                                                          \
         (this->cbegin(), STREAMS_BOOST_PP_ENUM(n, STREAMS_BOOST_CONTAINERS_PP_PARAM_FORWARD, _));         \
   }                                                                                         \
                                                                                             \
   template<STREAMS_BOOST_PP_ENUM_PARAMS(n, class P)>                                                \
   iterator emplace                                                                          \
      (const_iterator p, STREAMS_BOOST_PP_ENUM(n, STREAMS_BOOST_CONTAINERS_PP_PARAM_LIST, _))              \
   {                                                                                         \
      return this->emplace_after                                                             \
         (this->previous(p), STREAMS_BOOST_PP_ENUM(n, STREAMS_BOOST_CONTAINERS_PP_PARAM_FORWARD, _));      \
   }                                                                                         \
                                                                                             \
   template<STREAMS_BOOST_PP_ENUM_PARAMS(n, class P)>                                                \
   iterator emplace_after                                                                    \
      (const_iterator prev, STREAMS_BOOST_PP_ENUM(n, STREAMS_BOOST_CONTAINERS_PP_PARAM_LIST, _))           \
   {                                                                                         \
      typename AllocHolder::Deallocator d(AllocHolder::create_node_and_deallocator());       \
...
template <class T, class A>
struct has_trivial_destructor_after_move<streams_boost::container::slist<T, A> >
{
...
}} //namespace streams_boost{  namespace container {
...
template <class T, class A>
class insert_iterator<streams_boost::container::slist<T, A> > 
{
 protected:
   typedef streams_boost::container::slist<T, A> Container;
   Container* container;
...
#endif /* STREAMS_BOOST_CONTAINERS_SLIST_HPP */
