...
#ifndef STREAMS_BOOST_CONTAINERS_SET_HPP
#define STREAMS_BOOST_CONTAINERS_SET_HPP
...
class set 
{
   /// @cond
   private:
   STREAMS_BOOST_COPYABLE_AND_MOVABLE(set)
   ...
   set(STREAMS_BOOST_INTERPROCESS_RV_REF(set) x) 
   ...
   //! 
   //! <b>Complexity</b>: Linear in x.size().
   set& operator=(STREAMS_BOOST_INTERPROCESS_COPY_ASSIGN_REF(set) x)
   {  m_tree = x.m_tree;   return *this;  }

   //! <b>Effects</b>: this->swap(x.get()).
   //! 
   //! <b>Complexity</b>: Constant.
   set& operator=(STREAMS_BOOST_INTERPROCESS_RV_REF(set) x)
   ...
   //!
   //! <b>Complexity</b>: Logarithmic.
   iterator insert(const_iterator p, STREAMS_BOOST_INTERPROCESS_RV_REF(value_type) x) 
   {  return m_tree.insert_unique(p, streams_boost::interprocess::move(x)); }
...
struct has_trivial_destructor_after_move<streams_boost::container::set<T, C, A> >
{
...
class multiset 
{
   /// @cond
   private:
   STREAMS_BOOST_COPYABLE_AND_MOVABLE(multiset)
   typedef containers_detail::rbtree<T, T, 
                     containers_detail::identity<T>, Pred, Alloc> tree_t;
   ...
   //! 
   //! <b>Postcondition</b>: x is emptied.
   multiset(STREAMS_BOOST_INTERPROCESS_RV_REF(multiset) x) 
      : m_tree(streams_boost::interprocess::move(x.m_tree))
   {}
   ...
   //! 
   //! <b>Complexity</b>: Linear in x.size().
   multiset& operator=(STREAMS_BOOST_INTERPROCESS_COPY_ASSIGN_REF(multiset) x) 
   {  m_tree = x.m_tree;   return *this;  }

   //! <b>Effects</b>: this->swap(x.get()).
   //! 
   //! <b>Complexity</b>: Constant.
   multiset& operator=(STREAMS_BOOST_INTERPROCESS_RV_REF(multiset) x) 
   {  m_tree = streams_boost::interprocess::move(x.m_tree);   return *this;  }
   ...
   //!   is inserted right before p.
   iterator insert(const_iterator p, STREAMS_BOOST_INTERPROCESS_RV_REF(value_type) x) 
   {  return m_tree.insert_equal(p, streams_boost::interprocess::move(x));  }
...
//!specialization for optimizations
template <class T, class C, class A>
struct has_trivial_destructor_after_move<streams_boost::container::multiset<T, C, A> >
{
   static const bool value = has_trivial_destructor<A>::value && has_trivial_destructor<C>::value;
};
*/
...
#endif /* STREAMS_BOOST_CONTAINERS_SET_HPP */
