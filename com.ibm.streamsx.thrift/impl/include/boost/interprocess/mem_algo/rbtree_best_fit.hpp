...
#ifndef STREAMS_BOOST_INTERPROCESS_MEM_ALGO_RBTREE_BEST_FIT_HPP
#define STREAMS_BOOST_INTERPROCESS_MEM_ALGO_RBTREE_BEST_FIT_HPP
...
#include <streams_boost/interprocess/containers/allocation_type.hpp>
...
#include <streams_boost/interprocess/detail/min_max.hpp>
...
class rbtree_best_fit
{
   ...
   private:
   struct block_ctrl;
   typedef typename streams_boost::
   ...
   multiallocation_chain allocate_many(std::size_t elem_bytes, std::size_t num_elements)
   {
      ...
      //-----------------------
      streams_boost::interprocess::scoped_lock<interprocess_mutex> guard(m_header);
      //-----------------------
   ...
   template<class T>
   std::pair<T *, bool>
      allocation_command  (streams_boost::interprocess::allocation_type command,   std::size_t limit_size,
                           std::size_t preferred_size,std::size_t &received_size, 
                           T *reuse_ptr = 0);

   std::pair<void *, bool>
     raw_allocation_command  (streams_boost::interprocess::allocation_type command,   std::size_t limit_object,
   ...
   std::pair<void*, bool>
      priv_allocation_command(streams_boost::interprocess::allocation_type command,   std::size_t limit_size,
                        std::size_t preferred_size,std::size_t &received_size, 
                        void *reuse_ptr, std::size_t sizeof_object);


   //!Real allocation algorithm with min allocation option
   std::pair<void *, bool> priv_allocate(streams_boost::interprocess::allocation_type command
   ...
   void* priv_expand_both_sides(streams_boost::interprocess::allocation_type command
                               ,std::size_t min_size
                               ,std::size_t preferred_size
                               ,std::size_t &received_size
                               ,void *reuse_ptr
   ...
   STREAMS_BOOST_STATIC_ASSERT((Alignment >= 4));
   //Due to rbtree size optimizations, Alignment must have at least pointer alignment
   STREAMS_BOOST_STATIC_ASSERT((Alignment >= detail::alignment_of<void_pointer>::value));
   ...
   STREAMS_BOOST_STATIC_ASSERT((0 == (Alignment & (Alignment - std::size_t(1u)))));
   /// @endcond
   public:
...
{  
   ...
   if(priv_next_block(first_block) == old_end_block){
      std::size_t ignore;
      unique_buffer = priv_allocate(streams_boost::interprocess::allocate_new, 0, 0, ignore).first;
      if(!unique_buffer)
         return;
...
{  
   ...
   //-----------------------
   std::size_t ignore;
   void * ret = priv_allocate(streams_boost::interprocess::allocate_new, nbytes, nbytes, ignore).first;
   return ret;
}
                        ...
                        T *reuse_ptr)
{
   ...
   STREAMS_BOOST_ASSERT(0 == ((std::size_t)ret.first % detail::alignment_of<T>::value));
...
inline std::pair<void*, bool> rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::
   raw_allocation_command  (streams_boost::interprocess::allocation_type command,   std::size_t limit_objects,
                        std::size_t preferred_objects,std::size_t &received_objects, 
                        void *reuse_ptr, std::size_t sizeof_object)
{
   if(!sizeof_object)
      return std::pair<void *, bool>(static_cast<void*>(0), 0);
   if(command & streams_boost::interprocess::try_shrink_in_place){
...
template<class MutexFamily, class VoidPointer, std::size_t MemAlignment>
void* rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::
   priv_expand_both_sides(streams_boost::interprocess::allocation_type command
                         ,std::size_t min_size
                         ,std::size_t preferred_size
                         ,std::size_t &received_size
                         ,void *reuse_ptr
                         ...
                         ,std::size_t backwards_multiple)
{
   ...
   if(command & streams_boost::interprocess::expand_fwd){
   ...
   if(backwards_multiple){
      STREAMS_BOOST_ASSERT(0 == (min_size       % backwards_multiple));
      STREAMS_BOOST_ASSERT(0 == (preferred_size % backwards_multiple));
   }

   if(command & streams_boost::interprocess::expand_bwd){
      //Obtain the real size of the block
   ...
   deallocate_many(typename rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::multiallocation_chain chain)
{
   ...
   streams_boost::interprocess::scoped_lock<interprocess_mutex> guard(m_header);
   //-----------------------
   algo_impl_t::deallocate_many(this, streams_boost::interprocess::move(chain));
}

template<class MutexFamily, class VoidPointer, std::size_t MemAlignment>
std::pair<void *, bool> rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::
   priv_allocate(streams_boost::interprocess::allocation_type command
                ,std::size_t limit_size
                ,std::size_t preferred_size
                ,std::size_t &received_size
                ,void *reuse_ptr
               ,std::size_t backwards_multiple)
{
   ...
   //command &= (~streams_boost::interprocess::expand_bwd);

   if(command & streams_boost::interprocess::shrink_in_place){
      bool success = 
   ...
   //Expand in place
   if(reuse_ptr && (command & (streams_boost::interprocess::expand_fwd | streams_boost::interprocess::expand_bwd))){
   ...
   if(command & streams_boost::interprocess::allocate_new){
...
#endif   //#ifndef STREAMS_BOOST_INTERPROCESS_MEM_ALGO_RBTREE_BEST_FIT_HPP
