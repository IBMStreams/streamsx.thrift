...
#ifndef STREAMS_BOOST_INTERPROCESS_MEM_ALGO_DETAIL_SIMPLE_SEQ_FIT_IMPL_HPP
#define STREAMS_BOOST_INTERPROCESS_MEM_ALGO_DETAIL_SIMPLE_SEQ_FIT_IMPL_HPP
...
class simple_seq_fit_impl
{
   ...
   std::pair<void *, bool>
      raw_allocation_command  (streams_boost::interprocess::allocation_type command,   std::size_t limit_size,
                               std::size_t preferred_size,std::size_t &received_size, 
   ...
   std::pair<void *, bool> priv_allocation_command(streams_boost::interprocess::allocation_type command
                                        ,std::size_t min_size
                                        ,std::size_t preferred_size
                                        ,std::size_t &received_size
                                        ,void *reuse_ptr
...
void simple_seq_fit_impl<MutexFamily, VoidPointer>::shrink_to_fit()
{
   ...
   if(!m_header.m_allocated){
      assert(prev == root);
      std::size_t ignore;
      unique_block = priv_allocate(streams_boost::interprocess::allocate_new, 0, 0, ignore).first;
      if(!unique_block)
         return;
   ...
   allocate(std::size_t nbytes)
{
   ...
   //-----------------------
   std::size_t ignore;
   return priv_allocate(streams_boost::interprocess::allocate_new, nbytes, nbytes, ignore).first;
}
...
#endif   //#ifndef STREAMS_BOOST_INTERPROCESS_MEM_ALGO_DETAIL_SIMPLE_SEQ_FIT_IMPL_HPP
