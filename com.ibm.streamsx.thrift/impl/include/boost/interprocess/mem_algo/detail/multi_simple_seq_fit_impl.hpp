...
class simple_seq_fit_impl
{
   ...
   enum { Alignment      = streams_boost::alignment_of<streams_boost::detail::max_align>::value  };
...
inline std::pair<void *, bool> simple_seq_fit_impl<MutexFamily, VoidPointer>::
   allocation_command  (streams_boost::interprocess::allocation_type command,   std::size_t min_size,
                        std::size_t preferred_size,std::size_t &received_size, 
   ...
   multi_allocate(std::size_t nbytes)
{
   ...
   //since it's faster.
   std::size_t ignore;
   void *addr = this->priv_allocate(streams_boost::interprocess::allocate_new, nbytes, nbytes, ignore).first;
   if(!addr){
      ...
      streams_boost::interprocess::multi_segment_services *p_services = 
         static_cast<streams_boost::interprocess::multi_segment_services*>
      ...
      if(ret.first){
         ...
         addr = this->priv_allocate(streams_boost::interprocess::allocate_new, nbytes, nbytes, ignore).first;
      }
   }
   return addr;
}
...
{  
   ...
   void *buffer = priv_allocate(streams_boost::interprocess::allocate_new, request, request, ignore).first;
   if(!buffer)
      return 0;
