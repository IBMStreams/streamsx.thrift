...
#ifndef STREAMS_BOOST_INTERPROCESS_ALLOCATOR_DETAIL_ALLOCATOR_COMMON_HPP
#define STREAMS_BOOST_INTERPROCESS_ALLOCATOR_DETAIL_ALLOCATOR_COMMON_HPP
...
#include <streams_boost/interprocess/detail/utilities.hpp> //get_pointer
#include <utility>   //std::pair
#include <streams_boost/utility/addressof.hpp> //streams_boost::addressof
#include <streams_boost/assert.hpp>   //STREAMS_BOOST_ASSERT
#include <streams_boost/interprocess/exceptions.hpp> //bad_alloc
#include <streams_boost/interprocess/sync/scoped_lock.hpp> //scoped_lock
#include <streams_boost/interprocess/containers/allocation_type.hpp> //streams_boost::interprocess::allocation_type
...
#include <streams_boost/interprocess/detail/segment_manager_helper.hpp>
...
struct get_or_create_node_pool_func
{
   ...
   //!Can throw streams_boost::interprocess::bad_alloc
   void operator()()
   {
      //Find or create the node_pool_t
      mp_node_pool =    mp_segment_manager->template find_or_construct
                        <NodePool>(streams_boost::interprocess::unique_instance)(mp_segment_manager);
...
struct destroy_if_last_link_func
{
   ...
   void operator()()
   {
      ...
      mp_node_pool->get_segment_manager()->template destroy<NodePool>(streams_boost::interprocess::unique_instance); 
   }  
...
class cache_impl
{
   ...
   multiallocation_chain cached_allocation(std::size_t n)
   {
      ...
      STREAMS_BOOST_TRY{
         ...
         return streams_boost::interprocess::move(chain);
      }
      STREAMS_BOOST_CATCH(...){
         this->cached_deallocation(streams_boost::interprocess::move(chain));
   ...
   void deallocate_all_cached_nodes()
   {
      ...
      mp_node_pool->deallocate_nodes(streams_boost::interprocess::move(m_cached_nodes));
   }
   ...
   void priv_deallocate_n_nodes(std::size_t n)
   {
      ...
      mp_node_pool->deallocate_nodes(streams_boost::interprocess::move(chain));
   }
};
...
class array_allocation_impl
{
   ...
   std::pair<pointer, bool>
      allocation_command(streams_boost::interprocess::allocation_type command,
   ...
   {  return this->derived()->get_segment_manager()->deallocate_many(streams_boost::interprocess::move(chain)); }
   ...
   {  return pointer(streams_boost::addressof(value));  }

   //!Returns address of non mutable object.
   //!Never throws
   const_pointer address(const_reference value) const
   {  return const_pointer(streams_boost::addressof(value));  }
   ...
   {  STREAMS_BOOST_ASSERT(ptr != 0); (*ptr).~value_type();  }
};
      ...
      , SegmentManager>
{
   ...
   //!Throws streams_boost::interprocess::bad_alloc if there is no enough memory
         ...
         <cached_allocator_impl<T, NodePool, Version>, T, typename NodePool::segment_manager>
{
   ...
   void deallocate_individual(multiallocation_chain chain)
   {
      typename node_pool_t::multiallocation_chain mem
         (chain.extract_multiallocation_chain());
      m_cache.cached_deallocation(streams_boost::interprocess::move(mem));
   }
   ...
   : public private_node_allocator_t
{
 ...
 public:
   ...
   //!Allocates array of count elements. Can throw streams_boost::interprocess::bad_alloc
   void *allocate_node()
   {
      //-----------------------
      streams_boost::interprocess::scoped_lock<mutex_type> guard(m_header);
      //-----------------------
...
*/
   ...
   void deallocate_nodes(multiallocation_chain chain)
   {
      ...
      streams_boost::interprocess::scoped_lock<mutex_type> guard(m_header);
      //-----------------------
      private_node_allocator_t::deallocate_nodes(streams_boost::interprocess::move(chain));
   }
...
#endif   //#ifndef STREAMS_BOOST_INTERPROCESS_ALLOCATOR_DETAIL_ALLOCATOR_COMMON_HPP
