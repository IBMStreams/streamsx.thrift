...
#ifndef STREAMS_BOOST_INTERPROCESS_WINDOWS_SHARED_MEMORY_HPP
#define STREAMS_BOOST_INTERPROCESS_WINDOWS_SHARED_MEMORY_HPP
...
class windows_shared_memory
{
   ...
   STREAMS_BOOST_INTERPROCESS_MOVABLE_BUT_NOT_COPYABLE(windows_shared_memory)
   /// @endcond
   ...
   //!Does not throw
   windows_shared_memory(STREAMS_BOOST_INTERPROCESS_RV_REF(windows_shared_memory) moved)
   {  this->swap(moved);   }

   //!Moves the ownership of "moved"'s shared memory to *this.
   //!After the call, "moved" does not represent any shared memory. 
   //!Does not throw
   windows_shared_memory &operator=(STREAMS_BOOST_INTERPROCESS_RV_REF(windows_shared_memory) moved)
   {  
      windows_shared_memory tmp(streams_boost::interprocess::move(moved));
      this->swap(tmp);
      return *this;  
   }
   ...
   (detail::create_enum_t type, const char *filename, mode_t mode, std::size_t size)
{
   ...
   switch(type){
      ...
      {
         __int64 s = size;
         unsigned long high_size(s >> 32), low_size((streams_boost::uint32_t)s);
...
#endif   //STREAMS_BOOST_INTERPROCESS_WINDOWS_SHARED_MEMORY_HPP
