...
#ifndef STREAMS_BOOST_INTERPROCESS_XSI_SHARED_MEMORY_DEVICE_HPP
#define STREAMS_BOOST_INTERPROCESS_XSI_SHARED_MEMORY_DEVICE_HPP
...
#include <streams_boost/interprocess/detail/tmp_dir_helpers.hpp>
#include <streams_boost/interprocess/interprocess_fwd.hpp>
#include <streams_boost/interprocess/exceptions.hpp>

#include <streams_boost/interprocess/detail/xsi_shared_memory.hpp>
#include <streams_boost/interprocess/sync/xsi/xsi_named_mutex.hpp>
#include <streams_boost/interprocess/mapped_region.hpp>
...
class xsi_shared_memory_device
{
   ...
   public:
   STREAMS_BOOST_INTERPROCESS_ENABLE_MOVE_EMULATION(xsi_shared_memory_device)
   ...
   xsi_shared_memory_device(create_only_t, const char *filepath, streams_boost::uint8_t id, mode_t mode, std::size_t size)
   {  this->priv_open_or_create_name_id(detail::DoCreate, name, id, mode, size);  }

   xsi_shared_memory_device(open_or_create_t, const char *filepath, streams_boost::uint8_t id, mode_t mode, std::size_t size)
   {  this->priv_open_or_create_name_id(detail::DoOpenOrCreate, id, name, mode, size);  }

   xsi_shared_memory_device(open_only_t, const char *filepath, streams_boost::uint8_t id, mode_t mode)
   {  this->priv_open_or_create_name_id(detail::DoOpen, name, id, mode, 0);  }

   xsi_shared_memory_device(STREAMS_BOOST_INTERPROCESS_RV_REF(xsi_shared_memory_device) moved)
   {  this->swap(moved);   }

   xsi_shared_memory_device &operator=(STREAMS_BOOST_INTERPROCESS_RV_REF(xsi_shared_memory_device) moved)
   {  
      xsi_shared_memory_device tmp(streams_boost::interprocess::move(moved));
      this->swap(tmp);
      return *this;  
   }
   ...
   (mapped_region &reg, xsi_named_mutex &mut, std::string &path)
{
   ...
   reg = streams_boost::interprocess::move(r);
   mut = streams_boost::interprocess::move(m);
   ...
   (detail::create_enum_t type, const char *filepath, mode_t mode, std::size_t size)
{
   ...
   if(type == detail::DoOpen){
      ...
      m_shm = streams_boost::interprocess::move(temp);
   }
...
#endif   //STREAMS_BOOST_INTERPROCESS_XSI_SHARED_MEMORY_DEVICE_HPP
