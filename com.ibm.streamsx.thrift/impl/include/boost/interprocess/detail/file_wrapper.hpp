...
#ifndef STREAMS_BOOST_INTERPROCESS_DETAIL_FILE_WRAPPER_HPP
#define STREAMS_BOOST_INTERPROCESS_DETAIL_FILE_WRAPPER_HPP
...
{
   /// @cond
   STREAMS_BOOST_INTERPROCESS_MOVABLE_BUT_NOT_COPYABLE(file_wrapper)
   /// @endcond
   public:
   ...
   //!Does not throw
   file_wrapper(STREAMS_BOOST_INTERPROCESS_RV_REF(file_wrapper) moved)
   {  this->swap(moved);   }

   //!Moves the ownership of "moved"'s file to *this.
   //!After the call, "moved" does not represent any file.
   //!Does not throw
   file_wrapper &operator=(STREAMS_BOOST_INTERPROCESS_RV_REF(file_wrapper) moved)
   {  
      file_wrapper tmp(streams_boost::interprocess::move(moved));
      this->swap(tmp);
      return *this;  
   }
...
#endif   //STREAMS_BOOST_INTERPROCESS_DETAIL_FILE_WRAPPER_HPP
