...
#ifndef STREAMS_BOOST_INTERPROCESS_NAMED_PROXY_HPP
#define STREAMS_BOOST_INTERPROCESS_NAMED_PROXY_HPP
...
#include <new>
#include <iterator>
#include <streams_boost/interprocess/detail/in_place_interface.hpp>
#include <streams_boost/interprocess/detail/mpl.hpp>

#ifndef STREAMS_BOOST_INTERPROCESS_PERFECT_FORWARDING
#include <streams_boost/interprocess/detail/preprocessor.hpp> 
#else
#include <streams_boost/interprocess/detail/move.hpp>
#include <streams_boost/interprocess/detail/variadic_templates_tools.hpp>
#endif   //#ifdef STREAMS_BOOST_INTERPROCESS_PERFECT_FORWARDING
...
#ifdef STREAMS_BOOST_INTERPROCESS_PERFECT_FORWARDING
...
struct CtorNArg : public placement_destroy<T>
{
   ...
   {  new((void*)mem)T(*streams_boost::interprocess::forward<Args>(get<IdxPack>(args_))...); }
...
class named_proxy
{
   ...
   {  
      CtorNArg<T, is_iterator, Args...> &&ctor_obj = CtorNArg<T, is_iterator, Args...>(streams_boost::interprocess::forward<Args>(args)...);
...
#else //#ifdef STREAMS_BOOST_INTERPROCESS_PERFECT_FORWARDING
...
#define STREAMS_BOOST_PP_LOCAL_MACRO(n)                                            \
   template<class T, bool is_iterator, STREAMS_BOOST_PP_ENUM_PARAMS(n, class P) >  \
   struct STREAMS_BOOST_PP_CAT(STREAMS_BOOST_PP_CAT(Ctor, n), Arg)                         \
      :  public placement_destroy<T>                                       \
   {                                                                       \
      typedef detail::bool_<is_iterator> IsIterator;                       \
      typedef STREAMS_BOOST_PP_CAT(STREAMS_BOOST_PP_CAT(Ctor, n), Arg) self_t;             \
                                                                           \
      void do_increment(detail::true_)                                     \
         { STREAMS_BOOST_PP_ENUM(n, STREAMS_BOOST_INTERPROCESS_AUX_PARAM_INC, _); }        \
                                                                           \
      void do_increment(detail::false_){}                                  \
                                                                           \
      self_t& operator++()                                                 \
      ...
      self_t  operator++(int) {  return ++*this;   *this;  }               \
                                                                           \
      STREAMS_BOOST_PP_CAT(STREAMS_BOOST_PP_CAT(Ctor, n), Arg)                             \
         ( STREAMS_BOOST_PP_ENUM(n, STREAMS_BOOST_INTERPROCESS_PP_PARAM_LIST, _) )        \
         : STREAMS_BOOST_PP_ENUM(n, STREAMS_BOOST_INTERPROCESS_AUX_PARAM_INIT, _) {}       \
                                                                           \
      virtual void construct_n(void *mem                                   \
                        , std::size_t num                                  \
                        , std::size_t &constructed)                        \
      ...
      {                                                                    \
         new((void*)mem) T                                                 \
         (STREAMS_BOOST_PP_ENUM(n, STREAMS_BOOST_INTERPROCESS_PP_MEMBER_IT_FORWARD, _));   \
      }                                                                    \
                                                                           \
      void construct(void *mem, detail::false_)                            \
      {                                                                    \
         new((void*)mem) T                                                 \
            (STREAMS_BOOST_PP_ENUM(n, STREAMS_BOOST_INTERPROCESS_PP_MEMBER_FORWARD, _));   \
      }                                                                    \
                                                                           \
      STREAMS_BOOST_PP_REPEAT(n, STREAMS_BOOST_INTERPROCESS_AUX_PARAM_DEFINE, _)           \
   };                                                                      \
//!
#define STREAMS_BOOST_PP_LOCAL_LIMITS (1, STREAMS_BOOST_INTERPROCESS_MAX_CONSTRUCTOR_PARAMETERS)
...
class named_proxy
{
   ...
   #define STREAMS_BOOST_PP_LOCAL_MACRO(n)                                               \
      ...
      T *operator()(STREAMS_BOOST_PP_ENUM (n, STREAMS_BOOST_INTERPROCESS_PP_PARAM_LIST, _)) const\
      {                                                                          \
         typedef STREAMS_BOOST_PP_CAT(STREAMS_BOOST_PP_CAT(Ctor, n), Arg)                        \
            <T, is_iterator, STREAMS_BOOST_PP_ENUM_PARAMS(n, P)>                         \
            ctor_obj_t;                                                          \
         ctor_obj_t ctor_obj                                                     \
            (STREAMS_BOOST_PP_ENUM(n, STREAMS_BOOST_INTERPROCESS_PP_PARAM_FORWARD, _));          \
...
#endif //#ifndef STREAMS_BOOST_INTERPROCESS_NAMED_PROXY_HPP
