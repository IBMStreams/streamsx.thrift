...
#ifndef STREAMS_BOOST_INTERPROCESS_MANAGED_OPEN_OR_CREATE_IMPL
#define STREAMS_BOOST_INTERPROCESS_MANAGED_OPEN_OR_CREATE_IMPL
...
#include <streams_boost/interprocess/detail/interprocess_tester.hpp>
#include <streams_boost/interprocess/creation_tags.hpp>
...
{
   //Non-copyable
   STREAMS_BOOST_INTERPROCESS_MOVABLE_BUT_NOT_COPYABLE(managed_open_or_create_impl)
   ...
   managed_open_or_create_impl(STREAMS_BOOST_INTERPROCESS_RV_REF(managed_open_or_create_impl) moved)
   {  this->swap(moved);   }

   managed_open_or_create_impl &operator=(STREAMS_BOOST_INTERPROCESS_RV_REF(managed_open_or_create_impl) moved)
   {  
      managed_open_or_create_impl tmp(streams_boost::interprocess::move(moved));
      this->swap(tmp);
      return *this;  
   }
       ...
       ConstructFunc construct_func)
   {
      ...
      if(created){
         try{
            ...
            streams_boost::uint32_t *patomic_word = 0;  //avoid gcc warning
            patomic_word = static_cast<streams_boost::uint32_t*>(region.get_address());
            streams_boost::uint32_t previous = detail::atomic_cas32(patomic_word, InitializingSegment, UninitializedSegment);
         ...
         streams_boost::uint32_t *patomic_word = static_cast<streams_boost::uint32_t*>(region.get_address());
         streams_boost::uint32_t value = detail::atomic_read32(patomic_word);
...
#endif   //#ifndef STREAMS_BOOST_INTERPROCESS_MANAGED_OPEN_OR_CREATE_IMPL
