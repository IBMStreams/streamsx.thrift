...
#ifndef STREAMS_BOOST_INTERPROCESS_XSI_SHARED_MEMORY_HPP
#define STREAMS_BOOST_INTERPROCESS_XSI_SHARED_MEMORY_HPP
...
//!shared memory (streams_boost::interprocess::shared_memory_object)
...
class xsi_shared_memory
{
   ...
   public:
   STREAMS_BOOST_INTERPROCESS_ENABLE_MOVE_EMULATION(xsi_shared_memory)
   ...
   xsi_shared_memory(create_only_t, const char *path, streams_boost::uint8_t id, std::size_t size, int perm = 0666)
   ...
   xsi_shared_memory(open_or_create_t, const char *path, streams_boost::uint8_t id, std::size_t size, int perm = 0666)
   ...
   xsi_shared_memory(open_only_t, const char *path, streams_boost::uint8_t id, int perm = 0666)
   {  this->priv_open_or_create(detail::DoOpen, path, id, perm, 0);  }

   //!Moves the ownership of "moved"'s shared memory object to *this. 
   //!After the call, "moved" does not represent any shared memory object. 
   //!Does not throw
   xsi_shared_memory(STREAMS_BOOST_INTERPROCESS_RV_REF(xsi_shared_memory) moved)
   {  this->swap(moved);   }

   //!Moves the ownership of "moved"'s shared memory to *this.
   //!After the call, "moved" does not represent any shared memory. 
   //!Does not throw
   xsi_shared_memory &operator=(STREAMS_BOOST_INTERPROCESS_RV_REF(xsi_shared_memory) moved)
   {  
      xsi_shared_memory tmp(streams_boost::interprocess::move(moved));
      this->swap(tmp);
      return *this;  
   }
...
inline bool xsi_shared_memory::priv_open_or_create
   (detail::create_enum_t type, const char *filename, streams_boost::uint8_t id, int perm, std::size_t size)
{
   key_t key;   
   if(filename){
...
#endif   //STREAMS_BOOST_INTERPROCESS_XSI_SHARED_MEMORY_HPP
