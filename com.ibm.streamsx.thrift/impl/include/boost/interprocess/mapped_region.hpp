...
#ifndef STREAMS_BOOST_INTERPROCESS_MAPPED_REGION_HPP
#define STREAMS_BOOST_INTERPROCESS_MAPPED_REGION_HPP
...
#else
#  ifdef STREAMS_BOOST_HAS_UNISTD_H
#    include <fcntl.h>
...
class mapped_region
{
   /// @cond
   //Non-copyable
   STREAMS_BOOST_INTERPROCESS_MOVABLE_BUT_NOT_COPYABLE(mapped_region)
   /// @endcond
   ...
   mapped_region(STREAMS_BOOST_INTERPROCESS_RV_REF(mapped_region) other);
   ...
   mapped_region &operator=(STREAMS_BOOST_INTERPROCESS_RV_REF(mapped_region) other);
...
inline mapped_region &mapped_region::operator=(STREAMS_BOOST_INTERPROCESS_RV_REF(mapped_region) moved)
{
   mapped_region tmp(streams_boost::interprocess::move(moved));
   this->swap(tmp);
   return *this;
}
...
inline mapped_region::mapped_region(STREAMS_BOOST_INTERPROCESS_RV_REF(mapped_region) other)
...
#else    //#if (defined STREAMS_BOOST_INTERPROCESS_WINDOWS)
   ...
   :  m_base(MAP_FAILED), m_size(0), m_offset(0),  m_extra_offset(0), m_mode(mode), m_is_xsi(false)
{
   ...
   #ifdef STREAMS_BOOST_INTERPROCESS_XSI_SHARED_MEMORY_OBJECTS
   if(map_hnd.is_xsi){
      //Get the size
      ::shmid_ds xsi_ds;
   ...
   #endif   //ifdef STREAMS_BOOST_INTERPROCESS_XSI_SHARED_MEMORY_OBJECTS
   if(size == 0){
      struct ::stat buf;
...
#endif   //STREAMS_BOOST_INTERPROCESS_MAPPED_REGION_HPP
