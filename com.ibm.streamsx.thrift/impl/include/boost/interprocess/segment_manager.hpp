...
#ifndef STREAMS_BOOST_INTERPROCESS_SEGMENT_MANAGER_HPP
#define STREAMS_BOOST_INTERPROCESS_SEGMENT_MANAGER_HPP
...
#include <streams_boost/interprocess/detail/transform_iterator.hpp>

#include <streams_boost/interprocess/detail/mpl.hpp>
#include <streams_boost/interprocess/detail/segment_manager_helper.hpp>
#include <streams_boost/interprocess/detail/named_proxy.hpp>
#include <streams_boost/interprocess/detail/utilities.hpp>
#include <streams_boost/interprocess/offset_ptr.hpp>
#include <streams_boost/interprocess/indexes/iset_index.hpp>
#include <streams_boost/interprocess/exceptions.hpp>
#include <streams_boost/interprocess/allocators/allocator.hpp>
#include <streams_boost/interprocess/smart_ptr/deleter.hpp>
   ...
   :  private MemoryAlgorithm
{
   ...
   multiallocation_chain allocate_many(std::size_t elem_bytes, std::size_t num_elements)
   {
      ...
      return streams_boost::interprocess::move(mem);
   }
   ...
   {  MemoryAlgorithm::deallocate_many(streams_boost::interprocess::move(chain)); }

   /// @endcond

   //!Allocates nbytes bytes. Throws streams_boost::interprocess::bad_alloc
   //!on failure
                           ...
                           T *reuse_ptr = 0)
   {
      std::pair<T *, bool> ret = MemoryAlgorithm::allocation_command
         ( command | streams_boost::interprocess::nothrow_allocation, limit_size, preferred_size, received_size
         , reuse_ptr);
      if(!(command & streams_boost::interprocess::nothrow_allocation) && !ret.first)
         throw bad_alloc();
      return ret;
   }
                           ...
                           void *reuse_ptr = 0, std::size_t sizeof_object = 1)
   {
      std::pair<void *, bool> ret = MemoryAlgorithm::raw_allocation_command
         ( command | streams_boost::interprocess::nothrow_allocation, limit_objects, preferred_objects, received_objects
...
{ 
   ...
   //!Can throw streams_boost::interprocess::bad_alloc if there is no enough memory.
   ...
   struct allocator
   {
      typedef streams_boost::interprocess::allocator<T, segment_manager> type;
   };
   ...
   struct deleter
   {
      typedef streams_boost::interprocess::deleter<T, segment_manager> type;
   };
   ...
   {
      streams_boost::interprocess::allocation_type type = ctrl_data->alloc_type();
      if(type != named_type){
   ...
   scoped_lock<rmutex> priv_get_lock(bool use_lock)
   {
      ...
      return scoped_lock<rmutex>(streams_boost::interprocess::move(local));
   }
...
#endif //#ifndef STREAMS_BOOST_INTERPROCESS_SEGMENT_MANAGER_HPP
