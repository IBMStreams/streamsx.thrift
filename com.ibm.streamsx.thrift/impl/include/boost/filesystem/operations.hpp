//  streams_boost/filesystem/operations.hpp  -----------------------------------------//
...
#ifndef STREAMS_BOOST_FILESYSTEM_OPERATIONS_HPP
#define STREAMS_BOOST_FILESYSTEM_OPERATIONS_HPP
...
#ifdef STREAMS_BOOST_WINDOWS_API
#  include <fstream>
#  if !defined(_WIN32_WINNT) || _WIN32_WINNT >= 0x0500
#    define STREAMS_BOOST_FS_HARD_LINK // Default for Windows 2K or later 
#  endif
#endif
...
// typedef streams_boost::filesystem::path Path; needs to be in namespace streams_boost::filesystem
...
#   define STREAMS_BOOST_INLINE_FS_FUNC(STREAMS_BOOST_FS_TYPE) \
      template<class Path> inline typename streams_boost::enable_if<is_basic_path<Path>, \
      STREAMS_BOOST_FS_TYPE>::type
#   define STREAMS_BOOST_FS_TYPENAME typename
# else
#   define STREAMS_BOOST_FS_FUNC(STREAMS_BOOST_FS_TYPE) inline STREAMS_BOOST_FS_TYPE
#   define STREAMS_BOOST_INLINE_FS_FUNC(STREAMS_BOOST_FS_TYPE) inline STREAMS_BOOST_FS_TYPE
...
# endif
    ...
    // STREAMS_BOOST_FILESYSTEM_NARROW_ONLY needs this:
...
# endif
    ...
    struct space_info
    {
      ...
      streams_boost::uintmax_t capacity;
      streams_boost::uintmax_t free;      // <= capacity
      streams_boost::uintmax_t available; // <= free
    };
    ...
    namespace detail
    {
      ...
      typedef std::pair< system::error_code, streams_boost::uintmax_t >
        uintmax_pair;
...
#     if !defined(STREAMS_BOOST_FILESYSTEM_NARROW_ONLY)

      STREAMS_BOOST_FILESYSTEM_DECL  streams_boost::filesystem::file_status
...
# ifdef STREAMS_BOOST_FS_HARD_LINK
      STREAMS_BOOST_FILESYSTEM_DECL system::error_code
...
//  query functions  ---------------------------------------------------------//
    ...
    { 
      ...
      if ( ec )
        ...
        "streams_boost::filesystem::status", ph, ec ) );
      return result;
    }
...
#   endif
    ...
    { 
      ...
      if ( ec )
        streams_boost::throw_exception( basic_filesystem_error<Path>(
        "streams_boost::filesystem::symlink_status", ph, ec ) );
      return result;
    }
...
# endif
    ...
    STREAMS_BOOST_FS_FUNC(bool) exists( const Path & ph )
    { 
      system::error_code ec;
      file_status result( detail::status_api( ph.external_file_string(), ec ) );
      if ( ec )
        streams_boost::throw_exception( basic_filesystem_error<Path>(
          "streams_boost::filesystem::exists", ph, ec ) );
      return exists( result );
    }

    STREAMS_BOOST_FS_FUNC(bool) is_directory( const Path & ph )
    { 
      system::error_code ec;
      file_status result( detail::status_api( ph.external_file_string(), ec ) );
      if ( ec )
        streams_boost::throw_exception( basic_filesystem_error<Path>(
          "streams_boost::filesystem::is_directory", ph, ec ) );
      return is_directory( result );
    }

    STREAMS_BOOST_FS_FUNC(bool) is_regular_file( const Path & ph )
    { 
      system::error_code ec;
      file_status result( detail::status_api( ph.external_file_string(), ec ) );
      if ( ec )
        streams_boost::throw_exception( basic_filesystem_error<Path>(
          "streams_boost::filesystem::is_regular_file", ph, ec ) );
      return is_regular_file( result );
    }

# ifndef STREAMS_BOOST_FILESYSTEM_NO_DEPRECATED
    STREAMS_BOOST_FS_FUNC(bool) is_regular( const Path & ph )
    { 
      system::error_code ec;
      file_status result( detail::status_api( ph.external_file_string(), ec ) );
      if ( ec )
        streams_boost::throw_exception( basic_filesystem_error<Path>(
          "streams_boost::filesystem::is_regular", ph, ec ) );
      return is_regular( result );
    }
# endif

    STREAMS_BOOST_FS_FUNC(bool) is_other( const Path & ph )
    { 
      system::error_code ec;
      file_status result( detail::status_api( ph.external_file_string(), ec ) );
      if ( ec )
        streams_boost::throw_exception( basic_filesystem_error<Path>(
          "streams_boost::filesystem::is_other", ph, ec ) );
      return is_other( result );
    }

    STREAMS_BOOST_FS_FUNC(bool) is_symlink(
#   ifdef STREAMS_BOOST_WINDOWS_API
      const Path & )
    {
      return false;
#   else
      const Path & ph)
    {
      ...
      if ( ec )
        streams_boost::throw_exception( basic_filesystem_error<Path>(
          "streams_boost::filesystem::is_symlink", ph, ec ) );
      return is_symlink( result );
#   endif
    }

    // VC++ 7.0 and earlier has a serious namespace bug that causes a clash
    // between streams_boost::filesystem::is_empty and the unrelated type trait
    // streams_boost::is_empty.

# if !defined( STREAMS_BOOST_MSVC ) || STREAMS_BOOST_MSVC > 1300
    STREAMS_BOOST_FS_FUNC(bool) is_empty( const Path & ph )
# else
    STREAMS_BOOST_FS_FUNC(bool) _is_empty( const Path & ph )
# endif
    {
      ...
      if ( result.first )
        streams_boost::throw_exception( basic_filesystem_error<Path>(
          "streams_boost::filesystem::is_empty", ph, result.first ) );
      return result.second;
    }

    STREAMS_BOOST_FS_FUNC(bool) equivalent( const Path & ph1, const Path & ph2 )
    {
      detail::query_pair result( detail::equivalent_api(
        ph1.external_file_string(), ph2.external_file_string() ) );
      if ( result.first )
        streams_boost::throw_exception( basic_filesystem_error<Path>(
          "streams_boost::filesystem::equivalent", ph1, ph2, result.first ) );
      return result.second;
    }

    STREAMS_BOOST_FS_FUNC(streams_boost::uintmax_t) file_size( const Path & ph )
    {
      detail::uintmax_pair result
        ( detail::file_size_api( ph.external_file_string() ) );
      if ( result.first )
        streams_boost::throw_exception( basic_filesystem_error<Path>(
          "streams_boost::filesystem::file_size", ph, result.first ) );
      return result.second;
    }

    STREAMS_BOOST_FS_FUNC(space_info) space( const Path & ph )
    {
      detail::space_pair result
        ( detail::space_api( ph.external_file_string() ) );
      if ( result.first )
        streams_boost::throw_exception( basic_filesystem_error<Path>(
          "streams_boost::filesystem::space", ph, result.first ) );
      return result.second;
    }

    STREAMS_BOOST_FS_FUNC(std::time_t) last_write_time( const Path & ph )
    {
      detail::time_pair result
        ( detail::last_write_time_api( ph.external_file_string() ) );
      if ( result.first )
        streams_boost::throw_exception( basic_filesystem_error<Path>(
          "streams_boost::filesystem::last_write_time", ph, result.first ) );
      return result.second;
    }
...
//  operations  --------------------------------------------------------------//
    ...
    STREAMS_BOOST_FS_FUNC(bool) create_directory( const Path & dir_ph )
    {
      detail::query_pair result(
        detail::create_directory_api( dir_ph.external_directory_string() ) );
      if ( result.first )
        streams_boost::throw_exception( basic_filesystem_error<Path>(
          "streams_boost::filesystem::create_directory",
          dir_ph, result.first ) );
      return result.second;
    }

#if !defined(STREAMS_BOOST_WINDOWS_API) || defined(STREAMS_BOOST_FS_HARD_LINK)
    STREAMS_BOOST_FS_FUNC(void)
    create_hard_link( const Path & to_ph, const Path & from_ph )
    {
      ...
      if ( ec )
        streams_boost::throw_exception( basic_filesystem_error<Path>(
          "streams_boost::filesystem::create_hard_link",
          to_ph, from_ph, ec ) );
    }

    STREAMS_BOOST_FS_FUNC(system::error_code)
    create_hard_link( const Path & to_ph, const Path & from_ph,
      system::error_code & ec )
    {
    ...
    create_symlink( const Path & to_ph, const Path & from_ph )
    {
      ...
      if ( ec )
        streams_boost::throw_exception( basic_filesystem_error<Path>(
          "streams_boost::filesystem::create_symlink",
          to_ph, from_ph, ec ) );
    }
    ...
    STREAMS_BOOST_FS_FUNC(bool) remove( const Path & ph )
    {
      system::error_code ec;
      file_status f = symlink_status( ph, ec );
      if ( ec )
        streams_boost::throw_exception( basic_filesystem_error<Path>(
          "streams_boost::filesystem::remove", ph, ec ) );
      return detail::remove_aux( ph, f );
    }

    STREAMS_BOOST_FS_FUNC(unsigned long) remove_all( const Path & ph )
    {
      system::error_code ec;
      file_status f = symlink_status( ph, ec );
      if ( ec )
        streams_boost::throw_exception( basic_filesystem_error<Path>(
          "streams_boost::filesystem::remove_all", ph, ec ) );
      return exists( f ) ? detail::remove_all_aux( ph, f ) : 0;
    }

    STREAMS_BOOST_FS_FUNC(void) rename( const Path & from_path, const Path & to_path )
    {
      ...
      if ( ec )
        streams_boost::throw_exception( basic_filesystem_error<Path>(
          "streams_boost::filesystem::rename",
          from_path, to_path, ec ) );
    }

    STREAMS_BOOST_SCOPED_ENUM_START(copy_option)
      { fail_if_exists, overwrite_if_exists };
    STREAMS_BOOST_SCOPED_ENUM_END

    STREAMS_BOOST_FS_FUNC(void) copy_file( const Path & from_path, const Path & to_path,
      STREAMS_BOOST_SCOPED_ENUM(copy_option) option = copy_option::fail_if_exists )
    {
      ...
      if ( ec )
        streams_boost::throw_exception( basic_filesystem_error<Path>(
          "streams_boost::filesystem::copy_file",
          from_path, to_path, ec ) );
    }
    ...
    Path current_path()
    {
      ...
      if ( ec )
          streams_boost::throw_exception( basic_filesystem_error<Path>(
            "streams_boost::filesystem::current_path", ec ) );
      return Path( Path::traits_type::to_internal( ph ) );
    }

    STREAMS_BOOST_FS_FUNC(void) current_path( const Path & ph )
    {
      system::error_code ec( detail::set_current_path_api(
        ph.external_directory_string() ) );
      if ( ec )
          streams_boost::throw_exception( basic_filesystem_error<Path>(
            "streams_boost::filesystem::current_path", ph, ec ) );
    }
...
# ifndef STREAMS_BOOST_FILESYSTEM_NO_DEPRECATED
    ...
    inline path current_path()  // overload supports pre-i18n apps
      { return current_path<streams_boost::filesystem::path>(); }
    inline const path & initial_path() // overload supports pre-i18n apps
      { return initial_path<streams_boost::filesystem::path>(); }
# endif

    STREAMS_BOOST_FS_FUNC(Path) system_complete( const Path & ph )
    {
# ifdef STREAMS_BOOST_WINDOWS_API
      if ( ph.empty() ) return ph;
      STREAMS_BOOST_FS_TYPENAME Path::external_string_type sys_ph;
      system::error_code ec( detail::get_full_path_name_api( ph.external_file_string(),
              sys_ph ) );
      if ( ec )
          streams_boost::throw_exception( basic_filesystem_error<Path>(
            "streams_boost::filesystem::system_complete", ph, ec ) );
...
# endif
    ...
    STREAMS_BOOST_FS_FUNC(Path)
    complete( const Path & ph,
      const Path & base/* = initial_path<Path>() */)
    {
      STREAMS_BOOST_ASSERT( base.is_complete()
        && (ph.is_complete() || !ph.has_root_name())
        && "streams_boost::filesystem::complete() precondition not met" );
#   ifdef STREAMS_BOOST_WINDOWS_PATH
...
#   endif
    ...
    STREAMS_BOOST_FS_FUNC(Path) complete( const Path & ph )
    ...
    last_write_time( const Path & ph, const std::time_t new_time )
    {
      ...
      if ( ec )
        streams_boost::throw_exception( basic_filesystem_error<Path>(
          "streams_boost::filesystem::last_write_time", ph, ec ) );
    }
...
# endif
    ...
    inline bool is_empty( const path & ph )
      { return streams_boost::filesystem::is_empty<path>( ph ); }
    inline bool is_empty( const wpath & ph )
      { return streams_boost::filesystem::is_empty<wpath>( ph ); }
    ...
    inline streams_boost::uintmax_t file_size( const path & ph )
      { return file_size<path>( ph ); }
    inline streams_boost::uintmax_t file_size( const wpath & ph )
...
# endif // ifndef STREAMS_BOOST_FILESYSTEM_NARROW_ONLY
    ...
    namespace detail
    {
      ...
      unsigned long remove_all_aux( const Path & ph, file_status f )
      {
        static const streams_boost::filesystem::basic_directory_iterator<Path> end_itr;
        unsigned long count = 1;
        if ( !streams_boost::filesystem::is_symlink( f ) // don't recurse symbolic links
          && streams_boost::filesystem::is_directory( f ) )
        {
          for ( streams_boost::filesystem::basic_directory_iterator<Path> itr( ph );
                itr != end_itr; ++itr )
          {
            streams_boost::system::error_code ec;
            streams_boost::filesystem::file_status fn = streams_boost::filesystem::symlink_status( itr->path(), ec );
            if ( ec )
              streams_boost::throw_exception( basic_filesystem_error<Path>( 
                "streams_boost::filesystem:remove_all", ph, ec ) );
...
//  test helper  -------------------------------------------------------------//
    ...
    // actually supports large file sizes.
      STREAMS_BOOST_FILESYSTEM_DECL bool possible_large_file_size_support();

//  directory_iterator helpers  ----------------------------------------------//

//    forwarding functions avoid need for STREAMS_BOOST_FILESYSTEM_DECL for class
...
#if       defined(STREAMS_BOOST_POSIX_API)
            void *& buffer,
#endif
...
#     if defined(STREAMS_BOOST_WINDOWS_API) && !defined(STREAMS_BOOST_FILESYSTEM_NARROW_ONLY)
      STREAMS_BOOST_FILESYSTEM_DECL system::error_code
...
#       endif
      ...
      STREAMS_BOOST_FILESYSTEM_DECL system::error_code not_found_error();
...
//  basic_directory_iterator  ------------------------------------------------//
    ...
    private:
      ...
      streams_boost::shared_ptr< detail::dir_itr_imp< Path > >  m_imp;
      ...
      typename streams_boost::iterator_facade<
        basic_directory_iterator<Path>,
        basic_directory_entry<Path>,
        streams_boost::single_pass_traversal_tag >::reference dereference() const 
      {
...
# endif
      ...
      : m_imp( new detail::dir_itr_imp<Path> )
    {
      ...
      {
        streams_boost::throw_exception( basic_filesystem_error<Path>( 
          "streams_boost::filesystem::basic_directory_iterator constructor",
          dir_path, ec ) );
      }
    }
    ...
    {
      STREAMS_BOOST_ASSERT( m_imp.get() && "attempt to increment end iterator" );
      STREAMS_BOOST_ASSERT( m_imp->m_handle != 0 && "internal program error" );
      ...
      for (;;)
      {
        ...
        {
          streams_boost::throw_exception( basic_filesystem_error<Path>(  
            "streams_boost::filesystem::basic_directory_iterator increment",
...
#     ifndef STREAMS_BOOST_WINDOWS_API
        ...
        else { m_status = streams_boost::filesystem::status( m_path ); }
#     else
        m_status = streams_boost::filesystem::status( m_path );
#     endif
      }
      return m_status;
    }
...
#     ifndef STREAMS_BOOST_WINDOWS_API
        if ( status_known( m_symlink_status )
          && !is_symlink( m_symlink_status ) )
          { ec = streams_boost::system::error_code();; m_status = m_symlink_status; }
        else { m_status = streams_boost::filesystem::status( m_path, ec ); }
#     else
        m_status = streams_boost::filesystem::status( m_path, ec );
#     endif
      }
      else ec = streams_boost::system::error_code();;
      return m_status;
    }
...
#   ifndef STREAMS_BOOST_WINDOWS_API
      if ( !status_known( m_symlink_status ) )
        { m_symlink_status = streams_boost::filesystem::symlink_status( m_path ); }
...
#   ifndef STREAMS_BOOST_WINDOWS_API
      if ( !status_known( m_symlink_status ) )
        { m_symlink_status = streams_boost::filesystem::symlink_status( m_path, ec ); }
      else ec = streams_boost::system::error_code();;
      return m_symlink_status;
#   else
      return status( ec );
...
#endif // STREAMS_BOOST_FILESYSTEM_OPERATIONS_HPP
