...
#ifndef STREAMS_BOOST_SIGNALS_SLOT_HEADER
#define STREAMS_BOOST_SIGNALS_SLOT_HEADER
...
namespace streams_boost {
  namespace STREAMS_BOOST_SIGNALS_NAMESPACE {
    namespace detail {
      class STREAMS_BOOST_SIGNALS_DECL slot_base {
    ...
    get_invocable_slot(const F& f, STREAMS_BOOST_SIGNALS_NAMESPACE::detail::signal_tag)
      { return reference_wrapper<const F>(f); }

    template<typename F>
    const F&
    get_invocable_slot(const F& f, STREAMS_BOOST_SIGNALS_NAMESPACE::detail::reference_tag)
      { return f; }

    template<typename F>
    const F&
    get_invocable_slot(const F& f, STREAMS_BOOST_SIGNALS_NAMESPACE::detail::value_tag)
      { return f; }

    // Get the slot so that it can be inspected for trackable objects
    template<typename F>
    const F&
    get_inspectable_slot(const F& f, STREAMS_BOOST_SIGNALS_NAMESPACE::detail::signal_tag)
      { return f; }

    template<typename F>
    const F&
    get_inspectable_slot(const reference_wrapper<F>& f, STREAMS_BOOST_SIGNALS_NAMESPACE::detail::reference_tag)
      { return f.get(); }

    template<typename F>
    const F&
    get_inspectable_slot(const F& f, STREAMS_BOOST_SIGNALS_NAMESPACE::detail::value_tag)
      { return f; }

    // Determines the type of the slot - is it a signal, a reference to a
    // slot or just a normal slot.
    template<typename F>
    typename STREAMS_BOOST_SIGNALS_NAMESPACE::detail::get_slot_tag<F>::type
    tag_type(const F&)
    {
      typedef typename STREAMS_BOOST_SIGNALS_NAMESPACE::detail::get_slot_tag<F>::type
        the_tag_type;
  ...
  class slot : public STREAMS_BOOST_SIGNALS_NAMESPACE::detail::slot_base {
    typedef STREAMS_BOOST_SIGNALS_NAMESPACE::detail::slot_base inherited;
    typedef typename inherited::data_t data_t;

  public:
    template<typename F>
    slot(const F& f) : slot_function(STREAMS_BOOST_SIGNALS_NAMESPACE::get_invocable_slot(f, STREAMS_BOOST_SIGNALS_NAMESPACE::tag_type(f)))
    {
      ...
      // have been made.
      STREAMS_BOOST_SIGNALS_NAMESPACE::detail::bound_objects_visitor 
        do_bind(this->data->bound_objects);
      visit_each(do_bind, 
                 STREAMS_BOOST_SIGNALS_NAMESPACE::get_inspectable_slot
                   (f, STREAMS_BOOST_SIGNALS_NAMESPACE::tag_type(f)));
      create_connection();
    }
...
#endif // STREAMS_BOOST_SIGNALS_SLOT_HEADER
