...
#ifndef STREAMS_BOOST_TEST_FRAMEWORK_IPP_021005GER
#define STREAMS_BOOST_TEST_FRAMEWORK_IPP_021005GER
...
#include <streams_boost/test/unit_test_monitor.hpp>
...
// Boost
#include <streams_boost/timer.hpp>
...
public:
    ...
    void            visit( test_case const& tc )
    {
        if( !tc.check_dependencies() ) {
            STREAMS_BOOST_TEST_FOREACH( test_observer*, to, m_observers )
...
init( init_unit_test_func init_func, int argc, char* argv[] )
{
    ...
    try {
        streams_boost::execution_monitor em;
...
{
    STREAMS_BOOST_TEST_SETUP_ASSERT( tc->p_id == INV_TEST_UNIT_ID, STREAMS_BOOST_TEST_L( "test case already registered" ) );

    test_unit_id new_id = s_frk_impl().m_next_test_case_id;

    STREAMS_BOOST_TEST_SETUP_ASSERT( new_id != MAX_TEST_CASE_ID, STREAMS_BOOST_TEST_L( "too many test cases" ) );
...
{
    STREAMS_BOOST_TEST_SETUP_ASSERT( ts->p_id == INV_TEST_UNIT_ID, STREAMS_BOOST_TEST_L( "test suite already registered" ) );

    test_unit_id new_id = s_frk_impl().m_next_test_suite_id;

    STREAMS_BOOST_TEST_SETUP_ASSERT( new_id != MAX_TEST_SUITE_ID, STREAMS_BOOST_TEST_L( "too many test suites" ) );
...
run( test_unit_id id, bool continue_test )
{
    ...
    STREAMS_BOOST_TEST_SETUP_ASSERT( tcc.p_count != 0 , runtime_config::test_to_run().is_empty() 
        ? STREAMS_BOOST_TEST_L( "test tree is empty" ) 
        : STREAMS_BOOST_TEST_L( "no test cases matching filter" ) );
    ...
    if( call_start_finish ) {
        STREAMS_BOOST_TEST_FOREACH( test_observer*, to, s_frk_impl().m_observers ) {
            streams_boost::execution_monitor em;
    ...
    case 1: {
        ...
        STREAMS_BOOST_TEST_MESSAGE( "Test cases order is shuffled using seed: " << seed );
        std::srand( seed );
        break;
    }
    default:
        STREAMS_BOOST_TEST_MESSAGE( "Test cases order is shuffled using seed: " << runtime_config::random_seed() );
...
#endif // STREAMS_BOOST_TEST_FRAMEWORK_IPP_021005GER
