...
//  streams_boost libraries.
// ***************************************************************************

#ifndef STREAMS_BOOST_TEST_EXECUTION_MONITOR_IPP_012205GER
#define STREAMS_BOOST_TEST_EXECUTION_MONITOR_IPP_012205GER
...
#include <streams_boost/test/debug.hpp>

// Boost
#include <streams_boost/cstdlib.hpp>    // for exit codes
#include <streams_boost/config.hpp>     // for workarounds
#include <streams_boost/exception/get_error_info.hpp> // for get_error_info
#include <streams_boost/exception/current_exception_cast.hpp> // for current_exception_cast
...
#if defined(_WIN32) && !defined(STREAMS_BOOST_DISABLE_WIN32) &&                  \
    (!defined(__COMO__) && !defined(__MWERKS__) && !defined(__GNUC__) || \
     STREAMS_BOOST_WORKAROUND(__MWERKS__, >= 0x3000))

#  define STREAMS_BOOST_SEH_BASED_SIGNAL_HANDLING
...
#  if STREAMS_BOOST_WORKAROUND(_MSC_VER,  < 1300 ) || defined(UNDER_CE)
...
#    include <crtdbg.h>
#    define STREAMS_BOOST_TEST_CRT_HOOK_TYPE    _CRT_REPORT_HOOK
#    define STREAMS_BOOST_TEST_CRT_ASSERT       _CRT_ASSERT
#    define STREAMS_BOOST_TEST_CRT_ERROR        _CRT_ERROR
#    define STREAMS_BOOST_TEST_CRT_SET_HOOK(H)  _CrtSetReportHook(H)
#  else
#    define STREAMS_BOOST_TEST_CRT_HOOK_TYPE    void*
#    define STREAMS_BOOST_TEST_CRT_ASSERT       2
#    define STREAMS_BOOST_TEST_CRT_ERROR        1
#    define STREAMS_BOOST_TEST_CRT_SET_HOOK(H)  (void*)(H)
#  endif

#  if !STREAMS_BOOST_WORKAROUND(_MSC_VER,  >= 1400 ) || defined(UNDER_CE)
...
#  if STREAMS_BOOST_WORKAROUND(__BORLANDC__, STREAMS_BOOST_TESTED_AT(0x0564)) || defined(UNDER_CE)

namespace { void _set_se_translator( void* ) {} }

#  endif

#elif defined(STREAMS_BOOST_HAS_SIGACTION)

#  define STREAMS_BOOST_SIGACTION_BASED_SIGNAL_HANDLING
...
#      define STREAMS_BOOST_TEST_LIMITED_SIGNAL_DETAILS
#      define STREAMS_BOOST_TEST_IGNORE_SIGCHLD

#    endif 
#  endif 

#  if !defined(__CYGWIN__) && !defined(__QNXNTO__)
#   define STREAMS_BOOST_TEST_USE_ALT_STACK
#  endif
...
#  if defined(SIGPOLL) && !defined(__CYGWIN__)                              && \
      ...
      !defined(__NetBSD__)                                                  && \
      !defined(__QNXNTO__)
#    define STREAMS_BOOST_TEST_CATCH_SIGPOLL
#  endif

#  ifdef STREAMS_BOOST_TEST_USE_ALT_STACK
#    define STREAMS_BOOST_TEST_ALT_STACK_SIZE SIGSTKSZ
#  endif

#else

#  define STREAMS_BOOST_NO_SIGNAL_HANDLING
...
#ifdef __BORLANDC__
#  define STREAMS_BOOST_TEST_VSNPRINTF( a1, a2, a3, a4 ) std::vsnprintf( (a1), (a2), (a3), (a4) )
#elif STREAMS_BOOST_WORKAROUND(_MSC_VER, <= 1310) || \
      STREAMS_BOOST_WORKAROUND(__MWERKS__, STREAMS_BOOST_TESTED_AT(0x3000)) || \
      defined(UNDER_CE)
#  define STREAMS_BOOST_TEST_VSNPRINTF( a1, a2, a3, a4 ) _vsnprintf( (a1), (a2), (a3), (a4) )
#else
#  define STREAMS_BOOST_TEST_VSNPRINTF( a1, a2, a3, a4 ) vsnprintf( (a1), (a2), (a3), (a4) )
#endif

template <typename ErrorInfo>
typename ErrorInfo::value_type
extract( streams_boost::exception const* ex )
{
    if( !ex )
        return 0;

    typename ErrorInfo::value_type const * val = streams_boost::get_error_info<ErrorInfo>( *ex );
...
static void
report_error( execution_exception::error_code ec, streams_boost::exception const* be, char const* format, va_list* args )
{
    static const int REPORT_ERROR_BUFFER_SIZE = 512;
    static char buf[REPORT_ERROR_BUFFER_SIZE];

    STREAMS_BOOST_TEST_VSNPRINTF( buf, sizeof(buf)-1, format, *args ); 
    buf[sizeof(buf)-1] = 0;
...
static void
report_error( execution_exception::error_code ec, streams_boost::exception const* be, char const* format, ... )
{
    va_list args;
    va_start( args, format );
...
#if defined(STREAMS_BOOST_SIGACTION_BASED_SIGNAL_HANDLING)
...
// ************************************************************************** //
// **************    streams_boost::detail::system_signal_exception    ************** //
// ************************************************************************** //
...
system_signal_exception::report() const
{
    ...
    case SIGILL:
        switch( m_sig_info->si_code ) {
#ifndef STREAMS_BOOST_TEST_LIMITED_SIGNAL_DETAILS
        case ILL_ILLOPC:
...
#if defined(STREAMS_BOOST_TEST_CATCH_SIGPOLL)
...
// ************************************************************************** //
// **************         streams_boost::detail::signal_action         ************** //
// ************************************************************************** //
...
, m_installed( install )
{
    ...
    STREAMS_BOOST_TEST_SYS_ASSERT( ::sigaction( m_sig , sigaction_ptr(), &m_new_action ) != -1 );
    ...
    STREAMS_BOOST_TEST_SYS_ASSERT( sigemptyset( &m_new_action.sa_mask ) != -1 );
    ...
    STREAMS_BOOST_TEST_SYS_ASSERT( ::sigaction( m_sig, &m_new_action, &m_old_action ) != -1 );
}
...
// ************************************************************************** //
// **************        streams_boost::detail::signal_handler         ************** //
// ************************************************************************** //
...
#ifndef STREAMS_BOOST_TEST_IGNORE_SIGCHLD
, m_CHLD_action( SIGCHLD, catch_system_errors, attach_dbg, alt_stack )
#endif
#ifdef STREAMS_BOOST_TEST_CATCH_SIGPOLL
...
, m_ALRM_action( SIGALRM, timeout > 0        , attach_dbg, alt_stack )
{
    ...
    if( alt_stack ) {
        ...
        STREAMS_BOOST_TEST_SYS_ASSERT( ::sigaltstack( 0, &sigstk ) != -1 );

        if( sigstk.ss_flags & SS_DISABLE ) {
            sigstk.ss_sp    = alt_stack;
            sigstk.ss_size  = STREAMS_BOOST_TEST_ALT_STACK_SIZE;
            sigstk.ss_flags = 0;
            STREAMS_BOOST_TEST_SYS_ASSERT( ::sigaltstack( &sigstk, 0 ) != -1 );
        }
    }
#endif
}
...
#endif
#ifdef STREAMS_BOOST_TEST_IGNORE_NON_ZERO_CHILD_CODE
            ;
#else
...
static void execution_monitor_attaching_signal_handler( int sig, siginfo_t* info, void* context )
{
    ...
    STREAMS_BOOST_TEST_SYS_ASSERT( ::signal( sig, SIG_DFL ) != SIG_ERR );
}
...
execution_monitor::catch_signals( unit_test::callback0<int> const& F )
{
    ...
    if( !!p_use_alt_stack && !m_alt_stack )
        m_alt_stack.reset( new char[STREAMS_BOOST_TEST_ALT_STACK_SIZE] );
#else
...
#elif defined(STREAMS_BOOST_SEH_BASED_SIGNAL_HANDLING)
...
{
    switch( reportType ) {
    case STREAMS_BOOST_TEST_CRT_ASSERT:
    ...
    case STREAMS_BOOST_TEST_CRT_ERROR:
...
void STREAMS_BOOST_TEST_CALL_DECL
...
execution_monitor::catch_signals( unit_test::callback0<int> const& F )
{
    ...
    STREAMS_BOOST_TEST_CRT_HOOK_TYPE old_crt_hook = 0;
    ...
    if( !p_catch_system_errors )
       ...
       old_crt_hook = STREAMS_BOOST_TEST_CRT_SET_HOOK( &detail::assert_reporting_function );
    ...
    __finally {
        if( !!p_catch_system_errors ) {
            ...
            STREAMS_BOOST_TEST_CRT_SET_HOOK( old_crt_hook );
...
execution_monitor::execute( unit_test::callback0<int> const& F )
{
    ...
    //  std:: exceptions
      ...
      { detail::report_error( execution_exception::cpp_exception_error, 
                              current_exception_cast<streams_boost::exception const>(),
                              "std::bad_alloc: %s", ex.what() ); }

#if STREAMS_BOOST_WORKAROUND(__BORLANDC__, <= 0x0551)
    ...
    catch( streams_boost::exception const& ex )
    { detail::report_error( execution_exception::cpp_exception_error, 
                            &ex,
                            "unknown streams_boost::exception" ); }
...
: m_error_code( ec_ )
, m_what( what_msg_.empty() ? STREAMS_BOOST_TEST_L( "uncaught exception, system error or abort requested" ) : what_msg_ )
...
#endif // STREAMS_BOOST_TEST_EXECUTION_MONITOR_IPP_012205GER
