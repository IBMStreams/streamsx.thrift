...
#ifndef STREAMS_BOOST_TEST_COMPILER_LOG_FORMATTER_IPP_020105GER
#define STREAMS_BOOST_TEST_COMPILER_LOG_FORMATTER_IPP_020105GER

// Boost.Test
#include <streams_boost/test/output/compiler_log_formatter.hpp>
...
test_phase_identifier()
{
    return framework::is_initialized() 
            ...
            : STREAMS_BOOST_TEST_L( "Test setup" );
}
...
{
    output  << "Platform: " << STREAMS_BOOST_PLATFORM            << '\n'
            << "Compiler: " << STREAMS_BOOST_COMPILER            << '\n'
            << "STL     : " << STREAMS_BOOST_STDLIB              << '\n'
            << "Boost   : " << STREAMS_BOOST_VERSION/100000      << "."
                            << STREAMS_BOOST_VERSION/100 % 1000  << "."
                            << STREAMS_BOOST_VERSION % 100       << std::endl;
}
...
{
    switch( let ) {
        case STREAMS_BOOST_UTL_ET_INFO:
            print_prefix( output, entry_data.m_file_name, entry_data.m_line_num );
            output << "info: ";
            break;
        case STREAMS_BOOST_UTL_ET_MESSAGE:
            break;
        case STREAMS_BOOST_UTL_ET_WARNING:
            print_prefix( output, entry_data.m_file_name, entry_data.m_line_num );
            output << "warning in \"" << test_phase_identifier() << "\": ";
            break;
        case STREAMS_BOOST_UTL_ET_ERROR:
            print_prefix( output, entry_data.m_file_name, entry_data.m_line_num );
            output << "error in \"" << test_phase_identifier() << "\": ";
            break;
        case STREAMS_BOOST_UTL_ET_FATAL_ERROR:
            print_prefix( output, entry_data.m_file_name, entry_data.m_line_num );
...
compiler_log_formatter::print_prefix( std::ostream& output, const_string file, std::size_t line )
{
    ...
    // <http://richarddingwall.name/2008/06/01/using-the-streams_boost-unit-test-framework-with-xcode-3/>. 
    output << file << ':' << line << ": ";
#else
    output << file << '(' << line << "): ";
#endif
...
#endif // STREAMS_BOOST_TEST_COMPILER_LOG_FORMATTER_IPP_020105GER
