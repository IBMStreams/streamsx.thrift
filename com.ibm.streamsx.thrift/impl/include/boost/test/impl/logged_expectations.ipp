...
#ifndef STREAMS_BOOST_TEST_LOGGED_EXPECTATIONS_IPP_120905GER
#define STREAMS_BOOST_TEST_LOGGED_EXPECTATIONS_IPP_120905GER
...
{
    STREAMS_BOOST_REQUIRE_MESSAGE( !log_file_name.is_empty(), "Empty expectations log file name" );

    m_log_file.open( log_file_name.begin(), test_or_log ? std::ios::in : std::ios::out );

    STREAMS_BOOST_REQUIRE_MESSAGE( m_log_file.is_open(),
    ...
    if( m_test_or_log ) {
        ...
        STREAMS_BOOST_CHECK_EQUAL( *tit, FILE_SIG ); 
        ++tit;
        STREAMS_BOOST_CHECK_EQUAL( *tit, ELOG_VER );
    }
    else {
...
expectations_logger::decision_point( const_string, std::size_t )
{
    if( m_test_or_log ) {
        ...
        STREAMS_BOOST_CHECK_EQUAL( *tit, DP_SIG ); ++tit;
...
expectations_logger::enter_scope( const_string, std::size_t, const_string scope_name )
{
    if( m_test_or_log ) {
        ...
        STREAMS_BOOST_CHECK_EQUAL( *tit, SCOPE_SIG ); ++tit;
        STREAMS_BOOST_CHECK_EQUAL( *tit, scope_name );
    }
    else {
...
expectations_logger::allocated( const_string, std::size_t, void*, std::size_t s )
{
    if( m_test_or_log ) {
        ...
        STREAMS_BOOST_CHECK_EQUAL( *tit, ALLOC_SIG ); ++tit;
        STREAMS_BOOST_CHECK_EQUAL( lexical_cast<std::size_t>( *tit ), s );
    }
    else {
...
expectations_logger::data_flow( const_string d )
{
    if( m_test_or_log ) {
        ...
        STREAMS_BOOST_CHECK_EQUAL( *tit, DATA_SIG ); ++tit;
        STREAMS_BOOST_CHECK_EQUAL( *tit, d );
    }
    else {
...
expectations_logger::return_value( const_string default_value )
{
    if( m_test_or_log ) {
        ...
        STREAMS_BOOST_CHECK_EQUAL( *tit, RETURN_SIG ); ++tit;
...
#endif // STREAMS_BOOST_TEST_LOGGED_EXPECTATIONS_IPP_120905GER
