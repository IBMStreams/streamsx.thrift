...
#ifndef STREAMS_BOOST_TEST_EXECUTION_MONITOR_HPP_071894GER
#define STREAMS_BOOST_TEST_EXECUTION_MONITOR_HPP_071894GER
...
class STREAMS_BOOST_TEST_DECL translate_exception_base {
public:
    // Constructor
    explicit    translate_exception_base( streams_boost::scoped_ptr<translate_exception_base>& next )
    {
        next.swap( m_next );
    }
...
protected:
    // Data members
    streams_boost::scoped_ptr<translate_exception_base> m_next;
};
...
class STREAMS_BOOST_TEST_DECL execution_exception {
...
public:
    ...
    struct STREAMS_BOOST_TEST_DECL location {
...
class STREAMS_BOOST_TEST_DECL execution_monitor {
public:
    // Constructor
    execution_monitor()
    ...
    void        register_exception_translator( ExceptionTranslator const& tr, streams_boost::type<Exception>* = 0 );
...
private:
    ...
    // Data members
    streams_boost::scoped_ptr<detail::translate_exception_base> m_custom_translators;
    streams_boost::scoped_array<char>                           m_alt_stack;
}; // execution_monitor
...
{
    typedef streams_boost::scoped_ptr<translate_exception_base> base_ptr;
public:
    ...
    int operator()( unit_test::callback0<int> const& F )
    {
        try {
            ...
            m_translator( e );
            return streams_boost::exit_exception_failure;
        }
    }
...
template<typename Exception, typename ExceptionTranslator>
void
execution_monitor::register_exception_translator( ExceptionTranslator const& tr, streams_boost::type<Exception>* )
{
...
#define STREAMS_BOOST_TEST_SYS_ASSERT( exp ) if( (exp) ) ; else throw ::streams_boost::system_error( STREAMS_BOOST_STRINGIZE( exp ) )
