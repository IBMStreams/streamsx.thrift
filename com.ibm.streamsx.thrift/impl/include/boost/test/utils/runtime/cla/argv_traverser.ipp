...
#ifndef STREAMS_BOOST_RT_CLA_ARGV_TRAVERSER_IPP_070604GER
#define STREAMS_BOOST_RT_CLA_ARGV_TRAVERSER_IPP_070604GER
...
: p_ignore_mismatch( false ), p_separator( STREAMS_BOOST_RT_PARAM_LITERAL( ' ' ) )
{
}
...
argv_traverser::init( int argc, char_type** argv )
{
    for( int index = 1; index < argc; ++index ) {
        m_buffer += argv[index];
        if( index != argc-1 )
            m_buffer += STREAMS_BOOST_RT_PARAM_LITERAL( ' ' );
    }
    ...
    STREAMS_BOOST_RT_PARAM_TRACE( "Input buffer: " << m_buffer );
...
argv_traverser::remainder( int& argc, char_type** argv )
{
    ...
    while(pos < m_remainder_size ) {
        ...
        pos = std::find( m_remainder.get() + pos, m_remainder.get() + m_remainder_size, 
                         STREAMS_BOOST_RT_PARAM_LITERAL( ' ' ) ) - m_remainder.get();
        m_remainder[pos++] = STREAMS_BOOST_RT_PARAM_LITERAL( '\0' );
    }
}
...
STREAMS_BOOST_RT_PARAM_INLINE std::size_t
...
#endif // STREAMS_BOOST_RT_CLA_ARGV_TRAVERSER_IPP_070604GER
