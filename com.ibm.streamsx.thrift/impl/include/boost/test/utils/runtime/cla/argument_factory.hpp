...
#ifndef STREAMS_BOOST_RT_CLA_ARGUMENT_FACTORY_HPP_062604GER
#define STREAMS_BOOST_RT_CLA_ARGUMENT_FACTORY_HPP_062604GER
...
#include <streams_boost/test/utils/runtime/interpret_argument_value.hpp>

#include <streams_boost/test/utils/runtime/cla/fwd.hpp>
#include <streams_boost/test/utils/runtime/cla/value_generator.hpp>
#include <streams_boost/test/utils/runtime/cla/value_handler.hpp>
...
#include <streams_boost/test/utils/runtime/cla/detail/argument_value_usage.hpp>

#include <streams_boost/test/utils/runtime/cla/iface/argument_factory.hpp>
...
struct default_value_interpreter {
    template<typename T>
    void operator()( argv_traverser& tr, streams_boost::optional<T>& value )
    {
...
struct typed_argument_factory : public argument_factory {
    ...
    {}
    STREAMS_BOOST_RT_PARAM_UNNEEDED_VIRTUAL ~typed_argument_factory() {}
    ...
    void                accept_modifier( Modifier const& m )
    {
        ...
        if( m.has( default_value ) ) {
            STREAMS_BOOST_RT_PARAM_VALIDATE_LOGIC( !m_value_generator, 
                STREAMS_BOOST_RT_PARAM_LITERAL( "multiple value generators for parameter" ) );
        ...
        if( m.has( assign_to ) ) {
            STREAMS_BOOST_RT_PARAM_VALIDATE_LOGIC( !m_value_handler, 
                STREAMS_BOOST_RT_PARAM_LITERAL( "multiple value handlers for parameter" ) );
...
// !! private?
    ...
    unit_test::callback2<parser const&,streams_boost::optional<T>&>     m_value_generator;
    unit_test::callback2<argv_traverser&,streams_boost::optional<T>&>   m_value_interpreter;
};
...
typed_argument_factory<T>::produce_using( parameter& p, argv_traverser& tr )
{
    ...
    try {
        ...
        STREAMS_BOOST_RT_PARAM_TRACE( "Fail to parse argument value" );
    ...
    STREAMS_BOOST_RT_CLA_VALIDATE_INPUT( !!value || p.p_optional_value, tr, 
        STREAMS_BOOST_RT_PARAM_LITERAL( "Argument value missing for parameter " ) << p.id_2_report() );

    STREAMS_BOOST_RT_CLA_VALIDATE_INPUT( !actual_arg || p.p_multiplicable, tr, 
        STREAMS_BOOST_RT_PARAM_LITERAL( "Unexpected repetition of the parameter " ) << p.id_2_report() );
    ...
    if( !p.p_multiplicable )
        actual_arg.reset( p.p_optional_value && (rtti::type_id<T>() != rtti::type_id<bool>())
            ? static_cast<argument*>(new typed_argument<streams_boost::optional<T> >( p, value ))
            : static_cast<argument*>(new typed_argument<T>( p, *value )) );
    else {
        typedef std::list<streams_boost::optional<T> > optional_list;
...
#endif // STREAMS_BOOST_RT_CLA_ARGUMENT_FACTORY_HPP_062604GER
