#ifndef _GREGORIAN__CONVERSION_STREAMS_STREAMS_HPP___
#define _GREGORIAN__CONVERSION_STREAMS_STREAMS_HPP___
...
namespace gregorian {
  ...
  std::tm to_tm(const date& d)
  {
    if (d.is_special())
    {
        ...
        streams_boost::throw_exception(std::out_of_range(s));
    }

    std::tm datetm = {}; // zero initialization is needed for extension members, like tm_zone
    streams_boost::gregorian::date::ymd_type ymd = d.year_month_day();
