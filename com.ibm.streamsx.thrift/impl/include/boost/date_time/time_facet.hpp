...
#ifndef _DATE_TIME_FACET__STREAMS_STREAMS_HPP__
#define _DATE_TIME_FACET__STREAMS_STREAMS_HPP__
...
#include <streams_boost/date_time/string_convert.hpp>
#include <streams_boost/date_time/special_defs.hpp>
#include <streams_boost/date_time/time_resolution_traits.hpp> // absolute_value
...
namespace date_time {
  ...
  class time_facet :
    public streams_boost::date_time::date_facet<typename time_type::date_type , CharT, OutItrT> {
   public:
    typedef typename time_type::date_type date_type;
    typedef typename time_type::time_duration_type time_duration_type;
    typedef streams_boost::date_time::period<time_type,time_duration_type> period_type;
    typedef streams_boost::date_time::date_facet<typename time_type::date_type, CharT, OutItrT> base_type;
                ...
                const time_type& a_time) const
    {
      ...
      if (format.find(seconds_with_fractional_seconds_format) != string_type::npos) {
        ...
        streams_boost::algorithm::replace_all(format,
      ...
      if (format.find(posix_zone_string_format) != string_type::npos) {
        if(a_time.zone_abbrev().empty()) {
          ...
          streams_boost::algorithm::erase_all(format, posix_zone_string_format);
        }
        else{
          streams_boost::algorithm::replace_all(format,
      ...
      if (format.find(zone_name_format) != string_type::npos) {
        if(a_time.zone_name().empty()) {
          ...
          ss << ' ' << zone_name_format;
          streams_boost::algorithm::erase_all(format, ss.str());
        }
        else{
          streams_boost::algorithm::replace_all(format,
                                        zone_name_format,
      ...
      if (format.find(zone_iso_extended_format) != string_type::npos) {
        if(a_time.zone_name(true).empty()) {
          ...
          streams_boost::algorithm::erase_all(format, zone_iso_extended_format);
        }
        else{
          streams_boost::algorithm::replace_all(format,
      ...
      if (format.find(zone_iso_format) != string_type::npos) {
        if(a_time.zone_abbrev(true).empty()) {
          ...
          streams_boost::algorithm::erase_all(format, zone_iso_format);
        }
        else{
          streams_boost::algorithm::replace_all(format,
                                        zone_iso_format,
      ...
      if (format.find(fractional_seconds_or_none_format) != string_type::npos) {
        ...
        if (frac_str.size()) {
          ...
          streams_boost::algorithm::erase_all(format,
                                      fractional_seconds_or_none_format);
        }
      }
                ...
                const time_duration_type& a_time_dur) const
    {
      ...
      if (a_time_dur.is_negative()) {
        ...
        streams_boost::algorithm::erase_all(format, duration_sign_negative_only);
        // remove all the %+ in the string with '+'
        streams_boost::algorithm::replace_all(format,
                                      duration_sign_always,
                                      positive_sign);
      ...
      if (format.find(unrestricted_hours_format) != string_type::npos) {
        ...
        streams_boost::algorithm::replace_all(format, unrestricted_hours_format, hours_str);
      }
      ...
      if (format.find(hours_format) != string_type::npos) {
        if (hours_str.empty())
          hours_str = hours_as_string(a_time_dur);
        STREAMS_BOOST_ASSERT(hours_str.length() <= 2);
        streams_boost::algorithm::replace_all(format, hours_format, hours_str);
      }
  ...
  protected:
    ...
    integral_as_string(IntT val, int width = 2)
    {
      ...
      char_type buff[34];
      ss << _i64toa(static_cast<streams_boost::int64_t>(val), buff, 10);
#else
      ss << val;
#endif
      return ss.str();
  ...
  class time_input_facet :
    public streams_boost::date_time::date_input_facet<typename time_type::date_type , CharT, InItrT> {
    public:
      typedef typename time_type::date_type date_type;
      typedef typename time_type::time_duration_type time_duration_type;
      typedef typename time_duration_type::fractional_seconds_type fracional_seconds_type;
      typedef streams_boost::date_time::period<time_type,time_duration_type> period_type;
      typedef streams_boost::date_time::date_input_facet<typename time_type::date_type, CharT, InItrT> base_type;
    ...
    protected:
      ...
      InItrT check_special_value(InItrT& sitr,InItrT& stream_end, temporal_type& tt, char_type c='\0') const
      {
        ...
        if(mr.current_match == match_results::PARSE_ERROR) {
          ...
          streams_boost::throw_exception(std::ios_base::failure("Parse failed. No match found for '" + tmp + "'"));
          STREAMS_BOOST_DATE_TIME_UNREACHABLE_EXPRESSION(return sitr); // should never reach
        }
