#ifndef DATE_TIME_TZ_DB_BASE_STREAMS_HPP__
#define DATE_TIME_TZ_DB_BASE_STREAMS_HPP__
...
#include <streams_boost/date_time/time_zone_names.hpp>
#include <streams_boost/date_time/time_zone_base.hpp>
#include <streams_boost/date_time/time_parsing.hpp>
...
namespace streams_boost {
  namespace date_time {
    ...
    public:
      ...
      typedef streams_boost::date_time::dst_adjustment_offsets<time_duration_type> dst_adjustment_offsets;
      ...
      void load_from_file(const std::string& pathspec)
      {
        ...
        if(!ifs){
          streams_boost::throw_exception(data_not_accessible(pathspec));
        }
      ...
      bool add_record(const string_type& region, 
                      streams_boost::shared_ptr<time_zone_base_type> tz)
      {
      ...
      streams_boost::shared_ptr<time_zone_base_type> 
      time_zone_from_region(const string_type& region) const 
      {
        ...
        if(record == m_zone_map.end()){
          return streams_boost::shared_ptr<time_zone_base_type>(); //null pointer
        }
    ...
    private:
      typedef std::map<string_type, streams_boost::shared_ptr<time_zone_base_type> > map_type;
      map_type m_zone_map;
      ...
      {
        typedef streams_boost::char_separator<char_type, std::char_traits<char_type> > char_separator_type;
        typedef streams_boost::tokenizer<char_separator_type,
      ...
      bool parse_string(string_type& s)
      {
        ...
        typedef streams_boost::token_iterator_generator<streams_boost::escaped_list_separator<char_type>, string_type::const_iterator, string_type >::type token_iter_type;

        token_iter_type i = streams_boost::make_token_iterator<string_type>(s.begin(), s.end(),streams_boost::escaped_list_separator<char_type>());
        ...
        if (result.size() != expected_fields) { 
          ...
          streams_boost::throw_exception(bad_field_count(msg.str()));
          STREAMS_BOOST_DATE_TIME_UNREACHABLE_EXPRESSION(return false); // should never reach
        }
        ...
        streams_boost::shared_ptr<rule_type> rules;
        ...
        if(has_dst){
          ...
          rules = 
            streams_boost::shared_ptr<rule_type>(parse_rules(result[START_DATE_RULE],
                                                     result[END_DATE_RULE]));
        }
        string_type id(result[ID]);
        streams_boost::shared_ptr<time_zone_base_type> zone(new time_zone_type(names, utc_offset, adjust, rules));
...
#endif // DATE_TIME_TZ_DB_BASE_STREAMS_HPP__
