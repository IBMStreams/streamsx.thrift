#ifndef POSIX_TIME_SERIALIZE_STREAMS_HPP___
#define POSIX_TIME_SERIALIZE_STREAMS_HPP___
...
#include "streams_boost/date_time/gregorian/greg_serialize.hpp"
...
STREAMS_BOOST_SERIALIZATION_SPLIT_FREE(streams_boost::posix_time::ptime)
STREAMS_BOOST_SERIALIZATION_SPLIT_FREE(streams_boost::posix_time::time_duration)
STREAMS_BOOST_SERIALIZATION_SPLIT_FREE(streams_boost::posix_time::time_period)
          ...
          unsigned int /*version*/)
{
  ...
  if(!d.is_special()) {
    ar & make_nvp("ptime_time_duration", td);
    pt = streams_boost::posix_time::ptime(d,td);
  }
  else {
    pt = streams_boost::posix_time::ptime(d.as_special());
  }
                                ...
                                const unsigned int /*file_version*/)
{
  ...
  new(pt) streams_boost::posix_time::ptime(streams_boost::posix_time::not_a_date_time);
}
...
template<class Archive>
void load(Archive & ar, 
          streams_boost::posix_time::time_period & tp, 
          unsigned int /*version*/)
{
  ...
  ar & make_nvp("time_period_begin", beg);
  ar & make_nvp("time_period_end", end);
  tp = streams_boost::posix_time::time_period(beg, end);
}
                                ...
                                const unsigned int /*file_version*/)
{
  ...
  posix_time::ptime beg(d,td);
  posix_time::ptime end(d,td);
  new(tp) streams_boost::posix_time::time_period(beg,end);
}
