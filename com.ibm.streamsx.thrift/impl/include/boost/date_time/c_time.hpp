#ifndef DATE_TIME_C_TIME_STREAMS_HPP___
#define DATE_TIME_C_TIME_STREAMS_HPP___
...
namespace date_time {
  ...
  struct c_time {
    public:
#if defined(STREAMS_BOOST_DATE_TIME_HAS_REENTRANT_STD_FUNCTIONS)
      ...
      static std::tm* localtime(const std::time_t* t, std::tm* result)
      {
        ...
        if (!result)
          streams_boost::throw_exception(std::runtime_error("could not convert calendar time to local time"));
        return result;
      }
      //! requires a pointer to a user created std::tm struct
      inline
      static std::tm* gmtime(const std::time_t* t, std::tm* result)
      {
        ...
        if (!result)
          streams_boost::throw_exception(std::runtime_error("could not convert calendar time to UTC time"));
        return result;
      }
#else // STREAMS_BOOST_HAS_THREADS
...
#endif // STREAMS_BOOST_HAS_THREADS
  };
}} // namespaces

#endif // DATE_TIME_C_TIME_STREAMS_HPP___
