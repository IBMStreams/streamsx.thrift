#ifndef DATE_TIME_FILETIME_FUNCTIONS_STREAMS_HPP__
#define DATE_TIME_FILETIME_FUNCTIONS_STREAMS_HPP__
 ...
 * STREAMS_BOOST_HAS_FTIME defined.
 */

#include <streams_boost/date_time/compiler_config.hpp>

#if defined(STREAMS_BOOST_HAS_FTIME) // skip this file if no FILETIME
...
#include <streams_boost/date_time/date_defs.hpp>
...
namespace winapi {
    ...
    extern "C" {
        ...
        struct FILETIME
        {
            streams_boost::uint32_t dwLowDateTime;
            streams_boost::uint32_t dwHighDateTime;
        };
        struct SYSTEMTIME
        {
            streams_boost::uint16_t wYear;
            streams_boost::uint16_t wMonth;
            streams_boost::uint16_t wDayOfWeek;
            streams_boost::uint16_t wDay;
            streams_boost::uint16_t wHour;
            streams_boost::uint16_t wMinute;
            streams_boost::uint16_t wSecond;
            streams_boost::uint16_t wMilliseconds;
        };
...
#endif // defined(STREAMS_BOOST_USE_WINDOWS_H)
...
#elif defined(STREAMS_BOOST_NO_GETSYSTEMTIMEASFILETIME)
        system_time st;
        GetSystemTime(&st);
     ...
     *       streams_boost::date_time::winapi::file_time type.
     */
    template< typename FileTimeT >
    inline streams_boost::uint64_t file_time_to_microseconds(FileTimeT const& ft)
    {
...
TimeT time_from_ftime(const FileTimeT& ft)
{
    ...
    // https://svn.streams_boost.org/trac/streams_boost/ticket/2523
...
#if !defined(STREAMS_BOOST_DATE_TIME_POSIX_TIME_STD_CONFIG)
...
}} // streams_boost::date_time

#endif // STREAMS_BOOST_HAS_FTIME

#endif // DATE_TIME_FILETIME_FUNCTIONS_STREAMS_HPP__
