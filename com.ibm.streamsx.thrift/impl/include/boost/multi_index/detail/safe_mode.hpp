...
#ifndef STREAMS_BOOST_MULTI_INDEX_DETAIL_SAFE_MODE_HPP
#define STREAMS_BOOST_MULTI_INDEX_DETAIL_SAFE_MODE_HPP
 ...
 * !defined(STREAMS_BOOST_MULTI_INDEX_ENABLE_SAFE_MODE).
 */

#if !defined(STREAMS_BOOST_MULTI_INDEX_ENABLE_SAFE_MODE)
#undef STREAMS_BOOST_MULTI_INDEX_SAFE_MODE_ASSERT
#define STREAMS_BOOST_MULTI_INDEX_SAFE_MODE_ASSERT(expr,error_code) ((void)0)
#else
#if !defined(STREAMS_BOOST_MULTI_INDEX_SAFE_MODE_ASSERT)
#include <streams_boost/assert.hpp>
#define STREAMS_BOOST_MULTI_INDEX_SAFE_MODE_ASSERT(expr,error_code) STREAMS_BOOST_ASSERT(expr)
#endif
#endif

#define STREAMS_BOOST_MULTI_INDEX_CHECK_VALID_ITERATOR(it)                           \
  STREAMS_BOOST_MULTI_INDEX_SAFE_MODE_ASSERT(                                        \
    safe_mode::check_valid_iterator(it),                                     \
    safe_mode::invalid_iterator);

#define STREAMS_BOOST_MULTI_INDEX_CHECK_DEREFERENCEABLE_ITERATOR(it)                 \
  STREAMS_BOOST_MULTI_INDEX_SAFE_MODE_ASSERT(                                        \
    safe_mode::check_dereferenceable_iterator(it),                           \
    safe_mode::not_dereferenceable_iterator);

#define STREAMS_BOOST_MULTI_INDEX_CHECK_INCREMENTABLE_ITERATOR(it)                   \
  STREAMS_BOOST_MULTI_INDEX_SAFE_MODE_ASSERT(                                        \
    safe_mode::check_incrementable_iterator(it),                             \
    safe_mode::not_incrementable_iterator);

#define STREAMS_BOOST_MULTI_INDEX_CHECK_DECREMENTABLE_ITERATOR(it)                   \
  STREAMS_BOOST_MULTI_INDEX_SAFE_MODE_ASSERT(                                        \
    safe_mode::check_decrementable_iterator(it),                             \
    safe_mode::not_decrementable_iterator);

#define STREAMS_BOOST_MULTI_INDEX_CHECK_IS_OWNER(it,cont)                            \
  STREAMS_BOOST_MULTI_INDEX_SAFE_MODE_ASSERT(                                        \
    safe_mode::check_is_owner(it,cont),                                      \
    safe_mode::not_owner);

#define STREAMS_BOOST_MULTI_INDEX_CHECK_SAME_OWNER(it0,it1)                          \
  STREAMS_BOOST_MULTI_INDEX_SAFE_MODE_ASSERT(                                        \
    safe_mode::check_same_owner(it0,it1),                                    \
    safe_mode::not_same_owner);

#define STREAMS_BOOST_MULTI_INDEX_CHECK_VALID_RANGE(it0,it1)                         \
  STREAMS_BOOST_MULTI_INDEX_SAFE_MODE_ASSERT(                                        \
    safe_mode::check_valid_range(it0,it1),                                   \
    safe_mode::invalid_range);

#define STREAMS_BOOST_MULTI_INDEX_CHECK_OUTSIDE_RANGE(it,it0,it1)                    \
  STREAMS_BOOST_MULTI_INDEX_SAFE_MODE_ASSERT(                                        \
    safe_mode::check_outside_range(it,it0,it1),                              \
    safe_mode::inside_range);

#define STREAMS_BOOST_MULTI_INDEX_CHECK_IN_BOUNDS(it,n)                              \
  STREAMS_BOOST_MULTI_INDEX_SAFE_MODE_ASSERT(                                        \
    safe_mode::check_in_bounds(it,n),                                        \
    safe_mode::out_of_bounds);

#define STREAMS_BOOST_MULTI_INDEX_CHECK_DIFFERENT_CONTAINER(cont0,cont1)             \
  STREAMS_BOOST_MULTI_INDEX_SAFE_MODE_ASSERT(                                        \
...
#include <streams_boost/multi_index/detail/iter_adaptor.hpp>
#include <streams_boost/multi_index/safe_mode_errors.hpp>
...
inline void detach_equivalent_iterators(Iterator& it)
{
  if(it.valid()){
    {
#if defined(STREAMS_BOOST_HAS_THREADS)
      streams_boost::detail::lightweight_mutex::scoped_lock lock(it.cont->mutex);
#endif
...
STREAMS_BOOST_MULTI_INDEX_PRIVATE_IF_MEMBER_TEMPLATE_FRIENDS:
...
#if defined(STREAMS_BOOST_HAS_THREADS)
  streams_boost::detail::lightweight_mutex mutex;
#endif
};
...
void safe_iterator_base::attach(safe_container_base* cont_)
{
  ...
  if(cont){
#if defined(STREAMS_BOOST_HAS_THREADS)
    streams_boost::detail::lightweight_mutex::scoped_lock lock(cont->mutex);
#endif
...
public:
  ...
  {
    STREAMS_BOOST_MULTI_INDEX_CHECK_VALID_ITERATOR(x);
...
private:
  friend class streams_boost::multi_index::detail::iter_adaptor_access;

  reference dereference()const
  {
    STREAMS_BOOST_MULTI_INDEX_CHECK_VALID_ITERATOR(*this);
    STREAMS_BOOST_MULTI_INDEX_CHECK_DEREFERENCEABLE_ITERATOR(*this);
  ...
  bool equal(const safe_iterator& x)const
  {
    STREAMS_BOOST_MULTI_INDEX_CHECK_VALID_ITERATOR(*this);
    STREAMS_BOOST_MULTI_INDEX_CHECK_VALID_ITERATOR(x);
    STREAMS_BOOST_MULTI_INDEX_CHECK_SAME_OWNER(*this,x);
  ...
  {
    STREAMS_BOOST_MULTI_INDEX_CHECK_VALID_ITERATOR(*this);
    STREAMS_BOOST_MULTI_INDEX_CHECK_INCREMENTABLE_ITERATOR(*this);
    ++(this->base_reference());
  }
  ...
  {
    STREAMS_BOOST_MULTI_INDEX_CHECK_VALID_ITERATOR(*this);
    STREAMS_BOOST_MULTI_INDEX_CHECK_DECREMENTABLE_ITERATOR(*this);
    --(this->base_reference());
  }
  ...
  {
    STREAMS_BOOST_MULTI_INDEX_CHECK_VALID_ITERATOR(*this);
    STREAMS_BOOST_MULTI_INDEX_CHECK_IN_BOUNDS(*this,n);
...
#endif /* STREAMS_BOOST_MULTI_INDEX_ENABLE_SAFE_MODE */
