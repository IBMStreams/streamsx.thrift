...
typename detail::channel_pointer_type<HomogeneousView>::type interleaved_view_get_raw_data(const HomogeneousView& view) {
    STREAMS_BOOST_STATIC_ASSERT((!is_planar<HomogeneousView>::value && view_is_basic<HomogeneousView>::value));
    STREAMS_BOOST_STATIC_ASSERT((streams_boost::is_pointer<typename HomogeneousView::x_iterator>::value));
...
struct color_converted_view_type : public detail::_color_converted_view_type<SrcView,
    ...
    GIL_CLASS_REQUIRE(DstP, streams_boost::gil, MutablePixelConcept)//why does it have to be mutable???
};
...
namespace detail {
    ...
    private:
        ...
        STREAMS_BOOST_STATIC_CONSTANT(bool, adjacent=
    ...
    struct nth_channel_deref_fn {
        STREAMS_BOOST_STATIC_CONSTANT(bool, is_mutable=pixel_is_reference<SrcP>::value && pixel_reference_is_mutable<SrcP>::value);
    private:
...
private:
    GIL_CLASS_REQUIRE(View, streams_boost::gil, ImageViewConcept)
