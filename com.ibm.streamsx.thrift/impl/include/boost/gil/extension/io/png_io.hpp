...
inline void png_read_view(const char* filename,const View& view) {
    STREAMS_BOOST_STATIC_ASSERT(png_read_support<View>::is_supported);
    detail::png_reader m(filename);
    m.apply(view);
}
...
inline void png_read_image(const char* filename,Image& im) {
    STREAMS_BOOST_STATIC_ASSERT(png_read_support<typename Image::view_t>::is_supported);
    detail::png_reader m(filename);
    m.read_image(im);
}
...
inline void png_write_view(const char* filename,const View& view) {
    STREAMS_BOOST_STATIC_ASSERT(png_write_support<View>::is_supported);
