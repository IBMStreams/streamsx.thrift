...
#include <streams_boost/mpl/max.hpp>
...
GIL's variant is similar to streams_boost::variant in spirit (hence we borrow the name from there) but it differs in several ways from the current streams_boost
