...
Example:
\code
STREAMS_BOOST_STATIC_ASSERT((size<rgb8_pixel_t>::value == 3));
STREAMS_BOOST_STATIC_ASSERT((size<cmyk8_planar_ptr_t>::value == 4));
\endcode
*/
...
template <typename ColorBase, int K> struct kth_semantic_element_type {
    STREAMS_BOOST_STATIC_CONSTANT(int, semantic_index = (mpl::at_c<typename ColorBase::layout_t::channel_mapping_t,K>::type::value));
...
void set_red_to_max(Pixel& pixel) {
    streams_boost::function_requires<MutablePixelConcept<Pixel> >();
    STREAMS_BOOST_STATIC_ASSERT((contains_color<Pixel, red_t>::value));
...
STREAMS_BOOST_STATIC_ASSERT((streams_boost::is_same<element_t, const bits8*>::value));
\endcode
*/
