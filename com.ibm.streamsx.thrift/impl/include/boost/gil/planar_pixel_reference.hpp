...
#include <streams_boost/mpl/range_c.hpp>
...
public:
    STREAMS_BOOST_STATIC_CONSTANT(bool, is_mutable = channel_traits<ChannelReference>::is_mutable);
...
#if STREAMS_BOOST_WORKAROUND(__HP_aCC, >= 61700) || STREAMS_BOOST_WORKAROUND(__INTEL_COMPILER, >= 1000)
...
void swap(const streams_boost::gil::planar_pixel_reference<CR,CS> x, R& y) { 
    streams_boost::gil::swap_proxy<typename streams_boost::gil::planar_pixel_reference<CR,CS>::value_type>(x,y); 
}
...
void swap(typename streams_boost::gil::planar_pixel_reference<CR,CS>::value_type& x, const streams_boost::gil::planar_pixel_reference<CR,CS> y) { 
    streams_boost::gil::swap_proxy<typename streams_boost::gil::planar_pixel_reference<CR,CS>::value_type>(x,y); 
}
...
/// \ingroup PixelModelPlanarRef
template <typename CR, typename CS> inline
void swap(const streams_boost::gil::planar_pixel_reference<CR,CS> x, const streams_boost::gil::planar_pixel_reference<CR,CS> y) { 
    streams_boost::gil::swap_proxy<typename streams_boost::gil::planar_pixel_reference<CR,CS>::value_type>(x,y); 
}
}   // namespace std
