...
STREAMS_BOOST_STATIC_ASSERT((sizeof(rgb565_pixel_t)==2));
...
template <typename BitField,      // A type that holds the bits of the pixel. Typically an integral type, like streams_boost::uint16_t
...
struct packed_pixel {
    ...
    STREAMS_BOOST_STATIC_CONSTANT(bool, is_mutable = channel_traits<typename mpl::front<ChannelRefVec>::type>::is_mutable);
    ...
    packed_pixel(int chan0, int chan1) : _bitfield(0) { 
        STREAMS_BOOST_STATIC_ASSERT((num_channels<packed_pixel>::value==2)); 
        at_c<0>(*this)=chan0; at_c<1>(*this)=chan1; 
    } 
    packed_pixel(int chan0, int chan1, int chan2) : _bitfield(0) { 
        STREAMS_BOOST_STATIC_ASSERT((num_channels<packed_pixel>::value==3)); 
    ...
    packed_pixel(int chan0, int chan1, int chan2, int chan3) : _bitfield(0) { 
        STREAMS_BOOST_STATIC_ASSERT((num_channels<packed_pixel>::value==4)); 
    ...
    packed_pixel(int chan0, int chan1, int chan2, int chan3, int chan4) : _bitfield(0) { 
        STREAMS_BOOST_STATIC_ASSERT((num_channels<packed_pixel>::value==5)); 
