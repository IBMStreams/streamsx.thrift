#ifndef STREAMS_BOOST_TT_IS_ABSTRACT_CLASS_HPP
#define STREAMS_BOOST_TT_IS_ABSTRACT_CLASS_HPP
...
//              and John Maddock.
//  - Dec 2004: Added new config macro STREAMS_BOOST_NO_IS_ABSTRACT which causes the template
//              to degrade gracefully, rather than trash the compiler (John Maddock).
//

#include <streams_boost/type_traits/intrinsics.hpp>
#ifndef STREAMS_BOOST_IS_ABSTRACT
...
#ifdef STREAMS_BOOST_IS_ABSTRACT
template <class T>
struct is_abstract_imp
{
   STREAMS_BOOST_STATIC_CONSTANT(bool, value = STREAMS_BOOST_IS_ABSTRACT(T));
};
...
struct is_abstract_imp2
{
   ...
   //
   STREAMS_BOOST_STATIC_ASSERT(sizeof(T) != 0);

   // GCC2 won't even parse this template if we embed the computation
   // of s1 in the computation of value.
#ifdef __GNUC__
   STREAMS_BOOST_STATIC_CONSTANT(std::size_t, s1 = sizeof(is_abstract_imp2<T>::template check_sig<T>(0)));
#else
#if STREAMS_BOOST_WORKAROUND(STREAMS_BOOST_MSVC_FULL_VER, >= 140050000)
#pragma warning(push)
#pragma warning(disable:6334)
#endif
   STREAMS_BOOST_STATIC_CONSTANT(std::size_t, s1 = sizeof(check_sig<T>(0)));
#if STREAMS_BOOST_WORKAROUND(STREAMS_BOOST_MSVC_FULL_VER, >= 140050000)
#pragma warning(pop)
#endif
#endif
...
struct is_abstract_imp
{
   typedef is_abstract_select< ::streams_boost::is_class<T>::value> selector;
...
STREAMS_BOOST_TT_AUX_BOOL_TRAIT_DEF1(is_abstract,T,::streams_boost::detail::is_abstract_imp<T>::value)
#else
STREAMS_BOOST_TT_AUX_BOOL_TRAIT_DEF1(is_abstract,T,::streams_boost::detail::is_polymorphic_imp<T>::value)
#endif
...
#endif //STREAMS_BOOST_TT_IS_ABSTRACT_CLASS_HPP
