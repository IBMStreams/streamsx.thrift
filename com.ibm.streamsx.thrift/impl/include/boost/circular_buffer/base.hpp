...
#if !defined(STREAMS_BOOST_CIRCULAR_BUFFER_BASE_HPP)
#define STREAMS_BOOST_CIRCULAR_BUFFER_BASE_HPP
...
/*!
    ...
    http://www.streams_boost.org/libs/circular_buffer/doc/circular_buffer.html
*/
...
// Requirements
    STREAMS_BOOST_CLASS_REQUIRE(T, streams_boost, SGIAssignableConcept);
...
// Element access
    ...
    */
    reference operator [] (size_type index) {
        STREAMS_BOOST_CB_ASSERT(index < size()); // check for invalid index
    ...
    */
    reference front() {
        STREAMS_BOOST_CB_ASSERT(!empty()); // check for empty buffer (front element not available)
        return *m_first;
    }
    ...
    */
    reference back() {
        STREAMS_BOOST_CB_ASSERT(!empty()); // check for empty buffer (back element not available)
    ...
    */
    void rotate(const_iterator new_begin) {
        STREAMS_BOOST_CB_ASSERT(new_begin.is_valid(this)); // check for uninitialized or invalidated iterator
        STREAMS_BOOST_CB_ASSERT(new_begin.m_it != 0);      // check for iterator pointing to end()
        if (full()) {
...
// Construction/Destruction
    ...
    */
    ...
    : m_size(n), m_alloc(alloc) {
        STREAMS_BOOST_CB_ASSERT(buffer_capacity >= size()); // check for capacity lower than size
...
// Assign methods
    ...
    */
    void assign(capacity_type buffer_capacity, size_type n, param_value_type item) {
        STREAMS_BOOST_CB_ASSERT(buffer_capacity >= n); // check for new capacity lower than n
...
// Insert
    ...
    */
    iterator insert(iterator pos, param_value_type item = value_type()) {
        STREAMS_BOOST_CB_ASSERT(pos.is_valid(this)); // check for uninitialized or invalidated iterator
        iterator b = begin();
        if (full() && pos == b)
            return b;
...
// Erase
    ...
    */
    iterator erase(iterator pos) {
        STREAMS_BOOST_CB_ASSERT(pos.is_valid(this)); // check for uninitialized or invalidated iterator
        STREAMS_BOOST_CB_ASSERT(pos.m_it != 0);      // check for iterator pointing to end()
    ...
    */
    iterator erase(iterator first, iterator last) {
        STREAMS_BOOST_CB_ASSERT(first.is_valid(this)); // check for uninitialized or invalidated iterator
        STREAMS_BOOST_CB_ASSERT(last.is_valid(this));  // check for uninitialized or invalidated iterator
        STREAMS_BOOST_CB_ASSERT(first <= last);        // check for wrong range
        if (first == last)
            return first;
    ...
    */
    void erase_begin(size_type n) {
        STREAMS_BOOST_CB_ASSERT(n <= size()); // check for n greater than size
#if STREAMS_BOOST_CB_ENABLE_DEBUG
...
// Helper methods
    ...
    */
    ...
    void initialize(Iterator first, Iterator last, const false_type&) {
        STREAMS_BOOST_CB_IS_CONVERTIBLE(Iterator, value_type); // check for invalid iterator type
#if STREAMS_BOOST_WORKAROUND(__BORLANDC__, STREAMS_BOOST_TESTED_AT(0x581))
        initialize(first, last, STREAMS_BOOST_ITERATOR_CATEGORY<Iterator>::type());
#else
        initialize(first, last, STREAMS_BOOST_DEDUCED_TYPENAME STREAMS_BOOST_ITERATOR_CATEGORY<Iterator>::type());
#endif
    }
    ...
    void initialize(InputIterator first, InputIterator last, const std::input_iterator_tag&) {
        STREAMS_BOOST_CB_ASSERT_TEMPLATED_ITERATOR_CONSTRUCTORS // check if the STL provides templated iterator constructors
                                                        // for containers
    ...
    void initialize(ForwardIterator first, ForwardIterator last, const std::forward_iterator_tag&) {
        STREAMS_BOOST_CB_ASSERT(std::distance(first, last) >= 0); // check for wrong range
    ...
    void initialize(capacity_type buffer_capacity, IntegralType n, IntegralType item, const true_type&) {
        STREAMS_BOOST_CB_ASSERT(buffer_capacity >= static_cast<size_type>(n)); // check for capacity lower than n
        m_size = static_cast<size_type>(n);
        initialize_buffer(buffer_capacity, item);
        m_first = m_buff;
    ...
    void initialize(capacity_type buffer_capacity, Iterator first, Iterator last, const false_type&) {
        STREAMS_BOOST_CB_IS_CONVERTIBLE(Iterator, value_type); // check for invalid iterator type
#if STREAMS_BOOST_WORKAROUND(__BORLANDC__, STREAMS_BOOST_TESTED_AT(0x581))
        initialize(buffer_capacity, first, last, STREAMS_BOOST_ITERATOR_CATEGORY<Iterator>::type());
#else
        initialize(buffer_capacity, first, last, STREAMS_BOOST_DEDUCED_TYPENAME STREAMS_BOOST_ITERATOR_CATEGORY<Iterator>::type());
#endif
    }
    ...
    void assign(Iterator first, Iterator last, const false_type&) {
        STREAMS_BOOST_CB_IS_CONVERTIBLE(Iterator, value_type); // check for invalid iterator type
#if STREAMS_BOOST_WORKAROUND(__BORLANDC__, STREAMS_BOOST_TESTED_AT(0x581))
        assign(first, last, STREAMS_BOOST_ITERATOR_CATEGORY<Iterator>::type());
#else
        assign(first, last, STREAMS_BOOST_DEDUCED_TYPENAME STREAMS_BOOST_ITERATOR_CATEGORY<Iterator>::type());
#endif
    }
    ...
    void assign(InputIterator first, InputIterator last, const std::input_iterator_tag&) {
        ...
        assign_n(distance, distance,
            cb_details::assign_range<STREAMS_BOOST_DEDUCED_TYPENAME std::deque<value_type, allocator_type>::iterator,
    ...
    void assign(capacity_type new_capacity, Iterator first, Iterator last, const false_type&) {
        STREAMS_BOOST_CB_IS_CONVERTIBLE(Iterator, value_type); // check for invalid iterator type
#if STREAMS_BOOST_WORKAROUND(__BORLANDC__, STREAMS_BOOST_TESTED_AT(0x581))
        assign(new_capacity, first, last, STREAMS_BOOST_ITERATOR_CATEGORY<Iterator>::type());
#else
        assign(new_capacity, first, last, STREAMS_BOOST_DEDUCED_TYPENAME STREAMS_BOOST_ITERATOR_CATEGORY<Iterator>::type());
#endif
    }
    ...
    void insert(const iterator& pos, Iterator first, Iterator last, const false_type&) {
        STREAMS_BOOST_CB_IS_CONVERTIBLE(Iterator, value_type); // check for invalid iterator type
#if STREAMS_BOOST_WORKAROUND(__BORLANDC__, STREAMS_BOOST_TESTED_AT(0x581))
        insert(pos, first, last, STREAMS_BOOST_ITERATOR_CATEGORY<Iterator>::type());
#else
        insert(pos, first, last, STREAMS_BOOST_DEDUCED_TYPENAME STREAMS_BOOST_ITERATOR_CATEGORY<Iterator>::type());
#endif
    }
    ...
    void rinsert(const iterator& pos, Iterator first, Iterator last, const false_type&) {
        STREAMS_BOOST_CB_IS_CONVERTIBLE(Iterator, value_type); // check for invalid iterator type
#if STREAMS_BOOST_WORKAROUND(__BORLANDC__, STREAMS_BOOST_TESTED_AT(0x581))
        rinsert(pos, first, last, STREAMS_BOOST_ITERATOR_CATEGORY<Iterator>::type());
#else
        rinsert(pos, first, last, STREAMS_BOOST_DEDUCED_TYPENAME STREAMS_BOOST_ITERATOR_CATEGORY<Iterator>::type());
#endif
    }
...
#if !defined(STREAMS_BOOST_NO_FUNCTION_TEMPLATE_ORDERING) || defined(STREAMS_BOOST_MSVC)
...
#endif // #if !defined(STREAMS_BOOST_NO_FUNCTION_TEMPLATE_ORDERING) || defined(STREAMS_BOOST_MSVC)

} // namespace streams_boost

#endif // #if !defined(STREAMS_BOOST_CIRCULAR_BUFFER_BASE_HPP)
