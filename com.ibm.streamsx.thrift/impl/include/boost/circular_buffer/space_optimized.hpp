...
#if !defined(STREAMS_BOOST_CIRCULAR_BUFFER_SPACE_OPTIMIZED_HPP)
#define STREAMS_BOOST_CIRCULAR_BUFFER_SPACE_OPTIMIZED_HPP
...
/*!
    ...
    http://www.streams_boost.org/libs/circular_buffer/doc/space_optimized.html
*/
...
// Overridden
...
#if defined(STREAMS_BOOST_CB_TEST)
    ...
    */
...
#endif // #if defined(STREAMS_BOOST_CB_TEST)
    ...
    /*! \brief Change the capacity (and the minimal guaranteed amount of allocated memory) of the
        ...
        \note To explicitly clear the extra allocated memory use the <b>shrink-to-fit</b> technique:<br><br>
              <code>%streams_boost::%circular_buffer_space_optimized\<int\> cb(1000);<br>
              ...<br>
              %streams_boost::%circular_buffer_space_optimized\<int\>(cb).swap(cb);</code><br><br>
    ...
    */
    ...
    , m_capacity_ctrl(capacity_ctrl) {
        reduce_capacity(
            is_same< STREAMS_BOOST_DEDUCED_TYPENAME STREAMS_BOOST_ITERATOR_CATEGORY<InputIterator>::type, std::input_iterator_tag >());
    }
    /*! \endcond */
...
#if defined(STREAMS_BOOST_CB_NEVER_DEFINED)
...
// Declared only for documentation purpose.
...
#endif // #if defined(STREAMS_BOOST_CB_NEVER_DEFINED)
    ...
    */
    void assign(capacity_type capacity_ctrl, size_type n, param_value_type item) {
       STREAMS_BOOST_CB_ASSERT(capacity_ctrl.capacity() >= n); // check for new capacity lower than n
...
# endif
    ...
    static size_type init_capacity(const capacity_type& capacity_ctrl, size_type n) {
        STREAMS_BOOST_CB_ASSERT(capacity_ctrl.capacity() >= n); // check for capacity lower than n
    ...
    static size_type init_capacity(const capacity_type& capacity_ctrl, Iterator first, Iterator last,
        ...
        return init_capacity(capacity_ctrl, first, last, STREAMS_BOOST_ITERATOR_CATEGORY<Iterator>::type());
#else
        return init_capacity(
            capacity_ctrl, first, last, STREAMS_BOOST_DEDUCED_TYPENAME STREAMS_BOOST_ITERATOR_CATEGORY<Iterator>::type());
#endif
    }
...
#if !defined(STREAMS_BOOST_NO_FUNCTION_TEMPLATE_ORDERING) || STREAMS_BOOST_WORKAROUND(STREAMS_BOOST_MSVC, STREAMS_BOOST_TESTED_AT(1310))
...
#endif // #if !defined(STREAMS_BOOST_NO_FUNCTION_TEMPLATE_ORDERING) || STREAMS_BOOST_WORKAROUND(STREAMS_BOOST_MSVC, STREAMS_BOOST_TESTED_AT(1310))

} // namespace streams_boost

#endif // #if !defined(STREAMS_BOOST_CIRCULAR_BUFFER_SPACE_OPTIMIZED_HPP)
