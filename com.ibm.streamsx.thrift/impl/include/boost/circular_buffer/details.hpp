...
#if !defined(STREAMS_BOOST_CIRCULAR_BUFFER_DETAILS_HPP)
#define STREAMS_BOOST_CIRCULAR_BUFFER_DETAILS_HPP
...
public:
    ...
    : m_capacity(buffer_capacity), m_min_capacity(min_buffer_capacity) {
        STREAMS_BOOST_CB_ASSERT(buffer_capacity >= min_buffer_capacity); // check for capacity lower than min_capacity
    }
...
#endif // #if STREAMS_BOOST_CB_ENABLE_DEBUG
{
// Helper types
...
// Random access iterator methods
    ...
    reference operator * () const {
        STREAMS_BOOST_CB_ASSERT(is_valid(m_buff)); // check for uninitialized or invalidated iterator
        STREAMS_BOOST_CB_ASSERT(m_it != 0);        // check for iterator pointing to end()
        return *m_it;
    }
    ...
    difference_type operator - (const iterator<Buff, Traits0>& it) const {
        STREAMS_BOOST_CB_ASSERT(is_valid(m_buff));    // check for uninitialized or invalidated iterator
        STREAMS_BOOST_CB_ASSERT(it.is_valid(m_buff)); // check for uninitialized or invalidated iterator
    ...
    iterator& operator -- () {
        STREAMS_BOOST_CB_ASSERT(is_valid(m_buff));        // check for uninitialized or invalidated iterator
        STREAMS_BOOST_CB_ASSERT(m_it != m_buff->m_first); // check for iterator pointing to begin()
        if (m_it == 0)
            m_it = m_buff->m_last;
    ...
    iterator& operator += (difference_type n) {
        STREAMS_BOOST_CB_ASSERT(is_valid(m_buff)); // check for uninitialized or invalidated iterator
        if (n > 0) {
            STREAMS_BOOST_CB_ASSERT(m_buff->end() - *this >= n); // check for too large n
    ...
    iterator& operator -= (difference_type n) {
        STREAMS_BOOST_CB_ASSERT(is_valid(m_buff)); // check for uninitialized or invalidated iterator
        if (n > 0) {
            STREAMS_BOOST_CB_ASSERT(*this - m_buff->begin() >= n); // check for too large n
...
#if defined(STREAMS_BOOST_NO_TEMPLATE_PARTIAL_SPECIALIZATION) && !defined(STREAMS_BOOST_MSVC_STD_ITERATOR)
...
#endif // #if defined(STREAMS_BOOST_NO_TEMPLATE_PARTIAL_SPECIALIZATION) && !defined(STREAMS_BOOST_MSVC_STD_ITERATOR)
...
#endif // #if !defined(STREAMS_BOOST_CIRCULAR_BUFFER_DETAILS_HPP)
