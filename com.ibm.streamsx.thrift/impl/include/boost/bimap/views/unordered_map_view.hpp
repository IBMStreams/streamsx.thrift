...
#ifndef STREAMS_BOOST_BIMAP_VIEWS_UNOREDERED_MAP_VIEW_HPP
#define STREAMS_BOOST_BIMAP_VIEWS_UNOREDERED_MAP_VIEW_HPP
...
#include <streams_boost/bimap/container_adaptor/unordered_map_adaptor.hpp>
...

{
    ...
    STREAMS_BOOST_BIMAP_MAP_VIEW_BASE_FRIEND(unordered_map_view,Tag,BimapType)
    ...
    unordered_map_view(STREAMS_BOOST_DEDUCED_TYPENAME base_::base_type & c)
        : base_(c) {}
    ...
    {
        STREAMS_BOOST_DEDUCED_TYPENAME base_::const_iterator iter = this->find(k);
        if( iter == this->end() )
        {
    ...
    {
        STREAMS_BOOST_DEDUCED_TYPENAME base_::iterator iter = this->find(k);
        if( iter == this->end() )
        {
            ::streams_boost::throw_exception(
                std::out_of_range("bimap<>: invalid key")
...
template< class Tag, class BimapType >
struct left_map_view_extra_typedefs< ::streams_boost::bimaps::views::unordered_map_view<Tag,BimapType> >
{
    private: typedef ::streams_boost::bimaps::views::unordered_map_view<Tag,BimapType> map_view_;
    public : STREAMS_BOOST_BIMAP_MAP_VIEW_EXTRA_TYPEDEFS_BODY(map_view_,left)
};

template< class Tag, class BimapType >
struct right_map_view_extra_typedefs< ::streams_boost::bimaps::views::unordered_map_view<Tag,BimapType> >
{
    private: typedef ::streams_boost::bimaps::views::unordered_map_view<Tag,BimapType> map_view_;
...
#endif // STREAMS_BOOST_BIMAP_VIEWS_UNOREDERED_MAP_VIEW_HPP
