...
#ifndef STREAMS_BOOST_INTRUSIVE_SLIST_HPP
#define STREAMS_BOOST_INTRUSIVE_SLIST_HPP

#include <streams_boost/intrusive/detail/config_begin.hpp>
#include <streams_boost/static_assert.hpp>
#include <streams_boost/intrusive/detail/assert.hpp>
#include <streams_boost/intrusive/intrusive_fwd.hpp>
#include <streams_boost/intrusive/slist_hook.hpp>
#include <streams_boost/intrusive/circular_slist_algorithms.hpp>
#include <streams_boost/intrusive/linear_slist_algorithms.hpp>
#include <streams_boost/intrusive/detail/pointer_to_other.hpp>
#include <streams_boost/intrusive/detail/clear_on_destructor_base.hpp>
   ...
   :  private detail::clear_on_destructor_base<slist_impl<Config> >
{
   ...
   STREAMS_BOOST_STATIC_ASSERT(!(constant_time_size && ((int)real_value_traits::link_mode == (int)auto_unlink)));
   //Linear singly linked lists are incompatible with auto-unlink hooks!
   STREAMS_BOOST_STATIC_ASSERT(!(linear && ((int)real_value_traits::link_mode == (int)auto_unlink)));
   //A list with cached last node is incompatible with auto-unlink hooks!
   STREAMS_BOOST_STATIC_ASSERT(!(cache_last && ((int)real_value_traits::link_mode == (int)auto_unlink)));
   ...
   void push_front(reference value) 
   {
      ...
      if(safemode_or_autounlink)
         STREAMS_BOOST_INTRUSIVE_SAFE_HOOK_DEFAULT_ASSERT(node_algorithms::inited(to_insert));
      if(cache_last){
         if(this->empty()){
   ...
   {
      STREAMS_BOOST_STATIC_ASSERT((cache_last != 0));
   ...
   iterator insert_after(const_iterator prev_p, reference value)
   {
      ...
      if(safemode_or_autounlink)
         STREAMS_BOOST_INTRUSIVE_SAFE_HOOK_DEFAULT_ASSERT(node_algorithms::inited(n));
   ...
   {
      STREAMS_BOOST_INTRUSIVE_INVARIANT_ASSERT(std::distance(++const_iterator(before_first), last) == difference_type(n));
      if(safemode_or_autounlink){
   ...
   {
      STREAMS_BOOST_STATIC_ASSERT(((!cache_last)&&(!constant_time_size)&&(!stateful_value_traits)));
      const_iterator it(prev);
      ++it;
      node_ptr to_erase(it.pointed_node());
      ++it;
   ...
   {
      if(n){
         STREAMS_BOOST_INTRUSIVE_INVARIANT_ASSERT(std::distance(before_first, before_last) == n);
         this->priv_splice_after
            (prev_pos.pointed_node(), x, before_first.pointed_node(), before_last.pointed_node());
         if(constant_time_size){
            this->priv_size_traits().set_size(this->priv_size_traits().get_size() + n);
   ...
   void sort(Predicate p)
   {
      if (node_traits::get_next(node_traits::get_next(this->get_root_node()))
         ...
         while(!this->empty()){
            ...
            STREAMS_BOOST_INTRUSIVE_INVARIANT_ASSERT(counter[i].empty());
   ...
   {
      STREAMS_BOOST_STATIC_ASSERT((!stateful_value_traits));
      //STREAMS_BOOST_INTRUSIVE_INVARIANT_ASSERT (!node_algorithms::inited(value_traits::to_node_ptr(value)));
   ...
   {
      STREAMS_BOOST_STATIC_ASSERT((!stateful_value_traits));
      //STREAMS_BOOST_INTRUSIVE_INVARIANT_ASSERT (!node_algorithms::inited(value_traits::to_node_ptr(const_cast<reference> (value))));
   ...
   {
      if(n){
         STREAMS_BOOST_INTRUSIVE_INVARIANT_ASSERT(std::distance(iterator(first, this), iterator(before_last, this))+1 == n);
   ...
   static slist_impl &priv_container_from_end_iterator(const const_iterator &end_iterator)
   {
      ...
      STREAMS_BOOST_STATIC_ASSERT(!linear);
...
#ifndef STREAMS_BOOST_INTRUSIVE_DOXYGEN_INVOKED
      ...
      >::type
{
   ...
   STREAMS_BOOST_STATIC_ASSERT((detail::is_same<typename real_value_traits::value_type, T>::value));
   public:
...
#endif //STREAMS_BOOST_INTRUSIVE_SLIST_HPP
