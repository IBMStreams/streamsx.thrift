...
#ifndef STREAMS_BOOST_INTRUSIVE_LIST_HPP
#define STREAMS_BOOST_INTRUSIVE_LIST_HPP
...
#include <streams_boost/intrusive/list_hook.hpp>
#include <streams_boost/intrusive/circular_list_algorithms.hpp>
   ...
   :  private detail::clear_on_destructor_base< list_impl<Config> >
{
   ...
   {
      STREAMS_BOOST_INTRUSIVE_INVARIANT_ASSERT(std::distance(b, e) == difference_type(n));
      if(safemode_or_autounlink || constant_time_size){
         return this->erase_and_dispose(b, e, detail::null_disposer());
      }
      else{
   ...
   void splice(const_iterator p, list_impl&x, const_iterator start, const_iterator end, difference_type n)
   {
      if(n){
         if(constant_time_size){
            size_traits &thist = this->priv_size_traits();
            size_traits &xt = x.priv_size_traits();
            STREAMS_BOOST_INTRUSIVE_INVARIANT_ASSERT(n == std::distance(start, end));
   ...
   static iterator s_iterator_to(reference value)
   {
      ...
      STREAMS_BOOST_INTRUSIVE_INVARIANT_ASSERT(!node_algorithms::inited(real_value_traits::to_node_ptr(value)));
   ...
   {
      STREAMS_BOOST_STATIC_ASSERT((!stateful_value_traits));
      STREAMS_BOOST_INTRUSIVE_INVARIANT_ASSERT(!node_algorithms::inited(real_value_traits::to_node_ptr(const_cast<reference> (value))));
...
#endif //STREAMS_BOOST_INTRUSIVE_LIST_HPP
