...
#ifndef STREAMS_BOOST_INTRUSIVE_TREE_ALGORITHMS_HPP
#define STREAMS_BOOST_INTRUSIVE_TREE_ALGORITHMS_HPP
...
class tree_algorithms
{
   ...
   static void swap_nodes(node_ptr node1, node_ptr header1, node_ptr node2, node_ptr header2)
   {
      ...
      //STREAMS_BOOST_INTRUSIVE_INVARIANT_ASSERT((header1 != node1 && header2 != node2));
      if(header1 != header2){
      ...
      (node_ptr header, node_ptr new_node, const insert_commit_data &commit_data)
   {
      ...
      STREAMS_BOOST_INTRUSIVE_INVARIANT_ASSERT(commit_data.node != 0);
   ...
   static node_ptr subtree_to_vine(node_ptr old_root, std::size_t *plen = 0)
   {
      ...
      STREAMS_BOOST_INTRUSIVE_INVARIANT_ASSERT(super_root);
   ...
   {
      STREAMS_BOOST_INTRUSIVE_INVARIANT_ASSERT((!inited(node)));
      node_ptr x = NodeTraits::get_parent(node);
      if(x){
         while(!is_header(x)){
            x = NodeTraits::get_parent(x);
...
#endif //STREAMS_BOOST_INTRUSIVE_TREE_ALGORITHMS_HPP
