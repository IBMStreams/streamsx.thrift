...
#ifndef STREAMS_BOOST_MATH_COMPLEX_ACOS_INCLUDED
#define STREAMS_BOOST_MATH_COMPLEX_ACOS_INCLUDED
   ...
   else
   {
      ...
      if((x < safe_max) && (x > safe_min) && (y < safe_max) && (y > safe_min))
      {
         ...
         if(a <= a_crossover)
         {
            ...
            imag = streams_boost::math::log1p(am1 + std::sqrt(am1 * (a + one)));
         }
         else
         {
      ...
      else
      {
         ...
         if(y <= (std::numeric_limits<T>::epsilon() * std::fabs(xm1)))
         {
            ...
            else
            {
               ...
               if(((std::numeric_limits<T>::max)() / xp1) > xm1)
               {
                  ...
                  imag = streams_boost::math::log1p(xm1 + std::sqrt(xp1*xm1));
               }
               else
               {
         ...
         else if(x > one)
         {
            real = std::atan(y/x);
            T xoy = x/y;
            imag = log_two + std::log(y) + half * streams_boost::math::log1p(xoy*xoy);
         }
         else
         {
            real = half_pi;
            T a = std::sqrt(one + y*y);
            imag = half * streams_boost::math::log1p(static_cast<T>(2)*y*(y+a));
         }
      }
   }
...
#endif // STREAMS_BOOST_MATH_COMPLEX_ACOS_INCLUDED
