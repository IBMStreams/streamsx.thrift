...
#define STREAMS_BOOST_MATH_COMPLEX_ATANH_INCLUDED
...
std::complex<T> atanh(const std::complex<T>& z)
{
   ...
   else if((x > safe_lower) && (x < safe_upper) && (y > safe_lower) && (y < safe_upper))
   {
      ...
      {
         real = streams_boost::math::log1p(alpha) - streams_boost::math::log1p(-alpha);
      }
      else
      {
         T xm1 = x - one;
         real = streams_boost::math::log1p(x2 + xx + yy) - std::log(xm1*xm1 + yy);
      }
      real /= four;
      if(z.real() < 0)
         real = -real;
   ...
   else
   {
      ...
      else
      {
         ...
         STREAMS_BOOST_ASSERT(x >= safe_lower);
         STREAMS_BOOST_ASSERT(x <= safe_upper);
         //STREAMS_BOOST_ASSERT(y <= safe_lower);
         T xm1 = x - one;
...
#endif // STREAMS_BOOST_MATH_COMPLEX_ATANH_INCLUDED
