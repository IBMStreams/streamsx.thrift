...
#ifndef STREAMS_BOOST_MATH_ELLINT_RJ_HPP
#define STREAMS_BOOST_MATH_ELLINT_RJ_HPP
...
#include <streams_boost/math/special_functions/ellint_rc.hpp>
...
T ellint_rj_imp(T x, T y, T z, T p, const Policy& pol)
{
    ...
    static const char* function = "streams_boost::math::ellint_rj<%1%>(%1%,%1%,%1%)";
    ...
    if (p < 0)
    {
       ...
       STREAMS_BOOST_ASSERT(pmy >= 0);

       T p = pmy + y;
       value = streams_boost::math::ellint_rj(x, y, z, p, pol);
       value *= pmy;
       value -= 3 * streams_boost::math::ellint_rf(x, y, z, pol);
       value += 3 * sqrt((x * y * z) / (x * z + p * q)) * streams_boost::math::ellint_rc(x * z + p * q, p * q, pol);
       value /= (y + q);
       return value;
    }
    ...
    do
    {
        ...
        sigma += factor * streams_boost::math::ellint_rc(alpha, beta, pol);
        factor /= 4;
        x = (x + lambda) / 4;
        y = (y + lambda) / 4;
        z = (z + lambda) / 4;
   ...
   ellint_rj(T1 x, T2 y, T3 z, T4 p, const Policy& pol)
{
   ...
   return policies::checked_narrowing_cast<result_type, Policy>(
      detail::ellint_rj_imp(
         ...
         pol), "streams_boost::math::ellint_rj<%1%>(%1%,%1%,%1%,%1%)");
}
...
#endif // STREAMS_BOOST_MATH_ELLINT_RJ_HPP
