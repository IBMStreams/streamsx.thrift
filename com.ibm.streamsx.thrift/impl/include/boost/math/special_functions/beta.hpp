...
#ifndef STREAMS_BOOST_MATH_SPECIAL_BETA_HPP
#define STREAMS_BOOST_MATH_SPECIAL_BETA_HPP
...
#include <streams_boost/math/special_functions/gamma.hpp>
#include <streams_boost/math/special_functions/factorials.hpp>
...
{
   STREAMS_BOOST_MATH_STD_USING  // for ADL of std names

   if(a <= 0)
      policies::raise_domain_error<T>("streams_boost::math::beta<%1%>(%1%,%1%)", "The arguments to the beta function must be greater than zero (got a=%1%).", a, pol);
   if(b <= 0)
      policies::raise_domain_error<T>("streams_boost::math::beta<%1%>(%1%,%1%)", "The arguments to the beta function must be greater than zero (got b=%1%).", b, pol);
   ...
   if((c == a) && (b < tools::epsilon<T>()))
      return streams_boost::math::tgamma(b, pol);
   else if((c == b) && (a < tools::epsilon<T>()))
      return streams_boost::math::tgamma(a, pol);
   if(b == 1)
      return 1/a;
   else if(a == 1)
      return 1/b;
   ...
   */
   ...
   if((fabs(b * ambh) < (cgh * 100)) && (a > 100))
   {
      ...
      result *= exp(ambh * streams_boost::math::log1p(-b / cgh, pol));
   }
   else
   {
   ...
   result *= sqrt(streams_boost::math::constants::e<T>() / bgh);
...
T beta_imp(T a, T b, const lanczos::undefined_lanczos& /* l */, const Policy& pol)
{
   ...
   sa += detail::upper_gamma_fraction(a, la, ::streams_boost::math::policies::get_epsilon<T, Policy>());
   T sb = detail::lower_gamma_series(b, lb, pol) / b;
   sb += detail::upper_gamma_fraction(b, lb, ::streams_boost::math::policies::get_epsilon<T, Policy>());
   T sc = detail::lower_gamma_series(c, lc, pol) / c;
   sc += detail::upper_gamma_fraction(c, lc, ::streams_boost::math::policies::get_epsilon<T, Policy>());
                        ...
                        const Policy& pol)
{
   ...
   if(((std::min)(fabs(l1), fabs(l2)) < 0.2))
   {
      ...
      if((l1 * l2 > 0) || ((std::min)(a, b) < 1))
      {
         ...
         if(fabs(l1) < 0.1)
         {
            result *= exp(a * streams_boost::math::log1p(l1, pol));
         ...
         if(fabs(l2) < 0.1)
         {
            result *= exp(b * streams_boost::math::log1p(l2, pol));
            STREAMS_BOOST_MATH_INSTRUMENT_VARIABLE(result);
         }
         else
         {
      ...
      else if((std::max)(fabs(l1), fabs(l2)) < 0.5)
      {
         ...
         {
            T l3 = streams_boost::math::expm1(ratio * streams_boost::math::log1p(l2, pol), pol);
            l3 = l1 + l3 + l3 * l1;
            l3 = a * streams_boost::math::log1p(l3, pol);
            result *= exp(l3);
            STREAMS_BOOST_MATH_INSTRUMENT_VARIABLE(result);
         }
         else
         {
            T l3 = streams_boost::math::expm1(streams_boost::math::log1p(l1, pol) / ratio, pol);
            l3 = l2 + l3 + l3 * l2;
            l3 = b * streams_boost::math::log1p(l3, pol);
            result *= exp(l3);
            STREAMS_BOOST_MATH_INSTRUMENT_VARIABLE(result);
         }
      }
      else if(fabs(l1) < fabs(l2))
      {
         ...
         T l = a * streams_boost::math::log1p(l1, pol)
            + b * log((y * cgh) / bgh);
         result *= exp(l);
         STREAMS_BOOST_MATH_INSTRUMENT_VARIABLE(result);
      }
      else
      {
         ...
         T l = b * streams_boost::math::log1p(l2, pol)
            + a * log((x * cgh) / agh);
         result *= exp(l);
         STREAMS_BOOST_MATH_INSTRUMENT_VARIABLE(result);
      }
   ...
   result *= sqrt(bgh / streams_boost::math::constants::e<T>());
   result *= sqrt(agh / cgh);
   result *= prefix;
...
T ibeta_power_terms(T a,
                        ...
                        T x,
                        T y,
                        const streams_boost::math::lanczos::undefined_lanczos&,
                        bool normalised,
                        const Policy& pol)
{
   STREAMS_BOOST_MATH_STD_USING
   ...
   else
   {
      ...
      if((fabs(b1 - 1) * a < 10) && (a > 1))
         p1 = exp(a * streams_boost::math::log1p((x * b - y * la) / la, pol));
      else
         p1 = pow(b1, a);
      if((fabs(b2 - 1) * b < 10) && (b > 1))
         p2 = exp(b * streams_boost::math::log1p((y * a - x * lb) / lb, pol));
      else
         p2 = pow(b2, b);
      T p3 = exp(e1);
      result = p1 * p2 / p3;
...
T ibeta_series(T a, T b, T x, T s0, const L&, bool normalised, T* p_derivative, T y, const Policy& pol)
{
   ...
   STREAMS_BOOST_ASSERT((p_derivative == 0) || normalised);
   ...
   if(normalised)
   {
      ...
      result = L::lanczos_sum_expG_scaled(c) / (L::lanczos_sum_expG_scaled(a) * L::lanczos_sum_expG_scaled(b));
      if(a * b < bgh * 10)
         result *= exp((b - 0.5f) * streams_boost::math::log1p(a / bgh, pol));
      else
         result *= pow(cgh / bgh, b - 0.5f);
      result *= pow(x * cgh / agh, a);
      result *= sqrt(agh / streams_boost::math::constants::e<T>());

      if(p_derivative)
      {
         *p_derivative = result * pow(y, b);
         STREAMS_BOOST_ASSERT(*p_derivative >= 0);
      }
   }
   else
   {
   ...
   result = streams_boost::math::tools::sum_series(s, streams_boost::math::policies::get_epsilon<T, Policy>(), max_iter, s0);
   policies::check_series_iterations("streams_boost::math::ibeta<%1%>(%1%, %1%, %1%) in ibeta_series (with lanczos)", max_iter, pol);
   return result;
}
//
...
//
template <class T, class Policy>
T ibeta_series(T a, T b, T x, T s0, const streams_boost::math::lanczos::undefined_lanczos&, bool normalised, T* p_derivative, T y, const Policy& pol)
{
   STREAMS_BOOST_MATH_STD_USING
   ...
   if(normalised)
   {
      ...
      else
      {
         result = pow(b1, a);
         if(a * b < lb * 10)
            result *= exp(b * streams_boost::math::log1p(a / lb, pol));
         else
            result *= pow(b2, b);
         result /= exp(e1);
      }
   ...
   streams_boost::uintmax_t max_iter = policies::get_max_series_iterations<Policy>();
   result = streams_boost::math::tools::sum_series(s, streams_boost::math::policies::get_epsilon<T, Policy>(), max_iter, s0);
   policies::check_series_iterations("streams_boost::math::ibeta<%1%>(%1%, %1%, %1%) in ibeta_series (without lanczos)", max_iter, pol);
   return result;
}
...
inline T ibeta_fraction2(T a, T b, T x, T y, const Policy& pol, bool normalised, T* p_derivative)
{
   ...
   T fract = streams_boost::math::tools::continued_fraction_b(f, streams_boost::math::policies::get_epsilon<T, Policy>());
   return result / fract;
}
//
...
struct Pn_size
{
   ...
   STREAMS_BOOST_STATIC_CONSTANT(unsigned, value = 50);
   STREAMS_BOOST_STATIC_ASSERT(::streams_boost::math::max_factorial<T>::value >= 100);
};
template <>
struct Pn_size<float>
{
   STREAMS_BOOST_STATIC_CONSTANT(unsigned, value = 15); // ~8-15 digit accuracy
   STREAMS_BOOST_STATIC_ASSERT(::streams_boost::math::max_factorial<float>::value >= 30);
};
template <>
struct Pn_size<double>
{
   STREAMS_BOOST_STATIC_CONSTANT(unsigned, value = 30); // 16-20 digit accuracy
   STREAMS_BOOST_STATIC_ASSERT(::streams_boost::math::max_factorial<double>::value >= 60);
};
template <>
struct Pn_size<long double>
{
   STREAMS_BOOST_STATIC_CONSTANT(unsigned, value = 50); // ~35-50 digit accuracy
   STREAMS_BOOST_STATIC_ASSERT(::streams_boost::math::max_factorial<long double>::value >= 100);
};
...
T beta_small_b_large_a_series(T a, T b, T x, T y, T s0, T mult, const Policy& pol, bool normalised)
{
   ...
   T lx, u;
   if(y < 0.35)
      lx = streams_boost::math::log1p(-y, pol);
   else
      lx = log(x);
   u = -t * lx;
   // and from from 9.2:
   ...
   if(normalised)
   {
      prefix = h / streams_boost::math::tgamma_delta_ratio(a, b, pol);
      prefix /= pow(t, b);
   }
   else
   {
   ...
   //
   T p[ ::streams_boost::math::detail::Pn_size<T>::value ] = { 1 };  // see 9.3.
   //
   // Now an initial value for J, see 9.6:
   //
   T j = streams_boost::math::gamma_q(b, u, pol) / h;
   //
   ...
   for(unsigned n = 1; n < sizeof(p)/sizeof(p[0]); ++n)
   {
      ...
      */
      ...
      {
         mbn = m * b - n;
         p[n] += mbn * p[n-m] / streams_boost::math::unchecked_factorial<T>(tmp1);
         tmp1 += 2;
      }
      p[n] /= n;
      p[n] += bm1 / streams_boost::math::unchecked_factorial<T>(tnp1);
      //
...
{
   static const char* function = "streams_boost::math::ibeta<%1%>(%1%, %1%, %1%)";
   typedef typename lanczos::lanczos<T, Policy>::type lanczos_type;
   STREAMS_BOOST_MATH_STD_USING // for ADL of std math functions.

   STREAMS_BOOST_MATH_INSTRUMENT_VARIABLE(a);
   STREAMS_BOOST_MATH_INSTRUMENT_VARIABLE(b);
   STREAMS_BOOST_MATH_INSTRUMENT_VARIABLE(x);
   STREAMS_BOOST_MATH_INSTRUMENT_VARIABLE(inv);
   STREAMS_BOOST_MATH_INSTRUMENT_VARIABLE(normalised);
   ...
   if(x == 0)
   {
      ...
      return (invert ? (normalised ? T(1) : streams_boost::math::beta(a, b, pol)) : T(0));
   }
   if(x == 1)
   {
      if(p_derivative)
      ...
      return (invert == 0 ? (normalised ? 1 : streams_boost::math::beta(a, b, pol)) : 0);
   }
   ...
   if((std::min)(a, b) <= 1)
   {
      if(x > 0.5)
      {
         ...
         std::swap(x, y);
         invert = !invert;
         STREAMS_BOOST_MATH_INSTRUMENT_VARIABLE(invert);
      }
      if((std::max)(a, b) <= 1)
      {
         // Both a,b < 1:
         if((a >= (std::min)(T(0.2), b)) || (pow(x, a) <= 0.9))
         {
            if(!invert)
            {
               ...
               STREAMS_BOOST_MATH_INSTRUMENT_VARIABLE(fract);
            }
            else
            {
               fract = -(normalised ? 1 : streams_boost::math::beta(a, b, pol));
               invert = false;
   ...
   else
   {
      ...
      if(b < 40)
      {
         if((floor(a) == a) && (floor(b) == b))
         {
            ...
            if(!normalised)
               fract *= streams_boost::math::beta(a, b, pol);
            STREAMS_BOOST_MATH_INSTRUMENT_VARIABLE(fract);
         }
         else if(b * x <= 0.7)
         {
   ...
   return invert ? (normalised ? 1 : streams_boost::math::beta(a, b, pol)) - fract : fract;
...
T ibeta_derivative_imp(T a, T b, T x, const Policy& pol)
{
   ...
   {
      return (a > 1) ? 0 : 
         (a == 1) ? 1 / streams_boost::math::beta(a, b, pol) : policies::raise_overflow_error<T>(function, 0, pol);
   }
   else if(x == 1)
   {
      return (b > 1) ? 0 :
         (b == 1) ? 1 / streams_boost::math::beta(a, b, pol) : policies::raise_overflow_error<T>(function, 0, pol);
   }
   //
   ...
   beta(RT1 a, RT2 b, const Policy&, const mpl::true_*)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::beta_imp(static_cast<value_type>(a), static_cast<value_type>(b), evaluation_type(), forwarding_policy()), "streams_boost::math::beta<%1%>(%1%,%1%)");
}
template <class RT1, class RT2, class RT3>
inline typename tools::promote_args<RT1, RT2, RT3>::type 
   beta(RT1 a, RT2 b, RT3 x, const mpl::false_*)
{
   return streams_boost::math::beta(a, b, x, policies::policy<>());
}
} // namespace detail
   ...
   beta(RT1 a, RT2 b, A arg)
{
   ...
   return streams_boost::math::detail::beta(a, b, arg, static_cast<tag*>(0));
}
...
inline typename tools::promote_args<RT1, RT2>::type 
   beta(RT1 a, RT2 b)
{
   return streams_boost::math::beta(a, b, policies::policy<>());
}
   ...
   beta(RT1 a, RT2 b, RT3 x, const Policy&)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::ibeta_imp(static_cast<value_type>(a), static_cast<value_type>(b), static_cast<value_type>(x), forwarding_policy(), false, false), "streams_boost::math::beta<%1%>(%1%,%1%,%1%)");
}
   ...
   betac(RT1 a, RT2 b, RT3 x, const Policy&)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::ibeta_imp(static_cast<value_type>(a), static_cast<value_type>(b), static_cast<value_type>(x), forwarding_policy(), true, false), "streams_boost::math::betac<%1%>(%1%,%1%,%1%)");
}
template <class RT1, class RT2, class RT3>
inline typename tools::promote_args<RT1, RT2, RT3>::type 
   betac(RT1 a, RT2 b, RT3 x)
{
   return streams_boost::math::betac(a, b, x, policies::policy<>());
}
   ...
   ibeta(RT1 a, RT2 b, RT3 x, const Policy&)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::ibeta_imp(static_cast<value_type>(a), static_cast<value_type>(b), static_cast<value_type>(x), forwarding_policy(), false, true), "streams_boost::math::ibeta<%1%>(%1%,%1%,%1%)");
}
template <class RT1, class RT2, class RT3>
inline typename tools::promote_args<RT1, RT2, RT3>::type 
   ibeta(RT1 a, RT2 b, RT3 x)
{
   return streams_boost::math::ibeta(a, b, x, policies::policy<>());
}
   ...
   ibetac(RT1 a, RT2 b, RT3 x, const Policy&)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::ibeta_imp(static_cast<value_type>(a), static_cast<value_type>(b), static_cast<value_type>(x), forwarding_policy(), true, true), "streams_boost::math::ibetac<%1%>(%1%,%1%,%1%)");
}
template <class RT1, class RT2, class RT3>
inline typename tools::promote_args<RT1, RT2, RT3>::type 
   ibetac(RT1 a, RT2 b, RT3 x)
{
   return streams_boost::math::ibetac(a, b, x, policies::policy<>());
}
   ...
   ibeta_derivative(RT1 a, RT2 b, RT3 x, const Policy&)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::ibeta_derivative_imp(static_cast<value_type>(a), static_cast<value_type>(b), static_cast<value_type>(x), forwarding_policy()), "streams_boost::math::ibeta_derivative<%1%>(%1%,%1%,%1%)");
}
template <class RT1, class RT2, class RT3>
inline typename tools::promote_args<RT1, RT2, RT3>::type 
   ibeta_derivative(RT1 a, RT2 b, RT3 x)
{
   return streams_boost::math::ibeta_derivative(a, b, x, policies::policy<>());
}

} // namespace math
} // namespace streams_boost

#include <streams_boost/math/special_functions/detail/ibeta_inverse.hpp>
#include <streams_boost/math/special_functions/detail/ibeta_inv_ab.hpp>

#endif // STREAMS_BOOST_MATH_SPECIAL_BETA_HPP
