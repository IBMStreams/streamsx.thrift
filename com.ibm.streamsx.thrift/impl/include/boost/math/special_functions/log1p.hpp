...
#ifndef STREAMS_BOOST_MATH_LOG1P_INCLUDED
#define STREAMS_BOOST_MATH_LOG1P_INCLUDED
...
{ // The function returns the natural logarithm of 1 + x.
   ...
   static const char* function = "streams_boost::math::log1p<%1%>(%1%)";
...
#if !STREAMS_BOOST_WORKAROUND(__BORLANDC__, STREAMS_BOOST_TESTED_AT(0x582)) && !STREAMS_BOOST_WORKAROUND(__EDG_VERSION__, <= 245)
...
{ 
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(
      detail::log1p_imp(static_cast<value_type>(x), forwarding_policy(), tag_type()), "streams_boost::math::log1p<%1%>(%1%)");
}
...
#if defined(STREAMS_BOOST_HAS_LOG1P) && !(defined(__osf__) && defined(__DECCXX_VER))
#  ifdef STREAMS_BOOST_MATH_USE_C99
...
#elif defined(_MSC_VER) && (STREAMS_BOOST_MSVC >= 1400)
//
...
inline float log1p(float x, const Policy& pol)
{
   return static_cast<float>(streams_boost::math::log1p(static_cast<double>(x), pol));
}
template <class Policy>
inline long double log1p(long double x, const Policy& pol)
{
...
{
   return streams_boost::math::log1p(x, policies::policy<>());
}
//
// Compute log(1+x)-x:
//
   ...
   log1pmx(T x, const Policy& pol)
{
   typedef typename tools::promote_args<T>::type result_type;
   STREAMS_BOOST_MATH_STD_USING
   static const char* function = "streams_boost::math::log1pmx<%1%>(%1%)";
   ...
   if(a < tools::epsilon<result_type>())
      return -x * x / 2;
   streams_boost::math::detail::log1p_series<T> s(x);
   s();
   streams_boost::uintmax_t max_iter = policies::get_max_series_iterations<Policy>();
#if STREAMS_BOOST_WORKAROUND(__BORLANDC__, STREAMS_BOOST_TESTED_AT(0x582))
   T zero = 0;
   T result = streams_boost::math::tools::sum_series(s, policies::get_epsilon<T, Policy>(), max_iter, zero);
#else
   T result = streams_boost::math::tools::sum_series(s, policies::get_epsilon<T, Policy>(), max_iter);
#endif
   policies::check_series_iterations(function, max_iter, pol);
   return result;
}
...
#endif // STREAMS_BOOST_MATH_LOG1P_INCLUDED
