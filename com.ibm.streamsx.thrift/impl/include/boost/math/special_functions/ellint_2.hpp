...
#ifndef STREAMS_BOOST_MATH_ELLINT_2_HPP
#define STREAMS_BOOST_MATH_ELLINT_2_HPP
...
#include <streams_boost/math/special_functions/ellint_rd.hpp>
...
T ellint_e_imp(T phi, T k, const Policy& pol)
{
    ...
    if(phi >= tools::max_value<T>())
    {
       ...
       result = policies::raise_overflow_error<T>("streams_boost::math::ellint_e<%1%>(%1%,%1%)", 0, pol);
    }
...
T ellint_e_imp(T k, const Policy& pol)
{
    ...
    if (abs(k) > 1)
    {
       return policies::raise_domain_error<T>("streams_boost::math::ellint_e<%1%>(%1%)",
...
inline typename tools::promote_args<T>::type ellint_2(T k, const Policy& pol, const mpl::true_&)
{
   ...
   return policies::checked_narrowing_cast<result_type, Policy>(detail::ellint_e_imp(static_cast<value_type>(k), pol), "streams_boost::math::ellint_2<%1%>(%1%)");
}
...
{
   return streams_boost::math::ellint_2(k, phi, policies::policy<>());
}
...
inline typename tools::promote_args<T1, T2>::type ellint_2(T1 k, T2 phi, const Policy& pol)
{
   ...
   typedef typename policies::evaluation<result_type, Policy>::type value_type;
   return policies::checked_narrowing_cast<result_type, Policy>(detail::ellint_e_imp(static_cast<value_type>(phi), static_cast<value_type>(k), pol), "streams_boost::math::ellint_2<%1%>(%1%,%1%)");
}

}} // namespaces

#endif // STREAMS_BOOST_MATH_ELLINT_2_HPP
