...
#ifndef STREAMS_BOOST_MATH_SPECIAL_NEXT_HPP
#define STREAMS_BOOST_MATH_SPECIAL_NEXT_HPP
...
{
#if defined(STREAMS_BOOST_MSVC) && (STREAMS_BOOST_MSVC <= 1310)
...
T float_next(const T& val, const Policy& pol)
{
   ...
   if(!(streams_boost::math::isfinite)(val))
...
{
   return val < direction ? streams_boost::math::float_next(val, pol) : val == direction ? val : streams_boost::math::float_prior(val, pol);
}
...
T float_distance(const T& a, const T& b, const Policy& pol)
{
   ...
   if(!(streams_boost::math::isfinite)(b))
      return policies::raise_domain_error<T>(
         function,
   ...
   if(a == b)
      return 0;
   if(a == 0)
      return 1 + fabs(float_distance(static_cast<T>(streams_boost::math::sign(b) * detail::get_smallest_value<T>()), b, pol));
   if(b == 0)
      return 1 + fabs(float_distance(static_cast<T>(streams_boost::math::sign(a) * detail::get_smallest_value<T>()), a, pol));
   if(streams_boost::math::sign(a) != streams_boost::math::sign(b))
      return 2 + fabs(float_distance(static_cast<T>(streams_boost::math::sign(b) * detail::get_smallest_value<T>()), b, pol))
         + fabs(float_distance(static_cast<T>(streams_boost::math::sign(a) * detail::get_smallest_value<T>()), a, pol));
   //
   ...
   STREAMS_BOOST_ASSERT(a >= 0);
   STREAMS_BOOST_ASSERT(b >= a);
   ...
   //
   frexp(((streams_boost::math::fpclassify)(a) == FP_SUBNORMAL) ? tools::min_value<T>() : a, &expon);
   ...
   //
   STREAMS_BOOST_ASSERT(result == floor(result));
   return result;
}
...
{
   return streams_boost::math::float_distance(a, b, policies::policy<>());
}
...
{
   return streams_boost::math::float_advance(val, distance, policies::policy<>());
}

}} // namespaces

#endif // STREAMS_BOOST_MATH_SPECIAL_NEXT_HPP
