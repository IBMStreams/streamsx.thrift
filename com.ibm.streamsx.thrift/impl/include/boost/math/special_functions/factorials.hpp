...
#ifndef STREAMS_BOOST_MATH_SP_FACTORIALS_HPP
#define STREAMS_BOOST_MATH_SP_FACTORIALS_HPP
...
{
   STREAMS_BOOST_STATIC_ASSERT(!streams_boost::is_integral<T>::value);
   STREAMS_BOOST_MATH_STD_USING // Aid ADL for floor.

   if(i <= max_factorial<T>::value)
      return unchecked_factorial<T>(i);
   T result = streams_boost::math::tgamma(static_cast<T>(i+1), pol);
...
inline float factorial<float>(unsigned i)
{
   ...
   return tools::overflow_error<float>(STREAMS_BOOST_CURRENT_FUNCTION);
}
...
inline double factorial<double>(unsigned i)
{
   ...
   return tools::overflow_error<double>(STREAMS_BOOST_CURRENT_FUNCTION);
}
*/
template <class T, class Policy>
...
{
   STREAMS_BOOST_STATIC_ASSERT(!streams_boost::is_integral<T>::value);
   STREAMS_BOOST_MATH_STD_USING  // ADL lookup of std names
   if(i & 1)
   {
      // odd i:
      if(i < max_factorial<T>::value)
      ...
      //
      T result = streams_boost::math::tgamma(static_cast<T>(i) / 2 + 1, pol) / sqrt(constants::pi<T>());
   ...
   //
   return policies::raise_overflow_error<T>("streams_boost::math::double_factorial<%1%>(unsigned)", 0, pol);
}
...
T rising_factorial_imp(T x, int n, const Policy& pol)
{
   ...
   // use case:
   //
   return 1 / streams_boost::math::tgamma_delta_ratio(x, static_cast<T>(n), pol);
}
...
inline T falling_factorial_imp(T x, unsigned n, const Policy& pol)
{
   ...
   if(x < n-1)
   {
      ...
      if(n2 == xp1)
         return 0;
      T result = streams_boost::math::tgamma_delta_ratio(xp1, -static_cast<T>(n2), pol);
      x -= n2;
      result *= x;
      ++n2;
      if(n2 < n)
   ...
   // for that use case:
   //
   return streams_boost::math::tgamma_delta_ratio(x + 1, -static_cast<T>(n), pol);
}
...
#endif // STREAMS_BOOST_MATH_SP_FACTORIALS_HPP
