...
#ifndef STREAMS_BOOST_ACOSH_HPP
#define STREAMS_BOOST_ACOSH_HPP
...
namespace streams_boost
{
    namespace math
    {
       namespace detail
       {
        ...
        inline T    acosh_imp(const T x, const Policy& pol)
        {
            ...
            else if    ((x - 1) >= tools::root_epsilon<T>())
            {
                ...
                else if(x < 1.5f)
                {
                   ...
                   T y = x - 1;
                   return streams_boost::math::log1p(y + sqrt(y * y + 2 * y), pol);
                }
                else
                {
        ...
        {
           return streams_boost::math::acosh(x, policies::policy<>());
        }

    }
}

#endif /* STREAMS_BOOST_ACOSH_HPP */
