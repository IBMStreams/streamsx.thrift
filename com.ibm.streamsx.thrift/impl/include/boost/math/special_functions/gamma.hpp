...
#ifndef STREAMS_BOOST_MATH_SF_GAMMA_HPP
#define STREAMS_BOOST_MATH_SF_GAMMA_HPP
...
# pragma warning(pop)
#endif
#include <streams_boost/math/tools/series.hpp>
#include <streams_boost/math/tools/fraction.hpp>
...
#include <streams_boost/math/special_functions/powm1.hpp>
#include <streams_boost/math/special_functions/sqrt1pm1.hpp>
#include <streams_boost/math/special_functions/lanczos.hpp>
#include <streams_boost/math/special_functions/fpclassify.hpp>
#include <streams_boost/math/special_functions/detail/igamma_large.hpp>
#include <streams_boost/math/special_functions/detail/unchecked_factorial.hpp>
#include <streams_boost/math/special_functions/detail/lgamma_small.hpp>
...
template <class T>
inline bool is_odd(T v, const streams_boost::true_type&)
{
   int i = static_cast<int>(v);
   return i&1;
}
template <class T>
inline bool is_odd(T v, const streams_boost::false_type&)
{
...
{
   return is_odd(v, ::streams_boost::is_convertible<T, int>());
}
...
T sinpx(T z)
{
   ...
   STREAMS_BOOST_ASSERT(fl >= 0);
   if(dist > 0.5)
      dist = 1 - dist;
   T result = sin(dist*streams_boost::math::constants::pi<T>());
...
T gamma_imp(T z, const Policy& pol, const L& l)
{
...
#endif
   static const char* function = "streams_boost::math::tgamma<%1%>(%1%)";
   ...
   if(z <= 0)
   {
      ...
      if(z <= -20)
      {
         ...
         if((fabs(result) < 1) && (tools::max_value<T>() * fabs(result) < streams_boost::math::constants::pi<T>()))
            return policies::raise_overflow_error<T>(function, "Result of tgamma is too large to represent.", pol);
         result = -streams_boost::math::constants::pi<T>() / result;
         if(result == 0)
            return policies::raise_underflow_error<T>(function, "Result of tgamma is too small to represent.", pol);
         if((streams_boost::math::fpclassify)(result) == (int)FP_SUBNORMAL)
   ...
   else
   {
      ...
      if(z * log(z) > tools::log_max_value<T>())
      {
         ...
         T zgh = (z + static_cast<T>(L::g()) - streams_boost::math::constants::half<T>());
      ...
      {
         T zgh = (z + static_cast<T>(L::g()) - streams_boost::math::constants::half<T>());
         result *= pow(zgh, z - streams_boost::math::constants::half<T>()) / exp(zgh);
      }
   }
   return result;
}
...
T lgamma_imp(T z, const Policy& pol, const L& l, int* sign = 0)
{
   ...
   static const char* function = "streams_boost::math::lgamma<%1%>(%1%)";
   ...
   if(z <= 0)
   {
      ...
      result = log(streams_boost::math::constants::pi<T>()) - lgamma_imp(z, pol, l) - log(t);
   }
   else if(z < 15)
   {
   ...
   {
      // regular evaluation:
      T zgh = static_cast<T>(z + L::g() - streams_boost::math::constants::half<T>());
      result = log(zgh) - 1;
      result *= z - 0.5f;
...
inline T upper_gamma_fraction(T a, T z, T eps)
{
   ...
   return 1 / (z - a + 1 + streams_boost::math::tools::continued_fraction_a(f, eps));
}
...
inline T lower_gamma_series(T a, T z, const Policy& pol, T init_value = 0)
{
   ...
   streams_boost::uintmax_t max_iter = policies::get_max_series_iterations<Policy>();
   T factor = policies::get_epsilon<T, Policy>();
   T result = streams_boost::math::tools::sum_series(s, factor, max_iter, init_value);
   policies::check_series_iterations("streams_boost::math::detail::lower_gamma_series<%1%>(%1%)", max_iter, pol);
   return result;
}
...
T gamma_imp(T z, const Policy& pol, const lanczos::undefined_lanczos& l)
{
   ...
   STREAMS_BOOST_MATH_INSTRUMENT_CODE(prefix);
   if((floor(z) == z) && (z < max_factorial<T>::value))
   {
   ...
   {
      prefix = prefix * pow(z / streams_boost::math::constants::e<T>(), z);
      STREAMS_BOOST_MATH_INSTRUMENT_CODE(prefix);
      T sum = detail::lower_gamma_series(z, z, pol) / z;
      STREAMS_BOOST_MATH_INSTRUMENT_CODE(sum);
      sum += detail::upper_gamma_fraction(z, z, ::streams_boost::math::policies::get_epsilon<T, Policy>());
      STREAMS_BOOST_MATH_INSTRUMENT_CODE(sum);
      if(fabs(tools::max_value<T>() / prefix) < fabs(sum))
         return policies::raise_overflow_error<T>(function, "Result of tgamma is too large to represent.", pol);
      STREAMS_BOOST_MATH_INSTRUMENT_CODE((sum * prefix));
      return sum * prefix;
   }
   return prefix;
}
...
T lgamma_imp(T z, const Policy& pol, const lanczos::undefined_lanczos& l, int*sign)
{
   ...
   if(z <= 0)
   {
      ...
      result = log(streams_boost::math::constants::pi<T>()) - lgamma_imp(z, pol, l, 0) - log(t);
   }
   else if((z != 1) && (z != 2))
   {
      ...
      sum += detail::upper_gamma_fraction(z, limit, ::streams_boost::math::policies::get_epsilon<T, Policy>());
...
T tgammap1m1_imp(T dz, Policy const& pol, const L& l)
{
   ...
   if(dz < 0)
   {
      if(dz < -0.5)
      {
         ...
         result = streams_boost::math::tgamma(1+dz, pol) - 1;
         STREAMS_BOOST_MATH_INSTRUMENT_CODE(result);
      }
      else
      {
         // Use expm1 on lgamma:
         result = streams_boost::math::expm1(-streams_boost::math::log1p(dz, pol) 
   ...
   else
   {
      ...
      {
         // Use expm1 on lgamma:
         result = streams_boost::math::expm1(lgamma_small_imp<T>(dz+1, dz, dz-1, tag_type(), pol, l), pol);
         STREAMS_BOOST_MATH_INSTRUMENT_CODE(result);
      }
      else
      {
...
inline T tgammap1m1_imp(T dz, Policy const& pol,
                 const ::streams_boost::math::lanczos::undefined_lanczos& l)
{
   ...
   //
   STREAMS_BOOST_MATH_INSTRUMENT_CODE((dz > -0.5));
   STREAMS_BOOST_MATH_INSTRUMENT_CODE((dz < 2));
   STREAMS_BOOST_MATH_INSTRUMENT_CODE((ldexp(1.0, streams_boost::math::policies::digits<T, Policy>()) * fabs(result) < 1e34));
   if((dz > -0.5) && (dz < 2) && (ldexp(1.0, streams_boost::math::policies::digits<T, Policy>()) * fabs(result) < 1e34))
   {
      result = tgammap1m1_imp(dz, pol, streams_boost::math::lanczos::lanczos24m113());
      STREAMS_BOOST_MATH_INSTRUMENT_CODE(result);
   }
   return result;
}
...
T full_igamma_prefix(T a, T z, const Policy& pol)
{
   ...
   //
   if((streams_boost::math::fpclassify)(prefix) == (int)FP_INFINITE)
      policies::raise_overflow_error<T>("streams_boost::math::detail::full_igamma_prefix<%1%>(%1%, %1%)", "Result of incomplete gamma function is too large to represent.", pol);
...
T regularised_gamma_prefix(T a, T z, const Policy& pol, const L& l)
{
   ...
   else if((fabs(d*d*a) <= 100) && (a > 150))
   {
      ...
      prefix = a * streams_boost::math::log1pmx(d, pol) + z * static_cast<T>(0.5 - L::g()) / agh;
      prefix = exp(prefix);
   }
   else
   {
   ...
   prefix *= sqrt(agh / streams_boost::math::constants::e<T>()) / L::lanczos_sum_expG_scaled(a);
   return prefix;
}
//
...
T regularised_gamma_prefix(T a, T z, const Policy& pol, const lanczos::undefined_lanczos&)
{
   ...
   sum += detail::upper_gamma_fraction(a, limit, ::streams_boost::math::policies::get_epsilon<T, Policy>());
...
inline T tgamma_small_upper_part(T a, T x, const Policy& pol, T* pgam = 0, bool invert = false, T* pderivative = 0)
{
   ...
   //
   T result;
   result = streams_boost::math::tgamma1pm1(a, pol);
   if(pgam)
      *pgam = (result + 1) / a;
   T p = streams_boost::math::powm1(x, a, pol);
   result -= p;
   result /= a;
   detail::small_gamma2_series<T> s(a, x);
   streams_boost::uintmax_t max_iter = policies::get_max_series_iterations<Policy>() - 10;
   p += 1;
   if(pderivative)
      *pderivative = p / (*pgam * exp(x));
   T init_value = invert ? *pgam : 0;
   result = -p * tools::sum_series(s, streams_boost::math::policies::get_epsilon<T, Policy>(), max_iter, (init_value - result) / p);
   policies::check_series_iterations("streams_boost::math::tgamma_small_upper_part<%1%>(%1%, %1%)", max_iter, pol);
   if(invert)
      result = -result;
   return result;
}
...
inline T finite_gamma_q(T a, T x, Policy const& pol, T* pderivative = 0)
{
   ...
   if(pderivative)
   {
      *pderivative = e * pow(x, a) / streams_boost::math::unchecked_factorial<T>(itrunc(T(a - 1), pol));
   }
   return sum;
}
//
...
T finite_half_gamma_q(T a, T x, T* p_derivative, const Policy& pol)
{
   ...
   //
   STREAMS_BOOST_MATH_STD_USING
   T e = streams_boost::math::erfc(sqrt(x), pol);
   if((e != 0) && (a > 1))
   {
...
{
   static const char* function = "streams_boost::math::gamma_p<%1%>(%1%, %1%)";
   if(a <= 0)
   ...
   STREAMS_BOOST_ASSERT((p_derivative == 0) || (normalised == true));
   ...
   case 0:
      {
         ...
         if(normalised == false)
            result *= streams_boost::math::tgamma(a, pol);
         break;
      }
   case 1:
      {
   ...
   case 2:
      {
         ...
         if(result != 0)
         {
            ...
            if(invert)
            {
               init_value = -a * (normalised ? 1 : streams_boost::math::tgamma(a, pol)) / result;
            }
   ...
   if(invert)
   {
      T gam = normalised ? 1 : streams_boost::math::tgamma(a, pol);
      result = gam - result;
   }
   if(p_derivative)
   {
...
T tgamma_delta_ratio_imp_lanczos(T z, T delta, const Policy& pol, const L&)
{
   ...
   if(fabs(delta) < 10)
   {
      result = exp((constants::half<T>() - z) * streams_boost::math::log1p(delta / zgh, pol));
   }
   else
   {
...
T tgamma_delta_ratio_imp_lanczos(T z, T delta, const Policy& pol, const lanczos::undefined_lanczos&)
{
   ...
   if(delta < 10)
   {
      prefix *= exp(-z * streams_boost::math::log1p(delta / z, pol));
   }
   else
   {
      prefix *= pow(z / zd, z);
   ...
   sumd += detail::upper_gamma_fraction(zd, zd, ::streams_boost::math::policies::get_epsilon<T, Policy>());
   sum /= sumd;
   if(fabs(tools::max_value<T>() / prefix) < fabs(sum))
      return policies::raise_overflow_error<T>("streams_boost::math::tgamma_delta_ratio<%1%>(%1%, %1%)", "Result of tgamma is too large to represent.", pol);
   return sum * prefix;
}
...
T tgamma_delta_ratio_imp(T z, T delta, const Policy& pol)
{
   ...
   if(z <= 0)
      policies::raise_domain_error<T>("streams_boost::math::tgamma_delta_ratio<%1%>(%1%, %1%)", "Gamma function ratios only implemented for positive arguments (got a=%1%).", z, pol);
   if(z+delta <= 0)
      policies::raise_domain_error<T>("streams_boost::math::tgamma_delta_ratio<%1%>(%1%, %1%)", "Gamma function ratios only implemented for positive arguments (got b=%1%).", z+delta, pol);
...
T gamma_p_derivative_imp(T a, T x, const Policy& pol)
{
   ...
   //
   if(a <= 0)
      policies::raise_domain_error<T>("streams_boost::math::gamma_p_derivative<%1%>(%1%, %1%)", "Argument a to the incomplete gamma function must be greater than zero (got a=%1%).", a, pol);
   if(x < 0)
      policies::raise_domain_error<T>("streams_boost::math::gamma_p_derivative<%1%>(%1%, %1%)", "Argument x to the incomplete gamma function must be >= 0 (got x=%1%).", x, pol);
   //
   // Now special cases:
   //
   if(x == 0)
   {
      return (a > 1) ? 0 :
         (a == 1) ? 1 : policies::raise_overflow_error<T>("streams_boost::math::gamma_p_derivative<%1%>(%1%, %1%)", 0, pol);
   }
   //
   // Normal case:
   //
   ...
   {
      // overflow:
      return policies::raise_overflow_error<T>("streams_boost::math::gamma_p_derivative<%1%>(%1%, %1%)", 0, pol);
   }
   ...
   tgamma(T z, const Policy& /* pol */, const mpl::true_)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::gamma_imp(static_cast<value_type>(z), forwarding_policy(), evaluation_type()), "streams_boost::math::tgamma<%1%>(%1%)");
}
   ...
   tgamma(T1 a, T2 z, const Policy&, const mpl::false_)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(
      ...
      forwarding_policy(), static_cast<value_type*>(0)), "streams_boost::math::tgamma<%1%>(%1%, %1%)");
}
   ...
   lgamma(T z, int* sign, const Policy&)
{
   ...
   typedef typename policies::normalise<
      ...
      policies::discrete_quantile<>,
      policies::assert_undefined<> >::type forwarding_policy;
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::lgamma_imp(static_cast<value_type>(z), forwarding_policy(), evaluation_type(), sign), "streams_boost::math::lgamma<%1%>(%1%)");
}
...
{
   return ::streams_boost::math::lgamma(x, 0, pol);
}
...
inline typename tools::promote_args<T>::type 
   lgamma(T x)
{
   return ::streams_boost::math::lgamma(x, 0, policies::policy<>());
}
   ...
   tgamma1pm1(T z, const Policy& /* pol */)
{
   ...
   return policies::checked_narrowing_cast<typename remove_cv<result_type>::type, forwarding_policy>(detail::tgammap1m1_imp(static_cast<value_type>(z), forwarding_policy(), evaluation_type()), "streams_boost::math::tgamma1pm1<%!%>(%1%)");
}
   ...
   tgamma_delta_ratio(T1 z, T2 delta, const Policy& /* pol */)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::tgamma_delta_ratio_imp(static_cast<value_type>(z), static_cast<value_type>(delta), forwarding_policy()), "streams_boost::math::tgamma_delta_ratio<%1%>(%1%, %1%)");
}
template <class T1, class T2>
inline typename tools::promote_args<T1, T2>::type 
   ...
   tgamma_ratio(T1 a, T2 b, const Policy&)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::tgamma_delta_ratio_imp(static_cast<value_type>(a), static_cast<value_type>(static_cast<value_type>(b) - static_cast<value_type>(a)), forwarding_policy()), "streams_boost::math::tgamma_delta_ratio<%1%>(%1%, %1%)");
}
template <class T1, class T2>
inline typename tools::promote_args<T1, T2>::type 
   ...
   gamma_p_derivative(T1 a, T2 x, const Policy&)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::gamma_p_derivative_imp(static_cast<value_type>(a), static_cast<value_type>(x), forwarding_policy()), "streams_boost::math::gamma_p_derivative<%1%>(%1%, %1%)");
}
template <class T1, class T2>
inline typename tools::promote_args<T1, T2>::type 
...
#include <streams_boost/math/special_functions/detail/igamma_inverse.hpp>
#include <streams_boost/math/special_functions/detail/gamma_inva.hpp>
#include <streams_boost/math/special_functions/erf.hpp>

#endif // STREAMS_BOOST_MATH_SF_GAMMA_HPP
