...
#ifndef STREAMS_BOOST_MATH_SPECIAL_ERF_HPP
#define STREAMS_BOOST_MATH_SPECIAL_ERF_HPP
...
struct erf_asympt_series_t
{
   erf_asympt_series_t(T z) : xx(2 * -z * z), tk(1)
   {
      ...
      result = -exp(-z * z) / sqrt(streams_boost::math::constants::pi<T>());
      result /= z;
   }
...
T erf_imp(T z, bool invert, const Policy& pol, const Tag& t)
{
   ...
   STREAMS_BOOST_MATH_INSTRUMENT_CODE("Generic erf_imp called");
   ...
   if(!invert && (z > detail::erf_asymptotic_limit<T, Policy>()))
   {
      ...
      result = streams_boost::math::tools::sum_series(s, policies::get_epsilon<T, Policy>(), max_iter, 1);
      policies::check_series_iterations("streams_boost::math::erf<%1%>(%1%, %1%)", max_iter, pol);
   }
   else
   {
      T x = z * z;
      if(x < 0.6)
      {
         // Compute P:
         result = z * exp(-x);
         result /= sqrt(streams_boost::math::constants::pi<T>());
         if(result != 0)
...
T erf_imp(T z, bool invert, const Policy& pol, const mpl::int_<53>& t)
{
   ...
   STREAMS_BOOST_MATH_INSTRUMENT_CODE("53-bit precision erf_imp called");
...
T erf_imp(T z, bool invert, const Policy& pol, const mpl::int_<64>& t)
{
   ...
   STREAMS_BOOST_MATH_INSTRUMENT_CODE("64-bit precision erf_imp called");
...
T erf_imp(T z, bool invert, const Policy& pol, const mpl::int_<113>& t)
{
   ...
   STREAMS_BOOST_MATH_INSTRUMENT_CODE("113-bit precision erf_imp called");
...
inline typename tools::promote_args<T>::type erf(T z, const Policy& /* pol */)
{
   ...
   STREAMS_BOOST_MATH_INSTRUMENT_CODE("result_type = " << typeid(result_type).name());
   STREAMS_BOOST_MATH_INSTRUMENT_CODE("value_type = " << typeid(value_type).name());
   STREAMS_BOOST_MATH_INSTRUMENT_CODE("precision_type = " << typeid(precision_type).name());
   ...
   STREAMS_BOOST_MATH_INSTRUMENT_CODE("tag_type = " << typeid(tag_type).name());

   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::erf_imp(
      static_cast<value_type>(z),
      false,
      forwarding_policy(),
      tag_type()), "streams_boost::math::erf<%1%>(%1%, %1%)");
}
...
inline typename tools::promote_args<T>::type erfc(T z, const Policy& /* pol */)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::erf_imp(
      ...
      true,
      forwarding_policy(),
      tag_type()), "streams_boost::math::erfc<%1%>(%1%, %1%)");
}

template <class T>
inline typename tools::promote_args<T>::type erf(T z)
{
   return streams_boost::math::erf(z, policies::policy<>());
}

template <class T>
inline typename tools::promote_args<T>::type erfc(T z)
{
   return streams_boost::math::erfc(z, policies::policy<>());
}

} // namespace math
} // namespace streams_boost

#include <streams_boost/math/special_functions/detail/erf_inv.hpp>

#endif // STREAMS_BOOST_MATH_SPECIAL_ERF_HPP
