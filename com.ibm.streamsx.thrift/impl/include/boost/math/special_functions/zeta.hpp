...
#ifndef STREAMS_BOOST_MATH_ZETA_HPP
#define STREAMS_BOOST_MATH_ZETA_HPP
...
struct zeta_series_cache_size
{
   ...
   // to zeta for example.
   //
   typedef typename streams_boost::math::policies::precision<T,Policy>::type precision_type;
...
T zeta_series_imp(T s, T sc, const Policy&)
{
   ...
   return sum * 1 / -streams_boost::math::powm1(T(2), sc);
}
//
// Classical p-series:
//
...
inline T zeta_series2_imp(T s, const Policy& pol)
{
   ...
   T result = tools::sum_series(
      ...
      max_iter);
   policies::check_series_iterations("streams_boost::math::zeta_series2<%1%>(%1%)", max_iter, pol);
   return result;
}
...
T zeta_imp(T s, T sc, const Policy& pol, const Tag& tag)
{
   ...
   else if(s < 0)
   {
      ...
      {
         result = streams_boost::math::sin_pi(0.5f * sc, pol)
            * 2 * pow(2 * constants::pi<T>(), -s) 
            * streams_boost::math::tgamma(s, pol) 
...
inline typename tools::promote_args<T>::type zeta(T s, const Policy&)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::zeta_imp(
      ...
      forwarding_policy(),
      tag_type()), "streams_boost::math::zeta<%1%>(%1%)");
}
...
#endif // STREAMS_BOOST_MATH_ZETA_HPP
