...
#ifndef STREAMS_BOOST_MATH_SIN_PI_HPP
#define STREAMS_BOOST_MATH_SIN_PI_HPP
...
inline typename tools::promote_args<T>::type sin_pi(T x, const Policy& pol)
{
   ...
   return streams_boost::math::detail::sin_pi_imp<result_type>(x, pol);
}

template <class T>
inline typename tools::promote_args<T>::type sin_pi(T x)
{
   return streams_boost::math::sin_pi(x, policies::policy<>());
}
