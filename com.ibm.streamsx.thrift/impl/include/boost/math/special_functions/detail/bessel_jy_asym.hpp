...
// functions, as x -> INF.
//
#ifndef STREAMS_BOOST_MATH_SF_DETAIL_BESSEL_JY_ASYM_HPP
#define STREAMS_BOOST_MATH_SF_DETAIL_BESSEL_JY_ASYM_HPP
...
{
   T c = 1;
   T p = (v / streams_boost::math::sin_pi(v, pol)) * pow(x / 2, -v) / streams_boost::math::tgamma(1 - v, pol);
   T q = (v / streams_boost::math::sin_pi(v, pol)) * pow(x / 2, v) / streams_boost::math::tgamma(1 + v, pol);
   T f = (p - q) / v;
   T g_prefix = streams_boost::math::sin_pi(v / 2, pol);
...
T asymptotic_bessel_i_large_x(T v, T x, const Policy& pol)
{
   ...
   return (streams_boost::math::isfinite)(s) ? 
      s : policies::raise_overflow_error<T>("streams_boost::math::asymptotic_bessel_i_large_x<%1%>(%1%,%1%)", 0, pol);
}
