...
#ifndef STREAMS_BOOST_MATH_SPECIAL_FUNCTIONS_DETAIL_LGAMMA_SMALL
#define STREAMS_BOOST_MATH_SPECIAL_FUNCTIONS_DETAIL_LGAMMA_SMALL
...
T lgamma_small_imp(T z, T zm1, T zm2, const mpl::int_<113>&, const Policy& /* l */, const L&)
{
   ...
   else if(z > 2)
   {
      ...
      STREAMS_BOOST_MATH_INSTRUMENT_CODE(zm2);
      STREAMS_BOOST_MATH_INSTRUMENT_CODE(z);
      STREAMS_BOOST_MATH_INSTRUMENT_CODE(result);
...
T lgamma_small_imp(T z, T zm1, T zm2, const mpl::int_<0>&, const Policy& pol, const L&)
{
   ...
   else if(z >= 1.5)
   {
      // special case near 2:
      T dz = zm2;
      result = dz * log((z + L::g() - T(0.5)) / streams_boost::math::constants::e<T>());
      result += streams_boost::math::log1p(dz / (L::g() + T(1.5)), pol) * T(1.5);
      result += streams_boost::math::log1p(L::lanczos_sum_near_2(dz), pol);
   }
   else
   {
      // special case near 1:
      T dz = zm1;
      result = dz * log((z + L::g() - T(0.5)) / streams_boost::math::constants::e<T>());
      result += streams_boost::math::log1p(dz / (L::g() + T(0.5)), pol) / 2;
      result += streams_boost::math::log1p(L::lanczos_sum_near_1(dz), pol);
   }
   return result;
}
...
#endif // STREAMS_BOOST_MATH_SPECIAL_FUNCTIONS_DETAIL_LGAMMA_SMALL
