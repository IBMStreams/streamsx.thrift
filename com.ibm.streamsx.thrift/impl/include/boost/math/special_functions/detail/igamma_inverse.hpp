...
#ifndef STREAMS_BOOST_MATH_SPECIAL_FUNCTIONS_IGAMMA_INVERSE_HPP
#define STREAMS_BOOST_MATH_SPECIAL_FUNCTIONS_IGAMMA_INVERSE_HPP
...
inline T didonato_FN(T p, T a, T x, unsigned N, T tolerance, const Policy& pol)
{
   ...
   //
   STREAMS_BOOST_MATH_STD_USING
   T u = log(p) + streams_boost::math::lgamma(a + 1, pol);
...
T find_inverse_gamma(T a, T p, T q, const Policy& pol, bool* p_has_10_digits)
{
   ...
   *p_has_10_digits = false;
   ...
   {
      T g = streams_boost::math::tgamma(a, pol);
      T b = q * g;
      STREAMS_BOOST_MATH_INSTRUMENT_VARIABLE(g);
   ...
   else
   {
      ...
      STREAMS_BOOST_MATH_INSTRUMENT_VARIABLE(s);
      ...
      STREAMS_BOOST_MATH_INSTRUMENT_VARIABLE(ra);
      ...
      STREAMS_BOOST_MATH_INSTRUMENT_VARIABLE(w);
      ...
      else if (p > 0.5)
      {
         ...
         else
         {
            ...
            T lg = streams_boost::math::lgamma(a, pol);
            T lb = log(q) + lg;
            if(lb < -D * 2.3)
            {
      ...
      else
      {
         ...
         if(w < 0.15f * ap1)
         {
            ...
            T v = log(p) + streams_boost::math::lgamma(ap1, pol);
            T s = 1;
            z = exp((v + w) / a);
            s = streams_boost::math::log1p(z / ap1 * (1 + z / (a + 2)));
            z = exp((v + z - s) / a);
            z = exp((v + z - s) / a);
            s = streams_boost::math::log1p(z / ap1 * (1 + z / (a + 2) * (1 + z / (a + 3))));
            z = exp((v + z - s) / a);
...
struct gamma_p_inverse_func
{
   ...
   std::tr1::tuple<T, T, T> operator()(const T& x)const
   {
      ...
      T f, f1;
      value_type ft;
      f = static_cast<T>(streams_boost::math::detail::gamma_incomplete_imp(
...
T gamma_p_inv_imp(T a, T p, const Policy& pol)
{
   ...
   static const char* function = "streams_boost::math::gamma_p_inv<%1%>(%1%, %1%)";
   ...
   STREAMS_BOOST_MATH_INSTRUMENT_VARIABLE(guess);
   //
...
T gamma_q_inv_imp(T a, T q, const Policy& pol)
{
   ...
   static const char* function = "streams_boost::math::gamma_q_inv<%1%>(%1%, %1%)";
