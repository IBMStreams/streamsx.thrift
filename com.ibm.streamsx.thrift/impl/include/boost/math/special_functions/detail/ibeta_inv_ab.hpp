...
#ifndef STREAMS_BOOST_MATH_SP_DETAIL_BETA_INV_AB
#define STREAMS_BOOST_MATH_SP_DETAIL_BETA_INV_AB
...
struct beta_inv_ab_t
{
   ...
   {
      return invert ? 
         p - streams_boost::math::ibetac(swap_ab ? b : a, swap_ab ? a : b, z, Policy()) 
         : streams_boost::math::ibeta(swap_ab ? b : a, swap_ab ? a : b, z, Policy()) - p;
   }
private:
   T b, z, p;
   bool invert, swap_ab;
...
{
   STREAMS_BOOST_MATH_STD_USING  // for ADL of std lib math functions
   //
   // Special cases first:
   //
   STREAMS_BOOST_MATH_INSTRUMENT_CODE("b = " << b << " z = " << z << " p = " << p << " q = " << " swap = " << swap_ab);
   if(p == 0)
   {
   ...
   if(max_iter >= policies::get_max_root_iterations<Policy>())
      policies::raise_evaluation_error<T>("streams_boost::math::ibeta_invab_imp<%1%>(%1%,%1%,%1%)", "Unable to locate the root within a reasonable number of iterations, closest approximation so far was %1%", r.first, pol);
      ...
      ibeta_inva(RT1 b, RT2 x, RT3 p, const Policy& pol)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(
      detail::ibeta_inv_ab_imp(
         ...
         false, pol), 
      "streams_boost::math::ibeta_inva<%1%>(%1%,%1%,%1%)");
}
      ...
      ibetac_inva(RT1 b, RT2 x, RT3 q, const Policy& pol)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(
      detail::ibeta_inv_ab_imp(
         ...
         false, pol),
      "streams_boost::math::ibetac_inva<%1%>(%1%,%1%,%1%)");
}
      ...
      ibeta_invb(RT1 a, RT2 x, RT3 p, const Policy& pol)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(
      detail::ibeta_inv_ab_imp(
         ...
         static_cast<value_type>(1 - static_cast<value_type>(p)), 
         true, pol),
      "streams_boost::math::ibeta_invb<%1%>(%1%,%1%,%1%)");
}
      ...
      ibetac_invb(RT1 a, RT2 x, RT3 q, const Policy& pol)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(
      detail::ibeta_inv_ab_imp(
         ...
         static_cast<value_type>(q), 
         true, pol),
         "streams_boost::math::ibetac_invb<%1%>(%1%,%1%,%1%)");
}
...
{
   return streams_boost::math::ibeta_inva(b, x, p, policies::policy<>());
}
...
{
   return streams_boost::math::ibetac_inva(b, x, q, policies::policy<>());
}
...
{
   return streams_boost::math::ibeta_invb(a, x, p, policies::policy<>());
}
...
{
   return streams_boost::math::ibetac_invb(a, x, q, policies::policy<>());
}

} // namespace math
} // namespace streams_boost

#endif // STREAMS_BOOST_MATH_SP_DETAIL_BETA_INV_AB
