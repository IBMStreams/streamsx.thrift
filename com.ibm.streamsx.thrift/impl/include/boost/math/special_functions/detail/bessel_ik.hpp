...
#ifndef STREAMS_BOOST_MATH_BESSEL_IK_HPP
#define STREAMS_BOOST_MATH_BESSEL_IK_HPP
...
int temme_ik(T v, T x, T* K, T* K1, const Policy& pol)
{
    ...
    STREAMS_BOOST_ASSERT(abs(x) <= 2);
    STREAMS_BOOST_ASSERT(abs(v) <= 0.5f);

    T gp = streams_boost::math::tgamma1pm1(v, pol);
    T gm = streams_boost::math::tgamma1pm1(-v, pol);

    a = log(x / 2);
    b = exp(v * a);
    sigma = -a * v;
    c = abs(v) < tools::epsilon<T>() ?
       T(1) : T(streams_boost::math::sin_pi(v) / (v * pi<T>()));
    ...
    policies::check_series_iterations("streams_boost::math::bessel_ik<%1%>(%1%,%1%) in temme_ik", k, pol);
...
int CF1_ik(T v, T x, T* fv, const Policy& pol)
{
    ...
    STREAMS_BOOST_MATH_INSTRUMENT_VARIABLE(tolerance);
    tiny = sqrt(tools::min_value<T>());
    STREAMS_BOOST_MATH_INSTRUMENT_VARIABLE(tiny);
    ...
    for (k = 1; k < policies::get_max_series_iterations<Policy>(); k++)
    {
        ...
        delta = C * D;
        f *= delta;
        STREAMS_BOOST_MATH_INSTRUMENT_VARIABLE(delta-1);
    ...
    policies::check_series_iterations("streams_boost::math::bessel_ik<%1%>(%1%,%1%) in CF1_ik", k, pol);
...
int CF2_ik(T v, T x, T* Kv, T* Kv1, const Policy& pol)
{
    ...
    STREAMS_BOOST_MATH_INSTRUMENT_VARIABLE(D);
    STREAMS_BOOST_MATH_INSTRUMENT_VARIABLE(f);
    for (k = 2; k < policies::get_max_series_iterations<Policy>(); k++)     // starting from 2
    {
        ...
        STREAMS_BOOST_MATH_INSTRUMENT_VARIABLE(Q * delta);
        STREAMS_BOOST_MATH_INSTRUMENT_VARIABLE(abs(S) * tolerance);
        if (abs(Q * delta) < abs(S) * tolerance) 
        { 
           break; 
        }
    }
    policies::check_series_iterations("streams_boost::math::bessel_ik<%1%>(%1%,%1%) in CF2_ik", k, pol);

    *Kv = sqrt(pi<T>() / (2 * x)) * exp(-x) / S;
    *Kv1 = *Kv * (0.5f + v + x + (v * v - 0.25f) * f) / x;
    STREAMS_BOOST_MATH_INSTRUMENT_VARIABLE(*Kv);
    STREAMS_BOOST_MATH_INSTRUMENT_VARIABLE(*Kv1);
...
int bessel_ik(T v, T x, T* I, T* K, int kind, const Policy& pol)
{
    ...
    STREAMS_BOOST_MATH_INSTRUMENT_VARIABLE(kind);

    STREAMS_BOOST_MATH_STD_USING
    using namespace streams_boost::math::tools;
    using namespace streams_boost::math::constants;

    static const char* function = "streams_boost::math::bessel_ik<%1%>(%1%,%1%)";
    ...
    STREAMS_BOOST_MATH_INSTRUMENT_VARIABLE(u);
    ...
    if (x == 0)
    {
       ...
       {
           T z = (u + n % 2);
           Iv = streams_boost::math::sin_pi(z, pol) == 0 ? 
               Iv : 
    ...
    {
        T z = (u + n % 2);
        *I = Iv + (2 / pi<T>()) * streams_boost::math::sin_pi(z) * Kv;   // reflection formula
        *K = Kv;
    }
    else
    {
...
#endif // STREAMS_BOOST_MATH_BESSEL_IK_HPP
