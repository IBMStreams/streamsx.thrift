...
#ifndef STREAMS_BOOST_MATH_SF_ERF_INV_HPP
#define STREAMS_BOOST_MATH_SF_ERF_INV_HPP
...
T erf_inv_imp(const T& p, const T& q, const Policy&, const streams_boost::mpl::int_<64>*)
{
...
struct erf_roots
{
   std::tr1::tuple<T,T,T> operator()(const T& guess)
   {
      ...
      return std::tr1::make_tuple(((sign > 0) ? streams_boost::math::erf(guess, Policy()) : streams_boost::math::erfc(guess, Policy())) - target, derivative, derivative2);
   }
...
template <class T, class Policy>
T erf_inv_imp(const T& p, const T& q, const Policy& pol, const streams_boost::mpl::int_<0>*)
{
   //
   ...
   if(policies::digits<T, Policy>() > 64)
   {
      ...
      policies::check_root_iterations("streams_boost::math::erf_inv<%1%>", max_iter, pol);
   }
   else
   {
      result = guess;
...
typename tools::promote_args<T>::type erfc_inv(T z, const Policy& pol)
{
   ...
   //
   static const char* function = "streams_boost::math::erfc_inv<%1%>(%1%, %1%)";
   if((z < 0) || (z > 2))
...
typename tools::promote_args<T>::type erf_inv(T z, const Policy& pol)
{
   ...
   // Begin by testing for domain errors, and other special cases:
   //
   static const char* function = "streams_boost::math::erf_inv<%1%>(%1%, %1%)";
   if((z < -1) || (z > 1))
...
#endif // STREAMS_BOOST_MATH_SF_ERF_INV_HPP
