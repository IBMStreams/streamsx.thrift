...
#ifndef STREAMS_BOOST_MATH_BESSEL_JY_HPP
#define STREAMS_BOOST_MATH_BESSEL_JY_HPP
...
#include <streams_boost/math/special_functions/detail/simple_complex.hpp>
#include <streams_boost/math/special_functions/detail/bessel_jy_asym.hpp>
...
int temme_jy(T v, T x, T* Y, T* Y1, const Policy& pol)
{
    ...
    STREAMS_BOOST_ASSERT(fabs(v) <= 0.5f);  // precondition for using this routine

    T gp = streams_boost::math::tgamma1pm1(v, pol);
    T gm = streams_boost::math::tgamma1pm1(-v, pol);
    T spv = streams_boost::math::sin_pi(v, pol);
    T spv2 = streams_boost::math::sin_pi(v/2, pol);
    T xp = pow(x/2, v);
    ...
    policies::check_series_iterations("streams_boost::math::bessel_jy<%1%>(%1%,%1%) in temme_jy", k, pol);
    *Y = -sum;
    *Y1 = -2 * sum1 / x;
...
int CF1_jy(T v, T x, T* fv, int* sign, const Policy& pol)
{
    ...
    policies::check_series_iterations("streams_boost::math::bessel_jy<%1%>(%1%,%1%) in CF1_jy", k / 100, pol);
    *fv = -f;
...
int CF2_jy(T v, T x, T* p, T* q, const Policy& pol)
{
    ...
    policies::check_series_iterations("streams_boost::math::bessel_jy<%1%>(%1%,%1%) in CF2_jy", k, pol);
    *p = real(f);
    *q = imag(f);
...
int bessel_jy(T v, T x, T* J, T* Y, int kind, const Policy& pol)
{
    ...
    static const char* function = "streams_boost::math::bessel_jy<%1%>(%1%,%1%)";
    ...
    {
        T z = (u + n % 2);
        *J = streams_boost::math::cos_pi(z, pol) * Jv - streams_boost::math::sin_pi(z, pol) * Yv;     // reflection formula
        *Y = streams_boost::math::sin_pi(z, pol) * Jv + streams_boost::math::cos_pi(z, pol) * Yv;
    }
    else
    {
        *J = Jv;
...
#endif // STREAMS_BOOST_MATH_BESSEL_JY_HPP
