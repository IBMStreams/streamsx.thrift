...
#ifndef STREAMS_BOOST_MATH_SF_DETAIL_INV_T_HPP
#define STREAMS_BOOST_MATH_SF_DETAIL_INV_T_HPP

#ifdef _MSC_VER
#pragma once
#endif

#include <streams_boost/math/special_functions/cbrt.hpp>
...
T inverse_students_t_hill(T ndf, T u, const Policy& pol)
{
   ...
   STREAMS_BOOST_ASSERT(u <= 0.5);

   T a, b, c, d, q, x, y;

   if (ndf > 1e20f)
      return -streams_boost::math::erfc_inv(2 * u, pol) * constants::root_two<T>();
   ...
   if (y > (0.05f + a))
   {
      ...
      //
      x = -streams_boost::math::erfc_inv(2 * u, pol) * constants::root_two<T>();
      y = x * x;
      ...
      y = streams_boost::math::expm1(a * y * y, pol);
   }
   else
   {
...
T inverse_students_t_tail_series(T df, T v, const Policy& pol)
{
   ...
   T w = streams_boost::math::tgamma_delta_ratio(df / 2, constants::half<T>(), pol)
...
T inverse_students_t_body_series(T df, T u, const Policy& pol)
{
   ...
   //
   T v = streams_boost::math::tgamma_delta_ratio(df / 2, constants::half<T>(), pol)
...
calculate_real:
      if(df < 3)
      {
         ...
         if(u > crossover)
         {
            result = streams_boost::math::detail::inverse_students_t_body_series(df, u, pol);
         }
         else
         {
            result = streams_boost::math::detail::inverse_students_t_tail_series(df, u, pol);
         }
      }
      else
      {
         ...
         if(u > crossover)
         {
            result = streams_boost::math::detail::inverse_students_t_hill(df, u, pol);
         }
         else
         {
            result = streams_boost::math::detail::inverse_students_t_tail_series(df, u, pol);
...
inline T find_ibeta_inv_from_t_dist(T a, T p, T q, T* py, const Policy& pol)
{
   ...
   T df = a * 2;
   T t = streams_boost::math::detail::inverse_students_t(df, u, v, pol);
   T x = df / (df + t * t);
   *py = t * t / (df + t * t);
   return x;
}
...
inline T fast_students_t_quantile_imp(T df, T p, const Policy& pol, const mpl::false_*)
{
   ...
   if(df * y > tools::max_value<T>() * x)
      t = policies::raise_overflow_error<T>("streams_boost::math::students_t_quantile<%1%>(%1%,%1%)", 0, pol);
   else
      t = sqrt(df * y / x);
   //
...
T fast_students_t_quantile_imp(T df, T p, const Policy& pol, const mpl::true_*)
{
   ...
   bool invert = false;
   if((df < 2) && (floor(df) != df))
      return streams_boost::math::detail::fast_students_t_quantile_imp(df, p, pol, static_cast<mpl::false_*>(0));
   if(p > 0.5)
   {
      p = 1 - p;
      invert = true;
...
inline T fast_students_t_quantile(T df, T p, const Policy& pol)
{
   ...
   return policies::checked_narrowing_cast<T, forwarding_policy>(fast_students_t_quantile_imp(static_cast<value_type>(df), static_cast<value_type>(p), pol, static_cast<tag_type*>(0)), "streams_boost::math::students_t_quantile<%1%>(%1%,%1%,%1%)");
}

}}} // namespaces

#endif // STREAMS_BOOST_MATH_SF_DETAIL_INV_T_HPP
