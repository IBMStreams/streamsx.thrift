...
#ifndef STREAMS_BOOST_MATH_SPECIAL_FUNCTIONS_IBETA_INVERSE_HPP
#define STREAMS_BOOST_MATH_SPECIAL_FUNCTIONS_IBETA_INVERSE_HPP

#ifdef _MSC_VER
#pragma once
#endif

#include <streams_boost/math/special_functions/beta.hpp>
#include <streams_boost/math/special_functions/erf.hpp>
#include <streams_boost/math/tools/roots.hpp>
#include <streams_boost/math/special_functions/detail/t_distribution_inv.hpp>
...
T temme_method_1_ibeta_inverse(T a, T b, T z, const Policy& pol)
{
   ...
   //
   T eta0 = streams_boost::math::erfc_inv(2 * z, pol);
   eta0 /= -sqrt(a / 2);
   ...
   STREAMS_BOOST_ASSERT(x >= 0);
   STREAMS_BOOST_ASSERT(x <= 1);
   STREAMS_BOOST_ASSERT(eta * (x - 0.5) >= 0);
#ifdef STREAMS_BOOST_INSTRUMENT
...
T temme_method_2_ibeta_inverse(T /*a*/, T /*b*/, T z, T r, T theta, const Policy& pol)
{
   ...
   static const STREAMS_BOOST_MATH_INT_TABLE_TYPE(T, int) co1[] = { -1, -5, 5 };
   workspace[1] = -tools::evaluate_even_polynomial(co1, s, 3) / (36 * sc_2);
   static const STREAMS_BOOST_MATH_INT_TABLE_TYPE(T, int) co2[] = { 1, 21, -69, 46 };
   workspace[2] = tools::evaluate_even_polynomial(co2, s, 4) / (1620 * sc_3);
   static const STREAMS_BOOST_MATH_INT_TABLE_TYPE(T, int) co3[] = { 7, -2, 33, -62, 31 };
   workspace[3] = -tools::evaluate_even_polynomial(co3, s, 5) / (6480 * sc_4);
   static const STREAMS_BOOST_MATH_INT_TABLE_TYPE(T, int) co4[] = { 25, -52, -17, 88, -115, 46 };
   workspace[4] = tools::evaluate_even_polynomial(co4, s, 6) / (90720 * sc_5);
   terms[1] = tools::evaluate_polynomial(workspace, eta0, 5);
   //
   // Now evaluate e2 and put it in terms[2]:
   //
   static const STREAMS_BOOST_MATH_INT_TABLE_TYPE(T, int) co5[] = { 7, 12, -78, 52 };
   workspace[0] = -tools::evaluate_even_polynomial(co5, s, 4) / (405 * sc_3);
   static const STREAMS_BOOST_MATH_INT_TABLE_TYPE(T, int) co6[] = { -7, 2, 183, -370, 185 };
   workspace[1] = tools::evaluate_even_polynomial(co6, s, 5) / (2592 * sc_4);
   static const STREAMS_BOOST_MATH_INT_TABLE_TYPE(T, int) co7[] = { -533, 776, -1835, 10240, -13525, 5410 };
   workspace[2] = -tools::evaluate_even_polynomial(co7, s, 6) / (204120 * sc_5);
   static const STREAMS_BOOST_MATH_INT_TABLE_TYPE(T, int) co8[] = { -1579, 3747, -3372, -15821, 45588, -45213, 15071 };
   workspace[3] = -tools::evaluate_even_polynomial(co8, s, 7) / (2099520 * sc_6);
   terms[2] = tools::evaluate_polynomial(workspace, eta0, 4);
   //
   // And e3, and put it in terms[3]:
   //
   static const STREAMS_BOOST_MATH_INT_TABLE_TYPE(T, int) co9[] = {449, -1259, -769, 6686, -9260, 3704 };
   workspace[0] = tools::evaluate_even_polynomial(co9, s, 6) / (102060 * sc_5);
   static const STREAMS_BOOST_MATH_INT_TABLE_TYPE(T, int) co10[] = { 63149, -151557, 140052, -727469, 2239932, -2251437, 750479 };
   workspace[1] = -tools::evaluate_even_polynomial(co10, s, 7) / (20995200 * sc_6);
   static const STREAMS_BOOST_MATH_INT_TABLE_TYPE(T, int) co11[] = { 29233, -78755, 105222, 146879, -1602610, 3195183, -2554139, 729754 };
   ...
   if(fabs(eta) < 0.7)
   {
      ...
      static const STREAMS_BOOST_MATH_INT_TABLE_TYPE(T, int) co12[] = { 1, -13, 13 };
      workspace[3] = tools::evaluate_polynomial(co12, workspace[0], 3) / (36 * s * c);
      static const STREAMS_BOOST_MATH_INT_TABLE_TYPE(T, int) co13[] = { 1, 21, -69, 46 };
...
T temme_method_3_ibeta_inverse(T a, T b, T p, T q, const Policy& pol)
{
   ...
   T eta0;
   if(p < q)
      eta0 = streams_boost::math::gamma_q_inv(b, p, pol);
   else
      eta0 = streams_boost::math::gamma_p_inv(b, q, pol);
   eta0 /= a;
   //
...
{
   STREAMS_BOOST_MATH_STD_USING  // For ADL of math functions.
   ...
   else if(a + b > 5)
   {
      ...
      else
      {
         ...
         if((lambda >= 0.2) && (lambda <= 0.8) && (lambda >= 10))
         {
            ...
            {
               x = ppa * pow(a * streams_boost::math::beta(a, b, pol), 1/a);
            }
            else
         ...
         else
         {
            ...
            T bet = 0;
            if(b < 2)
               bet = streams_boost::math::beta(a, b, pol);
            if(bet != 0)
            {
               y = pow(b * q * bet, 1/b);
               x = 1 - y;
   ...
   else if((a < 1) && (b < 1))
   {
      ...
      //
      T fs = streams_boost::math::ibeta(a, b, xs, pol) - p;
      ...
      T xg = pow(a * p * streams_boost::math::beta(a, b, pol), 1/a);
      x = xg / (1 + xg);
      y = 1 / (1 + xg);
      //
   ...
   else if((a > 1) && (b > 1))
   {
      ...
      T xs = (a - 1) / (a + b - 2);
      T xs2 = (b - 1) / (a + b - 2);
      T ps = streams_boost::math::ibeta(a, b, xs, pol) - p;
      ...
      //
      T lx = log(p * a * streams_boost::math::beta(a, b, pol)) / a;
      x = exp(lx);
      y = x < 0.9 ? T(1 - x) : (T)(-streams_boost::math::expm1(lx, pol));
   ...
   else /*if((a <= 1) != (b <= 1))*/
   {
      ...
      if(pow(p, 1/a) < 0.5)
      {
         x = pow(p * a * streams_boost::math::beta(a, b, pol), 1 / a);
         if(x == 0)
            x = streams_boost::math::tools::min_value<T>();
         y = 1 - x;
      }
      else /*if(pow(q, 1/b) < 0.1)*/
      {
         // model a distorted quarter circle:
         y = pow(1 - pow(p, b * streams_boost::math::beta(a, b, pol)), 1/b);
         if(y == 0)
            y = streams_boost::math::tools::min_value<T>();
         x = 1 - y;
      }
   }
   ...
   if(lower == 0)
   {
      if(invert && (py == 0))
      {
         ...
         //
         lower = streams_boost::math::tools::epsilon<T>();
         if(x < lower)
            x = lower;
      }
      else
         lower = streams_boost::math::tools::min_value<T>();
      if(x < lower)
         x = lower;
   }
   //
   ...
   //
   int digits = streams_boost::math::policies::digits<T, Policy>() / 2;
   if((x < 1e-50) && ((a < 1) || (b < 1)))
   {
      //
   ...
   //
   streams_boost::uintmax_t max_iter = policies::get_max_root_iterations<Policy>();
   x = streams_boost::math::tools::halley_iterate(
      streams_boost::math::detail::ibeta_roots<T, Policy>(a, b, (p < q ? p : q), (p < q ? false : true)), x, lower, upper, digits, max_iter);
   policies::check_root_iterations("streams_boost::math::ibeta<%1%>(%1%, %1%, %1%)", max_iter, pol);
   //
   ...
   //
   //STREAMS_BOOST_ASSERT(x != upper);
   //STREAMS_BOOST_ASSERT((x != lower) || (x == streams_boost::math::tools::min_value<T>()) || (x == streams_boost::math::tools::epsilon<T>()));
   //
...
{
   static const char* function = "streams_boost::math::ibeta_inv<%1%>(%1%,%1%,%1%)";
   STREAMS_BOOST_FPU_EXCEPTION_GUARD
...
{
   static const char* function = "streams_boost::math::ibetac_inv<%1%>(%1%,%1%,%1%)";
   STREAMS_BOOST_FPU_EXCEPTION_GUARD
   typedef typename tools::promote_args<T1, T2, T3, T4>::type result_type;
   typedef typename policies::evaluation<result_type, Policy>::type value_type;
   typedef typename policies::normalise<
...
#endif // STREAMS_BOOST_MATH_SPECIAL_FUNCTIONS_IGAMMA_INVERSE_HPP
