...
//
#ifndef STREAMS_BOOST_MATH_BESSEL_HPP
#define STREAMS_BOOST_MATH_BESSEL_HPP
...
#include <streams_boost/math/special_functions/detail/bessel_jn.hpp>
#include <streams_boost/math/special_functions/detail/bessel_yn.hpp>
#include <streams_boost/math/special_functions/detail/bessel_ik.hpp>
#include <streams_boost/math/special_functions/detail/bessel_i0.hpp>
#include <streams_boost/math/special_functions/detail/bessel_i1.hpp>
#include <streams_boost/math/special_functions/detail/bessel_kn.hpp>
#include <streams_boost/math/special_functions/sin_pi.hpp>
#include <streams_boost/math/special_functions/cos_pi.hpp>
#include <streams_boost/math/special_functions/sinc.hpp>
...
struct bessel_j_small_z_series_term
{
      ...
      : N(0), v(v_)
   {
      ...
      mult = x / 2;
      term = pow(mult, v) / streams_boost::math::tgamma(v+1, Policy());
      mult *= -mult;
   }
   T operator()()
   {
...
struct sph_bessel_j_small_z_series_term
{
      ...
      : N(0), v(v_)
   {
      STREAMS_BOOST_MATH_STD_USING
      mult = x / 2;
      term = pow(mult, T(v)) / streams_boost::math::tgamma(v+1+T(0.5f), Policy());
      mult *= -mult;
   }
   T operator()()
   {
...
inline T bessel_j_small_z_series(T v, T x, const Policy& pol)
{
   ...
   T zero = 0;
   T result = streams_boost::math::tools::sum_series(s, streams_boost::math::policies::get_epsilon<T, Policy>(), max_iter, zero);
#else
   T result = streams_boost::math::tools::sum_series(s, streams_boost::math::policies::get_epsilon<T, Policy>(), max_iter);
#endif
   policies::check_series_iterations("streams_boost::math::bessel_j_small_z_series<%1%>(%1%,%1%)", max_iter, pol);
   return result;
}
...
inline T sph_bessel_j_small_z_series(unsigned v, T x, const Policy& pol)
{
   ...
   T result = streams_boost::math::tools::sum_series(s, streams_boost::math::policies::get_epsilon<T, Policy>(), max_iter);
#endif
   policies::check_series_iterations("streams_boost::math::sph_bessel_j_small_z_series<%1%>(%1%,%1%)", max_iter, pol);
...
{
   STREAMS_BOOST_MATH_STD_USING
   static const char* function = "streams_boost::math::bessel_j<%1%>(%1%,%1%)";
   if(x < 0)
   {
...
inline T sph_bessel_j_imp(unsigned n, T x, const Policy& pol)
{
   ...
   if(x < 0)
      return policies::raise_domain_error<T>(
         "streams_boost::math::sph_bessel_j<%1%>(%1%,%1%)",
         "Got x = %1%, but function requires x > 0.", x, pol);
   //
   // Special case, n == 0 resolves down to the sinus cardinal of x:
   //
   if(n == 0)
      return streams_boost::math::sinc_pi(x, pol);
   //
...
T cyl_bessel_i_imp(T v, T x, const Policy& pol)
{
   ...
   if(x < 0)
   {
      ...
      if(floor(v) == v)
      {
         ...
         return policies::raise_domain_error<T>(
         "streams_boost::math::cyl_bessel_i<%1%>(%1%,%1%)",
            "Got x = %1%, but we need x >= 0", x, pol);
   }
   if(x == 0)
   {
...
{
   static const char* function = "streams_boost::math::cyl_bessel_k<%1%>(%1%,%1%)";
   STREAMS_BOOST_MATH_STD_USING
   if(x < 0)
   {
      return policies::raise_domain_error<T>(
...
{
   static const char* function = "streams_boost::math::cyl_neumann<%1%>(%1%,%1%)";

   STREAMS_BOOST_MATH_INSTRUMENT_VARIABLE(v);
   ...
   // is -INF:
   //
   if(!(streams_boost::math::isfinite)(y))
...
{
   STREAMS_BOOST_MATH_STD_USING // ADL of std names
   static const char* function = "streams_boost::math::sph_neumann<%1%>(%1%,%1%)";
   //
...
inline typename detail::bessel_traits<T1, T2, Policy>::result_type cyl_bessel_j(T1 v, T2 x, const Policy& pol)
{
   ...
   return policies::checked_narrowing_cast<result_type, Policy>(detail::cyl_bessel_j_imp<value_type>(v, static_cast<value_type>(x), tag_type(), pol), "streams_boost::math::cyl_bessel_j<%1%>(%1%,%1%)");
}
...
inline typename detail::bessel_traits<T, T, Policy>::result_type sph_bessel(unsigned v, T x, const Policy& pol)
{
   ...
   typedef typename policies::evaluation<result_type, Policy>::type value_type;
   return policies::checked_narrowing_cast<result_type, Policy>(detail::sph_bessel_j_imp<value_type>(v, static_cast<value_type>(x), pol), "streams_boost::math::sph_bessel<%1%>(%1%,%1%)");
}
...
inline typename detail::bessel_traits<T1, T2, Policy>::result_type cyl_bessel_i(T1 v, T2 x, const Policy& pol)
{
   ...
   typedef typename detail::bessel_traits<T1, T2, Policy>::result_type result_type;
   typedef typename policies::evaluation<result_type, Policy>::type value_type;
   return policies::checked_narrowing_cast<result_type, Policy>(detail::cyl_bessel_i_imp<value_type>(v, static_cast<value_type>(x), pol), "streams_boost::math::cyl_bessel_i<%1%>(%1%,%1%)");
}
...
inline typename detail::bessel_traits<T1, T2, Policy>::result_type cyl_bessel_k(T1 v, T2 x, const Policy& pol)
{
   ...
   typedef typename detail::bessel_traits<T1, T2, Policy>::optimisation_tag tag_type;
   typedef typename policies::evaluation<result_type, Policy>::type value_type;
   return policies::checked_narrowing_cast<result_type, Policy>(detail::cyl_bessel_k_imp<value_type>(v, static_cast<value_type>(x), tag_type(), pol), "streams_boost::math::cyl_bessel_k<%1%>(%1%,%1%)");
}
...
inline typename detail::bessel_traits<T1, T2, Policy>::result_type cyl_neumann(T1 v, T2 x, const Policy& pol)
{
   ...
   typedef typename detail::bessel_traits<T1, T2, Policy>::optimisation_tag tag_type;
   typedef typename policies::evaluation<result_type, Policy>::type value_type;
   return policies::checked_narrowing_cast<result_type, Policy>(detail::cyl_neumann_imp<value_type>(v, static_cast<value_type>(x), tag_type(), pol), "streams_boost::math::cyl_neumann<%1%>(%1%,%1%)");
}
...
inline typename detail::bessel_traits<T, T, Policy>::result_type sph_neumann(unsigned v, T x, const Policy& pol)
{
   ...
   typedef typename detail::bessel_traits<T, T, Policy>::result_type result_type;
   typedef typename policies::evaluation<result_type, Policy>::type value_type;
   return policies::checked_narrowing_cast<result_type, Policy>(detail::sph_neumann_imp<value_type>(v, static_cast<value_type>(x), pol), "streams_boost::math::sph_neumann<%1%>(%1%,%1%)");
}
...
#endif // STREAMS_BOOST_MATH_BESSEL_HPP
