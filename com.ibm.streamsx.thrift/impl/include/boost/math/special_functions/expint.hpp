...
#ifndef STREAMS_BOOST_MATH_EXPINT_HPP
#define STREAMS_BOOST_MATH_EXPINT_HPP
...
#include <streams_boost/math/special_functions/digamma.hpp>
...
T expint_1_rational(const T& z, const mpl::int_<53>&)
{
   ...
   else if(z < -streams_boost::math::tools::log_min_value<T>())
   {
...
{
   STREAMS_BOOST_MATH_STD_USING
   STREAMS_BOOST_MATH_INSTRUMENT_VARIABLE(z)
   ...
   T result = tools::continued_fraction_b(
      f, 
      streams_boost::math::policies::get_epsilon<T, Policy>(),
      max_iter);
   policies::check_series_iterations("streams_boost::math::expint_continued_fraction<%1%>(unsigned,%1%)", max_iter, pol);
   STREAMS_BOOST_MATH_INSTRUMENT_VARIABLE(result)
   STREAMS_BOOST_MATH_INSTRUMENT_VARIABLE(max_iter)
   result = exp(-z) / result;
   STREAMS_BOOST_MATH_INSTRUMENT_VARIABLE(result)
   return result;
}
...
inline T expint_as_series(unsigned n, T z, const Policy& pol)
{
   ...
   result += pow(-z, static_cast<T>(n - 1)) 
      * (streams_boost::math::digamma(static_cast<T>(n)) - log(z)) / fact;
   STREAMS_BOOST_MATH_INSTRUMENT_VARIABLE(result)

   expint_series<T> s(k, z, x_k, denom, fact);
   result = tools::sum_series(s, policies::get_epsilon<T, Policy>(), max_iter, result);
   policies::check_series_iterations("streams_boost::math::expint_series<%1%>(unsigned,%1%)", max_iter, pol);
   STREAMS_BOOST_MATH_INSTRUMENT_VARIABLE(result)
   STREAMS_BOOST_MATH_INSTRUMENT_VARIABLE(max_iter)
   return result;
}
...
{
   STREAMS_BOOST_MATH_STD_USING
   static const char* function = "streams_boost::math::expint<%1%>(unsigned, %1%)";
   if(z < 0)
...
T expint_i_as_series(T z, const Policy& pol)
{
   ...
   streams_boost::uintmax_t max_iter = policies::get_max_series_iterations<Policy>();
   result = tools::sum_series(s, policies::get_epsilon<T, Policy>(), max_iter, result);
   policies::check_series_iterations("streams_boost::math::expint_i_series<%1%>(%1%)", max_iter, pol);
   return result;
}
...
{
   static const char* function = "streams_boost::math::expint<%1%>(%1%)";
   if(z < 0)
...
T expint_i_imp(T z, const Policy& pol, const mpl::int_<53>& tag)
{
   ...
   if(z <= 6)
   {
      ...
      if(fabs(t) < 0.1)
      {
         result += streams_boost::math::log1p(t / r);
      }
      else
      {
         result += log(z / r);
   ...
   expint_forwarder(T z, const Policy& /*pol*/, mpl::true_ const&)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::expint_i_imp(
      ...
      forwarding_policy(),
      tag_type()), "streams_boost::math::expint<%1%>(%1%)");
}
...
{
   return streams_boost::math::expint(n, z, policies::policy<>());
}
   ...
   expint(unsigned n, T z, const Policy& /*pol*/)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::expint_imp(
      ...
      static_cast<value_type>(z),
      forwarding_policy(),
      tag_type()), "streams_boost::math::expint<%1%>(unsigned, %1%)");
}
...
#endif // STREAMS_BOOST_MATH_EXPINT_HPP
