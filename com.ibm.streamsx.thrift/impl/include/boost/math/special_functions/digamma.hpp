...
#ifndef STREAMS_BOOST_MATH_SF_DIGAMMA_HPP
#define STREAMS_BOOST_MATH_SF_DIGAMMA_HPP
...
T digamma_imp(T x, const Tag* t, const Policy& pol)
{
   ...
   if(x < 0)
   {
      ...
      if(remainder == 0)
      {
         return policies::raise_pole_error<T>("streams_boost::math::digamma<%1%>(%1%)", 0, (1-x), pol);
      }
   ...
   digamma(T x, const Policy& pol)
{
   ...
   return policies::checked_narrowing_cast<result_type, Policy>(detail::digamma_imp(
      ...
      static_cast<const tag_type*>(0), pol), "streams_boost::math::digamma<%1%>(%1%)");
}
