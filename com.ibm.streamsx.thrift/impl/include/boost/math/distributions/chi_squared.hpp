...
#ifndef STREAMS_BOOST_MATH_DISTRIBUTIONS_CHI_SQUARED_HPP
#define STREAMS_BOOST_MATH_DISTRIBUTIONS_CHI_SQUARED_HPP

#include <streams_boost/math/distributions/fwd.hpp>
#include <streams_boost/math/special_functions/gamma.hpp> // for incomplete beta.
...
public:
   ...
   chi_squared_distribution(RealType i) : m_df(i)
   {
      RealType result;
      detail::check_df(
         "streams_boost::math::chi_squared_distribution<%1%>::chi_squared_distribution", m_df, &result, Policy());
...
RealType pdf(const chi_squared_distribution<RealType, Policy>& dist, const RealType& chi_square)
{
   ...
   static const char* function = "streams_boost::math::pdf(const chi_squared_distribution<%1%>&, %1%)";

   if(false == detail::check_df(
         function, degrees_of_freedom, &error_result, Policy()))
      return error_result;

   if((chi_square < 0) || !(streams_boost::math::isfinite)(chi_square))
   {
...
inline RealType cdf(const chi_squared_distribution<RealType, Policy>& dist, const RealType& chi_square)
{
   ...
   // Error check:
   RealType error_result;
   static const char* function = "streams_boost::math::cdf(const chi_squared_distribution<%1%>&, %1%)";
   ...
   return streams_boost::math::gamma_p(degrees_of_freedom / 2, chi_square / 2, Policy());
} // cdf
...
{
   RealType degrees_of_freedom = dist.degrees_of_freedom();
   static const char* function = "streams_boost::math::quantile(const chi_squared_distribution<%1%>&, %1%)";
   // Error check:
   RealType error_result;
   if(false == detail::check_df(
         function, degrees_of_freedom, &error_result, Policy())
   ...
   return 2 * streams_boost::math::gamma_p_inv(degrees_of_freedom / 2, p, Policy());
} // quantile
...
inline RealType cdf(const complemented2_type<chi_squared_distribution<RealType, Policy>, RealType>& c)
{
   ...
   return streams_boost::math::gamma_q(degrees_of_freedom / 2, chi_square / 2, Policy());
}
...
inline RealType quantile(const complemented2_type<chi_squared_distribution<RealType, Policy>, RealType>& c)
{
   ...
   return 2 * streams_boost::math::gamma_q_inv(degrees_of_freedom / 2, q, Policy());
}
...
inline RealType mode(const chi_squared_distribution<RealType, Policy>& dist)
{
   ...
   static const char* function = "streams_boost::math::mode(const chi_squared_distribution<%1%>&)";
...
//         STREAMS_BOOST_CURRENT_FUNCTION,
...
{
   STREAMS_BOOST_MATH_STD_USING // For ADL
   RealType df = dist.degrees_of_freedom();
...
RealType chi_squared_distribution<RealType, Policy>::find_degrees_of_freedom(
   ...
   RealType hint)
{
   static const char* function = "streams_boost::math::chi_squared_distribution<%1%>::find_degrees_of_freedom(%1%,%1%,%1%,%1%,%1%)";
...
#endif // STREAMS_BOOST_MATH_DISTRIBUTIONS_CHI_SQUARED_HPP
