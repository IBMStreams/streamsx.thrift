...
#ifndef STREAMS_BOOST_STATS_rayleigh_HPP
#define STREAMS_BOOST_STATS_rayleigh_HPP
...
public:
   ...
   {
      RealType err;
      detail::verify_sigma("streams_boost::math::rayleigh_distribution<%1%>::rayleigh_distribution", sigma, &err, Policy());
...
inline RealType pdf(const rayleigh_distribution<RealType, Policy>& dist, const RealType& x)
{
   ...
   RealType result;
   static const char* function = "streams_boost::math::pdf(const rayleigh_distribution<%1%>&, %1%)";
...
inline RealType cdf(const rayleigh_distribution<RealType, Policy>& dist, const RealType& x)
{
   ...
   RealType result;
   RealType sigma = dist.sigma();
   static const char* function = "streams_boost::math::cdf(const rayleigh_distribution<%1%>&, %1%)";
   if(false == detail::verify_sigma(function, sigma, &result, Policy()))
   {
      return result;
   }
   ...
   result = -streams_boost::math::expm1(-x * x / ( 2 * sigma * sigma), Policy());
   return result;
} // cdf
...
inline RealType quantile(const rayleigh_distribution<RealType, Policy>& dist, const RealType& p)
{
   ...
   RealType result;
   RealType sigma = dist.sigma();
   static const char* function = "streams_boost::math::quantile(const rayleigh_distribution<%1%>&, %1%)";
   if(false == detail::verify_sigma(function, sigma, &result, Policy()))
      return result;
   ...
   result = sqrt(-2 * sigma * sigma * streams_boost::math::log1p(-p, Policy()));
   return result;
} // quantile
...
{
   STREAMS_BOOST_MATH_STD_USING // for ADL of std functions, log & sqrt.
...
inline RealType mean(const rayleigh_distribution<RealType, Policy>& dist)
{
   RealType result;
   RealType sigma = dist.sigma();
   static const char* function = "streams_boost::math::mean(const rayleigh_distribution<%1%>&, %1%)";
   if(false == detail::verify_sigma(function, sigma, &result, Policy()))
   {
      return result;
   }
   using streams_boost::math::constants::root_half_pi;
...
inline RealType variance(const rayleigh_distribution<RealType, Policy>& dist)
{
   RealType result;
   RealType sigma = dist.sigma();
   static const char* function = "streams_boost::math::variance(const rayleigh_distribution<%1%>&, %1%)";
   if(false == detail::verify_sigma(function, sigma, &result, Policy()))
   {
      return result;
   }
   using streams_boost::math::constants::four_minus_pi;
...
{
   using streams_boost::math::constants::root_ln_four;
...
#endif // STREAMS_BOOST_STATS_rayleigh_HPP
