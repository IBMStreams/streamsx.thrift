...
#ifndef STREAMS_BOOST_STATS_PARETO_HPP
#define STREAMS_BOOST_STATS_PARETO_HPP
...
#include <utility> // for STREAMS_BOOST_CURRENT_VALUE?
  ...
  namespace math
  {
    ...
    { // Parameter checking.
      ...
      inline bool check_pareto_shape(
        ...
        RealType* result, const Policy& pol)
      {
        if((streams_boost::math::isfinite)(shape))
    ...
    public:
      ...
      { // Constructor.
        RealType result;
        detail::check_pareto("streams_boost::math::pareto_distribution<%1%>::pareto_distribution", scale, shape, &result, Policy());
      }
    ...
    inline RealType pdf(const pareto_distribution<RealType, Policy>& dist, const RealType& x)
    {
      ...
      static const char* function = "streams_boost::math::pdf(const pareto_distribution<%1%>&, %1%)";
    ...
    {
      STREAMS_BOOST_MATH_STD_USING  // for ADL of std function pow.
      static const char* function = "streams_boost::math::cdf(const pareto_distribution<%1%>&, %1%)";
      RealType scale = dist.scale();
      RealType shape = dist.shape();
      RealType result;
      ...
      result = -streams_boost::math::powm1(scale/x, shape, Policy()); // should be more accurate.
      return result;
    } // cdf
    ...
    {
      STREAMS_BOOST_MATH_STD_USING  // for ADL of std function pow.
      static const char* function = "streams_boost::math::quantile(const pareto_distribution<%1%>&, %1%)";
      RealType result;
      RealType scale = dist.scale();
      RealType shape = dist.shape();
    ...
    {
      RealType result;
      static const char* function = "streams_boost::math::mean(const pareto_distribution<%1%>&, %1%)";
    ...
    {
      RealType result;
      static const char* function = "streams_boost::math::median(const pareto_distribution<%1%>&, %1%)";
      if(false == detail::check_pareto(function, dist.scale(), dist.shape(), &result, Policy()))
      {
        return result;
      }
    ...
    inline RealType variance(const pareto_distribution<RealType, Policy>& dist)
    {
      ...
      RealType scale = dist.scale();
      RealType shape = dist.shape();
      static const char* function = "streams_boost::math::variance(const pareto_distribution<%1%>&, %1%)";
...
#endif // STREAMS_BOOST_STATS_PARETO_HPP
