...
#ifndef STREAMS_BOOST_STATS_NORMAL_HPP
#define STREAMS_BOOST_STATS_NORMAL_HPP
...
#include <streams_boost/math/special_functions/erf.hpp> // for erf/erfc.
...
public:
   ...
   { // Default is a 'standard' normal distribution N01.
     static const char* function = "streams_boost::math::normal_distribution<%1%>::normal_distribution";
...
inline RealType pdf(const normal_distribution<RealType, Policy>& dist, const RealType& x)
{
   ...
   static const char* function = "streams_boost::math::pdf(const normal_distribution<%1%>&, %1%)";
   if((streams_boost::math::isinf)(x))
   {
...
inline RealType cdf(const normal_distribution<RealType, Policy>& dist, const RealType& x)
{
   ...
   RealType sd = dist.standard_deviation();
   RealType mean = dist.mean();
   static const char* function = "streams_boost::math::cdf(const normal_distribution<%1%>&, %1%)";
   RealType result;
   if(false == detail::check_scale(function, sd, &result, Policy()))
   {
      return result;
   ...
   result = streams_boost::math::erfc(-diff, Policy()) / 2;
   return result;
} // cdf
...
inline RealType quantile(const normal_distribution<RealType, Policy>& dist, const RealType& p)
{
   ...
   RealType sd = dist.standard_deviation();
   RealType mean = dist.mean();
   static const char* function = "streams_boost::math::quantile(const normal_distribution<%1%>&, %1%)";
   ...
   result= streams_boost::math::erfc_inv(2 * p, Policy());
   result = -result;
...
inline RealType cdf(const complemented2_type<normal_distribution<RealType, Policy>, RealType>& c)
{
   ...
   RealType x = c.param;
   static const char* function = "streams_boost::math::cdf(const complement(normal_distribution<%1%>&), %1%)";
...
inline RealType quantile(const complemented2_type<normal_distribution<RealType, Policy>, RealType>& c)
{
   ...
   RealType sd = c.dist.standard_deviation();
   RealType mean = c.dist.mean();
   static const char* function = "streams_boost::math::quantile(const complement(normal_distribution<%1%>&), %1%)";
   RealType result;
   if(false == detail::check_scale(function, sd, &result, Policy()))
      return result;
   if(false == detail::check_location(function, mean, &result, Policy()))
   ...
   if(false == detail::check_probability(function, q, &result, Policy()))
      return result;
   result = streams_boost::math::erfc_inv(2 * q, Policy());
   result *= sd * constants::root_two<RealType>();
   result += mean;
   return result;
} // quantile
...
#endif // STREAMS_BOOST_STATS_NORMAL_HPP
