// streams_boost\math\distributions\non_central_chi_squared.hpp
...
#ifndef STREAMS_BOOST_MATH_SPECIAL_NON_CENTRAL_CHI_SQUARE_HPP
#define STREAMS_BOOST_MATH_SPECIAL_NON_CENTRAL_CHI_SQUARE_HPP

#include <streams_boost/math/distributions/fwd.hpp>
#include <streams_boost/math/special_functions/gamma.hpp> // for incomplete gamma. gamma_q
#include <streams_boost/math/special_functions/bessel.hpp> // for cyl_bessel_i
#include <streams_boost/math/special_functions/round.hpp> // for iround
#include <streams_boost/math/distributions/complement.hpp> // complements
#include <streams_boost/math/distributions/chi_squared.hpp> // central distribution
#include <streams_boost/math/distributions/detail/common_error_handling.hpp> // error checks
#include <streams_boost/math/special_functions/fpclassify.hpp> // isnan.
#include <streams_boost/math/tools/roots.hpp> // for root finding.
#include <streams_boost/math/distributions/detail/generic_mode.hpp>
#include <streams_boost/math/distributions/detail/generic_quantile.hpp>
   ...
   namespace math
   {
      ...
      namespace detail{
         ...
         T non_central_chi_square_q(T x, T f, T theta, const Policy& pol, T init_sum = 0)
         {
            ...
            T errtol = streams_boost::math::policies::get_epsilon<T, Policy>();
            T sum = init_sum;
            //
            ...
            T poisf = streams_boost::math::gamma_p_derivative(1 + k, lambda, pol);
            T poisb = poisf * k / lambda;
            // Initial forwards central chi squared term:
            T gamf = streams_boost::math::gamma_q(del + k, y, pol);
            // Forwards and backwards recursion terms on the central chi squared:
            T xtermf = streams_boost::math::gamma_p_derivative(del + 1 + k, y, pol);
            ...
            //
            int i;
            for(i = k; static_cast<streams_boost::uintmax_t>(i-k) < max_iter; ++i)
            {
               T term = poisf * gamf;
               sum += term;
               poisf *= lambda / (i + 1);
            ...
            //Error check:
            if(static_cast<streams_boost::uintmax_t>(i-k) >= max_iter)
         ...
         T non_central_chi_square_p_ding(T x, T f, T theta, const Policy& pol, T init_sum = 0)
         {
            ...
            if(x == 0)
               return 0;
            T tk = streams_boost::math::gamma_p_derivative(f/2 + 1, x/2, pol);
            T lambda = theta / 2;
            T vk = exp(-lambda);
            T uk = vk;
            ...
            int i;
            T lterm(0), term(0);
            for(i = 1; static_cast<streams_boost::uintmax_t>(i) < max_iter; ++i)
            {
               tk = tk * x / (f + 2 * i);
               uk = uk * lambda / i;
               vk = vk + uk;
            ...
            //Error check:
            if(static_cast<streams_boost::uintmax_t>(i) >= max_iter)
               policies::raise_evaluation_error(
                  "cdf(non_central_chi_squared_distribution<%1%>, %1%)", 
                  "Series did not converge, closest value was %1%", sum, pol);
            return sum;
         ...
         T non_central_chi_square_p(T y, T n, T lambda, const Policy& pol, T init_sum)
         {
            ...
            T gamkf = streams_boost::math::gamma_p(a, x, pol);
            ...
            T xtermf = streams_boost::math::gamma_p_derivative(a, x, pol);
            ...
            do
            {
               ...
               sum += errorf;
               ++i;
            }while((fabs(errorf / sum) > errtol) && (static_cast<streams_boost::uintmax_t>(i) < max_iter));
         ...
         inline RealType non_central_chi_squared_cdf(RealType x, RealType k, RealType l, bool invert, const Policy&)
         {
            ...
            value_type result;
            if(l == 0)
               result = cdf(streams_boost::math::chi_squared_distribution<RealType, Policy>(k), x);
            else if(x > k + l)
            {
            ...
            return policies::checked_narrowing_cast<RealType, forwarding_policy>(
               result, 
               "streams_boost::math::non_central_chi_squared_cdf<%1%>(%1%, %1%, %1%)");
         }
         ...
         STREAMS_BOOST_MATH_STD_USING
         if(l == 0)
            return pdf(streams_boost::math::chi_squared_distribution<RealType, forwarding_policy>(dist.degrees_of_freedom()), x);
         ...
         else
         {
            ...
            else
            {
               r = exp(r);
               r = 0.5f * r
                  * streams_boost::math::cyl_bessel_i(k/2 - 1, sqrt(l * x), forwarding_policy());
            }
         }
         return policies::checked_narrowing_cast<RealType, forwarding_policy>(
               r, 
      ...
      public:
         ...
         { 
            const char* function = "streams_boost::math::non_central_chi_squared_distribution<%1%>::non_central_chi_squared_distribution(%1%,%1%)";
            RealType r;
            detail::check_df(
               function,
               df, &r, Policy());
      ...
      { // Range of permissible values for random variable k.
         using streams_boost::math::tools::max_value;
      ...
      { // Mean of poisson distribution = lambda.
         const char* function = "streams_boost::math::non_central_chi_squared_distribution<%1%>::mean()";
      ...
      { // variance.
         const char* function = "streams_boost::math::non_central_chi_squared_distribution<%1%>::variance()";
         RealType k = dist.degrees_of_freedom();
         RealType l = dist.non_centrality();
         RealType r;
         if(!detail::check_df(
      ...
      { // skewness = sqrt(l).
         const char* function = "streams_boost::math::non_central_chi_squared_distribution<%1%>::skewness()";
         RealType k = dist.degrees_of_freedom();
         RealType l = dist.non_centrality();
         RealType r;
         if(!detail::check_df(
      ...
      { 
         const char* function = "streams_boost::math::non_central_chi_squared_distribution<%1%>::kurtosis_excess()";
         RealType k = dist.degrees_of_freedom();
         RealType l = dist.non_centrality();
         RealType r;
         if(!detail::check_df(
      ...
      { 
         const char* function = "streams_boost::math::non_central_chi_squared_distribution<%1%>::cdf(%1%)";
         RealType k = dist.degrees_of_freedom();
         RealType l = dist.non_centrality();
         RealType r;
         if(!detail::check_df(
...
#include <streams_boost/math/distributions/detail/derived_accessors.hpp>

#endif // STREAMS_BOOST_MATH_SPECIAL_NON_CENTRAL_CHI_SQUARE_HPP
