...
#ifndef STREAMS_BOOST_STATS_STUDENTS_T_HPP
#define STREAMS_BOOST_STATS_STUDENTS_T_HPP

// http://en.wikipedia.org/wiki/Student%27s_t_distribution
// http://www.itl.nist.gov/div898/handbook/eda/section3/eda3664.htm

#include <streams_boost/math/distributions/fwd.hpp>
#include <streams_boost/math/special_functions/beta.hpp> // for ibeta(a, b, x).
...
public:
   ...
   { // Constructor.
      RealType result;
      detail::check_df(
         "streams_boost::math::students_t_distribution<%1%>::students_t_distribution", m_df, &result, Policy());
...
inline RealType pdf(const students_t_distribution<RealType, Policy>& dist, const RealType& t)
{
   ...
   if(false == detail::check_df(
      "streams_boost::math::pdf(const students_t_distribution<%1%>&, %1%)", degrees_of_freedom, &error_result, Policy()))
      return error_result;
   ...
   if(basem1 < 0.125)
   {
      result = exp(-streams_boost::math::log1p(basem1, Policy()) * (1+degrees_of_freedom) / 2);
   }
   else
   {
      result = pow(1 / (1 + basem1), (degrees_of_freedom + 1) / 2);
   }
   result /= sqrt(degrees_of_freedom) * streams_boost::math::beta(degrees_of_freedom / 2, RealType(0.5f), Policy());
   return result;
} // pdf
...
inline RealType cdf(const students_t_distribution<RealType, Policy>& dist, const RealType& t)
{
   ...
   RealType error_result;
   if(false == detail::check_df(
      "streams_boost::math::cdf(const students_t_distribution<%1%>&, %1%)", degrees_of_freedom, &error_result, Policy()))
      return error_result;
...
inline RealType quantile(const students_t_distribution<RealType, Policy>& dist, const RealType& p)
{
   ...
   //
   static const char* function = "streams_boost::math::quantile(const students_t_distribution<%1%>&, %1%)";
   RealType error_result;
   if(false == detail::check_df(
   ...
   //
   return streams_boost::math::detail::fast_students_t_quantile(degrees_of_freedom, probability, Policy());
} // quantile
...
RealType students_t_distribution<RealType, Policy>::find_degrees_of_freedom(
      ...
      RealType hint)
{
   static const char* function = "streams_boost::math::students_t_distribution<%1%>::find_degrees_of_freedom";
   //
   // Check for domain errors:
   //
   RealType error_result;
...
inline RealType variance(const students_t_distribution<RealType, Policy>& dist)
{
   ...
   RealType error_result;
   if(false == detail::check_df(
      "streams_boost::math::variance(students_t_distribution<%1%> const&, %1%)", dist.degrees_of_freedom(), &error_result, Policy()))
      return error_result;
...
inline RealType skewness(const students_t_distribution<RealType, Policy>& dist)
{
   if(dist.degrees_of_freedom() <= 3)
   {
      policies::raise_domain_error<RealType>(
         "streams_boost::math::skewness(students_t_distribution<%1%> const&, %1%)",
...
inline RealType kurtosis(const students_t_distribution<RealType, Policy>& dist)
{
   ...
   {
      policies::raise_domain_error<RealType>(
         "streams_boost::math::kurtosis(students_t_distribution<%1%> const&, %1%)",
         "Skewness is undefined for degrees of freedom <= 3, but got %1%.",
         df, Policy());
   }
   return 3 * (df - 2) / (df - 4);
...
inline RealType kurtosis_excess(const students_t_distribution<RealType, Policy>& dist)
{
   ...
   {
      policies::raise_domain_error<RealType>(
         "streams_boost::math::kurtosis_excess(students_t_distribution<%1%> const&, %1%)",
         "Skewness is undefined for degrees of freedom <= 3, but got %1%.",
         df, Policy());
   }
   return 6 / (df - 4);
