...
#ifndef STREAMS_BOOST_MATH_SPECIAL_NON_CENTRAL_T_HPP
#define STREAMS_BOOST_MATH_SPECIAL_NON_CENTRAL_T_HPP

#include <streams_boost/math/distributions/fwd.hpp>
#include <streams_boost/math/distributions/non_central_beta.hpp> // for nc beta
#include <streams_boost/math/distributions/normal.hpp> // for normal CDF and quantile
#include <streams_boost/math/distributions/students_t.hpp>
#include <streams_boost/math/distributions/detail/generic_quantile.hpp> // quantile
   ...
   namespace math
   {
      ...
      namespace detail{
         ...
         T non_central_t2_p(T n, T delta, T x, T y, const Policy& pol, T init_val)
         {
            ...
            //
            int k = streams_boost::math::itrunc(d2);
         ...
         T non_central_t_cdf(T n, T delta, T t, bool invert, const Policy& pol)
         {
            ...
            if(x < cross)
            {
               ...
               if(x != 0)
               {
                  ...
                  result = 0;
               result += cdf(streams_boost::math::normal_distribution<T, Policy>(), -delta);
            }
            else
            {
               //
               ...
               if(x != 0)
               {
                  ...
                  result = cdf(complement(streams_boost::math::normal_distribution<T, Policy>(), -delta));
            }
            if(invert)
               result = 1 - result;
            return result;
         ...
         T non_central_t_quantile(T v, T delta, T p, T q, const Policy&)
         {
            ...
            int s;
            if(p < q)
               s = streams_boost::math::sign(p - pzero);
            else
               s = streams_boost::math::sign(pzero - q);
            if(s != streams_boost::math::sign(guess))
            {
               guess = s;
            }
         ...
         {
            STREAMS_BOOST_MATH_STD_USING
            T mean = streams_boost::math::detail::mean(v, delta, pol);
            T l2 = delta * delta;
      ...
      public:
         ...
         { 
            const char* function = "streams_boost::math::non_central_t_distribution<%1%>::non_central_t_distribution(%1%,%1%)";
            RealType r;
            detail::check_df(
               function,
               v, &r, Policy());
      ...
      { 
         const char* function = "streams_boost::math::non_central_t_distribution<%1%>::cdf(%1%)";
         typedef typename policies::evaluation<RealType, Policy>::type value_type;
         typedef typename policies::normalise<
            Policy, 
            policies::promote_float<false>, 
...
#endif // STREAMS_BOOST_MATH_SPECIAL_NON_CENTRAL_T_HPP
