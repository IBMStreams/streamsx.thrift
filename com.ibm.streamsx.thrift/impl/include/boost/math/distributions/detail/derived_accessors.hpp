...
#ifndef STREAMS_BOOST_STATS_DERIVED_HPP
#define STREAMS_BOOST_STATS_DERIVED_HPP
...
{
   STREAMS_BOOST_MATH_STD_USING  // ADL of sqrt.
...
{ // hazard function
  // http://www.itl.nist.gov/div898/handbook/eda/section3/eda362.htm#HAZ
   ...
   if(d > p * tools::max_value<value_type>())
      ...
      "streams_boost::math::hazard(const Distribution&, %1%)", 0, policy_type());
   if(d == 0)
   {
...
inline typename Distribution::value_type coefficient_of_variation(const Distribution& dist)
{
   ...
   { // Checks too that m is not zero,
      return policies::raise_overflow_error<value_type>("streams_boost::math::coefficient_of_variation(const Distribution&, %1%)", 0, policy_type());
   }
...
#endif // STREAMS_BOOST_STATS_DERIVED_HPP
