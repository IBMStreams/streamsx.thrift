...
#ifndef STREAMS_BOOST_MATH_DISTRIBUTIONS_COMMON_ERROR_HANDLING_HPP
#define STREAMS_BOOST_MATH_DISTRIBUTIONS_COMMON_ERROR_HANDLING_HPP
...
{
   if((prob < 0) || (prob > 1) || !(streams_boost::math::isfinite)(prob))
   {
...
{
   if((df <= 0) || !(streams_boost::math::isfinite)(df))
   {
      *result = policies::raise_domain_error<RealType>(
         function,
...
inline bool check_scale(
      ...
      const Policy& pol)
{
   if((scale <= 0) || !(streams_boost::math::isfinite)(scale))
...
inline bool check_location(
      ...
      const Policy& pol)
{
   if(!(streams_boost::math::isfinite)(location))
   {
      *result = policies::raise_domain_error<RealType>(
         function,
...
inline bool check_positive_x(
      ...
      const Policy& pol)
{
   if(!(streams_boost::math::isfinite)(x) || (x < 0))
   {
      *result = policies::raise_domain_error<RealType>(
         function,
...
inline bool check_non_centrality(
      ...
      const Policy& pol)
{
   if((ncp < 0) || !(streams_boost::math::isfinite)(ncp))
   { // Assume scale == 0 is NOT valid for any distribution.
      *result = policies::raise_domain_error<RealType>(
         function,
...
#endif // STREAMS_BOOST_MATH_DISTRIBUTIONS_COMMON_ERROR_HANDLING_HPP
