...
#ifndef STREAMS_BOOST_MATH_DISTRIBUTIONS_DETAIL_HG_PDF_HPP
#define STREAMS_BOOST_MATH_DISTRIBUTIONS_DETAIL_HG_PDF_HPP
...
#include <streams_boost/math/special_functions/pow.hpp>
#include <streams_boost/math/special_functions/prime.hpp>
...
T hypergeometric_pdf_lanczos_imp(T /*dummy*/, unsigned x, unsigned r, unsigned n, unsigned N, const Lanczos&, const Policy&)
{
   ...
   STREAMS_BOOST_MATH_INSTRUMENT_FPU
   for(unsigned i = 0; i < 9; ++i)
   {
      STREAMS_BOOST_MATH_INSTRUMENT_VARIABLE(i);
      STREAMS_BOOST_MATH_INSTRUMENT_VARIABLE(bases[i]);
      STREAMS_BOOST_MATH_INSTRUMENT_VARIABLE(exponents[i]);
      STREAMS_BOOST_MATH_INSTRUMENT_VARIABLE(base_e_factors[i]);
      STREAMS_BOOST_MATH_INSTRUMENT_VARIABLE(sorted_indexes[i]);
   }
#endif

   T result;
   STREAMS_BOOST_MATH_INSTRUMENT_VARIABLE(bases[sorted_indexes[0]] * exp(static_cast<T>(base_e_factors[sorted_indexes[0]])));
   STREAMS_BOOST_MATH_INSTRUMENT_VARIABLE(exponents[sorted_indexes[0]]);
   {
...
T hypergeometric_pdf_lanczos_imp(T /*dummy*/, unsigned x, unsigned r, unsigned n, unsigned N, const streams_boost::math::lanczos::undefined_lanczos&, const Policy& pol)
{
   STREAMS_BOOST_MATH_STD_USING
   return exp(
      streams_boost::math::lgamma(T(n + 1), pol)
      + streams_boost::math::lgamma(T(r + 1), pol)
      + streams_boost::math::lgamma(T(N - n + 1), pol)
      + streams_boost::math::lgamma(T(N - r + 1), pol)
      - streams_boost::math::lgamma(T(N + 1), pol)
      - streams_boost::math::lgamma(T(x + 1), pol)
      - streams_boost::math::lgamma(T(n - x + 1), pol)
      - streams_boost::math::lgamma(T(r - x + 1), pol)
      - streams_boost::math::lgamma(T(N - n - r + x + 1), pol));
}
...
inline T integer_power(const T& x, int ex)
{
   ...
   case 3:
      return x * x * x;
   case 4:
      return streams_boost::math::pow<4>(x);
   case 5:
      return streams_boost::math::pow<5>(x);
   case 6:
      return streams_boost::math::pow<6>(x);
   case 7:
      return streams_boost::math::pow<7>(x);
   case 8:
      return streams_boost::math::pow<8>(x);
   }
   STREAMS_BOOST_MATH_STD_USING
#ifdef __SUNPRO_CC
   return pow(x, T(ex));
...
{
   STREAMS_BOOST_MATH_STD_USING
   STREAMS_BOOST_ASSERT(N < streams_boost::math::max_factorial<T>::value);
   T result = streams_boost::math::unchecked_factorial<T>(n);
   T num[3] = {
      streams_boost::math::unchecked_factorial<T>(r),
      streams_boost::math::unchecked_factorial<T>(N - n),
      streams_boost::math::unchecked_factorial<T>(N - r)
   };
   T denom[5] = {
      streams_boost::math::unchecked_factorial<T>(N),
      streams_boost::math::unchecked_factorial<T>(x),
      streams_boost::math::unchecked_factorial<T>(n - x),
      streams_boost::math::unchecked_factorial<T>(r - x),
      streams_boost::math::unchecked_factorial<T>(N - n - r + x)
   };
   int i = 0;
   int j = 0;
   while((i < 3) || (j < 5))
   ...
   hypergeometric_pdf(unsigned x, unsigned r, unsigned n, unsigned N, const Policy&)
{
   ...
   value_type result;
   if(N <= streams_boost::math::max_factorial<value_type>::value)
   {
      //
   ...
   else if(N <= streams_boost::math::prime(streams_boost::math::max_prime - 1))
   {
      //
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(result, "streams_boost::math::hypergeometric_pdf<%1%>(%1%,%1%,%1%,%1%)");
}
