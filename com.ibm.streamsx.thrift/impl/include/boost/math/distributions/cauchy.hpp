...
#ifndef STREAMS_BOOST_STATS_CAUCHY_HPP
#define STREAMS_BOOST_STATS_CAUCHY_HPP
...
RealType cdf_imp(const cauchy_distribution<RealType, Policy>& dist, const RealType& x, bool complement)
{
   ...
   static const char* function = "streams_boost::math::cdf(cauchy<%1%>&, %1%)";
   RealType result;
      ...
      bool complement)
{
   ...
   //
   static const char* function = "streams_boost::math::quantile(cauchy<%1%>&, %1%)";
   STREAMS_BOOST_MATH_STD_USING // for ADL of std functions
...
public:
   ...
   {
    static const char* function = "streams_boost::math::cauchy_distribution<%1%>::cauchy_distribution";
     RealType result;
...
{  
   ...
   static const char* function = "streams_boost::math::pdf(cauchy<%1%>&, %1%)";
   RealType result;
   RealType location = dist.location();
   RealType scale = dist.scale();
   if(false == detail::check_scale("streams_boost::math::pdf(cauchy<%1%>&, %1%)", scale, &result, Policy()))
   {
      return result;
   }
   if(false == detail::check_location("streams_boost::math::pdf(cauchy<%1%>&, %1%)", location, &result, Policy()))
   {
      return result;
   }
...
{  // There is no mean:
   ...
   STREAMS_BOOST_STATIC_ASSERT(assert_type::value == 0);

   return policies::raise_domain_error<RealType>(
      "streams_boost::math::mean(cauchy<%1%>&)",
...
inline RealType variance(const cauchy_distribution<RealType, Policy>& /*dist*/)
{
   ...
   return policies::raise_domain_error<RealType>(
      "streams_boost::math::variance(cauchy<%1%>&)",
...
inline RealType skewness(const cauchy_distribution<RealType, Policy>& /*dist*/)
{
   ...
   return policies::raise_domain_error<RealType>(
      "streams_boost::math::skewness(cauchy<%1%>&)",
...
inline RealType kurtosis(const cauchy_distribution<RealType, Policy>& /*dist*/)
{
   ...
   return policies::raise_domain_error<RealType>(
      "streams_boost::math::kurtosis(cauchy<%1%>&)",
...
inline RealType kurtosis_excess(const cauchy_distribution<RealType, Policy>& /*dist*/)
{
   ...
   return policies::raise_domain_error<RealType>(
      "streams_boost::math::kurtosis_excess(cauchy<%1%>&)",
      "The Cauchy distribution does not have a kurtosis: "
      "the only possible return value is %1%.",
      std::numeric_limits<RealType>::quiet_NaN(), Policy());
}
...
#endif // STREAMS_BOOST_STATS_CAUCHY_HPP
