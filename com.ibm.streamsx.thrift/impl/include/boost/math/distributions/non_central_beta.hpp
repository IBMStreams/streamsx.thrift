...
#ifndef STREAMS_BOOST_MATH_SPECIAL_NON_CENTRAL_BETA_HPP
#define STREAMS_BOOST_MATH_SPECIAL_NON_CENTRAL_BETA_HPP

#include <streams_boost/math/distributions/fwd.hpp>
#include <streams_boost/math/special_functions/beta.hpp> // for incomplete gamma. gamma_q
#include <streams_boost/math/distributions/complement.hpp> // complements
#include <streams_boost/math/distributions/beta.hpp> // central distribution
...
namespace streams_boost
{
   namespace math
   {
      ...
      namespace detail{
         ...
         T non_central_beta_p(T a, T b, T lam, T x, T y, const Policy& pol, T init_val = 0)
         {
            STREAMS_BOOST_MATH_STD_USING
               using namespace streams_boost::math;
            //
            ...
            //
            T last_term = 0;
            streams_boost::uintmax_t count = k;
            for(int i = k; i >= 0; --i)
            {
               T term = beta * pois;
               sum += term;
            ...
            for(int i = k + 1; ; ++i)
            {
               ...
               if(static_cast<streams_boost::uintmax_t>(count + i - k) > max_iter)
               {
         ...
         T non_central_beta_q(T a, T b, T lam, T x, T y, const Policy& pol, T init_val = 0)
         {
            ...
            //
            T last_term = 0;
            streams_boost::uintmax_t count = 0;
            for(int i = k + 1; ; ++i)
            {
               poisf *= l2 / i;
               xtermf *= (x * (a + b + i - 2)) / (a + i - 1);
            ...
            for(int i = k; i >= 0; --i)
            {
               ...
               if(static_cast<streams_boost::uintmax_t>(count + k - i) > max_iter)
               {
                  return policies::raise_evaluation_error(
                     "cdf(non_central_beta_distribution<%1%>, %1%)", 
                     "Series did not converge, closest value was %1%", sum, pol);
         ...
         inline RealType non_central_beta_cdf(RealType x, RealType y, RealType a, RealType b, RealType l, bool invert, const Policy&)
         {
            ...
            if(l == 0)
               result = cdf(streams_boost::math::beta_distribution<RealType, Policy>(a, b), x);
            else if(x > cross)
            {
            ...
            return policies::checked_narrowing_cast<RealType, forwarding_policy>(
               result, 
               "streams_boost::math::non_central_beta_cdf<%1%>(%1%, %1%, %1%)");
         }
         ...
         std::pair<T, T> bracket_and_solve_root_01(F f, const T& guess, T factor, bool rising, Tol tol, streams_boost::uintmax_t& max_iter, const Policy& pol)
         {
            STREAMS_BOOST_MATH_STD_USING
               static const char* function = "streams_boost::math::tools::bracket_and_solve_root_01<%1%>";
            //
            ...
            //
            streams_boost::uintmax_t count = max_iter - 1;
            ...
            if((fa < 0) == (guess < 0 ? !rising : rising))
            {
               ...
               // until we find it:
               //
               while((streams_boost::math::sign)(fb) == (streams_boost::math::sign)(fa))
               {
                  if(count == 0)
                  {
                  ...
                  fb = f(b);
                  --count;
                  STREAMS_BOOST_MATH_INSTRUMENT_CODE("a = " << a << " b = " << b << " fa = " << fa << " fb = " << fb << " count = " << count);
               }
            }
            else
            {
            ...
            std::pair<T, T> r = toms748_solve(
               ...
               pol);
            max_iter += count;
            STREAMS_BOOST_MATH_INSTRUMENT_CODE("max_iter = " << max_iter << " count = " << count);
            return r;
         }
         ...
         RealType nc_beta_pdf(const non_central_beta_distribution<RealType, Policy>& dist, const RealType& x)
         {
            ...
            STREAMS_BOOST_MATH_STD_USING
            if(l == 0)
               return pdf(streams_boost::math::beta_distribution<RealType, Policy>(dist.alpha(), dist.beta()), x);
            return policies::checked_narrowing_cast<RealType, forwarding_policy>(
      ...
      public:
         ...
         { 
            const char* function = "streams_boost::math::non_central_beta_distribution<%1%>::non_central_beta_distribution(%1%,%1%)";
            RealType r;
      ...
      { // variance.
         const char* function = "streams_boost::math::non_central_beta_distribution<%1%>::variance()";
         // TODO
         return 0;
      }
      ...
      { // skewness = sqrt(l).
         const char* function = "streams_boost::math::non_central_beta_distribution<%1%>::skewness()";
         // TODO
         return 0;
      }
      ...
      { 
         const char* function = "streams_boost::math::non_central_beta_distribution<%1%>::kurtosis_excess()";
         // TODO
         return 0;
      } // kurtosis_excess
      ...
      { 
         const char* function = "streams_boost::math::non_central_beta_distribution<%1%>::cdf(%1%)";
            RealType a = dist.alpha();
            RealType b = dist.beta();
            RealType l = dist.non_centrality();
            RealType r;
...
#endif // STREAMS_BOOST_MATH_SPECIAL_NON_CENTRAL_BETA_HPP
