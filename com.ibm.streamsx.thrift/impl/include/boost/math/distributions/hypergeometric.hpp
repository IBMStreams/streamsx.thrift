...
#ifndef STREAMS_BOOST_MATH_DISTRIBUTIONS_HYPERGEOMETRIC_HPP
#define STREAMS_BOOST_MATH_DISTRIBUTIONS_HYPERGEOMETRIC_HPP
...
#include <streams_boost/math/distributions/detail/hypergeometric_cdf.hpp>
#include <streams_boost/math/distributions/detail/hypergeometric_quantile.hpp>
...
namespace streams_boost { namespace math {
   ...
   public:
      ...
      hypergeometric_distribution(unsigned r, unsigned n, unsigned N) // Constructor.
         : m_n(n), m_N(N), m_r(r)
      {
         static const char* function = "streams_boost::math::hypergeometric_distribution<%1%>::hypergeometric_distribution";
         RealType ret;
      ...
      bool check_params(const char* function, RealType* result)const
      {
         if(m_r > m_N)
         {
            *result = streams_boost::math::policies::raise_domain_error<RealType>(
...
#  pragma warning(pop)
   ...
   {
      static const char* function = "streams_boost::math::pdf(const hypergeometric_distribution<%1%>&, const %1%&)";
      RealType result;
      ...
      return streams_boost::math::detail::hypergeometric_pdf<RealType>(
   ...
   inline RealType pdf(const hypergeometric_distribution<RealType, Policy>& dist, const U& x)
   {
      ...
      unsigned u = streams_boost::math::itrunc(r);
      if(u != r)
      {
         return streams_boost::math::policies::raise_domain_error<RealType>(
   ...
   {
      static const char* function = "streams_boost::math::cdf(const hypergeometric_distribution<%1%>&, const %1%&)";
      RealType result;
      if(!dist.check_params(function, &result))
         return result;
      if(!dist.check_x(x, function, &result))
      ...
      return streams_boost::math::detail::hypergeometric_cdf<RealType>(
   ...
   inline RealType quantile(const hypergeometric_distribution<RealType, Policy>& dist, const RealType& p)
   {
      STREAMS_BOOST_MATH_STD_USING // for ADL of std functions
         ...
         RealType result;
      const char* function = "streams_boost::math::quantile(const hypergeometric_distribution<%1%>&, %1%)";
