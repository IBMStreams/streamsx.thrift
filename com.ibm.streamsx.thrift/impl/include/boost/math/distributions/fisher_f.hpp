...
#ifndef STREAMS_BOOST_MATH_DISTRIBUTIONS_FISHER_F_HPP
#define STREAMS_BOOST_MATH_DISTRIBUTIONS_FISHER_F_HPP

#include <streams_boost/math/distributions/fwd.hpp>
#include <streams_boost/math/special_functions/beta.hpp> // for incomplete beta.
...
RealType pdf(const fisher_f_distribution<RealType, Policy>& dist, const RealType& x)
{
   ...
   static const char* function = "streams_boost::math::pdf(fisher_f_distribution<%1%> const&, %1%)";
...
{
   static const char* function = "streams_boost::math::cdf(fisher_f_distribution<%1%> const&, %1%)";
   RealType df1 = dist.degrees_of_freedom1();
   RealType df2 = dist.degrees_of_freedom2();
   // Error check:
   RealType error_result;
   ...
   //
   return v1x > df2
      ? streams_boost::math::ibetac(df2 / 2, df1 / 2, df2 / (df2 + v1x), Policy())
      : streams_boost::math::ibeta(df1 / 2, df2 / 2, v1x / (df2 + v1x), Policy());
} // cdf

template <class RealType, class Policy>
inline RealType quantile(const fisher_f_distribution<RealType, Policy>& dist, const RealType& p)
{
   static const char* function = "streams_boost::math::quantile(fisher_f_distribution<%1%> const&, %1%)";
   RealType df1 = dist.degrees_of_freedom1();
   RealType df2 = dist.degrees_of_freedom2();
   // Error check:
   RealType error_result;
   ...
   x = streams_boost::math::ibeta_inv(df1 / 2, df2 / 2, p, &y, Policy());
...
inline RealType cdf(const complemented2_type<fisher_f_distribution<RealType, Policy>, RealType>& c)
{
   ...
   //
   return v1x > df2
      ? streams_boost::math::ibeta(df2 / 2, df1 / 2, df2 / (df2 + v1x), Policy())
      : streams_boost::math::ibetac(df1 / 2, df2 / 2, v1x / (df2 + v1x), Policy());
}
...
inline RealType quantile(const complemented2_type<fisher_f_distribution<RealType, Policy>, RealType>& c)
{
   ...
   x = streams_boost::math::ibetac_inv(df1 / 2, df2 / 2, p, &y, Policy());
...
{ // Mean of F distribution = v.
   static const char* function = "streams_boost::math::mean(fisher_f_distribution<%1%> const&)";
   RealType df1 = dist.degrees_of_freedom1();
   RealType df2 = dist.degrees_of_freedom2();
   // Error check:
   RealType error_result;
...
{ // Variance of F distribution.
   static const char* function = "streams_boost::math::variance(fisher_f_distribution<%1%> const&)";
   RealType df1 = dist.degrees_of_freedom1();
   RealType df2 = dist.degrees_of_freedom2();
   // Error check:
   RealType error_result;
...
{
   static const char* function = "streams_boost::math::mode(fisher_f_distribution<%1%> const&)";
   RealType df1 = dist.degrees_of_freedom1();
   RealType df2 = dist.degrees_of_freedom2();
   // Error check:
   RealType error_result;
...
//  return tools::domain_error<RealType>(STREAMS_BOOST_CURRENT_FUNCTION, "Median is not implemented, result is %1%!", std::numeric_limits<RealType>::quiet_NaN());
//  } // median
...
{
   static const char* function = "streams_boost::math::skewness(fisher_f_distribution<%1%> const&)";
...
{
   static const char* function = "streams_boost::math::kurtosis_excess(fisher_f_distribution<%1%> const&)";
   // See http://mathworld.wolfram.com/F-Distribution.html
   RealType df1 = dist.degrees_of_freedom1();
   RealType df2 = dist.degrees_of_freedom2();
   // Error check:
...
#endif // STREAMS_BOOST_MATH_DISTRIBUTIONS_FISHER_F_HPP
