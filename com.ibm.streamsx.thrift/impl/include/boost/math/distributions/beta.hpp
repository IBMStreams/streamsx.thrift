// streams_boost\math\distributions\beta.hpp
...
#ifndef STREAMS_BOOST_MATH_DIST_BETA_HPP
#define STREAMS_BOOST_MATH_DIST_BETA_HPP

#include <streams_boost/math/distributions/fwd.hpp>
#include <streams_boost/math/special_functions/beta.hpp> // for beta.
...
namespace streams_boost
{
  namespace math
  {
    namespace beta_detail
    {
      ...
      {
        if(!(streams_boost::math::isfinite)(alpha) || (alpha <= 0))
        {
      ...
      {
        if(!(streams_boost::math::isfinite)(beta) || (beta <= 0))
        {
          *result = policies::raise_domain_error<RealType>(
            function,
      ...
      {
        if(!(streams_boost::math::isfinite)(x) || (x < 0) || (x > 1))
        {
          *result = policies::raise_domain_error<RealType>(
            function,
      ...
      {
        if(!(streams_boost::math::isfinite)(variance) || (variance <= 0))
        {
          *result = policies::raise_domain_error<RealType>(
            function,
    ...
    public:
      ...
      beta_distribution(RealType alpha = 1, RealType beta = 1) : m_alpha(alpha), m_beta(beta)
      {
        ...
        beta_detail::check_dist(
           "streams_boost::math::beta_distribution<%1%>::beta_distribution",
          m_alpha,
          m_beta,
          &result, Policy());
      ...
      {
        static const char* function = "streams_boost::math::beta_distribution<%1%>::find_alpha";
      ...
      static RealType find_beta(
        ...
        RealType variance) // Expected value of variance.
      {
        static const char* function = "streams_boost::math::beta_distribution<%1%>::find_beta";
        RealType result; // of error checks.
        if(false ==
          beta_detail::check_mean(
          function, mean, &result, Policy())
    ...
    {
      static const char* function = "streams_boost::math::mode(beta_distribution<%1%> const&)";
    ...
    { // Probability Density/Mass Function.
      ...
      static const char* function = "streams_boost::math::pdf(beta_distribution<%1%> const&, %1%)";
    ...
    { // Cumulative Distribution Function beta.
      ...
      static const char* function = "streams_boost::math::cdf(beta_distribution<%1%> const&, %1%)";
    ...
    { // Quantile or Percent Point beta function or
      ...
      static const char* function = "streams_boost::math::quantile(beta_distribution<%1%> const&, %1%)";
...
#endif // STREAMS_BOOST_MATH_DIST_BETA_HPP
