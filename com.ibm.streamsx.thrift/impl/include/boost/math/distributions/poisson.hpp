// streams_boost\math\distributions\poisson.hpp
...
#ifndef STREAMS_BOOST_MATH_SPECIAL_POISSON_HPP
#define STREAMS_BOOST_MATH_SPECIAL_POISSON_HPP
...
#include <streams_boost/math/special_functions/trunc.hpp> // for incomplete gamma. gamma_q
...
#include <streams_boost/math/special_functions/factorials.hpp> // factorials.
...
namespace streams_boost
{
  namespace math
  {
     namespace detail{
      ...
      inline typename Dist::value_type 
         inverse_discrete_quantile(
            ...
            streams_boost::uintmax_t& max_iter);
      template <class Dist>
      inline typename Dist::value_type 
         inverse_discrete_quantile(
            const Dist& dist,
    ...
    namespace poisson_detail
    {
      ...
      {
        if(!(streams_boost::math::isfinite)(mean) || (mean < 0))
        {
      ...
      {
        if((k < 0) || !(streams_boost::math::isfinite)(k))
        {
          *result = policies::raise_domain_error<RealType>(
            function,
    ...
    public:
      ...
      { // Expected mean number of events that occur during the given interval.
        ...
        poisson_detail::check_dist(
           "streams_boost::math::poisson_distribution<%1%>::poisson_distribution",
          m_l,
          &r, Policy());
    ...
    { // Probability Density/Mass Function.
      ...
      if(false == poisson_detail::check_dist_and_k(
        "streams_boost::math::pdf(const poisson_distribution<%1%>&, %1%)",
        mean,
        k,
        &result, Policy()))
      {
      ...
      return streams_boost::math::gamma_p_derivative(k+1, mean, Policy());
    } // pdf
    ...
    { // Cumulative Distribution Function Poisson.
      ...
      STREAMS_BOOST_MATH_STD_USING // for ADL of std function exp.

      RealType mean = dist.mean();
      // Error checks:
      RealType result;
      if(false == poisson_detail::check_dist_and_k(
        "streams_boost::math::cdf(const poisson_distribution<%1%>&, %1%)",
        mean,
        k,
        &result, Policy()))
      {
    ...
    { // Complemented Cumulative Distribution Function Poisson
      ...
      { // Avoid repeated checks on k and mean in gamma_p.
         return -streams_boost::math::expm1(-mean, Policy());
      }
    ...
    { // Quantile (or Percent Point) Poisson function.
      ...
      if(false == poisson_detail::check_prob(
        "streams_boost::math::quantile(const poisson_distribution<%1%>&, %1%)",
        p,
        &result, Policy()))
      {
        return result;
      ...
      */
      ...
      if(z < 1)
         ...
         guess = streams_boost::math::detail::inverse_poisson_cornish_fisher(z, p, RealType(1-p), Policy());
      if(z > 5)
      {
         if(z > 1000)
            factor = 1.01f;
    ...
    { // Quantile (or Percent Point) of Poisson function.
      ...
      /*
      if (-q <= streams_boost::math::expm1(-dist.mean()))
      ...
      */
      ...
      if(z < 1)
         ...
         guess = streams_boost::math::detail::inverse_poisson_cornish_fisher(z, RealType(1-q), q, Policy());
      if(z > 5)
      {
         if(z > 1000)
            factor = 1.01f;
...
#endif // STREAMS_BOOST_MATH_SPECIAL_POISSON_HPP
