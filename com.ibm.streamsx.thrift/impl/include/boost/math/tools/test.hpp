...
#ifndef STREAMS_BOOST_MATH_TOOLS_TEST_HPP
#define STREAMS_BOOST_MATH_TOOLS_TEST_HPP
...
#include <streams_boost/math/tools/stats.hpp>
...
{
private:
   streams_boost::math::tools::stats<T> stat;   // Statistics for the test.
...
public:
   ...
   T min STREAMS_BOOST_PREVENT_MACRO_SUBSTITUTION()const{ return (stat.min)(); }
   T max STREAMS_BOOST_PREVENT_MACRO_SUBSTITUTION()const{ return (stat.max)(); }
   T total()const{ return stat.total(); }
   T mean()const{ return stat.mean(); }
   streams_boost::uintmax_t count()const{ return stat.count(); }
...
//
// Function test accepts an matrix of input values (probably a 2D streams_boost::array)
...
test_result<typename calculate_result_type<A>::value_type> test(const A& a, F1 test_func, F2 expect_func)
{
   ...
   for(unsigned i = 0; i < a.size(); ++i)
   {
      ...
      catch(const std::exception& e)
      {
         ...
         print_row(row);
         STREAMS_BOOST_ERROR("Unexpected exception.");
...
#endif
      if(!(streams_boost::math::isfinite)(point) && (streams_boost::math::isfinite)(expected))
      {
         ...
         print_row(row);
         STREAMS_BOOST_ERROR("Unexpected non-finite result");
      }
      if(err > 0.5)
      {
         ...
         std::cout << "Found: " << point << " Expected " << expected << " Error: " << err << std::endl;
         print_row(row);
         STREAMS_BOOST_ERROR("Gross error");
      }
      result.add(err);
      if((result.max)() == err)
         result.set_worst(i);
