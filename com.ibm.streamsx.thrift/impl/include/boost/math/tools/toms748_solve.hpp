...
#ifndef STREAMS_BOOST_MATH_TOOLS_SOLVE_ROOT_HPP
#define STREAMS_BOOST_MATH_TOOLS_SOLVE_ROOT_HPP
...
void bracket(F f, T& a, T& b, T c, T& fa, T& fb, T& d, T& fd)
{
   ...
   //
   if(streams_boost::math::sign(fa) * streams_boost::math::sign(fc) < 0)
   {
      d = b;
      fd = fb;
      b = c;
                           ...
                           unsigned count)
{
   ...
   //
   T c;
   if(streams_boost::math::sign(A) * streams_boost::math::sign(fa) > 0)
   {
      c = a;
   }
   else
                    ...
                    const T& fd, const T& fe)
{
   ...
   //
   STREAMS_BOOST_MATH_INSTRUMENT_CODE(" a = " << a << " b = " << b
      << " d = " << d << " e = " << e << " fa = " << fa << " fb = " << fb 
      << " fd = " << fd << " fe = " << fe);
   T q11 = (d - e) * fd / (fe - fd);
   T q21 = (b - d) * fb / (fd - fb);
   ...
   T d21 = (b - d) * fd / (fd - fb);
   T d31 = (a - b) * fb / (fb - fa);
   STREAMS_BOOST_MATH_INSTRUMENT_CODE(
...
std::pair<T, T> toms748_solve(F f, const T& ax, const T& bx, const T& fax, const T& fbx, Tol tol, streams_boost::uintmax_t& max_iter, const Policy& pol)
{
   //
   ...
   static const char* function = "streams_boost::math::tools::toms748_solve<%1%>";

   streams_boost::uintmax_t count = max_iter;
   ...
   if(streams_boost::math::sign(fa) * streams_boost::math::sign(fb) > 0)
      policies::raise_domain_error(
         function, 
   ...
   while(count && (fa != 0) && !tol(a, b))
   {
      ...
      {
         c = detail::quadratic_interpolate(a, b, d, fa, fb, fd, 2);
         STREAMS_BOOST_MATH_INSTRUMENT_CODE("Can't take cubic step!!!!");
      }
      else
      {
      ...
      --count;
      STREAMS_BOOST_MATH_INSTRUMENT_CODE("Not converging: Taking a bisection!!!!");
      STREAMS_BOOST_MATH_INSTRUMENT_CODE(" a = " << a << " b = " << b);
   } // while loop
...
inline std::pair<T, T> toms748_solve(F f, const T& ax, const T& bx, const T& fax, const T& fbx, Tol tol, streams_boost::uintmax_t& max_iter)
{
   return toms748_solve(f, ax, bx, fax, fbx, tol, max_iter, policies::policy<>());
}

template <class F, class T, class Tol, class Policy>
inline std::pair<T, T> toms748_solve(F f, const T& ax, const T& bx, Tol tol, streams_boost::uintmax_t& max_iter, const Policy& pol)
{
   max_iter -= 2;
...
template <class F, class T, class Tol>
inline std::pair<T, T> toms748_solve(F f, const T& ax, const T& bx, Tol tol, streams_boost::uintmax_t& max_iter)
{
   return toms748_solve(f, ax, bx, tol, max_iter, policies::policy<>());
}

template <class F, class T, class Tol, class Policy>
std::pair<T, T> bracket_and_solve_root(F f, const T& guess, T factor, bool rising, Tol tol, streams_boost::uintmax_t& max_iter, const Policy& pol)
{
   STREAMS_BOOST_MATH_STD_USING
   static const char* function = "streams_boost::math::tools::bracket_and_solve_root<%1%>";
   //
...
template <class F, class T, class Tol>
inline std::pair<T, T> bracket_and_solve_root(F f, const T& guess, const T& factor, bool rising, Tol tol, streams_boost::uintmax_t& max_iter)
{
...
#endif // STREAMS_BOOST_MATH_TOOLS_SOLVE_ROOT_HPP
