...
#ifndef STREAMS_BOOST_MATH_TOOLS_NEWTON_SOLVER_HPP
#define STREAMS_BOOST_MATH_TOOLS_NEWTON_SOLVER_HPP
...
#include <streams_boost/math/tools/toms748_solve.hpp>
...
std::pair<T, T> bisect(F f, T min, T max, Tol tol, streams_boost::uintmax_t& max_iter, const Policy& pol)
{
   T fmin = f(min);
   T fmax = f(max);
   if(fmin == 0)
   ...
   // Error checking:
   //
   static const char* function = "streams_boost::math::tools::bisect<%1%>";
   if(min >= max)
   {
      policies::raise_evaluation_error(function, 
         "Arguments in wrong order in streams_boost::math::tools::bisect (first arg=%1%)", min, pol);
   }
   if(fmin * fmax >= 0)
   {
      policies::raise_evaluation_error(function, 
         "No change of sign in streams_boost::math::tools::bisect, either there is no root to find, or there are multiple roots in the interval (f(min) = %1%).", fmin, pol);
   }
   ...
   static streams_boost::uintmax_t max_count = 0;
...
inline std::pair<T, T> bisect(F f, T min, T max, Tol tol, streams_boost::uintmax_t& max_iter)
{
...
{
   streams_boost::uintmax_t m = (std::numeric_limits<streams_boost::uintmax_t>::max)();
   return bisect(f, min, max, tol, m, policies::policy<>());
}

template <class F, class T>
T newton_raphson_iterate(F f, T guess, T min, T max, int digits, streams_boost::uintmax_t& max_iter)
{
...
template <class F, class T>
T halley_iterate(F f, T guess, T min, T max, int digits, streams_boost::uintmax_t& max_iter)
{
   STREAMS_BOOST_MATH_STD_USING
   ...
   do{
      ...
      STREAMS_BOOST_MATH_INSTRUMENT_VARIABLE(f0);
      STREAMS_BOOST_MATH_INSTRUMENT_VARIABLE(f1);
      STREAMS_BOOST_MATH_INSTRUMENT_VARIABLE(f2);
      ...
      else
      {
         if(f2 != 0)
         {
            ...
            STREAMS_BOOST_MATH_INSTRUMENT_VARIABLE(denom);
            STREAMS_BOOST_MATH_INSTRUMENT_VARIABLE(num);
      ...
      if((convergence > 0.8) && (convergence < 2))
      {
         ...
         // next iteration:
         delta2 = delta * 3;
         STREAMS_BOOST_MATH_INSTRUMENT_VARIABLE(delta);
      }
      guess = result;
      result -= delta;
...
template <class F, class T>
T schroeder_iterate(F f, T guess, T min, T max, int digits, streams_boost::uintmax_t& max_iter)
{
   STREAMS_BOOST_MATH_STD_USING
...
#endif // STREAMS_BOOST_MATH_TOOLS_NEWTON_SOLVER_HPP
