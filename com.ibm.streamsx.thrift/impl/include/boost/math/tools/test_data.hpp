...
#ifndef STREAMS_BOOST_MATH_TOOLS_TEST_DATA_HPP
#define STREAMS_BOOST_MATH_TOOLS_TEST_DATA_HPP
...
#include <streams_boost/tr1/random.hpp>
...
#  pragma warning(push)
#  pragma warning(disable: 4130) // '==' : logical operation on address of string constant.
// Used as a warning with STREAMS_BOOST_ASSERT
#endif
...
inline void unpack_and_append_tuple(Seq& s,
                                    const Item& data,
                                    const streams_boost::integral_constant<int, N>&,
                                    ...
                                    const streams_boost::true_type&)
{
   ...
   typedef streams_boost::integral_constant<int, N+1> next_value;
   typedef streams_boost::integral_constant<bool, (std::tr1::tuple_size<Item>::value > N+1)> terminate;

   unpack_and_append_tuple(s, data, next_value(), terminate());
}

template <class Seq, class Item>
inline void unpack_and_append(Seq& s, const Item& data, const streams_boost::true_type&)
{
   s.push_back(data);
}

template <class Seq, class Item>
inline void unpack_and_append(Seq& s, const Item& data, const streams_boost::false_type&)
{
   // Item had better be a tuple-like type or we've had it!!!!
   typedef streams_boost::integral_constant<int, 0> next_value;
   typedef streams_boost::integral_constant<bool, (std::tr1::tuple_size<Item>::value > 0)> terminate;
...
{
   typedef typename Seq::value_type value_type;
   unpack_and_append(s, data, ::streams_boost::is_convertible<Item, value_type>());
}
   ...
   test_data& insert(F func, const parameter_info<T>& arg1)
   {
      ...
      while(a != b)
      {
         ...
         try{
            ...
            streams_boost::math::tools::detail::unpack_and_append(row, func(*a));
            m_data.insert(row);
         }
...
void test_data<T>::create_test_points(std::set<T>& points, const parameter_info<T>& arg1)
{
   ...
   case random_in_range:
      {
         STREAMS_BOOST_ASSERT(arg1.z1 < arg1.z2);
         STREAMS_BOOST_ASSERT(arg1.n1 > 0);
   ...
   case power_series:
      {
         STREAMS_BOOST_ASSERT(arg1.n1 < arg1.n2);

         typedef float random_type;
         typedef typename streams_boost::mpl::if_<
            ::streams_boost::is_floating_point<T>,
      ...
      break;
   default:
      STREAMS_BOOST_ASSERT(0 == "Invalid parameter_info object");
...
#  pragma warning(disable: 4127)
   ...
   case periodic_in_range:
      ...
      do{
         ...
         try{
            info.z1 = streams_boost::lexical_cast<T>(line);
            break;
         }
      ...
      do{
         ...
         else
         {
            try
            {
               info.z2 = streams_boost::lexical_cast<T>(line);
            }
            catch(const streams_boost::bad_lexical_cast&)
            {
               std::cout << "Sorry, that was not valid input, try again [y/n]?";
      ...
      do{
         ...
         streams_boost::algorithm::trim(line);
         try{
            info.n1 = streams_boost::lexical_cast<int>(line);
            info.n2 = 0;
            if(info.n1 <= 0)
            {
   ...
   case power_series:
      ...
      do{
         ...
         streams_boost::algorithm::trim(line);
         try{
            info.n2 = streams_boost::lexical_cast<int>(line);
            break;
         }
         catch(const streams_boost::bad_lexical_cast&)
         {
      ...
      break;
   default:
      STREAMS_BOOST_ASSERT(0); // should never get here!!
   }
                         ...
                         const char* name)
{
   ...
   os << "#define SC_(x) static_cast<T>(STREAMS_BOOST_JOIN(x, L))\n"
   "   static const streams_boost::array<streams_boost::array<T, "
...
#endif // STREAMS_BOOST_MATH_TOOLS_TEST_DATA_HPP
