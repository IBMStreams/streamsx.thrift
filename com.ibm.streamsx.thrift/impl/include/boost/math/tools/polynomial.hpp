...
#ifndef STREAMS_BOOST_MATH_TOOLS_POLYNOMIAL_HPP
#define STREAMS_BOOST_MATH_TOOLS_POLYNOMIAL_HPP
...
#include <streams_boost/math/special_functions/binomial.hpp>
...
T chebyshev_coefficient(unsigned n, unsigned m)
{
   ...
   STREAMS_BOOST_ASSERT(n - 2 * r == m);

   if(r & 1)
      result = -result;
   result /= n - r;
   result *= streams_boost::math::binomial_coefficient<T>(n - r, r);
   result *= ldexp(1.0f, m);
   return result;
}
...
public:
   ...
   polynomial(const polynomial<U>& p)
   {
      ...
      {
         m_data.push_back(streams_boost::math::tools::real_cast<T>(p[i]));
      }
   }
   ...
   {
      return streams_boost::math::tools::evaluate_polynomial(&m_data[0], z, m_data.size());;
   }
   ...
   polynomial& operator *=(const polynomial<U>& value)
   {
      ...
      STREAMS_BOOST_ASSERT(value.size());
...
#endif // STREAMS_BOOST_MATH_TOOLS_POLYNOMIAL_HPP
