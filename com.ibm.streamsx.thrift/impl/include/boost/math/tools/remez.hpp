...
#ifndef STREAMS_BOOST_MATH_TOOLS_REMEZ_HPP
#define STREAMS_BOOST_MATH_TOOLS_REMEZ_HPP

#ifdef _MSC_VER
#pragma once
#endif

#include <streams_boost/math/tools/solve.hpp>
#include <streams_boost/math/tools/minima.hpp>
#include <streams_boost/math/tools/roots.hpp>
#include <streams_boost/math/tools/polynomial.hpp>
...
{
   typedef streams_boost::function1<T, T const &> function_type;
public:
...
public:
   typedef streams_boost::function1<T, T const &> function_type;
   typedef streams_boost::numeric::ublas::vector<T> vector_type;
   typedef streams_boost::numeric::ublas::matrix<T> matrix_type;
   ...
   {
      STREAMS_BOOST_ASSERT(b < 100);
      STREAMS_BOOST_ASSERT(b >= 0);
      m_brake = b;
   }
...
void remez_minimax<T>::init_chebyshev()
{
   ...
   //
   vector_type l_solution = streams_boost::math::tools::solve(A, b);
         ...
         int bits)
{
   ...
   if(bits == 0)
   {
      ...
      m_precision = (std::min)(24, (streams_boost::math::policies::digits<T, streams_boost::math::policies::policy<> >() / 2) - 2);
   }
   else
   {
      // can't be more accurate than half the bits of T:
      m_precision = (std::min)(bits, (streams_boost::math::policies::digits<T, streams_boost::math::policies::policy<> >() / 2) - 2);
   }
...
T remez_minimax<T>::iterate()
{
   ...
   do{
      ...
      // Now go ahead and solve the expression to get our solution:
      //
      solution = streams_boost::math::tools::solve(A, b);
   ...
   for(unsigned i = 1; i < control_points.size(); ++i)
   {
      ...
      streams_boost::uintmax_t max_iter = 1000;
...
{
   streams_boost::scoped_array<T> a(new T[orderN + 1]);
   if(pinned)
      a[0] = 0;
   unsigned terms = pinned ? orderN : orderN + 1;
   for(unsigned i = 0; i < terms; ++i)
      a[pinned ? i+1 : i] = solution[i];
   return streams_boost::math::tools::polynomial<T>(&a[0], orderN);
}
...
polynomial<T> remez_minimax<T>::denominator()const
{
   ...
   streams_boost::scoped_array<T> a(new T[terms]);
   a[0] = 1;
   for(unsigned i = 0; i < orderD; ++i)
      a[i+1] = solution[i + offsetD];
   return streams_boost::math::tools::polynomial<T>(&a[0], orderD);
}


}}} // namespaces

#endif // STREAMS_BOOST_MATH_TOOLS_REMEZ_HPP
