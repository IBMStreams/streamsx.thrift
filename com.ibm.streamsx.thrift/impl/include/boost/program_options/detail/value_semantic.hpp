...
namespace streams_boost { namespace program_options { 
    ...
    extern STREAMS_BOOST_PROGRAM_OPTIONS_DECL std::string arg;
    ...
    template<class T, class charT>
    void 
    typed_value<T, charT>::notify(const streams_boost::any& value_store) const
    {
        const T* value = streams_boost::any_cast<T>(&value_store);
        if (m_store_to) {
            *m_store_to = *value;
        }
        if (m_notifier) {
    ...
    namespace validators {
            ...
            bool allow_empty = false)
        {
            ...
            if (v.size() > 1)
                streams_boost::throw_exception(validation_error(validation_error::multiple_values_not_allowed));
            else if (v.size() == 1)
                return v.front();
            else if (!allow_empty)
                streams_boost::throw_exception(validation_error(validation_error::at_least_one_value_required));
            return empty;
        }

        /* Throws multiple_occurrences if 'value' is not empty. */
        STREAMS_BOOST_PROGRAM_OPTIONS_DECL void 
        check_first_occurrence(const streams_boost::any& value);
    }
    ...
    */
    template<class T, class charT>
    void validate(streams_boost::any& v, 
                  ...
                  T*, long)
    {
        ...
        try {
            ...
            streams_boost::throw_exception(invalid_option_value(s));
        }
    }

    STREAMS_BOOST_PROGRAM_OPTIONS_DECL void validate(streams_boost::any& v, 
    ...
    // value_semantic.cpp
          ...
          STREAMS_BOOST_WORKAROUND(__GNUC_MINOR__, < 3) ) || \
    ...
    {
        if (v.empty()) {
            v = streams_boost::any(std::vector<T>());
        }
        std::vector<T>* tv = streams_boost::any_cast< std::vector<T> >(&v);
        assert(NULL != tv);
        for (unsigned i = 0; i < s.size(); ++i)
        {
            try {
                ...
                v.push_back(s[i]);
                validate(a, v, (T*)0, 0);                
                tv->push_back(streams_boost::any_cast<T>(a));
            }
            catch(const bad_lexical_cast& /*e*/) {
                streams_boost::throw_exception(invalid_option_value(s[i]));
            }
        }
    }
    ...
    void 
    typed_value<T, charT>::
    xparse(streams_boost::any& value_store, 
    ...
    value()
    {
        ...
        return streams_boost::program_options::value<T>(0);
    }
