...
void PBGL_DISTRIB_PMAP::prune_ghost_cells() const
{
  ...
  while (data->ghost_cells->size() > data->max_ghost_cells) {
    ...
    if (data->model & cm_flush) {
      ...
      streams_boost::parallel::detail::untracked_pair<key_type, value_type> const& victim
...
handle_put(int /*source*/, int /*tag*/, 
           const streams_boost::parallel::detail::untracked_pair<key_type, value_type>& req, trigger_receive_context)
{
                ...
                trigger_receive_context)
{
  ...
  typedef streams_boost::parallel::detail::untracked_pair<key_type, value_type> key_value;
...
PBGL_DISTRIB_PMAP::set_max_ghost_cells(std::size_t max_ghost_cells)
{
  if ((data->model & cm_backward) && max_ghost_cells > 0)
      streams_boost::throw_exception(std::runtime_error("distributed_property_map::set_max_ghost_cells: "
