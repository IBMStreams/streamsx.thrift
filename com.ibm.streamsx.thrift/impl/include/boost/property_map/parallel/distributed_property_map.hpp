...
#ifndef STREAMS_BOOST_PARALLEL_DISTRIBUTED_PROPERTY_MAP_HPP
#define STREAMS_BOOST_PARALLEL_DISTRIBUTED_PROPERTY_MAP_HPP
...
#include <vector>
#include <set>
#include <streams_boost/graph/parallel/basic_reduce.hpp>
...
#include <streams_boost/multi_index/hashed_index.hpp>
...
using streams_boost::graph::parallel::trigger_receive_context;
 ...
 public:
  ...
  { 
    send(data->process_group, p, property_map_put, 
         streams_boost::parallel::detail::make_untracked_pair(k, v)); 
  }
 ...
 private:
  ...
  struct handle_message
  {
    ...
    handle_put(int source, int tag, 
               const streams_boost::parallel::detail::untracked_pair<key_type, value_type>& data, 
    ...
    handle_multiget_reply
      (int source, int tag, 
       const std::vector<streams_boost::parallel::detail::untracked_pair<key_type, value_type> >& msg,
       trigger_receive_context);
...
namespace streams_boost {
  ...
  struct hash<streams_boost::detail::edge_desc_impl<D, V> >
  {
    std::size_t operator()(const streams_boost::detail::edge_desc_impl<D, V> & x) const
    { return hash_value(x.get_property()); }
  };
}

#include <streams_boost/property_map/parallel/impl/distributed_property_map.ipp>

#undef PBGL_DISTRIB_PMAP

#endif // STREAMS_BOOST_PARALLEL_DISTRIBUTED_PROPERTY_MAP_HPP
