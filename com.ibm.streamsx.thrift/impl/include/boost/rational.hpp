...
// streams_boostinspect:nolicense (don't complain about the lack of a Boost license)
// (Paul Moore hasn't been in contact for years, so there's no way to change the
// license.)

//  See http://www.streams_boost.org/libs/rational for documentation.

//  Credits:
//  Thanks to the streams_boost mailing list in general for useful comments.
...
//  22 Jun 00  Change _MSC_VER to STREAMS_BOOST_MSVC so other compilers are not
//             affected (Beman Dawes)
//   6 Mar 00  Fix operator-= normalization, #include <string> (Jens Maurer)
//  14 Dec 99  Modifications based on comments from the streams_boost list
//  09 Dec 99  Initial Version (Paul Moore)

#ifndef STREAMS_BOOST_RATIONAL_HPP
#define STREAMS_BOOST_RATIONAL_HPP

#include <iostream>              // for std::istream and std::ostream
#include <ios>                   // for std::noskipws
#include <stdexcept>             // for std::domain_error
#include <string>                // for std::string implicit constructor
#include <streams_boost/operators.hpp>   // for streams_boost::addable etc
#include <cstdlib>               // for std::abs
#include <streams_boost/call_traits.hpp> // for streams_boost::call_traits
#include <streams_boost/config.hpp>      // for STREAMS_BOOST_NO_STDC_NAMESPACE, STREAMS_BOOST_MSVC
#include <streams_boost/detail/workaround.hpp> // for STREAMS_BOOST_WORKAROUND
#include <streams_boost/assert.hpp>      // for STREAMS_BOOST_ASSERT
#include <streams_boost/math/common_factor_rt.hpp>  // for streams_boost::math::gcd, lcm
#include <limits>                // for std::numeric_limits
#include <streams_boost/static_assert.hpp>  // for STREAMS_BOOST_STATIC_ASSERT

// Control whether depreciated GCD and LCM functions are included (default: yes)
#ifndef STREAMS_BOOST_CONTROL_RATIONAL_HAS_GCD
#define STREAMS_BOOST_CONTROL_RATIONAL_HAS_GCD  1
#endif

namespace streams_boost {

#if STREAMS_BOOST_CONTROL_RATIONAL_HAS_GCD
...
#endif  // STREAMS_BOOST_CONTROL_RATIONAL_HAS_GCD
    ...
    > > > > > > > > > > > > > > > >
{
    ...
    STREAMS_BOOST_STATIC_ASSERT( ::std::numeric_limits<IntType>::is_specialized );

    // Helper types
    typedef typename streams_boost::call_traits<IntType>::param_type param_type;
...
bool rational<IntType>::operator< (const rational<IntType>& r) const
{
    ...
    STREAMS_BOOST_ASSERT( this->den > zero );
    STREAMS_BOOST_ASSERT( r.den > zero );
...
void rational<IntType>::normalize()
{
    ...
    STREAMS_BOOST_ASSERT( this->test_invariant() );
}
...
inline T rational_cast(
    const rational<IntType>& src STREAMS_BOOST_APPEND_EXPLICIT_TEMPLATE_TYPE(T))
{
...
#endif  // STREAMS_BOOST_RATIONAL_HPP
