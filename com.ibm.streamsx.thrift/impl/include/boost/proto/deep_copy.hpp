    ...
    #ifndef STREAMS_BOOST_PROTO_DEEP_COPY_HPP_EAN_11_21_2006
    #define STREAMS_BOOST_PROTO_DEEP_COPY_HPP_EAN_11_21_2006
    ...
    namespace streams_boost { namespace proto
    {
        namespace detail
        {
            ...
            {
                typedef STREAMS_BOOST_PROTO_UNCVREF(typename Expr::proto_child0) raw_terminal_type;
            ...
            struct deep_copy
            {
                typedef
                    typename detail::deep_copy_impl<
                        STREAMS_BOOST_PROTO_UNCVREF(Expr)
                    >::result_type
                type;
            };
        }
          ...
          : proto::transform<_deep_copy>
        {
            ...
            struct impl
              : detail::deep_copy_impl<STREAMS_BOOST_PROTO_UNCVREF(E)>
            {};
        };

        namespace detail
        {
        #define STREAMS_BOOST_PROTO_DEFINE_DEEP_COPY_TYPE(Z, N, DATA)                                       \
            typename deep_copy_impl<                                                                \
                typename remove_reference<                                                          \
                  typename Expr::STREAMS_BOOST_PP_CAT(proto_child, N)                                       \
                >::type::proto_derived_expr                                                         \
            >::result_type                                                                          \
            /**/

        #define STREAMS_BOOST_PROTO_DEFINE_DEEP_COPY_FUN(Z, N, DATA)                                        \
            proto::deep_copy(e.proto_base().STREAMS_BOOST_PP_CAT(child, N))                                 \
            /**/

        #define STREAMS_BOOST_PP_ITERATION_PARAMS_1 (3, (1, STREAMS_BOOST_PROTO_MAX_ARITY, <streams_boost/proto/deep_copy.hpp>))
        #include STREAMS_BOOST_PP_ITERATE()

        #undef STREAMS_BOOST_PROTO_DEFINE_DEEP_COPY_FUN
        #undef STREAMS_BOOST_PROTO_DEFINE_DEEP_COPY_TYPE
        }

    }}

    #endif // STREAMS_BOOST_PROTO_COMPILER_DEEP_COPY_HPP_EAN_11_21_2006
    ...
    #define N STREAMS_BOOST_PP_ITERATION()
            ...
            struct deep_copy_impl<Expr, N>
            {
                typedef
                    proto::expr<
                      ...
                      , STREAMS_BOOST_PP_CAT(list, N)<
                            STREAMS_BOOST_PP_ENUM(N, STREAMS_BOOST_PROTO_DEFINE_DEEP_COPY_TYPE, ~)
                        >
                      , N
                    >
                expr_type;
                ...
                {
                    expr_type that = {
                        STREAMS_BOOST_PP_ENUM(N, STREAMS_BOOST_PROTO_DEFINE_DEEP_COPY_FUN, ~)
                    };
