...
#ifndef STREAMS_BOOST_PROTO_TRANSFORM_ARG_HPP_EAN_11_01_2007
#define STREAMS_BOOST_PROTO_TRANSFORM_ARG_HPP_EAN_11_01_2007
...
namespace streams_boost { namespace proto
{
    ...
    /// assert( streams_boost::addressof(i) == streams_boost::addressof(j) );
    /// \endcode
    ...
    /// in a \c streams_boost::reference_wrapper\<\>.
    ///
    /// Example:
    ///
    /// \code
    /// proto::terminal<int>::type i = {42};
    /// streams_boost::reference_wrapper<proto::terminal<int>::type> j
    ///     = proto::when<_, proto::_byref(_)>()(i);
    /// assert( streams_boost::addressof(i) == streams_boost::addressof(j.get()) );
    /// \endcode
    struct _byref : callable
    {
        ...
        {
            typedef streams_boost::reference_wrapper<T const> const type;
        };

        template<typename This, typename T>
        struct result<This(T &)>
        {
            typedef streams_boost::reference_wrapper<T> const type;
        };

        /// Wrap the parameter \c t in a \c streams_boost::reference_wrapper\<\>
        /// \param t The object to wrap
        /// \return <tt>streams_boost::ref(t)</tt>
        /// \throw nothrow
        template<typename T>
        streams_boost::reference_wrapper<T> const operator ()(T &t) const
        {
            return streams_boost::reference_wrapper<T>(t);
        }

        /// \overload
        ///
        template<typename T>
        streams_boost::reference_wrapper<T const> const operator ()(T const &t) const
        {
            return streams_boost::reference_wrapper<T const>(t);
        }
    };

    /// \brief A unary CallableTransform that strips references
    /// and \c streams_boost::reference_wrapper\<\> from its argument.
    ///
    /// Example:
    ///
    /// \code
    /// proto::terminal<int>::type i = {42};
    /// int j = 67;
    /// int k = proto::when<_, proto::_byval(proto::_state)>()(i, streams_boost::ref(j));
    /// assert( 67 == k );
    /// \endcode
    struct _byval : callable
    {
        ...
        template<typename This, typename T>
        struct result<This(streams_boost::reference_wrapper<T>)>
          : result<This(T)>
        {};
        ...
        ///
        template<typename T>
        T operator ()(streams_boost::reference_wrapper<T> const &t) const
        {
            return t;
        }
    };
