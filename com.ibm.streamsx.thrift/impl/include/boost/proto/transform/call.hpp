    ...
    #ifndef STREAMS_BOOST_PROTO_TRANSFORM_CALL_HPP_EAN_11_02_2007
    #define STREAMS_BOOST_PROTO_TRANSFORM_CALL_HPP_EAN_11_02_2007
    ...
    namespace streams_boost { namespace proto
    {
        ...
        struct call<Fun()> : transform<call<Fun()> >
        {
            ...
            {
                typedef typename streams_boost::result_of<Fun()>::type result_type;
            ...
            /// for <tt>streams_boost::result_of\<Fun()\>::::type</tt>. Otherwise, it is
            /// a typedef for <tt>streams_boost::result_of\<Fun(Expr, State, Data)\>::::type</tt>.
            template<typename Expr, typename State, typename Data>
            struct impl
        ...
        struct call<Fun(A0)> : transform<call<Fun(A0)> >
        {
            ...
            /// then \c type is a typedef for <tt>streams_boost::result_of\<Fun(X)\>::::type</tt>.
            /// Otherwise, it is a typedef for <tt>streams_boost::result_of\<Fun(X, State, Data)\>::::type</tt>.
        ...
        struct call<Fun(A0, A1)> : transform<call<Fun(A0, A1)> >
        {
              ...
              : transform_impl<Expr, State, Data>
            {
                ...
                /// <tt>streams_boost::result_of\<Fun(X, Y)\>::::type</tt>. Otherwise, it is
                /// a typedef for <tt>streams_boost::result_of\<Fun(X, Y, Data)\>::::type</tt>.
        ...
        #define STREAMS_BOOST_PP_ITERATION_PARAMS_1 (3, (4, STREAMS_BOOST_PROTO_MAX_ARITY, <streams_boost/proto/transform/call.hpp>))
    ...
    #define N STREAMS_BOOST_PP_ITERATION()
        ...
        template<typename Fun STREAMS_BOOST_PP_ENUM_TRAILING_PARAMS(N, typename A)>
        struct call<Fun(STREAMS_BOOST_PP_ENUM_PARAMS(N, A))> : transform<call<Fun(STREAMS_BOOST_PP_ENUM_PARAMS(N, A))> >
        {
            template<typename Expr, typename State, typename Data>
            struct impl : transform_impl<Expr, State, Data>
            {
                #define M0(Z, M, DATA)                                                              \
                    typedef                                                                         \
                        ...
                        ::result_type                                                               \
                    STREAMS_BOOST_PP_CAT(a, M);                                                             \
                    /**/
                STREAMS_BOOST_PP_REPEAT(N, M0, ~)
                #undef M0

                typedef
                    typename detail::poly_function_traits<Fun, Fun(STREAMS_BOOST_PP_ENUM_PARAMS(N, a))>::result_type
                result_type;
                ...
                ) const
                {
                    #define M0(Z, M, DATA)                                                          \
                        ...
                        /**/
                    return typename detail::poly_function_traits<Fun, Fun(STREAMS_BOOST_PP_ENUM_PARAMS(N, a))>::function_type()(
                        STREAMS_BOOST_PP_ENUM(N, M0, ~)
                    );
                    #undef M0
                }
            };
