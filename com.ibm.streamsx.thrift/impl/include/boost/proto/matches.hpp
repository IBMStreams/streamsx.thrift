    ...
    #ifndef STREAMS_BOOST_PROTO_MATCHES_HPP_EAN_11_03_2006
    #define STREAMS_BOOST_PROTO_MATCHES_HPP_EAN_11_03_2006
    ...
    #include <streams_boost/proto/transform/when.hpp>
    #include <streams_boost/proto/transform/impl.hpp>

    // Some compilers (like GCC) need extra help figuring out a template's arity.
    // I use MPL's STREAMS_BOOST_MPL_AUX_LAMBDA_ARITY_PARAM() macro to disambiguate, which
    // which is controlled by the STREAMS_BOOST_MPL_LIMIT_METAFUNCTION_ARITY macro. If
    // You define STREAMS_BOOST_PROTO_MAX_ARITY to be greater than
    // STREAMS_BOOST_MPL_LIMIT_METAFUNCTION_ARITY on these compilers, things don't work.
    // You must define STREAMS_BOOST_MPL_LIMIT_METAFUNCTION_ARITY to be greater.
    #ifdef STREAMS_BOOST_MPL_CFG_EXTENDED_TEMPLATE_PARAMETERS_MATCHING
    # if STREAMS_BOOST_PROTO_MAX_ARITY > STREAMS_BOOST_MPL_LIMIT_METAFUNCTION_ARITY
    #  error STREAMS_BOOST_MPL_LIMIT_METAFUNCTION_ARITY must be at least as large as STREAMS_BOOST_PROTO_MAX_ARITY
    # endif
    #endif
    ...
    namespace streams_boost { namespace proto
    {
        ...
        namespace detail
        {
            ...
            template<typename T, typename U
                STREAMS_BOOST_MPL_AUX_LAMBDA_ARITY_PARAM(long Arity = mpl::aux::template_arity<U>::value)
            >
            ...
            template<typename T>
            struct lambda_matches<T, proto::_ STREAMS_BOOST_MPL_AUX_LAMBDA_ARITY_PARAM(-1)>
              : mpl::true_
            {};

            template<typename T>
            struct lambda_matches<T, T STREAMS_BOOST_MPL_AUX_LAMBDA_ARITY_PARAM(-1)>
              : mpl::true_
            {};

            template<typename T, std::size_t M, typename U>
            struct lambda_matches<T[M], U STREAMS_BOOST_MPL_AUX_LAMBDA_ARITY_PARAM(-1)>
              : array_matches<T[M], U>
            {};

            template<typename T, std::size_t M>
            struct lambda_matches<T[M], T[M] STREAMS_BOOST_MPL_AUX_LAMBDA_ARITY_PARAM(-1)>
              : mpl::true_
            {};

            template<template<typename> class T, typename Expr0, typename Grammar0>
            struct lambda_matches<T<Expr0>, T<Grammar0> STREAMS_BOOST_MPL_AUX_LAMBDA_ARITY_PARAM(1) >
            ...
            template<
                typename T
              , typename U
              , bool B = is_array<STREAMS_BOOST_PROTO_UNCVREF(T)>::value
            >
            ...
            struct terminal_matches
              : mpl::or_<
                    mpl::and_<
                      ...
                      , lambda_matches<
                            STREAMS_BOOST_PROTO_UNCVREF(T)
                          , STREAMS_BOOST_PROTO_UNCVREF(U)
                        >
                    >
        ...
        #define STREAMS_BOOST_PROTO_MATCHES_N_FUN(Z, N, DATA)                                               \
            matches_<                                                                               \
                typename detail::expr_traits<typename Args1::STREAMS_BOOST_PP_CAT(child, N)>::value_type::proto_base_expr\
              , typename Args2::STREAMS_BOOST_PP_CAT(child, N)::proto_base_expr                             \
            >

        #define STREAMS_BOOST_PROTO_DEFINE_MATCHES(Z, N, DATA)                                              \
            matches_<                                                                               \
                typename Expr::proto_base_expr                                                      \
              , typename STREAMS_BOOST_PP_CAT(G, N)::proto_base_expr                                        \
            >

        #define STREAMS_BOOST_PROTO_DEFINE_LAMBDA_MATCHES(Z, N, DATA)                                       \
            lambda_matches<                                                                         \
                STREAMS_BOOST_PP_CAT(Expr, N)                                                               \
              , STREAMS_BOOST_PP_CAT(Grammar, N)                                                            \
            >

        #if STREAMS_BOOST_PROTO_MAX_LOGICAL_ARITY > STREAMS_BOOST_PROTO_MAX_ARITY
            #define STREAMS_BOOST_PP_ITERATION_PARAMS_1 (4, (2, STREAMS_BOOST_PROTO_MAX_LOGICAL_ARITY, <streams_boost/proto/matches.hpp>, 1))
        #else
            #define STREAMS_BOOST_PP_ITERATION_PARAMS_1 (4, (2, STREAMS_BOOST_PROTO_MAX_ARITY, <streams_boost/proto/matches.hpp>, 1))
        #endif
        #include STREAMS_BOOST_PP_ITERATE()

        #define STREAMS_BOOST_PP_ITERATION_PARAMS_1 (4, (2, STREAMS_BOOST_PROTO_MAX_ARITY, <streams_boost/proto/matches.hpp>, 2))
        #include STREAMS_BOOST_PP_ITERATE()

        #undef STREAMS_BOOST_PROTO_MATCHES_N_FUN
        #undef STREAMS_BOOST_PROTO_DEFINE_MATCHES
        #undef STREAMS_BOOST_PROTO_DEFINE_LAMBDA_MATCHES
        ...
        namespace result_of
        {
            ...
            ///     <tt>streams_boost::result_of\<when\<_,T\>(E,int,int)\>::::type::value</tt>
        ...
        namespace control
        {
            ...
            /// <tt>streams_boost::result_of\<when\<_,If\>(E,int,int)\>::::type::value</tt>
            /// is \c true and \c E matches \c U; or, if
            /// <tt>streams_boost::result_of\<when\<_,If\>(E,int,int)\>::::type::value</tt>
            ...
            /// if <tt>streams_boost::result_of\<when\<_,If\>(E,S,V)\>::::type::value</tt>
            ...
            template<STREAMS_BOOST_PP_ENUM_PARAMS(STREAMS_BOOST_PROTO_MAX_LOGICAL_ARITY, typename G)>
            struct or_ : transform<or_<STREAMS_BOOST_PP_ENUM_PARAMS(STREAMS_BOOST_PROTO_MAX_LOGICAL_ARITY, G)> >
            {
            ...
            template<STREAMS_BOOST_PP_ENUM_PARAMS(STREAMS_BOOST_PROTO_MAX_LOGICAL_ARITY, typename G)>
            struct and_ : transform<and_<STREAMS_BOOST_PP_ENUM_PARAMS(STREAMS_BOOST_PROTO_MAX_LOGICAL_ARITY, G)> >
            {
        ...
        ///
        template<STREAMS_BOOST_PP_ENUM_PARAMS(STREAMS_BOOST_PROTO_MAX_LOGICAL_ARITY, typename G)>
        struct is_callable<or_<STREAMS_BOOST_PP_ENUM_PARAMS(STREAMS_BOOST_PROTO_MAX_LOGICAL_ARITY, G)> >
          : mpl::true_
        {};
        ...
        ///
        template<STREAMS_BOOST_PP_ENUM_PARAMS(STREAMS_BOOST_PROTO_MAX_LOGICAL_ARITY, typename G)>
        struct is_callable<and_<STREAMS_BOOST_PP_ENUM_PARAMS(STREAMS_BOOST_PROTO_MAX_LOGICAL_ARITY, G)> >
          : mpl::true_
        {};
    ...
    #define N STREAMS_BOOST_PP_ITERATION()
            ...
            // and_N is used by lambda_matches up to STREAMS_BOOST_PROTO_MAX_ARITY,
            // regardless of how low STREAMS_BOOST_PROTO_MAX_LOGICAL_ARITY is.
            template<bool B, STREAMS_BOOST_PP_ENUM_PARAMS(STREAMS_BOOST_PP_DEC(N), typename P)>
            struct STREAMS_BOOST_PP_CAT(and_, N)
            #if 2 == N
              : mpl::bool_<P0::value>
            {};
            #else
              : STREAMS_BOOST_PP_CAT(and_, STREAMS_BOOST_PP_DEC(N))<
                    P0::value STREAMS_BOOST_PP_COMMA_IF(STREAMS_BOOST_PP_SUB(N,2))
                    STREAMS_BOOST_PP_ENUM_SHIFTED_PARAMS(STREAMS_BOOST_PP_DEC(N), P)
                >
            {};
            #endif

            template<STREAMS_BOOST_PP_ENUM_PARAMS(STREAMS_BOOST_PP_DEC(N), typename P)>
            struct STREAMS_BOOST_PP_CAT(and_, N)<false, STREAMS_BOOST_PP_ENUM_PARAMS(STREAMS_BOOST_PP_DEC(N), P)>
              : mpl::false_
            {};

        #if N <= STREAMS_BOOST_PROTO_MAX_LOGICAL_ARITY
            template<STREAMS_BOOST_PP_ENUM_PARAMS(N, typename G)>
            struct last<proto::and_<STREAMS_BOOST_PP_ENUM_PARAMS(N, G)> >
            {
                typedef STREAMS_BOOST_PP_CAT(G, STREAMS_BOOST_PP_DEC(N)) type;
            };

            template<bool B, typename Expr, STREAMS_BOOST_PP_ENUM_PARAMS(N, typename G)>
            struct STREAMS_BOOST_PP_CAT(or_, N)
            #if 2 == N
            ...
            #else
              : STREAMS_BOOST_PP_CAT(or_, STREAMS_BOOST_PP_DEC(N))<
                    matches_<Expr, typename G1::proto_base_expr>::value
                  , Expr, STREAMS_BOOST_PP_ENUM_SHIFTED_PARAMS(N, G)
                >
            {};
            #endif

            template<typename Expr STREAMS_BOOST_PP_ENUM_TRAILING_PARAMS(N, typename G)>
            struct STREAMS_BOOST_PP_CAT(or_, N)<true, Expr, STREAMS_BOOST_PP_ENUM_PARAMS(N, G)>
              : mpl::true_
            {
                typedef G0 which;
            };
            ...
            // handle proto::or_
            template<typename Expr, STREAMS_BOOST_PP_ENUM_PARAMS(N, typename G)>
            struct matches_<Expr, proto::or_<STREAMS_BOOST_PP_ENUM_PARAMS(N, G)> >
              : STREAMS_BOOST_PP_CAT(or_, N)<
                    matches_<typename Expr::proto_base_expr, typename G0::proto_base_expr>::value,
                    typename Expr::proto_base_expr, STREAMS_BOOST_PP_ENUM_PARAMS(N, G)
                >
            {};

            // handle proto::and_
            template<typename Expr, STREAMS_BOOST_PP_ENUM_PARAMS(N, typename G)>
            struct matches_<Expr, proto::and_<STREAMS_BOOST_PP_ENUM_PARAMS(N, G)> >
              : detail::STREAMS_BOOST_PP_CAT(and_, N)<
                    STREAMS_BOOST_PROTO_DEFINE_MATCHES(~, 0, ~)::value,
                    STREAMS_BOOST_PP_ENUM_SHIFTED(N, STREAMS_BOOST_PROTO_DEFINE_MATCHES, ~)
                >
            {};
        #endif
    ...
    #define N STREAMS_BOOST_PP_ITERATION()
            ...
            struct vararg_matches_impl<Args, Back, N, To>
              : and_2<
                    matches_<typename detail::expr_traits<typename Args::STREAMS_BOOST_PP_CAT(child, STREAMS_BOOST_PP_DEC(N))>::value_type::proto_base_expr, Back>::value
            ...
            struct vararg_matches_impl<Args, Back, N, N>
              : matches_<typename detail::expr_traits<typename Args::STREAMS_BOOST_PP_CAT(child, STREAMS_BOOST_PP_DEC(N))>::value_type::proto_base_expr, Back>
            {};

            template<
                template<STREAMS_BOOST_PP_ENUM_PARAMS(N, typename STREAMS_BOOST_PP_INTERCEPT)> class T
                STREAMS_BOOST_PP_ENUM_TRAILING_PARAMS(N, typename Expr)
                STREAMS_BOOST_PP_ENUM_TRAILING_PARAMS(N, typename Grammar)
            >
            struct lambda_matches<T<STREAMS_BOOST_PP_ENUM_PARAMS(N, Expr)>, T<STREAMS_BOOST_PP_ENUM_PARAMS(N, Grammar)> STREAMS_BOOST_MPL_AUX_LAMBDA_ARITY_PARAM(N) >
              : STREAMS_BOOST_PP_CAT(and_, N)<
                    STREAMS_BOOST_PROTO_DEFINE_LAMBDA_MATCHES(~, 0, ~)::value,
                    STREAMS_BOOST_PP_ENUM_SHIFTED(N, STREAMS_BOOST_PROTO_DEFINE_LAMBDA_MATCHES, ~)
                >
            {};
            ...
            struct matches_< proto::expr<Tag, Args1, N>, proto::expr<Tag, Args2, N> >
              : STREAMS_BOOST_PP_CAT(and_, N)<
                    STREAMS_BOOST_PROTO_MATCHES_N_FUN(~, 0, ~)::value,
                    STREAMS_BOOST_PP_ENUM_SHIFTED(N, STREAMS_BOOST_PROTO_MATCHES_N_FUN, ~)
                >
            {};
