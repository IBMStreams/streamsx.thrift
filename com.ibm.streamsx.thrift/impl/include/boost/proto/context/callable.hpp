    ...
    #ifndef STREAMS_BOOST_PROTO_CONTEXT_CALLABLE_HPP_EAN_06_23_2007
    #define STREAMS_BOOST_PROTO_CONTEXT_CALLABLE_HPP_EAN_06_23_2007
    ...
    #include <streams_boost/proto/traits.hpp> // for child_c
    ...
    #define STREAMS_BOOST_PROTO_CHILD_N_TYPE(Z, N, Expr)                                                    \
        typedef typename proto::result_of::child_c<Expr const &, N>::type STREAMS_BOOST_PP_CAT(child, N);   \
        /**/

    #define STREAMS_BOOST_PROTO_CHILD_N(Z, N, expr)                                                         \
        proto::child_c<N>(expr)                                                                     \
        /**/

    #define STREAMS_BOOST_PP_ITERATION_PARAMS_1                                                             \
        (3, (1, STREAMS_BOOST_PROTO_MAX_ARITY, <streams_boost/proto/context/callable.hpp>))                         \
        /**/

    #include STREAMS_BOOST_PP_ITERATE()

    #undef STREAMS_BOOST_PROTO_CHILD_N_TYPE
    #undef STREAMS_BOOST_PROTO_CHILD_N
    ...
    #define N STREAMS_BOOST_PP_ITERATION()
                ...
                (
                    ...
                    sizeof(
                        detail::check_is_expr_handled(
                            (sctx_(
                                stag_
                                STREAMS_BOOST_PP_ENUM_TRAILING(N, STREAMS_BOOST_PROTO_CHILD_N, sexpr_)
                            ), 0)
                        )
                )));
        ...
        namespace context
        {
            ...
            {
                STREAMS_BOOST_PP_REPEAT(N, STREAMS_BOOST_PROTO_CHILD_N_TYPE, Expr)

                typedef
                    typename streams_boost::result_of<
                        Context(
                            typename Expr::proto_tag
                            STREAMS_BOOST_PP_ENUM_TRAILING_PARAMS(N, child)
                        )
                    >::type
                result_type;
                ...
                result_type operator ()(Expr &expr, Context &context) const
                {
                    return context(
                        typename Expr::proto_tag()
                        STREAMS_BOOST_PP_ENUM_TRAILING(N, STREAMS_BOOST_PROTO_CHILD_N, expr)
                    );
                }
            };
        }
