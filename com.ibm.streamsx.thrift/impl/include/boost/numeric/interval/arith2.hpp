...
#ifndef STREAMS_BOOST_NUMERIC_INTERVAL_ARITH2_HPP
#define STREAMS_BOOST_NUMERIC_INTERVAL_ARITH2_HPP
...
#include <streams_boost/numeric/interval/arith.hpp>
#include <streams_boost/numeric/interval/policies.hpp>
#include <algorithm>
#include <cassert>
...
interval<T, Policies> pow(const interval<T, Policies>& x, int pwr)
{
  ...
  if (interval_lib::user::is_neg(x.upper())) {        // [-2,-1]
    ...
    if (pwr & 1) {   // [-1,1]^1
      ...
      return I(static_cast<T>(0), pow_up(max STREAMS_BOOST_PREVENT_MACRO_SUBSTITUTION(static_cast<T>(-x.lower()), x.upper()), pwr, rnd), true);
    }
  } else {                                // [1,2]
...
#endif // STREAMS_BOOST_NUMERIC_INTERVAL_ARITH2_HPP
