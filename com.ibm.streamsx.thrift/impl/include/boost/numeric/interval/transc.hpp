...
#ifndef STREAMS_BOOST_NUMERIC_INTERVAL_TRANSC_HPP
#define STREAMS_BOOST_NUMERIC_INTERVAL_TRANSC_HPP
...
#include <streams_boost/numeric/interval/constants.hpp>
#include <streams_boost/numeric/interval/arith.hpp>
#include <streams_boost/numeric/interval/arith2.hpp>
#include <algorithm>
...
interval<T, Policies> cos(const interval<T, Policies>& x)
{
  ...
  if (u <= interval_lib::constants::pi_lower<T>())
    ...
    return I(static_cast<T>(-1), rnd.cos_up(min STREAMS_BOOST_PREVENT_MACRO_SUBSTITUTION(rnd.sub_down(pi2.lower(), u), l)), true);
  else
...
#endif // STREAMS_BOOST_NUMERIC_INTERVAL_TRANSC_HPP
