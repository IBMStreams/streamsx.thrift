...
#ifndef _STREAMS_BOOST_UBLAS_FUNCTIONAL_
#define _STREAMS_BOOST_UBLAS_FUNCTIONAL_

#include <functional>

#include <streams_boost/numeric/ublas/traits.hpp>
#ifdef STREAMS_BOOST_UBLAS_USE_DUFF_DEVICE
#include <streams_boost/numeric/ublas/detail/duff.hpp>
#endif
#ifdef STREAMS_BOOST_UBLAS_USE_SIMD
#include <streams_boost/numeric/ublas/detail/raw.hpp>
#else
namespace streams_boost { namespace numeric { namespace ublas { namespace raw {
}}}}
#endif
#ifdef STREAMS_BOOST_UBLAS_HAVE_BINDINGS
#include <streams_boost/numeric/bindings/traits/std_vector.hpp>
#include <streams_boost/numeric/bindings/traits/ublas_vector.hpp>
#include <streams_boost/numeric/bindings/traits/ublas_matrix.hpp>
#include <streams_boost/numeric/bindings/atlas/cblas.hpp>
#endif
...
namespace streams_boost { namespace numeric { namespace ublas {
    ...
    struct scalar_identity:
        ...
        static STREAMS_BOOST_UBLAS_INLINE
    ...
    struct scalar_binary_assign_functor {
        ...
        typedef typename type_traits<typename streams_boost::remove_reference<T1>::type>::reference argument1_type;
        typedef typename type_traits<T2>::const_reference argument2_type;
    };
    ...
    struct scalar_binary_swap_functor {
        typedef typename type_traits<typename streams_boost::remove_reference<T1>::type>::reference argument1_type;
        typedef typename type_traits<typename streams_boost::remove_reference<T2>::type>::reference argument2_type;
    };
    ...
    struct vector_norm_2:
        ...
        static STREAMS_BOOST_UBLAS_INLINE
        result_type apply (const vector_expression<E> &e) {
#ifndef STREAMS_BOOST_UBLAS_SCALED_NORM
            real_type t = real_type ();
            typedef typename E::size_type vector_size_type;
            vector_size_type size (e ().size ());
            for (vector_size_type i = 0; i < size; ++ i) {
    ...
    struct vector_inner_prod:
        ...
        result_type apply (const vector_container<C1> &c1,
            ...
            vector_size_type size (STREAMS_BOOST_UBLAS_SAME (c1 ().size (), c2 ().size ()));
            ...
            return t;
#elif defined(STREAMS_BOOST_UBLAS_HAVE_BINDINGS)
            return streams_boost::numeric::bindings::atlas::dot (c1 (), c2 ());
#else
        ...
        result_type apply (const vector_expression<E1> &e1,
            ...
            vector_size_type size (STREAMS_BOOST_UBLAS_SAME (e1 ().size (), e2 ().size ()));
            result_type t = result_type (0);
#ifndef STREAMS_BOOST_UBLAS_USE_DUFF_DEVICE
            for (vector_size_type i = 0; i < size; ++ i)
                t += e1 () (i) * e2 () (i);
#else
    ...
    struct matrix_vector_prod1:
        ...
        result_type apply (const matrix_container<C1> &c1,
...
#ifdef STREAMS_BOOST_UBLAS_USE_SIMD
            using namespace raw;
            size_type size = STREAMS_BOOST_UBLAS_SAME (c1 ().size2 (), c2 ().size ());
            ...
            return t;
#elif defined(STREAMS_BOOST_UBLAS_HAVE_BINDINGS)
            return streams_boost::numeric::bindings::atlas::dot (c1 ().row (i), c2 ());
#else
        ...
        result_type apply (const matrix_expression<E1> &e1,
                           const vector_expression<E2> &e2,
                           size_type i) {
            size_type size = STREAMS_BOOST_UBLAS_SAME (e1 ().size2 (), e2 ().size ());
            result_type t = result_type (0);
#ifndef STREAMS_BOOST_UBLAS_USE_DUFF_DEVICE
            for (size_type j = 0; j < size; ++ j)
                t += e1 () (i, j) * e2 () (j);
    ...
    struct matrix_vector_prod2:
        ...
        result_type apply (const vector_container<C1> &c1,
...
#ifdef STREAMS_BOOST_UBLAS_USE_SIMD
            using namespace raw;
            size_type size = STREAMS_BOOST_UBLAS_SAME (c1 ().size (), c2 ().size1 ());
            ...
            return t;
#elif defined(STREAMS_BOOST_UBLAS_HAVE_BINDINGS)
            return streams_boost::numeric::bindings::atlas::dot (c1 (), c2 ().column (i));
#else
        ...
        result_type apply (const vector_expression<E1> &e1,
                           ...
                           size_type i) {
            size_type size = STREAMS_BOOST_UBLAS_SAME (e1 ().size (), e2 ().size1 ());
            result_type t = result_type (0);
#ifndef STREAMS_BOOST_UBLAS_USE_DUFF_DEVICE
            for (size_type j = 0; j < size; ++ j)
                t += e1 () (j) * e2 () (j, i);
    ...
    struct matrix_matrix_prod:
        ...
        result_type apply (const matrix_container<C1> &c1,
...
#ifdef STREAMS_BOOST_UBLAS_USE_SIMD
            using namespace raw;
            size_type size = STREAMS_BOOST_UBLAS_SAME (c1 ().size2 (), c2 ().sizc1 ());
            const typename M1::value_type *data1 = data_const (c1 ()) + i * stride1 (c1 ());
            ...
            return t;
#elif defined(STREAMS_BOOST_UBLAS_HAVE_BINDINGS)
            return streams_boost::numeric::bindings::atlas::dot (c1 ().row (i), c2 ().column (j));
#else
        ...
        result_type apply (const matrix_expression<E1> &e1,
                           const matrix_expression<E2> &e2,
                           size_type i, size_type j) {
            size_type size = STREAMS_BOOST_UBLAS_SAME (e1 ().size2 (), e2 ().size1 ());
            result_type t = result_type (0);
#ifndef STREAMS_BOOST_UBLAS_USE_DUFF_DEVICE
            for (size_type k = 0; k < size; ++ k)
                t += e1 () (i, k) * e2 () (k, j);
    ...
    struct basic_row_major {
        ...
        size_type storage_size (size_type size_i, size_type size_j) {
            ...
            STREAMS_BOOST_UBLAS_CHECK (size_j == 0 || size_i <= (std::numeric_limits<size_type>::max) () / size_j, bad_size ());
        ...
        size_type element (size_type i, size_type size_i, size_type j, size_type size_j) {
            STREAMS_BOOST_UBLAS_CHECK (i < size_i, bad_index ());
            STREAMS_BOOST_UBLAS_CHECK (j < size_j, bad_index ());
            detail::ignore_unused_variable_warning(size_i);
            // Guard against size_type overflow
            STREAMS_BOOST_UBLAS_CHECK (i <= ((std::numeric_limits<size_type>::max) () - j) / size_j, bad_index ());
            return i * size_j + j;
        }
        static
        STREAMS_BOOST_UBLAS_INLINE
        size_type address (size_type i, size_type size_i, size_type j, size_type size_j) {
            ...
            STREAMS_BOOST_UBLAS_CHECK (size_j == 0 || i <= ((std::numeric_limits<size_type>::max) () - j) / size_j, bad_index ());
            detail::ignore_unused_variable_warning(size_i);
            return i * size_j + j;
        }
        ...
        size_type triangular_size (size_type size_i, size_type size_j) {
            ...
            STREAMS_BOOST_UBLAS_CHECK (size == 0 || size / 2 < (std::numeric_limits<size_type>::max) () / size /* +1/2 */, bad_size ());
        ...
        size_type lower_element (size_type i, size_type size_i, size_type j, size_type size_j) {
            STREAMS_BOOST_UBLAS_CHECK (i < size_i, bad_index ());
            STREAMS_BOOST_UBLAS_CHECK (j < size_j, bad_index ());
            STREAMS_BOOST_UBLAS_CHECK (i >= j, bad_index ());
            detail::ignore_unused_variable_warning(size_i);
    ...
    struct basic_column_major {
        ...
        size_type storage_size (size_type size_i, size_type size_j) {
            // Guard against size_type overflow
            STREAMS_BOOST_UBLAS_CHECK (size_i == 0 || size_j <= (std::numeric_limits<size_type>::max) () / size_i, bad_size ());
            return size_i * size_j;
        }
        ...
        size_type element (size_type i, size_type size_i, size_type j, size_type size_j) {
            ...
            detail::ignore_unused_variable_warning(size_j);
            // Guard against size_type overflow
            STREAMS_BOOST_UBLAS_CHECK (j <= ((std::numeric_limits<size_type>::max) () - i) / size_i, bad_index ());
            return i + j * size_i;
        }
        static
        STREAMS_BOOST_UBLAS_INLINE
        size_type address (size_type i, size_type size_i, size_type j, size_type size_j) {
            ...
            STREAMS_BOOST_UBLAS_CHECK (size_i == 0 || j <= ((std::numeric_limits<size_type>::max) () - i) / size_i, bad_index ());
            return i + j * size_i;
        }
    ...
    struct basic_unit_lower : public basic_lower<Z> {
        ...
        size_type packed_size (L, size_type size_i, size_type size_j) {
            ...
            STREAMS_BOOST_UBLAS_CHECK (size_i != 0 && size_j != 0, bad_index ());
        ...
        size_type element (L, size_type i, size_type size_i, size_type j, size_type size_j) {
            // Zero size strict triangles are bad at this point
            STREAMS_BOOST_UBLAS_CHECK (size_i != 0 && size_j != 0 && i != 0, bad_index ());
        ...
        STREAMS_BOOST_UBLAS_INLINE
        size_type global_mutable_restrict2 (size_type index1, size_type size1, size_type index2, size_type size2) {
            STREAMS_BOOST_UBLAS_CHECK( size2 >= 1 , external_logic() );
