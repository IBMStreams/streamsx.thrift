...
#ifndef _STREAMS_BOOST_UBLAS_MATRIX_SPARSE_
#define _STREAMS_BOOST_UBLAS_MATRIX_SPARSE_

#include <streams_boost/numeric/ublas/vector_sparse.hpp>
#include <streams_boost/numeric/ublas/matrix_expression.hpp>
#include <streams_boost/numeric/ublas/detail/matrix_assign.hpp>
...
namespace streams_boost { namespace numeric { namespace ublas {
...
#ifdef STREAMS_BOOST_UBLAS_STRICT_MATRIX_SPARSE
...
#ifndef STREAMS_BOOST_UBLAS_STRICT_MATRIX_SPARSE
    ...
    public:
        ...
        void resize (size_type size1, size_type size2, bool preserve = true) {
            ...
            STREAMS_BOOST_UBLAS_CHECK (!preserve, internal_logic ());
            size1_ = size1;
            size2_ = size2;
            data ().clear ();
        }
        ...
        const_pointer find_element (size_type i, size_type j) const {
            ...
            if (it == data ().end ())
                return 0;
            STREAMS_BOOST_UBLAS_CHECK ((*it).first == element, internal_logic ());   // broken map
            return &(*it).second;
        }
        ...
        reference operator () (size_type i, size_type j) {
            ...
            STREAMS_BOOST_UBLAS_CHECK ((ii.first)->first == element, internal_logic ());   // broken map
        ...
        true_reference insert_element (size_type i, size_type j, const_reference t) {
            STREAMS_BOOST_UBLAS_CHECK (!find_element (i, j), bad_index ());        // duplicate element
            const size_type element = layout_type::element (i, size1_, j, size2_);
    ...
    private:
        ...
        true_reference at_element (size_type i, size_type j) {
            ...
            STREAMS_BOOST_UBLAS_CHECK (it != data ().end(), bad_index ());
            STREAMS_BOOST_UBLAS_CHECK ((*it).first == element, internal_logic ());   // broken map
            return it->second;
        }
    ...
    public:
        ...
        // Element lookup
        // STREAMS_BOOST_UBLAS_INLINE This function seems to be big. So we do not let the compiler inline it.    
        ...
        public:
            ...
            const_reference operator * () const {
                STREAMS_BOOST_UBLAS_CHECK (index1 () < (*this) ().size1 (), bad_index ());
                STREAMS_BOOST_UBLAS_CHECK (index2 () < (*this) ().size2 (), bad_index ());
                if (rank_ == 1) {
                    return (*it_).second;
                } else {
                    return (*this) () (i_, j_);
...
#ifndef STREAMS_BOOST_UBLAS_NO_NESTED_CLASS_RELATION
            STREAMS_BOOST_UBLAS_INLINE
#ifdef STREAMS_BOOST_UBLAS_MSVC_NESTED_CLASS_RELATION
            typename self_type::
#endif
            ...
            size_type index1 () const {
                STREAMS_BOOST_UBLAS_CHECK (*this != (*this) ().find1 (0, (*this) ().size1 (), j_), bad_index ());
                if (rank_ == 1) {
                    const self_type &m = (*this) ();
                    STREAMS_BOOST_UBLAS_CHECK (layout_type::index_i ((*it_).first, m.size1 (), m.size2 ()) < (*this) ().size1 (), bad_index ());
            ...
            size_type index2 () const {
                if (rank_ == 1) {
                    const self_type &m = (*this) ();
                    STREAMS_BOOST_UBLAS_CHECK (layout_type::index_j ((*it_).first, m.size1 (), m.size2 ()) < (*this) ().size2 (), bad_index ());
            ...
            bool operator == (const const_iterator1 &it) const {
                ...
                // STREAMS_BOOST_UBLAS_CHECK (rank_ == it.rank_, internal_logic ());
        ...
        public:
            ...
            STREAMS_BOOST_UBLAS_INLINE
            size_type index2 () const {
                STREAMS_BOOST_UBLAS_CHECK (*this != (*this) ().find2 (0, i_, (*this) ().size2 ()), bad_index ());
                if (rank_ == 1) {
                    const self_type &m = (*this) ();
                    STREAMS_BOOST_UBLAS_CHECK (layout_type::index_j ((*it_).first, m.size1 (), m.size2 ()) < (*this) ().size2 (), bad_index ());
                    return layout_type::index_j ((*it_).first, m.size1 (), m.size2 ());
    ...
    public:
        ...
        const_pointer find_element (size_type i, size_type j) const {
            ...
            if (itv == data ().end ())
                return 0;
            STREAMS_BOOST_UBLAS_CHECK ((*itv).first == element1, internal_logic ());   // broken map
            const_subiterator_type it ((*itv).second.find (element2));
            if (it == (*itv).second.end ())
                return 0;
            STREAMS_BOOST_UBLAS_CHECK ((*it).first == element2, internal_logic ());   // broken map
            return &(*it).second;
        }
        ...
        reference operator () (size_type i, size_type j) {
            ...
            STREAMS_BOOST_UBLAS_CHECK ((ii.first)->first == element2, internal_logic ());   // broken map
            return (ii.first)->second;
#else
            return reference (*this, i, j);
#endif
    ...
    private:
        ...
        true_reference at_element (size_type i, size_type j) {
            ...
            STREAMS_BOOST_UBLAS_CHECK (itv != data ().end(), bad_index ());
            STREAMS_BOOST_UBLAS_CHECK ((*itv).first == element1, internal_logic ());   // broken map
            subiterator_type it ((*itv).second.find (element2));
            STREAMS_BOOST_UBLAS_CHECK (it != (*itv).second.end (), bad_index ());
            STREAMS_BOOST_UBLAS_CHECK ((*it).first == element2, internal_logic ());    // broken map
    ...
    public:
        ...
        // STREAMS_BOOST_UBLAS_INLINE This function seems to be big. So we do not let the compiler inline it.    
        const_iterator1 find1 (int rank, size_type i, size_type j, int direction = 1) const {
            STREAMS_BOOST_UBLAS_CHECK (data ().begin () != data ().end (), internal_logic ());
            for (;;) {
        ...
        public:
            ...
            size_type index1 () const {
                STREAMS_BOOST_UBLAS_CHECK (*this != (*this) ().find1 (0, (*this) ().size1 (), j_), bad_index ());
                if (rank_ == 1) {
                    STREAMS_BOOST_UBLAS_CHECK (layout_type::index_M ((*itv_).first, (*it_).first) < (*this) ().size1 (), bad_index ());
            ...
            size_type index2 () const {
                if (rank_ == 1) {
                    STREAMS_BOOST_UBLAS_CHECK (layout_type::index_m ((*itv_).first, (*it_).first) < (*this) ().size2 (), bad_index ());
                    return layout_type::index_m ((*itv_).first, (*it_).first);
                } else {
                    return j_;
                }
    ...
    public:
        ...
        true_reference insert_element (size_type i, size_type j, const_reference t) {
            ...
            STREAMS_BOOST_UBLAS_CHECK (filled2_ < capacity_, internal_logic ());
            size_type element1 = layout_type::index_M (i, j);
            size_type element2 = layout_type::index_m (i, j);
            ...
            STREAMS_BOOST_UBLAS_CHECK (it == it_end || *it != k_based (element2), internal_logic ());   // duplicate bound by lower_bound
            ++ filled2_;
        ...
        void push_back (size_type i, size_type j, const_reference t) {
            ...
            STREAMS_BOOST_UBLAS_CHECK ((filled1_ == element1 + 2 &&
        ...
        STREAMS_BOOST_UBLAS_INLINE
        void pop_back () {
            STREAMS_BOOST_UBLAS_CHECK (filled1_ > 0 && filled2_ > 0, external_logic ());
            -- filled2_;
            while (index1_data_ [filled1_ - 2] > k_based (filled2_)) {
                index1_data_ [filled1_ - 1] = 0;
                -- filled1_;
    ...
    private:
        ...
        true_reference at_element (size_type i, size_type j) {
            pointer p = find_element (i, j);
            STREAMS_BOOST_UBLAS_CHECK (p, bad_index ());
            return *p;
        }
    ...
    public:
        ...
        public:
            ...
            size_type index1 () const {
                STREAMS_BOOST_UBLAS_CHECK (*this != (*this) ().find1 (0, (*this) ().size1 (), j_), bad_index ());
                if (rank_ == 1) {
                    STREAMS_BOOST_UBLAS_CHECK (layout_type::index_M (itv_ - (*this) ().index1_data_.begin (), (*this) ().zero_based (*it_)) < (*this) ().size1 (), bad_index ());
            ...
            size_type index2 () const {
                if (rank_ == 1) {
                    STREAMS_BOOST_UBLAS_CHECK (layout_type::index_m (itv_ - (*this) ().index1_data_.begin (), (*this) ().zero_based (*it_)) < (*this) ().size2 (), bad_index ());
                    return layout_type::index_m (itv_ - (*this) ().index1_data_.begin (), (*this) ().zero_based (*it_));
                } else {
                    return j_;
                }
    ...
    private:
        void storage_invariants () const {
            STREAMS_BOOST_UBLAS_CHECK (layout_type::size_M (size1_, size2_) + 1 == index1_data_.size (), internal_logic ());
            STREAMS_BOOST_UBLAS_CHECK (capacity_ == index2_data_.size (), internal_logic ());
            STREAMS_BOOST_UBLAS_CHECK (capacity_ == value_data_.size (), internal_logic ());
            STREAMS_BOOST_UBLAS_CHECK (filled1_ > 0 && filled1_ <= layout_type::size_M (size1_, size2_) + 1, internal_logic ());
            STREAMS_BOOST_UBLAS_CHECK (filled2_ <= capacity_, internal_logic ());
            STREAMS_BOOST_UBLAS_CHECK (index1_data_ [filled1_ - 1] == k_based (filled2_), internal_logic ());
        }
    ...
    public:
        ...
        void append_element (size_type i, size_type j, const_reference t) {
            ...
            STREAMS_BOOST_UBLAS_CHECK (filled_ < capacity_, internal_logic ());
            size_type element1 = layout_type::index_M (i, j);
            size_type element2 = layout_type::index_m (i, j);
        ...
        coordinate_matrix &operator = (const coordinate_matrix &m) {
            if (this != &m) {
                ...
                index2_data_ = m.index2_data_;
                value_data_ = m.value_data_;
                STREAMS_BOOST_UBLAS_CHECK (capacity_ == index1_data_.size (), internal_logic ());
                STREAMS_BOOST_UBLAS_CHECK (capacity_ == index2_data_.size (), internal_logic ());
                STREAMS_BOOST_UBLAS_CHECK (capacity_ == value_data_.size (), internal_logic ());
            }
            storage_invariants ();
        ...
        void push_back (size_type i, size_type j, const_reference t) {
            ...
            size_type element2 = layout_type::index_m (i, j);
            // must maintain sort order
            STREAMS_BOOST_UBLAS_CHECK (sorted_ && 
                    (filled_ == 0 ||
        ...
        void pop_back () {
            ...
            STREAMS_BOOST_UBLAS_CHECK (filled_ > 0, external_logic ());
            -- filled_;
    ...
    public:
        ...
        public:
            ...
            size_type index1 () const {
                STREAMS_BOOST_UBLAS_CHECK (*this != (*this) ().find1 (0, (*this) ().size1 (), j_), bad_index ());
                if (rank_ == 1) {
                    STREAMS_BOOST_UBLAS_CHECK (layout_type::index_M ((*this) ().zero_based (*itv_), (*this) ().zero_based (*it_)) < (*this) ().size1 (), bad_index ());
            ...
            size_type index2 () const {
                if (rank_ == 1) {
                    STREAMS_BOOST_UBLAS_CHECK (layout_type::index_m ((*this) ().zero_based (*itv_), (*this) ().zero_based (*it_)) < (*this) ().size2 (), bad_index ());
                    return layout_type::index_m ((*this) ().zero_based (*itv_), (*this) ().zero_based (*it_));
                } else {
                    return j_;
                }
    ...
    private:
        void storage_invariants () const
        {
            ...
            STREAMS_BOOST_UBLAS_CHECK (capacity_ == value_data_.size (), internal_logic ());
            STREAMS_BOOST_UBLAS_CHECK (filled_ <= capacity_, internal_logic ());
            STREAMS_BOOST_UBLAS_CHECK (sorted_filled_ <= filled_, internal_logic ());
            STREAMS_BOOST_UBLAS_CHECK (sorted_ == (sorted_filled_ == filled_), internal_logic ());
        }
