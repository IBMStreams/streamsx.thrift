...
#ifndef STREAMS_BOOST_UBLAS_STORAGE_H
#define STREAMS_BOOST_UBLAS_STORAGE_H

#include <algorithm>
#ifdef STREAMS_BOOST_UBLAS_SHALLOW_ARRAY_ADAPTOR
...
#include <streams_boost/numeric/ublas/exception.hpp>
...
namespace streams_boost { namespace numeric { namespace ublas {
    ...
    public:
        ...
        bounded_array (size_type size):
            size_ (size) /*, data_ ()*/ {
            STREAMS_BOOST_UBLAS_CHECK (size_ <= N, bad_size ());
        ...
        STREAMS_BOOST_UBLAS_INLINE
        void resize (size_type size) {
            STREAMS_BOOST_UBLAS_CHECK (size <= N, bad_size ());
            size_ = size;
        }
        STREAMS_BOOST_UBLAS_INLINE
        void resize (size_type size, value_type init) {
    ...
    private:
        size_type size_;
        STREAMS_BOOST_UBLAS_BOUNDED_ARRAY_ALIGN value_type data_ [N];
    };
    ...
    public:
        ...
        basic_range (size_type start, size_type stop):
            ...
            STREAMS_BOOST_UBLAS_CHECK (start_ <= stop, bad_index ());
        }
    ...
    public:
        ...
        public:
            ...
            STREAMS_BOOST_UBLAS_INLINE
            const_iterator &operator -- () {
                STREAMS_BOOST_UBLAS_CHECK (it_ > 0, bad_index ());
                -- it_;
                return *this;
            }
            STREAMS_BOOST_UBLAS_INLINE
            const_iterator &operator += (difference_type n) {
                STREAMS_BOOST_UBLAS_CHECK (n >= 0 || it_ >= size_type(-n), bad_index ());
                it_ += n;
                return *this;
            }
            STREAMS_BOOST_UBLAS_INLINE
            ...
            const_reference operator * () const {
                STREAMS_BOOST_UBLAS_CHECK ((*this) ().start () <= it_, bad_index ());
                STREAMS_BOOST_UBLAS_CHECK (it_ < (*this) ().start () + (*this) ().size (), bad_index ());
                return it_;
            }
    ...
    public:
        ...
        const_reference operator () (size_type i) const {
            STREAMS_BOOST_UBLAS_CHECK (i < size_, bad_index ());
            STREAMS_BOOST_UBLAS_CHECK (stride_ >= 0 || start_ >= i * -stride_, bad_index ());
        ...
        basic_slice compose (const basic_range<size_type, difference_type> &r) const {
            STREAMS_BOOST_UBLAS_CHECK (stride_ >=0 || start_ >= -stride_ * r.start(), bad_index ());
            return basic_slice (start_ + stride_ * r.start (), stride_, r.size ());
        }
        STREAMS_BOOST_UBLAS_INLINE
        basic_slice compose (const basic_slice &s) const {
            STREAMS_BOOST_UBLAS_CHECK (stride_ >=0 || start_ >= -stride_ * s.start_, bad_index ());
    ...
    public:
        ...
        indirect_array compose (const basic_range<size_type, difference_type> &r) const {
            STREAMS_BOOST_UBLAS_CHECK (r.start () + r.size () <= size_, bad_size ());
        ...
        indirect_array compose (const basic_slice<size_type, difference_type> &s) const {
            STREAMS_BOOST_UBLAS_CHECK (s.start () + s.stride () * (s.size () - (s.size () > 0)) <= size (), bad_size ());
        ...
        indirect_array compose (const indirect_array &ia) const {
            ...
            for (size_type i = 0; i < ia.size_; ++ i) {
                STREAMS_BOOST_UBLAS_CHECK (ia.data_ [i] <= size_, bad_size ());
        ...
        bool operator == (const indirect_array<OA> &ia) const {
            if (size_ != ia.size_)
                return false;
            for (size_type i = 0; i < STREAMS_BOOST_UBLAS_SAME (size_, ia.size_); ++ i)
