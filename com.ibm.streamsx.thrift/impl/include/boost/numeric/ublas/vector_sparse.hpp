...
#ifndef _STREAMS_BOOST_UBLAS_VECTOR_SPARSE_
#define _STREAMS_BOOST_UBLAS_VECTOR_SPARSE_

#include <streams_boost/numeric/ublas/storage_sparse.hpp>
...
namespace streams_boost { namespace numeric { namespace ublas {
...
#ifdef STREAMS_BOOST_UBLAS_STRICT_VECTOR_SPARSE
    ...
    public:
        ...
        typedef const value_type &const_reference;
#ifndef STREAMS_BOOST_UBLAS_STRICT_VECTOR_SPARSE
    ...
    public:
        ...
        const_pointer find_element (size_type i) const {
            ...
            if (it == data ().end ())
                return 0;
            STREAMS_BOOST_UBLAS_CHECK ((*it).first == i, internal_logic ());   // broken map
            return &(*it).second;
        }
        ...
        true_reference ref (size_type i) {
            ...
            STREAMS_BOOST_UBLAS_CHECK ((ii.first)->first == i, internal_logic ());   // broken map
        ...
        true_reference insert_element (size_type i, const_reference t) {
            ...
            STREAMS_BOOST_UBLAS_CHECK (ii.second, bad_index ());        // duplicate element
            STREAMS_BOOST_UBLAS_CHECK ((ii.first)->first == i, internal_logic ());   // broken map
    ...
    public:
        ...
        public:
            ...
            size_type index () const {
                STREAMS_BOOST_UBLAS_CHECK (*this != (*this) ().end (), bad_index ());
                STREAMS_BOOST_UBLAS_CHECK ((*it_).first < (*this) ().size (), bad_index ());
                return (*it_).first;
            }
    ...
    public:
        ...
        STREAMS_BOOST_UBLAS_INLINE
        true_reference insert_element (size_type i, const_reference t) {
            STREAMS_BOOST_UBLAS_CHECK (!find_element (i), bad_index ());        // duplicate element
            ...
            STREAMS_BOOST_UBLAS_CHECK (filled_ == 0 || filled_ == typename index_array_type::size_type (n) || *it != k_based (i), internal_logic ());   // duplicate found by lower_bound
            ++ filled_;
        ...
        void push_back (size_type i, const_reference t) {
            STREAMS_BOOST_UBLAS_CHECK (filled_ == 0 || index_data_ [filled_ - 1] < k_based (i), external_logic ());
            if (filled_ >= capacity_)
                reserve (2 * capacity_, true);
    ...
    private:
        ...
        true_reference at_element (size_type i) {
            STREAMS_BOOST_UBLAS_CHECK (i < size_, bad_index ());
            subiterator_type it (detail::lower_bound (index_data_.begin (), index_data_.begin () + filled_, k_based (i), std::less<size_type> ()));
            STREAMS_BOOST_UBLAS_CHECK (it != index_data_.begin () + filled_ && *it == k_based (i), bad_index ());
            return value_data_ [it - index_data_.begin ()];
        }
    ...
    public:
        ...
        public:
            ...
            size_type index () const {
                STREAMS_BOOST_UBLAS_CHECK (*this != (*this) ().end (), bad_index ());
                STREAMS_BOOST_UBLAS_CHECK ((*this) ().zero_based (*it_) < (*this) ().size (), bad_index ());
    ...
    private:
        ...
        {
            STREAMS_BOOST_UBLAS_CHECK (capacity_ == index_data_.size (), internal_logic ());
            STREAMS_BOOST_UBLAS_CHECK (capacity_ == value_data_.size (), internal_logic ());
            STREAMS_BOOST_UBLAS_CHECK (filled_ <= capacity_, internal_logic ());
            STREAMS_BOOST_UBLAS_CHECK ((0 == filled_) || (zero_based(index_data_[filled_ - 1]) < size_), internal_logic ());
        }
    ...
    public:
        ...
        void push_back (size_type i, const_reference t) {
            ...
            STREAMS_BOOST_UBLAS_CHECK (sorted_ && (filled_ == 0 || index_data_ [filled_ - 1] < k_based (i)), external_logic ());
            if (filled_ >= capacity_)
                reserve (2 * filled_, true);
            STREAMS_BOOST_UBLAS_CHECK (filled_ < capacity_, internal_logic ());
            index_data_ [filled_] = k_based (i);
