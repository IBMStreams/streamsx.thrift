...
#ifndef _STREAMS_BOOST_UBLAS_STORAGE_SPARSE_
#define _STREAMS_BOOST_UBLAS_STORAGE_SPARSE_
...
namespace streams_boost { namespace numeric { namespace ublas {
...
#ifdef STREAMS_BOOST_UBLAS_STRICT_MAP_ARRAY
    template<class A>
    ...
    public:
        ...
        ~sparse_storage_element () {
            if (dirty_) {
                if (! it_)
                    it_ = (*this) ().find (i_);
                STREAMS_BOOST_UBLAS_CHECK (it_ != (*this) ().end (), internal_logic ());
                it_->second = d_;
            }
        }
    ...
    public:
        ...
        void serialize(Archive & ar, const unsigned int /* file_version */){
            ar & serialization::make_nvp("base", streams_boost::serialization::base_object< std::map<I, T /*, ALLOC */> >(*this));
        }
    };
...
#ifndef STREAMS_BOOST_UBLAS_STRICT_MAP_ARRAY
    ...
    private:
        ...
        void resize (size_type size) {
            STREAMS_BOOST_UBLAS_CHECK (size_ <= capacity_, internal_logic ());
            if (size > capacity_) {
                const size_type capacity = size << 1;
                STREAMS_BOOST_UBLAS_CHECK (capacity, internal_logic ());
    ...
    public:
        ...
        void reserve (size_type capacity) {
            ...
            STREAMS_BOOST_UBLAS_CHECK (capacity >= size_, bad_size ());
            pointer data;
            if (capacity) {
        ...
        data_reference operator [] (key_type i) {
            ...
            STREAMS_BOOST_UBLAS_CHECK (it != end (), internal_logic ());
            return it->second;
#else
        ...
        void erase (iterator it) {
            STREAMS_BOOST_UBLAS_CHECK (begin () <= it && it < end (), bad_index ());
        ...
        void erase (iterator it1, iterator it2) {
            ...
            STREAMS_BOOST_UBLAS_CHECK (begin () <= it1 && it1 < it2 && it2 <= end (), bad_index ());
