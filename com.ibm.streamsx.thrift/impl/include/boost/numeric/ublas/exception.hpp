...
#ifndef _STREAMS_BOOST_UBLAS_EXCEPTION_
#define _STREAMS_BOOST_UBLAS_EXCEPTION_

#if ! defined (STREAMS_BOOST_NO_EXCEPTIONS) && ! defined (STREAMS_BOOST_UBLAS_NO_EXCEPTIONS)
#include <stdexcept>
#else
#include <cstdlib>
#endif
#ifndef STREAMS_BOOST_UBLAS_NO_STD_CERR
#include <iostream>
#endif
...
#if STREAMS_BOOST_UBLAS_CHECK_ENABLE
...
//    }
#ifndef STREAMS_BOOST_UBLAS_NO_STD_CERR
#define STREAMS_BOOST_UBLAS_CHECK_FALSE(e) \
    std::cerr << "Check failed in file " << __FILE__ << " at line " << __LINE__ << ":" << std::endl; \
    e.raise ();
#define STREAMS_BOOST_UBLAS_CHECK(expression, e) \
    if (! (expression)) { \
        std::cerr << "Check failed in file " << __FILE__ << " at line " << __LINE__ << ":" << std::endl; \
        std::cerr << #expression << std::endl; \
        e.raise (); \
    }
#define STREAMS_BOOST_UBLAS_CHECK_EX(expression, file, line, e) \
    if (! (expression)) { \
...
#ifndef STREAMS_BOOST_UBLAS_USE_FAST_SAME
// Macro is equivilent to 
//    template<class T>
//    STREAMS_BOOST_UBLAS_INLINE
//    const T &same_impl (const T &size1, const T &size2) {
//        STREAMS_BOOST_UBLAS_CHECK (size1 == size2, bad_argument ());
//        return (std::min) (size1, size2);
//    }
// #define STREAMS_BOOST_UBLAS_SAME(size1, size2) same_impl ((size1), (size2))
    ...
    T1 same_impl_ex (const T1 &size1, const T2 &size2, const char *file, int line) {
        STREAMS_BOOST_UBLAS_CHECK_EX (size1 == size2, file, line, bad_argument ());
...
#define STREAMS_BOOST_UBLAS_SAME(size1, size2) same_impl_ex ((size1), (size2), __FILE__, __LINE__)
#else
// Macros are equivilent to 
//    template<class T>
//    STREAMS_BOOST_UBLAS_INLINE
...
//    }
// #define STREAMS_BOOST_UBLAS_SAME(size1, size2) same_impl ((size1), (size2))
#define STREAMS_BOOST_UBLAS_SAME(size1, size2) (size1)
#endif
