...
#ifndef _STREAMS_BOOST_UBLAS_VECTOR_ASSIGN_
#define _STREAMS_BOOST_UBLAS_VECTOR_ASSIGN_

#include <streams_boost/numeric/ublas/functional.hpp> // scalar_assign
...
namespace detail {
    ...
    void make_conformant (V &v, const vector_expression<E> &e) {
        STREAMS_BOOST_UBLAS_CHECK (v.size () == e ().size (), bad_size ());
    ...
    void iterating_vector_assign_scalar (V &v, const T &t) {
        ...
        typename V::iterator it (v.begin ());
        STREAMS_BOOST_UBLAS_CHECK (v.end () - it == size, bad_size ());
    ...
    void iterating_vector_assign (V &v, const vector_expression<E> &e) {
        ...
        typedef typename V::difference_type difference_type;
        difference_type size (STREAMS_BOOST_UBLAS_SAME (v.size (), e ().size ()));
        typename V::iterator it (v.begin ());
        STREAMS_BOOST_UBLAS_CHECK (v.end () - it == size, bad_size ());
        typename E::const_iterator ite (e ().begin ());
        STREAMS_BOOST_UBLAS_CHECK (e ().end () - ite == size, bad_size ());
#ifndef STREAMS_BOOST_UBLAS_USE_DUFF_DEVICE
        while (-- size >= 0)
    ...
    void indexing_vector_assign (V &v, const vector_expression<E> &e) {
        ...
        typedef typename V::size_type size_type;
        size_type size (STREAMS_BOOST_UBLAS_SAME (v.size (), e ().size ()));
#ifndef STREAMS_BOOST_UBLAS_USE_DUFF_DEVICE
        for (size_type i = 0; i < size; ++ i)
    ...
    void vector_assign (V &v, const vector_expression<E> &e, packed_proxy_tag) {
        ...
        if (! disable_type_check<bool>::value) 
            STREAMS_BOOST_UBLAS_CHECK (detail::expression_type_check (v, cv), 
