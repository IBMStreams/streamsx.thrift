...
#ifndef _STREAMS_BOOST_UBLAS_LU_
#define _STREAMS_BOOST_UBLAS_LU_

#include <streams_boost/numeric/ublas/operation.hpp>
#include <streams_boost/numeric/ublas/vector_proxy.hpp>
#include <streams_boost/numeric/ublas/matrix_proxy.hpp>
...
namespace streams_boost { namespace numeric { namespace ublas {
...
#if STREAMS_BOOST_UBLAS_TYPE_CHECK
        STREAMS_BOOST_UBLAS_CHECK (singular != 0 ||
    ...
    typename M::size_type lu_factorize (M &m, PM &pm) {
        ...
        for (size_type i = 0; i < size; ++ i) {
            ...
            STREAMS_BOOST_UBLAS_CHECK (i_norm_inf < size1, external_logic ());
            if (m (i_norm_inf, i) != value_type/*zero*/()) {
                if (i_norm_inf != i) {
                    pm (i) = i_norm_inf;
                    row (m, i_norm_inf).swap (mri);
                } else {
                    STREAMS_BOOST_UBLAS_CHECK (pm (i) == i_norm_inf, external_logic ());
                }
                value_type m_inv = value_type (1) / m (i, i);
                project (mci, range (i + 1, size1)) *= m_inv;
            } else if (singular == 0) {
    ...
    typename M::size_type axpy_lu_factorize (M &m, PM &pm) {
        ...
        size_type size2 = m.size2 ();
        size_type size = (std::min) (size1, size2);
#ifndef STREAMS_BOOST_UBLAS_LU_WITH_INPLACE_SOLVE
        matrix_type mr (m);
...
#if STREAMS_BOOST_UBLAS_TYPE_CHECK
        STREAMS_BOOST_UBLAS_CHECK (detail::expression_type_check (prod (triangular_adaptor<const_matrix_type, unit_lower> (m), e), cv1), internal_logic ());
        vector_type cv2 (e);
#endif
        inplace_solve (m, e, upper_tag ());
#if STREAMS_BOOST_UBLAS_TYPE_CHECK
        STREAMS_BOOST_UBLAS_CHECK (detail::expression_type_check (prod (triangular_adaptor<const_matrix_type, upper> (m), e), cv2), internal_logic ());
#endif
    }
    template<class M, class E>
    void lu_substitute (const M &m, matrix_expression<E> &e) {
        ...
        inplace_solve (m, e, unit_lower_tag ());
#if STREAMS_BOOST_UBLAS_TYPE_CHECK
        STREAMS_BOOST_UBLAS_CHECK (detail::expression_type_check (prod (triangular_adaptor<const_matrix_type, unit_lower> (m), e), cm1), internal_logic ());
        matrix_type cm2 (e);
#endif
        inplace_solve (m, e, upper_tag ());
#if STREAMS_BOOST_UBLAS_TYPE_CHECK
        STREAMS_BOOST_UBLAS_CHECK (detail::expression_type_check (prod (triangular_adaptor<const_matrix_type, upper> (m), e), cm2), internal_logic ());
#endif
    }
...
#if STREAMS_BOOST_UBLAS_TYPE_CHECK
        STREAMS_BOOST_UBLAS_CHECK (detail::expression_type_check (prod (e, triangular_adaptor<const_matrix_type, upper> (m)), cv1), internal_logic ());
        vector_type cv2 (e);
#endif
        inplace_solve (e, m, unit_lower_tag ());
#if STREAMS_BOOST_UBLAS_TYPE_CHECK
        STREAMS_BOOST_UBLAS_CHECK (detail::expression_type_check (prod (e, triangular_adaptor<const_matrix_type, unit_lower> (m)), cv2), internal_logic ());
#endif
    }
    template<class E, class M>
    void lu_substitute (matrix_expression<E> &e, const M &m) {
        ...
        inplace_solve (e, m, upper_tag ());
#if STREAMS_BOOST_UBLAS_TYPE_CHECK
        STREAMS_BOOST_UBLAS_CHECK (detail::expression_type_check (prod (e, triangular_adaptor<const_matrix_type, upper> (m)), cm1), internal_logic ());
        matrix_type cm2 (e);
#endif
        inplace_solve (e, m, unit_lower_tag ());
#if STREAMS_BOOST_UBLAS_TYPE_CHECK
        STREAMS_BOOST_UBLAS_CHECK (detail::expression_type_check (prod (e, triangular_adaptor<const_matrix_type, unit_lower> (m)), cm2), internal_logic ());
#endif
    }
