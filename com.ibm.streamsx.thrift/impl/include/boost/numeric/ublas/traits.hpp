...
#ifndef _STREAMS_BOOST_UBLAS_TRAITS_
#define _STREAMS_BOOST_UBLAS_TRAITS_
...
#include <streams_boost/numeric/ublas/detail/iterator.hpp>
#include <streams_boost/numeric/ublas/detail/returntype_deduction.hpp>
...
namespace {
  template<class T> T streams_boost_numeric_ublas_sqrt (const T& t) {
    using namespace std;
    // we'll find either std::sqrt or else another version via ADL:
    return sqrt (t);
  }
  template<class T> T streams_boost_numeric_ublas_abs (const T& t) {
    using namespace std;
...
namespace streams_boost { namespace numeric { namespace ublas {
    ...
    struct scalar_traits {
        ...
        real_type type_abs (const_reference t) {
            return streams_boost_numeric_ublas_abs (t);
        }
        static
        STREAMS_BOOST_UBLAS_INLINE
        value_type type_sqrt (const_reference t) {
            // force a type conversion back to value_type for intgral types
            return value_type (streams_boost_numeric_ublas_sqrt (t));
        }
        ...
        bool equals (const_reference t1, const_reference t2) {
            return self_type::norm_inf (t1 - t2) < STREAMS_BOOST_UBLAS_TYPE_CHECK_EPSILON *
                   (std::max) ((std::max) (self_type::norm_inf (t1),
                                       self_type::norm_inf (t2)),
                             STREAMS_BOOST_UBLAS_TYPE_CHECK_MIN);
        }
    };
...
#ifdef STREAMS_BOOST_UBLAS_USE_INTERVAL
    // Define scalar interval type traits
    template<>
    struct type_traits<streams_boost::numeric::interval<float> > : scalar_traits<streams_boost::numeric::interval<float> > {
        typedef type_traits<streams_boost::numeric::interval<float> > self_type;
        typedef streams_boost::numeric::interval<float> value_type;
        typedef const value_type &const_reference;
        typedef value_type &reference;
        typedef value_type real_type;
        typedef streams_boost::numeric::interval<double> precision_type;

    };
    template<>
    struct type_traits<streams_boost::numeric::interval<double> > : scalar_traits<streams_boost::numeric::interval<double> > {
        typedef type_traits<streams_boost::numeric::interval<double> > self_type;
        typedef streams_boost::numeric::interval<double> value_type;
        typedef const value_type &const_reference;
        typedef value_type &reference;
        typedef value_type real_type;
        typedef streams_boost::numeric::interval<long double> precision_type;
    };
    template<>
    struct type_traits<streams_boost::numeric::interval<long double> > : scalar_traits<streams_boost::numeric::interval<long double> > {
        typedef type_traits<streams_boost::numeric::interval<long double> > self_type;
        typedef streams_boost::numeric::interval<long double> value_type;
        typedef const value_type &const_reference;
        typedef value_type &reference;
        typedef value_type real_type;
        typedef value_type precision_type;
    ...
    void increment (I &it, const I &it_end) {
        ...
        I cit (it);
        while (cit != it_end) {
            STREAMS_BOOST_UBLAS_CHECK (*cit == typename I::value_type/*zero*/(), internal_logic ());
            ++ cit;
        }
#endif
        it = it_end;
    ...
    namespace detail {
        ...
        template<typename T>
        struct has_trivial_constructor : public streams_boost::has_trivial_constructor<T> {};

        template<typename T>
        struct has_trivial_destructor : public streams_boost::has_trivial_destructor<T> {};

        template<typename FLT>
        struct has_trivial_constructor<std::complex<FLT> > : public streams_boost::true_type {};
        
        template<typename FLT>
        struct has_trivial_destructor<std::complex<FLT> > : public streams_boost::true_type {};
