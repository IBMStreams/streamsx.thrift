...
#ifndef _STREAMS_BOOST_UBLAS_OPERATION_BLOCKED_
#define _STREAMS_BOOST_UBLAS_OPERATION_BLOCKED_

#include <streams_boost/numeric/ublas/traits.hpp>
#include <streams_boost/numeric/ublas/detail/vector_assign.hpp> // indexing_vector_assign
#include <streams_boost/numeric/ublas/detail/matrix_assign.hpp> // indexing_matrix_assign
...
namespace streams_boost { namespace numeric { namespace ublas {
    ...
    block_prod (const matrix_expression<E1> &e1,
        ...
        size_type j_size = STREAMS_BOOST_UBLAS_SAME (e1 ().size2 (), e2 ().size ());
...
#ifdef STREAMS_BOOST_UBLAS_NO_CACHE
...
#ifndef STREAMS_BOOST_UBLAS_NO_CACHE
    ...
    block_prod (const vector_expression<E1> &e1,
        ...
        indexing_vector_assign<scalar_assign> (cv, prod (e1, e2));
#endif
        size_type i_size = STREAMS_BOOST_UBLAS_SAME (e1 ().size (), e2 ().size1 ());
    ...
    block_prod (const matrix_expression<E1> &e1,
        ...
        size_type i_size = e1 ().size1 ();
        size_type j_size = e2 ().size2 ();
        size_type k_size = STREAMS_BOOST_UBLAS_SAME (e1 ().size2 (), e2 ().size1 ());
        for (size_type i_begin = 0; i_begin < i_size; i_begin += block_size) {
            size_type i_end = i_begin + (std::min) (i_size - i_begin, block_size);
            for (size_type j_begin = 0; j_begin < j_size; j_begin += block_size) {
                size_type j_end = j_begin + (std::min) (j_size - j_begin, block_size);
