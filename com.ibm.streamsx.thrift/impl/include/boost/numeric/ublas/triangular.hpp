...
#ifndef _STREAMS_BOOST_UBLAS_TRIANGULAR_
#define _STREAMS_BOOST_UBLAS_TRIANGULAR_
...
namespace streams_boost { namespace numeric { namespace ublas {
    ...
    namespace detail {
        using namespace streams_boost::numeric::ublas;
    ...
    public:
        ...
        const_reference operator () (size_type i, size_type j) const {
            STREAMS_BOOST_UBLAS_CHECK (i < size1_, bad_index ());
            STREAMS_BOOST_UBLAS_CHECK (j < size2_, bad_index ());
        ...
        void swap (triangular_matrix &m) {
            if (this != &m) {
                // STREAMS_BOOST_UBLAS_CHECK (size2_ == m.size2_, bad_size ());
   ...
   private:
    ...
    public:
        ...
        public:
            ...
            const_reference operator * () const {
                size_type i = index1 ();
                size_type j = index2 ();
                STREAMS_BOOST_UBLAS_CHECK (i < (*this) ().size1 (), bad_index ());
                STREAMS_BOOST_UBLAS_CHECK (j < (*this) ().size2 (), bad_index ());
                if (triangular_type::other (i, j))
                    return *it1_;
                else
                    return (*this) () (i, j);
    ...
    : vector_temporary_traits< typename streams_boost::remove_const<M>::type > {} ;
    template <class M, class TRI>
    ...
    : matrix_temporary_traits< typename streams_boost::remove_const<M>::type > {};
    template <class M, class TRI>
    ...
    void inplace_solve (const matrix_expression<E1> &e1, vector_expression<E2> &e2,
        ...
        STREAMS_BOOST_UBLAS_CHECK (e1 ().size1 () == e1 ().size2 (), bad_size ());
        STREAMS_BOOST_UBLAS_CHECK (e1 ().size2 () == e2 ().size (), bad_size ());
        size_type size = e2 ().size ();
        for (size_type n = 0; n < size; ++ n) {
#ifndef STREAMS_BOOST_UBLAS_SINGULAR_CHECK
            STREAMS_BOOST_UBLAS_CHECK (e1 () (n, n) != value_type/*zero*/(), singular ());
#else
    ...
    void inplace_solve (vector_expression<E1> &e1, const matrix_expression<E2> &e2,
        ...
        STREAMS_BOOST_UBLAS_CHECK (e1 ().size () == e2 ().size1 (), bad_size ());
        STREAMS_BOOST_UBLAS_CHECK (e2 ().size1 () == e2 ().size2 (), bad_size ());
        size_type size = e1 ().size ();
        for (difference_type n = size - 1; n >= 0; -- n) {
#ifndef STREAMS_BOOST_UBLAS_SINGULAR_CHECK
            STREAMS_BOOST_UBLAS_CHECK (e2 () (n, n) != value_type/*zero*/(), singular ());
#else
    ...
    void inplace_solve (const matrix_expression<E1> &e1, matrix_expression<E2> &e2,
        ...
        STREAMS_BOOST_UBLAS_CHECK (e1 ().size1 () == e1 ().size2 (), bad_size ());
        STREAMS_BOOST_UBLAS_CHECK (e1 ().size2 () == e2 ().size1 (), bad_size ());
