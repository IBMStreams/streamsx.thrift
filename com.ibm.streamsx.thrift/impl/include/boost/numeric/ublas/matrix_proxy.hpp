...
#ifndef _STREAMS_BOOST_UBLAS_MATRIX_PROXY_
#define _STREAMS_BOOST_UBLAS_MATRIX_PROXY_
...
#include <streams_boost/numeric/ublas/detail/temporary.hpp>
...
namespace streams_boost { namespace numeric { namespace ublas {
    ...
    public:
        ...
        typedef typename streams_boost::mpl::if_<streams_boost::is_const<M>,
        ...
        matrix_row (matrix_type &data, size_type i):
            ...
            // STREAMS_BOOST_UBLAS_CHECK (i_ < data_.size1 (), bad_index ());
        }
        ...
        // Element access
#ifndef STREAMS_BOOST_UBLAS_PROXY_CONST_MEMBER
        STREAMS_BOOST_UBLAS_INLINE
        ...
        void swap (matrix_row mr) {
            if (this != &mr) {
                STREAMS_BOOST_UBLAS_CHECK (size () == mr.size (), bad_size ());
    ...
    public:
        ...
        public:
            ...
            const_reference operator * () const {
                STREAMS_BOOST_UBLAS_CHECK (index () < (*this) ().size (), bad_index ());
                return *it_;
            }
            STREAMS_BOOST_UBLAS_INLINE
    ...
    public:
        ...
        matrix_column (matrix_type &data, size_type j):
            data_ (data), j_ (j) {
            // Early checking of preconditions here.
            // STREAMS_BOOST_UBLAS_CHECK (j_ < data_.size2 (), bad_index ());
        }
        ...
        void swap (matrix_column mc) {
            if (this != &mc) {
                STREAMS_BOOST_UBLAS_CHECK (size () == mc.size (), bad_size ());
                // Sparse ranges may be nonconformant now.
    ...
    public:
        ...
        matrix_vector_range (matrix_type &data, const range_type &r1, const range_type &r2):
            ...
            // Early checking of preconditions here.
            // STREAMS_BOOST_UBLAS_CHECK (r1_.start () <= data_.size1 () &&
            //                     r1_.start () + r1_.size () <= data_.size1 (), bad_index ());
            // STREAMS_BOOST_UBLAS_CHECK (r2_.start () <= data_.size2 () &&
            //                   r2_.start () + r2_.size () <= data_.size2 (), bad_index ());
            // STREAMS_BOOST_UBLAS_CHECK (r1_.size () == r2_.size (), bad_size ());
        }
        ...
        STREAMS_BOOST_UBLAS_INLINE
        size_type size () const {
            return STREAMS_BOOST_UBLAS_SAME (r1_.size (), r2_.size ());
        }
        ...
        void swap (matrix_vector_range mvr) {
            if (this != &mvr) {
                STREAMS_BOOST_UBLAS_CHECK (size () == mvr.size (), bad_size ());
                // Sparse ranges may be nonconformant now.
    ...
    public:
        ...
        public:
            ...
            STREAMS_BOOST_STATIC_ASSERT ((streams_boost::is_same<typename M::const_iterator1::iterator_category, typename M::const_iterator2::iterator_category>::value ));
            ...
            difference_type operator - (const const_iterator &it) const {
                STREAMS_BOOST_UBLAS_CHECK ((*this) ().same_closure  (it ()), external_logic ());
                return STREAMS_BOOST_UBLAS_SAME (it1_ - it.it1_, it2_ - it.it2_);
            }
            ...
            STREAMS_BOOST_UBLAS_INLINE
            size_type  index () const {
                return STREAMS_BOOST_UBLAS_SAME (it1_.index (), it2_.index ());
            }
    ...
    public:
        ...
        matrix_vector_slice (matrix_type &data, const slice_type &s1, const slice_type &s2):
            ...
            // Early checking of preconditions here.
            // STREAMS_BOOST_UBLAS_CHECK (s1_.start () <= data_.size1 () &&
            //                    s1_.start () + s1_.stride () * (s1_.size () - (s1_.size () > 0)) <= data_.size1 (), bad_index ());
            // STREAMS_BOOST_UBLAS_CHECK (s2_.start () <= data_.size2 () &&
        ...
        STREAMS_BOOST_UBLAS_INLINE
        size_type size () const {
            return STREAMS_BOOST_UBLAS_SAME (s1_.size (), s2_.size ());
        }
        ...
        void swap (matrix_vector_slice mvs) {
            if (this != &mvs) {
                STREAMS_BOOST_UBLAS_CHECK (size () == mvs.size (), bad_size ());
                // Sparse ranges may be nonconformant now.
    ...
    public:
        ...
        matrix_vector_indirect (matrix_type &data, const indirect_array_type &ia1, const indirect_array_type &ia2):
            ...
            // Early checking of preconditions here.
            // STREAMS_BOOST_UBLAS_CHECK (ia1_.size () == ia2_.size (), bad_size ());
        }

        // Accessors
        STREAMS_BOOST_UBLAS_INLINE
        size_type size () const {
            return STREAMS_BOOST_UBLAS_SAME (ia1_.size (), ia2_.size ());
        }
        STREAMS_BOOST_UBLAS_INLINE
        ...
        void swap (matrix_vector_indirect mvi) {
            if (this != &mvi) {
                STREAMS_BOOST_UBLAS_CHECK (size () == mvi.size (), bad_size ());
                // Sparse ranges may be nonconformant now.
    ...
    public:
        ...
        void swap (matrix_range mr) {
            if (this != &mr) {
                STREAMS_BOOST_UBLAS_CHECK (size1 () == mr.size1 (), bad_size ());
                STREAMS_BOOST_UBLAS_CHECK (size2 () == mr.size2 (), bad_size ());
    ...
    public:
        ...
        void swap (matrix_slice ms) {
            if (this != &ms) {
                STREAMS_BOOST_UBLAS_CHECK (size1 () == ms.size1 (), bad_size ());
                STREAMS_BOOST_UBLAS_CHECK (size2 () == ms.size2 (), bad_size ());
    ...
    public:
        ...
        void swap (matrix_indirect mi) {
            if (this != &mi) {
                STREAMS_BOOST_UBLAS_CHECK (size1 () == mi.size1 (), bad_size ());
                STREAMS_BOOST_UBLAS_CHECK (size2 () == mi.size2 (), bad_size ());
