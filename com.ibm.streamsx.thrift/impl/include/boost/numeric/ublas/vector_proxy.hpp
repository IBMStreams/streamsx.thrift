...
#ifndef _STREAMS_BOOST_UBLAS_VECTOR_PROXY_
#define _STREAMS_BOOST_UBLAS_VECTOR_PROXY_
...
namespace streams_boost { namespace numeric { namespace ublas {
    ...
    public:
        ...
        typedef typename streams_boost::mpl::if_<streams_boost::is_const<V>,
        ...
        vector_range (vector_type &data, const range_type &r):
            ...
            // STREAMS_BOOST_UBLAS_CHECK (r_.start () <= data_.size () &&
        ...
        void swap (vector_range vr) {
            if (this != &vr) {
                STREAMS_BOOST_UBLAS_CHECK (size () == vr.size (), bad_size ());
    ...
    public:
        ...
        vector_slice (vector_type &data, const slice_type &s):
            ...
            // Early checking of preconditions here.
            // STREAMS_BOOST_UBLAS_CHECK (s_.start () <= data_.size () &&
        ...
        void swap (vector_slice vs) {
            if (this != &vs) {
                STREAMS_BOOST_UBLAS_CHECK (size () == vs.size (), bad_size ());
                // Sparse ranges may be nonconformant now.
...
return true;
        ...
        void swap (vector_indirect vi) {
            if (this != &vi) {
                STREAMS_BOOST_UBLAS_CHECK (size () == vi.size (), bad_size ());
                // Sparse ranges may be nonconformant now.
