...
#ifndef STREAMS_BOOST_TYPEOF_INT_ENCODING_HPP_INCLUDED
#define STREAMS_BOOST_TYPEOF_INT_ENCODING_HPP_INCLUDED
...
namespace streams_boost { namespace type_of {
    ...
    struct pack
    {
        STREAMS_BOOST_STATIC_CONSTANT(std::size_t , value=((n + 1) * 2 + (Overflow ? 1 : 0)));
    };

    template<std::size_t m>
    struct unpack
    {
        STREAMS_BOOST_STATIC_CONSTANT(std::size_t, value = (m / 2) - 1);
        STREAMS_BOOST_STATIC_CONSTANT(std::size_t, overflow = (m % 2 == 1));
    };
    ...
    struct encode_size_t : push_back<
        V,
        streams_boost::mpl::size_t<pack<n, false>::value>
    >
    {};
    ...
    struct encode_size_t<V, n, true> : push_back<typename push_back<
        V,
        streams_boost::mpl::size_t<pack<n % 0x3ffffffe, true>::value> >::type,
        streams_boost::mpl::size_t<n / 0x3ffffffe>
    >
    {};
    ...
    {
        STREAMS_BOOST_STATIC_CONSTANT(std::size_t,m = Iter::type::value);

        STREAMS_BOOST_STATIC_CONSTANT(std::size_t,value = (std::size_t)m * 0x3ffffffe + n);
    ...
    struct decode_integral
    {
        ...
        STREAMS_BOOST_STATIC_CONSTANT(std::size_t,n = unpack<m>::value);

        STREAMS_BOOST_STATIC_CONSTANT(std::size_t,overflow = unpack<m>::overflow);
...
#endif//STREAMS_BOOST_TYPEOF_INT_ENCODING_HPP_INCLUDED
