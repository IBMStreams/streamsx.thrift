...
# if STREAMS_BOOST_WORKAROUND(STREAMS_BOOST_MSVC,>=1300) && defined(_MSC_EXTENSIONS)
        ...
        struct encode_counter
        {
            ...
            {
                STREAMS_BOOST_STATIC_CONSTANT(unsigned,count=(encode_counter<T,N + 257>::count));
            }
            __if_not_exists(the_counter<N + 256>)
            {
                __if_exists(the_counter<N + 64>)
                {
                    STREAMS_BOOST_STATIC_CONSTANT(unsigned,count=(encode_counter<T,N + 65>::count));
                }
                __if_not_exists(the_counter<N + 64>)
                {
                    __if_exists(the_counter<N + 16>)
                    {
                        STREAMS_BOOST_STATIC_CONSTANT(unsigned,count=(encode_counter<T,N + 17>::count));
                    }
                    __if_not_exists(the_counter<N + 16>)
                    {
                        __if_exists(the_counter<N + 4>)
                        {
                            STREAMS_BOOST_STATIC_CONSTANT(unsigned,count=(encode_counter<T,N + 5>::count));
                        }
                        __if_not_exists(the_counter<N + 4>)
                        {
                            __if_exists(the_counter<N>)
                            {
                                STREAMS_BOOST_STATIC_CONSTANT(unsigned,count=(encode_counter<T,N + 1>::count));
                            }
                            __if_not_exists(the_counter<N>)
                            {
                                STREAMS_BOOST_STATIC_CONSTANT(unsigned,count=N);
...
# define STREAMS_BOOST_TYPEOF_INDEX(T) (encode_counter<T>::count)
# define STREAMS_BOOST_TYPEOF_NEXT_INDEX(next)
# else
...
# define STREAMS_BOOST_TYPEOF_INDEX(T) (sizeof(*streams_boost::type_of::encode_index((streams_boost::type_of::encode_counter<1005>*)0)))
...
#elif STREAMS_BOOST_WORKAROUND(STREAMS_BOOST_MSVC,>=1400)
...
# define STREAMS_BOOST_TYPEOF(expr) \
    streams_boost::type_of::msvc_typeid_wrapper<typeid(streams_boost::type_of::encode_start(expr))>::type
...
struct name {\
    enum {_typeof_register_value=sizeof(typeid(streams_boost::type_of::typeof_register_type<name>(expr)))};\
