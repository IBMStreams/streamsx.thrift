...
#ifndef STREAMS_BOOST_MPL_APPLY_HPP_INCLUDED
#define STREAMS_BOOST_MPL_APPLY_HPP_INCLUDED
...
#   include <streams_boost/mpl/apply_fwd.hpp>
...
#   define STREAMS_BOOST_MPL_PREPROCESSED_HEADER apply.hpp
...
#define STREAMS_BOOST_PP_ITERATION_PARAMS_1 \
    (3,(0, STREAMS_BOOST_MPL_LIMIT_METAFUNCTION_ARITY, <streams_boost/mpl/apply.hpp>))
#include STREAMS_BOOST_PP_ITERATE()

#   if !defined(STREAMS_BOOST_MPL_CFG_NO_APPLY_TEMPLATE)
...
#   endif // STREAMS_BOOST_MPL_CFG_NO_APPLY_TEMPLATE
...
#endif // STREAMS_BOOST_MPL_APPLY_HPP_INCLUDED
...
template<
    ...
    >
struct STREAMS_BOOST_PP_CAT(apply,i_)
#if !STREAMS_BOOST_WORKAROUND(STREAMS_BOOST_MSVC, < 1300)
    : STREAMS_BOOST_PP_CAT(apply_wrap,i_)< 
...
#else
{
    ...
    STREAMS_BOOST_MPL_AUX_LAMBDA_SUPPORT(
          STREAMS_BOOST_PP_INC(i_)
        , STREAMS_BOOST_PP_CAT(apply,i_)
...
template<>
struct STREAMS_BOOST_PP_CAT(apply,i_)<AUX778076_APPLY_N_SPEC_PARAMS(i_, int)>
{
    typedef int type;
};
#endif
...
template<
    ...
    >
struct apply
    : STREAMS_BOOST_PP_CAT(apply,i_)< F AUX778076_APPLY_N_COMMA_PARAMS(i_, T) >
{
};
#else
template<
...
struct apply_chooser<i_>
{
    ...
    struct result_
    {
        typedef STREAMS_BOOST_PP_CAT(apply,i_)<
              F AUX778076_APPLY_N_COMMA_PARAMS(i_, T)
            > type;
    };
};
