...
template<
      STREAMS_BOOST_PP_ENUM_PARAMS(i, typename T)
    >
struct list1
    : l_item<
          long_<1>
...
#   define MPL_AUX_LIST_TAIL(list, i, T) \
    STREAMS_BOOST_PP_CAT(list,STREAMS_BOOST_PP_DEC(i))< \
      STREAMS_BOOST_PP_ENUM_SHIFTED_PARAMS(i, T) \
    > \
    /**/
...
template<
      STREAMS_BOOST_PP_ENUM_PARAMS(i, typename T)
    >
struct STREAMS_BOOST_PP_CAT(list,i)
    : l_item<
          long_<i>
        , T0
        ...
        >
{
    typedef STREAMS_BOOST_PP_CAT(list,i) type;
};
