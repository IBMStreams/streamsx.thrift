...
#   define AUX778076_VECTOR_TAIL(vector, i_, T) \
    STREAMS_BOOST_PP_CAT(vector,i_)< \
...
template<
    ...
    >
struct STREAMS_BOOST_PP_CAT(vector,i_)
    : v_item<
          STREAMS_BOOST_PP_CAT(T,STREAMS_BOOST_PP_DEC(i_))
        , AUX778076_VECTOR_TAIL(vector,STREAMS_BOOST_PP_DEC(i_),T)
        >
{
    typedef STREAMS_BOOST_PP_CAT(vector,i_) type;
};
#endif
...
#   define AUX778076_VECTOR_ITEM(unused, i_, unused2) \
    typedef STREAMS_BOOST_PP_CAT(T,i_) STREAMS_BOOST_PP_CAT(item,i_); \
    /**/

    STREAMS_BOOST_PP_REPEAT(i_, AUX778076_VECTOR_ITEM, unused)
#   undef AUX778076_VECTOR_ITEM
    typedef void_ STREAMS_BOOST_PP_CAT(item,i_);
    typedef STREAMS_BOOST_PP_CAT(T,STREAMS_BOOST_PP_DEC(i_)) back;
...
template<>
struct push_front_impl< aux::vector_tag<STREAMS_BOOST_PP_DEC(i_)> >
{
    template< typename Vector, typename T > struct apply
    {
        typedef STREAMS_BOOST_PP_CAT(vector,i_)<
              T
              STREAMS_BOOST_PP_COMMA_IF(STREAMS_BOOST_PP_DEC(i_))
              STREAMS_BOOST_PP_ENUM_PARAMS(STREAMS_BOOST_PP_DEC(i_), typename Vector::item)
            > type;
    };
};
...
struct pop_front_impl< aux::vector_tag<i_> >
{
    ...
    {
        typedef STREAMS_BOOST_PP_CAT(vector,STREAMS_BOOST_PP_DEC(i_))<
              STREAMS_BOOST_PP_ENUM_SHIFTED_PARAMS(i_, typename Vector::item)
            > type;
    };
};
...
template<>
struct push_back_impl< aux::vector_tag<STREAMS_BOOST_PP_DEC(i_)> >
{
    template< typename Vector, typename T > struct apply
    {
        typedef STREAMS_BOOST_PP_CAT(vector,i_)<
...
struct v_at<V,i_>
{
    typedef typename V::STREAMS_BOOST_PP_CAT(item,i_) type;
};
...
template<> struct v_at_impl<i_>
{
    ...
    {
        typedef typename V_::STREAMS_BOOST_PP_CAT(item,i_) type;
    };
};
}
...
struct at_impl< aux::vector_tag<i_> >
{
    ...
    {
        typedef typename aux::v_at_impl<STREAMS_BOOST_MPL_AUX_VALUE_WKND(N)::value>
