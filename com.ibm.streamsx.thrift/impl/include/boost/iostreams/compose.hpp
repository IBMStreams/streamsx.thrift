...
#ifndef STREAMS_BOOST_IOSTREAMS_COMPOSE_HPP_INCLUDED
#define STREAMS_BOOST_IOSTREAMS_COMPOSE_HPP_INCLUDED
...
template< typename First, 
          typename Second,
          typename FirstMode = 
              STREAMS_BOOST_DEDUCED_TYPENAME mode_of<First>::type,
          typename SecondMode = 
              STREAMS_BOOST_DEDUCED_TYPENAME mode_of<Second>::type >
...
template< typename Filter,
          typename Device,
          typename Mode =
              STREAMS_BOOST_DEDUCED_TYPENAME composite_mode<Filter, Device>::type >
...
private:
    ...
    STREAMS_BOOST_STATIC_ASSERT(is_device<Device>::value);
public:
...
template< typename Filter1, 
          typename Filter2,
          typename Mode =
              STREAMS_BOOST_DEDUCED_TYPENAME composite_mode<Filter1, Filter2>::type >
...
private:
    ...
    STREAMS_BOOST_STATIC_ASSERT(is_filter<Filter1>::value);
    STREAMS_BOOST_STATIC_ASSERT(is_filter<Filter2>::value);
public:
    ...
    {
        composite_device<filter_ref, Source> cmp(streams_boost::ref(filter2_), src);
    ...
    {
        composite_device<filter_ref, Sink> cmp(streams_boost::ref(filter2_), snk);
    ...
    std::streampos seek( Device& dev, stream_offset off, STREAMS_BOOST_IOS::seekdir way,
                         STREAMS_BOOST_IOS::openmode which =
                             STREAMS_BOOST_IOS::in | STREAMS_BOOST_IOS::out )
    {
        composite_device<filter_ref, Device> cmp(streams_boost::ref(filter2_), dev);
    ...
    {
        STREAMS_BOOST_STATIC_ASSERT((!is_convertible<category, two_sequence>::value));
        STREAMS_BOOST_STATIC_ASSERT((!is_convertible<category, dual_use>::value));
        ...
        if (!is_convertible<first_mode, dual_use>::value) {
            detail::execute_all(
                detail::call_close(filter2_, dev, STREAMS_BOOST_IOS::in),
                detail::call_close(filter1_, cmp, STREAMS_BOOST_IOS::in),
                detail::call_close(filter1_, cmp, STREAMS_BOOST_IOS::out),
                detail::call_close(filter2_, dev, STREAMS_BOOST_IOS::out)
            );
...
compose( const Filter& filter, const FilterOrDevice& fod
         STREAMS_BOOST_IOSTREAMS_DISABLE_IF_STREAM(FilterOrDevice) )
...
void composite_device<Filter, Device, Mode>::close()
{
    ...
    if (!is_convertible<filter_mode, dual_use>::value) {
        detail::execute_all(
            detail::call_close(device_, STREAMS_BOOST_IOS::in),
            detail::call_close(filter_, device_, STREAMS_BOOST_IOS::in),
            detail::call_close(filter_, device_, STREAMS_BOOST_IOS::out),
            detail::call_close(device_, STREAMS_BOOST_IOS::out)
        );
...
template<typename Filter, typename Device, typename Mode>
void composite_device<Filter, Device, Mode>::close(STREAMS_BOOST_IOS::openmode which)
{
    STREAMS_BOOST_STATIC_ASSERT((is_convertible<Mode, two_sequence>::value));
    STREAMS_BOOST_STATIC_ASSERT(!(is_convertible<filter_mode, dual_use>::value));
...
#endif // #ifndef STREAMS_BOOST_IOSTREAMS_COMPOSE_HPP_INCLUDED
