...
#ifndef STREAMS_BOOST_IOSTREAMS_DETAIL_CHAIN_HPP_INCLUDED
#define STREAMS_BOOST_IOSTREAMS_DETAIL_CHAIN_HPP_INCLUDED
...
#include <streams_boost/config.hpp>                     // STREAMS_BOOST_MSVC, template friends,
#include <streams_boost/detail/workaround.hpp>          // STREAMS_BOOST_NESTED_TEMPLATE 
...
#include <streams_boost/iostreams/device/null.hpp>
#include <streams_boost/iostreams/positioning.hpp>
#include <streams_boost/iostreams/traits.hpp>           // is_filter.
...
#include <streams_boost/iostreams/detail/execute.hpp>   // VC6.5 requires this
#if STREAMS_BOOST_WORKAROUND(STREAMS_BOOST_MSVC, < 1310)        // #include order
...
// Boost.Python and Boost.Function.
    ...
    /**/
# include <cstring>
# define STREAMS_BOOST_IOSTREAMS_COMPARE_TYPE_ID(X,Y) \
     (std::strcmp((X).name(),(Y).name()) == 0)
#else
# define STREAMS_BOOST_IOSTREAMS_COMPARE_TYPE_ID(X,Y) ((X)==(Y))
#endif

// Deprecated
#define STREAMS_BOOST_IOSTREAMS_COMPONENT_TYPE(chain, index) \
    chain.component_type( index ) \
    /**/

#if !STREAMS_BOOST_WORKAROUND(STREAMS_BOOST_MSVC, < 1310)
# define STREAMS_BOOST_IOSTREAMS_COMPONENT(chain, index, target) \
    chain.component< target >( index ) \
    /**/
#else
# define STREAMS_BOOST_IOSTREAMS_COMPONENT(chain, index, target) \
    chain.component( index, ::streams_boost::type< target >() ) \
    /**/
#endif
...
public:
    ...
    const std::type_info& component_type(int n) const
    {
        if (static_cast<size_type>(n) >= size())
            streams_boost::throw_exception(std::out_of_range("bad chain offset"));
        return (*streams_boost::next(list().begin(), n))->component_type();
    }
    ...
    template<typename T>
    T* component(int n) const { return component(n, streams_boost::type<T>()); }
...
#endif
    template<typename T>
    T* component(int n, streams_boost::type<T>) const
    {
        if (static_cast<size_type>(n) >= size())
            streams_boost::throw_exception(std::out_of_range("bad chain offset"));
        streambuf_type* link = *streams_boost::next(list().begin(), n);
        if (STREAMS_BOOST_IOSTREAMS_COMPARE_TYPE_ID(link->component_type(), typeid(T)))
...
public:
    ...
    STREAMS_BOOST_IOSTREAMS_DEFINE_PUSH(push, mode, char_type, push_impl)
    void pop();
    ...
    bool sync() { return front().STREAMS_BOOST_IOSTREAMS_PUBSYNC() != -1; }
    bool strict_sync();
private:
    template<typename T>
                   ...
                   std::streamsize pback_size = -1)
    {
        ...
        typedef stream_buffer<
                    component_type,
                    STREAMS_BOOST_IOSTREAMS_CHAR_TRAITS(char_type),
                    Alloc, Mode
                >                                         streambuf_t;
        typedef typename list_type::iterator              iterator;
        STREAMS_BOOST_STATIC_ASSERT((is_convertible<category, Mode>::value));
        if (is_complete())
            streams_boost::throw_exception(std::logic_error("chain complete"));
    ...
    static void close(streambuf_type* b, STREAMS_BOOST_IOS::openmode m)
    {
        if (m == STREAMS_BOOST_IOS::out && is_convertible<Mode, output>::value)
            b->STREAMS_BOOST_IOSTREAMS_PUBSYNC();
        b->close(m);
    }
    ...
    struct closer  : public std::unary_function<streambuf_type*, void>  {
        closer(STREAMS_BOOST_IOS::openmode m) : mode_(m) { }
        void operator() (streambuf_type* b)
        {
            close(b, mode_);
        }
        STREAMS_BOOST_IOS::openmode mode_;
    };
        ...
        void close()
            {
                if ((flags_ & f_open) != 0) {
                    ...
                    links_.front()->STREAMS_BOOST_IOSTREAMS_PUBSYNC();
                    try {
                        streams_boost::iostreams::detail::execute_foreach(
                            links_.rbegin(), links_.rend(), 
                            closer(STREAMS_BOOST_IOS::in)
                        );
                    } catch (...) {
                        try {
                            streams_boost::iostreams::detail::execute_foreach(
                                links_.begin(), links_.end(), 
                                closer(STREAMS_BOOST_IOS::out)
                            );
                        } catch (...) { }
                        throw;
                    }
...
//
// Macro: STREAMS_BOOST_IOSTREAMS_DECL_CHAIN(name, category)
...
//
#define STREAMS_BOOST_IOSTREAMS_DECL_CHAIN(name_, default_char_) \
    template< typename Mode, typename Ch = default_char_, \
              typename Tr = STREAMS_BOOST_IOSTREAMS_CHAR_TRAITS(Ch), \
              typename Alloc = std::allocator<Ch> > \
    class name_ : public streams_boost::iostreams::detail::chain_base< \
    ...
    public: \
        ...
        typedef Mode                                   mode; \
    private: \
        typedef streams_boost::iostreams::detail::chain_base< \
                    name_<Mode, Ch, Tr, Alloc>, \
                    Ch, Tr, Alloc, Mode \
                >                                      base_type; \
    public: \
    ...
    /**/
STREAMS_BOOST_IOSTREAMS_DECL_CHAIN(chain, char)
STREAMS_BOOST_IOSTREAMS_DECL_CHAIN(wchain, wchar_t)
#undef STREAMS_BOOST_IOSTREAMS_DECL_CHAIN
...
public:
    ...
    { return chain_->STREAMS_BOOST_NESTED_TEMPLATE component_type<N>(); }

    template<typename T>
    T* component(int n) const
    { return chain_->STREAMS_BOOST_NESTED_TEMPLATE component<T>(n); }

    // Deprecated.
    template<int N, typename T>
    T* component() const
    { return chain_->STREAMS_BOOST_NESTED_TEMPLATE component<N, T>(); }
#else
    template<typename T>
    T* component(int n, streams_boost::type<T> t) const
...
protected:
    ...
    { chain_->push(t STREAMS_BOOST_IOSTREAMS_PUSH_ARGS()); }
...
#endif // #ifndef STREAMS_BOOST_IOSTREAMS_DETAIL_CHAIN_HPP_INCLUDED
