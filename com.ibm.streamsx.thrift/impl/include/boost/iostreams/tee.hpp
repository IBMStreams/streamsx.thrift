...
#ifndef STREAMS_BOOST_IOSTREAMS_TEE_HPP_INCLUDED
#define STREAMS_BOOST_IOSTREAMS_TEE_HPP_INCLUDED
...
#include <cassert>
#include <streams_boost/config.hpp>  // STREAMS_BOOST_DEDUCE_TYPENAME.
...
#include <streams_boost/iostreams/detail/functional.hpp>  // call_close_all 
...
public:
    ...
    STREAMS_BOOST_STATIC_ASSERT((
        is_convertible< // Using mode_of causes failures on VC6-7.0.
            STREAMS_BOOST_DEDUCED_TYPENAME iostreams::category_of<Device>::type, output
        >::value
    ));
    ...
    void close(Next&, STREAMS_BOOST_IOS::openmode)
    { 
...
STREAMS_BOOST_IOSTREAMS_PIPABLE(tee_filter, 1)
...
public:
    ...
    STREAMS_BOOST_STATIC_ASSERT((
        is_same<
            char_type, 
            STREAMS_BOOST_DEDUCED_TYPENAME char_type_of<Sink>::type
        >::value
    ));
    STREAMS_BOOST_STATIC_ASSERT((
        is_convertible<
            STREAMS_BOOST_DEDUCED_TYPENAME iostreams::category_of<Sink>::type, 
            output
        >::value
    ));
    struct category
    ...
    std::streamsize read(char_type* s, std::streamsize n)
    {
        STREAMS_BOOST_STATIC_ASSERT((
            is_convertible<
                STREAMS_BOOST_DEDUCED_TYPENAME iostreams::category_of<Device>::type, input
            >::value
        ));
...
#endif // #ifndef STREAMS_BOOST_IOSTREAMS_TEE_HPP_INCLUDED
