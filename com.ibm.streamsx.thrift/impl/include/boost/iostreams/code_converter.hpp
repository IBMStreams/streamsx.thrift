...
#ifndef STREAMS_BOOST_IOSTREAMS_CODE_CONVERTER_HPP_INCLUDED
#define STREAMS_BOOST_IOSTREAMS_CODE_CONVERTER_HPP_INCLUDED
...
#if defined(STREAMS_BOOST_IOSTREAMS_NO_WIDE_STREAMS) || \
    defined(STREAMS_BOOST_IOSTREAMS_NO_LOCALE) \
    /**/
...
#include <streams_boost/iostreams/constants.hpp>   // default_filter_buffer_size.
#include <streams_boost/iostreams/detail/adapter/concept_adapter.hpp>
#include <streams_boost/iostreams/detail/adapter/direct_adapter.hpp>
#include <streams_boost/iostreams/detail/buffer.hpp>
#include <streams_boost/iostreams/detail/call_traits.hpp>
#include <streams_boost/iostreams/detail/codecvt_holder.hpp>
#include <streams_boost/iostreams/detail/codecvt_helper.hpp>
#include <streams_boost/iostreams/detail/double_object.hpp>
...
#include <streams_boost/iostreams/detail/ios.hpp> // failure, openmode, int types.
...
// Must come last.
#include <streams_boost/iostreams/detail/config/disable_warnings.hpp> // Borland 5.x

namespace streams_boost { namespace iostreams {

struct code_conversion_error : STREAMS_BOOST_IOSTREAMS_FAILURE {
    code_conversion_error() 
        : STREAMS_BOOST_IOSTREAMS_FAILURE("code conversion error")
        { }
};
...
struct strncpy_if_same_impl<true> {
    ...
    { return STREAMS_BOOST_IOSTREAMS_CHAR_TRAITS(Ch)::copy(tgt, src, n); }
};
...
class conversion_buffer 
    : public buffer<
                 STREAMS_BOOST_DEDUCED_TYPENAME detail::codecvt_extern<Codecvt>::type,
                 Alloc
             > 
{
public:
...
struct code_converter_impl {
    ...
    {
        detail::execute_all(
            detail::call_member_close(*this, STREAMS_BOOST_IOS::in),
            detail::call_member_close(*this, STREAMS_BOOST_IOS::out)
        );
    }

    void close(STREAMS_BOOST_IOS::openmode which)
    {
        if (which == STREAMS_BOOST_IOS::in && (flags_ & f_input_closed) == 0) {
            flags_ |= f_input_closed;
            iostreams::close(dev(), STREAMS_BOOST_IOS::in);
        }
        if (which == STREAMS_BOOST_IOS::out && (flags_ & f_output_closed) == 0) {
            flags_ |= f_output_closed;
            detail::execute_all(
                detail::flush_buffer(buf_.second(), dev(), can_write::value),
                detail::call_close(dev(), STREAMS_BOOST_IOS::out),
...
#define STREAMS_BOOST_IOSTREAMS_CONVERTER_PARAMS() , int buffer_size = -1
#define STREAMS_BOOST_IOSTREAMS_CONVERTER_ARGS() , buffer_size
...
public:
    ...
    code_converter(const code_converter& rhs) 
        ...
        { }
#endif
    STREAMS_BOOST_IOSTREAMS_FORWARD( code_converter, open_impl, Device,
                             STREAMS_BOOST_IOSTREAMS_CONVERTER_PARAMS, 
                             STREAMS_BOOST_IOSTREAMS_CONVERTER_ARGS )

        // fstream-like interface.

    bool is_open() const { return this->pimpl_->is_open(); }
    void close(STREAMS_BOOST_IOS::openmode which = STREAMS_BOOST_IOS::in | STREAMS_BOOST_IOS::out )
    { impl().close(which); }
...
private:
    ...
    void open_impl(const T& t STREAMS_BOOST_IOSTREAMS_CONVERTER_PARAMS()) 
    { 
        impl().open(t STREAMS_BOOST_IOSTREAMS_CONVERTER_ARGS()); 
    }
    ...
    (char_type* s, std::streamsize n)
{
    ...
    do {
        ...
        default:
            ...
            streams_boost::throw_exception(code_conversion_error());
        }
...
#include <streams_boost/iostreams/detail/config/enable_warnings.hpp> // Borland 5.x

#endif // #ifndef STREAMS_BOOST_IOSTREAMS_CODE_CONVERTER_HPP_INCLUDED
