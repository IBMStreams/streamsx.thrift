...
#ifndef STREAMS_BOOST_IOSTREAMS_COMBINE_HPP_INCLUDED
#define STREAMS_BOOST_IOSTREAMS_COMBINE_HPP_INCLUDED

#if defined(_MSC_VER) && (_MSC_VER >= 1020)
# pragma once
#endif              

#include <streams_boost/config.hpp> // NO_STD_LOCALE, DEDUCED_TYPENAME.
...
public:
    ...
    struct category
        : bidirectional, 
          ...
          localizable_tag
        { };
    STREAMS_BOOST_STATIC_ASSERT(is_device<Source>::value);
    STREAMS_BOOST_STATIC_ASSERT(is_device<Sink>::value);
    STREAMS_BOOST_STATIC_ASSERT((is_convertible<in_category, input>::value));
    STREAMS_BOOST_STATIC_ASSERT((is_convertible<out_category, output>::value));
    STREAMS_BOOST_STATIC_ASSERT((is_same<char_type, sink_char_type>::value));
...
public:
    ...
    struct category 
        : multichar_bidirectional_filter_tag,
          ...
          localizable_tag
        { };
    STREAMS_BOOST_STATIC_ASSERT(is_filter<InputFilter>::value);
    STREAMS_BOOST_STATIC_ASSERT(is_filter<OutputFilter>::value);
    STREAMS_BOOST_STATIC_ASSERT((is_convertible<in_category, input>::value));
    STREAMS_BOOST_STATIC_ASSERT((is_convertible<out_category, output>::value));
    STREAMS_BOOST_STATIC_ASSERT((is_same<char_type, output_char_type>::value));
    combined_filter(const InputFilter& in, const OutputFilter& out);

    template<typename Source>
    std::streamsize read(Source& src, char_type* s, std::streamsize n)
    { return streams_boost::iostreams::read(in_, src, s, n); }

    template<typename Sink>
    std::streamsize write(Sink& snk, const char_type* s, std::streamsize n)
    { return streams_boost::iostreams::write(out_, snk, s, n); }

    template<typename Sink>
    void close(Sink& snk, STREAMS_BOOST_IOS::openmode which)
    {
        if (which == STREAMS_BOOST_IOS::in) {
            if (is_convertible<in_category, dual_use>::value) {
                iostreams::close(in_, snk, STREAMS_BOOST_IOS::in);
            } else {
        ...
        if (which == STREAMS_BOOST_IOS::out) {
            if (is_convertible<out_category, dual_use>::value) {
                iostreams::close(out_, snk, STREAMS_BOOST_IOS::out);
            } else {
...
struct combine_traits {
    typedef combination<
                STREAMS_BOOST_DEDUCED_TYPENAME detail::unwrapped_type<In>::type, 
                STREAMS_BOOST_DEDUCED_TYPENAME detail::unwrapped_type<Out>::type
            > type;
};
...
template<typename Source, typename Sink>
inline void
combined_device<Source, Sink>::close(STREAMS_BOOST_IOS::openmode which)
{ 
    if (which == STREAMS_BOOST_IOS::in)
...
#endif // #ifndef STREAMS_BOOST_IOSTREAMS_COMBINE_HPP_INCLUDED
