...
#ifndef STREAMS_BOOST_IOSTREAMS_COPY_HPP_INCLUDED
#define STREAMS_BOOST_IOSTREAMS_COPY_HPP_INCLUDED

#if defined(_MSC_VER) && (_MSC_VER >= 1020)
# pragma once
#endif              

#include <streams_boost/config.hpp>                 // Make sure ptrdiff_t is in std.
...
#include <streams_boost/iostreams/operations.hpp>  // read, write, close.
...
namespace detail {
    ...
    // models Direct (see 
    // http://www.streams_boost.org/libs/iostreams/doc/index.html?path=4.1.1.4)
...
{ 
    ...
    // streams_boost::iostreams::detail::execute() in the primary 
...
std::streamsize copy_impl(Source src, Sink snk, std::streamsize buffer_size)
{
    ...
    STREAMS_BOOST_STATIC_ASSERT((is_same<src_char, snk_char>::value));
...
copy( const Source& src, const Sink& snk,
      ...
      STREAMS_BOOST_IOSTREAMS_DISABLE_IF_STREAM(Source)
      STREAMS_BOOST_IOSTREAMS_DISABLE_IF_STREAM(Sink) )
{ 
    typedef typename char_type_of<Source>::type char_type;
...
copy( Source& src, const Sink& snk,
      std::streamsize buffer_size = default_device_buffer_size
      STREAMS_BOOST_IOSTREAMS_ENABLE_IF_STREAM(Source)
      STREAMS_BOOST_IOSTREAMS_DISABLE_IF_STREAM(Sink) ) 
{ 
    typedef typename char_type_of<Source>::type char_type;
...
copy( const Source& src, Sink& snk,
      std::streamsize buffer_size = default_device_buffer_size
      STREAMS_BOOST_IOSTREAMS_DISABLE_IF_STREAM(Source)
      STREAMS_BOOST_IOSTREAMS_ENABLE_IF_STREAM(Sink) ) 
{ 
    typedef typename char_type_of<Source>::type char_type;
    return detail::copy_impl( detail::resolve<input, char_type>(src), 
...
#endif // #ifndef STREAMS_BOOST_IOSTREAMS_COPY_HPP_INCLUDED
