...
// STREAMS_BOOST_SELECT_BY_SIZE_MAX_CASE defined to be the largest N such that case_<N>
...
// Example usage:
//
//    #define STREAMS_BOOST_SELECT_BY_SIZE_MAX_CASE 7   // (Needed when default was 2)
//    #include <streams_boost/utility/select_by_size.hpp>
//
//    using namespace streams_boost::utility;
//
//    case_<0> helper(bool);
//    case_<1> helper(int);
...
//        STREAMS_BOOST_STATIC_ASSERT(value == 4);
//    };
//
...
//
//    struct test {
//        STREAMS_BOOST_SELECT_BY_SIZE(int, value, helper(9876UL));
//    };
//
...
#ifndef STREAMS_BOOST_IOSTREAMS_DETAIL_SELECT_BY_SIZE_HPP_INCLUDED
#define STREAMS_BOOST_IOSTREAMS_DETAIL_SELECT_BY_SIZE_HPP_INCLUDED
...
namespace streams_boost { namespace utility {
...
} } // End namespaces utility, streams_boost.
...
#define SELECT_BY_SIZE_SPEC(n) \
    namespace streams_boost { namespace iostreams { namespace detail { \
      static const int STREAMS_BOOST_PP_CAT(sizeof_case_, n) = sizeof(case_<n - 1>); \
      template<> \
      struct select_by_size< STREAMS_BOOST_PP_CAT(sizeof_case_, n) > { \
          struct type { STREAMS_BOOST_STATIC_CONSTANT(int, value = n - 1); }; \
...
#define STREAMS_BOOST_PP_LOCAL_MACRO(n) SELECT_BY_SIZE_SPEC(n)
#define STREAMS_BOOST_PP_LOCAL_LIMITS (0, 20)
#include STREAMS_BOOST_PP_LOCAL_ITERATE()
#undef STREAMS_BOOST_PP_LOCAL_MACRO

//--------------Definition of SELECT_BY_SIZE----------------------------------//

#define STREAMS_BOOST_SELECT_BY_SIZE(type_, name, expr) \
    STREAMS_BOOST_STATIC_CONSTANT( \
        unsigned, \
        STREAMS_BOOST_PP_CAT(streams_boost_select_by_size_temp_, name) = sizeof(expr) \
    ); \
    STREAMS_BOOST_STATIC_CONSTANT( \
        type_, \
        name = \
            ( ::streams_boost::iostreams::detail::select_by_size< \
                STREAMS_BOOST_PP_CAT(streams_boost_select_by_size_temp_, name) \
              >::value ) \
    ) \
    /**/

#endif // #ifndef STREAMS_BOOST_IOSTREAMS_DETAIL_SELECT_BY_SIZE_HPP_INCLUDED

//----------Specializations of SELECT_BY_SIZE (outside main inclued guards)---//

#if STREAMS_BOOST_SELECT_BY_SIZE_MAX_CASE > SELECT_BY_SIZE_MAX_SPECIALIZED

#define STREAMS_BOOST_PP_LOCAL_MACRO(n) SELECT_BY_SIZE_SPEC(n)
#define STREAMS_BOOST_PP_LOCAL_LIMITS \
    (SELECT_BY_SIZE_MAX_SPECIALIZED, STREAMS_BOOST_SELECT_BY_SIZE_MAX_CASE) \
    /**/
#include STREAMS_BOOST_PP_LOCAL_ITERATE()
#undef STREAMS_BOOST_PP_LOCAL_MACRO
#undef SELECT_BY_SIZE_MAX_SPECIALIZED
#define SELECT_BY_SIZE_MAX_SPECIALIZED STREAMS_BOOST_SELECT_BY_SIZE_MAX_CASE
