...
//
// // In global namespace:
// STREAMS_BOOST_IOSTREAMS_CODECVT_SPEC(mystate)
//
// // In user namespace:
...
#ifndef STREAMS_BOOST_IOSTREAMS_DETAIL_CODECVT_HELPER_HPP_INCLUDED
#define STREAMS_BOOST_IOSTREAMS_DETAIL_CODECVT_HELPER_HPP_INCLUDED

#if defined(_MSC_VER) && (_MSC_VER >= 1020)
# pragma once
#endif

#include <streams_boost/config.hpp>  // Put size_t in std, STREAMS_BOOST_MSVC, Dinkum.
#include <streams_boost/detail/workaround.hpp>
#include <algorithm>         // min.
#include <cstddef>           // size_t.
#include <locale>            // locale, codecvt_base, codecvt.
#include <streams_boost/iostreams/detail/config/codecvt.hpp>
...
#else // #if !STREAMS_BOOST_WORKAROUND(STREAMS_BOOST_DINKUMWARE_STDLIB, == 1) //--------------//
...
#endif // #if !STREAMS_BOOST_WORKAROUND(STREAMS_BOOST_DINKUMWARE_STDLIB, == 1) //-------------//
...
#if defined(STREAMS_BOOST_IOSTREAMS_NO_PRIMARY_CODECVT_DEFINITION) || \
    defined(STREAMS_BOOST_IOSTREAMS_EMPTY_PRIMARY_CODECVT_DEFINITION) || \
...
public:
    ...
    int length( STREAMS_BOOST_IOSTREAMS_CODECVT_CV_QUALIFIER State& state, 
...
protected:
    ...
    virtual int do_length( STREAMS_BOOST_IOSTREAMS_CODECVT_CV_QUALIFIER State&, 
                           const Extern* first1, const Extern* last1,
                           std::size_t len2 ) const throw()
    {
...
//------------------Definition of STREAMS_BOOST_IOSTREAMS_CODECVT_SPEC----------------//

#if defined(STREAMS_BOOST_IOSTREAMS_NO_PRIMARY_CODECVT_DEFINITION) || \
    defined(STREAMS_BOOST_IOSTREAMS_EMPTY_PRIMARY_CODECVT_DEFINITION) \
    /**/
# ifndef STREAMS_BOOST_NO_TEMPLATE_PARTIAL_SPECIALIZATION
#  define STREAMS_BOOST_IOSTREAMS_CODECVT_SPEC(state) \
    namespace std { \
        template<typename Intern, typename Extern> \
        class codecvt<Intern, Extern, state> \
            : public ::streams_boost::iostreams::detail::codecvt_impl< \
                         Intern, Extern, state \
                     > \
        { \
        public: \
            codecvt(std::size_t refs = 0) \
                : ::streams_boost::iostreams::detail::codecvt_impl< \
                      Intern, Extern, state \
                  >(refs) \
                { } \
...
struct codecvt_helper : std::codecvt<Intern, Extern, State> { 
    ...
    #if !defined(STREAMS_BOOST_IOSTREAMS_NO_CODECVT_CTOR_FROM_SIZE_T)
        : std::codecvt<Intern, Extern, State>(refs)
    #else
        : std::codecvt<Intern, Extern, State>()
    #endif
        { }
#ifdef STREAMS_BOOST_IOSTREAMS_NO_CODECVT_MAX_LENGTH
    int max_length() const throw() { return do_max_length(); }
protected:
    virtual int do_max_length() const throw() { return 1; }
#endif
...
#endif // #ifndef STREAMS_BOOST_IOSTREAMS_DETAIL_CODECVT_HELPER_HPP_INCLUDED
