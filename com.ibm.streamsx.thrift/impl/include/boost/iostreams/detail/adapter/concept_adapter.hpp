...
#ifndef STREAMS_BOOST_IOSTREAMS_DETAIL_CONCEPT_ADAPTER_HPP_INCLUDED
#define STREAMS_BOOST_IOSTREAMS_DETAIL_CONCEPT_ADAPTER_HPP_INCLUDED

#include <streams_boost/config.hpp>                             // SFINAE.
...
public:
    ...
    { STREAMS_BOOST_STATIC_ASSERT(is_std_io<T>::value); }
    ...
    std::streampos seek( stream_offset off, STREAMS_BOOST_IOS::seekdir way,
                         STREAMS_BOOST_IOS::openmode which, Device* dev )
    ...
    template<typename Device>
    void close(STREAMS_BOOST_IOS::openmode which, Device* dev)
    { any_impl::close(t_, dev, which); }

    bool flush( STREAMS_BOOST_IOSTREAMS_BASIC_STREAMBUF(char_type,
                STREAMS_BOOST_IOSTREAMS_CHAR_TRAITS(char_type))* sb )
    { 
        bool result = any_impl::flush(t_, sb);
        if (sb && sb->STREAMS_BOOST_IOSTREAMS_PUBSYNC() == -1)
            result = false;
        return result;
    }
...
struct device_wrapper_impl<any_tag> {
    ...
    seek( Device& dev, Dummy*, stream_offset off, 
          STREAMS_BOOST_IOS::seekdir way, STREAMS_BOOST_IOS::openmode which )
    { 
    ...
    template<typename Device>
    static std::streampos 
    seek( Device&, stream_offset, STREAMS_BOOST_IOS::seekdir, 
          STREAMS_BOOST_IOS::openmode, any_tag )
    { 
    ...
    template<typename Device, typename Dummy>
    static void close(Device& dev, Dummy*, STREAMS_BOOST_IOS::openmode which)
...
struct flt_wrapper_impl<any_tag> {
    ...
    seek( Filter&, Device*, stream_offset,
          STREAMS_BOOST_IOS::seekdir, STREAMS_BOOST_IOS::openmode, any_tag )
    { streams_boost::throw_exception(cant_seek());
      STREAMS_BOOST_IOSTREAMS_UNREACHABLE_RETURN(0) }
    ...
    static std::streampos
    seek( Filter& f, Device* dev, stream_offset off,
          STREAMS_BOOST_IOS::seekdir way, STREAMS_BOOST_IOS::openmode,
    ...
    template<typename Filter, typename Device>
    static void close(Filter& f, Device* dev, STREAMS_BOOST_IOS::openmode which)
...
#endif // #ifndef STREAMS_BOOST_IOSTREAMS_DETAIL_CONCEPT_ADAPTER_HPP_INCLUDED
