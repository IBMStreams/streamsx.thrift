...
#ifndef STREAMS_BOOST_IOSTREAMS_DETAIL_DIRECT_ADAPTER_HPP_INCLUDED
#define STREAMS_BOOST_IOSTREAMS_DETAIL_DIRECT_ADAPTER_HPP_INCLUDED

#if defined(_MSC_VER) && (_MSC_VER >= 1020)
# pragma once
#endif              

#include <streams_boost/config.hpp>       // SFINAE, MSVC, put ptrdiff_t in std.
#include <algorithm>              // copy, min.
#include <cstddef>                // ptrdiff_t.
#include <streams_boost/detail/workaround.hpp>
#include <streams_boost/iostreams/categories.hpp>
#include <streams_boost/iostreams/detail/config/limits.hpp>        // forwarding.
#include <streams_boost/iostreams/detail/config/wide_streams.hpp>  // locale.
...
// Must come last.
#include <streams_boost/iostreams/detail/config/disable_warnings.hpp> // VC7.1
...
# define STREAMS_BOOST_PP_LOCAL_LIMITS (1, STREAMS_BOOST_IOSTREAMS_MAX_FORWARDING_ARITY)
#else
    template<typename U>
    struct is_direct
        : mpl::or_< 
    ...
    direct_adapter(const U& u) 
        ...
        { }
# define STREAMS_BOOST_PP_LOCAL_LIMITS (2, STREAMS_BOOST_IOSTREAMS_MAX_FORWARDING_ARITY)
#endif

#define STREAMS_BOOST_PP_LOCAL_MACRO(n) \
    template<STREAMS_BOOST_PP_ENUM_PARAMS(n, typename P)> \
    direct_adapter(STREAMS_BOOST_PP_ENUM_BINARY_PARAMS(n, const P, &p)) \
        : base_type(Direct(STREAMS_BOOST_PP_ENUM_PARAMS(n, p))) \
        { } \
    /**/
    ...
    (const char_type* s, std::streamsize n)
{
    ...
    if (n > static_cast<std::streamsize>(put.end - put.ptr))
        streams_boost::throw_exception(write_area_exhausted());
      ...
      STREAMS_BOOST_IOS::openmode which )
{
    ...
    pointers& get = ptrs_.first();
    pointers& put = ptrs_.second();
    if (way == STREAMS_BOOST_IOS::cur && get.ptr != put.ptr)
       streams_boost::throw_exception(bad_seek());
    ptrdiff_t next = 0;
    if ((which & STREAMS_BOOST_IOS::in) || !is_double::value) {
    ...
    if ((which & STREAMS_BOOST_IOS::out) && is_double::value) {
        if (way == STREAMS_BOOST_IOS::beg)
            next = off; 
        else if (way == STREAMS_BOOST_IOS::cur)
...
template<typename Direct>
void direct_adapter<Direct>::close(STREAMS_BOOST_IOS::openmode which) 
{ 
    STREAMS_BOOST_STATIC_ASSERT((is_convertible<category, two_sequence>::value));
    streams_boost::iostreams::close(d_, which);
}

#ifndef STREAMS_BOOST_IOSTREAMS_NO_LOCALE
    template<typename Direct>
    void direct_adapter<Direct>::imbue(const std::locale& loc) 
    { streams_boost::iostreams::imbue(d_, loc); }
#endif
...
#endif // #ifndef STREAMS_BOOST_IOSTREAMS_DETAIL_DIRECT_ADAPTER_HPP_INCLUDED
