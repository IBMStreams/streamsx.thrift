...
#ifndef STREAMS_BOOST_IOSTREAMS_DETAIL_WRAP_UNWRAP_HPP_INCLUDED
#define STREAMS_BOOST_IOSTREAMS_DETAIL_WRAP_UNWRAP_HPP_INCLUDED

#if defined(_MSC_VER) && (_MSC_VER >= 1020)
# pragma once
#endif              

#include <streams_boost/config.hpp>                             // SFINAE, MSVC.
...
//------------------Definition of wrap----------------------------------------//
    ...
    template<typename T>
    inline T wrap(const T& t STREAMS_BOOST_IOSTREAMS_DISABLE_IF_STREAM(T)) 
    { return t; }

    template<typename T>
    inline typename wrapped_type<T>::type
    wrap(T& t STREAMS_BOOST_IOSTREAMS_ENABLE_IF_STREAM(T)) { return streams_boost::ref(t); }
#else // #ifndef STREAMS_BOOST_NO_SFINAE //-------------------------------------------//
    template<typename T>
    inline typename wrapped_type<T>::type // BCC 5.x needs namespace qualification.
    wrap_impl(const T& t, mpl::true_) { return streams_boost::ref(const_cast<T&>(t)); }

    template<typename T>
    inline typename wrapped_type<T>::type // BCC 5.x needs namespace qualification.
    wrap_impl(T& t, mpl::true_) { return streams_boost::ref(t); }
...
#endif // #ifndef STREAMS_BOOST_NO_SFINAE //------------------------------------------//
...
#else // #if !STREAMS_BOOST_WORKAROUND(STREAMS_BOOST_MSVC, < 1310) //-------------------------//
...
#endif // #if !STREAMS_BOOST_WORKAROUND(STREAMS_BOOST_MSVC, < 1310) //------------------------//

} } } // End namespaces detail, iostreams, streams_boost.

#endif // #ifndef STREAMS_BOOST_IOSTREAMS_DETAIL_WRAP_UNWRAP_HPP_INCLUDED
