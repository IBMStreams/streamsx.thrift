...
#ifndef STREAMS_BOOST_IOSTREAMS_DETAIL_INDIRECT_STREAMBUF_HPP_INCLUDED
#define STREAMS_BOOST_IOSTREAMS_DETAIL_INDIRECT_STREAMBUF_HPP_INCLUDED

#include <algorithm>                             // min, max.
#include <cassert>
#include <exception>
#include <typeinfo>
#include <streams_boost/config.hpp>                      // Member template friends.
...
// Must come last.
#include <streams_boost/iostreams/detail/config/disable_warnings.hpp>  // MSVC, BCC 5.x
...
public:
    ...
    void open(const T& t STREAMS_BOOST_IOSTREAMS_PUSH_PARAMS());
    bool is_open() const;
    void close();
...
void indirect_streambuf<T, Tr, Alloc, Mode>::close()
{
    ...
    base_type* self = this;
    detail::execute_all(
        detail::call_member_close(*self, STREAMS_BOOST_IOS::in),
...
{
    if ( gptr() != 0 && way == STREAMS_BOOST_IOS::cur && which == STREAMS_BOOST_IOS::in && 
         eback() - gptr() <= off && off <= egptr() - gptr() ) 
    {   // Small seek optimization
        gbump(off);
        return obj().seek(0, STREAMS_BOOST_IOS::cur, STREAMS_BOOST_IOS::in, next_) -
               static_cast<off_type>(egptr() - gptr());
    }
    if (pptr() != 0) 
        this->STREAMS_BOOST_IOSTREAMS_PUBSYNC(); // sync() confuses VisualAge 6.
    if (way == STREAMS_BOOST_IOS::cur && gptr())
...
inline void indirect_streambuf<T, Tr, Alloc, Mode>::close_impl
    (STREAMS_BOOST_IOS::openmode which)
{
    if (which == STREAMS_BOOST_IOS::in && is_convertible<Mode, input>::value) {
        setg(0, 0, 0);
    }
    if (which == STREAMS_BOOST_IOS::out && is_convertible<Mode, output>::value) {
        sync();
        setp(0, 0);
    }
    if ( !is_convertible<category, dual_use>::value ||
         is_convertible<Mode, input>::value == (which == STREAMS_BOOST_IOS::in) )
    {
...
#include <streams_boost/iostreams/detail/config/enable_warnings.hpp> // MSVC, BCC 5.x

#endif // #ifndef STREAMS_BOOST_IOSTREAMS_DETAIL_INDIRECT_STREAMBUF_HPP_INCLUDED
