 ...
 *
 * File:        streams_boost/iostreams/detail/path.hpp
 * Date:        Sat Jun 21 21:24:05 MDT 2008
 * Copyright:   2008 CodeRage, LLC
 * Author:      Jonathan Turkanis
 * Contact:     turkanis at coderage dot com
 *
 * Defines the class streams_boost::iostreams::detail::path, for storing a 
...
#ifndef STREAMS_BOOST_IOSTREAMS_DETAIL_PATH_HPP_INCLUDED
#define STREAMS_BOOST_IOSTREAMS_DETAIL_PATH_HPP_INCLUDED
...
public:
    ...
    // Constructor taking a streams_boost::filesystem::path or streams_boost::filesystem::wpath
    template<typename Path>
    explicit path(const Path& p)
    {
        typedef typename Path::external_string_type string_type;
        init(p, streams_boost::type<string_type>());
    }
...
private:
    ...
    // For wide-character paths, use a streams_boost::filesystem::wpath instead of a
    // std::wstring
    path(const std::wstring&);
    path& operator=(const std::wstring&);

    template<typename Path>
    void init(const Path& p, streams_boost::type<std::string>)
    {
    ...
    template<typename Path>
    void init(const Path& p, streams_boost::type<std::wstring>)
    {
        narrow_.clear();
...
#else // #ifndef STREAMS_BOOST_IOSTREAMS_NO_WIDE_STREAMS //---------------------------//
...
#endif // #ifndef STREAMS_BOOST_IOSTREAMS_DETAIL_PATH_HPP_INCLUDED
