...
#ifndef STREAMS_BOOST_IOSTREAMS_FILE_HPP_INCLUDED
#define STREAMS_BOOST_IOSTREAMS_FILE_HPP_INCLUDED
...
#include <streams_boost/iostreams/detail/fstream.hpp>
#include <streams_boost/iostreams/operations.hpp>       // seek.
...
public:
    ...
    basic_file( const std::string& path,
                ...
                STREAMS_BOOST_IOS::openmode base_mode =
                    STREAMS_BOOST_IOS::in | STREAMS_BOOST_IOS::out );
...
private:
    struct impl {
        impl(const std::string& path, STREAMS_BOOST_IOS::openmode mode)
            { file_.open(path.c_str(), mode); }
        ~impl() { if (file_.is_open()) file_.close(); }
        STREAMS_BOOST_IOSTREAMS_BASIC_FILEBUF(Ch) file_;
    };
...
struct basic_file_source : private basic_file<Ch> {
    ...
    basic_file_source( const std::string& path,
                       STREAMS_BOOST_IOS::openmode mode = 
                           STREAMS_BOOST_IOS::in )
        : basic_file<Ch>(path, mode & ~STREAMS_BOOST_IOS::out, STREAMS_BOOST_IOS::in)
        { }
    void open( const std::string& path,
               STREAMS_BOOST_IOS::openmode mode = STREAMS_BOOST_IOS::in )
    {
        basic_file<Ch>::open(path, mode & ~STREAMS_BOOST_IOS::out, STREAMS_BOOST_IOS::in);
    }
};
...
struct basic_file_sink : private basic_file<Ch> {
    ...
    basic_file_sink( const std::string& path,
        ...
        : basic_file<Ch>(path, mode & ~STREAMS_BOOST_IOS::in, STREAMS_BOOST_IOS::out)
        { }
    void open( const std::string& path,
               STREAMS_BOOST_IOS::openmode mode = STREAMS_BOOST_IOS::out )
    {
        basic_file<Ch>::open(path, mode & ~STREAMS_BOOST_IOS::in, STREAMS_BOOST_IOS::out);
    }
};
...
basic_file<Ch>::basic_file
    ( const std::string& path, STREAMS_BOOST_IOS::openmode mode, 
      STREAMS_BOOST_IOS::openmode base_mode )
{ 
...
#endif // #ifndef STREAMS_BOOST_IOSTREAMS_FILE_HPP_INCLUDED
