...
// (C) Copyright Craig Henderson 2002.   'streams_boost/memmap.hpp' from sandbox
// Distributed under the Boost Software License, Version 1.0. (See accompanying
// file LICENSE_1_0.txt or copy at http://www.streams_boost.org/LICENSE_1_0.txt.)

#ifndef STREAMS_BOOST_IOSTREAMS_MAPPED_FILE_HPP_INCLUDED
#define STREAMS_BOOST_IOSTREAMS_MAPPED_FILE_HPP_INCLUDED
...
#include <streams_boost/iostreams/close.hpp>
#include <streams_boost/iostreams/concepts.hpp>
...
#include <streams_boost/iostreams/detail/ios.hpp>     // openmode, failure
...
public:
    ...
    STREAMS_BOOST_IOS::openmode         mode;  // Deprecated
...
// streams_boost::filesystem::path, streams_boost::filesystem::wpath, 
// and streams_boost::iostreams::detail::path (used to store either a std::string or a
// std::wstring).
    ...
    : detail::mapped_file_params_base 
{
    ...
    // with streams_boost::filesystem::wpath
#ifndef STREAMS_BOOST_IOSTREAMS_NO_WIDE_STREAMS
    STREAMS_BOOST_STATIC_ASSERT((!is_same<Path, std::wstring>::value));
#endif
...
class STREAMS_BOOST_IOSTREAMS_DECL mapped_file_source : public mapped_file_base {
private:
    ...
    friend struct streams_boost::iostreams::operations<mapped_file_source>;
public:
    ...
    STREAMS_BOOST_STATIC_CONSTANT(size_type, max_length = static_cast<size_type>(-1));
    ...
    explicit mapped_file_source( const Path& path,
                                 ...
                                 streams_boost::intmax_t offset = 0 );
...
private:
    ...
    streams_boost::shared_ptr<impl_type> pimpl_;
};

//------------------Definition of mapped_file---------------------------------//

class STREAMS_BOOST_IOSTREAMS_DECL mapped_file : public mapped_file_base {
private:
    typedef mapped_file_source                      delegate_type;
    typedef delegate_type::safe_bool                safe_bool;
    typedef basic_mapped_file_params<detail::path>  param_type;
    friend struct streams_boost::iostreams::operations<mapped_file >;
...
public:
    ...
    STREAMS_BOOST_STATIC_CONSTANT(size_type, max_length = delegate_type::max_length);
...
class STREAMS_BOOST_IOSTREAMS_DECL mapped_file_sink : private mapped_file {
public:
    friend struct streams_boost::iostreams::operations<mapped_file_sink>;
...
mapped_file_source::mapped_file_source( 
    const Path& path, size_type length, streams_boost::intmax_t offset)
...
void mapped_file_source::open(const basic_mapped_file_params<Path>& p)
{
    ...
    if (params.flags) {
        if (params.flags != mapped_file::readonly)
            streams_boost::throw_exception(STREAMS_BOOST_IOSTREAMS_FAILURE("invalid flags"));
    } else {
        if (params.mode & STREAMS_BOOST_IOS::out)
            streams_boost::throw_exception(STREAMS_BOOST_IOSTREAMS_FAILURE("invalid mode"));
        params.mode |= STREAMS_BOOST_IOS::in;
    }
    open_impl(params);
}
...
template<typename Path>
mapped_file::mapped_file( 
    const Path& path, STREAMS_BOOST_IOS::openmode mode, 
    size_type length, stream_offset offset )
...
mapped_file_sink::mapped_file_sink(
    ...
    streams_boost::intmax_t offset, mapmode flags )
...
void mapped_file_sink::open(const basic_mapped_file_params<Path>& p)
{
    ...
    if (params.flags) {
        ...
        if (params.mode & STREAMS_BOOST_IOS::in)
            streams_boost::throw_exception(STREAMS_BOOST_IOSTREAMS_FAILURE("invalid mode"));
        params.mode |= STREAMS_BOOST_IOS::out;
    }
...
struct operations<mapped_file_source>
    : streams_boost::iostreams::detail::close_impl<closable_tag>
{
...
#endif // #ifndef STREAMS_BOOST_IOSTREAMS_MAPPED_FILE_HPP_INCLUDED
