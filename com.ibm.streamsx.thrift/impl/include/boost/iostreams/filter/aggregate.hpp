...
#ifndef STREAMS_BOOST_IOSTREAMS_AGGREGATE_FILTER_HPP_INCLUDED
#define STREAMS_BOOST_IOSTREAMS_AGGREGATE_FILTER_HPP_INCLUDED
...
#include <vector>
#include <streams_boost/iostreams/constants.hpp>      // default_device_buffer_size 
...
#include <streams_boost/iostreams/read.hpp>           // check_eof 
...
public:
    ...
    std::streamsize read(Source& src, char_type* s, std::streamsize n)
    {
        ...
        if (amt) {
            STREAMS_BOOST_IOSTREAMS_CHAR_TRAITS(char_type)::copy(s, &data_[ptr_], amt);
            ptr_ += amt;
        }
    ...
    template<typename Sink>
    void close(Sink& sink, STREAMS_BOOST_IOS::openmode which)
    {
        if ((state_ & f_read) != 0 && which == STREAMS_BOOST_IOS::in)
            close_impl();
        if ((state_ & f_write) != 0 && which == STREAMS_BOOST_IOS::out) {
            try {
...
private:
    ...
    void do_read(Source& src)
    {
        ...
        while (true) {
            ...
            Ch                     buf[size];
            std::streamsize        amt;
            if ((amt = streams_boost::iostreams::read(src, buf, size)) == -1)
                break;
...
STREAMS_BOOST_IOSTREAMS_PIPABLE(aggregate_filter, 1)

} } // End namespaces iostreams, streams_boost.

#include <streams_boost/iostreams/detail/config/enable_warnings.hpp>  // MSVC.

#endif // #ifndef STREAMS_BOOST_IOSTREAMS_AGGREGATE_FILTER_HPP_INCLUDED
