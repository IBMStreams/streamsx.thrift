...
#ifndef STREAMS_BOOST_IOSTREAMS_SYMMETRIC_FILTER_HPP_INCLUDED
#define STREAMS_BOOST_IOSTREAMS_SYMMETRIC_FILTER_HPP_INCLUDED
...
#include <streams_boost/iostreams/operations.hpp>       // read, write.
...
template< typename SymmetricFilter,
          typename Alloc =
              std::allocator<
                  STREAMS_BOOST_DEDUCED_TYPENAME char_type_of<SymmetricFilter>::type
              > >
class symmetric_filter {
public:
    typedef typename char_type_of<SymmetricFilter>::type      char_type;
    typedef STREAMS_BOOST_IOSTREAMS_CHAR_TRAITS(char_type)            traits_type;
    ...
    #define STREAMS_BOOST_PP_LOCAL_MACRO(n) \
        STREAMS_BOOST_IOSTREAMS_TEMPLATE_PARAMS(n, T) \
        explicit symmetric_filter( \
              int buffer_size STREAMS_BOOST_PP_COMMA_IF(n) \
              STREAMS_BOOST_PP_ENUM_BINARY_PARAMS(n, const T, &t) ) \
            : pimpl_(new impl(buffer_size STREAMS_BOOST_PP_COMMA_IF(n) \
                     STREAMS_BOOST_PP_ENUM_PARAMS(n, t))) \
            { } \
        /**/
    #define STREAMS_BOOST_PP_LOCAL_LIMITS (0, STREAMS_BOOST_IOSTREAMS_MAX_FORWARDING_ARITY)
    ...
    template<typename Sink>
    void close(Sink& snk, STREAMS_BOOST_IOS::openmode)
    {
        if ((state() & f_write) != 0) {
...
private:
    ...
    bool flush(Sink& snk, mpl::true_)
    {
        ...
        std::streamsize result =
            streams_boost::iostreams::write(snk, buf().data(), amt);
    ...
    #define STREAMS_BOOST_PP_LOCAL_MACRO(n) \
        STREAMS_BOOST_IOSTREAMS_TEMPLATE_PARAMS(n, T) \
        impl( int buffer_size STREAMS_BOOST_PP_COMMA_IF(n) \
              STREAMS_BOOST_PP_ENUM_BINARY_PARAMS(n, const T, &t) ) \
            : SymmetricFilter(STREAMS_BOOST_PP_ENUM_PARAMS(n, t)), \
...
STREAMS_BOOST_IOSTREAMS_PIPABLE(symmetric_filter, 2)
...
#endif // #ifndef STREAMS_BOOST_IOSTREAMS_SYMMETRIC_FILTER_HPP_INCLUDED
