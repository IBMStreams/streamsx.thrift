...
#ifndef STREAMS_BOOST_IOSTREAMS_LINE_FILTER_HPP_INCLUDED
#define STREAMS_BOOST_IOSTREAMS_LINE_FILTER_HPP_INCLUDED
...
#include <streams_boost/iostreams/checked_operations.hpp>
...
// Must come last.
#include <streams_boost/iostreams/detail/config/disable_warnings.hpp> // VC7.1 C4244.
...
public:
    ...
    {
        if ((flags_ & f_read) && which == STREAMS_BOOST_IOS::in)
            close_impl();

        if ((flags_ & f_write) && which == STREAMS_BOOST_IOS::out) {
            try {
                if (!cur_line_.empty())
                    write_line(snk);
            } catch (...) {
...
STREAMS_BOOST_IOSTREAMS_PIPABLE(basic_line_filter, 2)
...
#endif // #ifndef STREAMS_BOOST_IOSTREAMS_LINE_FILTER_HPP_INCLUDED
