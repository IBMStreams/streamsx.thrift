...
#ifndef STREAMS_BOOST_IOSTREAMS_NEWLINE_FILTER_HPP_INCLUDED
#define STREAMS_BOOST_IOSTREAMS_NEWLINE_FILTER_HPP_INCLUDED
...
#include <streams_boost/iostreams/detail/ios.hpp>  // STREAMS_BOOST_IOSTREAMS_FAILURE 
#include <streams_boost/iostreams/read.hpp>        // get 
#include <streams_boost/iostreams/write.hpp>       // put 
...
#define STREAMS_BOOST_IOSTREAMS_ASSERT_UNREACHABLE(val) \
...
class newline_error
    : public STREAMS_BOOST_IOSTREAMS_FAILURE, public detail::newline_base
{
private:
    friend class newline_checker;
    newline_error(int flags)
        : STREAMS_BOOST_IOSTREAMS_FAILURE("bad line endings"),
...
public:
    ...
    explicit newline_filter(int target) : flags_(target)
    {
        if ( target != newline::posix &&
             ...
             target != newline::mac )
        {
            streams_boost::throw_exception(std::logic_error("bad flags"));
        }
    }
    ...
    template<typename Sink>
    void close(Sink& dest, STREAMS_BOOST_IOS::openmode)
    {
...
private:
    ...
    int newline()
    {
        ...
        return STREAMS_BOOST_IOSTREAMS_ASSERT_UNREACHABLE(0);
    }
    ...
    bool newline(Sink& dest)
    {
        ...
        switch (flags_ & newline::platform_mask) {
        case newline::posix:
            success = streams_boost::iostreams::put(dest, LF);
            break;
        case newline::mac:
            success = streams_boost::iostreams::put(dest, CR);
            break;
        case newline::dos:
            if ((flags_ & f_has_LF) != 0) {
                if ((success = streams_boost::iostreams::put(dest, LF)))
                    flags_ &= ~f_has_LF;
            } else if (streams_boost::iostreams::put(dest, CR)) {
                if (!(success = streams_boost::iostreams::put(dest, LF)))
                    flags_ |= f_has_LF;
            }
            break;
...
STREAMS_BOOST_IOSTREAMS_PIPABLE(newline_filter, 0)
...
public:
    ...
    template<typename Sink>
    void close(Sink&, STREAMS_BOOST_IOS::openmode)
    {
...
private:
    void fail() { streams_boost::throw_exception(newline_error(source())); }
...
STREAMS_BOOST_IOSTREAMS_PIPABLE(newline_checker, 0)

} } // End namespaces iostreams, streams_boost.

#include <streams_boost/iostreams/detail/config/enable_warnings.hpp>

#endif // #ifndef STREAMS_BOOST_IOSTREAMS_NEWLINE_FILTER_HPP_INCLUDED
