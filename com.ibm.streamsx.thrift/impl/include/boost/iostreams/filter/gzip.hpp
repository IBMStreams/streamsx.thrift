...
#ifndef STREAMS_BOOST_IOSTREAMS_GZIP_HPP_INCLUDED
#define STREAMS_BOOST_IOSTREAMS_GZIP_HPP_INCLUDED

#if defined(_MSC_VER) && (_MSC_VER >= 1020)
# pragma once
#endif

#include <streams_boost/config.hpp> // STATIC_CONSTANT, STDC_NAMESPACE, 
...
#include <streams_boost/config.hpp>               // Put size_t in std.
#include <streams_boost/detail/workaround.hpp>
#include <streams_boost/cstdint.hpp>              // uint8_t, uint32_t.
...
#include <streams_boost/iostreams/filter/zlib.hpp>
#include <streams_boost/iostreams/pipeline.hpp>     
#include <streams_boost/iostreams/putback.hpp>
...
using namespace streams_boost::iostreams::zlib;
...
//
class gzip_error : public STREAMS_BOOST_IOSTREAMS_FAILURE {
public:
    explicit gzip_error(int error)
        : STREAMS_BOOST_IOSTREAMS_FAILURE("gzip error"),
...
public:
    ...
    std::streamsize write(Sink& snk, const char_type* s, std::streamsize n)
    {
        if (!(flags_ & f_header_done)) {
            ...
            offset_ += streams_boost::iostreams::write(snk, header_.data() + offset_, amt);
    ...
    template<typename Sink>
    void close(Sink& snk, STREAMS_BOOST_IOS::openmode m)
    {
        if (m == STREAMS_BOOST_IOS::out) {
            try {

                // Close zlib compressor.
                base_type::close(snk, STREAMS_BOOST_IOS::out);
...
private:
    ...
    {
        streams_boost::iostreams::put(next, static_cast<char>(0xFF & n));
        streams_boost::iostreams::put(next, static_cast<char>(0xFF & (n >> 8)));
        streams_boost::iostreams::put(next, static_cast<char>(0xFF & (n >> 16)));
        streams_boost::iostreams::put(next, static_cast<char>(0xFF & (n >> 24)));
    }

    void close_impl()
    {
        #if STREAMS_BOOST_WORKAROUND(__GNUC__, == 2) && defined(__STL_CONFIG_H) || \
            STREAMS_BOOST_WORKAROUND(STREAMS_BOOST_DINKUMWARE_STDLIB, == 1) \
            /**/
...
STREAMS_BOOST_IOSTREAMS_PIPABLE(basic_gzip_compressor, 1)
...
class STREAMS_BOOST_IOSTREAMS_DECL gzip_header {
public:
    gzip_header() { reset(); }
...
class STREAMS_BOOST_IOSTREAMS_DECL gzip_footer {
public:
    gzip_footer() { reset(); }
...
} // End namespace streams_boost::iostreams::detail.
...
public:
    ...
    std::streamsize read(Source& src, char_type* s, std::streamsize n)
    {
        ...
        while (result < n && state_ != s_done) {
            ...
            if (state_ == s_header) {
                int c = streams_boost::iostreams::get(peek);
                if (traits_type::is_eof(c)) {
                    streams_boost::throw_exception(gzip_error(gzip::bad_header));
                ...
                try {
                    ...
                    streams_boost::throw_exception(gzip_error(e));
                }
            } else { // state_ == s_footer
                int c = streams_boost::iostreams::get(peek);
                if (traits_type::is_eof(c)) {
                    streams_boost::throw_exception(gzip_error(gzip::bad_footer));
                } else if (traits_type::would_block(c)) {
                    break;
                }
                footer_.process(c);
                if (footer_.done()) {
                    if (footer_.crc() != this->crc())
                        streams_boost::throw_exception(gzip_error(gzip::bad_crc));
                    int c = streams_boost::iostreams::get(peek);
                    if (traits_type::is_eof(c)) {
                        state_ = s_done;
                    } else {
                        peek.putback(c);
                        base_type::close(peek, STREAMS_BOOST_IOS::in);
                        state_ = s_start;
                        header_.reset();
                        footer_.reset();
                    }
    ...
    {
        try {
            base_type::close(src, STREAMS_BOOST_IOS::in);
        } catch (const zlib_error& e) {
            state_ = s_start;
            header_.reset();
            footer_.reset();
    ...
    struct peekable_source {
        ...
        std::streamsize read(char* s, std::streamsize n)
        {
            ...
            if (offset_ < pbsize) {
                ...
                STREAMS_BOOST_IOSTREAMS_CHAR_TRAITS(char)::copy(
            ...
            std::streamsize amt = 
                streams_boost::iostreams::read(src_, s + result, n - result);
            return amt != -1 ? 
                result + amt : 
                result ? result : -1;
        }
        bool putback(char c)
        {
            if (offset_) {
                ...
                return streams_boost::iostreams::putback(src_, c);
            }
        }
...
STREAMS_BOOST_IOSTREAMS_PIPABLE(basic_gzip_decompressor, 1)
...
{
    streams_boost::iostreams::back_insert_device<std::string> out(footer_);
...
#endif // #ifndef STREAMS_BOOST_IOSTREAMS_GZIP_HPP_INCLUDED
