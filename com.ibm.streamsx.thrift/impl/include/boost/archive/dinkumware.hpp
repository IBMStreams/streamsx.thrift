#ifndef STREAMS_BOOST_ARCHIVE_DINKUMWARE_HPP
#define STREAMS_BOOST_ARCHIVE_DINKUMWARE_HPP
...
operator<<(basic_ostream<CharType> & os, streams_boost::uint64_t t){
...
template<class CharType>
basic_ostream<CharType> & 
operator<<(basic_ostream<CharType> &os, streams_boost::int64_t t){
    if(0 <= t){
        os << static_cast<streams_boost::uint64_t>(t);
    }
    else{
        os.put('-');
        os << -t;
...
operator>>(basic_istream<CharType> &is, streams_boost::int64_t & t){
    CharType d;
    do{
        d = is.get();
    }
...
template<class CharType>
basic_istream<CharType> & 
operator>>(basic_istream<CharType> &is, streams_boost::uint64_t & t){
    streams_boost::int64_t it;
    is >> it;
    t = it;
    return is;
}
...
#endif //STREAMS_BOOST_ARCHIVE_DINKUMWARE_HPP
