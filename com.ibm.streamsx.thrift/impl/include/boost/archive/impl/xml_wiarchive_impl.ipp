...
#include <streams_boost/detail/workaround.hpp> // Dinkumware and RogueWave
...
void copy_to_ptr(char * s, const std::wstring & ws){
    std::copy(
        iterators::mb_from_wchar<std::wstring::const_iterator>(
            STREAMS_BOOST_MAKE_PFTO_WRAPPER(ws.begin())
        ), 
        iterators::mb_from_wchar<std::wstring::const_iterator>(
            STREAMS_BOOST_MAKE_PFTO_WRAPPER(ws.end())
        ), 
        s
    );
    s[ws.size()] = 0;
...
xml_wiarchive_impl<Archive>::load_override(class_name_type & t, int){
    ...
    if(ws.size() > STREAMS_BOOST_SERIALIZATION_MAX_KEY_SIZE - 1)
        streams_boost::serialization::throw_exception(
    ...
    gimpl(new xml_wgrammar())
{
    if(0 == (flags & no_codecvt)){
        archive_locale.reset(
            add_facet(
                std::locale::classic(),
                new streams_boost::archive::detail::utf8_codecvt_facet
            )
        );
