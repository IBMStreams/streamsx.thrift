...
#include <streams_boost/archive/wcslen.hpp>
#include <streams_boost/archive/iterators/mb_from_wchar.hpp>
#endif
...
void save_iterator(std::ostream &os, InputIterator begin, InputIterator end){
    typedef streams_boost::archive::iterators::mb_from_wchar<
        streams_boost::archive::iterators::xml_escape<InputIterator>
    > translator;
    std::copy(
        translator(STREAMS_BOOST_MAKE_PFTO_WRAPPER(begin)), 
        translator(STREAMS_BOOST_MAKE_PFTO_WRAPPER(end)), 
        streams_boost::archive::iterators::ostream_iterator<char>(os)
    );
}
...
//  at least one library doesn't typedef value_type for strings
//  so rather than using string directly make a pointer iterator out of it
    typedef streams_boost::archive::iterators::xml_escape<
        const char * 
    > xml_escape_translator;
    std::copy(
        xml_escape_translator(STREAMS_BOOST_MAKE_PFTO_WRAPPER(s.data())),
        xml_escape_translator(STREAMS_BOOST_MAKE_PFTO_WRAPPER(s.data()+ s.size())), 
        streams_boost::archive::iterators::ostream_iterator<char>(os)
    );
}
...
xml_oarchive_impl<Archive>::save(const char * s){
    ...
    > xml_escape_translator;
    std::copy(
        xml_escape_translator(STREAMS_BOOST_MAKE_PFTO_WRAPPER(s)),
        xml_escape_translator(STREAMS_BOOST_MAKE_PFTO_WRAPPER(s + std::strlen(s))), 
        streams_boost::archive::iterators::ostream_iterator<char>(os)
    );
}
