#ifndef STREAMS_BOOST_ARCHIVE_BASIC_TEXT_OPRIMITIVE_HPP
#define STREAMS_BOOST_ARCHIVE_BASIC_TEXT_OPRIMITIVE_HPP
...
#include <iomanip>
#include <locale>
#include <streams_boost/config/no_tr1/cmath.hpp> // isnan
#include <cassert>
...
#include <streams_boost/archive/dinkumware.hpp>
#endif

#if defined(STREAMS_BOOST_NO_STDC_NAMESPACE)
namespace std{ 
    using ::size_t;
    #if ! defined(STREAMS_BOOST_DINKUMWARE_STDLIB) && ! defined(__SGI_STL_PORT)
        using ::locale;
    #endif
} // namespace std
#endif
...
public:
    ...
    basic_streambuf_locale_saver<
        STREAMS_BOOST_DEDUCED_TYPENAME OStream::char_type, 
        STREAMS_BOOST_DEDUCED_TYPENAME OStream::traits_type
    > locale_saver;
    #endif
...
public:
    ...
    void put(STREAMS_BOOST_DEDUCED_TYPENAME OStream::char_type c){
        if(os.fail())
            streams_boost::serialization::throw_exception(
                archive_exception(archive_exception::stream_error)
            );
...
#endif // STREAMS_BOOST_ARCHIVE_BASIC_TEXT_OPRIMITIVE_HPP
