...
#define STREAMS_BOOST_REGEX_V4_SUB_MATCH_HPP
...
struct sub_match : public std::pair<BidiIterator, BidiIterator>
{
...
#endif
   difference_type STREAMS_BOOST_REGEX_CALL length()const
   {
      difference_type n = ::streams_boost::re_detail::distance((BidiIterator)this->first, (BidiIterator)this->second);
      return n;
   }
   std::basic_string<value_type> str()const
   {
      std::basic_string<value_type> result;
      std::size_t len = ::streams_boost::re_detail::distance((BidiIterator)this->first, (BidiIterator)this->second);
      result.reserve(len);
   ...
   bool STREAMS_BOOST_REGEX_CALL operator !=(const sub_match& that)const
   { return compare(that) != 0; }
   bool operator<(const sub_match& that)const
   { return compare(that) < 0; }
   bool operator>(const sub_match& that)const
...
private:
   mutable streams_boost::scoped_ptr<capture_sequence_type> m_captures;
public:
...
#ifdef STREAMS_BOOST_OLD_REGEX_H
   //
...
#if !(defined(__GNUC__) && defined(STREAMS_BOOST_NO_STD_LOCALE))
template <class RandomAccessIterator>
