...
#ifndef STREAMS_BOOST_REGEX_V4_PERL_MATCHER_COMMON_HPP
#define STREAMS_BOOST_REGEX_V4_PERL_MATCHER_COMMON_HPP
...
void perl_matcher<BidiIterator, Allocator, traits>::estimate_max_state_count(std::random_access_iterator_tag*)
{
   ...
   //
   static const streams_boost::uintmax_t k = 100000;
   streams_boost::uintmax_t dist = streams_boost::re_detail::distance(base, last);
   if(dist == 0)
      dist = 1;
   streams_boost::uintmax_t states = re.size();
   if(states == 0)
      states = 1;
   states *= states;
   if((std::numeric_limits<streams_boost::uintmax_t>::max)() / dist < states)
   {
      max_state_count = (std::numeric_limits<streams_boost::uintmax_t>::max)() - 2;
      return;
   }
   states *= dist;
   if((std::numeric_limits<streams_boost::uintmax_t>::max)() - k < states)
   {
      max_state_count = (std::numeric_limits<streams_boost::uintmax_t>::max)() - 2;
      return;
   }
   ...
   //
   if(states > STREAMS_BOOST_REGEX_MAX_STATE_COUNT)
      states = STREAMS_BOOST_REGEX_MAX_STATE_COUNT;
   //
...
inline void perl_matcher<BidiIterator, Allocator, traits>::estimate_max_state_count(void*)
{
   ...
   max_state_count = STREAMS_BOOST_REGEX_MAX_STATE_COUNT;
}
...
{
   ::streams_boost::re_detail::concrete_protected_call
...
bool perl_matcher<BidiIterator, Allocator, traits>::match_imp()
{
   ...
   used_block_count = STREAMS_BOOST_REGEX_MAX_BLOCKS;
...
#if defined(STREAMS_BOOST_REGEX_NON_RECURSIVE) && !defined(STREAMS_BOOST_NO_EXCEPTIONS)
   }
   catch(...)
   {
...
bool perl_matcher<BidiIterator, Allocator, traits>::match_word_boundary()
{
   ...
   #if defined(STREAMS_BOOST_REGEX_USE_C_LOCALE) && defined(__GNUC__) && (__GNUC__ == 2) && (__GNUC_MINOR__ < 95)
...
#pragma warning(disable:4127)
   ...
   {
      std::ptrdiff_t maxlen = ::streams_boost::re_detail::distance(backstop, position);
