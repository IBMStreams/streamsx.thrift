...
#include <streams_boost/fusion/container/vector/vector_fwd.hpp>
#include <streams_boost/fusion/container/vector/detail/vector_n_chooser.hpp>
...
namespace streams_boost { namespace fusion
{
    ...
    struct vector
        : sequence_base<vector<STREAMS_BOOST_PP_ENUM_PARAMS(FUSION_MAX_VECTOR_SIZE, T)> >
    {
    private:

        typedef typename detail::vector_n_chooser<
            STREAMS_BOOST_PP_ENUM_PARAMS(FUSION_MAX_VECTOR_SIZE, T)>::type
        vector_n;

        template <STREAMS_BOOST_PP_ENUM_PARAMS(FUSION_MAX_VECTOR_SIZE, typename U)>
    ...
    public:
        ...
        template <STREAMS_BOOST_PP_ENUM_PARAMS(FUSION_MAX_VECTOR_SIZE, typename U)>
        vector(vector<STREAMS_BOOST_PP_ENUM_PARAMS(FUSION_MAX_VECTOR_SIZE, U)> const& rhs)
            : vec(rhs.vec) {}
...
#if STREAMS_BOOST_WORKAROUND(STREAMS_BOOST_MSVC, <= 1500)
        ...
        //    : vec(_0, _1) {}
        #include <streams_boost/fusion/container/vector/detail/vector_forward_ctor.hpp>

        template <STREAMS_BOOST_PP_ENUM_PARAMS(FUSION_MAX_VECTOR_SIZE, typename U)>
        vector&
        operator=(vector<STREAMS_BOOST_PP_ENUM_PARAMS(FUSION_MAX_VECTOR_SIZE, U)> const& rhs)
        {
            vec = rhs.vec;
            return *this;
        }
