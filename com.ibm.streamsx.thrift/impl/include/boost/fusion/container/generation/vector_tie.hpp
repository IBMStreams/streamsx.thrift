...
#define STREAMS_BOOST_PP_FILENAME_1 <streams_boost/fusion/container/generation/vector_tie.hpp>
...
#define N STREAMS_BOOST_PP_ITERATION()
    ...
    namespace result_of
    {
        ...
        struct vector_tie< STREAMS_BOOST_PP_ENUM_PARAMS(N, T) STREAMS_BOOST_PP_REPEAT_FROM_TO(STREAMS_BOOST_PP_DEC(N), FUSION_MAX_VECTOR_SIZE, TEXT, void_) >
        #undef TEXT
#else
        struct vector_tie<STREAMS_BOOST_PP_ENUM_PARAMS(N, T)>
#endif
        {
            typedef vector<STREAMS_BOOST_PP_ENUM(N, STREAMS_BOOST_FUSION_REF, _)> type;
        };
    }

    template <STREAMS_BOOST_PP_ENUM_PARAMS(N, typename T)>
    inline vector<STREAMS_BOOST_PP_ENUM(N, STREAMS_BOOST_FUSION_REF, _)>
    vector_tie(STREAMS_BOOST_PP_ENUM_BINARY_PARAMS(N, T, & _))
    {
        return vector<STREAMS_BOOST_PP_ENUM(N, STREAMS_BOOST_FUSION_REF, _)>(
