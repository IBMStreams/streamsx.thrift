...
#if !defined(STREAMS_BOOST_FUSION_FUNCTIONAL_INVOCATION_DETAIL_THAT_PTR_HPP_INCLUDED)
#define STREAMS_BOOST_FUSION_FUNCTIONAL_INVOCATION_DETAIL_THAT_PTR_HPP_INCLUDED
...
namespace streams_boost { namespace fusion { namespace detail
{
    ...
    struct that_ptr
    {
      ...
      public:
        ...
        static inline pointee * get(pointee & x)
        {
            return streams_boost::addressof(x); 
        }

        template <typename T> static inline pointee * get(T & x)
        {
            return do_get_pointer(x, streams_boost::addressof(x)); 
        }
    };
    ...
    namespace adl_barrier
    {
        ...
        void const * STREAMS_BOOST_TT_DECL get_pointer(...); // fallback
