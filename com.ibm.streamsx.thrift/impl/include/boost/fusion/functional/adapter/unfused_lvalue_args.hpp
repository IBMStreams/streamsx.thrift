...
#if !defined(STREAMS_BOOST_FUSION_FUNCTIONAL_ADAPTER_UNFUSED_LVALUE_ARGS_HPP_INCLUDED)
    ...
    template <class Function> class unfused_lvalue_args
    {
      ...
      public:
        ...
        #define  STREAMS_BOOST_PP_FILENAME_1 \
            <streams_boost/fusion/functional/adapter/unfused_lvalue_args.hpp>
        #define  STREAMS_BOOST_PP_ITERATION_LIMITS \
            (1,STREAMS_BOOST_FUSION_UNFUSED_LVALUE_ARGS_MAX_ARITY)
...
{
    template<class F>
    struct result_of< streams_boost::fusion::unfused_lvalue_args<F> const () >
    {
        typedef typename streams_boost::fusion::unfused_lvalue_args<F>::call_const_0_result type;
    };
    template<class F>
    struct result_of< streams_boost::fusion::unfused_lvalue_args<F>() >
    {
        typedef typename streams_boost::fusion::unfused_lvalue_args<F>::call_0_result type;
    };
}

#define STREAMS_BOOST_FUSION_FUNCTIONAL_ADAPTER_UNFUSED_LVALUE_ARGS_HPP_INCLUDED
...
#define N STREAMS_BOOST_PP_ITERATION()
        ...
        struct result< Self const (STREAMS_BOOST_PP_ENUM_PARAMS(N,T)) >
            : streams_boost::result_of< function_c(
                STREAMS_BOOST_PP_CAT(fusion::vector,N)< STREAMS_BOOST_PP_ENUM_BINARY_PARAMS(N,
                    typename detail::mref<T,>::type STREAMS_BOOST_PP_INTERCEPT) > & )>
        { };
        ...
        inline typename streams_boost::result_of<function_c(STREAMS_BOOST_PP_CAT(fusion::vector,N)
            <STREAMS_BOOST_PP_ENUM_BINARY_PARAMS(N,T,& STREAMS_BOOST_PP_INTERCEPT)> & )>::type
        operator()(STREAMS_BOOST_PP_ENUM_BINARY_PARAMS(N,T,& a)) const
        {
            STREAMS_BOOST_PP_CAT(fusion::vector,N)<
                  STREAMS_BOOST_PP_ENUM_BINARY_PARAMS(N,T,& STREAMS_BOOST_PP_INTERCEPT) >
        ...
        inline typename streams_boost::result_of<function(STREAMS_BOOST_PP_CAT(fusion::vector,N)
            <STREAMS_BOOST_PP_ENUM_BINARY_PARAMS(N,T,& STREAMS_BOOST_PP_INTERCEPT)> & )>::type
        operator()(STREAMS_BOOST_PP_ENUM_BINARY_PARAMS(N,T,& a)) 
        {
            STREAMS_BOOST_PP_CAT(fusion::vector,N)<
                  STREAMS_BOOST_PP_ENUM_BINARY_PARAMS(N,T,& STREAMS_BOOST_PP_INTERCEPT) >
                arg(STREAMS_BOOST_PP_ENUM_PARAMS(N,a));
