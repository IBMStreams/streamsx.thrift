...
#if !defined(STREAMS_BOOST_LAMBDA_CONTROL_CONSTRUCTS_HPP)
#define STREAMS_BOOST_LAMBDA_CONTROL_CONSTRUCTS_HPP
...
struct return_type_2_ifthenelsereturn<1, false, false, false, A, B> {
  ...
  typedef const typename streams_boost::remove_reference<A>::type plainA; 
  typedef const typename streams_boost::remove_reference<B>::type plainB; 
  // TODO: Add support for volatile ?

  typedef typename
       return_type_2_ifthenelsereturn<
         2,
         streams_boost::is_convertible<plainA,plainB>::value, 
         streams_boost::is_convertible<plainB,plainA>::value,
         streams_boost::is_same<plainA,plainB>::value,
         plainA, 
         plainB>::type type;
};
...
struct return_type_2<other_action<ifthenelsereturn_action>, A, B> { 
  ...
  typedef typename 
    streams_boost::add_const<typename detail::same_or_not<A1, B1>::type>::type type;
};
...
public:
  ...
  RET call(CALL_FORMAL_ARGS) const {
    for(detail::select(streams_boost::tuples::get<0>(args), CALL_ACTUAL_ARGS); 
        detail::select(streams_boost::tuples::get<1>(args), CALL_ACTUAL_ARGS); 
        detail::select(streams_boost::tuples::get<2>(args), CALL_ACTUAL_ARGS))
      
      detail::select(streams_boost::tuples::get<3>(args), CALL_ACTUAL_ARGS);
  }
};
...
public:
  ...
  template<class RET, CALL_TEMPLATE_ARGS>
  RET call(CALL_FORMAL_ARGS) const {
    while(detail::select(streams_boost::tuples::get<0>(args), CALL_ACTUAL_ARGS))
...
public:
  ...
  template<class RET, CALL_TEMPLATE_ARGS>
  RET call(CALL_FORMAL_ARGS) const {
          do {} while (detail::select(streams_boost::tuples::get<0>(args), CALL_ACTUAL_ARGS) );
  }
};
...
public:
  ...
  template<class RET, CALL_TEMPLATE_ARGS>
  RET call(CALL_FORMAL_ARGS) const {
    if (detail::select(streams_boost::tuples::get<0>(args), CALL_ACTUAL_ARGS)) detail::select(streams_boost::tuples::get<1>(args), CALL_ACTUAL_ARGS); 
  }
};
...
public:
  ...
  template<class RET, CALL_TEMPLATE_ARGS>
  RET call(CALL_FORMAL_ARGS) const {
    if (detail::select(streams_boost::tuples::get<0>(args), CALL_ACTUAL_ARGS)) 
      detail::select(streams_boost::tuples::get<1>(args), CALL_ACTUAL_ARGS); 
    else 
      detail::select(streams_boost::tuples::get<2>(args), CALL_ACTUAL_ARGS);
  }
...
#endif // STREAMS_BOOST_LAMBDA_CONTROL_CONSTRUCTS_HPP
