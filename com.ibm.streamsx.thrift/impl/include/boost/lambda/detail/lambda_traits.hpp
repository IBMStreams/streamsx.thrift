...
#ifndef STREAMS_BOOST_LAMBDA_LAMBDA_TRAITS_HPP
#define STREAMS_BOOST_LAMBDA_LAMBDA_TRAITS_HPP
...
struct IF_value_
{
  STREAMS_BOOST_STATIC_CONSTANT(int, value = T::value);
};
...
struct IF_value
{
  STREAMS_BOOST_STATIC_CONSTANT(int, value = (IF_value_<typename IF<C, T, E>::RET>::value));
};
...
template<class T> class remove_reference_if_valid
{
  ...
  typedef typename streams_boost::remove_reference<T>::type plainT;
public:
  typedef typename IF<
    streams_boost::is_function<plainT>::value,
    T,
    plainT
  >::RET type;
...
template<int N, class T> struct tuple_element_as_reference {   
  typedef typename
     streams_boost::tuples::access_traits<
       typename streams_boost::tuples::element<N, T>::type
     >::non_const_type type;
};
...
struct parameter_traits_<streams_boost::reference_wrapper<T>, Any >{
  typedef T& type;
};

template<class T, class Any> 
struct parameter_traits_<const streams_boost::reference_wrapper<T>, Any >{
  typedef T& type;
};

template<class T, class Any> 
struct parameter_traits_<volatile streams_boost::reference_wrapper<T>, Any >{
  typedef T& type;
};

template<class T, class Any> 
struct parameter_traits_<const volatile streams_boost::reference_wrapper<T>, Any >{
  typedef T& type;
};
...
struct const_copy_argument {
  typedef typename 
    detail::parameter_traits_<
      T,
      typename detail::IF<streams_boost::is_function<T>::value, T&, const T>::RET
    >::type type;
};
...
class bind_type_generator {
  ...
  STREAMS_BOOST_STATIC_CONSTANT(int, nof_elems = streams_boost::tuples::length<args_t>::value);
...
#endif // STREAMS_BOOST_LAMBDA_TRAITS_HPP
