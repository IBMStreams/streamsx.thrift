...
#ifndef STREAMS_BOOST_LAMBDA_ARITY_CODE_HPP
#define STREAMS_BOOST_LAMBDA_ARITY_CODE_HPP
...
template <class T> struct get_arity {
  ...
  STREAMS_BOOST_STATIC_CONSTANT(int, value = detail::get_arity_<typename streams_boost::remove_cv<typename streams_boost::remove_reference<T>::type>::type>::value);
...
struct get_arity_<lambda_functor<T> > {
  STREAMS_BOOST_STATIC_CONSTANT(int, value = get_arity<T>::value);
};

template<class Action, class Args>
struct get_arity_<lambda_functor_base<Action, Args> > {
  STREAMS_BOOST_STATIC_CONSTANT(int, value = get_tuple_arity<Args>::value);
};

template<int I>
struct get_arity_<placeholder<I> > {
  STREAMS_BOOST_STATIC_CONSTANT(int, value = I);
};
...
template<class T>
struct get_tuple_arity {
  STREAMS_BOOST_STATIC_CONSTANT(int, value = get_arity<typename T::head_type>::value | get_tuple_arity<typename T::tail_type>::value);
};
...
struct has_placeholder {
  STREAMS_BOOST_STATIC_CONSTANT(bool, value = (get_arity<T>::value & I) != 0);
}; 

template<int I, int J>
struct includes_placeholder {
  STREAMS_BOOST_STATIC_CONSTANT(bool, value = (J & I) != 0);
};
