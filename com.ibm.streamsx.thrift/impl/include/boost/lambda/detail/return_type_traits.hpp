...
#ifndef STREAMS_BOOST_LAMBDA_RETURN_TYPE_TRAITS_HPP
#define STREAMS_BOOST_LAMBDA_RETURN_TYPE_TRAITS_HPP
...
using ::streams_boost::type_traits::ice_and;
using ::streams_boost::type_traits::ice_or;
using ::streams_boost::type_traits::ice_not;
...
namespace detail {
  ...
  class protect_conversion {
      typedef typename streams_boost::remove_reference<T>::type non_ref_T;
    public:

  // add const to rvalues, so that all rvalues are stored as const in 
  // the args tuple
    typedef typename detail::IF_type<
//      streams_boost::is_reference<T>::value && !streams_boost::is_const<non_ref_T>::value,
      ice_and<streams_boost::is_reference<T>::value,
              ice_not<streams_boost::is_const<non_ref_T>::value>::value>::value,
...
// experimental feature
  ...
  typedef typename streams_boost::remove_reference<A>::type non_ref_A;
  typedef typename streams_boost::remove_reference<B>::type non_ref_B;
...
STREAMS_BOOST_MPL_HAS_XXX_TRAIT_DEF(result_type)
