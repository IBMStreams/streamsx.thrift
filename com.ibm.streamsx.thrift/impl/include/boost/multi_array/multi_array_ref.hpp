...
#ifndef STREAMS_BOOST_MULTI_ARRAY_REF_RG071801_HPP
#define STREAMS_BOOST_MULTI_ARRAY_REF_RG071801_HPP

//
// multi_array_ref.hpp - code for creating "views" of array data.
//

#include "streams_boost/multi_array/base.hpp"
#include "streams_boost/multi_array/collection_concept.hpp"
#include "streams_boost/multi_array/concept_checks.hpp"
#include "streams_boost/multi_array/iterator.hpp"
#include "streams_boost/multi_array/storage_order.hpp"
#include "streams_boost/multi_array/subarray.hpp"
#include "streams_boost/multi_array/view.hpp"
#include "streams_boost/multi_array/algorithm.hpp"
#include "streams_boost/type_traits/is_integral.hpp"
#include "streams_boost/array.hpp"
#include "streams_boost/concept_check.hpp"
#include "streams_boost/functional.hpp"
...
public: 
  ...
  explicit const_multi_array_ref(TPtr base, const ExtentList& extents) :
    ...
    streams_boost::function_requires<
  ...
  void assign(InputIterator begin, InputIterator end) {
    streams_boost::function_requires<InputIteratorConcept<InputIterator> >();
...
#else
  typename
  disable_if<typename streams_boost::is_integral<BaseList>::type,void >::type
#endif // STREAMS_BOOST_NO_SFINAE
  reindex(const BaseList& values) {
    streams_boost::function_requires<
      detail::multi_array::CollectionConcept<BaseList> >();
    streams_boost::detail::multi_array::
  ...
  void reshape(const SizeList& extents) {
    ...
    STREAMS_BOOST_ASSERT(num_elements_ ==
  ...
  const element& operator()(IndexList indices) const {
    ...
    return super_type::access_element(streams_boost::type<const element&>(),
                                      indices,origin(),
  ...
  const_reference operator[](index idx) const {
    return super_type::access(streams_boost::type<const_reference>(),
                              idx,origin(),
                              shape(),strides(),index_bases());
  }
  ...
  operator[](const detail::multi_array::
    ...
    return
      super_type::generate_array_view(streams_boost::type<return_type>(),
                                      indices,
                                      shape(),
                                      strides(),
                                      index_bases(),
...
public:
  ...
  typedef streams_boost::array<size_type,NumDims> size_list;
  typedef streams_boost::array<index,NumDims> index_list;
    ...
    base_(base), storage_(so), origin_offset_(0), directional_offset_(0)
 {
   ...
   if(extents) {
     ...
     streams_boost::array<index,NumDims> extent_list;
...
private:
  ...
  void init_from_extent_gen(const
    ...
    typedef streams_boost::array<index,NumDims> extent_list;

    // get the index_base values
    std::transform(ranges.ranges_.begin(),ranges.ranges_.end(),
              index_base_list_.begin(),
              streams_boost::mem_fun_ref(&extent_range::start));

    // calculate the extents
    extent_list extents;
    std::transform(ranges.ranges_.begin(),ranges.ranges_.end(),
              extents.begin(),
              streams_boost::mem_fun_ref(&extent_range::size));
...
public: 
  ...
  multi_array_ref& operator=(const ConstMultiArray& other) {
    ...
    STREAMS_BOOST_ASSERT(other.num_dimensions() == this->num_dimensions());
    STREAMS_BOOST_ASSERT(std::equal(other.shape(),other.shape()+this->num_dimensions(),
                            this->shape()));
    // iterator-based copy
  ...
  multi_array_ref& operator=(const multi_array_ref& other) {
    if (&other != this) {
      ...
      STREAMS_BOOST_ASSERT(other.num_dimensions() == this->num_dimensions());
      STREAMS_BOOST_ASSERT(std::equal(other.shape(),
  ...
  element& operator()(const IndexList& indices) {
    streams_boost::function_requires<
      detail::multi_array::CollectionConcept<IndexList> >();
    return super_type::access_element(streams_boost::type<element&>(),
                                      indices,origin(),
  ...
  reference operator[](index idx) {
    return super_type::access(streams_boost::type<reference>(),
                              idx,origin(),
                              this->shape(),this->strides(),
                              this->index_bases());
  }
...
#endif // STREAMS_BOOST_MULTI_ARRAY_REF_RG071801_HPP
