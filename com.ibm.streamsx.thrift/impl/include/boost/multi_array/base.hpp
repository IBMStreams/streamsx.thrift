...
//           Jeremy Siek
//           Andrew Lumsdaine
//  See http://www.streams_boost.org/libs/multi_array for documentation.
...
#include "streams_boost/multi_array/extent_range.hpp"
#include "streams_boost/multi_array/extent_gen.hpp"
#include "streams_boost/multi_array/index_range.hpp"
#include "streams_boost/multi_array/index_gen.hpp"
#include "streams_boost/multi_array/storage_order.hpp"
#include "streams_boost/multi_array/types.hpp"
#include "streams_boost/config.hpp"
#include "streams_boost/multi_array/concept_checks.hpp" //for ignore_unused_...
...
namespace multi_array_types {
  typedef streams_boost::detail::multi_array::size_type size_type;
  typedef std::ptrdiff_t difference_type;
  typedef streams_boost::detail::multi_array::index index;
...
// streams_boost::extents and streams_boost::indices are now a part of the public
...
// can be left undefined by defining STREAMS_BOOST_MULTI_ARRAY_NO_GENERATORS 
// before loading multi_array.hpp.
#if !STREAMS_BOOST_MULTI_ARRAY_NO_GENERATORS
namespace {
  multi_array_types::extent_gen extents;
  multi_array_types::index_gen indices;
}
#endif // STREAMS_BOOST_MULTI_ARRAY_NO_GENERATORS
...
public:
  ...
  //
  typedef T element;
  typedef streams_boost::multi_array<T,NumDims-1> value_type;
...
protected:
  ...
  Reference access(streams_boost::type<Reference>,index idx,TPtr base,
                   const size_type* extents,
                   const index* strides,
                   const index* index_bases) const {

    STREAMS_BOOST_ASSERT(idx - index_bases[0] >= 0);
    STREAMS_BOOST_ASSERT(size_type(idx - index_bases[0]) < extents[0]);
...
struct value_accessor_generator {
    STREAMS_BOOST_STATIC_CONSTANT(std::size_t, dimensionality = NumDims::value);
...
public:
  ...
  struct subarray {
    typedef streams_boost::detail::multi_array::sub_array<T,NDims> type;
  };

  template <std::size_t NDims>
  struct const_subarray {
    typedef streams_boost::detail::multi_array::const_sub_array<T,NDims> type;
  };

  template <std::size_t NDims>
  struct array_view {
    typedef streams_boost::detail::multi_array::multi_array_view<T,NDims> type;
  };

  template <std::size_t NDims>
  struct const_array_view {
  public:
    typedef streams_boost::detail::multi_array::const_multi_array_view<T,NDims> type;
  };
  ...
  STREAMS_BOOST_STATIC_CONSTANT(std::size_t, dimensionality = NumDims);
protected:
  ...
  Reference access_element(streams_boost::type<Reference>,
    ...
    ignore_unused_variable_warning(index_bases);
    ignore_unused_variable_warning(extents);
#if !defined(NDEBUG) && !defined(STREAMS_BOOST_DISABLE_ASSERTS)
    for (size_type i = 0; i != NumDims; ++i) {
      STREAMS_BOOST_ASSERT(indices[i] - index_bases[i] >= 0);
      STREAMS_BOOST_ASSERT(size_type(indices[i] - index_bases[i]) < extents[i]);
    }
#endif
  ...
  // for.  For reasons unbeknownst to me, a STREAMS_BOOST_STATIC_ASSERT
  ...
  ArrayRef
  generate_array_view(streams_boost::type<ArrayRef>,
                      const streams_boost::detail::multi_array::
    ...
    streams_boost::array<index,NDims> new_strides;
    streams_boost::array<index,NDims> new_extents;
    ...
    for (size_type n = 0; n != NumDims; ++n) {
      ...
      STREAMS_BOOST_ASSERT(index_bases[n] <= start &&
                   start <= index_bases[n]+index(extents[n]));
      STREAMS_BOOST_ASSERT(index_bases[n] <= finish &&
                   finish <= index_bases[n]+index(extents[n]));
      STREAMS_BOOST_ASSERT(index_factor != 0);
    ...
    STREAMS_BOOST_ASSERT(dim == NDims);
