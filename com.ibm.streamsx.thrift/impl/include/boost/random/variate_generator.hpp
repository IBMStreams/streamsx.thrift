/* streams_boost random/variate_generator.hpp header file
 *
...
#ifndef STREAMS_BOOST_RANDOM_RANDOM_GENERATOR_HPP
#define STREAMS_BOOST_RANDOM_RANDOM_GENERATOR_HPP
...
public:
  ...
  result_type min STREAMS_BOOST_PREVENT_MACRO_SUBSTITUTION () const { return (distribution().min)(); }
  result_type max STREAMS_BOOST_PREVENT_MACRO_SUBSTITUTION () const { return (distribution().max)(); }

private:
#if STREAMS_BOOST_WORKAROUND(__BORLANDC__, <= 0x564)
  typedef typename random::detail::engine_helper<
    streams_boost::is_integral<typename decorated_engine::result_type>::value,
    streams_boost::is_integral<typename Distribution::input_type>::value
    >::STREAMS_BOOST_NESTED_TEMPLATE impl<decorated_engine, typename Distribution::input_type>::type internal_engine_type;
#else
  enum {
  ...
  typedef typename random::detail::engine_helper<have_int, want_int>::STREAMS_BOOST_NESTED_TEMPLATE impl<decorated_engine, typename Distribution::input_type>::type internal_engine_type;
#endif
...
#endif // STREAMS_BOOST_RANDOM_RANDOM_GENERATOR_HPP
