/* streams_boost random/linear_congruential.hpp header file
 *
...
#ifndef STREAMS_BOOST_RANDOM_LINEAR_CONGRUENTIAL_HPP
#define STREAMS_BOOST_RANDOM_LINEAR_CONGRUENTIAL_HPP
...
public:
...
#endif
  STREAMS_BOOST_STATIC_CONSTANT(IntType, multiplier = a);
  STREAMS_BOOST_STATIC_CONSTANT(IntType, increment = c);
  ...
  // STREAMS_BOOST_STATIC_ASSERT(m == 0 || a < m);
  // STREAMS_BOOST_STATIC_ASSERT(m == 0 || c < m);
  ...
  result_type min STREAMS_BOOST_PREVENT_MACRO_SUBSTITUTION () const { return c == 0 ? 1 : 0; }
  result_type max STREAMS_BOOST_PREVENT_MACRO_SUBSTITUTION () const { return modulus-1; }
...
#if !defined(STREAMS_BOOST_RANDOM_NO_STREAM_OPERATORS) && !STREAMS_BOOST_WORKAROUND(__BORLANDC__, STREAMS_BOOST_TESTED_AT(0x551))
...
#if !defined(__SGI_STL_PORT) && STREAMS_BOOST_WORKAROUND(__GNUC__, == 2)
template<class IntType, IntType a, IntType c, IntType m, IntType val>
std::ostream&
...
#elif defined(STREAMS_BOOST_RANDOM_NO_STREAM_OPERATORS) || STREAMS_BOOST_WORKAROUND(__BORLANDC__, STREAMS_BOOST_TESTED_AT(0x551))
...
#if !defined(STREAMS_BOOST_NO_INT64_T) && !defined(STREAMS_BOOST_NO_INTEGRAL_INT64_T)
...
#endif
  int32_t min STREAMS_BOOST_PREVENT_MACRO_SUBSTITUTION () const { return 0; }
  int32_t max STREAMS_BOOST_PREVENT_MACRO_SUBSTITUTION () const { return std::numeric_limits<int32_t>::max STREAMS_BOOST_PREVENT_MACRO_SUBSTITUTION (); }
...
#endif /* !STREAMS_BOOST_NO_INT64_T && !STREAMS_BOOST_NO_INTEGRAL_INT64_T */

} // namespace streams_boost

#include <streams_boost/random/detail/enable_warnings.hpp>

#endif // STREAMS_BOOST_RANDOM_LINEAR_CONGRUENTIAL_HPP
