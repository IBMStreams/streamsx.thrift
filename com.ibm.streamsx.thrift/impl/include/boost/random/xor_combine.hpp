/* streams_boost random/xor_combine.hpp header file
 *
...
#ifndef STREAMS_BOOST_RANDOM_XOR_COMBINE_HPP
#define STREAMS_BOOST_RANDOM_XOR_COMBINE_HPP
...
#include <streams_boost/cstdint.hpp>     // uint32_t
...
namespace random {
  ...
  #define STREAMS_BOOST_RANDOM_VAL_TYPE typename URNG1::result_type 
#else
  #define STREAMS_BOOST_RANDOM_VAL_TYPE uint32_t
#endif

template<class URNG1, int s1, class URNG2, int s2, STREAMS_BOOST_RANDOM_VAL_TYPE val = 0>
class xor_combine
{
public:
  ...
  result_type operator()()
  {
    ...
    STREAMS_BOOST_STATIC_ASSERT(std::numeric_limits<typename base1_type::result_type>::is_integer);
    STREAMS_BOOST_STATIC_ASSERT(std::numeric_limits<typename base2_type::result_type>::is_integer);
    STREAMS_BOOST_STATIC_ASSERT(std::numeric_limits<typename base1_type::result_type>::digits >= std::numeric_limits<typename base2_type::result_type>::digits);
#endif
    return (_rng1() << s1) ^ (_rng2() << s2);
  }

  result_type min STREAMS_BOOST_PREVENT_MACRO_SUBSTITUTION () const { return std::min STREAMS_BOOST_PREVENT_MACRO_SUBSTITUTION((_rng1.min)(), (_rng2.min)()); }
  result_type max STREAMS_BOOST_PREVENT_MACRO_SUBSTITUTION () const { return std::max STREAMS_BOOST_PREVENT_MACRO_SUBSTITUTION((_rng1.min)(), (_rng2.max)()); }
...
template<class URNG1, int s1, class URNG2, int s2, STREAMS_BOOST_RANDOM_VAL_TYPE val>
...
#undef STREAMS_BOOST_RANDOM_VAL_TYPE

} // namespace random
} // namespace streams_boost

#endif // STREAMS_BOOST_RANDOM_XOR_COMBINE_HPP
