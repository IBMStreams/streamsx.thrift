...
#ifndef STREAMS_BOOST_FT_COMPONENTS_HPP_INCLUDED
#define STREAMS_BOOST_FT_COMPONENTS_HPP_INCLUDED
...
#include <streams_boost/mpl/vector/vector0.hpp>

#if STREAMS_BOOST_WORKAROUND(__BORLANDC__, STREAMS_BOOST_TESTED_AT(0x656))
#   include <streams_boost/type_traits/remove_cv.hpp>

#   include <streams_boost/mpl/identity.hpp>
#   include <streams_boost/mpl/bitand.hpp>
...
#   include <streams_boost/function_types/detail/classifier.hpp>
#endif
...
#   elif STREAMS_BOOST_FT_MAX_ARITY < 50
...
#include <streams_boost/function_types/detail/class_transform.hpp>
...
{ 
  namespace function_types 
  {
...
#if STREAMS_BOOST_WORKAROUND(__BORLANDC__, STREAMS_BOOST_TESTED_AT(0x565))
    ...
    { 
      ...
      STREAMS_BOOST_MPL_AUX_LAMBDA_SUPPORT(2,components,(T,ClassTypeTransform))
    };
...
#if !STREAMS_BOOST_FT_NO_CV_FUNC_SUPPORT
...
#if !STREAMS_BOOST_WORKAROUND(__BORLANDC__, STREAMS_BOOST_TESTED_AT(0x565))
#   define STREAMS_BOOST_FT_variations STREAMS_BOOST_FT_pointer|STREAMS_BOOST_FT_member_pointer
...
#else  
#   define STREAMS_BOOST_FT_variations STREAMS_BOOST_FT_pointer
    ...
    struct components_impl<T C::*, L>
      : detail::retagged_if
        < detail::components_impl<typename streams_boost::remove_cv<T>::type *, L>
    ...
    { typedef char (& type)[1]; STREAMS_BOOST_STATIC_CONSTANT(std::size_t, value = 1); };
    template<typename T> struct encode_cv<T const *>
    { typedef char (& type)[2]; STREAMS_BOOST_STATIC_CONSTANT(std::size_t, value = 2); };
    template<typename T> struct encode_cv<T volatile *>
    { typedef char (& type)[3]; STREAMS_BOOST_STATIC_CONSTANT(std::size_t, value = 3); };
    template<typename T> struct encode_cv<T const volatile *> 
    { typedef char (& type)[4]; STREAMS_BOOST_STATIC_CONSTANT(std::size_t, value = 4); };
    ...
    template<typename T> struct cv_tag_mfp
      : detail::cv_tag_mfp_impl
        < ::streams_boost::function_types::detail::encode_mfp_cv<T>::value >
    { };
    ...
    template<typename T, typename D, typename L> struct bcc_class_transform 
      : bcc_class_transform_impl
        < typename decode_cv
          < T
          , ::streams_boost::function_types::detail::encode_mfp_cv<D>::value 
          >::type
        , L
        > 
    { };
...
#define STREAMS_BOOST_FT_al_path streams_boost/function_types/detail/components_impl
#include <streams_boost/function_types/detail/pp_loop.hpp>

  } } // namespace function_types::detail

  STREAMS_BOOST_TT_AUX_TEMPLATE_ARITY_SPEC(2,function_types::components)

} // namespace ::streams_boost

#include <streams_boost/function_types/detail/components_as_mpl_sequence.hpp>
