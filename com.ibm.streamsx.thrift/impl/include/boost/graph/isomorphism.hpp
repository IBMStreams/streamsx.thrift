...
#ifndef STREAMS_BOOST_GRAPH_ISOMORPHISM_HPP
#define STREAMS_BOOST_GRAPH_ISOMORPHISM_HPP
...
#include <streams_boost/graph/depth_first_search.hpp>
#include <streams_boost/utility.hpp>
#include <streams_boost/detail/algorithm.hpp>
#include <streams_boost/pending/indirect_cmp.hpp> // for make_indirect_pmap

#ifndef STREAMS_BOOST_GRAPH_ITERATION_MACROS_HPP
#define STREAMS_BOOST_ISO_INCLUDED_ITER_MACROS // local macro, see bottom of file
...
namespace streams_boost {
          ...
          if (dfs_num[i] > dfs_num_k) {
            ...
            BGL_FORALL_ADJ_T(f[i], v, G2, Graph2)
              if (invariant2(v) == invariant1(j) && in_S[v] == false) {
                ...
                int next_k = max STREAMS_BOOST_PREVENT_MACRO_SUBSTITUTION(dfs_num_k, max STREAMS_BOOST_PREVENT_MACRO_SUBSTITUTION(dfs_num[i], dfs_num[j]));
                if (match(streams_boost::next(iter), next_k))
                  return true;
                in_S[v] = false;
              }
            ...
            if (container_contains(adjacent_vertices(f[i], G2), f[j])) {
              ++num_edges_on_k;
              if (match(streams_boost::next(iter), dfs_num_k))
                return true;
            }
  ...
  public:
    ...
    size_type max STREAMS_BOOST_PREVENT_MACRO_SUBSTITUTION () const { 
...

  {
    ...
    STREAMS_BOOST_STATIC_ASSERT((is_same<IsoMappingValue, vertex2_t>::value));
    
    function_requires< ReadablePropertyMapConcept<IndexMap1, vertex1_t> >();
    typedef typename property_traits<IndexMap1>::value_type IndexMap1Value;
    STREAMS_BOOST_STATIC_ASSERT((is_convertible<IndexMap1Value, size_type>::value));
    
    function_requires< ReadablePropertyMapConcept<IndexMap2, vertex2_t> >();
    typedef typename property_traits<IndexMap2>::value_type IndexMap2Value;
    STREAMS_BOOST_STATIC_ASSERT((is_convertible<IndexMap2Value, size_type>::value));
...
#ifdef STREAMS_BOOST_ISO_INCLUDED_ITER_MACROS
#undef STREAMS_BOOST_ISO_INCLUDED_ITER_MACROS
#include <streams_boost/graph/iteration_macros_undef.hpp>
#endif

#endif // STREAMS_BOOST_GRAPH_ISOMORPHISM_HPP
