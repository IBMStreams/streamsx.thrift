...
//           Douglas Gregor
//           Andrew Lumsdaine
#ifndef STREAMS_BOOST_GRAPH_GURSOY_ATUN_LAYOUT_HPP
#define STREAMS_BOOST_GRAPH_GURSOY_ATUN_LAYOUT_HPP
...
#include <streams_boost/graph/topology.hpp>
...
struct update_position_visitor {
  ...
  typedef streams_boost::on_examine_vertex event_filter;
...
struct gursoy_shortest
{
  ...
  {
    streams_boost::dijkstra_shortest_paths(g, s, weight_map(weight).
      visitor(streams_boost::make_dijkstra_visitor(std::make_pair(
       streams_boost::record_distances(node_distance, streams_boost::on_edge_relaxed()),
        update_position))));
  }
};
...
struct gursoy_shortest<dummy_property_map>
{
  ...
  run(const Graph& g, typename graph_traits<Graph>::vertex_descriptor s,
      ...
      dummy_property_map)
  {
    streams_boost::breadth_first_search(g, s,
      visitor(streams_boost::make_bfs_visitor(std::make_pair(
        streams_boost::record_distances(node_distance, streams_boost::on_tree_edge()),
        update_position))));
  }
};
   ...
   EdgeWeightMap weight)
{
  ...
  typedef streams_boost::iterator_property_map<std::vector<double>::iterator, 
                                       VertexIndexMap,
                                       double, double&>
  ...
  for (streams_boost::tie(i, iend) = vertices(graph); i != iend; ++i) {
  ...
  streams_boost::detail::update_position_visitor<
                        ...
                        EdgeWeightMap weight) 
{
  ...
  typedef streams_boost::iterator_property_map<std::vector<int>::iterator, 
...
#endif // STREAMS_BOOST_GRAPH_GURSOY_ATUN_LAYOUT_HPP
