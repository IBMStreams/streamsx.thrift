...
//=======================================================================
//
#ifndef STREAMS_BOOST_GRAPH_UTILITY_HPP
#define STREAMS_BOOST_GRAPH_UTILITY_HPP
...
namespace streams_boost {
  ...
  bool is_adj_dispatch(Graph& g, Vertex a, Vertex b, directed_tag)
  {
    ...
    tie(vi, viend) = adjacent_vertices(a, g);
#if defined(STREAMS_BOOST_MSVC) && STREAMS_BOOST_MSVC <= 1300 && defined(__SGI_STL_PORT)
  ...
  namespace graph {
    ...
    bool has_no_vertices(const Graph& g) {
      typedef typename streams_boost::graph_traits<Graph>::vertex_iterator vi;
    ...
    bool has_no_edges(const Graph& g) {
      typedef typename streams_boost::graph_traits<Graph>::edge_iterator ei;
      std::pair<ei, ei> p = edges(g);
      return (p.first == p.second);
    }

    template <typename Graph>
    bool has_no_out_edges(const typename streams_boost::graph_traits<Graph>::vertex_descriptor& v, const Graph& g) {
      typedef typename streams_boost::graph_traits<Graph>::out_edge_iterator ei;
...
#endif /* STREAMS_BOOST_GRAPH_UTILITY_HPP*/
