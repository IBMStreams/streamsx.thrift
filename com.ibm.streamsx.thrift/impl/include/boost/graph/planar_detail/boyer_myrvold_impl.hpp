...
#include <vector>
#include <list>
#include <streams_boost/utility.hpp>   //for streams_boost::next
#include <streams_boost/config.hpp>    //for std::min macros
...
#include <streams_boost/graph/planar_detail/face_handles.hpp>
#include <streams_boost/graph/planar_detail/face_iterators.hpp>
#include <streams_boost/graph/planar_detail/bucket_sort.hpp>
...
namespace streams_boost
{
  ...
  struct planar_dfs_visitor : public dfs_visitor<>
  {
    ...
    void back_edge(const Edge& e, Graph& g)
    {
      ...
      if ( t != get(parent, s) ) {
        ...
        put(low, s, 
            min STREAMS_BOOST_PREVENT_MACRO_SUBSTITUTION(s_low_df_number,
                                                 t_df_number)
            );
        
        put(least_ancestor, s, 
            min STREAMS_BOOST_PREVENT_MACRO_SUBSTITUTION(s_least_ancestor_df_number, 
                                                 t_df_number
                                                 )
            );
    ...
    void finish_vertex(const Vertex& u, Graph&)
    {
      ...
      if (u_parent != u)
        {
          put(low, u_parent, 
              min STREAMS_BOOST_PREVENT_MACRO_SUBSTITUTION(u_lowpoint, 
                                                   u_parent_lowpoint
                                                   )
              );
        }
  ...
  class boyer_myrvold_impl
  {
    ...
    typedef streams_boost::shared_ptr< face_handle_list_t > face_handle_list_ptr_t;
    typedef streams_boost::shared_ptr< vertex_list_t > vertex_list_ptr_t;
    typedef streams_boost::tuple<vertex_t, bool, bool> merge_stack_frame_t;
              ...
              for(; old_face_itr != old_face_end; ++old_face_itr)
                {
                  ...
                  if (w == graph_traits<Graph>::null_vertex() && 
                      ...
                      outer_face_edge[e] &&
                      outer_face_edge[*streams_boost::next(old_face_itr)] &&
                      !seen_x_or_y
                      )
                    {
                      w = current_vertex;
