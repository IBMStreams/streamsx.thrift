...
#ifndef STREAMS_BOOST_GRAPH_CLIQUE_HPP
#define STREAMS_BOOST_GRAPH_CLIQUE_HPP
...
namespace streams_boost {
    namespace concepts {
        STREAMS_BOOST_concept(CliqueVisitor,(Visitor)(Clique)(Graph))
        {
            STREAMS_BOOST_CONCEPT_USAGE(CliqueVisitor)
            {
                vis.clique(k, g);
            }
        private:
...
struct max_clique_visitor
{
    ...
    inline void clique(const Clique& p, const Graph& g)
    {
        ...
        maximum = max STREAMS_BOOST_PREVENT_MACRO_SUBSTITUTION (maximum, p.size());
    }
    std::size_t& maximum;
};
