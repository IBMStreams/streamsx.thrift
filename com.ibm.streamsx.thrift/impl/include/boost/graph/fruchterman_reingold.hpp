...
//           Andrew Lumsdaine
#ifndef STREAMS_BOOST_GRAPH_FRUCHTERMAN_REINGOLD_FORCE_DIRECTED_LAYOUT_HPP
#define STREAMS_BOOST_GRAPH_FRUCHTERMAN_REINGOLD_FORCE_DIRECTED_LAYOUT_HPP
...
#include <streams_boost/graph/topology.hpp> // For topology concepts
#include <vector>
#include <list>
...
struct grid_force_pairs
{
  ...
  STREAMS_BOOST_STATIC_ASSERT (Point::dimensions == 2);
  ...
  DisplacementMap displacement)
{
  ...
  do {
    ...
    for (tie(e, e_end) = edges(g); e != e_end; ++e) {
      ...
      // When the vertices land on top of each other, move the
      // first vertex away from the boundaries.
      ::streams_boost::detail::maybe_jitter_point(topology, position, u, get(position, v));
    ...
    if (double temp = cool()) {
      ...
      BGL_FORALL_VERTICES_T (v, g, Graph) {
        ...
        put(position, v, topology.adjust(get(position, v), get(displacement, v) * (min STREAMS_BOOST_PREVENT_MACRO_SUBSTITUTION (disp_size, temp) / disp_size)));
...
#  include <streams_boost/graph/distributed/fruchterman_reingold.hpp>
#endif

#endif // STREAMS_BOOST_GRAPH_FRUCHTERMAN_REINGOLD_FORCE_DIRECTED_LAYOUT_HPP
