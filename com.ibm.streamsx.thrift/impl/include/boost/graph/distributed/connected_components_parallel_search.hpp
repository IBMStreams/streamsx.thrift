...
//           Douglas Gregor
//           Andrew Lumsdaine
#ifndef STREAMS_BOOST_GRAPH_PARALLEL_CC_PS_HPP
#define STREAMS_BOOST_GRAPH_PARALLEL_CC_PS_HPP
...
namespace streams_boost { namespace graph { namespace distributed {
  namespace cc_ps_detail {
    ...
    private:
      ...
      streams_boost::processor_id_type pg_id;
    };
  ...
  connected_components_ps(const Graph& g, ComponentMap c)
  {
    ...
    while (!global_done) {
      ...
      synchronize(pg);
      global_done = all_reduce(pg, (q.empty() && (current == end)), streams_boost::parallel::minimum<bool>());
