...
#ifndef STREAMS_BOOST_GRAPH_DISTRIBUTED_ADJLIST_SERIALIZATION_070925_HPP
#define STREAMS_BOOST_GRAPH_DISTRIBUTED_ADJLIST_SERIALIZATION_070925_HPP
...
namespace detail { namespace parallel
{
      ...
      available_process_files(std::string const& filename)
      {
          ...
          for (filesystem::directory_iterator i(filename), end; i != end; ++i)
          {
              if (!filesystem::is_regular(*i))
                  streams_boost::throw_exception(std::runtime_error("directory contains non-regular entries"));
              ...
              for (std::string::size_type i = 0; i < process_name.size(); ++i)
                if (!is_digit(process_name[i]))
                  streams_boost::throw_exception(std::runtime_error("directory contains files with invalid names"));

              result.push_back(streams_boost::lexical_cast<int>(process_name));
          }
  ...
  void graph_loader<Graph, Archive, VertexListS>::load_prefix()
  {
      ...
      if (num_processes_ != num_processes(m_pg))
          streams_boost::throw_exception(std::runtime_error("number of processes mismatch"));
...
# endif
  ...
  {
      int V;
      m_ar >> STREAMS_BOOST_SERIALIZATION_NVP(V); 
...
# endif
  ...
  {
      int E;
      m_ar >> STREAMS_BOOST_SERIALIZATION_NVP(E);
...
# endif
  ...
  {
      std::size_t I;
      m_ar >> STREAMS_BOOST_SERIALIZATION_NVP(I);
...
# endif
      ...
      edge_descriptor const& edge, void* old_property_ptr, bidirectionalS)
  {
      if (owner(target(edge, m_g)) == m_g.processor())
      {
          ...
          streams_boost::graph_detail::push(get(
...
# endif
      ...
      streams_boost::parallel::inplace_all_to_all(m_pg, m_property_ptrs);
  }
  ...
  void graph_loader<Graph, Archive, VertexListS>::resolve_remote_vertices(directedS)
  {
      ...
      streams_boost::parallel::inplace_all_to_all(
          m_pg, m_requested_vertices, m_remote_vertices);

      for (int i = 0; i < num_processes(m_pg); ++i)
      {
          STREAMS_BOOST_FOREACH(serializable_vertex_descriptor& u, m_remote_vertices[i])
          {
              u = m_local_vertices[u];
          }
      }

      streams_boost::parallel::inplace_all_to_all(m_pg, m_remote_vertices);
  }
...
# endif
  ...
  void graph_loader<Graph, Archive, VertexListS>::commit_pending_edges(Anything)
  {
      ...
      STREAMS_BOOST_FOREACH(pending_edge_type const& e, m_pending_edges)
      {
  ...
  void graph_loader<Graph, Archive, VertexListS>::commit_pending_in_edges(bidirectionalS)
  {
      ...
      STREAMS_BOOST_FOREACH(pending_in_edge const& e, m_pending_in_edges)
      {
          ...
          streams_boost::graph_detail::push(
...
void save_in_edges(Archive& ar, Graph const& g, bidirectionalS)
{
    ...
    {
        STREAMS_BOOST_FOREACH(edge_descriptor const& e, in_edges(v, g))
        {
    ...
    ar << STREAMS_BOOST_SERIALIZATION_NVP(I);

    STREAMS_BOOST_FOREACH(edge_descriptor const& e, saved_in_edges)
    {
...
void save_edges(Archive& ar, Graph const& g, DirectedS)
{
    ...
    typedef typename streams_boost::detail::retag_property_list<
              edge_bundle_t,
              typename Graph::edge_property_type>::type
      edge_property_type;

    int E = num_edges(g);
    ar << STREAMS_BOOST_SERIALIZATION_NVP(E);
...
void PBGL_DISTRIB_ADJLIST_TYPE::load(std::string const& filename)
{
    ...
    if (available_files.empty())
        streams_boost::throw_exception(std::runtime_error("no file available"));
...
void PBGL_DISTRIB_ADJLIST_TYPE::save(std::string const& filename) const
{
    ...
    if (filesystem::exists(filename) && !filesystem::is_directory(filename))
        streams_boost::throw_exception(std::runtime_error("entry exists, but is not a directory"));
    ...
    ar << STREAMS_BOOST_SERIALIZATION_NVP(id);
    ar << make_nvp("mapping", this->distribution().mapping());

    int V = num_vertices(*this);
    ar << STREAMS_BOOST_SERIALIZATION_NVP(V);
...
#endif // STREAMS_BOOST_GRAPH_DISTRIBUTED_ADJLIST_SERIALIZATION_070925_HPP
