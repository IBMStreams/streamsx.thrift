...
//           Douglas Gregor
//           Andrew Lumsdaine
#ifndef STREAMS_BOOST_GRAPH_DISTRIBUTED_SCC_HPP
#define STREAMS_BOOST_GRAPH_DISTRIBUTED_SCC_HPP
...
#include <streams_boost/property_map/parallel/caching_property_map.hpp>
#include <streams_boost/graph/parallel/algorithm.hpp>
#include <streams_boost/graph/parallel/process_group.hpp>
#include <streams_boost/graph/distributed/queue.hpp>
#include <streams_boost/graph/distributed/filtered_graph.hpp>
#include <streams_boost/pending/indirect_cmp.hpp>
#include <streams_boost/graph/breadth_first_search.hpp>
...
#include <streams_boost/graph/distributed/local_subgraph.hpp>
#include <streams_boost/graph/parallel/properties.hpp>
#include <streams_boost/graph/named_function_params.hpp>
#include <streams_boost/graph/random.hpp>
#include <streams_boost/graph/distributed/reverse_graph.hpp>
#include <streams_boost/optional.hpp>
#include <streams_boost/graph/distributed/detail/filtered_queue.hpp>
#include <streams_boost/graph/distributed/adjacency_list.hpp>
#ifdef PBGL_SCC_DEBUG
  #include <iostream>
  #include <cstdlib>
  #include <iomanip>
  #include <sys/time.h>
  #include <streams_boost/graph/distributed/graphviz.hpp> // for ostringstream
#endif
#include <vector>
#include <map>
...
#ifdef PBGL_SCC_DEBUG
#  include <streams_boost/graph/accounting.hpp>
...
struct vertex_identity_property_map
  : public streams_boost::put_get_helper<T, vertex_identity_property_map<T> >
{
  typedef T key_type;
...
namespace streams_boost { namespace graph { namespace distributed {
  ...
  accounting::time_type start = accounting::get_time();
      ...
      // and may help in the case where there are lots of small strong components.
      {
        ...
        int num_comp = streams_boost::strong_components(ls, ls_component);
  ...
  if(id == 0) {
    ...
    start = accounting::get_time();
       ...
       for (std::size_t i = 0; i < vertex_sets.size(); ++i)
        ...
        typedef filtered_queue<queue<vertex_descriptor>, streams_boost::detail::has_not_been_seen<VertexIndexMap> >
          local_queue_type;
        typedef streams_boost::graph::distributed::distributed_queue<process_group_type, OwnerMap, local_queue_type>
          queue_t;

        typedef typename property_map<ReverseGraph, vertex_owner_t>::const_type
          RevOwnerMap;

        typedef filtered_queue<queue<rev_vertex_descriptor>, streams_boost::detail::has_not_been_seen<VertexIndexMap> >
          rev_local_queue_type;
        typedef streams_boost::graph::distributed::distributed_queue<process_group_type, RevOwnerMap, rev_local_queue_type>
          rev_queue_t;

        queue_t Q(process_group(g),
                  owner,
                  make_filtered_queue(queue<vertex_descriptor>(),
                                      streams_boost::detail::has_not_been_seen<VertexIndexMap>
  ...
  start = accounting::get_time();
        ...
        while (optional<std::pair<process_id_type, int> > m = probe(pg)) {
          ...
          if (m->second == fhp_succ_size_msg) {
            ...
            if (streams_boost::is_same<detail::vertex_identity_property_map<vertex_descriptor>, IsoMapRF>::value)
              for (std::size_t i = 0; i < local_start.size(); ++i)
                if (added[local_start[i]] > 0)
  ...
  start = accounting::get_time();
    ...
    build_reverse_graph( const Graph& g, ReverseGraph& gr, IsoMap& fr, IsoMap& rf )
    {
      ...
      typedef typename graph_traits<Graph>::vertex_descriptor vertex_descriptor;
      typedef typename graph_traits<Graph>::out_edge_iterator out_edge_iterator;
      typedef typename streams_boost::graph::parallel::process_group_type<Graph>::type process_group_type;
      typedef typename process_group_type::process_id_type process_id_type;
      typedef std::vector<std::pair<vertex_descriptor, vertex_descriptor> > VertexPairVec;
       ...
       incidence_graph_tag)
    {
      ...
      typename streams_boost::graph::parallel::process_group_type<Graph>::type pg = process_group(g);
  ...
  accounting::time_type start = accounting::get_time();
      ...
      typedef adjacency_list<listS,
                             distributedS<typename streams_boost::graph::parallel::process_group_type<Graph>::type, vecS>,
  ...
  fleischer_hendrickson_pinar_strong_components(g, r, gr, fr, rf, 
       ...
       VertexIndexMap vertex_index_map)
    {
      ...
      typename streams_boost::graph::parallel::process_group_type<Graph>::type pg 
        = process_group(g);

      if (num_processes(pg) == 1) {
        local_subgraph<const Graph> ls(g);
        return streams_boost::strong_components(ls, c);
      }
...
#endif // STREAMS_BOOST_GRAPH_DISTRIBUTED_SCC_HPP
