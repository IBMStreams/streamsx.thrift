...
// Copyright (C) 2007  Matthias Troyer  <troyer@streams_boost-consulting.com>
...
#include <queue>
#include <stack>
#include <streams_boost/graph/distributed/detail/tag_allocator.hpp>
#include <stdio.h>
...
namespace streams_boost { namespace graph { namespace distributed {
...
struct mpi_process_group::impl
{
   ...
   */
  streams_boost::mpi::communicator             comm;

  /**
   * The MPI communicator used to transmit out-of-band replies.
   */
  streams_boost::mpi::communicator             oob_reply_comm;
                             ...
                             mpl::true_ /*is_mpi_datatype*/) const
{
  ...
  streams_boost::mpi::packed_oarchive oa(impl_->comm, outgoing.buffer);
  oa << value;
                             ...
                             mpl::false_ /*is_mpi_datatype*/) const
{
  ...
  streams_boost::mpi::packed_oarchive out(impl_->comm, outgoing.buffer);
  out << value;
     ...
     int tag, const T& value)
{
  pg.send_impl(dest, pg.encode_tag(pg.my_block_number(), tag), value,
               streams_boost::mpi::is_mpi_datatype<T>());
}

template<typename T>
typename enable_if<streams_boost::mpi::is_mpi_datatype<T>, void>::type
send(const mpi_process_group& pg, mpi_process_group::process_id_type dest,
     int tag, const T values[], std::size_t n)
{
  pg.send_impl(dest, pg.encode_tag(pg.my_block_number(), tag),
                 streams_boost::serialization::make_array(values,n), 
                 streams_boost::mpl::true_());
}

template<typename T>
typename disable_if<streams_boost::mpi::is_mpi_datatype<T>, void>::type
mpi_process_group::
                                ...
                                mpl::true_ /*is_mpi_datatype*/) const
{
  ...
  // Unpack the data
  if (header->bytes > 0) {
    streams_boost::mpi::packed_iarchive ia(impl_->comm, incoming.buffer, 
                                ...
                                mpl::false_ /*is_mpi_datatype*/) const
{
  ...
  streams_boost::mpi::packed_iarchive in(impl_->comm, incoming.buffer, 
                                 archive::no_header, header->offset);
  in >> value;
...
template<typename T>
typename disable_if<streams_boost::mpi::is_mpi_datatype<T>, bool>::type
mpi_process_group::
...
{
  using streams_boost::mpi::get_mpi_datatype;
    ...
    int source, int tag, Type& data, mpl::false_ /*is_mpi_datatype*/) 
{
  ...
  streams_boost::mpi::status status;
  status = self.impl_->comm.probe(source, tag);

#if STREAMS_BOOST_VERSION >= 103600
  int size = status.count<streams_boost::mpi::packed>().get();
#else
  int size;
  ...
  streams_boost::mpi::packed_iarchive in(self.impl_->comm);
  in.resize(size);
...
void do_oob_receive(mpi_process_group const& self, int source, int tag, Type& data) 
{
  do_oob_receive(self, source, tag, data,
                           streams_boost::mpi::is_mpi_datatype<Type>());
}
        ...
        trigger_receive_context context, int block) const
{
  ...
  if (context == trc_out_of_band) {
    ...
    streams_boost::graph::distributed::receive(self, source, tag, data);
        ...
        trigger_receive_context context, int block) const
{
  ...
  streams_boost::parallel::detail::untracked_pair<int, Type> data;
        ...
        trigger_receive_context context, int block) const
{
  ...
  streams_boost::mpi::packed_iarchive ia(self.impl_->comm,self.impl_->buffers[tag]);
  ia >> data;
  // Start a new receive
...
prepare_receive(mpi_process_group const& self, int tag, bool force) const
{
 ...
 std::cerr << ("Posting Irecv for trigger") 
  ...
  //STREAMS_BOOST_MPL_ASSERT(mpl::not_<is_mpi_datatype<Type> >);
  if (force) {
...
receive(const mpi_process_group& pg, int tag, T& value)
{
  for (std::size_t source = 0; source < pg.impl_->incoming.size(); ++source) {
    if (pg.receive_impl(source, pg.encode_tag(pg.my_block_number(), tag),
                        value, streams_boost::mpi::is_mpi_datatype<T>()))
      return source;
  }
  assert (false);
}
...
template<typename T>
typename 
  enable_if<streams_boost::mpi::is_mpi_datatype<T>, 
...
template<typename T>
typename 
  disable_if<streams_boost::mpi::is_mpi_datatype<T>, 
             std::pair<mpi_process_group::process_id_type, std::size_t> >::type
receive(const mpi_process_group& pg, int tag, T values[], std::size_t n)
{
  for (std::size_t source = 0; source < pg.impl_->incoming.size(); ++source) {
           ...
           BinaryOperation bin_op)
{
  ...
  streams_boost::mpi::all_reduce(streams_boost::mpi::communicator(communicator(pg),
                                                  streams_boost::mpi::comm_attach), 
...
{
  // broadcast the seed  
  streams_boost::mpi::communicator comm(communicator(pg),streams_boost::mpi::comm_attach);
  streams_boost::mpi::broadcast(comm,val,root);
}
           ...
           BinaryOperation bin_op)
{
  ...
  streams_boost::mpi::scan(communicator(pg), first, last-first, out, bin_op);
...
{
/*
  streams_boost::mpi::group current_group = communicator(pg).group();
  streams_boost::mpi::group new_group = current_group.include(first,last);
  streams_boost::mpi::communicator new_comm(communicator(pg),new_group);
...
*/
  ...
  if (new_comm != MPI_COMM_NULL) {
    mpi_process_group result_pg(streams_boost::mpi::communicator(new_comm,streams_boost::mpi::comm_attach));
...
template<typename T>
typename enable_if<streams_boost::mpi::is_mpi_datatype<T> >::type
            ...
            mpi_process_group::process_id_type source, int tag, T& value, int block)
{
  ...
  std::pair<streams_boost::mpi::communicator, int> actual
...
template<typename T>
typename disable_if<streams_boost::mpi::is_mpi_datatype<T> >::type
receive_oob(const mpi_process_group& pg, 
            mpi_process_group::process_id_type source, int tag, T& value, int block)
{
  // Determine the actual message we expect to receive, and which
  ...
  streams_boost::optional<streams_boost::mpi::status> status;
  do {
  ...
  streams_boost::mpi::packed_iarchive in(actual.first);

#if STREAMS_BOOST_VERSION >= 103600
  in.resize(status->count<streams_boost::mpi::packed>().get());
#else
  int size;
  MPI_Status mpi_status = *status;
  MPI_Get_count(&mpi_status, MPI_PACKED, &size);
...
typename enable_if<streams_boost::mpi::is_mpi_datatype<ReplyT> >::type
                    ...
                    int block)
{
  ...
  send_oob(pg, dest, tag, streams_boost::parallel::detail::make_untracked_pair(
        (int)reply_tag, send_value), block);
  receive_oob(pg, dest, reply_tag, reply_value);
}

template<typename SendT, typename ReplyT>
typename disable_if<streams_boost::mpi::is_mpi_datatype<ReplyT> >::type
send_oob_with_reply(const mpi_process_group& pg, 
                    mpi_process_group::process_id_type dest,
                    int tag, const SendT& send_value, ReplyT& reply_value,
                    int block)
{
  ...
  send_oob(pg, dest, tag, 
           streams_boost::parallel::detail::make_untracked_pair((int)reply_tag, 
                                                        send_value), block);
  receive_oob(pg, dest, reply_tag, reply_value);
}

} } } // end namespace streams_boost::graph::distributed
