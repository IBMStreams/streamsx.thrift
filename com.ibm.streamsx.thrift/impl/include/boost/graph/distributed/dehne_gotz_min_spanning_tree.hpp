...
#ifndef STREAMS_BOOST_DEHNE_GOTZ_MIN_SPANNING_TREE_HPP
#define STREAMS_BOOST_DEHNE_GOTZ_MIN_SPANNING_TREE_HPP
...
#include <utility>
#include <streams_boost/pending/disjoint_sets.hpp>
#include <streams_boost/pending/indirect_cmp.hpp>
#include <streams_boost/property_map/parallel/caching_property_map.hpp>
#include <streams_boost/graph/vertex_and_edge_range.hpp>
#include <streams_boost/graph/kruskal_min_spanning_tree.hpp>
...
namespace detail {
                     ...
                     EdgeList& edge_list)
  {
    ...
    typedef typename property_traits<WeightMap>::value_type
                                                           weight_type;
    typedef streams_boost::parallel::detail::untracked_pair<edge_descriptor, 
                                       weight_type>        w_edge;
                                    ...
                                    SupervertexMap supervertex_map)
{
  ...
  STREAMS_BOOST_STATIC_ASSERT((is_convertible<traversal_category*,
  ...
  kruskal_minimum_spanning_tree
    (make_vertex_and_edge_range(g, vertices(g).first, vertices(g).second,
     ...
     streams_boost::weight_map(weight_map).
...
merge_local_minimum_spanning_trees(const Graph& g, WeightMap weight,
                                   ...
                                   GlobalIndexMap global_index)
{
  using streams_boost::graph::parallel::process_group_type;
  using streams_boost::graph::parallel::process_group;
  ...
  kruskal_minimum_spanning_tree
    (make_vertex_and_edge_range(g, vertices(g).first, vertices(g).second,
                                edges(g).first, edges(g).second),
     std::back_inserter(edge_list),
     streams_boost::weight_map(weight).vertex_index_map(global_index));
                   ...
                   ParentMap parent_map, SupervertexMap supervertex_map)
{
  ...
  kruskal_minimum_spanning_tree
    (make_vertex_and_edge_range(g, vertices(g).first, vertices(g).second,
                                edges(g).first, edges(g).second),
     std::back_inserter(edge_list),
     streams_boost::weight_map(weight).
...
#endif // STREAMS_BOOST_DEHNE_GOTZ_MIN_SPANNING_TREE_HPP
