...
#ifndef STREAMS_BOOST_GRAPH_CYCLE_RATIO_HOWARD_HPP
#define STREAMS_BOOST_GRAPH_CYCLE_RATIO_HOWARD_HPP
...
namespace streams_boost {
  ...
  namespace detail {
    ...
    public:
      ...
      typedef typename streams_boost::iterator_property_map<
    ...
    private:
        ...
        for ( tie(vi, vie) = vertices(m_g); vi != vie; ++vi )
          {
            ...
            typename graph_traits<Graph>::out_edge_iterator mei =
              std::max_element(oei, oeie,
                               streams_boost::bind(m_cmp,
                                           streams_boost::bind(&EdgeWeight1::operator[], m_ew1m, _1),
                                           streams_boost::bind(&EdgeWeight1::operator[], m_ew1m, _2)
                                          )
                               );
            if (mei == oeie)
              {
      ...
      {
        streams_boost::queue<vertex_t> Q;
        c[sv] = my_black;
        Q.push(sv);
        while (!Q.empty())
          {
      ...
      float_t policy_mcr()
      {
        ...
        while ( (uv_itr = std::find_if(uv_itr, vie,
                                       streams_boost::bind(std::equal_to<my_color_type>(),
                                                   my_white,
                                                   streams_boost::bind(&color_map_t::operator[], vcm_, _1)
                                                   )
                                       )
                 ) != vie )
                   ...
                   FloatTraits ft = FloatTraits())
{
    ...
    >::type Weight;
    typename std::vector<Weight> ed_w2(streams_boost::num_edges(g), 1);
