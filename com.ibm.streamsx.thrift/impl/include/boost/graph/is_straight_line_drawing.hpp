...
#include <streams_boost/utility.hpp> //for next and prior
...
namespace streams_boost
{
    ...
    if (x1 - x2 == 0)
      {
        ...
        double min_y = min STREAMS_BOOST_PREVENT_MACRO_SUBSTITUTION(y1,y2);
        double max_y = max STREAMS_BOOST_PREVENT_MACRO_SUBSTITUTION(y1,y2);
        double min_b = min STREAMS_BOOST_PREVENT_MACRO_SUBSTITUTION(b1,b2);
        double max_b = max STREAMS_BOOST_PREVENT_MACRO_SUBSTITUTION(b1,b2);
                                ...
                                )
  {
    ...
    typedef streams_boost::tuple<edge_t, x_coord_t, y_coord_t> edge_event_t;
        ...
        if (a_itr == active_edges.end())
        ...
        else
          {
            ...
            if (a_itr == active_edges.begin())
              ...
              before = prior(a_itr);
            after = streams_boost::next(a_itr);
