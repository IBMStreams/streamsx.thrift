...
#ifndef STREAMS_BOOST_GRAPH_COMPRESSED_SPARSE_ROW_STRUCT_HPP
#define STREAMS_BOOST_GRAPH_COMPRESSED_SPARSE_ROW_STRUCT_HPP

#ifndef STREAMS_BOOST_GRAPH_COMPRESSED_SPARSE_ROW_GRAPH_HPP
#error This file should only be included from streams_boost/graph/compressed_sparse_row_graph.hpp
#endif
...
#include <streams_boost/graph/detail/histogram_sort.hpp>
...
namespace detail {
  ...
  class compressed_sparse_row_structure :
    ...
    assign_unsorted_multi_pass_edges(MultiPassInputIterator edge_begin,
      ...
      typedef streams_boost::transform_iterator<streams_boost::graph::detail::project1st<edge_type>, MultiPassInputIterator> source_iterator;
      typedef streams_boost::transform_iterator<streams_boost::graph::detail::project2nd<edge_type>, MultiPassInputIterator> target_iterator;
      source_iterator sources_begin(edge_begin, streams_boost::graph::detail::project1st<edge_type>());
      source_iterator sources_end(edge_end, streams_boost::graph::detail::project1st<edge_type>());
      target_iterator targets_begin(edge_begin, streams_boost::graph::detail::project2nd<edge_type>());
      target_iterator targets_end(edge_end, streams_boost::graph::detail::project2nd<edge_type>());

      streams_boost::graph::detail::count_starts
        (sources_begin, sources_end, m_rowstart.begin(), numlocalverts,
         source_pred, streams_boost::make_property_map_function(global_to_local));

      m_column.resize(m_rowstart.back());

      streams_boost::graph::detail::histogram_sort
        (sources_begin, sources_end, m_rowstart.begin(), numlocalverts,
    ...
    void assign_from_sorted_edges(
      ...
      if (numedges == 0) {
        typedef typename std::iterator_traits<InputIterator>::iterator_category
          category;
        numedges = streams_boost::graph::detail::reserve_count_for_single_pass(edge_begin, edge_end);
      }
      m_column.clear();
      m_column.reserve(numedges_or_zero);
    ...
    void assign_sources_and_targets_global(std::vector<vertex_descriptor>& sources,
      ...
      streams_boost::graph::detail::count_starts
        (sources.begin(), sources.end(), m_rowstart.begin(), numverts,
         keep_all(), streams_boost::make_property_map_function(global_to_local));
      streams_boost::graph::detail::histogram_sort_inplace
        (sources.begin(), m_rowstart.begin(), numverts,
         targets.begin(), streams_boost::make_property_map_function(global_to_local));
    ...
    void assign_sources_and_targets_global(std::vector<vertex_descriptor>& sources,
      ...
      streams_boost::graph::detail::histogram_sort_inplace
        (sources.begin(), m_rowstart.begin(), numverts,
         ...
         streams_boost::make_property_map_function(global_to_local));
      // Now targets is the correct vector (properly sorted by source) for
           ...
           vertices_size_type numverts, edges_size_type numedges)
    {
      ...
      EdgeIndex current_edge = 0;
      typedef typename streams_boost::graph_traits<Graph>::vertex_descriptor g_vertex;
      typedef typename streams_boost::graph_traits<Graph>::edge_descriptor g_edge;
      typedef typename streams_boost::graph_traits<Graph>::out_edge_iterator
        g_out_edge_iter;
    ...
    add_edges_sorted_internal(
        ...
        const GlobalToLocal& global_to_local) {
      typedef streams_boost::reverse_iterator<BidirectionalIteratorOrig> BidirectionalIterator;
      typedef streams_boost::reverse_iterator<EPIterOrig> EPIter;
      // Flip sequence
  ...
  struct transpose_iterator_gen {
    ...
    typedef streams_boost::transform_iterator<transpose, Iter> type;
  ...
  {
    typedef typename streams_boost::graph_traits<GraphT>::vertices_size_type
      vertices_size_type;
    typedef typename streams_boost::graph_traits<GraphT>::edge_descriptor edge_descriptor;
  ...
  edge_to_index_pair
    <GraphT,
     typename streams_boost::property_map<GraphT,streams_boost::vertex_index_t>::const_type>
  make_edge_to_index_pair(const GraphT& g)
  {
    typedef typename streams_boost::property_map<GraphT,
                                         streams_boost::vertex_index_t>::const_type
      VertexIndexMap;
    return edge_to_index_pair<GraphT, VertexIndexMap>(g,
                                                     get(streams_boost::vertex_index,
                                                         g));
  }

  template<typename GraphT, typename VertexIndexMap, typename Iter>
  streams_boost::transform_iterator<edge_to_index_pair<GraphT, VertexIndexMap>, Iter>
  make_edge_to_index_pair_iter(const GraphT& g, const VertexIndexMap& index,
                               Iter it) {
    return streams_boost::transform_iterator<edge_to_index_pair<GraphT, VertexIndexMap>, Iter>(it, edge_to_index_pair<GraphT, VertexIndexMap>(g, index));
  }
...
#endif // STREAMS_BOOST_GRAPH_COMPRESSED_SPARSE_ROW_STRUCT_HPP
