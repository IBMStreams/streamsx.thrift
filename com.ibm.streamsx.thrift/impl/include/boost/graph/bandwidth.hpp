...
#ifndef STREAMS_BOOST_GRAPH_BANDWIDTH_HPP
#define STREAMS_BOOST_GRAPH_BANDWIDTH_HPP
...
namespace streams_boost {
                ...
                VertexIndexMap index)
  {
    ...
    for (tie(e, end) = out_edges(i, g); e != end; ++e) {
      ...
      if(f_i > f_j)
          b = max STREAMS_BOOST_PREVENT_MACRO_SUBSTITUTION (b, size_type(f_i - f_j));
    }
    return b;
  }
  ...
  bandwidth(const Graph& g, VertexIndexMap index)
  {
    ...
    for (tie(i, end) = vertices(g); i != end; ++i)
        b = max STREAMS_BOOST_PREVENT_MACRO_SUBSTITUTION (b, ith_bandwidth(*i, g, index));
    return b;
  }
...
#endif // STREAMS_BOOST_GRAPH_BANDWIDTH_HPP
