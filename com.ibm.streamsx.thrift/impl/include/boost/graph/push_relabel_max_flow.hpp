...
#ifndef STREAMS_BOOST_PUSH_RELABEL_MAX_FLOW_HPP
#define STREAMS_BOOST_PUSH_RELABEL_MAX_FLOW_HPP
   ...
   // not blocked).
    ...
    public:
      ...
      void add_to_active_list(vertex_descriptor u, Layer& layer) {
        ...
        max_active = max STREAMS_BOOST_PREVENT_MACRO_SUBSTITUTION(distance[u], max_active);
        min_active = min STREAMS_BOOST_PREVENT_MACRO_SUBSTITUTION(distance[u], min_active);
        ...
        while (! Q.empty()) {
          ...
          for (tie(ai, a_end) = out_edges(u, g); ai != a_end; ++ai) {
            ...
            if (color[v] == ColorTraits::white()
              ...
              max_distance = max STREAMS_BOOST_PREVENT_MACRO_SUBSTITUTION(d_v, max_distance);
      ...
      void push_flow(edge_descriptor u_v)
      {
        ...
        FlowValue flow_delta
          = min STREAMS_BOOST_PREVENT_MACRO_SUBSTITUTION(excess_flow[u], residual_capacity[u_v]);
      ...
      distance_size_type relabel_distance(vertex_descriptor u)
      {
        ...
        if (min_distance < n) {
          ...
          max_distance = max STREAMS_BOOST_PREVENT_MACRO_SUBSTITUTION(min_distance, max_distance);
        }
        return min_distance;
      } // relabel_distance()
              ...
              for (; current[u].first != current[u].second; ++current[u].first) {
                ...
                if (capacity[a] == 0 && is_residual_edge(a)) {
                  ...
                  if (color[v] == ColorTraits::white()) {
                    ...
                    while (1) {
                      STREAMS_BOOST_USING_STD_MIN();
                      delta = min STREAMS_BOOST_PREVENT_MACRO_SUBSTITUTION(delta, residual_capacity[*current[v].first]);
                      if (v == u)
                        break;
                      else
      ...
      streams_boost::queue<vertex_descriptor> Q;
...
#endif // STREAMS_BOOST_PUSH_RELABEL_MAX_FLOW_HPP
