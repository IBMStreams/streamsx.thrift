...
#ifndef STREAMS_BOOST_GRAPH_CYCLE_HPP
#define STREAMS_BOOST_GRAPH_CYCLE_HPP
...
namespace streams_boost {
    namespace concepts {
        STREAMS_BOOST_concept(CycleVisitor,(Visitor)(Path)(Graph))
        {
            STREAMS_BOOST_CONCEPT_USAGE(CycleVisitor)
            {
                vis.cycle(p, g);
            }
        private:
...
struct min_max_cycle_visitor
{
    ...
    inline void cycle(const Path& p, const Graph& g)
    {
        ...
        minimum = min STREAMS_BOOST_PREVENT_MACRO_SUBSTITUTION (minimum, len);
        maximum = max STREAMS_BOOST_PREVENT_MACRO_SUBSTITUTION (maximum, len);
    }
    std::size_t& minimum;
    std::size_t& maximum;
};
