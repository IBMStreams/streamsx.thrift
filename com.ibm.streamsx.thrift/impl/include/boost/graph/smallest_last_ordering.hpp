...
#ifndef STREAMS_BOOST_SMALLEST_LAST_VERTEX_ORDERING_HPP
#define STREAMS_BOOST_SMALLEST_LAST_VERTEX_ORDERING_HPP
/*
...
#include <streams_boost/pending/bucket_sorter.hpp>
...
namespace streams_boost {
  ...
  smallest_last_vertex_ordering(const VertexListGraph& G, Order order, 
    ...
    typedef typename streams_boost::graph_traits<VertexListGraph> GraphTraits;
    ...
    typedef typename streams_boost::detail::vertex_property_map<VertexListGraph, vertex_index_t>::type ID;
  ...
  smallest_last_vertex_ordering(const VertexListGraph& G, Order order, 
    ...
    for (streams_boost::tie(v, vend) = vertices(G); v != vend; ++v) {
      put(marker, *v, num);
    ...
    while ( 1 ) {
      ...
      for (streams_boost::tie(v,vend) = adjacent_vertices(node, G); v != vend; ++v)
        ...
        if ( get(marker,*v) > current_order ) { //*v is unordered vertex
          ...
          minimum_degree = min STREAMS_BOOST_PREVENT_MACRO_SUBSTITUTION(minimum_degree, get(degree, *v)); 
