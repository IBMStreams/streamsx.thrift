...
//           Andrew Lumsdaine
#ifndef STREAMS_BOOST_GRAPH_METIS_HPP
#define STREAMS_BOOST_GRAPH_METIS_HPP

#ifdef STREAMS_BOOST_GRAPH_METIS_NO_INLINE
#  define STREAMS_BOOST_GRAPH_METIS_INLINE_KEYWORD
#else
#  define STREAMS_BOOST_GRAPH_METIS_INLINE_KEYWORD inline
#endif
...
#if !defined(STREAMS_BOOST_GRAPH_METIS_NO_INLINE) || defined(STREAMS_BOOST_GRAPH_METIS_SOURCE)
STREAMS_BOOST_GRAPH_METIS_INLINE_KEYWORD
...
void metis_reader::edge_iterator::advance(bool skip_initial_read)
{
  do {
      ...
      if (self->line_in >> std::ws >> self->edge.second) {
        ...
        if (self->has_edge_weights()) {
          if (!(self->line_in >> self->edge_weight))
            streams_boost::throw_exception(metis_input_exception());
        }
        return;
      }
    ...
    if (!self->in) streams_boost::throw_exception(metis_input_exception());
    ...
    while (weights_left > 0) {
      ...
      else streams_boost::throw_exception(metis_input_exception());
      --weights_left;
    }
...
void metis_reader::start()
{
  ...
  if (!in || line.empty()) streams_boost::throw_exception(metis_input_exception());

  // Determine number of vertices and edges in the graph
  line_in.str(line);
  if (!(line_in >> n_vertices >> n_edges)) streams_boost::throw_exception(metis_input_exception());
...
#endif // STREAMS_BOOST_GRAPH_METIS_HPP
