...
#include <streams_boost/tuple/tuple.hpp>   //for tie
#include <streams_boost/graph/biconnected_components.hpp>
#include <streams_boost/property_map/property_map.hpp>
#include <vector>
#include <iterator>
#include <algorithm>

#include <streams_boost/graph/planar_face_traversal.hpp>
#include <streams_boost/graph/planar_detail/add_edge_visitors.hpp>
...
namespace streams_boost
{
  ...
  struct triangulation_visitor : public planar_face_traversal_visitor
  {
    ...
    void end_face()
    {
      ...
      for(face_iterator fi = streams_boost::next(streams_boost::next(vertices_on_face.begin())); 
          fi != fi_end; ++fi
          )
        {
      ...
      if (marked_neighbor == vertices_on_face.end())
        {
          add_edge_range(
                         vertices_on_face[0],
                         streams_boost::next(streams_boost::next(vertices_on_face.begin())),
      ...
      else
        {
          add_edge_range(
                         vertices_on_face[1],
                         streams_boost::next(marked_neighbor),
                         vertices_on_face.end()
                         );
