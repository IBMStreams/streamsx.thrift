...
//
#ifndef STREAMS_BOOST_GRAPH_DIJKSTRA_HPP
#define STREAMS_BOOST_GRAPH_DIJKSTRA_HPP
...
#include <streams_boost/graph/relax.hpp>
#include <streams_boost/pending/indirect_cmp.hpp>
#include <streams_boost/graph/exception.hpp>
#include <streams_boost/pending/relaxed_heap.hpp>
#include <streams_boost/graph/overloading.hpp>
#include <streams_boost/smart_ptr.hpp>
#include <streams_boost/graph/detail/d_ary_heap.hpp>
...
#ifdef STREAMS_BOOST_GRAPH_DIJKSTRA_TESTING
#  include <streams_boost/pending/mutable_queue.hpp>
#endif // STREAMS_BOOST_GRAPH_DIJKSTRA_TESTING
...
namespace streams_boost {
    ...
    struct dijkstra_bfs_visitor
    {
      ...
      void examine_edge(Edge e, Graph& g) {
        if (m_compare(get(m_weight, e), m_zero))
            streams_boost::throw_exception(negative_edge());
  ...
  namespace detail {
    ...
    struct vertex_property_map_generator_helper<Graph, IndexMap, Value, true> {
      typedef streams_boost::iterator_property_map<Value*, IndexMap> type;
      static type build(const Graph& g, const IndexMap& index, streams_boost::scoped_array<Value>& array_holder) {
    ...
    struct vertex_property_map_generator_helper<Graph, IndexMap, Value, false> {
      typedef streams_boost::vector_property_map<Value, IndexMap> type;
      static type build(const Graph& g, const IndexMap& index, streams_boost::scoped_array<Value>& array_holder) {
        return streams_boost::make_vector_property_map<Value>(index);
      }
    };

    template <class Graph, class IndexMap, class Value>
    struct vertex_property_map_generator {
      typedef streams_boost::is_base_and_derived<
                streams_boost::vertex_list_graph_tag,
                typename streams_boost::graph_traits<Graph>::traversal_category>
              known_num_vertices;
  ...
  namespace detail {
    ...
    struct default_color_map_generator_helper<Graph, IndexMap, true> {
      typedef streams_boost::two_bit_color_map<IndexMap> type;
      static type build(const Graph& g, const IndexMap& index) {
        size_t nv = num_vertices(g);
        return streams_boost::two_bit_color_map<IndexMap>(nv, index);
      }
    };

    template <class Graph, class IndexMap>
    struct default_color_map_generator_helper<Graph, IndexMap, false> {
      typedef streams_boost::vector_property_map<streams_boost::two_bit_color_type, IndexMap> type;
      static type build(const Graph& g, const IndexMap& index) {
        return streams_boost::make_vector_property_map<streams_boost::two_bit_color_type>(index);
      }
    };
...
#ifdef STREAMS_BOOST_GRAPH_DIJKSTRA_USE_RELAXED_HEAP
    typedef relaxed_heap<Vertex, IndirectCmp, IndexMap> MutableQueue;
    MutableQueue Q(num_vertices(g), icmp, index_map);
#else // Now the default: use a d-ary heap
      streams_boost::scoped_array<std::size_t> index_in_heap_map_holder;
      typedef
  ...
  dijkstra_shortest_paths
    (const VertexListGraph& g,
     ...
     STREAMS_BOOST_GRAPH_ENABLE_IF_MODELS_PARM(VertexListGraph,vertex_list_graph_tag))
  {
    streams_boost::two_bit_color_map<IndexMap> color(num_vertices(g), index_map);
...
#  include <streams_boost/graph/distributed/dijkstra_shortest_paths.hpp>
#endif

#endif // STREAMS_BOOST_GRAPH_DIJKSTRA_HPP
