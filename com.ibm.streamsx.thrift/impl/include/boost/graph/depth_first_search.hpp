...
#ifndef STREAMS_BOOST_GRAPH_RECURSIVE_DFS_HPP
#define STREAMS_BOOST_GRAPH_RECURSIVE_DFS_HPP
...
// Define STREAMS_BOOST_RECURSIVE_DFS to use older, recursive version.
// It is retained for a while in order to perform performance
// comparison.
#ifndef STREAMS_BOOST_RECURSIVE_DFS
    ...
    //
    // See http://lists.streams_boost.org/MailArchives/streams_boost/msg48752.php for FAQ.
...
#else // STREAMS_BOOST_RECURSIVE_DFS is defined
  ...
  depth_first_search(const VertexListGraph& g, DFSVisitor vis, ColorMap color)
  {
    typedef typename streams_boost::graph_traits<VertexListGraph>::vertex_iterator vi;
  ...
  public:
    ...
    void start_vertex(Vertex u, const Graph& g) {
      invoke_visitors(m_vis, u, g, ::streams_boost::on_start_vertex());
    }
    template <class Vertex, class Graph>
    ...
    void examine_edge(Edge u, const Graph& g) {
      invoke_visitors(m_vis, u, g, ::streams_boost::on_examine_edge());
    }
    template <class Edge, class Graph>
    void tree_edge(Edge u, const Graph& g) {
      invoke_visitors(m_vis, u, g, ::streams_boost::on_tree_edge());
    }
    template <class Edge, class Graph>
    void back_edge(Edge u, const Graph& g) {
      invoke_visitors(m_vis, u, g, ::streams_boost::on_back_edge());
    }
    template <class Edge, class Graph>
    void forward_or_cross_edge(Edge u, const Graph& g) {
      invoke_visitors(m_vis, u, g, ::streams_boost::on_forward_or_cross_edge());
    }
    template <class Vertex, class Graph>
    ...
    STREAMS_BOOST_GRAPH_EVENT_STUB(on_initialize_vertex,dfs)
    STREAMS_BOOST_GRAPH_EVENT_STUB(on_start_vertex,dfs)
    STREAMS_BOOST_GRAPH_EVENT_STUB(on_discover_vertex,dfs)
    STREAMS_BOOST_GRAPH_EVENT_STUB(on_examine_edge,dfs)
    STREAMS_BOOST_GRAPH_EVENT_STUB(on_tree_edge,dfs)
    STREAMS_BOOST_GRAPH_EVENT_STUB(on_back_edge,dfs)
    STREAMS_BOOST_GRAPH_EVENT_STUB(on_forward_or_cross_edge,dfs)
    STREAMS_BOOST_GRAPH_EVENT_STUB(on_finish_vertex,dfs)
                     ...
                     const bgl_named_params<P, T, R>& params)
  {
    ...
    if (verts.first == verts.second)
      return;
    using namespace streams_boost::graph::keywords;
    typedef bgl_named_params<P, T, R> params_type;
    STREAMS_BOOST_GRAPH_DECLARE_CONVERTED_PARAMETERS(params_type, params)
    depth_first_search
      (g,
       arg_pack[_visitor | make_dfs_visitor(null_visitor())],
       streams_boost::detail::color_map_maker<VertexListGraph, arg_pack_type>::make_map(g, arg_pack),
...
#  include <streams_boost/graph/distributed/depth_first_search.hpp>
#endif
