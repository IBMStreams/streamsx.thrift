...
struct tracking_level<streams_boost::adjacency_list<OEL,VL,D,VP,EP,GP,EL> > {
...
inline void save(
    Archive & ar,
    const streams_boost::adjacency_list<OEL,VL,D,VP,EP,GP,EL> &graph,
...
inline void load(
    Archive & ar,
    streams_boost::adjacency_list<OEL,VL,D,VP,EP,GP,EL> &graph,
    const unsigned int /* file_version */
){
  typedef adjacency_list<OEL,VL,D,VP,EP,GP,EL> Graph;
  typedef typename graph_traits<Graph>::vertex_descriptor Vertex;
  ...
  while(E-- > 0){
    int u; int v;
    ar >> STREAMS_BOOST_SERIALIZATION_NVP(u);
    ar >> STREAMS_BOOST_SERIALIZATION_NVP(v);
...
inline void serialize(
    ...
    streams_boost::serialization::split_free(ar, graph, file_version);
}
