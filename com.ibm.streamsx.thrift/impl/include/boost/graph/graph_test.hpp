...
#ifndef STREAMS_BOOST_GRAPH_TEST_HPP
#define STREAMS_BOOST_GRAPH_TEST_HPP

#include <vector>
#include <streams_boost/test/minimal.hpp>
#include <streams_boost/graph/filtered_graph.hpp>
#include <streams_boost/graph/iteration_macros.hpp>
#include <streams_boost/graph/isomorphism.hpp>
#include <streams_boost/graph/copy.hpp>
#include <streams_boost/graph/graph_utility.hpp> // for connects
...
namespace streams_boost {
  ...
  struct graph_test
  {
        ...
        const Graph& g)
    {
      ...
      for (vertex_iter ui = vertex_set.begin(); ui != vertex_set.end(); ++ui) {
        ...
        STREAMS_BOOST_CHECK(out_degree(u, g) == adj.size());
        STREAMS_BOOST_CHECK(deg_size_t(std::distance(p.first, p.second))
                   == out_degree(u, g));
        for (; p.first != p.second; ++p.first) {
          edge_t e = *p.first;
          STREAMS_BOOST_CHECK(source(e, g) == u);
          STREAMS_BOOST_CHECK(container_contains(adj, target(e, g)) == true);
        }
      }
    }
       ...
       const Graph& g)
    {
      ...
      for (vertex_iter vi = vertex_set.begin(); vi != vertex_set.end(); ++vi) {
        ...
        STREAMS_BOOST_CHECK(in_degree(v, g) == inv_adj.size());
        STREAMS_BOOST_CHECK(deg_size_t(std::distance(p.first, p.second))
                   == in_degree(v, g));
        for (; p.first != p.second; ++p.first) {
          edge_t e = *p.first;
          STREAMS_BOOST_CHECK(target(e, g) == v);
          STREAMS_BOOST_CHECK(container_contains(inv_adj, source(e, g)) == true);
        }
      }
    }
       ...
       const Graph& g)
    {
      ...
      for (vertex_iter ui = vertex_set.begin(); ui != vertex_set.end(); ++ui) {
        ...
        STREAMS_BOOST_CHECK(deg_size_t(std::distance(p.first, p.second)) == adj.size());
        for (; p.first != p.second; ++p.first) {
          vertex_t v = *p.first;
          STREAMS_BOOST_CHECK(container_contains(adj, v) == true);
        }
      }
    }      
      ...
      (const std::vector<vertex_t>& vertex_set, const Graph& g)
    {
      ...
      STREAMS_BOOST_CHECK(num_vertices(g) == vertex_set.size());
      v_size_t n = std::distance(p.first, p.second);
      STREAMS_BOOST_CHECK(n == num_vertices(g));
      for (; p.first != p.second; ++p.first) {
        vertex_t v = *p.first;
        STREAMS_BOOST_CHECK(container_contains(vertex_set, v) == true);
      }
    }
       ...
       const Graph& g)
    {
      ...
      STREAMS_BOOST_CHECK(num_edges(g) == edge_set.size());
      e_size_t m = std::distance(p.first, p.second);
      STREAMS_BOOST_CHECK(m == num_edges(g));
      for (; p.first != p.second; ++p.first) {
        edge_t e = *p.first;
        STREAMS_BOOST_CHECK(any_if(edge_set, connects(source(e, g), target(e, g), g)));
        STREAMS_BOOST_CHECK(container_contains(vertex_set, source(e, g)) == true);
        STREAMS_BOOST_CHECK(container_contains(vertex_set, target(e, g)) == true);
      }
    }
       ...
       const Graph& g)
    {
      ...
      for (typename std::vector<std::pair<vertex_t, vertex_t> >
        ...
        STREAMS_BOOST_CHECK(p.second == true);
        STREAMS_BOOST_CHECK(source(p.first, g) == i->first);
        STREAMS_BOOST_CHECK(target(p.first, g) == i->second);
      }
      ...
      for (j = vertex_set.begin(); j != vertex_set.end(); ++j)
        for (k = vertex_set.begin(); k != vertex_set.end(); ++k) {
          p = edge(*j, *k, g);
          if (p.second == true)
            STREAMS_BOOST_CHECK(any_if(edge_set, 
    ...
    void test_add_vertex(Graph& g)
    {
      ...
      STREAMS_BOOST_CHECK(num_vertices(g) == num_vertices(cpy) + 1);

      STREAMS_BOOST_CHECK(out_degree(v, g) == 0);

      // Make sure the rest of the graph stayed the same
      STREAMS_BOOST_CHECK((verify_isomorphism
    ...
    void test_add_edge(vertex_t u, vertex_t v, Graph& g)
    {
      ...
      if (is_undirected(g) && u == v) // self edge
        STREAMS_BOOST_CHECK(added == false);
      else if (parallel_edge_exists)
        STREAMS_BOOST_CHECK(allows_parallel_edges(g) && added == true
                   || !allows_parallel_edges(g) && added == false);
      else
        STREAMS_BOOST_CHECK(added == true);

      if (p.second == true) { // edge added
        STREAMS_BOOST_CHECK(num_edges(g) == num_edges(cpy) + 1);
        
        STREAMS_BOOST_CHECK(container_contains(out_edges(u, g), e) == true);
        ...
        STREAMS_BOOST_CHECK((verify_isomorphism(g, cpy, iso_map)));
      }
    } // test_add_edge()
    ...
    void test_remove_edge(vertex_t u, vertex_t v, Graph& g)
    {
      ...
      STREAMS_BOOST_CHECK(num_edges(g) + occurances == num_edges(cpy));
      STREAMS_BOOST_CHECK((verify_isomorphism
    ...
    void test_remove_edge(edge_t e, Graph& g)
    {
      ...
      STREAMS_BOOST_CHECK(num_edges(g) + 1 == num_edges(cpy));
      STREAMS_BOOST_CHECK(count(adjacent_vertices(u, g), v) + 1 == occurances);
      STREAMS_BOOST_CHECK((verify_isomorphism
    ...
    void test_clear_vertex(vertex_t v, Graph& g)
    {
      ...
      STREAMS_BOOST_CHECK(out_degree(v, g) == 0);
      STREAMS_BOOST_CHECK(num_vertices(g) == num_vertices(cpy));
      STREAMS_BOOST_CHECK((verify_isomorphism
  ...
  for (typename streams_boost::graph_traits<Graph>::vertex_iterator 
           bgl_first_9 = vertices(g).first, bgl_last_9 = vertices(g).second;
       bgl_first_9 != bgl_last_9; bgl_first_9 = bgl_last_9)
    for (typename streams_boost::graph_traits<Graph>::vertex_descriptor v;
      ...
      //BGL_FORALL_VERTICES_T(v, g, Graph) {
        ...
        STREAMS_BOOST_CHECK(pval1 == pval2);
        STREAMS_BOOST_CHECK(pval1 == *i++);
      }
    }
...
#include <streams_boost/graph/iteration_macros_undef.hpp>

#endif // STREAMS_BOOST_GRAPH_TEST_HPP
