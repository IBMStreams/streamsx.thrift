...
//=======================================================================
#ifndef STREAMS_BOOST_GRAPHVIZ_HPP
#define STREAMS_BOOST_GRAPHVIZ_HPP
...
#include <streams_boost/graph/subgraph.hpp>
...
#include <streams_boost/graph/dll_import_export.hpp>
...
namespace streams_boost {
  ...
  STREAMS_BOOST_INSTALL_PROPERTY(edge, attribute);
  STREAMS_BOOST_INSTALL_PROPERTY(vertex, attribute);
  STREAMS_BOOST_INSTALL_PROPERTY(graph, graph_attribute);
  STREAMS_BOOST_INSTALL_PROPERTY(graph, vertex_attribute);
  STREAMS_BOOST_INSTALL_PROPERTY(graph, edge_attribute);
  ...
  write_graphviz
    (std::ostream& out, const Graph& g,
     ...
     VertexID vertex_id
     STREAMS_BOOST_GRAPH_ENABLE_IF_MODELS_PARM(Graph,vertex_list_graph_tag))
  {
    STREAMS_BOOST_CONCEPT_ASSERT((EdgeListGraphConcept<Graph>));
...
class mutate_graph_impl : public mutate_graph
{
 ...
 public:
  ...
  {
    return
      streams_boost::is_convertible<
        typename streams_boost::graph_traits<MutableGraph>::directed_category,
        streams_boost::directed_tag>::value;
  }
  ...
  do_add_edge(const edge_t& edge, const node_t& source, const node_t& target)
  {
    ...
    if(!result.second) {
      // In the case of no parallel edges allowed
        streams_boost::throw_exception(bad_parallel_edge(source, target));
    } else {
...
#ifdef STREAMS_BOOST_GRAPH_USE_SPIRIT_PARSER
#  ifndef STREAMS_BOOST_GRAPH_READ_GRAPHVIZ_ITERATORS
#    define STREAMS_BOOST_GRAPH_READ_GRAPHVIZ_ITERATORS
#  endif
#  include <streams_boost/graph/detail/read_graphviz_spirit.hpp>
#else // New default parser
#  include <streams_boost/graph/detail/read_graphviz_new.hpp>
#endif // STREAMS_BOOST_GRAPH_USE_SPIRIT_PARSER

#ifdef STREAMS_BOOST_GRAPH_USE_MPI
#  include <streams_boost/graph/distributed/graphviz.hpp>
#endif

#endif // STREAMS_BOOST_GRAPHVIZ_HPP
