...
#ifndef STREAMS_BOOST_GRAPH_TRANSITIVE_CLOSURE_HPP
#define STREAMS_BOOST_GRAPH_TRANSITIVE_CLOSURE_HPP
...
#include <streams_boost/graph/vector_as_graph.hpp>
#include <streams_boost/graph/strong_components.hpp>
  ...
  namespace detail
  {
                           ...
                           std::vector < std::size_t > &s3)
    {
      ...
      for (std::size_t k = 0; k < s1.size(); ++k)
        s3[k] = min STREAMS_BOOST_PREVENT_MACRO_SUBSTITUTION(s1[k], s2[k]);
    }
  }                             // namespace detail
                            ...
                            VertexIndexMap index_map)
  {
    ...
    for (size_type i = 0; i < num_vertices(CG); ++i)
      std::sort(CG[i].begin(), CG[i].end(),
                streams_boost::bind(std::less<cg_vertex>(),
                            streams_boost::bind(detail::subscript(topo_number), _1),
                            streams_boost::bind(detail::subscript(topo_number), _2)));
      ...
      for (tie(i, i_end) = vertices(g); i != i_end; ++i)
        {
          ...
          for (streams_boost::tie(ab, ae) = adjacent_vertices(*i, g); ab != ae; ++ab)
            {
              if (*ab == *i)
...
#endif // STREAMS_BOOST_GRAPH_TRANSITIVE_CLOSURE_HPP
