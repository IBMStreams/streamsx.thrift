...
#ifndef STREAMS_BOOST_GRAPH_DIRECTED_GRAPH_HPP
#define STREAMS_BOOST_GRAPH_DIRECTED_GRAPH_HPP
...
public:
        ...
        , m_max_vertex_index(n), m_max_edge_index(0)
    {
        ...
        m_num_edges = m_max_edge_index = streams_boost::num_edges(m_graph);
    }
...
private:
    ...
    vertex_descriptor make_index(vertex_descriptor v) {
        streams_boost::put(vertex_index, m_graph, v, m_max_vertex_index);
        m_num_vertices++;
        m_max_vertex_index++;
        return v;
    }
public:
    ...
    { return make_index(streams_boost::add_vertex(m_graph)); }

    vertex_descriptor add_vertex(vertex_property_type const& p)
    { return make_index(streams_boost::add_vertex(vertex_property(0u, p), m_graph)); }

    void clear_vertex(vertex_descriptor v)
    {
        m_num_edges -= streams_boost::degree(v, m_graph);
        streams_boost::clear_vertex(v, m_graph);
    }

    void remove_vertex(vertex_descriptor v)
    {
        streams_boost::remove_vertex(v, m_graph);
        --m_num_vertices;
    }
...
private:
    ...
    {
        if(x.second) {
            streams_boost::put(edge_index, m_graph, x.first, m_max_edge_index);
            ++m_num_edges;
            ++m_max_edge_index;
        }
        return x;
...
public:
    ...
    { return make_index(streams_boost::add_edge(u, v, m_graph)); }

    std::pair<edge_descriptor, bool>
    add_edge(vertex_descriptor u, vertex_descriptor v, edge_property_type const& p)
    { return make_index(streams_boost::add_edge(u, v, edge_property(0u, p), m_graph)); }
    ...
    void remove_edge(vertex_descriptor u, vertex_descriptor v)
    {
        ...
        for(tie(i, i_end) = streams_boost::out_edges(u, m_graph); i != i_end; ++i) {
            if(streams_boost::target(*i, m_graph) == v) {
                edges.push_back(*i);
            }
        }
    ...
    {
        streams_boost::remove_edge(e, m_graph);
        --m_num_edges;
    }
