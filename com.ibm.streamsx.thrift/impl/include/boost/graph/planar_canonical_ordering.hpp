        ...
        for(ei = ei_start; ei != ei_end; ++ei)
          {
            ...
            edge_t e(*ei); // e = (u,v)
            next_edge_itr = streams_boost::next(ei) == ei_end ? ei_start : streams_boost::next(ei);
            ...
            while (next_vertex == v || next_vertex == u)
              {
                next_edge_itr = streams_boost::next(next_edge_itr) == ei_end ?
                  ei_start : streams_boost::next(next_edge_itr);
